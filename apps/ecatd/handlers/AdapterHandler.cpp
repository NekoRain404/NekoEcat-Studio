...
QJsonObject AdapterHandler::handleSet(const QString& id, const QJsonObject& params) {
    ...
    if (!found) {
        return CommandDispatcher::failure(id, QStringLiteral("Adapter '%1' not found.").arg(adapter));
    }
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        return CommandDispatcher::failure(id, QStringLiteral("Cannot read %1: %2").arg(confPath, file.errorString()));
    }
    ...
    const QRegularExpression re(QStringLiteral("^MASTER0_DEVICE\\s*=\\s*\"[^\"]*\""),
    ...
    if (!file.open(QIODevice::WriteOnly | QIODevice::Text | QIODevice::Truncate)) {
        return CommandDispatcher::failure(id, QStringLiteral("Cannot write %1: %2").arg(confPath, file.errorString()));
    }
...
QString AdapterHandler::currentAdapter() const {
    ...
    static const QRegularExpression re(QStringLiteral("MASTER0_DEVICE\\s*=\\s*\"([^\"]*)\""));
