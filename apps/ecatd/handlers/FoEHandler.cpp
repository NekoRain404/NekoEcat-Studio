...
QJsonObject FoEHandler::handleFoeRead(const QString& id, const QJsonObject& params) {
    ...
    const QStringList args = {"foe_read", "-p", QString::number(position), "--output", filePath};
    ...
    result["message"] =
        QString("Firmware read from slave %1 to '%2' (%3 bytes)").arg(position).arg(filePath).arg(size >= 0 ? size : 0);
...
QJsonObject FoEHandler::handleFoeWrite(const QString& id, const QJsonObject& params) {
    ...
    if (!fileInfo.exists()) {
        return CommandDispatcher::failure(id, QString("Input file '%1' does not exist.").arg(filePath));
    }
    if (!fileInfo.isReadable()) {
        return CommandDispatcher::failure(id, QString("Input file '%1' is not readable.").arg(filePath));
    }

    // Build the ethercat foe_write command.
    QStringList args = {"foe_write", "-p", QString::number(position), "--input", filePath};
    ...
    result["message"] =
        QString("Firmware '%1' written to slave %2 (%3 bytes)").arg(filePath).arg(position).arg(fileSize);
    return CommandDispatcher::success(id, result);
}

// Run an ethercat CLI command synchronously via QProcess.
// Returns the process exit code; captures stdout and stderr.
int FoEHandler::runEthercatCommand(const QStringList& args, QString* output, QString* errorOutput,
                                   int timeoutMs) const {
    QProcess proc;
...
bool FoEHandler::validateFilePath(const QString& path, QString* error) const {
    ...
    if (!info.isAbsolute()) {
        if (error)
            *error = QString("File path '%1' must be absolute.").arg(path);
        return false;
    }
    // Reject path traversal sequences
    if (path.contains("/../") || path.endsWith("/..") || path == QLatin1String("..") || path.contains("/..")) {
        if (error)
            *error = QString("File path '%1' must not contain '..' components.").arg(path);
        return false;
    }
    if (!info.dir().exists()) {
        if (error)
            *error = QString("Directory '%1' does not exist.").arg(info.path());
        return false;
    }
    ...
    if (resolved.isEmpty()) {
        ...
        if (parent.isEmpty()) {
            if (error)
                *error = QString("Cannot resolve '%1'.").arg(info.path());
            return false;
        }
        resolved = parent;
    }
    ...
    for (const QString& base : bases) {
        ...
        if (resolved == baseCanonical || baseCanonical == QLatin1String("/") // root base allows all
    ...
    if (error) {
        *error =
            QString("File path '%1' resolves to '%2', outside the allowed firmware directory.").arg(path, resolved);
    }
    return false;
}
...
bool FoEHandler::rejectUnsafeExistingFile(const QString& path, QString* error) const {
    ...
    if (::lstat(encoded.constData(), &st) != 0) {
        ...
        if (error)
            *error = QString("Cannot stat '%1'.").arg(path);
        return false;
    }
    if (S_ISLNK(st.st_mode)) {
        if (error)
            *error = QString("File '%1' must not be a symbolic link.").arg(path);
        return false;
    }
    if (!S_ISREG(st.st_mode)) {
        if (error)
            *error = QString("File '%1' must be a regular file.").arg(path);
        return false;
    }
    if (st.st_nlink > 1) {
        if (error)
            *error = QString("File '%1' must not be a hard link (link count %2).").arg(path).arg(st.st_nlink);
        return false;
    }
    return true;
}
