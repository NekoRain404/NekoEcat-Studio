...
private:
    ...
    bool readEoeSdo(const QString &master, int position, const QString &index,
                    const QString &subIndex, uint32_t *value, QString *error) const;

    // Write an EoE SDO (uint32) to a slave.
    bool writeEoeSdo(const QString &master, int position, const QString &index,
                     const QString &subIndex, uint32_t value, QString *error) const;
