...
SettingsDialog::SettingsDialog(const AppSettings& settings, QWidget* parent) : QDialog(parent) {
    ...
    tabWidget_->addTab(buildAppearanceTab(settings, zh), zh ? QStringLiteral("外观") : QStringLiteral("Appearance"));
    tabWidget_->addTab(buildEthercatTab(settings, zh), zh ? QStringLiteral("EtherCAT") : QStringLiteral("EtherCAT"));
    tabWidget_->addTab(buildTimingTab(settings, zh), zh ? QStringLiteral("定时") : QStringLiteral("Timing"));
    tabWidget_->addTab(buildFreeRunTab(settings, zh), zh ? QStringLiteral("Free Run") : QStringLiteral("Free Run"));
    tabWidget_->addTab(buildDisplayTab(settings, zh), zh ? QStringLiteral("显示") : QStringLiteral("Display"));
    tabWidget_->addTab(buildNotificationTab(settings, zh),
                       zh ? QStringLiteral("通知") : QStringLiteral("Notifications"));
    tabWidget_->addTab(buildExportTab(settings, zh), zh ? QStringLiteral("导出") : QStringLiteral("Export"));
    tabWidget_->addTab(buildShortcutsTab(settings, zh), zh ? QStringLiteral("快捷键") : QStringLiteral("Shortcuts"));
...
QWidget* SettingsDialog::buildAppearanceTab(const AppSettings& s, bool zh) {
    ...
    themeCombo_->addItems({QStringLiteral("Dark"), QStringLiteral("Light"), QStringLiteral("Nord"),
                           QStringLiteral("Catppuccin"), QStringLiteral("Dracula"), QStringLiteral("Solarized"),
                           QStringLiteral("Gruvbox"), QStringLiteral("Tokyo Night"), QStringLiteral("One Dark"),
                           QStringLiteral("Monokai"), QStringLiteral("Cyberpunk")});
    ...
    connect(themeCombo_, &QComboBox::currentTextChanged, this, &SettingsDialog::themePreviewRequested);
...
QWidget* SettingsDialog::buildEthercatTab(const AppSettings& s, bool zh) {
    ...
    masterTable_->setHorizontalHeaderLabels(
    ...
    if (!s.networkAdapter.isEmpty()) {
        ...
        if (idx >= 0)
            adapterCombo_->setCurrentIndex(idx);
    }

    connect(refreshAdapterBtn, &QPushButton::clicked, this, [this]() { emit adaptersRefreshRequested(); });
    ...
    backendModeCombo_->addItem(zh ? QStringLiteral("自动 (推荐)") : QStringLiteral("Auto (Recommended)"),
                               QStringLiteral("auto"));
    backendModeCombo_->addItem(zh ? QStringLiteral("IgH 原生 API") : QStringLiteral("IgH Native API"),
                               QStringLiteral("native"));
    backendModeCombo_->addItem(zh ? QStringLiteral("IgH 命令行") : QStringLiteral("IgH CLI"), QStringLiteral("cli"));
    backendModeCombo_->setCurrentIndex(s.backendMode == QStringLiteral("native") ? 1
                                       : s.backendMode == QStringLiteral("cli")  ? 2
                                                                                 : 0);
...
QWidget* SettingsDialog::buildTimingTab(const AppSettings& s, bool zh) {
    ...
    // Watch auto-refresh
    watchRefreshCombo_ =
        makeRefreshCombo(s.watchAutoRefreshMs, {{0, zh ? QStringLiteral("关闭") : QStringLiteral("Off")},
                                                {250, QStringLiteral("250 ms")},
                                                {500, QStringLiteral("500 ms")},
                                                {1000, QStringLiteral("1 s")},
                                                {2000, QStringLiteral("2 s")}});
    addFormRow(form, zh ? QStringLiteral("Watch 自动刷新") : QStringLiteral("Watch Auto-Refresh"), watchRefreshCombo_);

    // Overview auto-refresh
    overviewRefreshCombo_ =
        makeRefreshCombo(s.overviewAutoRefreshMs, {{0, zh ? QStringLiteral("关闭") : QStringLiteral("Off")},
                                                   {1000, QStringLiteral("1 s")},
                                                   {2000, QStringLiteral("2 s")},
                                                   {5000, QStringLiteral("5 s")}});
    addFormRow(form, zh ? QStringLiteral("总览自动刷新") : QStringLiteral("Overview Auto-Refresh"),
               overviewRefreshCombo_);
    ...
    // Topology poll
    topologyPollCombo_ =
        makeRefreshCombo(s.topologyPollIntervalMs, {{0, zh ? QStringLiteral("关闭") : QStringLiteral("Off")},
                                                    {5000, QStringLiteral("5 s")},
                                                    {10000, QStringLiteral("10 s")},
                                                    {30000, QStringLiteral("30 s")}});
    addFormRow(form, zh ? QStringLiteral("拓扑轮询间隔") : QStringLiteral("Topology Poll Interval"),
               topologyPollCombo_);
...
QWidget* SettingsDialog::buildFreeRunTab(const AppSettings& s, bool zh) {
    ...
    freeRunAutoNameCheck_ =
        new QCheckBox(zh ? QStringLiteral("从对象字典自动命名") : QStringLiteral("Auto-name from Object Dictionary"));
    freeRunAutoNameCheck_->setChecked(s.freeRunAutoName);
    form->addRow(QStringLiteral(""), freeRunAutoNameCheck_);

    freeRunHighlightCheck_ =
        new QCheckBox(zh ? QStringLiteral("高亮变化值") : QStringLiteral("Highlight changed values"));
...
QWidget* SettingsDialog::buildNotificationTab(const AppSettings& s, bool zh) {
    ...
    notifyStateCheck_ =
        new QCheckBox(zh ? QStringLiteral("从站状态变化时通知") : QStringLiteral("Notify on slave state change"));
    ...
    notifyDriftCheck_ =
        new QCheckBox(zh ? QStringLiteral("Watch 值偏移时通知") : QStringLiteral("Notify on watch value drift"));
...
QWidget* SettingsDialog::buildExportTab(const AppSettings& s, bool zh) {
    ...
    exportDirEdit_->setPlaceholderText(zh ? QStringLiteral("默认使用项目目录")
                                          : QStringLiteral("Defaults to project directory"));
    ...
    connect(browseBtn, &QPushButton::clicked, this, [this, zh] {
        QString dir = QFileDialog::getExistingDirectory(
            this, zh ? QStringLiteral("选择导出目录") : QStringLiteral("Select Export Directory"),
            exportDirEdit_->text());
        if (!dir.isEmpty())
            exportDirEdit_->setText(dir);
    });
    ...
    connect(esiBrowseBtn, &QPushButton::clicked, this, [this, zh] {
        QString dir = QFileDialog::getExistingDirectory(
            this, zh ? QStringLiteral("选择 ESI 仓库路径") : QStringLiteral("Select ESI Repository Path"),
            esiPathEdit_->text());
        if (!dir.isEmpty())
            esiPathEdit_->setText(dir);
    });
    ...
    exportTimestampCheck_ =
        new QCheckBox(zh ? QStringLiteral("导出文件包含时间戳") : QStringLiteral("Include timestamp in exports"));
    exportTimestampCheck_->setChecked(s.exportIncludeTimestamp);
    form->addRow(QStringLiteral(""), exportTimestampCheck_);

    exportMetadataCheck_ =
        new QCheckBox(zh ? QStringLiteral("导出文件包含元数据") : QStringLiteral("Include metadata in exports"));
    ...
    csvDelimiterCombo_->setCurrentIndex(s.csvDelimiter == QStringLiteral(";")    ? 1
                                        : s.csvDelimiter == QStringLiteral("\t") ? 2
                                                                                 : 0);
...
protected:
    void keyPressEvent(QKeyEvent* e) override {
        ...
        const int mods =
            e->modifiers().toInt() & (static_cast<int>(Qt::ControlModifier) | static_cast<int>(Qt::ShiftModifier) |
                                      static_cast<int>(Qt::AltModifier) | static_cast<int>(Qt::MetaModifier));
...
AppSettings SettingsDialog::settings() const {
    ...
    for (int row = 0; row < masterTable_->rowCount(); ++row) {
        ...
        if (p.name.isEmpty() && p.target.isEmpty())
            continue;
        if (p.target.isEmpty())
            p.target = QString::number(r.masters.size());
        if (p.name.isEmpty())
            p.name = QString(QStringLiteral("Master %1")).arg(p.target);
        r.masters.append(p);
    }
    if (r.masters.isEmpty())
        r.masters.append(MasterProfile{});
...
QWidget* SettingsDialog::buildShortcutsTab(const AppSettings& s, bool zh) {
    ...
    auto* hint = new QLabel(zh ? QStringLiteral("点击快捷键列，按下新的按键组合。Esc 清除，Backspace 恢复默认。")
                               : QStringLiteral("Click the shortcut column and press a new key combination. Esc "
                                                "clears, Backspace resets to default."));
    ...
    for (int i = 0; i < defs.size(); ++i) {
        ...
        auto* defaultItem = new QTableWidgetItem(QKeySequence(def.defaultKey).toString(QKeySequence::NativeText));
