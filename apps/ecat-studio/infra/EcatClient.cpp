...
EcatClient::EcatClient(QObject* parent) : QObject(parent) {
    ...
    connect(&socket_, &QTcpSocket::connected, this, [this] {
        ...
        if (requestSweepTimer_)
            requestSweepTimer_->start(2000);
        emit connected();
        emit reconnected();
    ...
    connect(&socket_, &QTcpSocket::errorOccurred, this, [this](QAbstractSocket::SocketError error) {
        ...
        const bool connectAttemptFailed =
            (connectionState_ == ConnectionState::Connecting || connectionState_ == ConnectionState::Reconnecting);
...
void EcatClient::connectToDaemon() {
    if (connectionState_ == ConnectionState::Connected || connectionState_ == ConnectionState::Connecting) {
        return;
    }
...
void EcatClient::setupConnectTimeout() {
    if (!connectTimeoutTimer_) {
        ...
        connect(connectTimeoutTimer_, &QTimer::timeout, this, [this]() {
            if (connectionState_ == ConnectionState::Connecting || connectionState_ == ConnectionState::Reconnecting) {
                socket_.abort();
                setConnectionState(ConnectionState::Disconnected);
                ++connectFailures_;
...
QString EcatClient::masterTarget() const {
    return masterTarget_;
}
...
void EcatClient::ping() {
    send("ping", {}, [this](const QJsonObject& result) {
        emit daemonInfo(QString("%1 %2").arg(result.value("name").toString(), result.value("version").toString()));
    });
}

// Request pre-flight host diagnostics from the daemon.
void EcatClient::hostDiagnostics() {
    send("hostDiagnostics", {},
         [this](const QJsonObject& result) { emit hostDiagnosticsReady(result.value("checks").toArray()); });
}

// Get raw `ethercat master` text for display.
void EcatClient::master() {
    send("master", {}, [this](const QJsonObject& result) { emit masterText(result.value("text").toString()); });
}

// Enumerate all slaves on the bus and emit the deserialized list.
void EcatClient::scan() {
    send("scan", {},
         [this](const QJsonObject& result) { emit slavesChanged(slavesFromJson(result.value("slaves").toArray())); });
}
...
void EcatClient::slaveInfo(int position) {
    send("slaveInfo", {{"position", position}}, [this, position](const QJsonObject& result) {
        emit slaveTextResult("Info", position, result.value("text").toString());
    });
}

// Fetch PDO dictionary for a single slave.
void EcatClient::pdos(int position) {
    send("pdos", {{"position", position}}, [this, position](const QJsonObject& result) {
...
void EcatClient::sdos(int position) {
    send("sdos", {{"position", position}}, [this, position](const QJsonObject& result) {
...
void EcatClient::xml(int position) {
    send("xml", {{"position", position}}, [this, position](const QJsonObject& result) {
        emit slaveTextResult("ESI XML", position, result.value("text").toString());
    });
}

// SDO upload (read) — emits the retrieved value via sdoValue signal.
void EcatClient::upload(int position, const QString& index, const QString& subIndex) {
    send("upload", {{"position", position}, {"index", index}, {"subIndex", subIndex}},
         [this, position, index, subIndex](const QJsonObject& result) {
             ...
             if (result.contains("error"))
                 return;
             emit sdoValue(position, index, subIndex, result.value("value").toString());
         });
}

// SDO download (write) — auto-reads back via upload() to verify the write took effect.
void EcatClient::download(int position, const QString& index, const QString& subIndex, const QString& value,
                          const QString& type) {
    send("download",
         {{"position", position}, {"index", index}, {"subIndex", subIndex}, {"value", value}, {"type", type}},
         [this, position, index, subIndex](const QJsonObject& result) {
             // Don't emit success or auto-read on error.
             if (result.contains("error"))
                 return;
             emit commandSucceeded(QString("SDO download complete #%1: %2:%3").arg(position).arg(index, subIndex));
...
void EcatClient::applyStartupSdos(const QJsonArray& items) {
    send("applyStartupSdos", {{"items", items}}, [this](const QJsonObject& result) {
        ...
        emit commandSucceeded(QString("Startup SDO apply complete: %1 applied, %2 failed").arg(applied).arg(failed));
        if (failed > 0) {
            ...
            for (const auto& failure : failures) {
                ...
                messages << QString("#%1 %2:%3 %4")
                                ...
                                .arg(object.value("index").toString(), object.value("subIndex").toString(),
...
void EcatClient::setState(int position, const QString& state) {
    send("setState", {{"position", position}, {"state", state}}, [this, state](const QJsonObject& result) {
        if (result.contains("error"))
            return;
...
void EcatClient::freeRunStatus() {
    send("freeRunStatus", {}, [this](const QJsonObject& result) {
        emit freeRunChanged(result.value("running").toBool(), result.value("status").toString());
        emit freeRunTelemetry(result);
    });
}
...
void EcatClient::rtTestStart(int cycleUsec) {
    send("rtTestStart", {{"cycleUsec", cycleUsec}}, [this](const QJsonObject& result) {
...
void EcatClient::rtTestStatus() {
    send("rtTestStatus", {}, [this](const QJsonObject& result) { emit rtTestTelemetry(result); });
}

// Request DC sync diagnostics from the daemon.
void EcatClient::dcSyncStatus() {
    send("dcSyncStatus", {}, [this](const QJsonObject& result) { emit dcSyncStatusResult(result); });
}

// Query DC configuration from a slave's ESI XML descriptor.
void EcatClient::dcConfigure(int position) {
    send("dcConfigure", {{"position", position}}, [this](const QJsonObject& result) {
...
void EcatClient::dcActivate(int refClockSlave) {
    send("dcActivate", {{"refClockSlave", refClockSlave}}, [this](const QJsonObject& result) {
...
void EcatClient::send(const QString& method, const QJsonObject& params, Handler handler) {
    ...
    socket_.write(JsonProtocol::encode(JsonProtocol::request(id, method, scopedParams)));
}
...
void EcatClient::sweepTimedOutRequests() {
    ...
    for (const QString& id : timedOut) {
        ...
        const auto handler = handlers_.take(id);
        requestTimestamps_.remove(id);
        const QString errorMsg = QString("Request %1 timed out after %2ms").arg(id).arg(requestTimeoutMs_);
...
void EcatClient::alEventLog(int limit) {
    send("alEventLog", {{"limit", limit}}, [this](const QJsonObject& result) { emit alEventLogResult(result); });
}

// Clear the daemon's AL event history.
void EcatClient::alEventClear() {
    send("alEventClear", {}, [this](const QJsonObject&) { emit commandSucceeded("AL event log cleared"); });
}

// Enumerate network adapters available on the daemon host.
void EcatClient::listAdapters() {
    send("listAdapters", {}, [this](const QJsonObject& result) { emit adaptersListResult(result); });
}

// Switch the IgH master's network adapter (requires daemon restart).
void EcatClient::setAdapter(const QString& name) {
    send("setAdapter", {{"adapter", name}},
         [this, name](const QJsonObject&) { emit commandSucceeded(QString("Adapter set to %1").arg(name)); });
}
...
void EcatClient::foeRead(int position, const QString& filePath) {
    send("foeRead", {{"position", position}, {"filePath", filePath}},
...
void EcatClient::foeWrite(int position, const QString& filePath, quint32 password) {
    ...
    send("foeWrite", params, [this, position](const QJsonObject& result) {
...
void EcatClient::soeRead(int position, const QString& idn, int drive, const QString& type) {
    ...
    if (!type.isEmpty())
        params["type"] = type;
    send("soeRead", params, [this, position, idn](const QJsonObject& result) {
        emit soeReadResult(position, idn, result.value("value").toString());
    });
}

// Write an SoE IDN to a slave.
void EcatClient::soeWrite(int position, const QString& idn, const QString& value, int drive, const QString& type) {
    QJsonObject params;
    params["position"] = position;
    params["idn"] = idn;
    params["value"] = value;
    ...
    if (!type.isEmpty())
        params["type"] = type;
    send("soeWrite", params, [this, position, idn](const QJsonObject& result) {
...
void EcatClient::eoeStatus(int position) {
    send("eoeStatus", {{"position", position}},
         [this, position](const QJsonObject& result) { emit eoeStatusResult(position, result); });
}

// Configure IP address for an EoE-capable slave.
void EcatClient::eoeConfigureIp(int position, const QString& ip, const QString& subnet, const QString& gateway,
                                const QString& dns) {
    QJsonObject params;
    params["position"] = position;
    params["ip"] = ip;
    ...
    if (!dns.isEmpty())
        params["dns"] = dns;
    send("eoeConfigureIp", params, [this, position, ip](const QJsonObject& result) {
...
void EcatClient::eoeGetIp(int position) {
    send("eoeGetIp", {{"position", position}},
         [this, position](const QJsonObject& result) { emit eoeIpResult(position, result); });
}

// Get EoE statistics for a slave.
void EcatClient::eoeStats(int position) {
    send("eoeStats", {{"position", position}},
         [this, position](const QJsonObject& result) { emit eoeStatsResult(position, result); });
}
...
void EcatClient::redundancyStatus() {
    send("redundancyStatus", {}, [this](const QJsonObject& result) { emit redundancyStatusResult(result); });
}

// Enable cable redundancy.
void EcatClient::redundancyEnable() {
    send("redundancyEnable", {}, [this](const QJsonObject& result) {
...
void EcatClient::redundancyDisable() {
    send("redundancyDisable", {}, [this](const QJsonObject& result) {
        bool success = result.value("success").toBool();
...
void EcatClient::redundancyFailover() {
    send("redundancyFailover", {}, [this](const QJsonObject& result) {
        bool success = result.value("success").toBool();
...
void EcatClient::redundancyFailback() {
    send("redundancyFailback", {}, [this](const QJsonObject& result) {
        bool success = result.value("success").toBool();
...
void EcatClient::redundancyHistory(int limit) {
    send("redundancyHistory", {{"limit", limit}},
         [this](const QJsonObject& result) { emit redundancyHistoryResult(result); });
}
...
void EcatClient::onlineChangePreview(const QJsonArray& changes) {
    send("onlineChangePreview", {{"changes", changes}},
         [this](const QJsonObject& result) { emit onlineChangePreviewResult(result); });
}
...
void EcatClient::onlineChangeApply(const QJsonArray& changes, const QString& targetState) {
    send("onlineChangeApply", {{"changes", changes}, {"targetState", targetState}},
         [this](const QJsonObject& result) { emit onlineChangeApplyResult(result); });
}

// Query online change status.
void EcatClient::onlineChangeStatus() {
    send("onlineChangeStatus", {}, [this](const QJsonObject& result) { emit onlineChangeStatusResult(result); });
}
    ...
    if (!heartbeatTimer_) {
        ...
        connect(heartbeatTimer_, &QTimer::timeout, this, [this]() {
            if (connectionState_ != ConnectionState::Connected)
                return;
            if (pendingPing_)
                return; // Don't send another ping while one is pending.
            pendingPing_ = true;
            send("ping", {}, [this](const QJsonObject& result) {
                Q_UNUSED(result);
                pendingPing_ = false;
...
void EcatClient::attemptReconnect() {
    ...
    QTimer::singleShot(5000, this, [this]() {
        if (connectionState_ != ConnectionState::Connected && autoReconnectEnabled_ &&
            connectFailures_ < kMaxConnectAttempts) {
