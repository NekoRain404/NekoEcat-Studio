...
PdoMapTableRow pdoMapTableRowFromTable(QTableWidget* table, int row) {
    ...
    result.subIndex = normalizeHexText(tableText(table, row, kPdoMapSubIndexColumn), 2);
...
FreeRunEntryTableRow freeRunEntryTableRowFromTable(QTableWidget* table, int row) {
    ...
    result.position = tableText(table, row, kFreeRunEntryPositionColumn).toInt(&result.positionValid);
    ...
    result.index = normalizeHexText(tableText(table, row, kFreeRunEntryIndexColumn), 4);
    result.subIndex = normalizeHexText(tableText(table, row, kFreeRunEntrySubIndexColumn), 2);
...
IoVariableTableRow ioVariableTableRowFromTable(QTableWidget* table, int row) {
    ...
    result.position = tableText(table, row, kIoVariablePositionColumn).toInt(&result.positionValid);
    result.direction = tableText(table, row, kIoVariableDirectionColumn);
    result.symbol = tableText(table, row, kIoVariableSymbolColumn);
    result.index = normalizeHexText(tableText(table, row, kIoVariableIndexColumn), 4);
    result.subIndex = normalizeHexText(tableText(table, row, kIoVariableSubIndexColumn), 2);
...
bool ioVariableTableRowHasValue(QTableWidget* table, int row) {
    if (!table || row < 0 || row >= table->rowCount() || table->isRowHidden(row)) {
        return false;
    }
...
QVector<int> selectedIoVariableTableRows(QTableWidget* table, bool visibleOnly) {
