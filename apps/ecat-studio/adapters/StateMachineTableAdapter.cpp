...
int stateMachinePositionFromTable(QTableWidget* table, int row) {
    int position = -1;
    if (stateMachineTableRowPosition(stateMachineTableRowFromTable(table, row), &position)) {
        return position;
    }
    return -1;
}
...
bool stateMachineRowHasRecommendation(QTableWidget* table, int row) {
    return stateMachineTableRowHasRecommendation(stateMachineTableRowFromTable(table, row));
}
