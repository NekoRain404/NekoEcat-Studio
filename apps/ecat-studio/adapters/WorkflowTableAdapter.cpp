...
bool isNoneText(const QString& text, const QString& noneText) {
    ...
    return normalized.isEmpty() || normalized == QStringLiteral("none") || normalized == noneText.trimmed().toLower();
}
...
bool hasEvidenceGap(const QString& riskText, const QString& evidence) {
    ...
    return riskLower.contains(QStringLiteral("missing")) || riskLower.contains(QStringLiteral("缺失")) ||
           riskLower.contains(QStringLiteral("offline")) || riskLower.contains(QStringLiteral("离线")) ||
           evidenceLower.contains(QStringLiteral("not loaded")) || evidenceLower.contains(QStringLiteral("not run")) ||
           evidenceLower.contains(QStringLiteral("missing")) || evidenceLower.contains(QStringLiteral("no active")) ||
           evidenceLower.contains(QStringLiteral("stopped")) || evidenceLower.contains(QStringLiteral("尚未")) ||
           evidenceLower.contains(QStringLiteral("缺")) || evidenceLower.contains(QStringLiteral("未运行")) ||
...
CommissioningWorkflowRowState commissioningWorkflowRowState(QTableWidget* table, int row, const QString& noneText) {
    ...
    const QString riskText = tableText(table, row, kCommissioningWorkflowRiskColumn).trimmed();
    const QString evidence = tableText(table, row, kCommissioningWorkflowEvidenceColumn).trimmed();
...
CommissioningWorkflowFilterStats filterCommissioningWorkflowTable(QTableWidget* table, const QString& scope,
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        const CommissioningWorkflowRowState state = commissioningWorkflowRowState(table, row, noneText);
        if (state.isReady) {
            ++stats.ready;
        } else {
            ++stats.open;
        ...
        const bool show =
            commissioningWorkflowScopeMatches(state, scope) && commissioningWorkflowSearchMatches(table, row, needle);
...
int firstCommissioningWorkflowIssueRow(QTableWidget* table) {
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        if (table->isRowHidden(row) || !commissioningWorkflowRowState(table, row, QStringLiteral("None")).reviewIssue) {
            continue;
        }
        return row;
    }
