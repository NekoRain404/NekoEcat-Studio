...
void applyWatchEvidence(SlaveEvidenceInput* input, QTableWidget* table) {
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        if (parsedPosition(tableText(table, row, kSlaveEvidenceWatchPositionColumn)) != input->position) {
            continue;
        }

        ++input->watchRows;
        const QString value = tableText(table, row, kSlaveEvidenceWatchValueColumn);
        const QString decoded = tableText(table, row, kSlaveEvidenceWatchDecodedColumn);
        if (!value.isEmpty()) {
            ++input->watchValueRows;
        }

        const QString index = normalizeHexText(tableText(table, row, kSlaveEvidenceWatchIndexColumn), 4);
...
void applyStartupEvidence(SlaveEvidenceInput* input, QTableWidget* table) {
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        if (parsedPosition(tableText(table, row, kSlaveEvidenceStartupPositionColumn)) != input->position) {
            continue;
        }
        ++input->startupRows;
        if (hasStartupDiffEvidence(tableText(table, row, kSlaveEvidenceStartupDeltaColumn))) {
            ++input->startupDiffs;
        }
    }
}
...
void applyProcessEvidence(SlaveEvidenceInput* input, QTableWidget* table) {
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        if (parsedPosition(tableText(table, row, kSlaveEvidenceProcessPositionColumn)) != input->position) {
            continue;
        }
        ++input->processRows;
        if (hasPdoMapIssueEvidence(tableText(table, row, kSlaveEvidenceProcessMapColumn))) {
            ++input->mapIssues;
        }
    }
}
...
SlaveEvidenceMatrixRowState slaveEvidenceMatrixRowState(QTableWidget* table, int row) {
    ...
    const QString priority = tableText(table, row, kSlaveEvidenceMatrixPriorityColumn).toLower();
    const QString readiness = tableText(table, row, kSlaveEvidenceMatrixReadinessColumn).toLower();
    const QString od = tableText(table, row, kSlaveEvidenceMatrixOdColumn).toLower();
    const QString pdo = tableText(table, row, kSlaveEvidenceMatrixPdoColumn).toLower();
    const QString watch = tableText(table, row, kSlaveEvidenceMatrixWatchColumn).toLower();
    const QString startup = tableText(table, row, kSlaveEvidenceMatrixStartupColumn).toLower();
    const QString process = tableText(table, row, kSlaveEvidenceMatrixProcessColumn).toLower();
    const QString risk = tableText(table, row, kSlaveEvidenceMatrixRiskColumn).trimmed().toLower();
    const QString next = tableText(table, row, kSlaveEvidenceMatrixNextColumn).trimmed().toLower();
    ...
    state.startupDiff =
        startup.contains("diff") || startup.contains("偏差") || risk.contains("startup") || risk.contains("启动");
    state.processMissing = isMissingText(process) || risk.contains("process evidence") || risk.contains("过程证据");
    return state;
}
...
SlaveEvidenceMatrixFilterStats filterSlaveEvidenceMatrixTable(QTableWidget* table, const QString& scope,
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        const SlaveEvidenceMatrixRowState state = slaveEvidenceMatrixRowState(table, row);
        if (state.priorityP0) {
            ++stats.p0;
        ...
        const bool show =
            slaveEvidenceMatrixScopeMatches(state, scope) && slaveEvidenceMatrixSearchMatches(table, row, needle);
...
void countSlaveEvidenceMatrixPriorities(QTableWidget* table, int* p0, int* p1, int* p2, int* p3) {
    ...
    const SlaveEvidenceMatrixPriorityCounts counts = slaveEvidenceMatrixPriorityCounts(table);
    if (p0) {
        *p0 = counts.p0;
    }
    if (p1) {
...
int firstSlaveEvidenceRowForPosition(QTableWidget* table, int position, int positionColumn) {
    if (!table || position < 0 || positionColumn < 0 || positionColumn >= table->columnCount()) {
        return -1;
    }
...
int firstSlaveEvidenceStartupDiffRow(QTableWidget* startupTable, int position) {
    ...
    for (int row = 0; row < startupTable->rowCount(); ++row) {
        if (parsedPosition(tableText(startupTable, row, kSlaveEvidenceStartupPositionColumn)) != position) {
            continue;
        }
        if (hasStartupDiffEvidence(tableText(startupTable, row, kSlaveEvidenceStartupDeltaColumn))) {
            return row;
        }
    }
    return -1;
...
int firstSlaveEvidenceProcessIssueRow(QTableWidget* processTable, int position) {
    ...
    for (int row = 0; row < processTable->rowCount(); ++row) {
        if (parsedPosition(tableText(processTable, row, kSlaveEvidenceProcessPositionColumn)) != position) {
            continue;
        }
        if (hasPdoMapIssueEvidence(tableText(processTable, row, kSlaveEvidenceProcessMapColumn))) {
            return row;
        }
    }
    return -1;
...
int firstSlaveEvidenceDriveWatchRow(QTableWidget* watchTable, int position) {
    ...
    for (int row = 0; row < watchTable->rowCount(); ++row) {
        if (parsedPosition(tableText(watchTable, row, kSlaveEvidenceWatchPositionColumn)) != position) {
            continue;
        }
        const QString index = normalizeHexText(tableText(watchTable, row, kSlaveEvidenceWatchIndexColumn), 4);
        if (index == "0x6041" || index == "0x603f" || index == "0x6061" || index == "0x6040" || index == "0x6060") {
            return row;
        }
    }
    return -1;
