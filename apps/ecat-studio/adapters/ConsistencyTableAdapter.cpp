...
ConsistencyIssueCounts consistencyTableIssueCounts(QTableWidget* table) {
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        addConsistencyIssueLevel(&counts,
                                 consistencyIssueLevelFromText(tableText(table, row, kConsistencyLevelColumn)));
    }
    return counts;
}
...
bool consistencyTableScopeMatches(const ConsistencyTableRowState& state, const QString& scope) {
    if (scope == QString::fromLatin1(kConsistencyScopeError)) {
        return consistencyIssueLevelFromText(state.level) == ConsistencyIssueLevel::Error;
    }
    if (scope == QString::fromLatin1(kConsistencyScopeWarning)) {
        return consistencyIssueLevelFromText(state.level) == ConsistencyIssueLevel::Warning;
    }
    ...
    if (scope == QString::fromLatin1(kConsistencyScopeReady)) {
        return consistencyIssueLevelFromText(state.level) == ConsistencyIssueLevel::Ready;
    }
    return true;
}
...
ConsistencyTableFilterStats filterConsistencyTableRows(QTableWidget* table, const QString& scope,
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        const bool show = consistencyTableScopeMatches(consistencyTableRowState(table, row), scope) &&
...
int firstConsistencyTableIoIssueRow(QTableWidget* table) {
    ...
    for (int row = 0; row < table->rowCount(); ++row) {
        ...
        if (consistencyTableScopeMatches(state, QString::fromLatin1(kConsistencyScopeIo)) &&
            consistencyIssueLevelFromText(state.level) != ConsistencyIssueLevel::Ready) {
            return row;
        }
    }
    return -1;
