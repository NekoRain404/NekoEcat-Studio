...
sdoEvidenceTableRowsForTarget(const SdoEvidenceTables &tables,
  ...
  if (target.position < 0 || target.index.trimmed().isEmpty() ||
      target.subIndex.trimmed().isEmpty()) {
    return rows;
  }

  if (target.dictionaryLoadedForPosition) {
    rows.dictionaryRow = tableRowForObjectIndex(
        tables.dictionaryTable, target.index, target.subIndex, 1, 2);
  }
  rows.watchRow =
      tableRowForObjectAddress(tables.watchTable, target.position, target.index,
                               target.subIndex, 1, 2, 3);
  rows.startupRow =
      tableRowForObjectAddress(tables.startupTable, target.position,
                               target.index, target.subIndex, 0, 1, 2);
  rows.bookmarkRow =
      tableRowForObjectAddress(tables.bookmarkTable, target.position,
                               target.index, target.subIndex, 0, 2, 3);
  rows.targetTrailRow =
      tableRowForObjectAddress(tables.targetTrailTable, target.position,
                               target.index, target.subIndex, 1, 2, 3);
  return rows;
}
...
sdoEvidenceCandidatesFromTables(const QString &readValue,
  ...
  if (rows.watchRow >= 0) {
    appendCandidate(&candidates, labels.watch,
                    tableText(tables.watchTable, rows.watchRow, 4));
  }

  if (rows.dictionaryRow >= 0) {
    appendCandidate(&candidates, labels.dictionary,
                    tableText(tables.dictionaryTable, rows.dictionaryRow, 7));
  }

  if (rows.startupRow >= 0) {
    appendCandidate(&candidates, labels.startup,
                    tableText(tables.startupTable, rows.startupRow, 3));
  }

  if (rows.bookmarkRow >= 0) {
    appendCandidate(&candidates, labels.bookmark,
                    tableText(tables.bookmarkTable, rows.bookmarkRow, 8));
  }

  if (rows.targetTrailRow >= 0) {
    const QString trailWrite =
        tableText(tables.targetTrailTable, rows.targetTrailRow, 7);
    appendCandidate(&candidates, labels.targetTrailWrite, trailWrite);
    if (trailWrite.trimmed().isEmpty()) {
      appendCandidate(
          &candidates, labels.targetTrail,
...
QVector<SdoEvidenceItem> sdoLocalEvidenceItemsFromTables(
  ...
  for (int row = 0; tables.watchTable && row < tables.watchTable->rowCount();
    ...
    if (tableObjectAddressMatches(tables.watchTable, row, position, index,
                                  subIndex, 1, 2, 3)) {
      appendEvidenceItem(&items,
                         QString("%1 #%2").arg(labels.watchPrefix).arg(row + 1),
  ...
  if (dictionaryTableLoaded) {
    for (int row = 0;
         ...
         ++row) {
      if (!tableObjectIndexMatches(tables.dictionaryTable, row, index, subIndex,
                                   1, 2)) {
        continue;
      }
  ...
  for (int row = 0;
    ...
    if (tableObjectAddressMatches(tables.startupTable, row, position, index,
                                  subIndex, 0, 1, 2)) {
      appendEvidenceItem(
          &items, QString("%1 #%2").arg(labels.startupPrefix).arg(row + 1),
  ...
  for (int row = 0;
    ...
    if (tableObjectAddressMatches(tables.bookmarkTable, row, position, index,
                                  subIndex, 0, 2, 3)) {
      appendEvidenceItem(
          &items, QString("%1 #%2").arg(labels.bookmarkPrefix).arg(row + 1),
...
QVector<SdoEvidenceItem> sdoWriteEvidenceItemsFromValues(
  ...
  appendEvidenceItem(&items, labels.targetTrail,
                     targetTrailWriteValue.trimmed().isEmpty()
                         ? targetTrailValue
                         : targetTrailWriteValue);
...
bool sdoWriteDeltaReviewEvidenceAvailable(const QString &readValue,
  ...
  if (rows.dictionaryRow >= 0 &&
      (!tableText(tables.dictionaryTable, rows.dictionaryRow, 7).isEmpty() ||
       !tableText(tables.dictionaryTable, rows.dictionaryRow, 8).isEmpty())) {
    return true;
  }

  if (rows.watchRow >= 0 &&
      !tableText(tables.watchTable, rows.watchRow, 4).isEmpty()) {
    return true;
  }

  if (rows.startupRow >= 0 &&
      !tableText(tables.startupTable, rows.startupRow, 3).isEmpty()) {
    return true;
  }

  if (rows.bookmarkRow >= 0 &&
      !tableText(tables.bookmarkTable, rows.bookmarkRow, 8).isEmpty()) {
    return true;
  }

  return rows.targetTrailRow >= 0 &&
         (!tableText(tables.targetTrailTable, rows.targetTrailRow, 7)
               .isEmpty() ||
          !tableText(tables.targetTrailTable, rows.targetTrailRow, 6)
               .isEmpty());
...
QString sdoTargetTrailRowKey(int position, const QString &index,
  ...
  return QString("%1|%2|%3|%4|%5|%6")
      .arg(position)
      .arg(normalizeHexText(index, 4), normalizeHexText(subIndex, 2), type,
