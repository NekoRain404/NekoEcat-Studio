...
void setSessionBriefActionKey(QTableWidget* table, int row, const QString& actionKey) {
...
QString sessionBriefActionKeyForRow(QTableWidget* table, int row) {
    ...
    for (int column = 0; column < table->columnCount(); ++column) {
        if (auto* item = table->item(row, column)) {
            const QString stableKey = item->data(kSessionBriefActionKeyRole).toString();
...
SessionBriefTableRow sessionBriefTableRowFromTable(QTableWidget* table, int row) {
