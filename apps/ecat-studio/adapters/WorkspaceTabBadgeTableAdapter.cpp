...
bool workspaceTabBadgeWatchDeltaIsIssue(const QString& delta) {
    ...
    return !normalized.isEmpty() && normalized != QStringLiteral("match") && normalized != QStringLiteral("匹配") &&
           normalized != QStringLiteral("pending") && normalized != QStringLiteral("待比较");
}
...
int countWorkspaceTabBadgeWatchStartupDiffs(QTableWidget* watchTable) {
    ...
    for (int row = 0; row < watchTable->rowCount(); ++row) {
        if (workspaceTabBadgeWatchDeltaIsIssue(tableText(watchTable, row, kWorkspaceTabBadgeWatchDeltaColumn))) {
            ++diffs;
        }
    }
    return diffs;
...
int countWorkspaceTabBadgeIoIssues(QTableWidget* ioVariableTable) {
    ...
    for (int row = 0; row < ioVariableTable->rowCount(); ++row) {
        const IoVariableTableRow variable = ioVariableTableRowFromTable(ioVariableTable, row);
        if (ioVariableTableRowHasStartupDiff(variable) || ioVariableTableRowHasPdoMapIssue(variable) ||
...
WorkspaceTabBadgeCounts workspaceTabBadgeCounts(const WorkspaceTabBadgeTables& tables) {
    WorkspaceTabBadgeCounts counts;
    counts.watchRows = tables.watchTable ? tables.watchTable->rowCount() : 0;
    counts.watchStartupDiffs = countWorkspaceTabBadgeWatchStartupDiffs(tables.watchTable);
    counts.startupRows = tables.startupSdoTable ? tables.startupSdoTable->rowCount() : 0;
    counts.startupDiffs =
        startupRowsWithWatchDiffs(evaluateStartupWatchDeltas(watchStartupStartupRows(tables.startupSdoTable),
                                                             watchStartupWatchRows(tables.watchTable)))
            .size();
    counts.freeRunRows = tables.freeRunEntryTable ? tables.freeRunEntryTable->rowCount() : 0;
    counts.ioRows = tables.ioVariableTable ? tables.ioVariableTable->rowCount() : 0;
    counts.ioIssues = countWorkspaceTabBadgeIoIssues(tables.ioVariableTable);
    counts.consistencyRows = tables.consistencyTable ? tables.consistencyTable->rowCount() : 0;

    const ConsistencyIssueCounts consistency = consistencyTableIssueCounts(tables.consistencyTable);
    ...
    if (tables.stateMachineTable) {
        ...
        for (int row = 0; row < tables.stateMachineTable->rowCount(); ++row) {
            if (!tableText(tables.stateMachineTable, row, kWorkspaceTabBadgeStateRiskColumn).trimmed().isEmpty()) {
    ...
    if (tables.diagnosticsTable) {
        ...
        for (int row = 0; row < tables.diagnosticsTable->rowCount(); ++row) {
            diagnosticLevels.append(tableText(tables.diagnosticsTable, row, kWorkspaceTabBadgeDiagnosticsLevelColumn));
        }
    }
    const DiagnosticsEventSummary diagnostics = diagnosticsEventCounts(diagnosticLevels);
    ...
    const SlaveEvidenceMatrixPriorityCounts matrix = slaveEvidenceMatrixPriorityCounts(tables.slaveEvidenceMatrixTable);
