...
WatchStartupWatchRow watchStartupWatchRow(QTableWidget* watchTable, int row, const QSet<QString>& changedKeys) {
    ...
    model.position = parsedPosition(tableText(watchTable, row, kWatchStartupWatchPositionColumn));
    ...
    model.baselineDelta = tableText(watchTable, row, kWatchStartupWatchBaselineDeltaColumn);
    model.startup = tableText(watchTable, row, kWatchStartupWatchExpectedColumn);
    model.startupDelta = tableText(watchTable, row, kWatchStartupWatchDeltaColumn);
    model.changed = changedKeys.contains(watchStartupTargetKey(model.position, model.index, model.subIndex));
    return model;
}
...
WatchStartupStartupRow watchStartupStartupRow(QTableWidget* startupTable, int row) {
    ...
    model.positionText = tableText(startupTable, row, kWatchStartupStartupPositionColumn);
    model.position = parsedPosition(model.positionText);
    model.index = tableText(startupTable, row, kWatchStartupStartupIndexColumn);
    model.subIndex = tableText(startupTable, row, kWatchStartupStartupSubIndexColumn);
    ...
    model.watchValue = tableText(startupTable, row, kWatchStartupStartupWatchValueColumn);
    model.watchDelta = tableText(startupTable, row, kWatchStartupStartupWatchDeltaColumn);
    return model;
}

// Bulk-extracts all startup rows for delta evaluation against the watch table.
QVector<WatchStartupStartupRow> watchStartupStartupRows(QTableWidget* startupTable) {
...
QTableWidgetItem* ensureWatchStartupTableItem(QTableWidget* table, int row, int column) {
