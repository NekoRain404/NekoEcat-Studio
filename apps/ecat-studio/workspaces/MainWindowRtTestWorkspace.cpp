...
protected:
    void paintEvent(QPaintEvent*) override {
        ...
        // Configure font for visual hierarchy
        p.setFont(QFont("monospace", 8));
        p.drawText(r.adjusted(4, 2, 0, 0), Qt::AlignTop, QString("Jitter: %1 µs").arg(samples_.last(), 0, 'f', 1));
    }
...
QWidget* MainWindow::buildRtTestPage() {
    ...
    auto updateFreqLabel = [this]() {
        const int usec = rtTest_->customCycle->isVisible() ? rtTest_->customCycle->text().toInt()
                                                           : rtTest_->cycleCombo->currentData().toInt();
        if (usec > 0) {
            const double hz = 1000000.0 / usec;
            rtTest_->freqLabel->setText(hz >= 1000 ? QString::number(hz / 1000.0, 'f', 1) + " kHz"
                                                   : QString::number(hz, 'f', 0) + " Hz");
        }
    };
    // Connect QComboBox::currentIndexChanged signal to handler
    connect(rtTest_->cycleCombo, &QComboBox::currentIndexChanged, this, [this, updateFreqLabel](int idx) {
        const bool custom = rtTest_->cycleCombo->itemData(idx).toInt() < 0;
        rtTest_->customCycle->setVisible(custom);
        if (custom)
            rtTest_->customCycle->setFocus();
        updateFreqLabel();
    });
    ...
    rtTest_->timelineText->setPlaceholderText(uiText("Cycle log...", "周期日志..."));
    // Add widget to layout
    ...
    connect(rtTest_->stopButton, &QPushButton::clicked, this, [this] { client_.rtTestStop(); });
...
void MainWindow::updateRtTestTelemetry(const QJsonObject& telemetry) {
    ...
    if (cycles > 0 && avgUs > 0) {
        rtTest_->durationLabel->setText(formatDuration(static_cast<double>(cycles) * avgUs / 1e6));
    }

    // Health bar.
    QString hc = "#45475a";
    if (cycles > 0) {
        hc = (lossRate > 0.1 || jitterUs > 500) ? "#ef4444" : (jitterUs > 100) ? "#f59e0b" : "#22c55e";
    }
    // Apply QSS stylesheet to widget
    rtTest_->healthLabel->setStyleSheet(QString("background: %1; border-radius: 2px;").arg(hc));

    // Jitter spark.
    if (rtTest_->jitterSpark)
        rtTest_->jitterSpark->pushSample(jitterUs);
    ...
    if (running && cycles > 0 && (cycles % 5000 == 0)) {
        rtTest_->timelineText->appendPlainText(QString("#%1  avg=%2µs  jit=%3µs  loss=%4%")
                                                   .arg(cycles)
                                                   .arg(avgUs, 0, 'f', 1)
                                                   .arg(jitterUs, 0, 'f', 1)
                                                   .arg(lossRate, 0, 'f', 3));
