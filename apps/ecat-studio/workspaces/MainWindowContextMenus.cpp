...
void MainWindow::showTopologyContextMenu(const QPoint& position) {
    ...
    auto* refresh =
        ...
        menu.addAction(style()->standardIcon(QStyle::SP_BrowserReload), uiText("Refresh Online Data", "刷新在线数据"));
    // Enable/disable based on action availability
    refresh->setEnabled(connected);
    auto* rescan =
        // Add menu action with icon and handler
        menu.addAction(style()->standardIcon(QStyle::SP_FileDialogDetailedView), uiText("Rescan Bus", "重新扫描总线"));
    // Enable/disable based on action availability
    ...
    // Add menu action with icon and handler
    auto* addStartup = menu.addAction(style()->standardIcon(QStyle::SP_FileDialogNewFolder),
    ...
    if (chosen == refresh) {
        ...
        QApplication::clipboard()->setText(QString("%1\t%2").arg(item->text(0), item->text(1)));
...
void MainWindow::showTableContextMenu(QTableWidget* table, const QPoint& position) {
    ...
    if (rowAtCursor >= 0) {
        const int currentColumn = table->currentColumn() >= 0 ? table->currentColumn() : 0;
        if (table->columnCount() > 0) {
            table->setCurrentCell(rowAtCursor, std::min(currentColumn, table->columnCount() - 1),
                                  QItemSelectionModel::NoUpdate);
        }
        if (!table->selectionModel() || !table->selectionModel()->isRowSelected(rowAtCursor, QModelIndex())) {
            table->selectRow(rowAtCursor);
        }
    }

    auto tableText = [](QTableWidget* source, int row, int column) {
        if (!source || row < 0 || row >= source->rowCount() || column < 0 || column >= source->columnCount()) {
            return QString();
        }
    ...
    if (currentRow >= 0) {
        if (table == sdo_->sdoTable) {
            ...
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 2), 2);
            ...
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 3), 2);
        } else if (table == watch_->watchTable) {
            objectClipboard.position = parsedPosition(tableText(table, currentRow, 1));
            objectClipboard.index = normalizedHex(tableText(table, currentRow, 2), 4);
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 3), 2);
            ...
            objectClipboard.position = parsedPosition(tableText(table, currentRow, 0));
            objectClipboard.index = normalizedHex(tableText(table, currentRow, 4), 4);
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 5), 2);
            ...
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 4), 2);
            objectClipboard.value = tableText(table, currentRow, 8);
            if (objectClipboard.value.isEmpty()) {
                objectClipboard.value = tableText(table, currentRow, 11);
            }
        } else if (table == sdoHistoryTable_) {
            objectClipboard.position = parsedPosition(tableText(table, currentRow, 2));
            objectClipboard.index = normalizedHex(tableText(table, currentRow, 3), 4);
            objectClipboard.subIndex = normalizedHex(tableText(table, currentRow, 4), 2);
    ...
    if (table == workflow_->workflowTable) {
        ...
        const bool hasWorkflowRow =
            workflowRow >= 0 && workflowRow < table->rowCount() && !table->isRowHidden(workflowRow);
        // Add menu action with icon and handler
        reviewFirstWorkflowIssue = menu.addAction(uiText("Review First Workflow Issue", "审阅首个工作流问题"));
        // Set icon for menu action
        reviewFirstWorkflowIssue->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
        // Enable/disable based on action availability
        reviewFirstWorkflowIssue->setEnabled(
            workflow_->workflowReviewButton ? workflow_->workflowReviewButton->isEnabled() : table->rowCount() > 0);
        // ── Table Context Menu ──────────────────────────────────────────────
        reviewNextWorkflowIssue = menu.addAction(uiText("Review Next Workflow Issue", "审阅下个工作流问题"));
        // Set icon for menu action
        reviewNextWorkflowIssue->setIcon(style()->standardIcon(QStyle::SP_ArrowForward));
        // Enable/disable based on action availability
        reviewNextWorkflowIssue->setEnabled(workflow_->workflowReviewNextButton
        ...
        // Set icon for menu action
        copyWorkflowStep->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
        // Enable/disable based on action availability
        ...
        // Set icon for menu action
        copySessionBriefRow->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
        // Enable/disable based on action availability
        ...
        // Enable/disable based on action availability
        reviewFirstMatrixIssue->setEnabled(slaveEvidence_->slaveEvidenceMatrixReviewButton
        ...
        // Enable/disable based on action availability
        reviewNextMatrixIssue->setEnabled(slaveEvidence_->slaveEvidenceMatrixReviewNextButton
        ...
        const bool dictionaryReady = selectedPosition() >= 0 && loadedSdoPosition_ == selectedPosition();
        ...
        // Enable/disable based on action availability
        readSdo->setEnabled(dictionaryReady && hasDictionaryRow && client_.isConnected());
        readSelectedDictionary =
            // Add menu action with icon and handler
            menu.addAction(uiText("Read Selected Objects", "读取所选对象"));
        // Enable/disable based on action availability
        readSelectedDictionary->setEnabled(hasDictionarySelection && client_.isConnected() && selectedPosition() >= 0);
        readVisibleDictionaryAction =
            // Add menu action with icon and handler
            menu.addAction(uiText("Read Visible Objects", "读取可见对象"));
        // Enable/disable based on action availability
        readVisibleDictionaryAction->setEnabled(dictionaryReady && hasVisibleDictionaryRows && client_.isConnected());
        readFailedDictionaryAction =
            // Add menu action with icon and handler
            menu.addAction(uiText("Retry Failed Objects", "重试失败对象"));
        // Enable/disable based on action availability
        readFailedDictionaryAction->setEnabled(dictionaryReady && hasFailedSdoEvidence() && client_.isConnected() &&
                                               selectedPosition() >= 0);
        // Add menu action with icon and handler
        ...
        // Enable/disable based on action availability
        addSelectedDictionaryWatch->setEnabled(hasDictionarySelection && selectedPosition() >= 0);
        addVisibleDictionaryWatch =
            // Add menu action with icon and handler
            menu.addAction(uiText("Add Visible to Watch", "将可见项加入监视"));
        // Enable/disable based on action availability
        addVisibleDictionaryWatch->setEnabled(dictionaryReady && hasVisibleDictionaryRows);
        bool hasDictionaryValueSelection = false;
        for (const int selectedRow : selectedDictionaryRows()) {
            if (selectedRow >= 0 && selectedRow < table->rowCount() && !table->isRowHidden(selectedRow) &&
        ...
        addSelectedDictionaryStartupEvidence =
            // Add menu action with icon and handler
            menu.addAction(uiText("Create Startup SDOs from Selected Evidence", "从所选证据创建 Startup SDO"));
        // Enable/disable based on action availability
        addSelectedDictionaryStartupEvidence->setEnabled(dictionaryReady && hasDictionaryValueSelection);
        // Add menu action with icon and handler
        addStartup = menu.addAction(uiText("Add Startup SDO", "添加启动 SDO"));
        // Enable/disable based on action availability
        addStartup->setEnabled(dictionaryReady && hasDictionaryRow && selectedSdoWritable_);
        ...
        // Enable/disable based on action availability
        bookmarkSelectedDictionaryObjects->setEnabled(dictionaryReady && hasDictionarySelection);
        odMonitorChart =
            // Add menu action with icon and handler
        ...
        // Enable/disable based on action availability
        filterDictionaryEvidence->setEnabled(dictionaryReady && table->rowCount() > 0);
        filterDictionaryFailed =
            // Add menu action with icon and handler
            menu.addAction(uiText("Show Failed Evidence", "只看失败证据"));
        // Enable/disable based on action availability
        filterDictionaryFailed->setEnabled(dictionaryReady && table->rowCount() > 0);
        copyDictionaryEvidence =
            // Add menu action with icon and handler
            menu.addAction(uiText("Copy Last Evidence", "复制最后证据"));
        // Enable/disable based on action availability
        copyDictionaryEvidence->setEnabled(dictionaryReady && hasDictionaryRow &&
        ...
        const bool pdoReady = selectedPosition() >= 0 && loadedPdoPosition_ == selectedPosition();
        ...
        // Add menu action with icon and handler
        addSelectedPdoWatch = menu.addAction(uiText("Add Selected PDOs to Watch", "将选中 PDO 加入监视"));
        // Enable/disable based on action availability
        ...
        const QString watchValue = hasWatchRow && table->item(table->currentRow(), 4)
        ...
        // Add menu action with icon and handler
        addStartupFromWatch = menu.addAction(uiText("Add Startup SDO from Watch", "从监视添加启动 SDO"));
        // Enable/disable based on action availability
        addStartupFromWatch->setEnabled(hasWatchRow && !watchValue.isEmpty());
        addStartupFromSelectedWatch =
            // Add menu action with icon and handler
            menu.addAction(uiText("Create Startup SDOs from Selected Watch", "从选中监视批量创建启动 SDO"));
        // Enable/disable based on action availability
        addStartupFromSelectedWatch->setEnabled(hasWatchValueSelection);
        // Add menu action with icon and handler
        syncStartupFromWatch = menu.addAction(uiText("Sync Startup SDO from Watch", "从监视同步 Startup SDO"));
        // Enable/disable based on action availability
        syncStartupFromWatch->setEnabled(hasWatchRow && !watchValue.isEmpty());
        // Add menu action with icon and handler
        syncStartupFromSelectedWatch =
            menu.addAction(uiText("Sync Startup SDOs from Selected Watch", "从选中监视同步 Startup SDO"));
        // Enable/disable based on action availability
        ...
        menu.addSeparator();
        realtimeMonitorFromFreeRun = menu.addAction(uiText("Real-time Monitor", "实时监视"));
        ...
        const bool hasSelectedIoValueRows = ioVariableTableRowsContainValue(table, selectedIoRows);
        const bool hasVisibleIoValueRows = ioVariableTableRowsContainValue(table, visibleIoRows);
        ...
        addStartupFromIoVariable =
            // Add menu action with icon and handler
            menu.addAction(uiText("Create Startup SDOs from Selected I/O", "从所选 I/O 创建 Startup SDO"));
        // Enable/disable based on action availability
        addStartupFromIoVariable->setEnabled(hasSelectedIoValueRows);
        // Add menu action with icon and handler
        addVisibleIoVariableStartup =
            menu.addAction(uiText("Create Startup SDOs from Visible I/O", "从可见 I/O 创建 Startup SDO"));
        // Enable/disable based on action availability
        ...
        // Add menu action with icon and handler
        bulkNameIoVariableAction = menu.addAction(uiText("Bulk Name Visible / Selected", "批量命名可见 / 所选"));
        // Enable/disable based on action availability
        bulkNameIoVariableAction->setEnabled(hasIoRow || hasVisibleIoRows);
        // Add menu action with icon and handler
        reviewPlcHandoffAction = menu.addAction(uiText("Review PLC Handoff Issues", "审阅 PLC 交接问题"));
        // Enable/disable based on action availability
        reviewPlcHandoffAction->setEnabled(hasVisibleIoRows);
        // Add menu action with icon and handler
        copySelectedPlcDeclarationsAction =
            menu.addAction(uiText("Copy Selected PLC Declarations", "复制所选 PLC 声明"));
        // Enable/disable based on action availability
        copySelectedPlcDeclarationsAction->setEnabled(hasIoRow);
        // Add menu action with icon and handler
        copyVisiblePlcDeclarationsAction = menu.addAction(uiText("Copy Visible PLC Declarations", "复制可见 PLC 声明"));
        // Enable/disable based on action availability
        copyVisiblePlcDeclarationsAction->setEnabled(hasVisibleIoRows);
        // Add menu action with icon and handler
        exportPlcDeclarationsAction = menu.addAction(uiText("Export PLC Declarations ST", "导出 PLC 声明 ST"));
        // Enable/disable based on action availability
        ...
        for (const int selectedRow : selectedRows) {
            if (selectedRow >= 0 && selectedRow < table->rowCount() && !table->isRowHidden(selectedRow)) {
                const QString selectedStatus =
                    table->item(selectedRow, 7) ? table->item(selectedRow, 7)->text() : QString();
                const QString selectedValue =
                    table->item(selectedRow, 6) ? table->item(selectedRow, 6)->text() : QString();
        ...
        const QString value = hasHistoryRow && table->item(row, 6) ? table->item(row, 6)->text().trimmed() : QString();
        fillSdoFromHistory =
            // Add menu action with icon and handler
            menu.addAction(uiText("Fill SDO Fields", "填充 SDO 字段"));
        // Enable/disable based on action availability
        ...
        // Add menu action with icon and handler
        addSelectedHistoryWatch = menu.addAction(uiText("Add Selected History to Watch", "将所选历史加入监视"));
        // Enable/disable based on action availability
        addSelectedHistoryWatch->setEnabled(hasHistorySelection);
        // Add menu action with icon and handler
        addStartupFromHistory = menu.addAction(uiText("Add Startup SDO from History", "从历史添加启动 SDO"));
        // Enable/disable based on action availability
        addStartupFromHistory->setEnabled(hasHistoryRow && !value.isEmpty());
        // Add menu action with icon and handler
        addStartupFromSelectedHistory =
            menu.addAction(uiText("Create Startup SDOs from Selected History", "从所选历史创建启动 SDO"));
        // Enable/disable based on action availability
        ...
        // Enable/disable based on action availability
        verifySelectedStartupRows->setEnabled(hasSelectedStartupRows && client_.isConnected());
        ...
        // Enable/disable based on action availability
        applyStartupWatchDiffRows->setEnabled(client_.isConnected() && hasStartupWatchDiffs);
        applySelectedStartupRows =
            // Add menu action with icon and handler
            menu.addAction(uiText("Apply Selected Rows", "应用所选行"));
        // Enable/disable based on action availability
        applySelectedStartupRows->setEnabled(hasSelectedStartupRows && client_.isConnected());
        // Add visual separator between menu groups
        menu.addSeparator();
        // Add menu action with icon and handler
        ...
        // Enable/disable based on action availability
        moveStartupRowDown->setEnabled(hasStartupRow && row < table->rowCount() - 1);
        removeStartupRow =
            // Add menu action with icon and handler
        ...
        // Add menu action with icon and handler
        addStartupFromBookmark = menu.addAction(uiText("Create Startup SDO from Bookmark", "从书签创建 Startup SDO"));
        // Enable/disable based on action availability
        ...
        const bool trailStartupReady = hasTrailRow && sdoTargetTrailRowCanCreateStartup(table->currentRow());
        ...
        // Add menu action with icon and handler
        startupSdoTargetTrail = menu.addAction(uiText("Create Startup SDO from Target", "从目标创建 Startup SDO"));
        // Enable/disable based on action availability
    ...
    if (chosen == reviewFirstWorkflowIssue) {
        ...
        addDictionaryRowsToBookmarks({table->currentRow()}, uiText("context row", "右键行"));
        ...
        QStringList parts;
        parts << (selectedPosition() >= 0 ? QString("#%1 %2:%3").arg(selectedPosition()).arg(index, subIndex)
        ...
        log(QString("Copied object dictionary evidence %1:%2 to clipboard").arg(index, subIndex));
        ...
        const int row = table->currentRow();
        const int position = table->item(row, 1) ? table->item(row, 1)->text().toInt() : -1;
        const QString index = table->item(row, 2) ? table->item(row, 2)->text().trimmed() : QString();
        const QString subIndex = table->item(row, 3) ? table->item(row, 3)->text().trimmed() : QString();
        if (position >= 0 && !index.isEmpty() && !subIndex.isEmpty()) {
            const QString type = table->item(row, 6) ? table->item(row, 6)->text().trimmed() : QString();
            requestSdoRead(position, index, subIndex, uiText("Watch context menu", "监视右键菜单"), type);
        }
        ...
        const QString key = QString("%1|%2|%3")
                                ...
                                .arg(table->item(row, 2) ? table->item(row, 2)->text().trimmed() : QString(),
        ...
        log(QString("Copied object address %1 to clipboard").arg(objectClipboard.addressText()));
        ...
        log(QString("Copied object value %1 to clipboard").arg(objectClipboard.valueText()));
...
void MainWindow::showSdoTargetPanelContextMenu(const QPoint& position) {
    ...
    if (rowAtCursor >= 0) {
        sdoInspector_->sdoTargetTable->setCurrentCell(rowAtCursor, 0, QItemSelectionModel::NoUpdate);
...
bool MainWindow::runLocalEvidenceAction(QTableWidget* table) {
    ...
    if (row < 0 || row >= table->rowCount()) {
        statusBar()->showMessage(uiText("Select a row before using Alt+Enter.", "使用 Alt+Enter 前请先选择一行。"),
                                 3000);
        return true;
    }
    ...
    if (table == workflow_->workflowTable) {
        ...
        statusBar()->showMessage(uiText("Alt+Enter is reserved for local evidence actions on object "
                                        "and evidence tables.",
    ...
    statusBar()->showMessage(
        uiText("Local evidence action complete. No bus access was requested.", "本地证据动作完成；未请求总线访问。"),
        3000);
    return true;
}
...
void MainWindow::copyTableToClipboard(QTableWidget* table, bool selectedOnly) {
    ...
    for (int column = 0; column < table->columnCount(); ++column) {
        headers << cleanCell(table->horizontalHeaderItem(column) ? table->horizontalHeaderItem(column)->text()
                                                                 : QString());
    }
    lines << headers.join('\t');
    ...
    for (const int row : rows) {
        QStringList values;
        for (int column = 0; column < table->columnCount(); ++column) {
            values << cleanCell(table->item(row, column) ? table->item(row, column)->text() : QString());
        }
        lines << values.join('\t');
    }
