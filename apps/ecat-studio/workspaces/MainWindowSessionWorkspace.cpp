...
QStyle::StandardPixmap nextBestActionStandardPixmap(NextBestActionIconKey icon) {
    switch (icon) {
...
void MainWindow::updateSessionBrief() {
    ...
    const bool hasSdoRows =
        hasSelectedSlave && loadedSdoPosition_ == selected && sdo_->sdoTable && sdo_->sdoTable->rowCount() > 0;
    const bool hasPdoRows =
        hasSelectedSlave && loadedPdoPosition_ == selected && sdo_->pdoTable && sdo_->pdoTable->rowCount() > 0;
    ...
    if (hasSelectedSlave) {
        ...
        for (const auto& slave : slaves_) {
            if (slave.position == selected) {
                ...
                selectedSlave = name.isEmpty() ? selectedSlave : QString("#%1 %2").arg(selected).arg(name);
                selectedState = slave.state.trimmed().isEmpty() ? uiText("unknown", "未知") : slave.state.trimmed();
                break;
            }
        }
    }
    ...
    consistencyIssueCounts(&consistencyErrors, &consistencyWarnings, &consistencyInfos, &consistencyReady);
    const bool hasConsistencyCheck = consistencyFresh_ && consistencyCheckAvailable();
    const bool hasConsistencyBlockingIssues =
        consistencyHasBlockingIssues({consistencyErrors, consistencyWarnings, consistencyInfos, consistencyReady});
    ...
    QString sdoEvidenceText = uiText("No complete SDO target", "尚无完整 SDO 目标");
    QString sdoNext = uiText("Select an OD row or fill SDO fields", "选择 OD 行或填写 SDO 字段");
    if (hasSelectedSlave && sdoInspector_->sdoIndex && sdoInspector_->sdoSubIndex &&
        ...
        if (groups.size() > 1) {
            sdoNext = uiText("Review evidence delta before writing", "写入前审阅证据差异");
        } else if (writeDiffers && !writeMatches) {
            sdoNext = uiText("Confirm write delta", "确认写入差异");
        } else if (!candidates.isEmpty()) {
            sdoNext = writeValue.isEmpty() ? uiText("Use evidence or read target", "使用证据或读取目标")
                                           : uiText("Write uses normal confirmation", "写入走普通确认流程");
        } else {
            sdoNext = uiText("Read target or add Watch evidence", "读取目标或添加 Watch 证据");
        }
        sdoEvidenceText = uiText("%1 | evidence %2 group(s)%3", "%1 | 证据 %2 组%3")
                              .arg(address)
                              .arg(groups.size())
                              .arg(facts.isEmpty() ? QString() : QString(" | %1").arg(facts.join("; ")));
    ...
    if (nextStep >= 0 && workflow_->workflowTable && nextStep < workflow_->workflowTable->rowCount()) {
        nextAction = commissioningWorkflowTableRowFromTable(workflow_->workflowTable, nextStep).nextAction;
    ...
    rows.append(sessionBriefUiRow(
        ...
        uiText("%1 | runtime %2 | %3 slave(s)", "%1 | 运行时 %2 | %3 个从站")
            .arg(selectedSlave, connected ? uiText("connected", "已连接") : uiText("offline", "离线"))
            .arg(slaves_.size()),
        !connected ? uiText("Connect runtime", "连接运行时")
                   : (!hasSlaves ? uiText("Rescan bus", "重新扫描总线")
                                 : (!hasSelectedSlave ? uiText("Select a slave", "选择从站")
                                                      : uiText("Continue evidence review", "继续证据复核"))),
        uiTexts));
    rows.append(
        sessionBriefUiRow(briefRows.at(1), uiText("Gate", "门禁"),
                          hasConsistencyCheck ? uiText("Consistency: %1 error(s), %2 warning(s), %3 info, %4 "
                                                       "ready",
                          ...
                          hasConsistencyCheck && hasConsistencyBlockingIssues
                              ...
                              : (hasConsistencyCheck ? uiText("Proceed when machine is safe", "设备安全时继续")
                                                     : uiText("Run read-only consistency gate", "运行只读一致性门禁")),
                          uiTexts));
    rows.append(sessionBriefUiRow(
        briefRows.at(2), uiText("Map", "映射"),
        uiText("OD %1 row(s) | PDO %2 row(s) | failed OD %3", "OD %1 行 | PDO %2 行 | 失败 OD %3")
        ...
        !hasSelectedSlave ? uiText("Select slave", "选择从站")
                          : (!hasSdoRows ? uiText("Load Object Dictionary", "加载对象字典")
                                         : (!hasPdoRows ? uiText("Load PDO Map", "加载 PDO 映射")
                                                        : (hasFailedOdEvidence
                                                               ? uiText("Review failed OD evidence", "审阅失败 OD 证据")
                                                               : uiText("Map evidence ready", "映射证据就绪")))),
        uiTexts));
    rows.append(
        sessionBriefUiRow(briefRows.at(3), uiText("Current SDO", "当前 SDO"), sdoEvidenceText, sdoNext, uiTexts));
    rows.append(sessionBriefUiRow(
        ...
        startupDiffs > 0 ? uiText("Review Startup diffs", "审阅 Startup 偏差")
                         : (!hasWatchRows ? uiText("Add Watch evidence", "添加 Watch 证据")
                                          : (!freeRun_ && !hasFreeRunRows
                                                 ? uiText("Validate process image when safe", "安全时验证过程映像")
                                                 : uiText("Runtime evidence ready", "运行证据就绪"))),
        uiTexts));
    rows.append(sessionBriefUiRow(briefRows.at(5), uiText("Next", "下一步"),
                                  uiText("State %1 | workflow row %2", "状态 %1 | 工作流行 %2")
                                      .arg(selectedState)
                                      .arg(nextStep >= 0 ? QString::number(nextStep + 1) : uiText("ready", "就绪")),
                                  nextAction, uiTexts));
    ...
    setTableRows(session_->sessionBriefTable, sessionBriefTableHeaders(uiTexts), tableRows);
    ...
    for (int row = 0; row < session_->sessionBriefTable->rowCount(); ++row) {
        ...
        const QColor color = colorKey == QStringLiteral("ready")
                                 ? readyColor
                                 : (colorKey == QStringLiteral("error")
                                        ? errorColor
                                        : (colorKey == QStringLiteral("warning") ? warningColor : actionColor));
...
void MainWindow::updateSessionBriefCopyButton() {
    ...
    if (!hasRow) {
        ...
        session_->sessionBriefCopyButton->setToolTip(
            uiText("Select a Session Brief row to copy its local evidence summary. No "
                   "bus request is sent.",
                   "选择一条会话简报行以复制本地证据摘要；不会发送总线请求。"));
        return;
    }

    const QString area = sessionBriefTableRowFromTable(session_->sessionBriefTable, row).area.trimmed();
    session_->sessionBriefCopyButton->setText(area.isEmpty() ? uiText("Copy Row", "复制本行")
                                                             : uiText("Copy: %1", "复制：%1").arg(area));
    session_->sessionBriefCopyButton->setToolTip(uiText("Copy the Session Brief row \"%1\" with status, evidence, next "
...
bool MainWindow::copySessionBriefRowDigest(int row) {
    if (!session_->sessionBriefTable || row < 0 || row >= session_->sessionBriefTable->rowCount()) {
        statusBar()->showMessage(uiText("Select a Session Brief row to copy.", "请选择一条会话简报行再复制。"), 3000);
        return false;
    }

    const SessionBriefTableRow briefRow = sessionBriefTableRowFromTable(session_->sessionBriefTable, row);

    QStringList lines;
    lines << uiText("NekoEcat Studio Session Brief Row", "NekoEcat Studio 会话简报本行");
    ...
    if (!briefRow.actionKey.isEmpty()) {
        lines << QString("%1: %2").arg(uiText("Local Route", "本地路由"), briefRow.actionKey);
    }
    if (nextBestActionButton_) {
        lines << QString("%1: %2").arg(uiText("Current Next Best Action", "当前下一最佳动作"),
                                       nextBestActionButton_->text().trimmed());
    }
    if (!briefRow.firstTooltip.isEmpty()) {
        lines << QString();
        lines << uiText("Row Detail", "行详情");
        const QStringList detailLines = briefRow.firstTooltip.split('\n', Qt::SkipEmptyParts);
    ...
    lines << QString();
    lines << uiText("Local Action: Open Session Brief Evidence routes this row "
    ...
    updateDiagnostics(
        "Info", "Session Brief",
        uiText("Copied Session Brief row #%1 to clipboard", "已复制会话简报第 %1 行到剪贴板").arg(row + 1));
    statusBar()->showMessage(
        uiText("Copied Session Brief row. No bus access was requested.", "已复制会话简报行；未请求总线访问。"), 3000);
    return true;
}
...
void MainWindow::updateWorkflowStepCopyButton() {
    ...
    const bool hasRow =
        row >= 0 && row < workflow_->workflowTable->rowCount() && !workflow_->workflowTable->isRowHidden(row);
    ...
    if (!hasRow) {
        ...
        workflow_->workflowStepCopyButton->setToolTip(
            uiText("Select a commissioning workflow step to copy its local evidence "
                   "summary. No bus request is sent.",
                   "选择一条调试工作流步骤以复制本地证据摘要；不会发送总线请求。"));
        return;
    }

    const QString step = commissioningWorkflowTableRowFromTable(workflow_->workflowTable, row).step.trimmed();
    workflow_->workflowStepCopyButton->setText(step.isEmpty() ? uiText("Copy Step", "复制步骤")
...
void MainWindow::updateWorkflowStepDetail() {
    ...
    const WorkflowStepDetailTexts texts = workflowStepDetailTexts();
    auto applyState = [this](const WorkflowStepDetailState& state) {
    ...
    const int row = workflow_->workflowTable->currentRow();
    if (row < 0 || row >= workflow_->workflowTable->rowCount() || workflow_->workflowTable->isRowHidden(row)) {
        applyState(workflowStepDetailNoSelectionState(texts));
        return;
    }

    applyState(buildWorkflowStepDetailState(commissioningWorkflowTableRowFromTable(workflow_->workflowTable, row),
...
void MainWindow::openSessionBriefRow(int row) {
    ...
    const QString combined = QString("%1 %2 %3 %4").arg(area, status, evidence, next);

    auto logNavigation = [this](const QString& target) {
        updateDiagnostics("Info", "Session Brief",
                          uiText("Opened local Session Brief evidence: %1", "已打开会话简报本地证据：%1").arg(target));
    };
    ...
    if (actionKey == QStringLiteral("target")) {
        if (topologyTree_) {
            ...
            if (topologyTree_->currentItem()) {
                topologyTree_->scrollToItem(topologyTree_->currentItem(), QAbstractItemView::PositionAtCenter);
            }
        }
    ...
    if (actionKey == QStringLiteral("map")) {
        ...
        if (sdo_->pdoTable && selectedPosition() >= 0 && loadedPdoPosition_ == selectedPosition() &&
    ...
    if (actionKey == QStringLiteral("runtime")) {
        ...
        if (freeRunWidgets_->freeRunEntryTable && freeRunWidgets_->freeRunEntryTable->currentRow() >= 0) {
            selectAndFocusTableRow(freeRunWidgets_->freeRunEntryTable, freeRunWidgets_->freeRunEntryTable->currentRow(),
                                   0);
        }
...
void MainWindow::updateNextBestAction() {
    ...
    const NextBestActionDetail uiState = buildNextBestActionDetail(decision, actionInput, nextBestActionTexts());
    ...
    nextBestActionButton_->setIcon(style()->standardIcon(nextBestActionStandardPixmap(uiState.icon)));
...
void MainWindow::runNextBestAction() {
    ...
    if (action == nextBestActionKey(NextBestActionKind::Connect)) {
    ...
    } else if (action == nextBestActionKey(NextBestActionKind::FailedOdEvidence)) {
        focusFailedSdoEvidence();
        ...
        focusStartupSdoWatchDiffs();
    } else if (action == nextBestActionKey(NextBestActionKind::ConsistencyEvidenceIssue)) {
    ...
    updateDiagnostics(
        "Info", "Navigator",
        uiText("Next Best Action executed: %1", "已执行下一最佳动作：%1").arg(nextBestActionButton_->text()));
...
void MainWindow::filterCommissioningWorkflow() {
    ...
    const QString needle = workflow_->workflowFilter ? workflow_->workflowFilter->text().trimmed() : QString();
    const QString scope = workflow_->workflowScopeFilter ? workflow_->workflowScopeFilter->currentData().toString()
                                                         : QString::fromLatin1(kCommissioningWorkflowScopeAll);
    const CommissioningWorkflowFilterStats stats =
        filterCommissioningWorkflowTable(workflow_->workflowTable, scope, needle, uiText("None", "无"));
    ...
    if (current < 0 || current >= workflow_->workflowTable->rowCount() ||
        ...
        if (stats.firstVisible >= 0) {
            workflow_->workflowTable->setCurrentCell(stats.firstVisible, kCommissioningWorkflowStepColumn);
    ...
    if (workflow_->workflowSummaryLabel) {
        const QString scopeLabel =
            workflow_->workflowScopeFilter ? workflow_->workflowScopeFilter->currentText() : uiText("All", "全部");
        ...
        workflow_->workflowSummaryLabel->setToolTip(
            uiText("%1\n\nVisible workflow rows: %2/%3\nScope: %4\nOpen: %5 | "
                   ...
                   "完成，不读取总线。")
                .arg(currentDetail.isEmpty() ? uiText("Commissioning Workflow", "调试工作流") : currentDetail)
                .arg(stats.visible)
    ...
    if (row >= 0) {
        ...
        updateDiagnostics("Info", "Commissioning Workflow",
                          uiText("Selected first visible workflow issue #%1 without running it",
                              ...
                              .arg(row + 1));
        statusBar()->showMessage(uiText("Selected first workflow issue. No bus access was requested.",
                                        "已选择首个工作流问题；未请求总线访问。"),
                                 3000);
        return;
    }
    statusBar()->showMessage(uiText("No visible workflow issue to review.", "当前没有可见工作流问题可审阅。"), 3000);
}
...
void MainWindow::reviewNextCommissioningWorkflowIssue() {
    ...
    if (rowCount <= 0) {
        statusBar()->showMessage(uiText("No workflow issue to review.", "当前没有工作流问题可审阅。"), 3000);
        return;
    }

    const int row = nextCommissioningWorkflowIssueRow(workflow_->workflowTable, workflow_->workflowTable->currentRow());
    if (row >= 0) {
        selectAndFocusTableRow(workflow_->workflowTable, row, 2);
        updateDiagnostics("Info", "Commissioning Workflow",
                          uiText("Selected next visible workflow issue #%1 without running it",
                              ...
                              .arg(row + 1));
        statusBar()->showMessage(uiText("Selected next workflow issue. No bus access was requested.",
                                        "已选择下个工作流问题；未请求总线访问。"),
                                 3000);
        return;
    }
...
bool MainWindow::copyWorkflowStepDigest(int row) {
    if (!workflow_->workflowTable || row < 0 || row >= workflow_->workflowTable->rowCount() ||
        ...
        statusBar()->showMessage(
            uiText("Select a visible workflow step to copy.", "请选择一条可见的工作流步骤再复制。"), 3000);
        return false;
    }
    ...
    lines << uiText("NekoEcat Studio Commissioning Workflow Step", "NekoEcat Studio 调试工作流步骤");
    lines << QString("%1: %2").arg(uiText("Master", "主站"), activeMasterName());
    ...
    lines << QString("%1: %2").arg(uiText("Evidence", "依据"), workflowRow.evidence);
    lines << QString("%1: %2").arg(uiText("Next Action", "下一步动作"), workflowRow.nextAction);
    if (auto* runNextButton = findChild<QPushButton*>("overviewRunNext")) {
        lines << QString("%1: %2").arg(uiText("Workflow Run Next", "工作流执行下一步"),
    ...
    if (workflow_->workflowSummaryLabel && !workflow_->workflowSummaryLabel->text().trimmed().isEmpty()) {
    ...
    lines << QString();
    lines << uiText("Local Action: Copy Step Evidence prepares this workflow row for "
    ...
    updateDiagnostics("Info", "Commissioning Workflow",
                      uiText("Copied commissioning workflow step #%1 to "
                          ...
                          .arg(row + 1));
    statusBar()->showMessage(
        uiText("Copied workflow step. No bus access was requested.", "已复制工作流步骤；未请求总线访问。"), 3000);
    return true;
}
...
void MainWindow::runNextCommissioningWorkflowStep() {
    ...
    if (row < 0) {
        updateDiagnostics("Info", "Workflow",
                          uiText("Commissioning workflow is already ready", "调试工作流当前已就绪"));
        return;
    }
...
void MainWindow::runCommissioningWorkflowStep(int row) {
    ...
    switch (commissioningWorkflowStepForIndex(row)) {
        ...
        case CommissioningWorkflowStep::RunConsistencyGate: {
            int consistencyErrors = 0;
            int consistencyWarnings = 0;
            consistencyIssueCounts(&consistencyErrors, &consistencyWarnings, nullptr, nullptr);
            const bool hasConsistencyCheck = consistencyFresh_ && consistencyCheckAvailable();
            const bool hasConsistencyBlockingIssues =
                consistencyHasBlockingIssues({consistencyErrors, consistencyWarnings, 0, 0});
...
void MainWindow::updateHostHealth(const QJsonArray& checks) {
    ...
    const HostHealthDetail uiState = buildHostHealthDetail(checks, hostHealthTexts());
...
void MainWindow::updateDiagnosticsSummary() {
    ...
    diagnostics_->diagnosticsSummaryLabel->setText(diagnosticsEventSummary(rows, diagnosticsEventTexts()).text);
    updateTabBadges();
}
...
void MainWindow::styleDiagnosticsRow(int row, const QString& level) {
    ...
    const QColor color = diagnosticsEventColorForKey(diagnosticsEventColorKey(level));
...
void MainWindow::updateDiagnostics(const QString& level, const QString& source, const QString& message) {
    if (!diagnostics_->diagnosticsTable->columnCount()) {
        diagnostics_->diagnosticsTable->setColumnCount(4);
        // Define column headers for the table
        diagnostics_->diagnosticsTable->setHorizontalHeaderLabels(diagnosticsEventHeaders(diagnosticsEventTexts()));
    }
    const int row = diagnostics_->diagnosticsTable->rowCount();
    diagnostics_->diagnosticsTable->insertRow(row);
    diagnostics_->diagnosticsTable->setItem(row, 0,
                                            // Create table cell
...
void MainWindow::updateTabBadges() {
    ...
    const WorkspaceTabBadgeCounts badgeCounts =
        workspaceTabBadgeCounts({.watchTable = watch_->watchTable,
    ...
    const WorkspaceTabBadgeDetail badges = buildWorkspaceTabBadgeDetail(badgeCounts, workspaceTabBadgeTexts());
    ...
    setTab(stateMachineTabIndex_, badges.stateMachine.text, badges.stateMachine.tip);
...
void MainWindow::updateStatusBar() {
    ...
    statusSummaryLabel_->setText(
        QString("%1: %2 | %3: %4 | %5: %6 | %7: %8")
            ...
            .arg(client_.isConnected() ? uiText("connected", "已连接") : uiText("offline", "离线"))
...
void MainWindow::updateWorkspaceBoundary() {
    ...
    const QString workspaceName = tabs_->tabText(tabs_->currentIndex()).remove(QRegularExpression(R"( !?\d+$)"));
    const WorkspaceBoundaryDetail state =
        buildWorkspaceBoundaryDetail(workspaceBoundaryKindForPage(tabs_->currentWidget()), workspaceName,
