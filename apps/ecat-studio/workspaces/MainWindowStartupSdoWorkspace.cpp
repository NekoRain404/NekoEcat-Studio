...
void MainWindow::ensureStartupSdoTable() {
    ...
    startupSdoTable_->setHorizontalHeaderLabels(
        {uiText("Slave", "从站"), uiText("Index", "索引"), uiText("Sub", "子项"), uiText("Value", "值"),
         uiText("Type", "类型"), uiText("Status", "状态"), uiText("Detail", "详情"), uiText("Watch Value", "Watch 值"),
...
void MainWindow::updateStartupSdoWatchEvidence() {
    ...
    const QVector<WatchStartupStartupDelta> deltas = evaluateStartupWatchDeltas(
        watchStartupStartupRows(startupSdoTable_), watchStartupWatchRows(watch_->watchTable));
    const WatchStartupSummary summary = summarizeStartupWatchDeltas(deltas);

    for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
        auto* watchValueItem = startupSdoTable_->item(row, kWatchStartupStartupWatchValueColumn);
        if (!watchValueItem) {
            // Create table cell
            watchValueItem = new QTableWidgetItem;
            startupSdoTable_->setItem(row, kWatchStartupStartupWatchValueColumn, watchValueItem);
        }
        auto* deltaItem = startupSdoTable_->item(row, kWatchStartupStartupWatchDeltaColumn);
        if (!deltaItem) {
            // Create table cell
            deltaItem = new QTableWidgetItem;
            startupSdoTable_->setItem(row, kWatchStartupStartupWatchDeltaColumn, deltaItem);
        }
        clearWatchStartupDeltaCells(watchValueItem, deltaItem);

        const WatchStartupStartupDelta delta = row < deltas.size() ? deltas.at(row) : WatchStartupStartupDelta{};
        if (delta.state == WatchStartupDeltaState::NoTarget) {
            continue;
        }

        applyWatchStartupDeltaCells(watchValueItem, deltaItem, delta.state, delta.watchValue, watchStartupDeltaTexts(),
    ...
    if (watch_->startupWatchSummaryLabel) {
        ...
        watch_->startupWatchSummaryLabel->setText(
            rows <= 0 ? uiText("No Startup SDO rows", "暂无 Startup SDO 行")
...
void MainWindow::filterStartupSdoTable() {
    ...
    const bool diffsOnly = watch_->startupWatchDiffsOnly && watch_->startupWatchDiffsOnly->isChecked();
    QSet<int> diffRows;
    if (diffsOnly) {
...
void MainWindow::updateStartupSdoRowDetail() {
    ...
    auto applyState = [this](const StartupSdoRowDetailState& state) {
    ...
    if (row < 0 || row >= startupSdoTable_->rowCount() || startupSdoTable_->isRowHidden(row)) {
        applyState(startupSdoRowDetailNoSelectionState(texts));
        return;
    }

    applyState(buildStartupSdoRowDetailState(watchStartupStartupRow(startupSdoTable_, row), texts));
}
...
void MainWindow::focusStartupSdoWatchDiffs() {
    ...
    startupSdoTable_->scrollToItem(startupSdoTable_->item(firstRow, 8), QAbstractItemView::PositionAtCenter);
    startupSdoTable_->setFocus();
    updateStartupSdoControls();
    updateDiagnostics(
        "Info", "Startup SDO",
        uiText("Focused %1 Startup SDO Watch diff row(s)", "已聚焦 %1 条 Startup SDO Watch 偏差行").arg(rows.size()));
}
...
void MainWindow::addStartupSdo() {
    ...
    startupSdoTable_->setItem(row, 0,
                              // Create table cell
                              new QTableWidgetItem(QString::number(selectedPosition() >= 0 ? selectedPosition() : 0)));
    // Create table cell
    ...
    startupSdoTable_->setItem(
        row, 3,
        // Create table cell
        new QTableWidgetItem(sdoInspector_->sdoWriteValue ? sdoInspector_->sdoWriteValue->text() : QString()));
    startupSdoTable_->setItem(
        row, 4,
        // Create table cell
...
void MainWindow::removeStartupSdo() {
    ...
    updateDiagnostics("Info", "Startup SDO", QString("Removed %1 selected startup SDO row(s)").arg(rows.size()));
    updateWatchStartupDeltas();
    filterStartupSdoTable();
    updateStartupSdoControls();
}
...
void MainWindow::moveStartupSdoRow(int delta) {
    ...
    QStringList next;
    for (int column = 0; column < startupSdoTable_->columnCount(); ++column) {
        current << (startupSdoTable_->item(row, column) ? startupSdoTable_->item(row, column)->text() : QString());
        next << (startupSdoTable_->item(target, column) ? startupSdoTable_->item(target, column)->text() : QString());
    }
    for (int column = 0; column < startupSdoTable_->columnCount(); ++column) {
    ...
    updateStartupSdoControls();
    updateDiagnostics("Info", "Startup SDO",
                      QString("Moved startup SDO row from %1 to %2").arg(row + 1).arg(target + 1));
}
...
void MainWindow::verifyStartupSdoList() {
    ensureStartupSdoTable();
    if (!client_.isConnected() || !startupSdoTable_ || startupSdoTable_->rowCount() <= 0) {
        return;
    }
    ...
    for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
        const int position = startupSdoTable_->item(row, 0) ? startupSdoTable_->item(row, 0)->text().toInt() : -1;
        const QString index =
            startupSdoTable_->item(row, 1) ? startupSdoTable_->item(row, 1)->text().trimmed() : QString();
        const QString subIndex =
            startupSdoTable_->item(row, 2) ? startupSdoTable_->item(row, 2)->text().trimmed() : QString();
        const QString type =
            startupSdoTable_->item(row, 4) ? startupSdoTable_->item(row, 4)->text().trimmed() : QString();
        ...
        if (position < 0 || index.isEmpty() || subIndex.isEmpty()) {
            ...
            detail->setText(uiText("Missing slave, index, or subindex", "缺少从站、索引或子项"));
            continue;
        }
        ...
        requestSdoRead(position, index, subIndex, uiText("Startup SDO verify", "Startup SDO 校验"), type);
        ++requested;
    }
    startupSdoTable_->resizeColumnsToContents(); // auto-fit column widths
    updateWatchStartupDeltas();
    updateStartupSdoControls();
    updateDiagnostics("Info", "Startup SDO", QString("Verification requested for %1 startup entries").arg(requested));
}
...
void MainWindow::verifyStartupSdoRow(int row) {
    ensureStartupSdoTable();
    if (!client_.isConnected() || !startupSdoTable_ || row < 0 || row >= startupSdoTable_->rowCount()) {
        return;
    }
    ...
    if (!positionOk || position < 0 || index.isEmpty() || subIndex.isEmpty()) {
        ...
        updateDiagnostics("Error", "Startup SDO",
                          QString("Cannot verify startup SDO row %1: missing address").arg(row + 1));
        return;
    }
    ...
    const QString key = sdoEvidenceKey(position, index, subIndex);
    pendingStartupSdoChecks_[key].append(row);
    requestSdoRead(position, index, subIndex, uiText("Startup SDO row verify", "Startup SDO 行校验"), type);
    startupSdoTable_->resizeColumnsToContents(); // auto-fit column widths
    updateWatchStartupDeltas();
    updateStartupSdoControls();
    updateDiagnostics("Info", "Startup SDO", QString("Verification requested for startup row %1").arg(row + 1));
}
...
void MainWindow::verifySelectedStartupSdoRows() {
    ...
    updateDiagnostics("Info", "Startup SDO",
                      QString("Verification requested for %1 selected startup row(s)").arg(requested));
}
...
void MainWindow::addStartupSdoRowToWatch(int row) {
    ...
    if (!positionOk || position < 0 || index.isEmpty() || subIndex.isEmpty()) {
        updateDiagnostics("Error", "Startup SDO",
                          QString("Cannot add startup row %1 to Watch: missing address").arg(row + 1));
        return;
    }
...
bool MainWindow::preflightStartupSdoList(bool showSuccess) {
    ...
    for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
        auto textAt = [this, row](int column) {
            return startupSdoTable_->item(row, column) ? startupSdoTable_->item(row, column)->text().trimmed()
                                                       : QString();
        };
        bool positionOk = false;
        const int position = textAt(0).toInt(&positionOk);
        ...
        } else if (!onlinePositions.isEmpty() && !onlinePositions.contains(position)) {
            rowErrors << uiText("slave is not present in current topology", "当前拓扑中不存在该从站");
        }
        validateSdoAddressAndValue(index, subIndex, value, type, &rowErrors, &rowWarnings);

        const QString key = QString("%1|%2|%3").arg(position).arg(index.toLower(), subIndex.toLower());
        if (!index.isEmpty() && !subIndex.isEmpty()) {
            if (firstOccurrence.contains(key)) {
                ...
                if (firstValue.trimmed().compare(value.trimmed(), Qt::CaseInsensitive) != 0) {
                    const int firstRowIndex = firstOccurrence.value(key);
                    if (firstRowIndex >= 0 && firstRowIndex < startupSdoTable_->rowCount()) {
                        ...
                        firstStatus->setBackground(settings_.theme == "Light"
                                                       ...
                                                       // Define color for visual feedback
                                                       : QColor("#3a1218"));
                        firstDetail->setText(uiText("conflicting duplicate object also appears on row %1",
                                                    "对象和第 %1 行重复且写入值冲突")
                                                 .arg(row + 1));
                    }
                    rowErrors << uiText("conflicting duplicate object on row %1 has value %2",
                                        "与第 %1 行对象重复且写入值冲突：%2")
                                     .arg(firstRow)
                                     .arg(firstValue);
                } else {
                    rowWarnings
                        << uiText("duplicate object also appears on row %1", "对象和第 %1 行重复").arg(firstRow);
                }
            } else {
    ...
    if (!globalIssues.isEmpty()) {
        updateDiagnostics("Warning", "Startup SDO",
                          uiText("Topology baseline issue(s): %1", "拓扑基线问题：%1").arg(globalIssues.join(" | ")));
    }

    const QString summary =
        uiText("Startup SDO preflight: %1 error(s), %2 warning(s)", "Startup SDO 预检查：%1 个错误，%2 个警告")
            .arg(errors)
            .arg(warnings);
    updateDiagnostics(errors > 0 ? "Error" : (warnings > 0 ? "Warning" : "Info"), "Startup SDO", summary);
    if (showSuccess && errors == 0) {
        QMessageBox::information(this, uiText("Startup SDO Preflight", "Startup SDO 预检查"),
                                 warnings > 0 ? summary
                                              : uiText("Startup SDO preflight passed.", "Startup SDO 预检查通过。"));
    }
    return errors == 0;
}
...
void MainWindow::applyStartupSdoRow(int row) {
    ...
    validateSdoAddressAndValue(index, subIndex, value, type, &validationErrors, &validationWarnings);
    if (!validationErrors.isEmpty()) {
        ...
        startupSdoTable_->resizeColumnsToContents(); // auto-fit column widths
        updateWatchStartupDeltas();
        QMessageBox::warning(this, uiText("Startup SDO Validation Failed", "Startup SDO 校验失败"),
    ...
    if (!confirmDangerousOperation(uiText("Confirm Startup SDO Row Apply", "确认应用 Startup SDO 行"),
    ...
    const QString key = sdoEvidenceKey(position, index, subIndex);
    pendingSdoWrites_.insert(key, {QString::number(position), index, subIndex, type, value});
    const QString verifyKey = sdoEvidenceKey(position, index, subIndex);
    pendingStartupSdoChecks_[verifyKey].append(row);
    appendSdoHistory(uiText("Write", "写入"), position, index, subIndex, type, value, uiText("Requested", "已请求"),
                     uiText("Startup SDO row apply", "Startup SDO 行应用"));
    client_.download(position, index, subIndex, value, type);
    updateDiagnostics("Info", "Startup SDO", QString("Apply requested for startup row %1").arg(row + 1));
    startupSdoTable_->resizeColumnsToContents(); // auto-fit column widths
    updateWatchStartupDeltas();
    updateStartupSdoControls();
}
...
void MainWindow::applyStartupSdoRows(const QVector<int>& rows, const QString& operationLabel, const QString& summary,
    ...
    for (const int row : rows) {
        ...
        if (!rowErrors.isEmpty()) {
            ...
            status->setBackground(settings_.theme == "Light" ? QColor("#fef2f2")
                                                             ...
                                                             : QColor("#3a1218"));
            detail->setText(rowErrors.join("; "));
            validationErrors << uiText("Row %1: %2", "第 %1 行：%2").arg(row + 1).arg(rowErrors.join("; "));
            continue;
        }

        if (!rowWarnings.isEmpty()) {
            validationWarnings << uiText("Row %1: %2", "第 %1 行：%2").arg(row + 1).arg(rowWarnings.join("; "));
        }
    ...
    if (!validationErrors.isEmpty()) {
        ...
        updateDiagnostics(
            "Error", "Startup SDO",
            uiText("%1 apply blocked: %2", "%1 应用已阻止：%2").arg(operationLabel).arg(validationErrors.join("; ")));
        QMessageBox::warning(this, uiText("Startup SDO Validation Failed", "Startup SDO 校验失败"),
                             validationErrors.join("\n"));
        return;
    }
    ...
    QStringList details = {
        uiText("Master: %1", "主站：%1").arg(activeMasterName()),
        uiText("%1 entries: %2", "%1 数量：%2").arg(operationLabel).arg(items.size()),
        summary,
    };
    updateStartupSdoWatchEvidence();
    ...
    // Safety gate: require explicit confirmation before bus write
    if (!confirmDangerousOperation(uiText("Confirm %1 Apply", "确认应用 %1").arg(operationLabel), summary, details,
                                   confirmText)) {
        return;
    }
    ...
    updateDiagnostics("Info", "Startup SDO",
                      QString("Apply requested for %1 %2 row(s)").arg(items.size()).arg(operationLabel));
    startupSdoTable_->resizeColumnsToContents(); // auto-fit column widths
    updateStartupSdoControls();
}
...
QVector<int> MainWindow::startupSdoRowsWithWatchDiffs() const {
    ...
    return ::startupRowsWithWatchDiffs(evaluateStartupWatchDeltas(watchStartupStartupRows(startupSdoTable_),
...
void MainWindow::applyStartupSdoWatchDiffRows() {
    ...
    applyStartupSdoRows(rows, uiText("Watch-diff Startup SDO", "Watch 偏差 Startup SDO"),
                        uiText("This operation writes only Startup SDO rows whose expected values "
...
void MainWindow::applyStartupSdoList() {
    ...
    if (!preflightStartupSdoList(false)) {
        QMessageBox::warning(this, uiText("Startup SDO Preflight Failed", "Startup SDO 预检查失败"),
                             uiText("Fix rows marked with Preflight Error before applying Startup SDO.",
                                    "请先修复标记为预检查错误的行，再应用 Startup SDO。"));
        return;
    }
    QStringList details = {
        uiText("Master: %1", "主站：%1").arg(activeMasterName()),
        uiText("Startup entries: %1", "启动项数量：%1").arg(startupSdoTable_->rowCount()),
        uiText("This applies the listed SDO writes in sequence.", "此操作会按顺序应用表格中的 SDO 写入。"),
    };
    QVector<int> rows;
    ...
    if (!confirmDangerousOperation(
            ...
            uiText("This operation writes multiple SDO values to the bus.", "此操作会向总线写入多条 SDO 参数。"),
    ...
    for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
        ...
        items.append(QJsonObject{
            {"row", row},
            {"position", startupSdoTable_->item(row, 0) ? startupSdoTable_->item(row, 0)->text().toInt() : 0},
            {"index", startupSdoTable_->item(row, 1) ? startupSdoTable_->item(row, 1)->text() : QString()},
            {"subIndex", startupSdoTable_->item(row, 2) ? startupSdoTable_->item(row, 2)->text() : QString()},
            {"value", startupSdoTable_->item(row, 3) ? startupSdoTable_->item(row, 3)->text() : QString()},
            {"type", startupSdoTable_->item(row, 4) ? startupSdoTable_->item(row, 4)->text() : QString()},
        });
    }
    client_.applyStartupSdos(items);
    updateDiagnostics("Info", "Startup SDO", QString("Apply requested for %1 startup entries").arg(items.size()));
}
