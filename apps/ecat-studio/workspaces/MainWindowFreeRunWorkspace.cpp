...
QStringList MainWindow::stateTransitionImpactDetails(int position,
                                                     const QString &requestedState) const {
  auto textFn = [this](const char *en, const char *zh) {
    return uiText(en, zh);
  };
  return ImpactAnalysisService::stateTransitionImpactDetails(
      position, requestedState,
      loadedSlaveInfoPosition_, loadedSdoPosition_, loadedPdoPosition_,
      identityTable_, sdo_->sdoTable, sdo_->pdoTable,
      watch_->watchTable, startupSdoTable_,
      freeRunWidgets_->freeRunEntryTable,
      topologyBaselineIssues(),
      consistencyGateDetails(uiText("state transition", "状态切换")),
      textFn);
}
...
QStringList MainWindow::freeRunImpactDetails() const {
  ...
  return ImpactAnalysisService::freeRunImpactDetails(
      selectedPosition(), slaves_,
      sdo_->pdoTable, freeRunWidgets_->freeRunEntryTable,
      watch_->watchTable, topologyBaselineIssues(),
      consistencyGateDetails(uiText("Free Run start", "启动自由运行")),
      textFn);
}
