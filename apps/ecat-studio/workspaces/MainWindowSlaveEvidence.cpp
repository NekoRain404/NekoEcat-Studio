...
void MainWindow::updateSlaveEvidenceMatrix() {
    ...
    if (slaveEvidence_->slaveEvidenceMatrixTable->currentRow() >= 0) {
        ...
        const int rowPosition =
            tableText(slaveEvidence_->slaveEvidenceMatrixTable, slaveEvidence_->slaveEvidenceMatrixTable->currentRow(),
    ...
    const SlaveEvidenceMatrix evidenceMatrix = buildSlaveEvidenceMatrix(evidenceInputs);
    ...
    setTableRows(slaveEvidence_->slaveEvidenceMatrixTable, slaveEvidenceMatrixHeaders(uiTexts), rows);
    ...
    int restoreRow = -1;
    for (int rowIndex = 0; rowIndex < slaveEvidence_->slaveEvidenceMatrixTable->rowCount(); ++rowIndex) {
        bool ok = false;
        const int rowPosition =
            tableText(slaveEvidence_->slaveEvidenceMatrixTable, rowIndex, kSlaveEvidenceMatrixPositionColumn)
                .toInt(&ok);
        ...
        const QColor readinessColor = evidence.risks.isEmpty() && evidence.readiness >= evidence.maxReadiness
                                          ? okColor
                                          : (!evidence.risks.isEmpty() ? warningColor : actionColor);
        const QString tooltip = uiRow.detailLines.join('\n');
        for (int column = 0; column < slaveEvidence_->slaveEvidenceMatrixTable->columnCount(); ++column) {
            if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, column)) {
                item->setToolTip(tooltip);
            }
        }
        if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixPriorityColumn)) {
            const int priorityRank = slaveEvidencePriorityRank(evidence.priority);
            item->setForeground(priorityRank <= 1 ? warningColor : priorityRank == 2 ? actionColor : okColor);
        }
        if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixStateColumn)) {
            const QString current = item->text().toUpper();
            item->setForeground((current == "OP" || current.startsWith("OP "))              ? okColor
                                : (current.contains("SAFEOP") || current.contains("PREOP")) ? actionColor
                                                                                            : infoColor);
        }
        if (auto* item =
                slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixReadinessColumn)) {
            item->setForeground(readinessColor);
        }
        for (int column : {5, 6, 7, 9}) {
            if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, column)) {
                const QString text = item->text().toLower();
                item->setForeground((text.contains("missing") || text.contains("缺失")) ? warningColor : okColor);
            }
        }
        if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixStartupColumn)) {
            item->setForeground(evidence.startupDiffs > 0 ? warningColor : okColor);
        }
        if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixRiskColumn)) {
            item->setForeground(evidence.risks.isEmpty() ? okColor : warningColor);
        }
        if (auto* item = slaveEvidence_->slaveEvidenceMatrixTable->item(rowIndex, kSlaveEvidenceMatrixNextColumn)) {
            item->setForeground(evidence.nextAction == SlaveEvidenceNextAction::Ready ? okColor : actionColor);
        }
    }

    if (restoreRow >= 0) {
        slaveEvidence_->slaveEvidenceMatrixTable->setCurrentCell(restoreRow, kSlaveEvidenceMatrixPositionColumn);
    }
    fitTableColumnsToViewport(slaveEvidence_->slaveEvidenceMatrixTable, kSlaveEvidenceMatrixNextColumn);

    if (slaveEvidence_->slaveEvidenceMatrixSummaryLabel) {
        const QString summary = slaves_.isEmpty() ? uiText("No slaves in current scan", "当前扫描没有从站")
        ...
        slaveEvidence_->slaveEvidenceMatrixSummaryLabel->setToolTip(
            uiText("This matrix is read-only and uses already loaded UI evidence only. "
        ...
        slaveEvidence_->slaveEvidenceMatrixSummaryLabel->setProperty(
            "severity", evidenceMatrix.riskRows > 0 ? "warning" : (evidenceMatrix.actionRows > 0 ? "action" : "ok"));
...
void MainWindow::filterSlaveEvidenceMatrix() {
    ...
    const QString needle = slaveEvidence_->slaveEvidenceMatrixFilter
                               ...
                               : QString();
    const QString scope = slaveEvidence_->slaveEvidenceMatrixScopeFilter
    ...
    if (slaveEvidence_->slaveEvidenceMatrixSummaryLabel) {
        const QString scopeLabel = slaveEvidence_->slaveEvidenceMatrixScopeFilter
...
void MainWindow::updateSlaveEvidenceMatrixTriageButtons() {
    ...
    const QString currentScope = slaveEvidence_->slaveEvidenceMatrixScopeFilter
                                     ? slaveEvidence_->slaveEvidenceMatrixScopeFilter->currentData().toString()
                                     : QString();
    for (auto* button : slaveEvidence_->slaveEvidenceMatrixTriageButtons) {
        ...
        button->setText(QString("%1 %2").arg(labels.value(scope, scope)).arg(count));
        ...
        button->setToolTip(uiText("Show %1 matrix rows. Count: %2. This is local UI filtering "
...
void MainWindow::reviewFirstSlaveEvidenceMatrixIssue() {
    ...
    statusBar()->showMessage(uiText("No visible matrix issue to review.", "当前没有可见的矩阵问题可审阅。"), 3000);
}
...
void MainWindow::reviewNextSlaveEvidenceMatrixIssue() {
    ...
    if (rowCount <= 0) {
        statusBar()->showMessage(uiText("No matrix issue to review.", "当前没有矩阵问题可审阅。"), 3000);
        return;
    }
    ...
    auto isIssueRow = [this](int row) {
        ...
        return slaveEvidenceMatrixRowState(slaveEvidence_->slaveEvidenceMatrixTable, row).reviewIssue;
    };
    ...
    statusBar()->showMessage(uiText("No visible matrix issue to review.", "当前没有可见的矩阵问题可审阅。"),
                             // ── Copy Digest ─────────────────────────────────────────────────────
                             3000);
}
...
bool MainWindow::copySlaveEvidenceMatrixRowDigest(int row) {
    if (!slaveEvidence_->slaveEvidenceMatrixTable || row < 0 ||
        ...
        statusBar()->showMessage(
            uiText("Select a visible slave evidence matrix row to copy.", "请选择一条可见的从站证据矩阵行再复制。"),
            3000);
        return false;
    }

    QStringList lines;
    lines << uiText("NekoEcat Studio Slave Evidence Matrix Row", "NekoEcat Studio 从站证据矩阵本行");
    lines << QString("%1: %2").arg(uiText("Master", "主站"), activeMasterName());
    if (slaveEvidence_->slaveEvidenceMatrixScopeFilter) {
        lines << QString("%1: %2").arg(uiText("Matrix Scope", "矩阵范围"),
                                       slaveEvidence_->slaveEvidenceMatrixScopeFilter->currentText());
    }
    if (slaveEvidence_->slaveEvidenceMatrixFilter &&
        !slaveEvidence_->slaveEvidenceMatrixFilter->text().trimmed().isEmpty()) {
        lines << QString("%1: %2").arg(uiText("Search", "搜索"),
                                       slaveEvidence_->slaveEvidenceMatrixFilter->text().trimmed());
    }
    if (slaveEvidence_->slaveEvidenceMatrixSummaryLabel &&
        !slaveEvidence_->slaveEvidenceMatrixSummaryLabel->text().trimmed().isEmpty()) {
        lines << QString("%1: %2").arg(uiText("Matrix Summary", "矩阵摘要"),
    ...
    for (int column = 0; column < slaveEvidence_->slaveEvidenceMatrixTable->columnCount(); ++column) {
        const auto* header = slaveEvidence_->slaveEvidenceMatrixTable->horizontalHeaderItem(column);
        const QString field = header ? header->text().trimmed() : QString::number(column + 1);
        const QString value = tableText(slaveEvidence_->slaveEvidenceMatrixTable, row, column);
        lines << QString("- %1: %2").arg(field, value.isEmpty() ? uiText("Empty", "空") : value);
    }
    ...
    updateDiagnostics(
        ...
        uiText("Copied slave evidence matrix row #%1 to clipboard", "已复制从站证据矩阵第 %1 行到剪贴板").arg(row + 1));
    statusBar()->showMessage(uiText("Copied slave evidence matrix row. No bus access was requested.",
                                    "已复制从站证据矩阵行；未请求总线访问。"),
                             // ── Open Row ────────────────────────────────────────────────────────
                             3000);
    return true;
...
void MainWindow::openSlaveEvidenceMatrixRow(int row) {
    if (!slaveEvidence_->slaveEvidenceMatrixTable || row < 0 ||
        row >= slaveEvidence_->slaveEvidenceMatrixTable->rowCount()) {
        statusBar()->showMessage(uiText("Select a slave evidence row first.", "请先选择一条从站证据矩阵行。"), 3000);
        return;
    }

    bool ok = false;
    const int position =
        tableText(slaveEvidence_->slaveEvidenceMatrixTable, row, kSlaveEvidenceMatrixPositionColumn).toInt(&ok);
    if (!ok || position < 0) {
        return;
    }

    if (!selectSlaveForLocalEvidence(position)) {
        statusBar()->showMessage(uiText("Slave is not present in the local topology tree.", "本地拓扑树中没有该从站。"),
                                 3000);
        return;
    }

    selectAndFocusTableRow(slaveEvidence_->slaveEvidenceMatrixTable, row, kSlaveEvidenceMatrixPositionColumn);
    ...
    switch (
        ...
        case SlaveEvidenceRouteTarget::ObjectDictionary:
            ...
            if (loadedSdoPosition_ == position && sdo_->sdoTable && sdo_->sdoTable->rowCount() > 0) {
                selectAndFocusTableRow(sdo_->sdoTable,
                                       sdo_->sdoTable->currentRow() >= 0 ? sdo_->sdoTable->currentRow() : 0, 0);
            }
        ...
        case SlaveEvidenceRouteTarget::PdoMap:
            ...
            if (loadedPdoPosition_ == position && sdo_->pdoTable && sdo_->pdoTable->rowCount() > 0) {
                selectAndFocusTableRow(sdo_->pdoTable,
                                       sdo_->pdoTable->currentRow() >= 0 ? sdo_->pdoTable->currentRow() : 0, 0);
            }
        ...
        case SlaveEvidenceRouteTarget::Watch: {
            ...
            filterWatchTable();
            if (watchRow >= 0) {
                selectAndFocusTableRow(watch_->watchTable, watchRow, kSlaveEvidenceWatchPositionColumn);
            } else {
                const int anyWatchRow =
                    firstSlaveEvidenceRowForPosition(watch_->watchTable, position, kSlaveEvidenceWatchPositionColumn);
                if (anyWatchRow >= 0) {
                    selectAndFocusTableRow(watch_->watchTable, anyWatchRow, kSlaveEvidenceWatchPositionColumn);
                }
            }
        ...
        case SlaveEvidenceRouteTarget::Startup: {
            ...
            const int startupRow = firstSlaveEvidenceStartupDiffRow(startupSdoTable_, position);
            ...
            if (startupRow >= 0) {
                selectAndFocusTableRow(startupSdoTable_, startupRow, kSlaveEvidenceStartupPositionColumn);
            }
        ...
        case SlaveEvidenceRouteTarget::Process: {
            const int processRow = firstSlaveEvidenceProcessIssueRow(freeRunWidgets_->freeRunEntryTable, position);
            ...
            if (processRow >= 0) {
                ...
                const int anyProcessRow = firstSlaveEvidenceRowForPosition(freeRunWidgets_->freeRunEntryTable, position,
                                                                           kSlaveEvidenceProcessPositionColumn);
                if (anyProcessRow >= 0) {
        ...
        case SlaveEvidenceRouteTarget::StateMachine: {
            ...
            const int stateRow = firstSlaveEvidenceRowForPosition(stateMachine_->stateMachineTable, position, 0);
            if (stateRow >= 0) {
    ...
    updateDiagnostics("Info", "Slave Evidence Matrix",
                      uiText("Opened %1 for slave #%2 from the read-only evidence matrix "
                          ...
                          .arg(position));
    statusBar()->showMessage(
        uiText("Local slave evidence opened. No bus access was requested.", "已打开本地从站证据；未请求总线访问。"),
        3000);
}
