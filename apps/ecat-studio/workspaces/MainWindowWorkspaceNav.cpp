...
WorkspaceBoundaryKind MainWindow::workspaceBoundaryKindForPage(const QWidget* page) const {
...
void MainWindow::recordWorkspaceHistory(int index) {
    if (!tabs_ || suppressWorkspaceHistory_ || index < 0 || index >= tabs_->count()) {
        return;
    }
...
void MainWindow::goWorkspaceForward() {
    ...
    activateWorkspacePage(target);
    suppressWorkspaceHistory_ = false;
    if (target && (workspaceBackStack_.isEmpty() || workspaceBackStack_.last() != target)) {
...
void MainWindow::updateActionAvailability() {
    ...
    const bool sdoLoadedForSelected =
        hasSlave && loadedSdoPosition_ == selectedPosition() && sdo_->sdoTable && sdo_->sdoTable->rowCount() > 0;
    const bool pdoLoadedForSelected =
        hasSlave && loadedPdoPosition_ == selectedPosition() && sdo_->pdoTable && sdo_->pdoTable->rowCount() > 0;
    const bool hasDictionarySelection = sdoLoadedForSelected && !selectedDictionaryRows().isEmpty();
    bool hasDictionaryValueSelection = false;
    if (sdoLoadedForSelected) {
        hasDictionaryValueSelection = sdoDictionaryRowsContainValue(sdo_->sdoTable, selectedDictionaryRows());
    }
    const bool hasObjectBookmarks = bookmark_->objectBookmarkTable && bookmark_->objectBookmarkTable->rowCount() > 0;
    const bool hasObjectBookmarkSelection = bookmark_->objectBookmarkTable && !selectedObjectBookmarkRows().isEmpty();
    const bool hasSdoTargetTrailSelection = sdoTargetTrailTable_ && sdoTargetTrailTable_->currentRow() >= 0;
    const bool hasSdoTargetTrailRows = sdoTargetTrailTable_ && sdoTargetTrailTable_->rowCount() > 0;
    const bool canCreateStartupFromSdoTargetTrail =
        hasSdoTargetTrailSelection && sdoTargetTrailRowCanCreateStartup(sdoTargetTrailTable_->currentRow());
    bool hasVisibleDictionaryRows = false;
    if (sdoLoadedForSelected) {
        hasVisibleDictionaryRows = !visibleTableRows(sdo_->sdoTable).isEmpty();
    }
    const bool hasPdoSelection = pdoLoadedForSelected && !selectedTableRows(sdo_->pdoTable).isEmpty();
    ...
    if (ioVar_->ioVariableTable) {
        ...
        hasIoVariableValueSelection = ioVariableTableRowsContainValue(ioVar_->ioVariableTable, selectedIoRows);
        const QVector<int> visibleIoRows = visibleIoVariableRows();
        hasVisibleIoVariables = !visibleIoRows.isEmpty();
        hasVisibleIoVariableValues = ioVariableTableRowsContainValue(ioVar_->ioVariableTable, visibleIoRows);
    }
    const bool hasHistorySelection =
        sdoHistoryTable_ && sdoHistoryTable_->rowCount() > 0 && !selectedSdoHistoryRows().isEmpty();
        ...
        for (const int row : rows) {
            if (row >= 0 && row < sdoHistoryTable_->rowCount() && !sdoHistoryTable_->isRowHidden(row)) {
                const QString status =
                    sdoHistoryTable_->item(row, 7) ? sdoHistoryTable_->item(row, 7)->text() : QString();
                const QString value =
                    sdoHistoryTable_->item(row, 6) ? sdoHistoryTable_->item(row, 6)->text() : QString();
    ...
    const bool hasHostCommand = hostHealthTable_ && hostHealthTable_->currentRow() >= 0 &&
                                hostHealthTable_->item(hostHealthTable_->currentRow(), 4) &&
                                !hostHealthTable_->item(hostHealthTable_->currentRow(), 4)->text().trimmed().isEmpty();
    ...
    setEnabled("readSelectedDictionary", connected && hasSlave && hasDictionarySelection);
    setEnabled("readVisibleDictionary", connected && hasSlave && hasVisibleDictionaryRows);
    setEnabled("readFailedDictionary", connected && hasSlave && hasFailedSdoEvidence());
    setEnabled("writeSdo", connected && hasSlave && selectedSdoWritable_);
    setEnabled("writeTargetSdo", connected && hasCurrentSdoTarget && selectedSdoWritable_);
    setEnabled("useSdoValue",
               selectedSdoWritable_ && sdoInspector_->sdoValue && !sdoInspector_->sdoValue->text().trimmed().isEmpty());
    setEnabled("useSdoEvidence", selectedSdoWritable_ && !currentSdoPreferredEvidenceValue().isEmpty());
    setEnabled("pickSdoEvidence", selectedSdoWritable_ && !currentSdoEvidenceCandidates().isEmpty());
    ...
    setEnabled("startupSelectedEvidence", hasSlave && hasDictionaryValueSelection);
    ...
    if (bookmark_->objectBookmarkTable) {
        bookmark_->objectBookmarkTable->setToolTip(
            uiText("%1 project object bookmark(s). Double-click fills the SDO target; "
    ...
    setEnabled("bulkNameIoVariables", hasIoVariableSelection || hasVisibleIoVariables);
    ...
    setEnabled("exportIoVariablesAction", ioVar_->ioVariableTable && ioVar_->ioVariableTable->rowCount() > 0);
    setEnabled("exportIoVariablesCsv", ioVar_->ioVariableTable && ioVar_->ioVariableTable->rowCount() > 0);
    setEnabled("exportIoPlcSymbolsAction", ioVar_->ioVariableTable && ioVar_->ioVariableTable->rowCount() > 0);
    setEnabled("exportIoPlcSymbolsCsv", ioVar_->ioVariableTable && ioVar_->ioVariableTable->rowCount() > 0);
    ...
    setEnabled("refreshWatch", connected && watch_->watchTable && watch_->watchTable->rowCount() > 0);
    setEnabled("captureWatchBaseline", watch_->watchTable && watch_->watchTable->rowCount() > 0);
