...
static int exportTableToCsv(QTableWidget* table, QTextStream& out, bool visibleOnly = true) {
...
void MainWindow::exportPdoMapCsv() {
    ...
    if (!table || table->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export PDO Map", "导出 PDO 映射"),
                                 uiText("No PDO entries are available to export.", "当前没有可导出的 PDO 条目。"));
        return;
    }
    const QString path =
        promptSavePath(this, uiText("Export PDO Map CSV", "导出 PDO 映射 CSV"),
                       QString("ethercat-pdo-map-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
                       "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportSdoDictionaryCsv() {
    ...
    if (!table || table->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export SDO Dictionary", "导出 SDO 字典"),
                                 uiText("No SDO entries are available to export.", "当前没有可导出的 SDO 条目。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export SDO Dictionary CSV", "导出 SDO 字典 CSV"),
        QString("ethercat-sdo-dictionary-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportSdoHistoryCsv() {
    if (!sdoHistoryTable_ || sdoHistoryTable_->rowCount() <= 0) {
        QMessageBox::information(
            this, uiText("Export SDO History", "导出 SDO 历史"),
            uiText("No SDO history entries are available to export.", "当前没有可导出的 SDO 历史记录。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export SDO History CSV", "导出 SDO 历史 CSV"),
        QString("ethercat-sdo-history-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportEsiRepositoryCsv() {
    if (!esiTable_ || esiTable_->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export ESI Repository", "导出 ESI 仓库"),
                                 uiText("No ESI entries are available to export.", "当前没有可导出的 ESI 条目。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export ESI Repository CSV", "导出 ESI 仓库 CSV"),
        QString("ethercat-esi-repository-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportEsiXml() {
    if (!rawText_ || !rawText_->xmlText || rawText_->xmlText->toPlainText().isEmpty()) {
        QMessageBox::information(this, uiText("Export ESI XML", "导出 ESI XML"),
                                 uiText("No ESI XML data is available to export.", "当前没有可导出的 ESI XML 数据。"));
        return;
    }
    const QString path =
        promptSavePath(this, uiText("Export ESI XML", "导出 ESI XML"),
                       QString("ethercat-esi-%1.xml").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
                       "XML (*.xml);;Text (*.txt)");
    if (path.isEmpty())
        return;
    ...
    updateDiagnostics("Info", "ESI XML", uiText("Exported ESI XML: %1", "已导出 ESI XML：%1").arg(path));
}
...
void MainWindow::exportWatchCsv() {
    ...
    if (!table || table->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export Watch", "导出监视"),
                                 uiText("No Watch items are available to export.", "当前没有可导出的监视项。"));
        return;
    }
    const QString path =
        promptSavePath(this, uiText("Export Watch CSV", "导出监视 CSV"),
                       QString("ethercat-watch-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
                       "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportStartupSdoCsv() {
    if (!startupSdoTable_ || startupSdoTable_->rowCount() <= 0) {
        QMessageBox::information(
            this, uiText("Export Startup SDO", "导出启动 SDO"),
            uiText("No Startup SDO entries are available to export.", "当前没有可导出的启动 SDO 条目。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export Startup SDO CSV", "导出启动 SDO CSV"),
        QString("ethercat-startup-sdo-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportTopologyCsv() {
    if (!topologyTree_ || topologyTree_->topLevelItemCount() == 0) {
        QMessageBox::information(this, uiText("Export Topology", "导出拓扑"),
                                 uiText("No topology data is available to export.", "当前没有可导出的拓扑数据。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export Topology CSV", "导出拓扑 CSV"),
        QString("ethercat-topology-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportHostHealthCsv() {
    if (!hostHealthTable_ || hostHealthTable_->rowCount() <= 0) {
        QMessageBox::information(
            this, uiText("Export Host Health", "导出主机健康"),
            uiText("No host health data is available to export.", "当前没有可导出的主机健康数据。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export Host Health CSV", "导出主机健康 CSV"),
        QString("ethercat-host-health-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty())
        return;
...
void MainWindow::exportPdoRawText() {
    if (!rawText_ || !rawText_->pdoText || rawText_->pdoText->toPlainText().isEmpty()) {
        QMessageBox::information(this, uiText("Export PDO Raw", "导出 PDO 原始输出"),
                                 uiText("No PDO raw data is available to export.", "当前没有可导出的 PDO 原始数据。"));
        return;
    }
    const QString path =
        promptSavePath(this, uiText("Export PDO Raw Text", "导出 PDO 原始输出"),
                       QString("ethercat-pdo-raw-%1.txt").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
                       "Text (*.txt);;All (*)");
    if (path.isEmpty())
        return;
    ...
    updateDiagnostics("Info", "PDO Raw", uiText("Exported PDO raw text: %1", "已导出 PDO 原始输出：%1").arg(path));
}
...
void MainWindow::exportSdoRawText() {
    if (!rawText_ || !rawText_->sdoText || rawText_->sdoText->toPlainText().isEmpty()) {
        QMessageBox::information(this, uiText("Export SDO Raw", "导出 SDO 原始输出"),
                                 uiText("No SDO raw data is available to export.", "当前没有可导出的 SDO 原始数据。"));
        return;
    }
    const QString path =
        promptSavePath(this, uiText("Export SDO Raw Text", "导出 SDO 原始输出"),
                       QString("ethercat-sdo-raw-%1.txt").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
                       "Text (*.txt);;All (*)");
    if (path.isEmpty())
        return;
    ...
    updateDiagnostics("Info", "SDO Raw", uiText("Exported SDO raw text: %1", "已导出 SDO 原始输出：%1").arg(path));
}
...
void MainWindow::exportMasterRawText() {
    if (!rawText_ || !rawText_->masterText || rawText_->masterText->toPlainText().isEmpty()) {
        QMessageBox::information(
            this, uiText("Export Master Raw", "导出主站原始输出"),
            uiText("No master raw data is available to export.", "当前没有可导出的主站原始数据。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export Master Raw Text", "导出主站原始输出"),
        QString("ethercat-master-raw-%1.txt").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "Text (*.txt);;All (*)");
    if (path.isEmpty())
        return;
    ...
    updateDiagnostics("Info", "Master Raw", uiText("Exported master raw text: %1", "已导出主站原始输出：%1").arg(path));
}
...
void MainWindow::exportSlaveRawText() {
    if (!rawText_ || !rawText_->infoText || rawText_->infoText->toPlainText().isEmpty()) {
        QMessageBox::information(this, uiText("Export Slave Raw", "导出从站原始输出"),
                                 uiText("No slave raw data is available to export.", "当前没有可导出的从站原始数据。"));
        return;
    }
    const QString path = promptSavePath(
        this, uiText("Export Slave Raw Text", "导出从站原始输出"),
        QString("ethercat-slave-raw-%1.txt").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
        "Text (*.txt);;All (*)");
    if (path.isEmpty())
        return;
    ...
    updateDiagnostics("Info", "Slave Raw", uiText("Exported slave raw text: %1", "已导出从站原始输出：%1").arg(path));
}
