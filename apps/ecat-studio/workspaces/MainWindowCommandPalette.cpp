...
void MainWindow::showCommandPalette() {
    ...
    search->setPlaceholderText(uiText("Search commands, workspaces, or masters", "搜索命令、工作区或主站"));
    ...
    safetyFilter->addItem(uiText("All action types  Alt+A", "全部操作类型  Alt+A"), QString());
    // Add command to palette list
    safetyFilter->addItem(uiText("Local navigation/data  Alt+L", "本地导航/数据  Alt+L"), QStringLiteral("local"));
    // Add command to palette list
    safetyFilter->addItem(uiText("Online read/runtime  Alt+O", "在线读取/运行  Alt+O"), QStringLiteral("online"));
    // Add command to palette list
    safetyFilter->addItem(uiText("Writes/state changes  Alt+D", "写入/状态切换  Alt+D"), QStringLiteral("danger"));
    // Add command to palette list
    safetyFilter->addItem(uiText("Host checks  Alt+H", "主机检查  Alt+H"), QStringLiteral("host"));
    // Add command to palette list
    safetyFilter->addItem(uiText("Project/file  Alt+F", "工程/文件  Alt+F"), QStringLiteral("file"));
    safetyFilter->setToolTip(uiText("Filter command types with Alt+A/L/O/D/H/F inside the palette",
    ...
    auto* commandPreview = new QLabel(uiText("Select a command to review its action type and safety boundary.",
                                             "选择命令以查看操作类型和安全边界。"));
    ...
    auto* commandStats =
        new QLabel(uiText("Type counts update with search and filter.", "类型计数会随搜索和过滤更新。"));
    ...
    auto actionByName = [this](const char* name) -> QAction* { return findChild<QAction*>(name); };
    auto actionCommand = [&](const char* name, const QString& title, const QString& subtitle) -> CommandItem {
    ...
    auto buttonCommand = [&](const char* name, const QString& title, const QString& subtitle,
    ...
    auto workspaceCommand = [&](QWidget* page, const QString& name, const QString& shortcut) -> CommandItem {
        const QString shortcutText = shortcut.isEmpty() ? QString() : QKeySequence(shortcut).toString();
        return CommandItem{
            uiText("Go to Workspace: ", "切换工作区：") + name,
            shortcutText.isEmpty()
                ? uiText("Activate workspace page without running online commands", "只切换工作区，不执行在线命令")
    ...
    auto currentSdoLabel = [this] {
        ...
        const QString object = !index.isEmpty() && !subIndex.isEmpty() ? QString("%1:%2").arg(index, subIndex)
    ...
    auto hasSdoTargetTrailRow = [this] { return sdoTargetTrailTable_ && sdoTargetTrailTable_->currentRow() >= 0; };
    auto hasWatchValueSelection = [this] { return selectedWatchRowsHaveValue(); };
    auto hasStartupWatchDiffs = [this] { return client_.isConnected() && !startupSdoRowsWithWatchDiffs().isEmpty(); };
    auto canReviewStartupWatchDiffs = [this] { return !startupSdoRowsWithWatchDiffs().isEmpty(); };
    auto canReviewFailedSdoEvidence = [this] { return hasFailedSdoEvidence(); };
    auto canRetryFailedSdoEvidence = [this] {
        return client_.isConnected() && selectedPosition() >= 0 && hasFailedSdoEvidence();
    };

    updateIoVariableTable();

    QVector<CommandItem> commands;
    commands.append(actionCommand("connectAction", uiText("Connect Runtime", "连接运行时"),
                                  uiText("Connect to ecatd", "连接到 ecatd")));
    commands.append(actionCommand("refreshAction", uiText("Refresh Online Data", "刷新在线数据"),
                                  uiText("Update master, slave, and telemetry data", "更新主站、从站和遥测数据")));
    commands.append(actionCommand("rescanAction", uiText("Rescan Bus", "重新扫描总线"),
                                  // ── Commissioning Workflow Commands ────────────────────────────────────
                                  uiText("Request a bus rescan on the active master", "请求当前主站重新扫描总线")));
    commands.append(CommandItem{
        uiText("Run Next Commissioning Step", "执行调试下一步"),
        uiText("Run the first actionable row in the Overview workflow", "执行总览工作流中第一条可执行步骤"),
    ...
    commands.append(CommandItem{
        ...
        uiText("Copy the selected Overview workflow step phase, status, risk, "
    ...
    const QVector<WorkflowScopeCommand> workflowScopeCommands = {
        {uiText("All", "全部"), "all", uiText("Show every commissioning workflow step", "显示全部调试工作流步骤")},
        {uiText("Open", "未完成"), "open",
         uiText("Show workflow steps that are not ready", "只显示尚未就绪的工作流步骤")},
        {uiText("Blocked", "受阻"), "blocked", uiText("Show blocked workflow steps", "只显示受阻工作流步骤")},
        {uiText("Action", "待执行"), "action", uiText("Show actionable workflow steps", "只显示待执行工作流步骤")},
        {uiText("Risk", "风险"), "risk", uiText("Show workflow steps with risk text", "只显示有风险文字的工作流步骤")},
        {uiText("Evidence Gap", "证据缺口"), "gap",
         uiText("Show workflow steps with missing or stale evidence", "只显示存在缺失或过期证据的工作流步骤")},
        {uiText("Ready", "就绪"), "ready", uiText("Show ready workflow steps", "只显示已就绪工作流步骤")},
    };
    for (const auto& scopeCommand : workflowScopeCommands) {
        ...
        commands.append(CommandItem{
            ...
            scopeCommand.description + uiText(" without bus access", "；不访问总线"),
    ...
    commands.append(CommandItem{
        ...
        [this] { return workflow_->workflowReviewButton && workflow_->workflowReviewButton->isEnabled(); },
    });
    commands.append(CommandItem{
        ...
        [this] { return workflow_->workflowReviewNextButton && workflow_->workflowReviewNextButton->isEnabled(); },
    });
    commands.append(CommandItem{
        uiText("Run Next Best Action", "执行下一最佳动作"),
        nextBestActionButton_ ? nextBestActionButton_->toolTip()
                              : uiText("Execute the current global navigator action", "执行当前全局导航推荐动作"),
        style()->standardIcon(QStyle::SP_CommandLink),
        [this] { runNextBestAction(); },
        [this] { return nextBestActionButton_ && nextBestActionButton_->isEnabled(); },
    });
    commands.append(CommandItem{
    ...
    commands.append(
        actionCommand("freeRunAction", uiText("Toggle Free Run", "切换自由运行"),
                      uiText("Start or stop cyclic process image telemetry", "启动或停止周期过程映像遥测")));
    commands.append(actionCommand("initAction", uiText("Set Selected Slave: INIT", "选中从站切换到 INIT"),
                                  uiText("Move the selected slave to INIT", "将选中从站切换到 INIT")));
    commands.append(actionCommand("preOpAction", uiText("Set Selected Slave: PREOP", "选中从站切换到 PREOP"),
                                  uiText("Move the selected slave to PREOP", "将选中从站切换到 PREOP")));
    commands.append(actionCommand("safeOpAction", uiText("Set Selected Slave: SAFEOP", "选中从站切换到 SAFEOP"),
                                  uiText("Move the selected slave to SAFEOP", "将选中从站切换到 SAFEOP")));
    commands.append(actionCommand("opAction", uiText("Set Selected Slave: OP", "选中从站切换到 OP"),
                                  uiText("Move the selected slave to OP", "将选中从站切换到 OP")));
    commands.append(actionCommand("allInitAction", uiText("Set All Slaves: INIT", "全部从站切换到 INIT"),
                                  uiText("Request INIT on every detected slave", "请求全部已检测从站进入 INIT")));
    commands.append(actionCommand("allPreOpAction", uiText("Set All Slaves: PREOP", "全部从站切换到 PREOP"),
                                  uiText("Request PREOP on every detected slave", "请求全部已检测从站进入 PREOP")));
    commands.append(actionCommand("allSafeOpAction", uiText("Set All Slaves: SAFEOP", "全部从站切换到 SAFEOP"),
                                  uiText("Request SAFEOP on every detected slave", "请求全部已检测从站进入 SAFEOP")));
    // ── Command Filtering ───────────────────────────────────────────────
    // Filter commands by search text and update the results list
    commands.append(actionCommand("allOpAction", uiText("Set All Slaves: OP", "全部从站切换到 OP"),
                                  uiText("Request OP on every detected slave", "请求全部已检测从站进入 OP")));
    commands.append(CommandItem{
        uiText("Read Current SDO", "读取当前 SDO"),
        uiText("Upload %1 using the current SDO fields", "使用当前 SDO 字段读取 %1").arg(currentSdoLabel()),
        style()->standardIcon(QStyle::SP_ArrowDown),
        [this] {
            if (auto* button = findChild<QPushButton*>("readSdo"); button && button->isEnabled()) {
                // ── SDO Commands ──────────────────────────────────────────────────────
                button->click();
            }
        },
    ...
    commands.append(buttonCommand(
        ...
        uiText("Download the current write value with validation and confirmation", "通过校验和确认后写入当前数值"),
        style()->standardIcon(QStyle::SP_ArrowUp)));
    commands.append(CommandItem{
        uiText("Use Read Value for Write", "使用读回值作为写入值"),
        uiText("Copy the current read-back value into the write field", "把当前读回值复制到写入框"),
        style()->standardIcon(QStyle::SP_ArrowForward),
    ...
    commands.append(CommandItem{
        ...
        [this] { return selectedSdoWritable_ && !currentSdoPreferredEvidenceValue().isEmpty(); },
    });
    commands.append(CommandItem{
        ...
        [this] { return selectedSdoWritable_ && !currentSdoEvidenceCandidates().isEmpty(); },
    });
    commands.append(CommandItem{
        uiText("Add Current SDO to Watch", "添加当前 SDO 到监视"),
        uiText("Track the selected slave/index/subindex", "监视当前选中的从站、索引和子项"),
    ...
    commands.append(CommandItem{
        ...
        style()->standardIcon(QStyle::SP_FileDialogContentsView),
        [this] {
            copySdoTargetPanelRowDigest(sdoInspector_->sdoTargetTable ? sdoInspector_->sdoTargetTable->currentRow()
                                                                      : -1);
        },
        hasSelectedObjectPanelRow,
    });
    ...
    commands.append(CommandItem{
        ...
        uiText("Refill the SDO fields from the selected local target trail row", "从所选本地目标轨迹行回填 SDO 字段"),
        style()->standardIcon(QStyle::SP_ArrowForward),
    ...
    commands.append(CommandItem{
        uiText("Add SDO Target Trail Row to Watch", "将 SDO 目标轨迹行加入 Watch"),
    ...
    commands.append(CommandItem{
        uiText("Create Startup SDO from Target Trail Row", "从目标轨迹行创建 Startup SDO"),
        ...
        [this, hasSdoTargetTrailRow] {
            return hasSdoTargetTrailRow() && sdoTargetTrailRowCanCreateStartup(sdoTargetTrailTable_->currentRow());
        },
    });
    commands.append(CommandItem{
        uiText("Open Current SDO Watch Evidence", "打开当前 SDO Watch 证据"),
        uiText("Open the matching Watch row without reading the bus", "打开匹配的 Watch 行，不读取总线"),
        style()->standardIcon(QStyle::SP_FileDialogDetailedView),
    ...
    commands.append(CommandItem{
        ...
        uiText("Open the matching Startup SDO row without writing the bus", "打开匹配的 Startup SDO 行，不写入总线"),
        style()->standardIcon(QStyle::SP_FileDialogDetailedView),
    ...
    commands.append(CommandItem{
        ...
        uiText("Open the matching Object Bookmark without bus access", "打开匹配的对象书签，不访问总线"),
        style()->standardIcon(QStyle::SP_FileDialogDetailedView),
    ...
    commands.append(CommandItem{
        ...
        uiText("Open the matching Target Trail row without bus access", "打开匹配的目标轨迹行，不访问总线"),
        style()->standardIcon(QStyle::SP_FileDialogDetailedView),
    ...
    commands.append(CommandItem{
        ...
        uiText("Save selected Object Dictionary rows as project bookmarks", "把选中的对象字典行保存为工程对象书签"),
        style()->standardIcon(QStyle::SP_FileDialogInfoView),
        [this] { addSelectedDictionaryRowsToBookmarks(); },
        [this] {
            return selectedPosition() >= 0 && loadedSdoPosition_ == selectedPosition() && sdo_->sdoTable &&
                   !selectedDictionaryRows().isEmpty();
        },
    });
    commands.append(CommandItem{
        uiText("Create Startup SDO from OD Evidence", "从 OD 证据创建 Startup SDO"),
        // ── Free Run Commands ─────────────────────────────────────────────────
        ...
        [this] {
            if (selectedPosition() < 0 || loadedSdoPosition_ != selectedPosition() || !sdo_->sdoTable) {
                return false;
            }
            return sdoDictionaryRowsContainValue(sdo_->sdoTable, selectedDictionaryRows());
        },
    });
    commands.append(CommandItem{
        ...
        [this] { return bookmark_->objectBookmarkTable && !selectedObjectBookmarkRows().isEmpty(); },
    });
    commands.append(CommandItem{
        uiText("Create Startup SDO from Object Bookmarks", "从对象书签创建 Startup SDO"),
    ...
    commands.append(CommandItem{
        ...
        [this] {
            if (!client_.isConnected() || selectedPosition() < 0 || loadedSdoPosition_ != selectedPosition() ||
                !sdo_->sdoTable) {
                return false;
            }
            for (int row = 0; row < sdo_->sdoTable->rowCount(); ++row) {
    ...
    commands.append(CommandItem{
        ...
        uiText("Add common drive objects to Watch for the selected slave", "把常见驱动对象加入选中从站的监视列表"),
        style()->standardIcon(QStyle::SP_DialogApplyButton),
    ...
    for (const auto& filterCommand : sdoFilterCommands) {
        commands.append(CommandItem{
            uiText("Filter Object Dictionary: ", "过滤对象字典：") + filterCommand.first,
            filterCommand.second.isEmpty()
                ? uiText("Clear Object Dictionary filter", "清除对象字典过滤")
                : uiText("Apply semantic Object Dictionary filter %1", "应用对象字典语义过滤 %1")
    ...
    const bool hasRecommendedCia402 =
        recommendedCia402Controlword(&recommendedLabel, &recommendedValue, &recommendedReason);
    commands.append(CommandItem{
        hasRecommendedCia402 ? uiText("Recommended CiA 402: ", "推荐 CiA 402：") + recommendedLabel
                             : uiText("Recommended CiA 402 Controlword", "推荐 CiA 402 控制字"),
        hasRecommendedCia402
            ? uiText("Based on statusword: %1. Write 0x6040:0x00 = %2", "基于状态字：%1。写入 0x6040:0x00 = %2")
        ...
        style()->standardIcon(QStyle::SP_DialogApplyButton),
        [this, recommendedLabel, recommendedValue] { prepareCia402Controlword(recommendedLabel, recommendedValue); },
        [this, hasRecommendedCia402] {
            return hasRecommendedCia402 && client_.isConnected() && selectedPosition() >= 0;
        },
    });
    for (const auto& command : cia402Controlwords) {
        commands.append(CommandItem{
            ...
            uiText("Prepare and write 0x6040:0x00 uint16 = %1 with confirmation",
                ...
                .arg(command.second),
            style()->standardIcon(QStyle::SP_CommandLink),
            [this, command] { prepareCia402Controlword(command.first, command.second); },
            [this] { return client_.isConnected() && selectedPosition() >= 0; },
        });
    }
    commands.append(CommandItem{
    ...
    commands.append(CommandItem{
        ...
        uiText("Copy %1 to the clipboard", "复制 %1 到剪贴板").arg(currentSdoLabel()),
        style()->standardIcon(QStyle::SP_FileDialogContentsView),
        [this, currentSdoLabel] {
            QApplication::clipboard()->setText(currentSdoLabel()); // copy to system clipboard
            updateDiagnostics("Info", "SDO", "Copied current SDO address: " + currentSdoLabel());
        },
    ...
    commands.append(CommandItem{
        ...
        uiText("Copy the current object with type and read/write value", "复制当前对象、类型和读回/写入值"),
        style()->standardIcon(QStyle::SP_FileDialogContentsView),
        [this, currentSdoValueLabel] {
            QApplication::clipboard()->setText(currentSdoValueLabel()); // copy to system clipboard
            updateDiagnostics("Info", "SDO", "Copied current SDO value: " + currentSdoValueLabel());
        },
        [hasCurrentSdoValue] { return hasCurrentSdoValue(); },
    });
    commands.append(CommandItem{
        uiText("Refresh Watch List", "刷新监视列表"),
        uiText("Read every SDO item in the Watch tab", "读取监视页中的每个 SDO 项"),
        style()->standardIcon(QStyle::SP_BrowserReload),
        [this] { refreshWatchList(); },
        [this] { return client_.isConnected() && watch_->watchTable && watch_->watchTable->rowCount() > 0; },
    });
    commands.append(CommandItem{
        uiText("Capture Watch Baseline", "捕获 Watch 基线"),
        uiText("Store current Watch values for drift comparison", "保存当前 Watch 值用于偏差比较"),
    ...
    commands.append(CommandItem{
        ...
        uiText("Clear saved baselines and delta results", "清除已保存基线和偏差结果"),
    ...
    const QVector<WatchScopeCommand> watchScopeCommands = {
        {uiText("All", "全部"), "all", uiText("Show every Watch row", "显示全部 Watch 行")},
        {uiText("Selected Slave", "当前从站"), "selected",
         uiText("Show only Watch rows for the selected slave", "只显示当前选中从站的 Watch 行")},
        {uiText("Changed", "变化项"), "changed",
         uiText("Show Watch rows whose value changed during the session", "只显示本会话中值发生变化的 Watch 行")},
        {uiText("Baseline Drift", "基线偏离"), "baselineDrift",
         uiText("Show Watch rows whose value differs from captured baseline", "只显示和已捕获基线不一致的 Watch 行")},
        ...
        {uiText("Missing Value", "缺失值"), "missingValue",
         uiText("Show Watch rows that have no live value yet", "只显示还没有实时值的 Watch 行")},
        {"CiA 402", "cia402", uiText("Show common CiA 402 drive Watch rows", "只显示常见 CiA 402 驱动 Watch 行")},
    };
    for (const auto& scopeCommand : watchScopeCommands) {
        commands.append(CommandItem{
            ...
            [this, scope = scopeCommand.scope, label = scopeCommand.label] {
                ...
                filterWatchTable();
                updateDiagnostics("Info", "Watch", uiText("Watch scope set to %1", "Watch 范围已切换为 %1").arg(label));
            },
    ...
    const QVector<IoScopeCommand> ioScopeCommands = {
        {uiText("All", "全部"), "all", uiText("Show every I/O variable row", "显示全部 I/O 变量")},
        {uiText("Selected Slave", "当前从站"), "selected",
         uiText("Show variables for the selected slave", "只显示当前从站变量")},
        {uiText("Process Image", "过程映像"), "process",
         uiText("Show variables with Free Run process-image evidence", "显示已有 Free Run 过程映像证据的变量")},
        {uiText("Startup Diff", "启动不一致"), "startupDiff",
         uiText("Show variables whose Startup expectation differs from live "
                "evidence",
                "显示 Startup 期望和实时证据不一致的变量")},
        {uiText("Missing Value", "缺失值"), "missingValue",
         uiText("Show variables without live or Watch value evidence", "显示缺少实时或 Watch 值证据的变量")},
        {"CiA 402", "cia402", uiText("Show common CiA 402 drive variables", "显示常见 CiA 402 驱动变量")},
        {uiText("Changed", "变化项"), "changed",
        ...
        {uiText("PLC Issues", "PLC 交接问题"), "plcIssues",
         uiText("Show variables whose PLC handoff symbol is not ready", "显示 PLC 交接符号尚未就绪的变量")},
    };
        ...
        commands.append(CommandItem{
            ...
            [this, scope = scopeCommand.scope, label = scopeCommand.label] {
                ...
                updateDiagnostics("Info", "I/O Variables",
                                  uiText("I/O Variables scope set to %1", "I/O 变量范围已切换为 %1").arg(label));
            },
            [this, scope = scopeCommand.scope] {
    ...
    commands.append(CommandItem{
        ...
        [this] { return ioVar_->ioVariableTable && ioVar_->ioVariableTable->currentRow() >= 0; },
    });
    commands.append(CommandItem{
    ...
    commands.append(CommandItem{
        uiText("Create Startup SDO from Selected I/O Variables", "从所选 I/O 变量创建 Startup SDO"),
        // ── Project Commands ──────────────────────────────────────────────────
    ...
    commands.append(CommandItem{
        uiText("Create Startup SDO from Visible I/O Variables", "从可见 I/O 变量创建 Startup SDO"),
    ...
    commands.append(CommandItem{
        ...
        [this] { return ioVar_->ioVariableTable && !selectedIoVariableRows(true).isEmpty(); },
    });
    commands.append(CommandItem{
    ...
    commands.append(CommandItem{
        ...
        [this] { return consistency_->consistencyTable && consistency_->consistencyTable->currentRow() >= 0; },
    });
    commands.append(CommandItem{
        ...
        [this] {
            activateWorkspaceTab(overviewTabIndex_);
            openSessionBriefRow(session_->sessionBriefTable ? session_->sessionBriefTable->currentRow() : -1);
        },
        [this] { return session_->sessionBriefTable && session_->sessionBriefTable->currentRow() >= 0; },
    });
    commands.append(CommandItem{
    ...
    commands.append(CommandItem{
        selectedRecommendedState.isEmpty()
            ...
            : uiText("Send Recommended EtherCAT State: %1", "发送推荐 EtherCAT 状态：%1").arg(selectedRecommendedState),
        selectedRecommendedState.isEmpty() ? uiText("No state recommendation is currently available for the "
                                                    "selected slave",
                                                    "当前选中从站暂无可发送的状态推荐")
        ...
        [this, stateCommandPosition, selectedRecommendedState] {
            requestSlaveStateWithConfirmation(stateCommandPosition, selectedRecommendedState);
        },
        [this, stateCommandPosition, selectedRecommendedState] {
            return client_.isConnected() && stateCommandPosition >= 0 && !selectedRecommendedState.isEmpty();
        },
    });
    commands.append(CommandItem{
        uiText("Run Host Check", "运行主机检查"),
        uiText("Validate kernel modules, EtherCAT config, NICs, blacklist, and DKMS",
    ...
    commands.append(workspaceCommand(overviewPage_, uiText("Overview", "总览"), "Ctrl+Alt+1"));
    commands.append(workspaceCommand(objectDictionaryPage_, uiText("Object Dictionary", "对象字典"), "Ctrl+Alt+2"));
    commands.append(workspaceCommand(pdoMapPage_, uiText("PDO Map", "PDO 映射"), "Ctrl+Alt+3"));
    commands.append(workspaceCommand(watchPage_, uiText("Watch", "监视"), "Ctrl+Alt+4"));
    commands.append(workspaceCommand(startupSdoPage_, uiText("Startup SDO", "启动 SDO"), "Ctrl+Alt+5"));
    commands.append(workspaceCommand(freeRunPage_, uiText("Free Run", "自由运行"), "Ctrl+Alt+6"));
    commands.append(workspaceCommand(ioVariablePage_, uiText("I/O Variables", "I/O 变量"), "Ctrl+Alt+7"));
    commands.append(workspaceCommand(consistencyPage_, uiText("Consistency", "一致性"), "Ctrl+Alt+8"));
    commands.append(workspaceCommand(stateMachinePage_, uiText("State Machine", "状态机"), "Ctrl+Alt+9"));
    commands.append(workspaceCommand(diagnosticsPage_, uiText("Diagnostics", "诊断"), "Ctrl+Alt+0"));
    commands.append(workspaceCommand(esiRepositoryPage_, uiText("ESI Repository", "ESI 仓库"), {}));
    commands.append(workspaceCommand(notesPage_, uiText("Notes", "备注"), {}));
    commands.append(workspaceCommand(esiXmlPage_, uiText("ESI XML", "ESI XML"), {}));
    commands.append(workspaceCommand(masterRawPage_, uiText("Master Raw", "主站原始输出"), {}));
    commands.append(workspaceCommand(slaveRawPage_, uiText("Slave Raw", "从站原始输出"), {}));
    commands.append(workspaceCommand(pdoRawPage_, uiText("PDO Raw", "PDO 原始输出"), {}));
    commands.append(workspaceCommand(sdoRawPage_, uiText("SDO Raw", "SDO 原始输出"), {}));
    commands.append(actionCommand("newProjectAction", uiText("New Project", "新建工程"),
                                  uiText("Create a clean project workspace", "创建干净工程工作区")));
    commands.append(actionCommand("openProjectAction", uiText("Open Project", "打开工程"),
                                  uiText("Load a NekoEcat Studio project", "加载 NekoEcat Studio 工程")));
    commands.append(actionCommand("saveProjectAction", uiText("Save Project", "保存工程"),
                                  uiText("Save the active project", "保存当前工程")));
    commands.append(actionCommand("saveProjectAsAction", uiText("Save Project As", "工程另存为"),
                                  uiText("Save the active project to a new file", "将当前工程保存为新文件")));
    commands.append(actionCommand("exportReportAction", uiText("Export Diagnostics Report", "导出诊断报告"),
                                  uiText("Write runtime diagnostics as Markdown", "将运行诊断导出为 Markdown")));
    commands.append(
        actionCommand("exportIoVariablesAction", uiText("Export I/O Variables CSV", "导出 I/O 变量 CSV"),
                      uiText("Write the current engineering signal table as CSV", "将当前工程信号表导出为 CSV")));
    commands.append(actionCommand(
        "exportIoPlcSymbolsAction", uiText("Export PLC Symbols CSV", "导出 PLC 符号 CSV"),
        uiText("Write visible I/O variables as PLC handoff records", "将可见 I/O 变量导出为 PLC 交接记录")));
    commands.append(actionCommand("reviewPlcHandoffAction", uiText("Review PLC Handoff Issues", "审阅 PLC 交接问题"),
                                  uiText("Open I/O Variables with the PLC Issues scope for handoff cleanup",
                                         "打开 I/O 变量并切到 PLC 交接问题范围，整理交接质量")));
    commands.append(actionCommand("importEsiAction", uiText("Import ESI XML", "导入 ESI XML"),
                                  uiText("Add device descriptions to the repository", "将设备描述加入仓库")));
    commands.append(
        actionCommand("settingsAction", uiText("Open Settings", "打开设置"),
                      uiText("Configure theme, language, scale, and masters", "配置主题、语言、缩放和主站")));
    commands.append(actionCommand(
        "manualAction", uiText("Open User Manual", "打开使用说明书"),
        uiText("Read detailed NekoEcat Studio operating instructions", "查看 NekoEcat Studio 的详细操作说明")));
    commands.append(actionCommand("aboutAction", uiText("About NekoEcat Studio", "关于 NekoEcat Studio"),
                                  uiText("Read version, architecture, capabilities, and safety information",
                                         "查看版本、架构、功能能力和安全说明")));
    commands.append(actionCommand("showLogAction", uiText("Show Runtime Log", "显示运行日志"),
    ...
    for (const auto& profile : settings_.masters) {
        ...
        commands.append(CommandItem{
            ...
            uiText("Make this master the active runtime target", "将此主站设为当前运行目标"),
            style()->standardIcon(QStyle::SP_ComputerIcon),
    ...
    QStringList recentCommandKeys = commandSettings.value("preferences/recentCommandKeys").toStringList();
    QStringList pinnedCommandKeys = commandSettings.value("preferences/pinnedCommandKeys").toStringList();
    ...
    auto rememberCommand = [&](const CommandItem& command) {
        ...
        commandSettings.setValue("preferences/recentCommandKeys", recentCommandKeys);
    };
    auto persistPinnedCommands = [&] {
        while (pinnedCommandKeys.size() > 24) {
            pinnedCommandKeys.removeLast();
        }
        commandSettings.setValue("preferences/pinnedCommandKeys", pinnedCommandKeys);
    };
    ...
    auto commandSafety = [this](const CommandItem& command) -> CommandSafety {
        const QString combined = QString("%1 %2").arg(command.title, command.subtitle).toLower();
        ...
        if (containsAny({"host check", "host health", "kernel modules", "dkms", "blacklist", "主机检查", "主机健康",
                         "内核模块", "黑名单"})) {
        ...
        if (containsAny({"write current sdo", "download the current write", "set selected slave", "set all slaves",
                         "send recommended ethercat state", "cia 402 controlword", "recommended cia 402",
                         "apply startup sdo watch diffs", "写入当前 sdo", "选中从站切换", "全部从站切换",
                         "发送推荐 ethercat 状态", "cia 402 控制字", "推荐 cia 402", "应用 startup sdo watch 偏差"})) {
        ...
        if (containsAny({"new project", "open project", "save project", "export diagnostics report", "export i/o",
                         "export plc", "export report", "import esi", "open user manual", "about ethercat studio",
                         "新建工程", "打开工程", "保存工程", "导出诊断报告", "导出 i/o", "导出 plc", "导入 esi",
                         "打开使用说明书", "关于 ethercat studio"})) {
        ...
        if (containsAny({"connect runtime", "refresh online data",
                         // ── Pin/Unpin Logic ───────────────────────────────────────────────────
                         "rescan bus", "read current sdo", "read visible object dictionary",
                         "retry failed object evidence", "refresh watch list", "prepare selected slave snapshot",
                         "toggle free run", "run next commissioning step", "run next best action",
                         "add current sdo to watch", "upload", "telemetry", "连接运行时", "刷新在线数据",
                         "重新扫描总线", "读取当前 sdo", "读取可见对象字典", "重试失败对象证据", "刷新监视列表",
                         "准备选中从站快照", "切换自由运行", "执行调试下一步", "执行下一最佳动作",
                         "添加当前 sdo 到监视", "遥测"})) {
    ...
    auto refill = [&] {
        ...
        for (int i = 0; i < commands.size(); ++i) {
            ...
            const bool matches = needle.isEmpty() || command.title.contains(needle, Qt::CaseInsensitive) ||
            ...
            const int pinnedRank = pinnedCommandKeys.indexOf(commandRecentKey(command));
            if (pinnedRank >= 0) {
                ++pinnedVisible;
            }
            const int recentRank = recentCommandKeys.indexOf(commandRecentKey(command));
            if (recentRank >= 0) {
                ++recentVisible;
            }
            displayRows.append(CommandDisplayRow{i, safety, isEnabled, pinnedRank, recentRank, i});
        }
        // Sort commands by relevance
        std::stable_sort(displayRows.begin(), displayRows.end(),
                         [](const CommandDisplayRow& left, const CommandDisplayRow& right) {
                             ...
                             if (leftPinned && rightPinned && left.pinnedRank != right.pinnedRank) {
                             ...
                             if (leftRecent && rightRecent && left.recentRank != right.recentRank) {
        ...
        for (const auto& row : displayRows) {
            ...
            item->setToolTip(QString("%1: %2\n%3\n%4")
                                 .arg(safety.label, safety.hint, command.subtitle,
                                      row.pinnedRank >= 0
                                          ? uiText("Pinned command. Alt+P unpins.", "已固定命令。Alt+P 取消固定。")
                                          : uiText("Alt+P pins this command.", "Alt+P 固定此命令。")));
            item->setFlags(isEnabled ? item->flags() : (item->flags() & ~Qt::ItemIsEnabled));
            if (isEnabled) {
                // Define color for visual feedback
                item->setForeground(QBrush(safety.color));
            }
            const QString pinnedPrefix = row.pinnedRank >= 0 ? uiText("[Pinned] ", "[固定] ") : QString();
            const QString recentPrefix = row.recentRank >= 0 ? uiText("[Recent] ", "[最近] ") : QString();
            item->setText(QString("%1%2[%3] %4\n%5")
                              .arg(pinnedPrefix, recentPrefix, safety.label, command.title, command.subtitle));
        ...
        QStringList parts;
        const QStringList order = {QStringLiteral("local"), QStringLiteral("online"), QStringLiteral("danger"),
                                   QStringLiteral("host"), QStringLiteral("file")};
        for (const QString& key : order) {
            if (visibleCounts.value(key) <= 0) {
                continue;
            }
            const QString label = safetyLabels.value(key);
            parts << QString("%1 %2/%3").arg(label).arg(enabledCounts.value(key)).arg(visibleCounts.value(key));
        }
        commandStats->setText(
            parts.isEmpty() ? uiText("No commands match the current search/filter.", "当前搜索/过滤没有匹配命令。")
    ...
    auto updatePreview = [&] {
        ...
        if (!item) {
            commandPreview->setText(uiText("No matching command. Adjust search text or action type filter.",
        ...
        commandPreview->setText(
            QString("%1: %2\n%3\n%4: %5")
                .arg(uiText("Type", "类型"), safetyLabel, safetyHint, uiText("Command", "命令"), command.subtitle) +
    ...
    auto runCurrent = [&] {
        ...
        if (index >= 0 && index < commands.size()) {
            ...
            updateDiagnostics((safety.key == "danger") ? "Warning" : "Info", "Command Palette",
                              uiText("Activated [%1] %2", "已触发 [%1] %2").arg(safety.label, commands[index].title));
            commands[index].run();
        }
    };
    ...
    connect(search, &QLineEdit::textChanged, &dialog,
            refill); // wire signal to slot
                     // Connect QComboBox::currentIndexChanged signal to handler
    connect(safetyFilter, QOverload<int>::of(&QComboBox::currentIndexChanged), &dialog, refill);
    // Connect QLineEdit::returnPressed signal to handler
    connect(search, &QLineEdit::returnPressed, &dialog,
            runCurrent); // wire signal to slot
                         // Connect QListWidget::currentRowChanged signal to handler
    connect(list, &QListWidget::currentRowChanged, &dialog, [&](int) { updatePreview(); });
    ...
    connect(list, &QListWidget::customContextMenuRequested, &dialog, [&](const QPoint& position) {
        ...
        const bool pinned = pinnedCommandKeys.contains(commandRecentKey(commands[index]));
        QMenu menu(&dialog);
        auto* pinAction =
            menu.addAction(pinned ? uiText("Unpin Command", "取消固定命令") : uiText("Pin Command", "固定命令"));
    ...
    connect(downShortcut, &QShortcut::activated, &dialog, [&] { moveSelection(1); });
    // Connect QShortcut::activated signal to handler
    connect(upShortcut, &QShortcut::activated, &dialog, [&] { moveSelection(-1); });
    // Connect QShortcut::activated signal to handler
    connect(pinShortcut, &QShortcut::activated, &dialog,
            toggleCurrentPinned); // wire signal to slot
                                  // Connect QShortcut::activated signal to handler
    connect(allFilterShortcut, &QShortcut::activated, &dialog, [setSafetyFilter] { setSafetyFilter(QString()); });
    // Connect QShortcut::activated signal to handler
