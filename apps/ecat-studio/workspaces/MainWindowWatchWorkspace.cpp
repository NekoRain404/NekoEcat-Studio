...
void MainWindow::ensureWatchTable() {
    ...
    watch_->watchTable->setHorizontalHeaderLabels(
        {uiText("Time", "时间"), uiText("Slave", "从站"), uiText("Index", "索引"), uiText("Sub", "子项"),
         uiText("Value", "值"), uiText("Decoded", "解析"), uiText("Type", "类型"), uiText("Mode", "模式"),
         uiText("Baseline", "基线"), uiText("Delta", "偏差"), uiText("Startup", "启动值"),
         uiText("Startup Delta", "启动偏差")});
}


// — Decode a raw Watch value using CiA 402 object semantics, if applicable
QString MainWindow::decodeWatchValue(const QString& index, const QString& subIndex, const QString& type,
                                     const QString& value, const QString& mode) const {
    Q_UNUSED(subIndex);
...
void MainWindow::addCurrentSdoToWatch(bool requestRead) {
    ...
    for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
        const bool match =
            (watch_->watchTable->item(row, 1) && watch_->watchTable->item(row, 1)->text().toInt() == position) &&
            (watch_->watchTable->item(row, 2) &&
             watch_->watchTable->item(row, 2)->text().compare(index, Qt::CaseInsensitive) == 0) &&
            (watch_->watchTable->item(row, 3) &&
             watch_->watchTable->item(row, 3)->text().compare(subIndex, Qt::CaseInsensitive) == 0);
        if (match) {
            watch_->watchTable->selectRow(row);
            const QString type = sdoInspector_->sdoType ? sdoInspector_->sdoType->currentText() : QString();
            if (!type.isEmpty() &&
                (!watch_->watchTable->item(row, 6) || watch_->watchTable->item(row, 6)->text().trimmed().isEmpty())) {
                // Create table cell
            ...
            if (requestRead && client_.isConnected()) {
                ...
                requestSdoRead(position, index, subIndex, uiText("Existing Watch item", "已有监视项"), type);
            }
    ...
    watch_->watchTable->setItem(row, 0,
                                // Create table cell
    ...
    watch_->watchTable->selectRow(row);
    updateDiagnostics("Info", "Watch", QString("Added SDO watch #%1 %2:%3").arg(position).arg(index, subIndex));
    if (requestRead && client_.isConnected()) {
        const QString type = sdoInspector_->sdoType ? sdoInspector_->sdoType->currentText() : QString();
        requestSdoRead(position, index, subIndex, uiText("Add to Watch", "加入监视"), type);
    }
    updateWatchAutoRefresh();
}
...
void MainWindow::addSelectedHistoryRowsToWatch() {
    ...
    for (const int row : rows) {
        ...
        int watchRow = -1;
        for (int existingRow = 0; existingRow < watch_->watchTable->rowCount(); ++existingRow) {
            const bool match =
                ...
                (watch_->watchTable->item(existingRow, 2) &&
                 watch_->watchTable->item(existingRow, 2)->text().compare(index, Qt::CaseInsensitive) == 0) &&
                (watch_->watchTable->item(existingRow, 3) &&
                 watch_->watchTable->item(existingRow, 3)->text().compare(subIndex, Qt::CaseInsensitive) == 0);
            if (match) {
        ...
        const QString effectiveValue = value.isEmpty() && watch_->watchTable->item(watchRow, 4)
                                           ? watch_->watchTable->item(watchRow, 4)->text().trimmed()
                                           : value;
        const QString decoded =
            decodeWatchValue(index, subIndex, type, effectiveValue, uiText("SDO History", "SDO 历史"));
        // Create table cell
        ...
        if (!status.isEmpty() && watch_->watchTable->item(watchRow, 7)) {
            watch_->watchTable->item(watchRow, 7)
                ->setToolTip(detail.isEmpty() ? status : QString("%1 - %2").arg(status, detail));
        }
    ...
    for (const int row : rows) {
...

        {
            ...
            if (sdoInspector_->sdoSubIndex) {
                sdoInspector_->sdoSubIndex->setText(QString("0x%1").arg(subIndex.rightJustified(2, '0')));
            }
        ...
        if (watchRow >= 0) {
            const QString mode =
                textAt(0).contains("Rx", Qt::CaseInsensitive) || textAt(1).contains("RxPDO", Qt::CaseInsensitive)
                    ? "RxPDO"
                    : (textAt(0).contains("Tx", Qt::CaseInsensitive) || textAt(1).contains("TxPDO", Qt::CaseInsensitive)
                           ? "TxPDO"
                           : "PDO");
            // Create table cell
...
void MainWindow::addCia402WatchPreset() {
    ...
    const QVector<PresetObject> objects = {
        {"0x6040", "0x00", "uint16", "Controlword"},          {"0x6041", "0x00", "uint16", "Statusword"},
        {"0x6060", "0x00", "int8", "Mode of operation"},      {"0x6061", "0x00", "int8", "Mode display"},
        {"0x603f", "0x00", "uint16", "Error code"},           {"0x6064", "0x00", "int32", "Position actual value"},
        {"0x606c", "0x00", "int32", "Velocity actual value"}, {"0x6077", "0x00", "int16", "Torque actual value"},
        {"0x607a", "0x00", "int32", "Target position"},       {"0x60ff", "0x00", "int32", "Target velocity"},
    ...
    updateDiagnostics(
        "Info", "Watch",
        QString("CiA 402 watch preset added for slave #%1: %2 new item(s)").arg(position).arg(qMax(0, after - before)));
    activateWorkspaceTab(watchTabIndex_);
}
...
void MainWindow::refreshWatchList(bool quiet) {
    ...
    for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
        ...
        const QString type =
            watch_->watchTable->item(row, 6) ? watch_->watchTable->item(row, 6)->text().trimmed() : QString();
    ...
    if (!quiet) {
        updateDiagnostics("Info", "Watch", QString("Refresh requested for %1 SDO watch item(s)").arg(requested));
    }
    updateWatchAutoRefresh();
}
...
void MainWindow::captureWatchBaseline() {
    ...
    filterWatchTable();
    updateActionAvailability();
    updateDiagnostics("Info", "Watch", QString("Captured Watch baseline for %1 row(s)").arg(captured));
}
...
void MainWindow::clearWatchBaseline() {
    ...
    int cleared = 0;
    for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
        if (watch_->watchTable->item(row, 8) && !watch_->watchTable->item(row, 8)->text().trimmed().isEmpty()) {
            ++cleared;
        }
        // Create table cell
        watch_->watchTable->setItem(row, 8, new QTableWidgetItem);
    ...
    filterWatchTable();
    updateActionAvailability();
    updateDiagnostics("Info", "Watch", QString("Cleared Watch baseline for %1 row(s)").arg(cleared));
}
...
void MainWindow::updateWatchBaselineDelta(int row) {
    ...
    const QString baseline =
        watch_->watchTable->item(row, 8) ? watch_->watchTable->item(row, 8)->text().trimmed() : QString();
    ...
    const bool numeric = parseNumber(value, &numericValue) && parseNumber(baseline, &numericBaseline);
    ...
    if (numeric) {
        ...
        deltaItem->setText(effectivelyZero ? QString("0") : QString::number(delta, 'g', 12));
    } else {
        deltaItem->setText(same ? uiText("match", "匹配") : uiText("changed", "已变化"));
    }

    const bool changed = numeric ? std::abs(numericValue - numericBaseline) >= 1e-12 : !same;
    if (changed) {
        // Define color for visual feedback
...
void MainWindow::updateWatchStartupDelta(int row) {
    ...
    auto* expectedItem = ensureWatchStartupTableItem(watch_->watchTable, row, kWatchStartupWatchExpectedColumn);
    auto* deltaItem = ensureWatchStartupTableItem(watch_->watchTable, row, kWatchStartupWatchDeltaColumn);
    clearWatchStartupDeltaCells(expectedItem, deltaItem);

    if (!startupSdoTable_ || !watchStartupHasTarget(watchRow.position, watchRow.index, watchRow.subIndex)) {
        return;
    }

    const WatchStartupWatchMatch match =
        watchStartupMatchForWatchRow(watchStartupStartupRows(startupSdoTable_), watchRow);
    if (match.state == WatchStartupDeltaState::NoStartup || match.state == WatchStartupDeltaState::NoTarget) {
        return;
    }

    expectedItem->setText(match.matchingStartupRows > 1 ? uiText("%1 (%2 startup rows)", "%1（%2 个启动项）")
                                                              .arg(match.expectedValue)
                                                              .arg(match.matchingStartupRows)
                                                        : match.expectedValue);
    if (!match.expectedType.isEmpty()) {
        expectedItem->setToolTip(uiText("Startup type: %1", "启动类型：%1").arg(match.expectedType));
    }

    applyWatchStartupDeltaCells(nullptr, deltaItem, match.state, QString(), watchStartupDeltaTexts(),
