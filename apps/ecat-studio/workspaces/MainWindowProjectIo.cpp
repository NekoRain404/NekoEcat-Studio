...
void MainWindow::newProject() {
    ...
    clearOnlineViews();
    setWindowTitle(uiText("NekoEcat Studio - Untitled", "NekoEcat Studio - 未命名"));
...
void MainWindow::openProject() {
    const QString path = QFileDialog::getOpenFileName(this, uiText("Open Project", "打开工程"), QDir::homePath(),
...
void MainWindow::saveProjectAs() {
    const QString path = QFileDialog::getSaveFileName(this, uiText("Save Project As", "工程另存为"),
...
bool MainWindow::writeProjectFile(const QString& path) {
    ...
    for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
        ...
        for (int column = 0; column < watch_->watchTable->columnCount(); ++column) {
            rowArray.append(watch_->watchTable->item(row, column) ? watch_->watchTable->item(row, column)->text()
                                                                  : QString());
        }
    ...
    if (sdoTargetTrailTable_) {
        ...
        for (int row = 0; row < sdoTargetTrailTable_->rowCount(); ++row) {
            ...
            if (trail.index.trimmed().isEmpty() || trail.subIndex.trimmed().isEmpty()) {
                continue;
            }
            // Serialize/deserialize JSON data
    ...
    if (bookmark_->objectBookmarkTable) {
        for (int row = 0; row < bookmark_->objectBookmarkTable->rowCount(); ++row) {
            ...
            if (bookmark.index.trimmed().isEmpty() || bookmark.subIndex.trimmed().isEmpty()) {
                continue;
            }
            // Serialize/deserialize JSON data
    ...
    for (const QString& key : ioVariableKeys) {
        ...
        if (parts.size() != 3 || (metadata.value(0).trimmed().isEmpty() && metadata.value(1).trimmed().isEmpty() &&
...
bool MainWindow::readProjectFile(const QString& path) {
    ...
    if (root.contains("activeMaster")) {
        settings_.activeMaster = root.value("activeMaster").toString(settings_.activeMaster).trimmed();
    ...
    if (savedPosition >= 0 && topologyTree_) {
        ...
        for (int top = 0; top < topologyTree_->topLevelItemCount() && !restoredSelection; ++top) {
            ...
            for (int child = 0; child < masterItem->childCount(); ++child) {
                ...
                if (slaveItem && slaveItem->data(0, Qt::UserRole).toInt() == savedPosition) {
    ...
    const auto snapshotPosition = [&snapshots, savedPosition](const char* key) {
        return snapshots.contains(QLatin1String(key)) ? snapshots.value(QLatin1String(key)).toInt(-1) : savedPosition;
    };
    ...
    loadedSlaveInfoPosition_ = !lastSlaveInfoText_.isEmpty() && slaveInfoSnapshotPosition == currentSelectedPosition
                                   ? slaveInfoSnapshotPosition
                                   : -1;
    loadedPdoPosition_ =
        !lastPdoText_.isEmpty() && pdoSnapshotPosition == currentSelectedPosition ? pdoSnapshotPosition : -1;
    loadedSdoPosition_ =
        !lastSdoText_.isEmpty() && sdoSnapshotPosition == currentSelectedPosition ? sdoSnapshotPosition : -1;
    loadedXmlPosition_ =
        !lastXmlText_.isEmpty() && xmlSnapshotPosition == currentSelectedPosition ? xmlSnapshotPosition : -1;
    sdoEvidence_.clear();
    for (const auto& value : root.value("sdoEvidence").toArray()) {
        ...
        sdoEvidence_.insert(key, {object.value("value").toString(), object.value("status").toString(),
                                  object.value("detail").toString(), object.value("time").toString()});
    }
    ioVariableMetadata_.clear();
    for (const auto& value : root.value("ioVariables").toArray()) {
        ...
        const QString key =
            ioVariableTableObjectKey(position, object.value("index").toString(), object.value("subIndex").toString());
        ...
        if (position >= 0 && !key.isEmpty() && (!alias.isEmpty() || !tags.isEmpty() || !note.isEmpty())) {
    ...
    if (bookmark_->objectBookmarkTable) {
        ...
        for (const auto& value : bookmarkArray) {
            ...
            addObjectBookmark(object.value("position").toInt(-1), index, subIndex, object.value("access").toString(),
                              object.value("type").toString(), object.value("bits").toString(),
                              object.value("name").toString(), object.value("lastValue").toString(),
    ...
    if (sdoTargetTrailTable_) {
        ...
        for (const auto& value : trailArray) {
            ...
            rememberedSdoTargetTrailKeys_.insert(sdoTargetTrailRowKeyFromTable(sdoTargetTrailTable_, targetRow));
            ++targetRow;
        }
    ...
    if (!startupArray.isEmpty()) {
        ...
        for (int row = 0; row < startupArray.size(); ++row) {
            ...
            startupSdoTable_->setItem(row, 0,
                                      // Create table cell
                                      new QTableWidgetItem(QString::number(item.value("position").toInt())));
    ...
    projectPath_ = path;
    projectName_ = root.value("name").toString(QFileInfo(path).completeBaseName());
    setWindowTitle(QString("NekoEcat Studio - %1").arg(projectName_));
...
void MainWindow::importEsiFiles() {
    const QStringList files =
        QFileDialog::getOpenFileNames(this, uiText("Import ESI XML", "导入 ESI XML"), QDir::homePath(),
    ...
    updateDiagnostics("Info", "ESI", QString("Imported %1 ESI file(s)").arg(files.size()));
...
void MainWindow::refreshEsiRepository() {
    ...
    for (const auto& path : repository) {
        ...
        if (file.open(QIODevice::ReadOnly | QIODevice::Text)) {
            ...
            while (!xml.atEnd()) {
                ...
                if (n == "Vendor" && vendor.isEmpty()) {
                    while (!xml.atEnd() && !(xml.isEndElement() && xml.name() == QStringLiteral("Vendor"))) {
                        xml.readNext();
        ...
        rows.append({QFileInfo(path).fileName(), vendor, product, revision, type, name, path});
    }
    setTableRows(esiTable_, {"File", "Vendor", "Product", "Revision", "Type", "Name", "Path"}, rows);
}
