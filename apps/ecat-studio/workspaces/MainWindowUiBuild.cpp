...
void MainWindow::buildUi() {
    ...
    setWindowTitle(projectName_ == "Untitled" ? uiText("NekoEcat Studio - Untitled", "NekoEcat Studio - 未命名")
                                              : QString("NekoEcat Studio - %1").arg(projectName_));

    auto* fileMenu = menuBar()->addMenu(uiText("Project", "工程"));
    auto* newProjectAction =
        fileMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogNewFolder), uiText("New Project", "新建工程"));
    newProjectAction->setObjectName("newProjectAction");
    newProjectAction->setShortcut(QKeySequence::New);
    auto* openProjectAction =
        fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogOpenButton), uiText("Open Project", "打开工程"));
    openProjectAction->setObjectName("openProjectAction");
    openProjectAction->setShortcut(QKeySequence::Open);
    auto* saveProjectAction =
        fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogSaveButton), uiText("Save Project", "保存工程"));
    saveProjectAction->setObjectName("saveProjectAction");
    saveProjectAction->setShortcut(QKeySequence::Save);
    auto* saveProjectAsAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogSaveButton),
    ...
    recentProjectsMenu_ = fileMenu->addMenu(style()->standardIcon(QStyle::SP_FileDialogListView),
                                            uiText("Recent Projects", "最近使用的工程"));
    recentProjectsMenu_->setObjectName("recentProjectsMenu");
    updateRecentProjectsMenu();
    fileMenu->addSeparator();
    auto* exportAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogDetailedView),
                                             uiText("Export Diagnostics Report", "导出诊断报告"));
    exportAction->setObjectName("exportReportAction");
    auto* exportIoVariablesAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogSaveButton),
                                                        uiText("Export I/O Variables CSV", "导出 I/O 变量 CSV"));
    exportIoVariablesAction->setObjectName("exportIoVariablesAction");
    auto* exportIoPlcAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogSaveButton),
                                                  uiText("Export PLC Symbols CSV", "导出 PLC 符号 CSV"));
    exportIoPlcAction->setObjectName("exportIoPlcSymbolsAction");
    auto* exportPlcDeclarationsAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogSaveButton),
                                                            uiText("Export PLC Declarations ST", "导出 PLC 声明 ST"));
    exportPlcDeclarationsAction->setObjectName("exportPlcDeclarationsAction");
    auto* reviewPlcHandoffAction = fileMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogDetailedView),
    ...
    fileMenu->addSeparator();
    auto* dataExportMenu =
        fileMenu->addMenu(style()->standardIcon(QStyle::SP_DialogSaveButton), uiText("Export Data...", "导出数据..."));

    auto* exportPdoAction = dataExportMenu->addAction(uiText("PDO Map CSV", "PDO 映射 CSV"));
    exportPdoAction->setObjectName("exportPdoMapAction");

    auto* exportSdoDictAction = dataExportMenu->addAction(uiText("SDO Dictionary CSV", "SDO 字典 CSV"));
    exportSdoDictAction->setObjectName("exportSdoDictionaryAction");

    auto* exportSdoHistAction = dataExportMenu->addAction(uiText("SDO History CSV", "SDO 历史 CSV"));
    exportSdoHistAction->setObjectName("exportSdoHistoryAction");

    auto* exportWatchAction = dataExportMenu->addAction(uiText("Watch CSV", "监视 CSV"));
    exportWatchAction->setObjectName("exportWatchAction");

    auto* exportStartupAction = dataExportMenu->addAction(uiText("Startup SDO CSV", "启动 SDO CSV"));
    exportStartupAction->setObjectName("exportStartupSdoAction");

    dataExportMenu->addSeparator();

    auto* exportEsiAction = dataExportMenu->addAction(uiText("ESI Repository CSV", "ESI 仓库 CSV"));
    exportEsiAction->setObjectName("exportEsiRepositoryAction");

    auto* exportEsiXmlAction = dataExportMenu->addAction(uiText("ESI XML", "ESI XML"));
    exportEsiXmlAction->setObjectName("exportEsiXmlAction");

    dataExportMenu->addSeparator();

    auto* exportTopoAction = dataExportMenu->addAction(uiText("Topology CSV", "拓扑 CSV"));
    exportTopoAction->setObjectName("exportTopologyAction");

    auto* exportHealthAction = dataExportMenu->addAction(uiText("Host Health CSV", "主机健康 CSV"));
    exportHealthAction->setObjectName("exportHostHealthAction");

    /* ── Raw text exports ─────────────────────────────────────── */
    dataExportMenu->addSeparator();
    auto* exportPdoRawAction = dataExportMenu->addAction(uiText("PDO Raw Text", "PDO 原始输出"));
    exportPdoRawAction->setObjectName("exportPdoRawAction");

    auto* exportSdoRawAction = dataExportMenu->addAction(uiText("SDO Raw Text", "SDO 原始输出"));
    exportSdoRawAction->setObjectName("exportSdoRawAction");

    auto* exportMasterRawAction = dataExportMenu->addAction(uiText("Master Raw Text", "主站原始输出"));
    exportMasterRawAction->setObjectName("exportMasterRawAction");

    auto* exportSlaveRawAction = dataExportMenu->addAction(uiText("Slave Raw Text", "从站原始输出"));
    exportSlaveRawAction->setObjectName("exportSlaveRawAction");

    fileMenu->addSeparator();
    fileMenu->addAction(style()->standardIcon(QStyle::SP_DialogCloseButton), uiText("Exit", "退出"), this,
                        &QWidget::close);

    auto* onlineMenu = menuBar()->addMenu(uiText("Online", "在线"));
    auto* menuConnectAction =
        onlineMenu->addAction(style()->standardIcon(QStyle::SP_DriveNetIcon), uiText("Connect", "连接"));
    menuConnectAction->setObjectName("menuConnectAction");
    menuConnectAction->setShortcut(QKeySequence("Ctrl+K"));
    auto* menuRefreshAction =
        onlineMenu->addAction(style()->standardIcon(QStyle::SP_BrowserReload), uiText("Refresh", "刷新"));
    menuRefreshAction->setObjectName("menuRefreshAction");
    menuRefreshAction->setShortcut(QKeySequence::Refresh);
    auto* menuRescanAction =
        onlineMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogDetailedView), uiText("Rescan", "重新扫描"));
    ...
    auto* allInitAction = onlineMenu->addAction(uiText("Set All Slaves to INIT", "全部从站切换到 INIT"));
    allInitAction->setObjectName("allInitAction");
    auto* allPreOpAction = onlineMenu->addAction(uiText("Set All Slaves to PREOP", "全部从站切换到 PREOP"));
    allPreOpAction->setObjectName("allPreOpAction");
    auto* allSafeOpAction = onlineMenu->addAction(uiText("Set All Slaves to SAFEOP", "全部从站切换到 SAFEOP"));
    allSafeOpAction->setObjectName("allSafeOpAction");
    auto* allOpAction = onlineMenu->addAction(uiText("Set All Slaves to OP", "全部从站切换到 OP"));
    allOpAction->setObjectName("allOpAction");

    auto* viewMenu = menuBar()->addMenu(uiText("View", "视图"));
    auto* showLogAction = viewMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogInfoView),
    ...
    auto* workspaceBackAction =
        viewMenu->addAction(style()->standardIcon(QStyle::SP_ArrowBack), uiText("Back Workspace", "后退工作区"));
    ...
    workspaceBackAction->setStatusTip(
        uiText("Return to the previous workspace without running online commands", "返回上一个工作区，不执行在线命令"));
    auto* workspaceForwardAction =
        viewMenu->addAction(style()->standardIcon(QStyle::SP_ArrowForward), uiText("Forward Workspace", "前进工作区"));
    workspaceForwardAction->setObjectName("workspaceForwardAction");
    workspaceForwardAction->setShortcut(QKeySequence("Alt+Right"));
    workspaceForwardAction->setStatusTip(uiText("Move forward in workspace history without running online commands",
                                                "前进到下一个工作区，不执行在线命令"));
    viewMenu->addSeparator();
    auto* workspaceMenu =
        viewMenu->addMenu(style()->standardIcon(QStyle::SP_FileDialogListView), uiText("Workspaces", "工作区"));
    ...
    const QList<WorkspaceActionSpec> workspaceActions = {
        ...
        {uiText("Object Dictionary", "对象字典"), "goObjectDictionaryAction", "Ctrl+Alt+2"},
        ...
        {uiText("I/O Variables", "I/O 变量"), "goIoVariablesAction", "Ctrl+Alt+7"},
    ...
    for (const auto& entry : workspaceActions) {
        auto* action = workspaceMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogListView), entry.text);
        action->setObjectName(entry.objectName);
        if (!entry.shortcut.isEmpty()) {
            action->setShortcut(QKeySequence(entry.shortcut));
        }
        action->setStatusTip(uiText("Switch workspace without running online commands", "切换工作区，不执行在线命令"));
    }

    auto* toolsMenu = menuBar()->addMenu(uiText("Tools", "工具"));
    auto* commandPaletteAction = toolsMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogContentsView),
    ...
    auto* importEsiAction = toolsMenu->addAction(style()->standardIcon(QStyle::SP_DialogOpenButton),
                                                 uiText("Import ESI XML", "导入 ESI XML"));
    importEsiAction->setObjectName("importEsiAction");
    auto* settingsAction =
        toolsMenu->addAction(style()->standardIcon(QStyle::SP_ComputerIcon), uiText("Settings", "设置"));
    settingsAction->setObjectName("settingsAction");
    settingsAction->setShortcut(QKeySequence::Preferences);

    auto* helpMenu = menuBar()->addMenu(uiText("Help", "帮助"));
    auto* manualAction =
        helpMenu->addAction(style()->standardIcon(QStyle::SP_FileDialogInfoView), uiText("User Manual", "使用说明书"));
    manualAction->setObjectName("manualAction");
    manualAction->setShortcut(QKeySequence::HelpContents);
    helpMenu->addSeparator();
    auto* aboutAction = helpMenu->addAction(style()->standardIcon(QStyle::SP_MessageBoxInformation),
    ...
    auto* commandPaletteToolbarAction =
        toolbar->addAction(style()->standardIcon(QStyle::SP_FileDialogContentsView), uiText("Commands", "命令"));
    commandPaletteToolbarAction->setObjectName("commandPaletteToolbarAction");
    commandPaletteToolbarAction->setToolTip(uiText("Open command palette (Ctrl+P)", "打开命令面板 (Ctrl+P)"));
    ...
    const QList<ToolbarActionSpec> actions = {
        {uiText("Connect", "连接"), "connectAction", QStyle::SP_DriveNetIcon,
         uiText("Connect to the EtherCAT runtime", "连接 EtherCAT 运行时"), QKeySequence("Ctrl+K")},
        {uiText("Refresh", "刷新"), "refreshAction", QStyle::SP_BrowserReload,
         uiText("Refresh master and slave data", "刷新主站和从站数据"), QKeySequence::Refresh},
        {uiText("Rescan", "重新扫描"), "rescanAction", QStyle::SP_FileDialogDetailedView,
         uiText("Rescan the active master bus", "重新扫描当前主站总线"), QKeySequence("Ctrl+Shift+R")},
        {uiText("Free Run", "自由运行"), "freeRunAction", QStyle::SP_MediaPlay,
         uiText("Toggle cyclic Free Run telemetry", "切换周期性自由运行遥测"), QKeySequence("Ctrl+F")},
        {"INIT", "initAction", QStyle::SP_DialogResetButton,
         uiText("Set selected slave to INIT", "将选中从站切换到 INIT"), QKeySequence("Alt+1")},
        {"PREOP", "preOpAction", QStyle::SP_CommandLink,
         uiText("Set selected slave to PREOP", "将选中从站切换到 PREOP"), QKeySequence("Alt+2")},
        {"SAFEOP", "safeOpAction", QStyle::SP_DialogApplyButton,
         uiText("Set selected slave to SAFEOP", "将选中从站切换到 SAFEOP"), QKeySequence("Alt+3")},
        {"OP", "opAction", QStyle::SP_DialogYesButton, uiText("Set selected slave to OP", "将选中从站切换到 OP"),
         QKeySequence("Alt+4")},
    };
    for (const auto& entry : actions) {
        auto* action = toolbar->addAction(style()->standardIcon(entry.icon), entry.text);
        action->setObjectName(entry.objectName);
        const QString objectName = QString::fromLatin1(entry.objectName);
        const bool shortcutAlreadyOnMenu =
            objectName == "connectAction" || objectName == "refreshAction" || objectName == "rescanAction";
        ...
        const QString visibleShortcut =
            shortcut.isEmpty() ? entry.shortcut.toString(QKeySequence::NativeText) : shortcut;
        action->setToolTip(visibleShortcut.isEmpty() ? entry.tip : QString("%1 (%2)").arg(entry.tip, visibleShortcut));
    ...
    auto* manageMastersAction =
        toolbar->addAction(style()->standardIcon(QStyle::SP_ComputerIcon), uiText("Manage Masters", "管理主站"));
    manageMastersAction->setObjectName("manageMastersAction");
    manageMastersAction->setToolTip(uiText("Configure master profiles and preferences", "配置主站档案和偏好设置"));
    ...
    captureTopology->setToolTip(
        uiText("Capture current online topology as expected baseline", "将当前在线拓扑捕获为期望基线"));
    ...
    clearTopology->setToolTip(uiText("Clear expected topology baseline", "清除期望拓扑基线"));
    ...
    topologyTree_->setHeaderLabels({uiText("Device", "设备"), uiText("State", "状态")});
    topologyTree_->header()->setSectionResizeMode(0, QHeaderView::Stretch);
    topologyTree_->header()->setSectionResizeMode(1, QHeaderView::ResizeToContents);
    ...
    diagnostics_->topologyBaselineLabel = new QLabel(uiText("No topology baseline", "未设置拓扑基线"));
    ...
    selectedSlaveNameLabel_ = new QLabel(uiText("No slave selected", "尚未选择从站"));
    ...
    selectedSlaveEvidenceLabel_ = new QLabel(uiText("Evidence: select a slave", "证据：请选择从站"));
    ...
    selectedSlaveEvidenceLabel_->setTextInteractionFlags(Qt::TextSelectableByMouse);
    selectedDriveSummaryLabel_ = new QLabel(uiText("Drive: no Watch evidence", "驱动：暂无监视证据"));
    selectedDriveSummaryLabel_->setObjectName("statusSummary");
    selectedDriveSummaryLabel_->setWordWrap(true);
    selectedDriveSummaryLabel_->setTextInteractionFlags(Qt::TextSelectableByMouse);
    selectedSlaveHintLabel_ = new QLabel(uiText("Select a slave in the I/O tree to enable contextual actions.",
    ...
    auto makeContextButton = [this](const QString& text, const char* objectName, QStyle::StandardPixmap icon) {
    ...
    slaveActions->addWidget(
        makeContextButton(uiText("OD", "对象"), "contextObjectDictionary", QStyle::SP_FileDialogDetailedView), 0, 0);
    slaveActions->addWidget(makeContextButton(uiText("PDO", "PDO"), "contextPdoMap", QStyle::SP_FileDialogListView), 0,
                            1);
    slaveActions->addWidget(
        makeContextButton(uiText("Watch", "监视"), "contextWatch", QStyle::SP_FileDialogContentsView), 0, 2);
    slaveActions->addWidget(makeContextButton(uiText("Free", "自由"), "contextFreeRun", QStyle::SP_MediaPlay), 1, 0);
    slaveActions->addWidget(makeContextButton(uiText("Diag", "诊断"), "contextDiagnostics", QStyle::SP_ComputerIcon), 1,
                            1);
    slaveActions->addWidget(
        makeContextButton(uiText("Refresh", "刷新"), "contextRefreshSlave", QStyle::SP_BrowserReload), 1, 2);
    slaveActions->addWidget(makeContextButton("INIT", "contextInit", QStyle::SP_DialogResetButton), 2, 0);
    slaveActions->addWidget(makeContextButton("PREOP", "contextPreOp", QStyle::SP_CommandLink), 2, 1);
    slaveActions->addWidget(makeContextButton("SAFEOP", "contextSafeOp", QStyle::SP_DialogApplyButton), 2, 2);
    slaveActions->addWidget(makeContextButton("OP", "contextOp", QStyle::SP_DialogYesButton), 3, 0, 1, 3);
    auto* driveNext =
        makeContextButton(uiText("Drive Next", "驱动下一步"), "contextDriveNext", QStyle::SP_DialogApplyButton);
    driveNext->setToolTip(uiText("Use Watch statusword evidence to prepare the recommended CiA 402 "
    ...
    auto* prepareSnapshot =
        makeContextButton(uiText("Snapshot", "准备快照"), "contextPrepareSnapshot", QStyle::SP_FileDialogDetailedView);
    prepareSnapshot->setToolTip(uiText("Prepare read-only evidence for the selected slave: refresh identity, "
    ...
    // Add widget to layout
    cards->addWidget(makeMetricCard(uiText("Master", "主站"), uiText("Idle", "空闲"), &masterStateLabel_), 0, 0);
    // Add widget to layout
    cards->addWidget(makeMetricCard(uiText("Slaves", "从站"), QString::number(slaves_.size()), &slaveCountLabel_),
                     // ── Tab Widget: all workspace pages ───────────────────────────────────
                     0, 1);
    // Add widget to layout
    cards->addWidget(makeMetricCard(uiText("Link", "链路"), uiText("Unknown", "未知"), &linkStateLabel_), 0, 2);
    // Add widget to layout
    cards->addWidget(makeMetricCard(uiText("Frame Loss", "丢帧"), "0", &lossLabel_), 0, 3);
    // Add widget to layout
    cards->addWidget(makeMetricCard(uiText("Free Run", "自由运行"),
                                    freeRun_ ? uiText("On", "开启") : uiText("Off", "关闭"), &freeRunLabel_),
                     0, 4);
    ...
    // Add widget to layout
    sessionHeader->addWidget(makeSectionTitle(uiText("Session Brief", "会话简报")));
    auto* sessionHint = new QLabel(uiText("Read-only decision context for the selected slave and current SDO.",
    ...
    session_->sessionBriefCopyButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    session_->sessionBriefCopyButton->setEnabled(false);
    session_->sessionBriefCopyButton->setToolTip(
        uiText("Copy the selected Session Brief row summary to the clipboard. No bus "
               "request is sent.",
    ...
    setTableRows(
        session_->sessionBriefTable,
        {uiText("Area", "区域"), uiText("Status", "状态"), uiText("Evidence", "依据"), uiText("Next", "下一步")}, {});
    session_->sessionBriefTable->setMinimumHeight(320);
    session_->sessionBriefTable->setToolTip(
        uiText("A read-only brief assembled from loaded UI evidence. It does not read "
    ...
    workflow_->workflowSummaryLabel =
        new QLabel(uiText("Follow the commissioning workflow from left to right.", "按调试流程从上到下推进。"));
    ...
    overviewRunNext->setToolTip(
        uiText("Run the first actionable commissioning workflow step.", "执行调试工作流中第一条可执行步骤。"));
    ...
    workflow_->workflowScopeFilter->addItem(uiText("Open", "未完成"), QStringLiteral("open"));
    // ── Watch Page ────────────────────────────────────────────────────────
    workflow_->workflowScopeFilter->addItem(uiText("Blocked", "受阻"), QStringLiteral("blocked"));
    workflow_->workflowScopeFilter->addItem(uiText("Action", "待执行"), QStringLiteral("action"));
    workflow_->workflowScopeFilter->addItem(uiText("Ready", "就绪"), QStringLiteral("ready"));
    workflow_->workflowScopeFilter->addItem(uiText("Risk", "风险"), QStringLiteral("risk"));
    workflow_->workflowScopeFilter->addItem(uiText("Evidence Gap", "证据缺口"), QStringLiteral("gap"));
    workflow_->workflowScopeFilter->setToolTip(
        uiText("Filter workflow steps by local status or evidence state. Filtering "
    ...
    workflow_->workflowFilter->setPlaceholderText(
        uiText("Search phase, step, risk, evidence, or next action", "搜索阶段、步骤、风险、依据或下一步"));
    workflow_->workflowFilter->setToolTip(uiText("Search the current workflow table locally without bus access.",
                                                 "只在当前工作流表格内本地搜索，不访问总线。"));
    workflow_->workflowReviewButton = new QPushButton(uiText("Review First", "审阅首个"));
    workflow_->workflowReviewButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    workflow_->workflowReviewButton->setEnabled(false);
    workflow_->workflowReviewButton->setToolTip(
        uiText("Select the first visible workflow issue without running the step.",
               "选择当前可见的首个工作流问题；不会执行该步骤。"));
    workflow_->workflowReviewNextButton = new QPushButton(uiText("Review Next", "审阅下个"));
    workflow_->workflowReviewNextButton->setIcon(style()->standardIcon(QStyle::SP_ArrowForward));
    workflow_->workflowReviewNextButton->setEnabled(false);
    workflow_->workflowReviewNextButton->setToolTip(
        uiText("Select the next visible workflow issue with wraparound. No bus request "
               "is sent.",
               "选择下一个可见工作流问题并在末尾回绕；不会发送总线请求。"));
    workflow_->workflowStepCopyButton = new QPushButton(uiText("Copy Step", "复制步骤"));
    workflow_->workflowStepCopyButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    workflow_->workflowStepCopyButton->setEnabled(false);
    workflow_->workflowStepCopyButton->setToolTip(
        uiText("Copy the selected workflow step evidence summary to the clipboard. No "
               "bus request is sent.",
               "把当前工作流步骤证据摘要复制到剪贴板；不会发送总线请求。"));
    workflowHeader->addWidget(makeSectionTitle(uiText("Commissioning Workflow", "调试工作流")));
    ...
    workflow_->workflowStepDetailLabel = makeStatusSummaryLabel(
        uiText("Select a workflow row to review its evidence boundary.", "选择工作流行以复核它的证据边界。"),
    ...
    setTableRows(workflow_->workflowTable, commissioningWorkflowHeaders(commissioningWorkflowTexts()), {});
    workflow_->workflowTable->setMinimumHeight(420);
    workflow_->workflowTable->setToolTip(uiText("Select a workflow row to review its local evidence boundary. "
    ...
    // Add widget to layout
    matrixHeader->addWidget(makeSectionTitle(uiText("Slave Evidence Matrix", "从站证据矩阵")));
    ...
    slaveEvidence_->slaveEvidenceMatrixSummaryLabel->setTextInteractionFlags(Qt::TextSelectableByMouse);
    slaveEvidence_->slaveEvidenceMatrixScopeFilter = new QComboBox;
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("All", "全部"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeAll));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("P0 Fault", "P0 故障"),
                                                            QString::fromLatin1(kSlaveEvidenceScopePriorityP0));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("P1 Risk", "P1 风险"),
                                                            QString::fromLatin1(kSlaveEvidenceScopePriorityP1));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("P2 Action", "P2 待执行"),
    ...
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Risk", "风险"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeRisk));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Action", "待执行"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeAction));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Ready", "就绪"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeReady));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Missing OD", "缺 OD"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeMissingOd));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Missing PDO", "缺 PDO"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeMissingPdo));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Missing Watch", "缺 Watch"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeMissingWatch));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Startup Diff", "启动偏差"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeStartupDiff));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->addItem(uiText("Process Missing", "缺过程证据"),
                                                            QString::fromLatin1(kSlaveEvidenceScopeProcessMissing));
    slaveEvidence_->slaveEvidenceMatrixScopeFilter->setToolTip(
        uiText("Filter the matrix by local evidence state. Filtering does not read the "
               "bus.",
    ...
    slaveEvidence_->slaveEvidenceMatrixFilter->setPlaceholderText(
        uiText("Search slave, state, risk, or next action", "搜索从站、状态、风险或下一步"));
    ...
    slaveEvidence_->slaveEvidenceMatrixReviewButton = new QPushButton(uiText("Review First", "审阅首个问题"));
    slaveEvidence_->slaveEvidenceMatrixReviewButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    slaveEvidence_->slaveEvidenceMatrixReviewButton->setToolTip(
        uiText("Open the first visible Risk or Action matrix row in local evidence. No "
               "bus request is sent.",
               "打开首个可见风险或待执行矩阵行的本地证据；不会发送总线请求。"));
    slaveEvidence_->slaveEvidenceMatrixReviewNextButton = new QPushButton(uiText("Review Next", "审阅下个问题"));
    slaveEvidence_->slaveEvidenceMatrixReviewNextButton->setIcon(style()->standardIcon(QStyle::SP_ArrowForward));
    slaveEvidence_->slaveEvidenceMatrixReviewNextButton->setToolTip(
        uiText("Open the next visible Risk or Action matrix row after the current row; "
               "wraps to the first issue. No bus request is sent.",
               "打开当前行之后的下一个可见风险或待执行矩阵行；到末尾后回到首个问题。"
               "不会发送总线请求。"));
    slaveEvidence_->slaveEvidenceMatrixCopyButton = new QPushButton(uiText("Copy Row", "复制本行"));
    slaveEvidence_->slaveEvidenceMatrixCopyButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    slaveEvidence_->slaveEvidenceMatrixCopyButton->setEnabled(false);
    slaveEvidence_->slaveEvidenceMatrixCopyButton->setToolTip(
        uiText("Copy the selected matrix row evidence summary to the clipboard. No bus "
               "request is sent.",
    ...
    for (const auto& triage : triageScopes) {
        ...
        button->setToolTip(uiText("Filter the slave evidence matrix by this priority. This is local UI "
                                  "filtering only.",
    ...
    setTableRows(slaveEvidence_->slaveEvidenceMatrixTable, slaveEvidenceMatrixHeaders(slaveEvidenceUiTexts()), {});
    slaveEvidence_->slaveEvidenceMatrixTable->setMinimumHeight(420);
    slaveEvidence_->slaveEvidenceMatrixTable->setToolTip(
        uiText("Read-only multi-slave evidence matrix built from loaded UI evidence. "
    ...
    // Add widget to layout
    overviewGrid->addWidget(makeSectionTitle(uiText("Master Metrics", "主站指标")), 0, 0);
    // Add widget to layout
    overviewGrid->addWidget(makeSectionTitle(uiText("Identity", "身份信息")), 0, 1);
    // Add widget to layout
    ...
    sdo_->sdoFilter->setPlaceholderText(uiText("Filter object dictionary", "过滤对象字典"));
    sdo_->sdoFilter->setToolTip(uiText("Select an object row to fill the SDO command fields; "
    ...
    sdoInspector_->sdoType->addItems({"", "bool", "int8", "int16", "int32", "int64", "uint8", "uint16", "uint32",
                                      "uint64", "float", "double", "string", "octet_string"});
    auto* readSdo = new QPushButton(uiText("Read", "读取"));
    readSdo->setObjectName("readSdo");
    auto* readSelectedDictionary = new QPushButton(uiText("Read Selected", "读取所选"));
    readSelectedDictionary->setObjectName("readSelectedDictionary");
    readSelectedDictionary->setIcon(style()->standardIcon(QStyle::SP_BrowserReload));
    ...
    auto* useSdoEvidenceButton = new QPushButton(uiText("Use Evidence", "使用证据"));
    useSdoEvidenceButton->setObjectName("useSdoEvidence");
    useSdoEvidenceButton->setIcon(style()->standardIcon(QStyle::SP_ArrowForward));
    useSdoEvidenceButton->setToolTip(uiText("Copy the best local Read/Watch/OD/Startup/Bookmark/Target Trail "
                                            "evidence into the write field without bus access.",
                                            "把最佳本地读回、Watch、OD、Startup "
                                            "或书签/目标轨迹证据复制到写入框，不访问总线。"));
    auto* pickSdoEvidenceButton = new QPushButton(uiText("Pick Evidence", "选择证据"));
    pickSdoEvidenceButton->setObjectName("pickSdoEvidence");
    pickSdoEvidenceButton->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    pickSdoEvidenceButton->setToolTip(uiText("Choose one local evidence value for the write field without bus access.",
                                             "从本地证据中选择一个值填入写入框，不访问总线。"));
    auto* writeSdo = new QPushButton(uiText("Write", "写入"));
    writeSdo->setObjectName("writeSdo");
    auto* watchSelectedDictionary = new QPushButton(uiText("Watch Selected", "监视所选"));
    watchSelectedDictionary->setObjectName("watchSelectedDictionary");
    watchSelectedDictionary->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    watchSelectedDictionary->setToolTip(
        uiText("Add selected Object Dictionary rows to Watch without duplicating items.",
               "把选中的对象字典行加入 Watch，并自动复用已有监视项。"));
    auto* startupSelectedEvidence = new QPushButton(uiText("Startup Evidence", "证据启动项"));
    startupSelectedEvidence->setObjectName("startupSelectedEvidence");
    startupSelectedEvidence->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupSelectedEvidence->setToolTip(
        uiText("Create or update Startup SDO rows from selected Object Dictionary rows "
    ...
    // Add widget to layout
    sdoTargetHeader->addWidget(makeSectionTitle(uiText("Selected Object", "选中对象")));
    ...
    reviewWriteDeltaSdo->setIcon(style()->standardIcon(QStyle::SP_MessageBoxWarning));
    reviewWriteDeltaSdo->setToolTip(uiText("Open the local evidence that explains the current Evidence Set conflict "
                                           "or Write Delta without reading or writing the bus.",
                                           "打开解释当前证据集冲突或写入差异的本地证据，不读取也不写入总线。"));
    auto* runSelectedObjectRowAction = new QPushButton(uiText("Run Row", "执行行"));
    runSelectedObjectRowAction->setObjectName("runSdoTargetRowAction");
    runSelectedObjectRowAction->setIcon(style()->standardIcon(QStyle::SP_CommandLink));
    runSelectedObjectRowAction->setToolTip(uiText("Select a Selected Object row to run its local Action column entry.",
    ...
    // ── Free Run Page ───────────────────────────────────────────────────
    copySelectedObjectRow->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    copySelectedObjectRow->setToolTip(uiText("Copy the selected Selected Object row evidence without bus access.",
    ...
    copyEvidenceDigestSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    copyEvidenceDigestSdo->setToolTip(uiText("Copy the current target, Evidence Set, Write Delta, and local evidence "
    ...
    openWatchLinkSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    openWatchLinkSdo->setToolTip(uiText("Open the matching Watch row for the current SDO target without reading "
                                        "the bus.",
    ...
    openStartupLinkSdo->setToolTip(uiText("Open the matching Startup SDO row for the current target without "
                                          "writing the bus.",
    ...
    openBookmarkLinkSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    openBookmarkLinkSdo->setToolTip(uiText("Open the matching Object Bookmark row for the current target without "
                                           "bus access.",
    ...
    openTargetTrailLinkSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    openTargetTrailLinkSdo->setToolTip(uiText("Open the matching SDO Target Trail row for the current target without "
                                              "bus access.",
    ...
    watchTargetSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    auto* bookmarkTargetSdo = new QPushButton(uiText("Bookmark", "书签"));
    bookmarkTargetSdo->setObjectName("bookmarkTargetSdo");
    bookmarkTargetSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogInfoView));
    bookmarkTargetSdo->setToolTip(uiText("Save the current SDO target as a project bookmark without bus access.",
    ...
    startupTargetSdo->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    ...
    sdoInspector_->sdoTargetTable->setHorizontalHeaderLabels(
        {uiText("Field", "字段"), uiText("Value", "值"), uiText("Action", "动作")});
    sdoInspector_->sdoTargetTable->verticalHeader()->setVisible(false);
    sdoInspector_->sdoTargetTable->horizontalHeader()->setSectionResizeMode(0, QHeaderView::ResizeToContents);
    sdoInspector_->sdoTargetTable->horizontalHeader()->setSectionResizeMode(2, QHeaderView::ResizeToContents);
    sdoInspector_->sdoTargetTable->horizontalHeader()->setSectionResizeMode(1, QHeaderView::Stretch);
    ...
    sdoInspector_->sdoTargetTable->setToolTip(
        uiText("Live context for the currently selected Object Dictionary, Watch, PDO, "
    ...
    // Add widget to layout
    sdoTargetTrailHeader->addWidget(makeSectionTitle(uiText("SDO Target Trail", "SDO 目标轨迹")));
    auto* sdoTargetTrailHint =
        new QLabel(uiText("Recent local SDO targets from OD, PDO, Watch, Free Run, I/O, Startup, "
    ...
    auto* restoreSdoTargetTrail = new QPushButton(uiText("Restore Target", "恢复目标"));
    restoreSdoTargetTrail->setObjectName("restoreSdoTargetTrail");
    restoreSdoTargetTrail->setIcon(style()->standardIcon(QStyle::SP_ArrowForward));
    restoreSdoTargetTrail->setToolTip(uiText("Restore the selected target into the SDO fields. This only changes "
                                             "local UI context.",
    ...
    watchSdoTargetTrail->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    watchSdoTargetTrail->setToolTip(uiText("Restore the selected trail target locally and add it to Watch without "
    ...
    bookmarkSdoTargetTrail->setIcon(style()->standardIcon(QStyle::SP_FileDialogInfoView));
    bookmarkSdoTargetTrail->setToolTip(uiText("Save the selected trail target as a project Object Bookmark without "
                                              "bus access.",
    ...
    startupSdoTargetTrail->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupSdoTargetTrail->setToolTip(uiText("Create a Startup SDO candidate from the selected trail write value, or "
    ...
    clearSdoTargetTrailButton->setIcon(style()->standardIcon(QStyle::SP_TrashIcon));
    ...
    sdoTargetTrailTable_->setToolTip(uiText("Double-click or use Restore Target to refill the SDO fields from a "
    ...
    sdoTargetTrailDetailLabel_ =
        makeStatusSummaryLabel(uiText("Select an SDO target trail row to review reuse readiness and "
                                      "local execution boundary.",
                                      "选择一条 SDO 目标轨迹行，以复核复用就绪度和本地执行边界。"),
                               uiText("Selecting trail rows and reading this detail strip are local "
    ...
    const QVector<SdoFilterChip> sdoFilterPresets = {
        {uiText("All", "全部"), QString(), uiText("Clear Object Dictionary filtering", "清除对象字典过滤")},
        {uiText("Writable", "可写"), "tag:writable",
         uiText("Show writable Object Dictionary entries", "显示可写对象字典条目")},
        {uiText("Readable", "可读"), "tag:readable",
         uiText("Show readable Object Dictionary entries", "显示可读对象字典条目")},
        {"CiA 402", "tag:cia402", uiText("Show common CiA 402 drive objects", "显示常见 CiA 402 驱动对象")},
        {uiText("Identity", "身份"), "tag:identity",
         uiText("Show device identity and diagnostic identity objects", "显示设备身份和诊断身份对象")},
        {"PDO", "tag:pdo",
         uiText("Show PDO assignment and mapping objects",
                // ── Slave Evidence Matrix Page ────────────────────────────────────────
                "显示 PDO 分配和映射对象")},
        {uiText("Errors", "错误"), "tag:error",
         uiText("Show error, emergency, and diagnostic objects", "显示错误、Emergency 和诊断对象")},
        ...
        {uiText("Failed", "失败"), "tag:failed",
         uiText("Show Object Dictionary rows whose latest SDO evidence failed", "显示最新 SDO 证据为失败的对象字典行")},
    };
    for (const auto& preset : sdoFilterPresets) {
        ...
        connect(button, &QPushButton::clicked, this, [this, query = preset.query] { setSdoFilterPreset(query); });
    ...
    readVisibleDictionary->setIcon(style()->standardIcon(QStyle::SP_BrowserReload));
    readVisibleDictionary->setToolTip(uiText("Read every currently visible Object Dictionary row. Large batches ask "
                                             "for confirmation.",
    ...
    readFailedDictionary->setIcon(style()->standardIcon(QStyle::SP_MessageBoxWarning));
    readFailedDictionary->setToolTip(uiText("Retry only Object Dictionary rows whose latest SDO evidence failed.",
    ...
    watchVisibleDictionary->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    watchVisibleDictionary->setToolTip(uiText("Add currently visible Object Dictionary rows to Watch without issuing "
                                              "immediate reads.",
    ...
    sdo_->sdoTable->setToolTip(uiText("Selecting a row fills Index/Sub/Type. Double-click reads the "
                                      "selected object.",
    ...
    auto* bookmarkSelectedDictionary = new QPushButton(uiText("Bookmark Selected", "收藏所选"));
    bookmarkSelectedDictionary->setObjectName("bookmarkSelectedDictionary");
    bookmarkSelectedDictionary->setIcon(style()->standardIcon(QStyle::SP_FileDialogInfoView));
    bookmarkSelectedDictionary->setToolTip(
        uiText("Save selected Object Dictionary rows as project bookmarks. This does "
    ...
    fillBookmarkSdo->setToolTip(
        uiText("Fill the SDO fields from the selected object bookmark.", "用选中的对象书签回填 SDO 指令字段。"));
    auto* watchBookmarkSdo = new QPushButton(uiText("Watch Bookmark", "监视书签"));
    watchBookmarkSdo->setObjectName("watchBookmarkSdo");
    watchBookmarkSdo->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    watchBookmarkSdo->setToolTip(uiText("Add selected object bookmarks to Watch without immediate reads.",
                                        "把选中的对象书签加入 Watch，不立即读取。"));
    auto* startupBookmarkSdo = new QPushButton(uiText("Create Startup", "创建启动项"));
    startupBookmarkSdo->setObjectName("startupBookmarkSdo");
    startupBookmarkSdo->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupBookmarkSdo->setToolTip(uiText("Create or update Startup SDO rows from selected bookmarks using their "
                                          "saved Last Value. This does not read or write the bus.",
                                          "使用所选书签保存的最后值创建或更新 Startup SDO 行；不会读写总线。"));
    auto* removeBookmarkSdo = new QPushButton(uiText("Remove Bookmark", "移除书签"));
    removeBookmarkSdo->setObjectName("removeBookmarkSdo");
    removeBookmarkSdo->setIcon(style()->standardIcon(QStyle::SP_TrashIcon));
    bookmarkHeader->addWidget(makeSectionTitle(uiText("Object Bookmarks", "对象书签")));
    ...
    bookmark_->objectBookmarkTable->setToolTip(
        uiText("Project-local SDO object bookmarks. Double-click fills the SDO target; "
               "reads and writes still require explicit user actions.",
               "工程内 SDO 对象书签。双击只回填 SDO "
               "目标；读取和写入仍需用户显式操作。"));
    ensureObjectBookmarkTable();
    objectBookmarkDetailLabel_ =
        makeStatusSummaryLabel(uiText("Select an object bookmark to review saved evidence, reuse "
    ...
    auto* historyWatchSelected = new QPushButton(uiText("Watch Selected", "监视所选"));
    historyWatchSelected->setObjectName("watchSelectedHistory");
    historyWatchSelected->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    historyWatchSelected->setToolTip(uiText("Add selected SDO history rows to Watch without issuing immediate reads.",
                                            "把选中的 SDO 历史行加入 Watch，不立即批量读取。"));
    auto* startupFromSelectedHistory = new QPushButton(uiText("Create Startup", "创建启动项"));
    startupFromSelectedHistory->setObjectName("startupFromSelectedHistory");
    startupFromSelectedHistory->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupFromSelectedHistory->setToolTip(uiText("Create Startup SDO rows from selected SDO history rows that contain "
                                                  "values.",
    ...
    sdoHistoryHeader->addWidget(makeSectionTitle(uiText("SDO Operation History", "SDO 操作历史")));
    ...
    sdoHistoryTable_->setToolTip(uiText("Audit trail for SDO reads and writes in this session. Select "
    ...
    sdoHistoryDetailLabel_ = makeStatusSummaryLabel(
        uiText("Select an SDO history row to review audit evidence.", "选择 SDO 历史行以复核审计证据。"),
        // ── Diagnostics Page ────────────────────────────────────────────────
    ...
    sdo_->pdoFilter->setPlaceholderText(uiText("Filter PDO map by SM, PDO, index, sub, bits, or name",
                                               "按 SM、PDO、索引、子项、位宽或名称过滤 PDO 映射"));
    auto* addSelectedPdoWatch = new QPushButton(uiText("Add Selected to Watch", "选中项加入监视"));
    addSelectedPdoWatch->setObjectName("addSelectedPdoWatch");
    addSelectedPdoWatch->setIcon(
        // ── State Machine Page ────────────────────────────────────────────────
        style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    addSelectedPdoWatch->setToolTip(uiText("Add the selected PDO map rows to Watch without duplicating existing "
                                           "items.",
    ...
    sdo_->pdoDetailLabel = makeStatusSummaryLabel(
        uiText("Select a PDO entry to review process-data evidence.", "选择 PDO 条目以复核过程数据证据。"),
    ...
    preflightStartup->setIcon(style()->standardIcon(QStyle::SP_MessageBoxInformation));
    ...
    auto* verifySelectedStartup = new QPushButton(uiText("Verify Selected", "校验所选"));
    verifySelectedStartup->setObjectName("verifySelectedStartupSdo");
    verifySelectedStartup->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    ...
    watch_->startupWatchDiffsOnly->setToolTip(
        uiText("Show only Startup SDO rows whose expected value differs from current "
               "Watch evidence.",
               "只显示期望启动值和当前 Watch 证据不一致的 Startup SDO 行。"));
    auto* focusWatchDiffStartup = new QPushButton(uiText("Review Diffs", "审阅偏差"));
    focusWatchDiffStartup->setObjectName("focusStartupSdoWatchDiffs");
    focusWatchDiffStartup->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    focusWatchDiffStartup->setToolTip(uiText("Open Startup SDO, enable Diffs Only, and select the first Watch "
                                             "mismatch.",
                                             "打开 Startup SDO，启用只看偏差，并选中第一条 Watch 不一致项。"));
    auto* applyWatchDiffStartup = new QPushButton(uiText("Apply Diffs", "应用偏差"));
    applyWatchDiffStartup->setObjectName("applyStartupSdoWatchDiffs");
    applyWatchDiffStartup->setIcon(style()->standardIcon(QStyle::SP_DialogYesButton));
    applyWatchDiffStartup->setToolTip(uiText("Apply only Startup SDO rows whose expected values differ from current "
                                             "Watch values.",
    ...
    auto* applySelectedStartup = new QPushButton(uiText("Apply Selected", "应用所选"));
    applySelectedStartup->setObjectName("applySelectedStartupSdo");
    applySelectedStartup->setIcon(style()->standardIcon(QStyle::SP_DialogYesButton));
    ...
    watch_->startupWatchSummaryLabel = new QLabel(uiText("No Startup SDO rows", "暂无 Startup SDO 行"));
    ...
    watch_->startupSdoDetailLabel =
        new QLabel(uiText("Select a Startup SDO row to review apply evidence.", "选择 Startup SDO 行以复核应用证据。"));
    ...
    watch_->startupSdoDetailLabel->setToolTip(
        uiText("This preview is local. It summarizes the selected Startup SDO target, "
    ...
    rawText_->projectNotes->setPlaceholderText(
        uiText("Project notes, commissioning steps, and handoff details", "工程备注、调试步骤和交接信息"));
    ...
    for (auto* editor :
         {rawText_->masterText, rawText_->infoText, rawText_->pdoText, rawText_->sdoText, rawText_->xmlText}) {
    ...
    freeRunLayout->addWidget(makeSectionTitle(uiText("Runtime Status", "运行时状态")));
    freeRunLayout->addWidget(freeRunWidgets_->freeRunTable, 1);
    freeRunLayout->addWidget(makeSectionTitle(uiText("Process Image", "过程映像")));
    freeRunWidgets_->freeRunFilter = new QLineEdit;
    freeRunWidgets_->freeRunFilter->setPlaceholderText(uiText(
        "Filter process image by name, index, PDO, direction, or value", "按名称、索引、PDO、方向或值过滤过程映像"));
    freeRunWidgets_->freeRunChangedOnly = new QCheckBox(uiText("Changed only", "仅变化项"));
    freeRunWidgets_->freeRunEntrySummaryLabel = new QLabel(uiText("No process image entries", "暂无过程映像条目"));
    ...
    freeRunWidgets_->freeRunEntryDetailLabel = new QLabel(uiText("Select a process image entry to review map and name "
                                                                 "evidence.",
                                                                 "选择过程映像条目以复核映射和名称证据。"));
    ...
    freeRunWidgets_->freeRunEntryDetailLabel->setToolTip(
        uiText("This preview is local. It summarizes the selected Free Run row, name "
    ...
    ioVar_->ioVariableFilter->setPlaceholderText(
        uiText("Filter variables by slave, symbol, index, PDO, value, meaning, or risk",
    ...
    ioVar_->ioVariableScopeFilter->addItem(uiText("Selected Slave", "当前从站"), "selected");
    ioVar_->ioVariableScopeFilter->addItem(uiText("Process Image", "过程映像"), "process");
    ioVar_->ioVariableScopeFilter->addItem(uiText("PDO Only", "仅 PDO"), "pdo");
    ioVar_->ioVariableScopeFilter->addItem(uiText("Watch Evidence", "Watch 证据"), "watch");
    ioVar_->ioVariableScopeFilter->addItem(uiText("Startup Diff", "启动不一致"), "startupDiff");
    ioVar_->ioVariableScopeFilter->addItem(uiText("Missing Value", "缺失值"), "missingValue");
    ...
    ioVar_->ioVariableScopeFilter->addItem(uiText("PLC Issues", "PLC 交接问题"), "plcIssues");
    auto* refreshIoVariables = new QPushButton(uiText("Refresh View", "刷新视图"));
    ...
    watchSelectedIo->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    auto* watchVisibleIo = new QPushButton(uiText("Watch Visible", "监视可见"));
    watchVisibleIo->setObjectName("watchVisibleIoVariables");
    watchVisibleIo->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    auto* startupSelectedIo = new QPushButton(uiText("Startup Selected", "启动所选"));
    startupSelectedIo->setObjectName("startupSelectedIoVariables");
    startupSelectedIo->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupSelectedIo->setToolTip(uiText("Create or update Startup SDO rows from selected I/O variable evidence. "
                                         "Watch value is preferred, Raw value is used as fallback, and no bus "
                                         "access is performed.",
                                         "用所选 I/O 变量证据创建或更新 Startup SDO；优先使用 Watch 值，缺失时"
                                         "使用 Raw 值，且不访问总线。"));
    auto* startupVisibleIo = new QPushButton(uiText("Startup Visible", "启动可见"));
    startupVisibleIo->setObjectName("startupVisibleIoVariables");
    startupVisibleIo->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    startupVisibleIo->setToolTip(uiText("Create or update Startup SDO rows from every visible I/O variable with "
    ...
    bulkNameIoVariables->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    auto* reviewPlcHandoff = new QPushButton(uiText("Review PLC", "审阅 PLC"));
    reviewPlcHandoff->setObjectName("reviewPlcHandoff");
    reviewPlcHandoff->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    reviewPlcHandoff->setToolTip(uiText("Show visible I/O variables that are not ready for PLC handoff.",
    ...
    exportIoVariables->setIcon(style()->standardIcon(QStyle::SP_DialogSaveButton));
    ...
    ioVar_->ioVariableSummaryLabel = new QLabel(uiText("No I/O variables", "暂无 I/O 变量"));
    ...
    ioVar_->ioVariableDetailLabel =
        new QLabel(uiText("Select an I/O variable to review signal evidence.", "选择 I/O 变量以复核信号证据。"));
    ...
    ioVar_->ioVariableDetailLabel->setToolTip(
        uiText("This preview is local. It summarizes the selected signal, address, "
    ...
    ioVar_->ioVariableTable->setToolTip(uiText("Engineering signal table merged from PDO Map, Free Run process image, "
    ...
    setTableRows(ioVar_->ioVariableTable,
                 {uiText("Slave", "从站"), uiText("Dir", "方向"), uiText("Symbol", "符号"), uiText("Index", "索引"),
                  ...
                  uiText("Raw", "原始值"), uiText("Decoded", "解码"), uiText("Meaning", "含义"),
                  uiText("Watch", "Watch"), uiText("Startup", "启动"), uiText("Map", "映射"), uiText("Changed", "变化"),
                  uiText("PLC", "PLC"), uiText("Alias", "别名"), uiText("Tags", "标签"), uiText("Note", "备注")},
                 {});
    ...
    consistency_->consistencyFilter->setPlaceholderText(
        uiText("Filter consistency by scope, object, variable, severity, or evidence",
    ...
    auto* refreshConsistency = new QPushButton(uiText("Refresh Check", "刷新检查"));
    refreshConsistency->setObjectName("refreshConsistency");
    refreshConsistency->setIcon(style()->standardIcon(QStyle::SP_BrowserReload));
    auto* openIoFromConsistency = new QPushButton(uiText("Open Evidence", "打开证据"));
    openIoFromConsistency->setObjectName("openIoVariablesFromConsistency");
    openIoFromConsistency->setIcon(style()->standardIcon(QStyle::SP_FileDialogDetailedView));
    openIoFromConsistency->setToolTip(uiText("Open the best local evidence table for the selected Consistency row. "
                                             // ── Session Page ────────────────────────────────────────────────────
                                             "This only navigates loaded tables and does not access the bus.",
                                             "打开当前一致性行最相关的本地证据表；只导航已加载表格，不访问总线。"));
    consistency_->consistencySummaryLabel = new QLabel(uiText("No consistency check yet", "尚未运行一致性检查"));
    ...
    consistency_->consistencyDetailLabel =
        new QLabel(uiText("Select a Consistency row to review gate evidence.", "选择一致性行以复核门禁证据。"));
    ...
    consistency_->consistencyDetailLabel->setToolTip(
        uiText("This preview is local. It summarizes the selected consistency issue, "
    ...
    consistency_->consistencyTable->setToolTip(
        uiText("Read-only online/offline consistency review. Double-click a row to "
    ...
    setTableRows(consistency_->consistencyTable,
                 {uiText("Level", "级别"), uiText("Scope", "范围"), uiText("Target", "目标"),
                  uiText("Evidence", "证据"),
                  // ── Status Bar ────────────────────────────────────────────────────────
                  uiText("Expected", "期望"), uiText("Actual", "实际"), uiText("Action", "建议动作")},
                 {});
    ...
    auto* addCia402Watch = new QPushButton(uiText("CiA 402 Preset", "CiA 402 预设"));
    addCia402Watch->setObjectName("addCia402WatchPreset");
    addCia402Watch->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    addCia402Watch->setToolTip(uiText("Add common drive objects such as controlword, statusword, mode, "
    ...
    auto* captureWatchBaselineButton = new QPushButton(uiText("Capture Baseline", "捕获基线"));
    captureWatchBaselineButton->setObjectName("captureWatchBaseline");
    captureWatchBaselineButton->setIcon(style()->standardIcon(QStyle::SP_DialogSaveButton));
    captureWatchBaselineButton->setToolTip(uiText("Store current Watch values as baseline for drift comparison.",
                                                  "把当前 Watch 值保存为基线，用于偏差比较。"));
    auto* clearWatchBaselineButton = new QPushButton(uiText("Clear Baseline", "清除基线"));
    clearWatchBaselineButton->setObjectName("clearWatchBaseline");
    clearWatchBaselineButton->setIcon(style()->standardIcon(QStyle::SP_TrashIcon));
    clearWatchBaselineButton->setToolTip(
        uiText("Clear Watch baselines and drift results.", "清除 Watch 基线和偏差结果。"));
    auto* startupFromSelectedWatch = new QPushButton(uiText("Create Startup", "创建启动项"));
    startupFromSelectedWatch->setObjectName("startupFromSelectedWatch");
    startupFromSelectedWatch->setIcon(style()->standardIcon(QStyle::SP_FileDialogNewFolder));
    startupFromSelectedWatch->setToolTip(
        uiText("Create Startup SDO rows from selected Watch values.", "用选中 Watch 行的当前值创建 Startup SDO 行。"));
    auto* syncStartupFromWatch = new QPushButton(uiText("Sync Startup", "同步启动"));
    syncStartupFromWatch->setObjectName("syncStartupFromWatch");
    syncStartupFromWatch->setIcon(style()->standardIcon(QStyle::SP_DialogApplyButton));
    syncStartupFromWatch->setToolTip(uiText("Update existing Startup SDO rows from selected Watch values, creating "
    ...
    watch_->watchFilter->setPlaceholderText(uiText("Filter watch by slave, index, sub, value, type, or mode",
    ...
    watch_->watchScopeFilter->setToolTip(
        uiText("Limit Watch rows to the active engineering scope.", "按当前工程关注范围筛选 Watch 行。"));
    ...
    watch_->watchScopeFilter->addItem(uiText("Baseline Drift", "基线偏离"), "baselineDrift");
    watch_->watchScopeFilter->addItem(uiText("Startup Diff", "启动不一致"), "startupDiff");
    ...
    watch_->watchDetailLabel =
        new QLabel(uiText("Select a Watch row to review value evidence.", "选择 Watch 行以复核数值证据。"));
    ...
    watch_->watchDetailLabel->setToolTip(
        uiText("This preview is local. It summarizes the selected Watch value, baseline "
    ...
    stateMachine_->stateMachineSummaryLabel = new QLabel(uiText("No state evidence", "暂无状态机证据"));
    ...
    auto* stateSelectedNext = new QPushButton(uiText("Send Recommended", "发送推荐状态"));
    stateSelectedNext->setObjectName("stateSelectedNext");
    stateSelectedNext->setIcon(style()->standardIcon(QStyle::SP_CommandLink));
    stateSelectedNext->setToolTip(uiText("Send the recommended EtherCAT state for the selected matrix row using "
    ...
    stateMachineHeader->addWidget(makeSectionTitle(uiText("EtherCAT State Machine", "EtherCAT 状态机")));
    ...
    stateMachine_->stateMachineDetailLabel =
        new QLabel(uiText("Select a state row to review transition evidence.", "选择状态机行以复核状态切换证据。"));
    ...
    stateMachine_->stateMachineDetailLabel->setToolTip(
        uiText("Selecting a state-machine row only updates this local evidence preview. "
    ...
    stateMachine_->stateMachineTable->setToolTip(
        uiText("State matrix for each detected slave. Select a row to review evidence; "
    ...
    setTableRows(stateMachine_->stateMachineTable,
                 {uiText("Slave", "从站"), uiText("Name", "名称"), uiText("Current", "当前"),
                  uiText("Recommended", "推荐"), uiText("Evidence", "证据"), uiText("Drive", "驱动"),
                  uiText("Startup", "启动"), uiText("PDO/Process", "PDO/过程"), uiText("Risk", "风险"),
                  uiText("Action", "动作")},
                 {});
    // ── Log Dock ──────────────────────────────────────────────────────────
    ...
    diagnostics_->diagnosticsFilter->setPlaceholderText(
        uiText("Filter diagnostics by source or message", "按来源或消息过滤诊断"));
    ...
    copyHostCommand->setIcon(style()->standardIcon(QStyle::SP_FileDialogContentsView));
    ...
    diagnostics_->hostHealthSummaryLabel =
        new QLabel(uiText("Host health has not been checked", "尚未运行主机健康检查"));
    ...
    diagnosticsLayout->addWidget(makeSectionTitle(uiText("Host Health", "主机健康")));
    diagnosticsLayout->addWidget(diagnostics_->hostHealthSummaryLabel);
    setTableRows(hostHealthTable_,
                 {uiText("Level", "级别"), uiText("Check", "检查项"), uiText("Result", "结果"),
                  uiText("Action", "建议动作"), uiText("Command", "命令"), uiText("Detail", "细节")},
                 {});
    diagnosticsLayout->addWidget(hostHealthTable_, 1);
    diagnosticsLayout->addWidget(makeSectionTitle(uiText("Event Stream", "事件流")));
    ...
    objectDictionaryTabIndex_ = tabs_->addTab(sdoPage, uiText("Object Dictionary", "对象字典"));
    pdoMapTabIndex_ = tabs_->addTab(pdoPage, uiText("PDO Map", "PDO 映射"));
    watchTabIndex_ = tabs_->addTab(watchPage, uiText("Watch", "监视"));
    startupSdoTabIndex_ = tabs_->addTab(startupPage, uiText("Startup SDO", "启动 SDO"));
    freeRunTabIndex_ = tabs_->addTab(freeRunPage, uiText("Free Run", "自由运行"));
    ioVariableTabIndex_ = tabs_->addTab(ioVariablePage, uiText("I/O Variables", "I/O 变量"));
    consistencyTabIndex_ = tabs_->addTab(consistencyPage, uiText("Consistency", "一致性"));
    stateMachineTabIndex_ = tabs_->addTab(stateMachinePage, uiText("State Machine", "状态机"));
    diagnosticsTabIndex_ = tabs_->addTab(diagnosticsPage, uiText("Diagnostics", "诊断"));
    esiRepositoryTabIndex_ = tabs_->addTab(esiTable_, uiText("ESI Repository", "ESI 仓库"));
    ...
    masterRawTabIndex_ = tabs_->addTab(rawText_->masterText, uiText("Master Raw", "主站原始输出"));
    slaveRawTabIndex_ = tabs_->addTab(rawText_->infoText, uiText("Slave Raw", "从站原始输出"));
    ...
    connect(nextTabSc, &QShortcut::activated, this,
            [this]() { tabs_->setCurrentIndex((tabs_->currentIndex() + 1) % tabs_->count()); });
    tabSwitchShortcuts_.append(nextTabSc);
    auto* prevTabSc = new QShortcut(QKeySequence("Ctrl+Shift+Tab"), this);
    connect(prevTabSc, &QShortcut::activated, this,
            [this]() { tabs_->setCurrentIndex((tabs_->currentIndex() - 1 + tabs_->count()) % tabs_->count()); });
    ...
    nextBestActionButton_ = new QPushButton(uiText("Next Best Action", "下一最佳动作"));
    ...
    nextBestActionButton_->setProperty("action", nextBestActionKey(NextBestActionKind::Connect));
...
void MainWindow::rebuildUi() {
    ...
    const QString notes = rawText_->projectNotes ? rawText_->projectNotes->toPlainText() : QString();
    ...
    const QString sdoSubIndex = sdoInspector_->sdoSubIndex ? sdoInspector_->sdoSubIndex->text() : QString("0x00");
    const QString sdoWriteValue = sdoInspector_->sdoWriteValue ? sdoInspector_->sdoWriteValue->text() : QString();
    ...
    const QList<QStringList> sdoTargetTrailRows = copyTableRows(sdoTargetTrailTable_);
    const QList<QStringList> objectBookmarkRows = copyTableRows(bookmark_->objectBookmarkTable);
    const QList<QStringList> watchRows = copyTableRows(watch_->watchTable);
    const bool watchAutoRefresh = watch_->watchAutoRefresh ? watch_->watchAutoRefresh->isChecked() : false;
    const int watchInterval = watch_->watchRefreshInterval ? watch_->watchRefreshInterval->currentData().toInt() : 1000;
    const QString watchFilter = watch_->watchFilter ? watch_->watchFilter->text() : QString();
    const QString watchScope = watch_->watchScopeFilter ? watch_->watchScopeFilter->currentData().toString() : "all";
    const bool watchChangedOnly = watch_->watchChangedOnly ? watch_->watchChangedOnly->isChecked() : false;
    ...
    const QString workflowFilter = workflow_->workflowFilter ? workflow_->workflowFilter->text() : QString();
    const QString workflowScope = workflow_->workflowScopeFilter
                                      ? workflow_->workflowScopeFilter->currentData().toString()
                                      : QStringLiteral("all");
    const QString pdoFilter = sdo_->pdoFilter ? sdo_->pdoFilter->text() : QString();
    const QString freeRunFilter = freeRunWidgets_->freeRunFilter ? freeRunWidgets_->freeRunFilter->text() : QString();
    ...
    const QString diagnosticsLevel = diagnostics_->diagnosticsLevelFilter
    ...
    if (!sdoTargetTrailRows.isEmpty()) {
        ...
        for (int row = 0; row < sdoTargetTrailRows.size(); ++row) {
            ...
            for (int column = 0; column < sdoTargetTrailTable_->columnCount(); ++column) {
                sdoTargetTrailTable_->setItem(
                    // Create table cell
                    row, column, new QTableWidgetItem(values.value(column)));
            }
            rememberedSdoTargetTrailKeys_.insert(sdoTargetTrailRowKeyFromTable(sdoTargetTrailTable_, row));
        }
    ...
    if (!currentSlaves.isEmpty()) {
        ...
        if (previousSelectedPosition >= 0 && topologyTree_) {
            ...
            for (int top = 0; top < topologyTree_->topLevelItemCount() && !restoredSelection; ++top) {
                ...
                for (int child = 0; child < masterItem->childCount(); ++child) {
                    ...
                    if (slaveItem && slaveItem->data(0, Qt::UserRole).toInt() == previousSelectedPosition) {
    ...
    loadedSlaveInfoPosition_ =
        !lastSlaveInfoText_.isEmpty() && previousLoadedSlaveInfoPosition == currentSelectedPosition
            ? previousLoadedSlaveInfoPosition
            : -1;
    loadedPdoPosition_ = !lastPdoText_.isEmpty() && previousLoadedPdoPosition == currentSelectedPosition
                             ? previousLoadedPdoPosition
                             : -1;
    // Preserve loaded position only if the selection hasn't changed
    loadedSdoPosition_ = !lastSdoText_.isEmpty() && previousLoadedSdoPosition == currentSelectedPosition
                             // Preserve loaded position only if the selection hasn't changed
                             ? previousLoadedSdoPosition
                             : -1;
    loadedXmlPosition_ = !lastXmlText_.isEmpty() && previousLoadedXmlPosition == currentSelectedPosition
    ...
    if (!objectBookmarkRows.isEmpty()) {
        ...
        for (int row = 0; row < objectBookmarkRows.size(); ++row) {
            ...
            for (int column = 0; column < bookmark_->objectBookmarkTable->columnCount(); ++column) {
    ...
    if (!watchRows.isEmpty()) {
        ...
        for (int row = 0; row < watchRows.size(); ++row) {
            ...
            if (legacySixColumnWatch) {
                migrated = {values.value(0), values.value(1), values.value(2), values.value(3), values.value(4),
                            QString(),       QString(),       values.value(5), QString(),       QString()};
                // Migrate older watch formats to current layout
            } else if (legacySevenColumnWatch) {
                migrated = {values.value(0), values.value(1), values.value(2), values.value(3), values.value(4),
                            QString(),       values.value(5), values.value(6), QString(),       QString()};
            } else {
                migrated = values;
            }
    ...
    if (!startupRows.isEmpty()) {
        setTableRows(startupSdoTable_,
                     {uiText("Slave", "从站"), uiText("Index", "索引"), uiText("Sub", "子项"), uiText("Value", "值"),
                      uiText("Type", "类型"), uiText("Status", "状态"), uiText("Detail", "详情"),
                      uiText("Watch Value", "Watch 值"), uiText("Watch Delta", "Watch 偏差")},
                     startupRows);
        ensureStartupSdoTable();
    ...
    if (!diagnosticsRows.isEmpty()) {
        setTableRows(
            diagnostics_->diagnosticsTable,
            {uiText("Time", "时间"), uiText("Level", "级别"), uiText("Source", "来源"), uiText("Message", "消息")},
            diagnosticsRows);
