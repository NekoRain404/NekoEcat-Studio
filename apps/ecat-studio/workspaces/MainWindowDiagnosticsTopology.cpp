...
void MainWindow::captureTopologyBaseline() {
    if (slaves_.isEmpty()) {
        updateDiagnostics("Warning", "Topology",
                          uiText("Cannot capture baseline: no slaves detected", "无法捕获基线：未检测到从站"));
        return;
    }
    ...
    updateDiagnostics("Info", "Topology",
                      QString("Captured topology baseline: %1 slave(s)").arg(topologyBaseline_.size()));
}
...
void MainWindow::exportDiagnosticsReport() {
    const QString path = QFileDialog::getSaveFileName(
        ...
        QDir::home().absoluteFilePath(
            QString("ethercat-report-%1.md").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss"))),
    ...
    if (hostHealthTable_) {
        for (int row = 0; row < hostHealthTable_->rowCount(); ++row) {
            const QString level = hostHealthTable_->item(row, 0) ? hostHealthTable_->item(row, 0)->text() : QString();
            if (level == "Error") {
                ++hostErrors;
    ...
    if (sdo_->sdoTable) {
        for (int row = 0; row < sdo_->sdoTable->rowCount(); ++row) {
            ...
            if (dictionary.status.contains("failed", Qt::CaseInsensitive) || dictionary.status.contains("失败")) {
                ++odFailedRows;
            }
        }
    }
    ...
    out << "- Topology Baseline: "
        << (topologyBaseline_.isEmpty() ? "not captured" : QString("%1 slave(s)").arg(topologyBaseline_.size()))
        << "\n";
    out << "- Watch Items: " << (watch_->watchTable ? watch_->watchTable->rowCount() : 0) << "\n";
    out << "- I/O Variables: " << (ioVar_->ioVariableTable ? ioVar_->ioVariableTable->rowCount() : 0) << "\n";
    out << "- Startup SDO Rows: " << (startupSdoTable_ ? startupSdoTable_->rowCount() : 0) << "\n";
    out << "- Object Dictionary Evidence: " << odEvidenceRows << " row(s), " << odFailedRows << " failed\n";
    out << "- Free Run: " << lastFreeRunStatus_ << "\n";
    out << "- Host Health: " << hostErrors << " error(s), " << hostWarnings << " warning(s), " << hostOk << " OK\n\n";
    out << "## Host Health\n\n";
    ...
    for (const auto& slave : slaves_) {
        out << "- #" << slave.position << " " << slave.state << " " << slave.flags << " " << slave.name << "\n";
    }
    out << "\n## Topology Baseline\n\n";
    if (topologyBaseline_.isEmpty()) {
        ...
        out << "\nBaseline Check: " << (issues.isEmpty() ? "matches current topology" : issues.join("; ")) << "\n\n";
    }
    out << "## Watch\n\n";
    ...
    if (!sdo_->sdoTable || sdo_->sdoTable->rowCount() <= 0) {
        ...
        for (const int column : columns) {
            ...
            out << " " << markdownCell(header ? header->text() : QString("Column %1").arg(column + 1)) << " |";
        }
        out << "\n|";
...
void MainWindow::filterDiagnosticsTable() {
    ...
    for (int row = 0; row < diagnostics_->diagnosticsTable->rowCount(); ++row) {
        ...
        if (match && !needle.isEmpty()) {
            match = false;
            for (int column = 0; column < diagnostics_->diagnosticsTable->columnCount() && !match; ++column) {
...
void MainWindow::updateSelectedSlavePanel() {
    if (!selectedSlaveNameLabel_ || !selectedSlaveStateLabel_ || !selectedSlaveFlagsLabel_ ||
        !selectedSlaveHintLabel_) {
        return;
    }

    const int position = selectedPosition();
    if (position < 0) {
        selectedSlaveNameLabel_->setText(uiText("No slave selected", "尚未选择从站"));
        ...
        if (selectedDriveSummaryLabel_) {
            selectedDriveSummaryLabel_->setText(uiText("Drive: no Watch evidence", "驱动：暂无监视证据"));
        ...
        selectedSlaveHintLabel_->setText(uiText("Select a slave in the I/O tree to enable contextual actions.",
    ...
    if (!found) {
        ...
        selectedSlaveHintLabel_->setText(
            uiText("Refresh online data to recover the selected slave context.", "刷新在线数据以恢复选中从站上下文。"));
        return;
    }

    const QString name =
        selected.name.trimmed().isEmpty() ? uiText("Unnamed slave", "未命名从站") : selected.name.trimmed();
    selectedSlaveNameLabel_->setText(QString("#%1  %2").arg(selected.position).arg(name));
    selectedSlaveStateLabel_->setText(
        uiText("State: %1", "状态：%1")
            .arg(selected.state.trimmed().isEmpty() ? uiText("Unknown", "未知") : selected.state.trimmed()));
    selectedSlaveFlagsLabel_->setText(
        uiText("Flags: %1", "标志：%1")
            .arg(selected.flags.trimmed().isEmpty() ? uiText("none", "无") : selected.flags.trimmed()));
    ...
    if (!client_.isConnected()) {
        ...
        hint = uiText("Operational. Prefer Watch and Free Run; review outputs before "
                      "writing.",
        ...
        hint = uiText("Review diagnostics and raw slave output before changing state.",
                      "切换状态前先查看诊断和从站原始输出。");
    }
...
void MainWindow::updateSlaveEvidenceSummary() {
    ...
    const SlaveEvidenceSummaryTexts texts = slaveEvidenceSummaryTexts();
    ...
    applyLoadedSlaveEvidence(&input,
...
void MainWindow::updateSelectedDriveSummary() {
    ...
    // Lambda to push UI state changes to the label widget
    auto applyState = [this](const SelectedDriveSummaryDetail& state) {
    ...
    applyState(buildSelectedDriveSummaryDetail(watchStartupWatchRows(watch_->watchTable), position, texts));
    updateDriveNextButton();
}
...
void MainWindow::updateDriveNextButton() {
    ...
    const bool hasRecommendation =
        client_.isConnected() && selectedPosition() >= 0 && recommendedCia402Controlword(&label, &value, &reason);
    ...
    button->setToolTip(uiText("Add or refresh the CiA 402 Watch preset for the selected slave "
                              "to derive "
    ...
    for (const auto& issue : compareTopologyBaseline(topologyBaseline_, slaves_)) {
        ...
        switch (issue.kind) {
            case TopologyBaselineIssueKind::MissingSlave:
                issues
                    << uiText("Missing slave #%1 %2", "缺失从站 #%1 %2").arg(issue.position).arg(issue.baseline.name);
                break;
            case TopologyBaselineIssueKind::NameChanged:
                issues << uiText("Slave #%1 name changed: %2 -> %3", "从站 #%1 名称变化：%2 -> %3")
                              .arg(issue.position)
                              .arg(issue.baseline.name, issue.current.name);
                break;
            case TopologyBaselineIssueKind::StateChanged:
                issues << uiText("Slave #%1 state differs: %2 -> %3", "从站 #%1 状态不同：%2 -> %3")
                              .arg(issue.position)
                              .arg(issue.baseline.state, issue.current.state);
                break;
            case TopologyBaselineIssueKind::UnexpectedSlave:
                issues
                    << uiText("Unexpected slave #%1 %2", "额外从站 #%1 %2").arg(issue.position).arg(issue.current.name);
                break;
        }
    }
...
void MainWindow::updateTopologyBaselineSummary() {
    ...
    if (topologyBaseline_.isEmpty()) {
        diagnostics_->topologyBaselineLabel->setText(uiText("No topology baseline", "未设置拓扑基线"));
        return;
    }
    const QStringList issues = topologyBaselineIssues();
    diagnostics_->topologyBaselineLabel->setText(
        issues.isEmpty()
            ? uiText("Baseline: %1 slave(s), current topology matches", "基线：%1 个从站，当前拓扑匹配")
                  .arg(topologyBaseline_.size())
            : uiText("Baseline: %1 issue(s) against %2 expected slave(s)", "基线：发现 %1 个问题，期望 %2 个从站")
                  .arg(issues.size())
                  .arg(topologyBaseline_.size()));
}
