...
QStringList MainWindow::ioVariableHandoffIssueLabels(const QStringList& keys) const {
    QStringList labels;
...
QString MainWindow::ioVariablePlcQuality(int row, const QSet<QString>* duplicateSymbols, QString* symbol) const {
    ...
    const QStringList keys = ioVariableHandoffIssueKeys(ioVariableHandoffIssues(variable, duplicateSymbols));
    return keys.isEmpty() ? uiText("Ready", "就绪") : ioVariableHandoffIssueLabels(keys).join(" | ");
}
...
QVector<int> MainWindow::plcHandoffIssueRows(const QVector<int>& rows) const {
    ...
    for (const int row : rows) {
        ...
        const QString quality = ioVariablePlcQuality(row, &duplicateSymbols, nullptr);
...
QStringList MainWindow::plcHandoffIssueDetails(const QVector<int>& rows, int previewLimit, int totalRows) const {
    QStringList details;
    ...
    for (const int row : rows) {
        ...
        const IoVariableTableRow variable = ioVariableTableRowFromTable(ioVar_->ioVariableTable, row);
        const QVector<IoVariableHandoffIssue> issues = ioVariableHandoffIssues(variable, &duplicateSymbols);
        const QStringList issueKeys = ioVariableHandoffIssueKeys(issues);
        const QString quality =
            issueKeys.isEmpty() ? uiText("Ready", "就绪") : ioVariableHandoffIssueLabels(issueKeys).join(" | ");
        const QString symbol = ioVariableHandoffName(variable).symbol;
        if (ioVariableHandoffHasIssue(issues, IoVariableHandoffIssue::MissingAlias)) {
            ++missingAlias;
        }
        ...
        if (ioVariableHandoffHasIssue(issues, IoVariableHandoffIssue::DuplicateSymbol)) {
            ++duplicateSymbol;
        }
        if (preview.size() < limit) {
            preview << QString("#%1 %2:%3  %4  -> %5  [%6]")
                           .arg(variable.positionValid ? QString::number(variable.position) : QStringLiteral("?"),
                                variable.index, variable.subIndex, symbol, quality);
        }
    }

    details << uiText("Rows with PLC handoff issues: %1 / %2", "存在 PLC 交接问题的行：%1 / %2")
                   .arg(rows.size())
                   .arg(totalRows >= 0 ? totalRows : ioVar_->ioVariableTable->rowCount());
    QStringList issueCounts;
    if (missingAlias > 0) {
        issueCounts << uiText("missing alias %1", "缺少 Alias %1").arg(missingAlias);
    }
    if (autoName > 0) {
    ...
    if (duplicateSymbol > 0) {
        issueCounts << uiText("duplicate symbol %1", "符号重复 %1").arg(duplicateSymbol);
    }
    if (!issueCounts.isEmpty()) {
        details << uiText("Issue types: %1", "问题类型：%1").arg(issueCounts.join("; "));
    }
    ...
    if (rows.size() > limit) {
        details << uiText("...and %1 more issue row(s)", "...另有 %1 条问题行").arg(rows.size() - limit);
    }
    return details;
}
...
QString MainWindow::plcDeclarationBlock(const QVector<int>& rows) const {
    ...
    for (const int row : rows) {
        ...
        variables << variable;
        qualityLabelsByRow << (quality == uiText("Ready", "就绪") ? QStringList() : quality.split(" | "));
    }
...
void MainWindow::editSelectedIoVariableMetadata() {
    ...
    aliasEdit->setPlaceholderText(uiText("Engineering name, for example Axis X Statusword", "工程名，例如 X 轴状态字"));
    auto* tagsEdit = new QLineEdit(metadata.value(1));
    tagsEdit->setPlaceholderText(uiText("Comma-separated tags, for example axis-x, cia402, safety",
                                        "逗号分隔标签，例如 axis-x, cia402, safety"));
    auto* noteEdit = new QPlainTextEdit;
    noteEdit->setPlainText(metadata.value(2));
    noteEdit->setPlaceholderText(uiText("Optional commissioning note", "可选调试备注"));
    ...
    updateDiagnostics("Info", "I/O Variables",
                      uiText("Updated I/O variable metadata for %1", "已更新 I/O 变量元数据：%1").arg(key));
}
// ── Bulk Naming ─────────────────────────────────────────────────────
...
void MainWindow::bulkNameIoVariables() {
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Bulk Name I/O Variables", "批量命名 I/O 变量"),
                                 uiText("No I/O variables are available.", "当前没有可命名的 I/O 变量。"));
        return;
    }
    ...
    if (selectedRows.isEmpty() && visibleRows.isEmpty()) {
        updateDiagnostics("Warning", "I/O Variables",
                          uiText("No visible I/O variable rows are available", "当前没有可见 I/O 变量行"));
        return;
    }
    ...
    auto* summary = new QLabel(uiText("Generate project-local aliases and tags. This does not read or "
    ...
    if (!selectedRows.isEmpty()) {
        scopeCombo->addItem(uiText("Selected rows (%1)", "所选行 (%1)").arg(selectedRows.size()), "selected");
    }
    scopeCombo->addItem(uiText("Visible rows (%1)", "可见行 (%1)").arg(visibleRows.size()), "visible");
    auto* prefixEdit = new QLineEdit;
    prefixEdit->setPlaceholderText(
        uiText("Optional prefix, for example MachineA or Line1", "可选前缀，例如 MachineA 或 Line1"));
    auto* tagsEdit = new QLineEdit(uiText("plc,handoff", "plc,交接"));
    tagsEdit->setPlaceholderText(uiText("Tags appended to every row", "追加到每行的标签"));
    auto* includeAddress =
        new QCheckBox(uiText("Include slave/index in generated alias", "生成 Alias 时包含从站和索引"));
    includeAddress->setChecked(true);
    auto* protectExisting = new QCheckBox(uiText("Keep existing aliases", "保留已有 Alias"));
    protectExisting->setChecked(true);
    auto* addDirectionTags = new QCheckBox(uiText("Add direction/type tags", "添加方向/类型标签"));
    ...
    QVector<int> rows = scopeCombo->currentData().toString() == "selected" ? selectedRows : visibleRows;
    if (rows.isEmpty()) {
        return;
    }
    ...
    const int existingAliases = countIoVariableBulkNamingExistingAliases(namingRows);
    if (!keepExisting && existingAliases > 0) {
        const QStringList details = {
            uiText("%1 existing alias(es) will be replaced.", "%1 个已有 Alias 将被覆盖。").arg(existingAliases),
            uiText("The change is project-local metadata only.", "此更改只影响工程内元数据。")};
        ...
        if (!confirmDangerousOperation(
                ...
                uiText("Bulk naming is about to replace existing aliases.", "批量命名将覆盖已有 Alias。"), details,
                uiText("Replace Aliases", "覆盖 Alias"))) {
            return;
        }
    }
    ...
    const IoVariableBulkNamingResult result =
        buildIoVariableBulkNamingPlan(allRows, rows, ioVariableMetadata_, options);
    for (auto it = result.metadataUpdates.cbegin(); it != result.metadataUpdates.cend(); ++it) {
...
void MainWindow::reviewPlcHandoffIssues() {
    updateIoVariableTable();
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Review PLC Handoff Issues", "审阅 PLC 交接问题"),
                                 uiText("No I/O variables are available.", "当前没有可审阅的 I/O 变量。"));
        return;
    }
    ...
    if (visibleRows.isEmpty()) {
        QMessageBox::information(
            ...
            uiText("No visible I/O variables match the current filter.", "当前筛选条件下没有可见 I/O 变量。"));
        return;
    }
    ...
    updateDiagnostics(
        "Info", "I/O Variables",
        uiText("PLC handoff review opened: %1 issue row(s)", "已打开 PLC 交接复核：%1 条问题行").arg(issueRows.size()));
}


// — Focus plc handoff issue rows
void MainWindow::focusPlcHandoffIssueRows(const QVector<int>& issueRows, bool showReadyMessage) {
    if (!ioVar_->ioVariableTable) {
        return;
    }
    ...
    if (selectedRow >= 0) {
        ...
        QMessageBox::information(
            this, uiText("Review PLC Handoff Issues", "审阅 PLC 交接问题"),
            uiText("All visible PLC handoff rows are ready.", "当前 I/O 变量没有 PLC 交接质量问题。"));
...
void MainWindow::copyIoVariablePlcDeclarations(bool selectedOnly) {
    updateIoVariableTable();
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Copy PLC Declarations", "复制 PLC 声明"),
                                 uiText("No I/O variables are available.", "当前没有可复制的 I/O 变量。"));
        return;
    }

    const QVector<int> rows = selectedOnly ? selectedIoVariableRows(true) : visibleIoVariableRows();
    if (rows.isEmpty()) {
        QMessageBox::information(
            this, uiText("Copy PLC Declarations", "复制 PLC 声明"),
            selectedOnly
                ? uiText("No selected visible I/O variables are available.", "当前没有选中的可见 I/O 变量。")
                : uiText("No visible I/O variables match the current filter.", "当前筛选条件下没有可见 I/O 变量。"));
        return;
    }
    if (!confirmPlcHandoffOperation(rows, uiText("copy PLC declarations", "复制 PLC 声明"),
...
void MainWindow::clearSelectedIoVariableMetadata() {
    ...
    updateDiagnostics("Info", "I/O Variables",
                      // ── CSV Export ──────────────────────────────────────────────────────
                      uiText("Cleared metadata for %1 I/O variable(s)", "已清除 %1 条 I/O 变量元数据").arg(cleared));
}
...
void MainWindow::exportIoVariablesCsv() {
    updateIoVariableTable();
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export I/O Variables", "导出 I/O 变量"),
                                 uiText("No I/O variables are available to export.", "当前没有可导出的 I/O 变量。"));
        return;
    }
    const QString path = QFileDialog::getSaveFileName(
        this, uiText("Export I/O Variables CSV", "导出 I/O 变量 CSV"),
        QDir::home().absoluteFilePath(
            QString("ethercat-io-variables-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss"))),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty()) {
        return;
    }
    ...
    for (int column = 0; column < ioVar_->ioVariableTable->columnCount(); ++column) {
        ...
        headers.append(csvCell(header ? header->text() : QString("Column %1").arg(column + 1)));
    }
    out << headers.join(',') << '\n';
    int exported = 0;
    // Iterate all rows and apply active filter predicates
    ...
    updateDiagnostics(
        "Info", "I/O Variables",
        uiText("Exported %1 visible I/O variable row(s): %2", "已导出 %1 条可见 I/O 变量：%2").arg(exported).arg(path));
}
...
void MainWindow::exportIoVariablesPlcCsv() {
    updateIoVariableTable();
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export PLC Symbols", "导出 PLC 符号"),
                                 uiText("No I/O variables are available to export.", "当前没有可导出的 I/O 变量。"));
        return;
    }
    ...
    if (!confirmPlcHandoffOperation(rows, uiText("export PLC symbols", "导出 PLC 符号"),
    ...
    const QString path = QFileDialog::getSaveFileName(
        ...
        QDir::home().absoluteFilePath(
            QString("ethercat-plc-symbols-%1.csv").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss"))),
        "CSV (*.csv);;Text (*.txt)");
    if (path.isEmpty()) {
        return;
    }
    ...
    for (const int row : rows) {
        const IoVariableTableRow variable = ioVariableTableRowFromTable(ioVar_->ioVariableTable, row);
        const QStringList values = ioVariableHandoffCsvRow(variable, &usedSymbols, exportedAt).values;
        QStringList cells;
    ...
    updateDiagnostics(
        "Info", "I/O Variables",
        uiText("Exported %1 visible PLC symbol row(s): %2", "已导出 %1 条可见 PLC 符号：%2").arg(exported).arg(path));
}
...
void MainWindow::exportIoVariablesPlcDeclarationsSt() {
    updateIoVariableTable();
    if (!ioVar_->ioVariableTable || ioVar_->ioVariableTable->rowCount() <= 0) {
        QMessageBox::information(this, uiText("Export PLC Declarations", "导出 PLC 声明"),
                                 uiText("No I/O variables are available to export.", "当前没有可导出的 I/O 变量。"));
        return;
    }
    const QVector<int> rows = visibleIoVariableRows();
    ...
    if (!confirmPlcHandoffOperation(rows, uiText("export PLC declarations", "导出 PLC 声明"),
                                    uiText("Continue Export", "继续导出"))) {
        return;
    }
    ...
    const QString path = QFileDialog::getSaveFileName(
        ...
        QDir::home().absoluteFilePath(
            QString("ethercat-plc-declarations-%1.st").arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss"))),
...
bool MainWindow::confirmPlcHandoffOperation(const QVector<int>& rows, const QString& operation,
    ...
    auto* review = box.addButton(uiText("Review Issues", "审阅问题"), QMessageBox::ActionRole);
    auto* continueOperation = box.addButton(continueText, QMessageBox::AcceptRole);
    ...
    if (box.clickedButton() == review) {
        updateDiagnostics(
            "Info", "I/O Variables",
            uiText("PLC handoff operation paused for issue review: %1", "PLC 交接操作已暂停，进入问题复核：%1")
                .arg(operation));
    ...
    updateDiagnostics("Info", "I/O Variables",
                      uiText("PLC handoff operation cancelled: %1", "PLC 交接操作已取消：%1").arg(operation));
    return false;
}
