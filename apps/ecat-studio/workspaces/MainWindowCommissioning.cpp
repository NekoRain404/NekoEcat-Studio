...
CommissioningWorkflowInput MainWindow::commissioningWorkflowInput() const {
    ...
    input.hasSdoRows =
        input.hasSelectedSlave && loadedSdoPosition_ == selected && sdo_->sdoTable && sdo_->sdoTable->rowCount() > 0;
    input.hasPdoRows =
        input.hasSelectedSlave && loadedPdoPosition_ == selected && sdo_->pdoTable && sdo_->pdoTable->rowCount() > 0;
    input.hasWatchRows = watch_->watchTable && watch_->watchTable->rowCount() > 0;
    input.hasFreeRunRows = freeRunWidgets_->freeRunEntryTable && freeRunWidgets_->freeRunEntryTable->rowCount() > 0;
    ...
    input.hasConsistencyBlockingIssues = consistencyHasBlockingIssues({consistencyErrors, consistencyWarnings, 0, 0});
...
void MainWindow::updateCommissioningWorkflow() {
    ...
    const QVector<CommissioningWorkflowStepState> stepStates = buildCommissioningWorkflowStepStates(workflowInput);
    const int selected = selectedPosition();
    int consistencyErrors = 0;
    int consistencyWarnings = 0;
    ...
    auto addWorkflowRow = [&workflowRows](const QString& phase, CommissioningWorkflowStatus status, const QString& step,
                                          const QString& risk, const QString& evidence, const QString& action) {
        workflowRows.append({phase, status, step, risk, evidence, action});
    };

    addWorkflowRow(uiText("Runtime", "运行时"),
                   commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ConnectRuntime),
                   uiText("Connect runtime", "连接运行时"),
                   workflowInput.connected ? uiText("None", "无") : uiText("Runtime offline", "运行时离线"),
                   workflowInput.connected ? uiText("ecatd is connected", "ecatd 已连接")
    ...
    addWorkflowRow(uiText("Runtime", "运行时"),
                   commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ScanTopology),
                   uiText("Scan bus topology", "扫描总线拓扑"),
                   workflowInput.hasSlaves ? uiText("None", "无")
                                           // ── Commissioning Workflow Input ────────────────────────────────────
                                           : (workflowInput.connected ? uiText("No topology", "无拓扑")
                                                                      : uiText("Runtime offline", "运行时离线")),
                   workflowInput.hasSlaves ? uiText("%1 slave(s) detected", "检测到 %1 个从站").arg(slaves_.size())
                                           : uiText("No slaves in current scan", "当前扫描没有从站"),
                   workflowInput.connected ? uiText("Refresh or Rescan", "刷新或重新扫描")
                                           : uiText("Connect runtime first", "先连接运行时"));
    addWorkflowRow(
        uiText("Target", "目标"), commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::SelectSlave),
        uiText("Select a slave", "选择从站"),
        workflowInput.hasSelectedSlave
            ? uiText("None", "无")
            : (workflowInput.hasSlaves ? uiText("No target", "无目标") : uiText("No topology", "无拓扑")),
        workflowInput.hasSelectedSlave ? QString("#%1").arg(selected) : uiText("No slave selected", "尚未选择从站"),
        workflowInput.hasSlaves ? uiText("Pick a slave in the I/O tree", "在 I/O 树中选择从站")
                                : uiText("Scan topology first", "先扫描拓扑"));
    addWorkflowRow(uiText("Evidence", "证据"),
                   commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::InspectObjectDictionary),
                   uiText("Inspect Object Dictionary", "检查对象字典"),
                   workflowInput.hasSdoRows ? uiText("None", "无") : uiText("OD missing", "OD 缺失"),
                   workflowInput.hasSdoRows
                       ? uiText("%1 object row(s)", "%1 个对象条目").arg(sdo_->sdoTable->rowCount())
                       : uiText("Object data not loaded", "对象数据尚未加载"),
                   workflowInput.hasSelectedSlave ? uiText("Open Object Dictionary", "打开对象字典")
                                                  : uiText("Select a slave first", "先选择从站"));
    addWorkflowRow(
        uiText("Evidence", "证据"),
        commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ReviewObjectDictionaryEvidence),
        uiText("Review OD evidence", "审阅 OD 证据"),
        workflowInput.hasFailedOdEvidence ? uiText("Failed mailbox evidence", "邮箱失败证据") : uiText("None", "无"),
        workflowInput.hasFailedOdEvidence ? uiText("Failed SDO evidence needs review", "存在需要复核的 SDO 失败证据")
                                          : uiText("No failed Object Dictionary evidence", "没有失败对象字典证据"),
        workflowInput.hasFailedOdEvidence ? uiText("Open Failed OD Evidence", "打开失败 OD 证据")
                                          : uiText("Continue", "继续"));
    addWorkflowRow(uiText("Map", "映射"),
                   commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ReviewPdoMap),
                   uiText("Review PDO Map", "核对 PDO 映射"),
                   workflowInput.hasPdoRows ? uiText("None", "无") : uiText("PDO missing", "PDO 缺失"),
                   workflowInput.hasPdoRows ? uiText("%1 PDO row(s)", "%1 个 PDO 条目").arg(sdo_->pdoTable->rowCount())
                                            : uiText("PDO data not loaded", "PDO 数据尚未加载"),
                   workflowInput.hasSelectedSlave ? uiText("Open PDO Map", "打开 PDO 映射")
                                                  : uiText("Select a slave first", "先选择从站"));
    addWorkflowRow(
        uiText("Watch", "监视"), commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::MonitorWatch),
        uiText("Monitor key SDOs", "监视关键 SDO"),
        workflowInput.hasWatchRows ? uiText("None", "无") : uiText("No live SDO evidence", "无实时 SDO 证据"),
        workflowInput.hasWatchRows ? uiText("%1 watch item(s)", "%1 个监视项").arg(watch_->watchTable->rowCount())
                                   : uiText("No watch items", "暂无监视项"),
        workflowInput.hasSelectedSlave ? uiText("Add current SDO to Watch", "添加当前 SDO 到监视")
                                       : uiText("Select a slave first", "先选择从站"));
    addWorkflowRow(
        uiText("Startup", "启动"),
        commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ReviewStartupDiffs),
        uiText("Review Startup diffs", "审阅启动偏差"),
        workflowInput.hasStartupWatchDiffs ? uiText("Startup mismatch", "启动值不一致") : uiText("None", "无"),
        workflowInput.hasStartupWatchDiffs ? uiText("Startup values differ from Watch", "启动值和 Watch 不一致")
                                           : uiText("No Startup Watch diffs", "没有 Startup Watch 偏差"),
        workflowInput.hasStartupWatchDiffs ? uiText("Open Startup Diffs", "打开启动偏差") : uiText("Continue", "继续"));
    addWorkflowRow(uiText("Gate", "门禁"),
                   commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::RunConsistencyGate),
                   uiText("Run consistency gate", "运行一致性门禁"),
                   !consistencyCheckAvailable()
                       ? uiText("Not run", "未运行")
                       : (!consistencyFresh_ ? uiText("Stale", "已过期")
                   ...
                   !consistencyCheckAvailable()
                       ...
                       : (!consistencyFresh_ ? uiText("Consistency check is stale", "一致性检查结果已过期")
                                             : (workflowInput.hasConsistencyBlockingIssues
                                                    ? uiText("Consistency has %1 error(s), %2 warning(s)",
                                                             "一致性检查有 %1 个错误、%2 个警告")
                                                          .arg(consistencyErrors)
                                                          .arg(consistencyWarnings)
                                                    : uiText("Consistency gate passed", "一致性门禁已通过"))),
                   workflowInput.hasConsistencyCheck && workflowInput.hasConsistencyBlockingIssues
                       ? uiText("Open Evidence", "打开证据")
                       : uiText("Refresh Consistency", "刷新一致性"));
    addWorkflowRow(
        uiText("Process", "过程"),
        commissioningWorkflowStepStatus(stepStates, CommissioningWorkflowStep::ValidateProcessImage),
        uiText("Validate process image", "验证过程映像"),
        workflowInput.freeRunEnabled || workflowInput.hasFreeRunRows ? uiText("None", "无")
                                                                     : uiText("No process image", "无过程映像"),
        workflowInput.freeRunEnabled
            ? uiText("Free Run active", "自由运行中")
            : (workflowInput.hasFreeRunRows
                   ? uiText("%1 process entries", "%1 个过程项").arg(freeRunWidgets_->freeRunEntryTable->rowCount())
    ...
    for (int row = 0; row < workflow_->workflowTable->rowCount(); ++row) {
        ...
        const QColor color = uiRow.colorKey == QStringLiteral("ready")
                                 ? readyColor
                                 : (uiRow.colorKey == QStringLiteral("action") ? actionColor : blockedColor);
    ...
    fitTableColumnsToViewport(workflow_->workflowTable, kCommissioningWorkflowEvidenceColumn);
    ...
    if (workflow_->workflowSummaryLabel) {
        ...
        const QString nextText = nextStep >= 0 && !nextAction.isEmpty() ? nextAction : uiText("Ready", "已就绪");
        const int openItems = stats.action + stats.blocked;
        // Set display text
        workflow_->workflowSummaryLabel->setText(
            uiText("Readiness %1 | Next: %2 | Open %3", "就绪度 %1 | 下一步：%2 | 未完成 %3")
                .arg(percent, nextText)
                .arg(openItems));
        workflow_->workflowSummaryLabel->setProperty(
            "severity", stats.blocked > 0 ? "warning" : (stats.action > 0 ? "action" : "ok"));
        ...
        for (int row = 0; row < workflow_->workflowTable->rowCount(); ++row) {
            const QString statusKey = commissioningWorkflowStatusKeyForRow(workflow_->workflowTable, row);
            const QString status = workflow_->workflowTable->item(row, 1)
                                       ? workflow_->workflowTable->item(row, 1)->text().trimmed()
                                       : QString();
            if (statusKey == QStringLiteral("ready") || (statusKey.isEmpty() && status == uiText("Ready", "就绪"))) {
                continue;
            }
            ...
            const QString risk = workflow_->workflowTable->item(row, 3)
                                     ...
                                     : QString();
            const QString evidence = workflow_->workflowTable->item(row, 4)
                                         ? workflow_->workflowTable->item(row, 4)->text().trimmed()
                                         : QString();
            const QString actionText = workflow_->workflowTable->item(row, 5)
    ...
    if (auto* runNextButton = findChild<QPushButton*>("overviewRunNext")) {
        ...
        // Set display text
        runNextButton->setText(nextStep >= 0 && !nextAction.isEmpty() ? uiText("Next: %1", "下一步：%1").arg(nextAction)
                                                                      : uiText("Run Next", "执行下一步"));
        runNextButton->setToolTip(
            nextStep >= 0 && !nextAction.isEmpty()
                ? uiText("Run workflow step %1: %2", "执行工作流第 %1 步：%2").arg(nextStep + 1).arg(nextAction)
...
SlaveEvidenceUiTexts MainWindow::slaveEvidenceUiTexts() const {
    return {
        ...
        .topologyBaselineIssue = uiText("topology baseline issue", "拓扑基线问题"),
        ...
        .processRowsPattern = uiText("%1 row(s), %2 issue(s)", "%1 行，%2 个问题"),
        ...
        .watchValuesDetailPattern = uiText("Watch values: %1/%2", "Watch 有值：%1/%2"),
        .startupRowsDetailPattern = uiText("Startup rows: %1, diffs: %2", "Startup 行：%1，偏差：%2"),
        .processRowsDetailPattern = uiText("Process rows: %1, map issues: %2", "过程行：%1，映射问题：%2"),
        ...
        // ── Slave Evidence Summary Texts ─────────────────────────────────────
    };
}

SlaveEvidenceSummaryTexts MainWindow::slaveEvidenceSummaryTexts() const {
    return {
        .selectSlaveText = uiText("Evidence: select a slave", "证据：请选择从站"),
        .ready = uiText("ready", "就绪"),
        .missing = uiText("missing", "缺失"),
        .summaryPattern = uiText("Evidence %1/5 | ID %2 | OD %3 | PDO %4 | Watch %5/%6 "
                                 "values | Startup %7 diff %8 | Free %9 map issue %10",
                                 "证据 %1/5 | 身份 %2 | OD %3 | PDO %4 | Watch %5/%6 有值 | "
                                 "启动 %7 偏差 %8 | Free %9 映射问题 %10"),
        .scorePattern = uiText("Selected slave evidence score: %1/5", "选中从站证据完整度：%1/5"),
        .missingIdentity =
            uiText("Missing identity evidence: run Snapshot or Refresh.", "缺少身份信息：运行准备快照或刷新。"),
        .missingOd =
            uiText("Missing Object Dictionary evidence: load OD metadata.", "缺少对象字典证据：加载 OD 元数据。"),
        .missingPdo = uiText("Missing PDO evidence: load PDO Map.", "缺少 PDO 证据：加载 PDO 映射。"),
        .missingWatch =
            uiText("Missing live SDO values: add Watch rows and refresh.", "缺少实时 SDO 值：添加 Watch 并刷新。"),
        .missingProcess =
            uiText("No process-image evidence yet: run Free Run when safe.", "暂无过程映像证据：安全时运行 Free Run。"),
        .startupDiffPattern =
            uiText("Startup SDO has %1 Watch mismatch row(s).", "Startup SDO 有 %1 条 Watch 不一致。"),
        .mapIssuePattern = uiText("Free Run has %1 PDO map evidence issue(s).", "Free Run 有 %1 个 PDO 映射证据问题。"),
        .topologyIssuePattern = uiText("Topology baseline has %1 issue(s).", "拓扑基线有 %1 个问题。"),
    };
    // ── Selected Drive Summary Texts ─────────────────────────────────────
}
...
SelectedDriveSummaryTexts MainWindow::selectedDriveSummaryTexts() const {
    return {
        .noWatchEvidence = uiText("Drive: no Watch evidence", "驱动：暂无监视证据"),
        .noCia402Evidence = uiText("Drive: no CiA 402 Watch evidence", "驱动：暂无 CiA 402 监视证据"),
