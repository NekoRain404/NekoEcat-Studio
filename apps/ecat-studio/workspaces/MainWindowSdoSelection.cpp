...
void MainWindow::applySdoSelectionFromDictionary(int row, bool readAfterFill) {
    ...
    if (sdoInspector_->sdoValue) {
        ...
        sdoInspector_->sdoValue->setPlaceholderText(
            dictionary.value.isEmpty()
                ...
                : uiText("Last value from Object Dictionary evidence", "来自对象字典证据的最后值"));
    }
    ...
    updateSdoInspector(uiText("Object Dictionary", "对象字典"),
                       QString("%1 %2 %3 bit %4")
                           .arg(access.toUpper(), dictionary.name, dictionary.bits,
                                writable ? uiText("writable", "可写") : uiText("read-only", "只读")));
    ...
    updateDiagnostics("Info", "SDO",
                      QString("Selected object %1:%2 %3 %4 bit %5%6")
                          .arg(dictionary.index, sdoInspector_->sdoSubIndex->text(), access.toUpper(), dictionary.bits,
                               dictionary.name, writable ? QString() : uiText(" (write disabled)", "（已禁用写入）")));
...
void MainWindow::addSelectedDictionaryRowsToWatch() {
    ...
    addDictionaryRowsToWatch(rows, uiText("selected object dictionary row(s)", "选中对象字典行"));
}
...
void MainWindow::addVisibleDictionaryRowsToWatch() {
    ...
    addDictionaryRowsToWatch(rows, uiText("visible object dictionary row(s)", "可见对象字典行"));
}


// — Add dictionary rows to watch
void MainWindow::addDictionaryRowsToWatch(const QVector<int>& rows, const QString& sourceLabel) {
    ...
    const QString previousSubIndex = sdoInspector_->sdoSubIndex ? sdoInspector_->sdoSubIndex->text() : QString();
    ...
    const QString previousWriteValue = sdoInspector_->sdoWriteValue ? sdoInspector_->sdoWriteValue->text() : QString();
...
void MainWindow::applySdoSelectionFromPdoMap(int row, bool readAfterFill) {
    ...
    updateSdoInspector(uiText("PDO Map", "PDO 映射"),
                       QString("%1 %2 %3 bit %4").arg(pdoRow.syncManager, pdoRow.pdo, pdoRow.bits, pdoRow.name));
    rememberCurrentSdoTarget(uiText("PDO Map", "PDO 映射"),
                             QString("%1 %2 %3 bit %4").arg(pdoRow.syncManager, pdoRow.pdo, pdoRow.bits, pdoRow.name));

    updateDiagnostics("Info", "PDO",
                      QString("Selected PDO entry #%1 %2 %3 %4:%5 %6 bit %7")
                          .arg(position)
                          .arg(pdoRow.syncManager, pdoRow.pdo, pdoRow.index, sdoInspector_->sdoSubIndex->text(),
                               pdoRow.bits, pdoRow.name));
    ...
    updateActionAvailability();
    // ── SDO Selection from FreeRun Entry ─────────────────────────────────
}


// — Fill the SDO target panel from the selected Free Run row
void MainWindow::applySdoSelectionFromFreeRunEntry(int row, bool readAfterFill) {
    if (!freeRunWidgets_->freeRunEntryTable || row < 0 || row >= freeRunWidgets_->freeRunEntryTable->rowCount()) {
        return;
    }

    const FreeRunEntryTableRow entry = freeRunEntryTableRowFromTable(freeRunWidgets_->freeRunEntryTable, row);
    ...
    updateSdoInspector(uiText("Free Run", "自由运行"), QString("%1 %2").arg(entry.direction, entry.name));
    rememberCurrentSdoTarget(uiText("Free Run", "自由运行"), QString("%1 %2").arg(entry.direction, entry.name));

    updateDiagnostics("Info", "Free Run",
                      QString("Selected process entry #%1 %2 %3:%4 %5")
                          .arg(entry.position)
                          .arg(entry.direction, entry.index, entry.subIndex, entry.name));

    if (readAfterFill && client_.isConnected()) {
        requestSdoRead(entry.position, entry.index, entry.subIndex, uiText("Free Run", "自由运行"));
    }
    // ── SDO Selection from I/O Variable ──────────────────────────────────
    updateActionAvailability();
}
...
void MainWindow::applySdoSelectionFromIoVariable(int row, bool readAfterFill) {
    ...
    updateSdoInspector(uiText("I/O Variables", "I/O 变量"),
                       QString("%1 | %2 | %3").arg(variable.direction, variable.source, variable.symbol));
    rememberCurrentSdoTarget(uiText("I/O Variables", "I/O 变量"),
                             QString("%1 | %2 | %3").arg(variable.direction, variable.source, variable.symbol));
    updateDiagnostics("Info", "I/O Variables",
                      QString("Selected I/O variable #%1 %2 %3:%4 %5")
                          .arg(variable.position)
                          .arg(variable.direction, variable.index, variable.subIndex, variable.symbol));

    if (readAfterFill && client_.isConnected()) {
        requestSdoRead(variable.position, variable.index, variable.subIndex, uiText("I/O Variables", "I/O 变量"),
...
void MainWindow::addIoVariableRowsToWatch(const QVector<int>& rows, const QString& sourceLabel) {
    ...
    for (const int row : rows) {
        ...
        for (int watchRow = 0; watchRow < watch_->watchTable->rowCount(); ++watchRow) {
            const bool match =
                ...
                tableText(watch_->watchTable, watchRow, 2).compare(variable.index, Qt::CaseInsensitive) == 0 &&
                tableText(watch_->watchTable, watchRow, 3).compare(variable.subIndex, Qt::CaseInsensitive) == 0;
            if (match) {
                existing = watchRow;
                break;
            }
        ...
        watch_->watchTable->setItem(watchRow, 5,
                                    // Create table cell
                                    new QTableWidgetItem(variable.decoded.isEmpty()
        ...
        watch_->watchTable->setItem(
            watchRow, 7,
            // Create table cell
            new QTableWidgetItem(variable.meaning.isEmpty() ? "I/O Variables" : variable.meaning));
        // Create table cell
    ...
    updateDiagnostics("Info", "I/O Variables",
                      uiText("Watch %1: added %2, reused %3, skipped %4", "监视 %1：新增 %2，复用 %3，跳过 %4")
                          .arg(sourceLabel)
                          .arg(added)
                          .arg(reused)
                          .arg(skipped));
    if (added > 0 || reused > 0) {
        // ── Add I/O Variables to Watch ───────────────────────────────────────
        activateWorkspaceTab(watchTabIndex_);
    }
}
...
void MainWindow::addSelectedIoVariablesToWatch() {
    ...
    addIoVariableRowsToWatch(selectedIoVariableRows(false), uiText("selected I/O variables", "所选 I/O 变量"));
}
...
void MainWindow::addVisibleIoVariablesToWatch() {
    ...
    for (int row = 0; row < ioVar_->ioVariableTable->rowCount(); ++row) {
        ...
        // ── Add I/O Variables to Startup SDO ────────────────────────────────
    }
    addIoVariableRowsToWatch(rows, uiText("visible I/O variables", "可见 I/O 变量"));
}
...
void MainWindow::addSelectedIoVariablesToStartupSdo() {
    ...
    addIoVariableRowsToStartupSdo(rows, uiText("selected I/O variables", "所选 I/O 变量"));
}
...
void MainWindow::addVisibleIoVariablesToStartupSdo() {
    ...
    addIoVariableRowsToStartupSdo(rows, uiText("visible I/O variables", "可见 I/O 变量"));
}


// — Add io variable rows to startup sdo
void MainWindow::addIoVariableRowsToStartupSdo(const QVector<int>& rows, const QString& sourceLabel) {
    if (!ioVar_->ioVariableTable || rows.isEmpty()) {
        return;
    }
    ...
    for (const int ioRow : uniqueRows) {
        if (ioRow < 0 || ioRow >= ioVar_->ioVariableTable->rowCount() || ioVar_->ioVariableTable->isRowHidden(ioRow)) {
            ++skipped;
            continue;
        }

        const IoVariableTableRow variable = ioVariableTableRowFromTable(ioVar_->ioVariableTable, ioRow);
        const QString value = ioVariableTableRowStartupValue(variable);
        const QString valueSource = !variable.watch.isEmpty() ? uiText("Watch", "Watch") : uiText("Raw", "Raw");
        if (!ioVariableTableRowHasTarget(variable) || value.isEmpty()) {
            ++skipped;
            continue;
        }

        const QString key = QString("%1|%2|%3").arg(variable.position).arg(variable.index, variable.subIndex);
        ...
        for (int startupRow = 0; startupRow < startupSdoTable_->rowCount(); ++startupRow) {
            if (tableObjectAddressMatches(startupSdoTable_, startupRow, variable.position, variable.index,
    ...
    if (candidates.isEmpty()) {
        updateDiagnostics("Warning", "Startup SDO",
                          uiText("Startup creation skipped: %1 have no Watch or Raw value%2",
                                 "创建启动项已跳过：%1 没有 Watch 或 Raw 值%2")
                              .arg(sourceLabel, skipped > 0 ? uiText(", skipped %1 row(s)", "，跳过 %1 行").arg(skipped)
                                                            : QString()));
        return;
    }
    ...
    QStringList details = {
        ...
        uiText("Accepted I/O values: %1", "可使用 I/O 值：%1").arg(candidates.size()),
        uiText("Value sources: Watch %1, Raw fallback %2", "值来源：Watch %1，Raw 兜底 %2")
            .arg(watchValueRows)
            .arg(rawFallbackRows),
    ...
    for (int i = 0; i < previewRows; ++i) {
        ...
        details << QString("#%1  %2:%3 = %4  [%5, %6]%7")
                       ...
                       .arg(candidate.index, candidate.subIndex, candidate.value, target, candidate.valueSource,
                            candidate.symbol.isEmpty() ? QString() : QString("  %1").arg(candidate.symbol));
    }
    if (candidates.size() > previewRows) {
        details << uiText("...and %1 more I/O value(s)", "...另有 %1 个 I/O 值").arg(candidates.size() - previewRows);
    }
    if (skipped > 0) {
        details << uiText("Skipped rows without address or usable value: %1", "已跳过缺少地址或可用值的行：%1")
                       .arg(skipped);
    }
    if (duplicateSkipped > 0) {
        details << uiText("Skipped duplicate selected I/O address(es): %1", "已跳过重复选中 I/O 地址：%1")
                       .arg(duplicateSkipped);
    }

    // Safety gate: require explicit confirmation before bus write
    if (!confirmDangerousOperation(uiText("Confirm Startup from I/O Variables", "确认从 I/O 变量创建 Startup SDO"),
    ...
    for (const auto& candidate : candidates) {
        const QString detailSource = candidate.source.isEmpty()
        ...
        if (candidate.startupRows.isEmpty()) {
            ...
            startupSdoTable_->setItem(startupRow, 6,
                                      // Create table cell
                                      new QTableWidgetItem(uiText("Created from I/O Variable row %1 at %2 (%3)",
            ...
            ensureStartupCell(startupRow, 6)
                ->setText(
                    previousValue.compare(candidate.value, Qt::CaseInsensitive) == 0
                        ? uiText("Confirmed from I/O Variable row %1 at %2 (%3)", "由 I/O 变量第 %1 行在 %2 确认（%3）")
                              .arg(candidate.ioRow + 1)
                              .arg(timestamp, detailSource)
...
void MainWindow::applySdoSelectionFromWatch(int row, bool readAfterFill) {
    ...
    updateDiagnostics("Info", "Watch", QString("Selected watch item #%1 %2:%3").arg(position).arg(index, subIndex));
    // ── SDO Selection from History ───────────────────────────────────────
...
void MainWindow::applySdoSelectionFromHistory(int row, bool readAfterFill) {
    ...
    if (sdoInspector_->sdoWriteValue) {
        ...
        sdoInspector_->sdoWriteValue->setPlaceholderText(uiText("Value from SDO history", "来自 SDO 历史的值"));
    }
    updateSdoInspector(uiText("SDO History", "SDO 历史"), QString("%1 %2").arg(action, status));
    rememberCurrentSdoTarget(uiText("SDO History", "SDO 历史"), QString("%1 %2").arg(action, status));

    updateDiagnostics(
        "Info", "SDO History",
        QString("Selected history row #%1 %2:%3 %4 %5").arg(position).arg(index, subIndex, action, status));
    // ── SDO Selection from Startup ──────────────────────────────────────

    if (readAfterFill && client_.isConnected()) {
        requestSdoRead(position, index, subIndex, uiText("SDO History", "SDO 历史"), type);
    }
    updateActionAvailability();
}
...
void MainWindow::applySdoSelectionFromStartup(int row, bool readAfterFill) {
    ...
    if (sdoInspector_->sdoWriteValue) {
        sdoInspector_->sdoWriteValue->setEnabled(true);
        sdoInspector_->sdoWriteValue->setText(value);
        sdoInspector_->sdoWriteValue->setPlaceholderText(uiText("Startup SDO value", "Startup SDO 的值"));
    }
    updateSdoInspector(uiText("Startup SDO", "Startup SDO"),
                       status.isEmpty() ? uiText("Startup row %1", "启动项第 %1 行").arg(row + 1) : status);
    rememberCurrentSdoTarget(uiText("Startup SDO", "Startup SDO"),
                             status.isEmpty() ? uiText("Startup row %1", "启动项第 %1 行").arg(row + 1) : status);
    ...
    if (readAfterFill && client_.isConnected()) {
        requestSdoRead(position, index, subIndex, uiText("Startup SDO", "Startup SDO"), type);
    }
    updateActionAvailability();
}
...
void MainWindow::readSelectedDictionaryRows() {
    ...
    readDictionaryRows(rows, uiText("selected object dictionary row(s)", "选中对象字典行"), false);
}
...
void MainWindow::readVisibleDictionaryRows() {
    ...
    readDictionaryRows(rows, uiText("visible object dictionary row(s)", "可见对象字典行"), true);
}
...
void MainWindow::readFailedDictionaryRows() {
    ...
    if (rows.isEmpty()) {
        updateDiagnostics("Info", "SDO",
                          uiText("No failed Object Dictionary rows to retry", "没有需要重试的对象字典失败行"));
        return;
    }
    ...
    readDictionaryRows(rows, uiText("failed object dictionary row(s)", "失败对象字典行"), true);
}


// Issue SDO read requests for the specified Object Dictionary rows via ecatd
void MainWindow::readDictionaryRows(const QVector<int>& rows, const QString& sourceLabel, bool confirmLargeBatch) {
    if (!client_.isConnected() || selectedPosition() < 0 || !sdo_->sdoTable ||
    ...
    int skipped = 0;
    const QVector<SdoDictionaryReadObject> objects = sdoDictionaryReadObjectsFromRows(sdo_->sdoTable, rows, &skipped);
    if (objects.isEmpty()) {
        return;
    }
    ...
    if (confirmLargeBatch && objects.size() > 24) {
        QStringList details = {
            ...
            uiText("Objects to read: %1 %2", "将读取对象：%1 %2").arg(objects.size()).arg(sourceLabel),
            uiText("This request is limited to %1.", "本次请求范围限定为%1。").arg(sourceLabel),
            uiText("This sends one mailbox SDO upload per object.", "此操作会对每个对象发送一次 mailbox SDO 上传。"),
        };
        const int preview = std::min(static_cast<int>(objects.size()), 8);
        for (int i = 0; i < preview; ++i) {
            const auto& object = objects.at(i);
            details << (object.type.isEmpty() ? QString("%1:%2").arg(object.index, object.subIndex)
                                              : QString("%1:%2 %3").arg(object.index, object.subIndex, object.type));
        }
        if (objects.size() > preview) {
            details << uiText("...and %1 more object(s)", "...另有 %1 个对象").arg(objects.size() - preview);
        }
        // Safety gate: require explicit confirmation before bus write
        if (!confirmDangerousOperation(
                uiText("Confirm Visible OD Read", "确认读取可见 OD"),
                uiText("Read the currently visible Object Dictionary rows.", "读取当前过滤后可见的对象字典行。"),
    ...
    for (const auto& object : objects) {
        requestSdoRead(selectedPosition(), object.index, object.subIndex,
                       uiText("Object Dictionary %1", "对象字典%1").arg(sourceLabel), object.type);
    }
    updateDiagnostics("Info", "SDO",
...
void MainWindow::focusFailedSdoEvidence() {
    ...
    if (row < 0) {
        updateDiagnostics("Info", "SDO",
                          uiText("No failed Object Dictionary evidence to review", "没有需要审阅的对象字典失败证据"));
        return;
    }
    ...
    const SdoDictionaryRow dictionary = sdoDictionaryRowFromTable(sdo_->sdoTable, row);
    updateDiagnostics("Info", "SDO",
                      uiText("Focused failed Object Dictionary evidence %1:%2", "已聚焦失败对象字典证据 %1:%2")
...
void MainWindow::filterSdoTable(const QString& text) {
    ...
    for (int row = 0; row < sdo_->sdoTable->rowCount(); ++row) {
        ...
        const bool failedEvidence = lastStatus.contains(uiText("Failed", "失败"), Qt::CaseInsensitive) ||
        ...
        if (tagMode) {
            ...
            if (tag == "writable") {
                ...
                match = rowHasIndexPrefix(row, {"0x603f", "0x6040", "0x6041", "0x6060", "0x6061", "0x6064", "0x606c",
                                                "0x6071", "0x6077", "0x607a", "0x60ff"}) ||
                        objectText.contains("cia") || nameText.contains("controlword") ||
                        nameText.contains("statusword") || nameText.contains("mode") || nameText.contains("position") ||
                        nameText.contains("velocity") || nameText.contains("torque");
            } else if (tag == "identity") {
                match = rowHasIndexPrefix(row, {"0x1000", "0x1008", "0x1009", "0x100a", "0x1018"}) ||
                        objectText.contains("identity") || objectText.contains("device") ||
                        nameText.contains("vendor") || nameText.contains("product") || nameText.contains("revision") ||
                        nameText.contains("serial") || nameText.contains("name");
            } else if (tag == "pdo") {
                match = rowHasIndexPrefix(row, {"0x160", "0x1a0", "0x1c1", "0x1c2", "0x1c3"}) ||
                        objectText.contains("pdo") || nameText.contains("pdo") || nameText.contains("mapping") ||
                        nameText.contains("assignment");
            } else if (tag == "error") {
                match = rowHasIndexPrefix(
                            row, {"0x1001", "0x1002", "0x1003", "0x1010", "0x1011", "0x1029", "0x10f3", "0x603f"}) ||
                        objectText.contains("error") || objectText.contains("emergency") ||
                        objectText.contains("diagnostic") || nameText.contains("error") || nameText.contains("fault") ||
                        nameText.contains("emergency") || nameText.contains("diagnostic");
            ...
            for (int column = 0; column < sdo_->sdoTable->columnCount() && !match; ++column) {
                const auto* item = sdo_->sdoTable->item(row, column);
    ...
    if (sdo_->sdoSummaryLabel) {
        sdo_->sdoSummaryLabel->setText(uiText("%1/%2 | W %3 | E %4 | F %5", "%1/%2 | 可写 %3 | 证据 %4 | 失败 %5")
                                           .arg(visible)
                                           .arg(sdo_->sdoTable->rowCount())
                                           .arg(writableRows)
                                           .arg(evidenceRows)
        ...
        sdo_->sdoSummaryLabel->setToolTip(
            uiText("%1/%2 visible, %3 writable, %4 with evidence, %5 failed", "%1/%2 可见，%3 可写，%4 有证据，%5 失败")
                .arg(visible)
                .arg(sdo_->sdoTable->rowCount())
                .arg(writableRows)
                .arg(evidenceRows)
