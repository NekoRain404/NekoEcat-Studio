...
void MainWindow::log(const QString& message) {
    rawText_->logText->appendPlainText(
...
QVector<int> MainWindow::selectedDictionaryRows() const {
    if (!sdo_->sdoTable || selectedPosition() < 0 || loadedSdoPosition_ != selectedPosition()) {
        return {};
    }
...
void MainWindow::setSelectedSlave(int position) {
    ...
    for (const auto& slave : slaves_) {
        if (slave.position == position) {
            selectedLabel_->setText(QString("#%1  %2").arg(slave.position).arg(slave.name));
...
void MainWindow::reportTopologyChanges(const QVector<SlaveInfo>& previous, const QVector<SlaveInfo>& current) {
    const QVector<TopologyChange> changes = detectTopologyChanges(previous, current);
    if (changes.isEmpty()) {
        return;
    }
            ...
            case TopologyChangeKind::StateChanged: {
                ...
                updateDiagnostics(level, "Topology",
                                  QString("Slave #%1 state changed: %2 -> %3 (%4)")
                                      .arg(change.position)
                                      .arg(change.previous.state, change.current.state, change.current.name));
                break;
            }
...
void MainWindow::updateSlaves(const QVector<SlaveInfo>& slaves) {
    ...
    auto* master = new QTreeWidgetItem(
        topologyTree_, {activeMasterName(), QString("%1 %2").arg(slaves.size()).arg(uiText("slaves", "从站"))});
    master->setData(0, Qt::UserRole, -1);
    master->setExpanded(true);

    for (const auto& slave : slaves) {
        auto* item = new QTreeWidgetItem(master, {QString("#%1  %2").arg(slave.position).arg(slave.name), slave.state});
    ...
    setMetricCard(slaveCountLabel_, uiText("Slaves", "从站"), QString::number(slaves.size()));
    log(QString("Scan complete: %1 slaves").arg(slaves.size()));
    updateDiagnostics("Info", "Scan", QString("%1 slave(s) detected").arg(slaves.size()));
    updateSelectedSlavePanel();
...
void MainWindow::updateMasterSummary(const QString& text) {
    ...
    setMetricCard(masterStateLabel_, uiText("Master", "主站"), phase.isEmpty() ? uiText("Unknown", "未知") : phase);
    if (!slaves.isEmpty()) {
        setMetricCard(slaveCountLabel_, uiText("Slaves", "从站"), slaves);
    }
    setMetricCard(linkStateLabel_, uiText("Link", "链路"), link.isEmpty() ? uiText("Unknown", "未知") : link);
    setMetricCard(lossLabel_, uiText("Frame Loss", "丢帧"), loss.isEmpty() ? "0" : loss);
...
void MainWindow::updateSlaveInfo(const QString& text) {
    ...
    for (const QString& key :
         {"Vendor Id", "Product code", "Revision number", "Serial number", "Group", "Order number", "Device name"}) {
        const QString value = capture(text, QString(R"(^\s*%1:\s*(.+)$)").arg(QRegularExpression::escape(key)));
        if (!value.isEmpty()) {
    ...
    for (const auto& line : text.split('\n')) {
        ...
        if (inPortTable) {
            ...
            if (parts.size() >= 5 && parts.first().at(0).isDigit()) {
                ports.append({parts.value(0), parts.value(1), parts.value(2), parts.value(3), parts.value(4)});
    ...
    for (const QString& prefix : {"Bootstrap", "Standard", "Supported protocols"}) {
        const QString value = capture(
            // Update the PDO table from daemon response.
            text, QString(R"(^\s*%1\s*(.*)$)").arg(QRegularExpression::escape(prefix)));
        if (!value.isEmpty()) {
...
void MainWindow::updatePdoTable(const QString& text) {
    ...
    const QRegularExpression pdoRe(R"(^\s+(RxPDO|TxPDO)\s+(0x[0-9a-fA-F]+)\s+\"(.+)\")");
    ...
    for (const auto& line : text.split('\n')) {
        ...
        m = pdoRe.match(line);
        if (m.hasMatch()) {
            pdo = QString("%1 %2 %3").arg(m.captured(1), m.captured(2), m.captured(3));
            continue;
        }
        m = entryRe.match(line);
        if (m.hasMatch()) {
            rows.append({sm, pdo, m.captured(1), m.captured(2), m.captured(3), m.captured(4)});
        }
...
void MainWindow::filterPdoTable() {
    ...
    for (int row = 0; row < sdo_->pdoTable->rowCount(); ++row) {
        ...
        for (int column = 0; column < sdo_->pdoTable->columnCount() && !match; ++column) {
    ...
    if (sdo_->pdoSummaryLabel) {
        ...
        sdo_->pdoSummaryLabel->setText(total > 0 ? uiText("%1/%2 PDO entries", "%1/%2 个 PDO 条目")
...
void MainWindow::updatePdoRowDetail() {
    ...
    const PdoMapDetailTexts texts = {
        .unavailableText = uiText("PDO Map evidence is not available.", "当前没有可用的 PDO 映射证据。"),
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        .noSelectionText = uiText("Select a visible PDO entry to review Sync Manager, PDO, object "
                                  "address, bit width, name, inferred type, and operation boundary.",
                                  "选择一条可见 PDO 条目，以复核 Sync "
                                  "Manager、PDO、对象地址、位宽、名称、"
                                  "推断类型和操作边界。"),
        .noSelectionTip = uiText("Selecting rows, filtering, and reading this detail strip are local "
        ...
        .roleRxOutput = uiText("Output/process command candidate", "输出/过程命令候选"),
        .roleTxInput = uiText("Input/process feedback candidate", "输入/过程反馈候选"),
        ...
        .summaryPattern = uiText("%1 | %2:%3 | %4 bit %5 | %6 | %7", "%1 | %2:%3 | %4 bit %5 | %6 | %7"),
        ...
        .localBoundary = uiText("Local preview boundary: after PDO Map data is loaded, selecting "
                                ...
                                "连接，也不运行 Host Health。"),
        .executionBoundary = uiText("Execution boundary: Load/refresh PDO Map is explicit online PDO "
    ...
    applyState(buildPdoMapDetailState(pdoMapTableRowFromTable(sdo_->pdoTable, row), selectedPosition(), texts));
}
...
void MainWindow::updateSdoTable(const QString& text) {
    ...
    auto normalizeHex = [](QString text, int minimumDigits) {
        ...
        return QString("0x%1").arg(parsed, minimumDigits, 16, QLatin1Char('0')).toLower();
    };
    ...
    for (const auto& line : text.split('\n')) {
        ...
        if (m.hasMatch()) {
            ...
            const QString key =
                QString("%1|%2|%3").arg(selectedPosition()).arg(normalizeHex(index, 4), normalizeHex(subIndex, 2));
            const QStringList evidence = sdoEvidence_.value(key);
            rows.append({object, index, subIndex, m.captured(3), m.captured(4).trimmed(), m.captured(5), m.captured(6),
                         evidence.value(0),
                         evidence.isEmpty() ? QString() : QString("%1  %2").arg(evidence.value(1), evidence.value(3))});
        }
    }
    setTableRows(sdo_->sdoTable,
                 {"Object", "Index", "Sub", "Access", "Type", "Bits", "Name", "Last Value", "Last Status"}, rows);
    for (int row = 0; row < sdo_->sdoTable->rowCount(); ++row) {
        const QString lastStatus = sdo_->sdoTable->item(row, 8) ? sdo_->sdoTable->item(row, 8)->text() : QString();
        if (lastStatus.isEmpty()) {
            continue;
        }
        const QColor color = lastStatus.contains(uiText("Complete", "完成")) ||
...
void MainWindow::updateFreeRunTelemetry(const QJsonObject& telemetry) {
    const QList<QStringList> rows = {
        ...
        {"Configured Slaves", QString::number(telemetry.value("configuredSlaves").toInt())},
        {"PDO Entries", QString::number(telemetry.value("pdoEntries").toInt())},
        {"Slaves Responding", QString::number(telemetry.value("slavesResponding").toInt())},
        {"AL State", telemetry.value("alStateText").toString()},
        {"AL State Mask", QString("0x%1").arg(telemetry.value("alStates").toInt(), 0, 16)},
        {"Link Up", telemetry.value("linkUp").toBool() ? "Yes" : "No"},
        {"Working Counter", QString::number(telemetry.value("workingCounter").toInt())},
        {"WKC State", telemetry.value("wcStateText").toString()},
        {"WKC State Code", QString::number(telemetry.value("wcState").toInt())},
        {"Redundancy Active", QString::number(telemetry.value("redundancyActive").toInt())},
    };
    ...
    auto pdoMapEvidence = [this, &directionClass](int slave, const QString& direction, const QString& index,
                                                  const QString& subIndex, int bits, QString* mappedName,
        ...
        for (int row = 0; row < sdo_->pdoTable->rowCount(); ++row) {
            const QString mapIndex = normalizeHexText(tableText(sdo_->pdoTable, row, 2), 4);
            const QString mapSubIndex = normalizeHexText(tableText(sdo_->pdoTable, row, 3), 2);
            ...
            QStringList issues;
            if (!runtimeDirection.isEmpty() && !mapDirection.isEmpty() && runtimeDirection != mapDirection) {
                issues << uiText("direction mismatch", "方向不一致");
            }
            if (bits > 0 && mapBits > 0 && bits != mapBits) {
                issues << uiText("bit mismatch %1/%2", "位宽不一致 %1/%2").arg(bits).arg(mapBits);
            }
            return issues.isEmpty() ? uiText("Mapped", "已映射")
                                    : uiText("Mapped warning: %1", "映射警告：%1").arg(issues.join("; "));
        }
    ...
    for (const auto& value : array) {
        ...
        const QString mapEvidence =
            pdoMapEvidence(object.value("slave").toInt(), direction, index, subIndex, bits, &mappedName, &mappedDetail);
        const QString nameKey = QString("%1|%2|%3|%4|%5|%6").arg(slave, sync, direction, pdo, index, subIndex);
        const QString objectNameKey = ioVariableTableObjectKey(object.value("slave").toInt(), index, subIndex);
        const QString aliasName = ioVariableMetadata_.value(objectNameKey).value(0);
        QString displayName = aliasName.trimmed();
        QString displayNameSource = displayName.isEmpty() ? QString() : uiText("I/O alias", "I/O 别名");
        auto takeDisplayName = [&](const QString& candidate, const QString& source) {
        ...
        takeDisplayName(object.value("displayName").toString(), uiText("runtime display name", "运行时显示名"));
        takeDisplayName(object.value("name").toString(), uiText("runtime name", "运行时名称"));
        takeDisplayName(mappedName, uiText("PDO map", "PDO 映射"));
        takeDisplayName(freeRunObjectNames_.value(objectNameKey), uiText("object cache", "对象缓存"));
        takeDisplayName(freeRunEntryNames_.value(nameKey), uiText("entry cache", "条目缓存"));
        ...
        if (meaning.trimmed().isEmpty()) {
            meaning =
                decodeWatchValue(index, subIndex, QString(), rawValue.isEmpty() ? decodedValue : rawValue, "CiA 402");
        }
        entries.append({
            slave, sync, direction, pdo, index, subIndex, QString::number(bits),
            QString::number(object.value("offset").toInt()), QString::number(object.value("bit").toInt()), displayName,
            rawValue, decodedValue, meaning, mapEvidence,
...
void MainWindow::updateFreeRunEntryTable(const QList<QStringList>& rows) {
    ...
    const QStringList headers = {"Slave", "SM",   "Dir", "PDO",     "Index",   "Sub",        "Bits",      "Offset",
                                 "Bit",   "Name", "Raw", "Decoded", "Meaning", "Map Status", "Map Detail"};
    const int selectedRow = freeRunWidgets_->freeRunEntryTable->currentRow();
    const int verticalScroll = freeRunWidgets_->freeRunEntryTable->verticalScrollBar()
                                   ? freeRunWidgets_->freeRunEntryTable->verticalScrollBar()->value()
                                   : 0;
    const int horizontalScroll = freeRunWidgets_->freeRunEntryTable->horizontalScrollBar()
    ...
    for (int row = 0; row < rows.size(); ++row) {
        ...
        const QString key = QString("%1|%2|%3|%4|%5|%6")
                                .arg(values.value(0), values.value(1), values.value(2), values.value(3),
                                     values.value(4), values.value(5));
        const QString valueSignature = values.value(10) + "|" + values.value(11);
        const bool changed = freeRunEntryValues_.contains(key) && freeRunEntryValues_.value(key) != valueSignature;
        ...
        for (int column = 0; column < headers.size(); ++column) {
            ...
            if (changed && (column == 10 || column == 11)) {
                ...
                const bool warning = status.contains(uiText("warning", "警告"), Qt::CaseInsensitive) ||
                                     status.contains(uiText("Missing", "缺失"), Qt::CaseInsensitive) ||
                                     status.contains("缺失") ||
                                     status.contains(uiText("No PDO map", "无 PDO 映射"), Qt::CaseInsensitive);
...
void MainWindow::filterFreeRunEntryTable() {
    ...
    const bool changedOnly = freeRunWidgets_->freeRunChangedOnly && freeRunWidgets_->freeRunChangedOnly->isChecked();
    int visibleRows = 0;
    int changedRows = 0;
    for (int row = 0; row < freeRunWidgets_->freeRunEntryTable->rowCount(); ++row) {
        ...
        bool match = needle.isEmpty();
        for (int column = 0; column < freeRunWidgets_->freeRunEntryTable->columnCount() && !match; ++column) {
    ...
    if (freeRunWidgets_->freeRunEntrySummaryLabel) {
        freeRunWidgets_->freeRunEntrySummaryLabel->setText(
            uiText("%1 visible / %2 entries, %3 changed", "%1 可见 / %2 条目，%3 变化")
                .arg(visibleRows)
...
void MainWindow::filterWatchTable() {
    ...
    const QString needle = watch_->watchFilter ? watch_->watchFilter->text().trimmed() : QString();
    const QString scope = watch_->watchScopeFilter ? watch_->watchScopeFilter->currentData().toString() : "all";
    ...
    for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
        const int position = watch_->watchTable->item(row, 1) ? watch_->watchTable->item(row, 1)->text().toInt() : -1;
        const QString index =
            watch_->watchTable->item(row, 2) ? watch_->watchTable->item(row, 2)->text().trimmed() : QString();
        const QString subIndex =
            watch_->watchTable->item(row, 3) ? watch_->watchTable->item(row, 3)->text().trimmed() : QString();
        ...
        const QString value =
            watch_->watchTable->item(row, 4) ? watch_->watchTable->item(row, 4)->text().trimmed() : QString();
        const bool missingValue = value.isEmpty();
        if (missingValue) {
            ++missingValueRows;
        }
        const QString delta =
            watch_->watchTable->item(row, 9) ? watch_->watchTable->item(row, 9)->text().trimmed() : QString();
        const QString normalizedDelta = delta.toLower();
        const bool baselineDrift =
            !delta.isEmpty() && normalizedDelta != "0" && normalizedDelta != "match" && delta != "匹配";
        if (baselineDrift) {
            ++driftRows;
        }
        const QString startupDelta =
            watch_->watchTable->item(row, 11) ? watch_->watchTable->item(row, 11)->text().trimmed() : QString();
        const QString normalizedStartupDelta = startupDelta.toLower();
        const bool startupDrift = !startupDelta.isEmpty() && normalizedStartupDelta != "match" &&
        ...
        const QString mode =
            watch_->watchTable->item(row, 7) ? watch_->watchTable->item(row, 7)->text().trimmed() : QString();
        const QString normalizedMode = mode.toLower();
        const QString normalizedIndex = normalizeHexText(index, 4);
        const bool cia402 = normalizedMode.contains("cia 402") || normalizedMode.contains("cia402") ||
                            normalizedIndex == "0x6040" || normalizedIndex == "0x6041" || normalizedIndex == "0x6060" ||
                            normalizedIndex == "0x6061" || normalizedIndex == "0x603f" || normalizedIndex == "0x6064" ||
                            normalizedIndex == "0x606c" || normalizedIndex == "0x6077" || normalizedIndex == "0x607a" ||
                            normalizedIndex == "0x60ff" || normalizedIndex == "0x6071";
        const bool selectedSlave = selected >= 0 && position == selected;

        bool match = needle.isEmpty();
        for (int column = 0; column < watch_->watchTable->columnCount() && !match; ++column) {
    ...
    if (watch_->watchSummaryLabel) {
        ...
        const int interval = watch_->watchRefreshInterval ? watch_->watchRefreshInterval->currentData().toInt() : 1000;
        const QString mode = autoEnabled ? uiText("auto %1 ms", "自动 %1 ms").arg(interval > 0 ? interval : 1000)
                                         : uiText("manual", "手动");
        const QString scopeLabel =
            watch_->watchScopeFilter ? watch_->watchScopeFilter->currentText() : uiText("All", "全部");
        const QString summary = uiText("%1/%2 | %3 | %4 | changed %5 | drift %6 | startup %7 | missing "
                                       "%8",
        ...
        watch_->watchSummaryLabel->setToolTip(uiText("Visible rows: %1/%2\nScope: %3\nRefresh: %4\nChanged rows: "
                                                     "%5\nBaseline drift: %6\nStartup diff: %7\nMissing values: %8",
                                                     "可见行：%1/%2\n范围：%3\n刷新：%4\n变化项：%5\n基线偏离：%6\n启动"
                                                     "不一致：%7\n缺失值：%8")
                                                  .arg(visible)
...
void MainWindow::updateWatchRowDetail() {
    ...
    const WatchRowDetailTexts texts = {
        .unavailableText = uiText("Watch evidence is not available.", "当前没有可用的 Watch 证据。"),
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        .noSelectionText = uiText("Select a visible Watch row to review value, decoded meaning, "
                                  "baseline drift, Startup comparison, and read boundary.",
                                  "选择一条可见 Watch 行，以复核数值、解析含义、基线偏离、Startup "
                                  "对照和读取边界。"),
        .noSelectionTip = uiText("Selecting rows, filtering scopes, and reading this detail strip are "
        ...
        .summaryPattern = uiText("#%1 %2:%3 | %4 | Value: %5 | Baseline: %6 | Startup: %7 | %8",
        ...
        .noText = uiText("No", "否"),
        .driveEvidenceLabel = uiText("Drive Evidence", "驱动证据"),
        .localBoundary = uiText("Local preview boundary: selecting this row, filtering Watch scopes, "
        ...
        .executionBoundary = uiText("Execution boundary: Refresh Watch and Auto polling read SDO "
    ...
    if (row < 0 || row >= watch_->watchTable->rowCount() || watch_->watchTable->isRowHidden(row)) {
        // Helper: bulk-set table rows with headers in a single operation.
        applyState(watchRowDetailNoSelectionState(texts));
        return;
    }

    applyState(buildWatchRowDetailState(watchStartupWatchRow(watch_->watchTable, row, watchChangedKeys_), texts));
}

// Helper: bulk-set table rows with headers in a single operation
void MainWindow::setTableRows(QTableWidget* table, const QStringList& headers, const QList<QStringList>& rows) {
...
// Helper: update a metric card label's title and value
void MainWindow::setMetricCard(QLabel* label, const QString& title, const QString& value) {
    if (!label) {
        return;
    }
