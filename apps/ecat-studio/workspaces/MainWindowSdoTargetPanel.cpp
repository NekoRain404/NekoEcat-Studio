...
bool MainWindow::openSdoTargetPanelRow(int row) {
    ...
    switch (decision.routeKind) {
        ...
        case SdoTargetPanelRouteKind::CopyDigest:
            statusBar()->showMessage(uiText("This Selected Object row has no local evidence target; copied "
                                            "the digest instead.",
...
bool MainWindow::copySdoTargetPanelRowDigest(int row) {
    if (!sdoInspector_->sdoTargetTable || row < 0 || row >= sdoInspector_->sdoTargetTable->rowCount()) {
        statusBar()->showMessage(uiText("Select a Selected Object row to copy.", "请选择一行选中对象复核内容再复制。"),
                                 3000);
        return false;
    }
    ...
    switch (decision.copyActionKind) {
        case SdoTargetPanelCopyActionKind::OpenWatch:
            action = uiText("Open the matching Watch evidence row locally", "本地打开匹配的 Watch 证据行");
            break;
        case SdoTargetPanelCopyActionKind::OpenStartup:
            action = uiText("Open the matching Startup SDO evidence row locally", "本地打开匹配的 Startup SDO 证据行");
            break;
        case SdoTargetPanelCopyActionKind::OpenBookmark:
            action = uiText("Open the matching Object Bookmark locally", "本地打开匹配的对象书签");
            break;
        case SdoTargetPanelCopyActionKind::OpenTargetTrail:
            action = uiText("Open the matching SDO Target Trail row locally", "本地打开匹配的 SDO 目标轨迹行");
            break;
        case SdoTargetPanelCopyActionKind::ReviewEvidence:
            action = uiText("Review the local evidence behind this row", "审阅该行背后的本地证据");
            break;
        ...
        case SdoTargetPanelCopyActionKind::FocusObjectDictionary:
            action = uiText("Focus the Object Dictionary context locally", "本地聚焦对象字典上下文");
            break;
        case SdoTargetPanelCopyActionKind::FullDigest:
            action = uiText("Copy the full evidence digest for broader context", "复制完整证据摘要以获得更完整上下文");
            break;
    }
    ...
    const QString target = position >= 0 && !index.isEmpty() && !subIndex.isEmpty()
                               ? QString("#%1 %2:%3").arg(position).arg(index, subIndex)
                               : uiText("No complete SDO target", "尚无完整 SDO 目标");

    QStringList lines;
    lines << uiText("NekoEcat Studio Selected Object Row Evidence", "NekoEcat Studio 选中对象本行证据");
    ...
    lines << QString("%1: %2").arg(uiText("Value", "值"), value.isEmpty() ? uiText("Empty", "空") : value);
    ...
    updateDiagnostics("Info", "SDO",
                      uiText("Copied Selected Object row evidence to clipboard", "已复制选中对象本行证据到剪贴板"));
    return true;
}
...
void MainWindow::copyCurrentSdoEvidenceDigest() {
    ...
    if (position < 0 || index.isEmpty() || subIndex.isEmpty()) {
        updateDiagnostics("Info", "SDO", uiText("No complete SDO target to copy", "没有可复制的完整 SDO 目标"));
        return;
    }

    QStringList lines;
    lines << uiText("NekoEcat Studio SDO Evidence Digest", "NekoEcat Studio SDO 证据摘要");
    lines << QString("%1: %2").arg(uiText("Master", "主站"), activeMasterName());
    lines << QString("%1: #%2 %3:%4").arg(uiText("Target", "目标")).arg(position).arg(index, subIndex);
    ...
    lines << QString("- Watch: %1")
                 .arg(watchRow >= 0 ? uiText("row %1", "第 %1 行").arg(watchRow + 1) : uiText("none", "无"));
    lines << QString("- Startup SDO: %1")
                 .arg(startupRow >= 0 ? uiText("row %1", "第 %1 行").arg(startupRow + 1) : uiText("none", "无"));
    lines << QString("- Object Bookmark: %1")
                 .arg(bookmarkRow >= 0 ? uiText("row %1", "第 %1 行").arg(bookmarkRow + 1) : uiText("none", "无"));
    lines << QString("- SDO Target Trail: %1")
                 .arg(trailRow >= 0 ? uiText("row %1", "第 %1 行").arg(trailRow + 1) : uiText("none", "无"));
    ...
    QApplication::clipboard()->setText(lines.join('\n')); // copy to system clipboard
    updateDiagnostics("Info", "SDO",
                      uiText("Copied current SDO evidence digest to clipboard", "已复制当前 SDO 证据摘要到剪贴板"));
}
...
void MainWindow::openCurrentSdoWatchLink() {
    ...
    if (row < 0 || !watch_->watchTable) {
        updateDiagnostics("Info", "Navigation",
                          uiText("No matching Watch row for current SDO target", "当前 SDO 目标没有匹配的 Watch 行"));
        return;
    }
    activateWorkspaceTab(watchTabIndex_);
    selectAndFocusTableRow(watch_->watchTable, row, 1);
    updateDiagnostics("Info", "Navigation",
                      uiText("Opened matching Watch row for current SDO target", "已打开当前 SDO 目标匹配的 Watch 行"));
}
...
void MainWindow::openCurrentSdoStartupLink() {
    ...
    if (row < 0 || !startupSdoTable_) {
        updateDiagnostics(
            "Info", "Navigation",
            uiText("No matching Startup SDO row for current target", "当前目标没有匹配的 Startup SDO 行"));
        return;
    }
    ...
    updateDiagnostics(
        "Info", "Navigation",
        uiText("Opened matching Startup SDO row for current target", "已打开当前目标匹配的 Startup SDO 行"));
}
...
void MainWindow::openCurrentSdoBookmarkLink() {
    ...
    if (row < 0 || !bookmark_->objectBookmarkTable) {
        updateDiagnostics("Info", "Navigation",
                          uiText("No matching Object Bookmark for current target", "当前目标没有匹配的对象书签"));
        return;
    }
    activateObjectDictionaryPaneFor(bookmark_->objectBookmarkTable);
    selectAndFocusTableRow(bookmark_->objectBookmarkTable, row, 0);
    updateDiagnostics("Info", "Navigation",
                      uiText("Opened matching Object Bookmark for current target", "已打开当前目标匹配的对象书签"));
}
...
void MainWindow::openCurrentSdoTargetTrailLink() {
    ...
    if (row < 0 || !sdoTargetTrailTable_) {
        updateDiagnostics(
            "Info", "Navigation",
            uiText("No matching SDO Target Trail row for current target", "当前目标没有匹配的 SDO 目标轨迹行"));
        return;
    }
    ...
    updateDiagnostics(
        "Info", "Navigation",
        uiText("Opened matching SDO Target Trail row for current target", "已打开当前目标匹配的 SDO 目标轨迹行"));
}


// — Return the sdo object category
QString MainWindow::sdoObjectCategory(const QString& index, const QString& name, const QString& object,
                                      // Navigate from SDO target to the corresponding Object Bookmark row.
                                      const QString& detail) const {
    const QString key = QString("%1 %2 %3 %4").arg(index, name, object, detail).toLower();
    if (key.contains("0x6040") || key.contains("0x6041") || key.contains("0x6060") || key.contains("0x6061") ||
        key.contains("0x603f") || key.contains("controlword") || key.contains("statusword") || key.contains("mode") ||
        key.contains("fault")) {
        return uiText("CiA 402 drive", "CiA 402 驱动");
    }
    if (key.contains("0x1000") || key.contains("0x1008") || key.contains("0x1009") || key.contains("0x100a") ||
        key.contains("0x1018") || key.contains("identity") || key.contains("vendor") || key.contains("product") ||
        key.contains("revision") || key.contains("serial")) {
        return uiText("Identity", "身份");
    }
    if (key.contains("0x160") || key.contains("0x1a0") || key.contains("0x1c1") || key.contains("0x1c2") ||
        key.contains("pdo") || key.contains("mapping") || key.contains("assignment")) {
        // Navigate from SDO target to the corresponding Target Trail row.
        return uiText("PDO mapping", "PDO 映射");
    }
    if (key.contains("0x1001") || key.contains("0x1002") || key.contains("0x1003") || key.contains("error") ||
...
void MainWindow::updateSdoTargetPanel(const QString& source, const QString& detail, const QString& status,
    ...
    const bool hasCompleteTarget = position >= 0 && !normalizedIndex.isEmpty() && !normalizedSubIndex.isEmpty();

    int dictionaryRow = -1;
    SdoDictionaryRow dictionary;
    if (hasCompleteTarget && sdo_->sdoTable && loadedSdoPosition_ == position) {
        dictionaryRow = tableRowForObjectIndex(sdo_->sdoTable, normalizedIndex, normalizedSubIndex, 1, 2);
    ...
    const QString address = position >= 0 && !index.isEmpty() && !subIndex.isEmpty()
                                ? QString("#%1 %2:%3").arg(position).arg(index, subIndex)
                                : uiText("No complete SDO target", "尚无完整 SDO 目标");
    const QString category = sdoObjectCategory(index, name, object, detail);
    const QString writable = selectedSdoWritable_ ? uiText("Writable", "可写") : uiText("Read only", "只读");
    ...
    if (hasCompleteTarget && startupSdoTable_) {
        ...
        for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
            if (!tableObjectAddressMatches(startupSdoTable_, row, position, normalizedIndex, normalizedSubIndex, 0, 1,
                                           2)) {
                continue;
            }
            ++startupRows;
            ...
            if (startupDelta.isEmpty() || rowDelta.compare(uiText("diff", "不一致"), Qt::CaseInsensitive) == 0 ||
    ...
    if (hasCompleteTarget && bookmark_->objectBookmarkTable) {
        ...
        for (int row = 0; row < bookmark_->objectBookmarkTable->rowCount(); ++row) {
            if (!tableObjectAddressMatches(bookmark_->objectBookmarkTable, row, position, normalizedIndex,
    ...
    if (hasCompleteTarget && sdoTargetTrailTable_) {
        ...
        if (targetTrailRow >= 0) {
            const SdoTargetTrailRow trail = sdoTargetTrailRowFromTable(sdoTargetTrailTable_, targetTrailRow);
    ...
    const QVector<SdoEvidenceItem> writeEvidence = sdoWriteEvidenceItemsFromValues(
        readValue, lastValue, watchValue, startupValue, bookmarkValue, targetTrailValue, targetTrailWriteValue,
        {.read = uiText("Read", "读回"),
    ...
    if (!writeEvidence.isEmpty()) {
        ...
        evidenceSetState = evidenceGroups.size() > 1 ? QStringLiteral("conflict") : QStringLiteral("match");
        evidenceSetSummary = evidenceGroups.size() > 1 ? uiText("Conflict: %1", "冲突：%1").arg(facts.join("; "))
    ...
    if (!hasCompleteTarget) {
        ...
        writeDeltaSummary = uiText("No comparable local evidence", "无可比较的本地证据");
    } else {
        const SdoWriteDeltaReview review = reviewSdoWriteDelta(writeEvidence, writeValue);
        ...
        for (const auto& group : review.differingGroups) {
            diffFacts << uiText("%1 %2 -> %3", "%1 %2 -> %3").arg(group.sources.join("/"), group.value, writeValue);
        }
        if (writeDeltaHasConflict && writeDeltaHasDiff) {
            writeDeltaSummary = uiText("Evidence conflict: %1; differs from %2", "证据冲突：%1；不同于 %2")
                                    .arg(review.conflictFacts.join("; "), diffFacts.join("; "));
        } else if (writeDeltaHasConflict) {
            writeDeltaSummary = uiText("Evidence conflict: %1", "证据冲突：%1").arg(review.conflictFacts.join("; "));
        } else if (writeDeltaHasDiff) {
            writeDeltaSummary = uiText("Differs from %1", "不同于 %1").arg(diffFacts.join("; "));
        } else {
            writeDeltaSummary = uiText("Matches %1", "匹配 %1").arg(review.matchingSources.join("/"));
        }
    }

    const bool hasValueEvidence = !readValue.isEmpty() || !lastValue.isEmpty() || !watchValue.isEmpty() ||
                                  !startupValue.isEmpty() || !bookmarkValue.isEmpty() || !targetTrailValue.isEmpty() ||
                                  !targetTrailWriteValue.isEmpty();
    QStringList nextActions;
    if (!hasCompleteTarget) {
        nextActions << uiText("select a slave and object first", "先选择从站和对象");
    } else {
        if (watchRow < 0) {
            nextActions << uiText("add to Watch for repeat observation", "加入 Watch 便于反复观察");
        } else if (watchValue.isEmpty()) {
            nextActions << uiText("refresh Watch or read target for current value", "刷新 Watch 或读取目标获取当前值");
        }
        if (selectedSdoWritable_) {
            if (writeValue.isEmpty() && hasValueEvidence) {
                ...
                if (writeDeltaHasConflict) {
                    nextActions << uiText("review conflicting local evidence before writing",
                                          "写入前先审阅冲突的本地证据");
                } else if (writeDeltaHasDiff) {
                    nextActions << uiText("review Write Delta before write or Startup changes",
                                          "写入或修改 Startup 前先审阅写入差异");
                } else if (writeDeltaMatchesEvidence) {
                    nextActions << uiText("write value already matches local evidence; skip if unchanged",
                                          "写入值已匹配本地证据；无变更时可跳过");
                } else {
                ...
                nextActions << uiText("fill Write Value before write or Startup", "填写写入值后再写入或创建 Startup");
            }
        } else {
            nextActions << uiText("read-only target: use Read or Watch, not Write",
                                  "只读目标：使用读取或 Watch，不执行写入");
        }
        if (startupRows <= 0 && selectedSdoWritable_ && (!writeValue.isEmpty() || hasValueEvidence)) {
            nextActions << uiText("create Startup when this parameter must persist", "需要持久化时创建 Startup");
        } else if (startupRows > 0 &&
                   (startupDelta.contains("diff", Qt::CaseInsensitive) || startupDelta.contains("不一致"))) {
            nextActions << uiText("review Startup diff before applying", "应用前审阅 Startup 偏差");
        }
        if (bookmarkRows <= 0) {
            nextActions << uiText("bookmark if this object is reused", "常用对象可加入书签");
        }
    ...
    const QString nextAction =
        nextActions.isEmpty() ? uiText("target context is ready", "目标上下文就绪") : nextActions.join("; ");

    QStringList safetyNotes;
    safetyNotes << uiText("Panel updates are local context only", "面板更新只整理本地上下文");
    safetyNotes << uiText("Read/Watch/Write/Startup buttons stay explicit", "读取、监视、写入和 Startup 仍需显式点击");
    safetyNotes << uiText("Host Health remains in Diagnostics", "Host Health 仍只在诊断页");
    ...
    QList<QPair<QString, QString>> rows = {
        ...
        {uiText("Source", "来源"), source.trimmed().isEmpty() ? uiText("Manual fields", "手动字段") : source.trimmed()},
        {uiText("Category", "类别"), category},
        {uiText("Name", "名称"), !name.isEmpty() ? name : (!object.isEmpty() ? object : detail.trimmed())},
        {uiText("Access", "权限"), access.isEmpty() ? writable : QString("%1 (%2)").arg(access, writable)},
        {uiText("Type / Bits", "类型 / 位宽"), bits.isEmpty() ? type : QString("%1 / %2 bit").arg(type, bits)},
        {uiText("Read Value", "读回值"),
         !readValue.isEmpty()
             ? readValue
             : (!lastValue.isEmpty() ? lastValue : uiText("Not read in this context", "当前上下文未读取"))},
        {uiText("OD Evidence", "OD 证据"),
         lastStatus.isEmpty() ? uiText("No table evidence", "表格暂无证据")
                              : QString("%1%2").arg(lastStatus, lastValue.isEmpty() ? QString() : " = " + lastValue)},
        {uiText("Watch Link", "Watch 关联"),
         watchRow < 0 ? uiText("Not in Watch", "未加入 Watch")
                      : QString("#%1%2%3")
                            .arg(watchRow + 1)
                            .arg(watchValue.isEmpty() ? QString() : QString(" = %1").arg(watchValue))
                            .arg(watchDecoded.isEmpty() ? QString() : QString(" (%1)").arg(watchDecoded))},
        {uiText("Startup Link", "Startup 关联"),
         startupRows <= 0 ? uiText("No Startup SDO row", "无 Startup SDO 行")
                          : uiText("%1 row(s)%2%3", "%1 行%2%3")
                                .arg(startupRows)
                                .arg(startupValue.isEmpty() ? QString() : QString(" = %1").arg(startupValue))
                                .arg(startupDelta.isEmpty() ? QString() : QString(" [%1]").arg(startupDelta))},
        {uiText("Bookmark", "书签"),
         bookmarkRows <= 0 ? uiText("Not bookmarked", "未收藏")
                           : uiText("%1 bookmark(s)%2", "%1 个书签%2")
                                 .arg(bookmarkRows)
                                 .arg(bookmarkValue.isEmpty() ? QString() : QString(" = %1").arg(bookmarkValue))},
        {uiText("Target Trail", "目标轨迹"),
         targetTrailRow < 0
             ? uiText("No matching target trail row", "无匹配目标轨迹行")
             : QString("#%1%2%3%4")
                   .arg(targetTrailRow + 1)
                   .arg(targetTrailSource.isEmpty() ? QString() : QString(" [%1]").arg(targetTrailSource))
                   .arg(targetTrailValue.isEmpty() ? QString() : QString(" value=%1").arg(targetTrailValue))
                   .arg(targetTrailWriteValue.isEmpty() ? QString() : QString(" write=%1").arg(targetTrailWriteValue))},
        {evidenceSetLabel, evidenceSetSummary},
        {uiText("Write Value", "写入值"), writeValue.isEmpty() ? uiText("Empty", "空") : writeValue},
    ...
    auto rowActionText = [this, evidenceSetLabel, writeDeltaLabel, watchRow, startupRows, bookmarkRows,
                          targetTrailRow](const QString& key) {
        if (key == uiText("Watch Link", "Watch 关联")) {
            return watchRow >= 0 ? uiText("Open Watch", "打开 Watch") : uiText("Copy row", "复制本行");
        }
        if (key == uiText("Startup Link", "Startup 关联")) {
            return startupRows > 0 ? uiText("Open Startup", "打开 Startup") : uiText("Copy row", "复制本行");
        }
        if (key == uiText("Bookmark", "书签")) {
            return bookmarkRows > 0 ? uiText("Open Bookmark", "打开书签") : uiText("Copy row", "复制本行");
        }
        if (key == uiText("Target Trail", "目标轨迹")) {
            return targetTrailRow >= 0 ? uiText("Open Trail", "打开轨迹") : uiText("Copy row", "复制本行");
        }
        if (key == evidenceSetLabel) {
            return currentSdoWriteDeltaReviewAvailable() ? uiText("Review evidence", "审阅证据")
                                                         : uiText("Copy digest", "复制摘要");
        }
        if (key == writeDeltaLabel) {
            return currentSdoWriteDeltaReviewAvailable() ? uiText("Review delta", "审阅差异")
                                                         : uiText("Copy digest", "复制摘要");
        }
        if (key == uiText("Target", "目标") || key == uiText("Read Value", "读回值") ||
            key == uiText("OD Evidence", "OD 证据")) {
            return uiText("Focus OD", "聚焦 OD");
        }
        if (key == uiText("Next Action", "下一步") || key == uiText("Safety", "安全边界") ||
            key == uiText("State", "状态") || key == uiText("Check", "检查")) {
    ...
    for (int row = 0; row < rows.size(); ++row) {
        ...
        actionItem->setToolTip(uiText("Double-click, press Alt+Enter, right-click, or use the Command "
        ...
        if (rows.at(row).first == uiText("State", "状态")) {
            const QColor color = status == uiText("Blocked", "受阻") || status == uiText("Read only", "只读")
                                     // Define color for visual feedback
                                     ? QColor("#ef4444")
                                     : (status == uiText("Warning", "警告") || status == uiText("Incomplete", "未完整")
                                            // Define color for visual feedback
                                            ? QColor("#f59e0b")
                                            // Define color for visual feedback
                                            : QColor("#22c55e"));
        ...
        if (rows.at(row).first == uiText("Watch Link", "Watch 关联") && watchRow >= 0) {
            // Define color for visual feedback
            valueItem->setForeground(settings_.theme == "Light" ? QColor("#166534")
                                                                // Define color for visual feedback
                                                                : QColor("#86efac"));
        }
        if (rows.at(row).first == uiText("Startup Link", "Startup 关联") && startupRows > 0) {
            const bool diff = startupDelta.contains("diff", Qt::CaseInsensitive) || startupDelta.contains("不一致");
            // Define color for visual feedback
        ...
        if (rows.at(row).first == uiText("Target Trail", "目标轨迹") && targetTrailRow >= 0) {
            // Define color for visual feedback
            valueItem->setForeground(settings_.theme == "Light" ? QColor("#1d4ed8")
                                                                // Define color for visual feedback
                                                                : QColor("#93c5fd"));
...
void MainWindow::updateSdoTargetRowActionButton() {
    ...
    if (action.isEmpty()) {
        ...
        button->setToolTip(uiText("Select a Selected Object row, then run its local Action column entry. "
    ...
    button->setToolTip(uiText("Run the selected row action \"%1\" for \"%2\" using local "
                              ...
                              "不写 SDO、不切换状态、不改变 Free Run，也不运行 Host Health。")
                           .arg(action, field.isEmpty() ? uiText("Selected Object row", "选中对象行") : field));
}
...
void MainWindow::updateSdoTargetRowCopyButton() {
    ...
    if (field.isEmpty()) {
        ...
        button->setToolTip(uiText("Select a Selected Object row to copy that row's target, value, local "
    ...
    button->setToolTip(uiText("Copy the Selected Object row \"%1\" with target, value, local "
...
void MainWindow::updateSdoInspector(const QString& source, const QString& detail) {
    ...
    facts << (!index.isEmpty() && !subIndex.isEmpty() ? uiText("Object %1:%2", "对象 %1:%2").arg(index, subIndex)
    ...
    if (!writeValue.isEmpty()) {
        ...
        validateSdoAddressAndValue(index, subIndex, writeValue, type, &validationErrors, &validationWarnings);
