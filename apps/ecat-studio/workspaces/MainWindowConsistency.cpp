...
void MainWindow::updateConsistencyView() {
    ...
    auto addRow = [&](const QString& level, const QString& scope, const QString& target, const QString& evidence,
                      const QString& expected, const QString& actual, const QString& action) {
        rows.append({level, scope, target, evidence, expected, actual, action});
    };
    if (topologyBaseline_.isEmpty()) {
        addRow(uiText("Warning", "警告"), uiText("Topology", "拓扑"), uiText("Topology baseline", "拓扑基线"),
               uiText("No offline baseline has been captured", "尚未捕获离线拓扑基线"),
               uiText("Captured baseline", "已捕获基线"), uiText("Missing", "缺失"),
               uiText("Capture baseline after a known-good scan", "在确认正常的扫描后捕获基线"));
    } else {
        // Collect issue details from the evidence model
        for (const auto& issue : compareTopologyBaseline(topologyBaseline_, slaves_)) {
            const QString target = QString("#%1 %2")
                                       .arg(issue.position)
            ...
            switch (issue.kind) {
                case TopologyBaselineIssueKind::MissingSlave:
                    addRow(uiText("Error", "错误"), uiText("Topology", "拓扑"), target,
                           uiText("Baseline slave is missing online", "基线从站当前在线缺失"), issue.baseline.name,
                           uiText("Missing", "缺失"),
                           uiText("Check cabling, power, and bus order", "检查线缆、供电和总线顺序"));
                    break;
                case TopologyBaselineIssueKind::NameChanged:
                    addRow(uiText("Warning", "警告"), uiText("Topology", "拓扑"), target,
                           uiText("Slave name differs from baseline", "从站名称和基线不一致"), issue.baseline.name,
                           issue.current.name,
                ...
                case TopologyBaselineIssueKind::StateChanged:
                    addRow(uiText("Warning", "警告"), uiText("Topology", "拓扑"), target,
                           uiText("Slave state differs from baseline", "从站状态和基线不一致"), issue.baseline.state,
                           issue.current.state, uiText("Review state machine before OP", "进入 OP 前复核状态机"));
                    break;
                case TopologyBaselineIssueKind::UnexpectedSlave:
                    addRow(uiText("Warning", "警告"), uiText("Topology", "拓扑"), target,
                           ...
                           uiText("Recapture baseline or verify bus order", "重新捕获基线或确认总线顺序"));
                    break;
            }
        }
    }
    ...
    if (ioVar_->ioVariableTable) {
        ...
        for (int row = 0; row < ioVar_->ioVariableTable->rowCount(); ++row) {
            ...
            const QString target =
                QString("#%1 %2:%3 %4").arg(variable.position).arg(variable.index, variable.subIndex, variable.symbol);
            ...
            if (startupDiff) {
                addRow(
                    uiText("Error", "错误"), uiText("Startup", "启动"), target,
                    uiText("Startup expectation differs from live/Watch evidence", "启动期望和实时/Watch 证据不一致"),
                    variable.startup, variable.watch.isEmpty() ? variable.raw : variable.watch,
                    uiText("Review Startup SDO diffs before applying", "应用前审阅 Startup SDO 偏差"));
            }
            if (mapIssue) {
                addRow(uiText("Warning", "警告"), uiText("I/O Variables", "I/O 变量"), target,
                       uiText("PDO/process map evidence is incomplete or inconsistent", "PDO/过程映像证据缺失或不一致"),
                       uiText("Mapped process signal", "已映射过程信号"), variable.map,
                       uiText("Reload PDO Map and refresh Free Run evidence", "重新加载 PDO Map 并刷新 Free Run 证据"));
            }
            if (missingValue) {
                addRow(uiText("Warning", "警告"), uiText("I/O Variables", "I/O 变量"), target,
                       uiText("No Raw or Watch value evidence", "缺少 Raw 或 Watch 值证据"),
                       uiText("Current value evidence", "当前值证据"), uiText("Missing", "缺失"),
                       uiText("Refresh Watch or Free Run before commissioning", "调试前刷新 Watch 或 Free Run"));
            }
            if (variable.alias.isEmpty() && !variable.tags.contains("ignore", Qt::CaseInsensitive)) {
                addRow(uiText("Info", "信息"), uiText("I/O Variables", "I/O 变量"), target,
                       uiText("No engineering alias assigned", "尚未设置工程别名"),
                       uiText("Alias for handoff", "交接用别名"), uiText("Empty", "空"),
                       uiText("Set Alias/Tags for reusable engineering context", "设置 Alias/Tags 以便工程复用"));
            }
            if (ioVariableTableRowHasPlcIssue(variable, uiText("Ready", "就绪"))) {
                addRow(uiText("Info", "信息"), uiText("I/O Variables", "I/O 变量"), target,
                       uiText("PLC handoff symbol quality needs review", "PLC 交接符号质量需要复核"),
    ...
    for (const QString& key : metadataKeys) {
        ...
        addRow(uiText("Warning", "警告"), uiText("I/O Variables", "I/O 变量"),
               QString("#%1 %2:%3 %4").arg(parts.value(0), parts.value(1), parts.value(2), metadata.value(0)),
               uiText("Project variable metadata has no online evidence", "工程变量元数据没有在线证据"),
               uiText("Online PDO/Watch/Free Run row", "在线 PDO/Watch/Free Run 行"), uiText("Missing", "缺失"),
               uiText("Load PDO/Free Run evidence or remove stale metadata", "加载 PDO/Free Run 证据或移除过期元数据"));
    }
        ...
        for (int row = 0; row < startupSdoTable_->rowCount(); ++row) {
            ...
            if (!ok || position < 0 || index.isEmpty() || subIndex.isEmpty()) {
                addRow(uiText("Error", "错误"), uiText("Startup", "启动"),
                       ...
                       uiText("Fix row before applying Startup SDO", "应用 Startup SDO 前修复该行"));
                continue;
            }
            const QString normalizedDelta = delta.toLower();
            const bool diff = !delta.isEmpty() && normalizedDelta != "match" && delta != "匹配" &&
                              normalizedDelta != "pending" && delta != "待比较";
            if (diff) {
                addRow(uiText("Error", "错误"), uiText("Startup", "启动"),
                       QString("#%1 %2:%3")
                           .arg(position)
                       ...
                       uiText("Startup row differs from Watch evidence", "Startup 行和 Watch 证据不一致"), value, delta,
                       uiText("Use Review Diffs, Sync Startup, or edit expected value",
                              "使用审阅偏差、同步启动或编辑期望值"));
            } else if (delta.isEmpty() || normalizedDelta == "pending" || delta == "待比较") {
                addRow(uiText("Warning", "警告"), uiText("Startup", "启动"),
                       QString("#%1 %2:%3")
                           .arg(position)
                           // Normalize hex address for consistent comparison
                           .arg(normalizeHexText(index, 4), normalizeHexText(subIndex, 2)),
                       uiText("Startup row has no Watch comparison", "Startup 行缺少 Watch 对比"), value,
                       uiText("Pending", "待比较"),
                       uiText("Add/refresh matching Watch row before applying", "应用前添加或刷新匹配 Watch 行"));
            }
        }
    }

    if (rows.isEmpty()) {
        addRow(uiText("Ready", "就绪"), uiText("Project", "工程"), uiText("Current evidence", "当前证据"),
               uiText("No consistency issues found in loaded evidence", "当前已加载证据未发现一致性问题"),
    ...
    setTableRows(consistency_->consistencyTable,
                 {uiText("Level", "级别"), uiText("Scope", "范围"), uiText("Target", "目标"),
                  uiText("Evidence", "证据"), uiText("Expected", "期望"), uiText("Actual", "实际"),
                  uiText("Action", "建议动作")},
                 rows);
    ...
    if (consistency_->consistencySummaryLabel) {
        const ConsistencyIssueCounts counts = consistencyTableIssueCounts(consistency_->consistencyTable);
        consistency_->consistencySummaryLabel->setText(uiText("%1 rows | errors %2 | warnings %3 | info %4 | ready %5",
...
void MainWindow::filterConsistencyTable() {
    ...
    const QString scope = consistency_->consistencyScopeFilter
                              ...
                              : QStringLiteral("all");
    const ConsistencyTableFilterStats stats = filterConsistencyTableRows(consistency_->consistencyTable, scope, needle);
    if (consistency_->consistencySummaryLabel) {
        const QString label = consistency_->consistencyScopeFilter ? consistency_->consistencyScopeFilter->currentText()
                                                                   : uiText("All", "全部");
...
void MainWindow::updateConsistencyRowDetail() {
    ...
    auto applyState = [this](const ConsistencyDetailState& state) {
    ...
    applyState(buildConsistencyDetailState({.level = textAt(kConsistencyLevelColumn),
...
void MainWindow::focusEvidenceFromConsistency(int row) {
    ...
    const ConsistencyEvidenceRouteDecision route = consistencyEvidenceRouteDecision(consistencyRow);

    if (route.kind == ConsistencyEvidenceRouteKind::Topology) {
        updateStateMachineView();
        const int stateRow = tableRowForPosition(stateMachine_->stateMachineTable, route.address.position, 0);
        ...
        updateDiagnostics("Info", "Consistency",
                          uiText("Opened state/topology evidence from Consistency", "已从一致性打开状态/拓扑证据"));
        return;
    }
    ...
    if (route.kind == ConsistencyEvidenceRouteKind::Startup) {
        ...
        int startupRow = tableRowForObjectAddress(startupSdoTable_, route.address.position, route.address.index,
        ...
        if (startupRow >= 0) {
            ...
            updateDiagnostics(
                "Info", "Consistency",
                uiText("Opened Startup SDO evidence from Consistency", "已从一致性打开 Startup SDO 证据"));
            return;
        }
    }

    if (route.kind == ConsistencyEvidenceRouteKind::Watch) {
        const int watchRow = tableRowForObjectAddress(watch_->watchTable, route.address.position, route.address.index,
        ...
        if (watchRow >= 0) {
            ...
            updateDiagnostics("Info", "Consistency",
                              uiText("Opened Watch evidence from Consistency", "已从一致性打开 Watch 证据"));
            return;
        }
    }
...
void MainWindow::focusIoVariablesFromConsistency(int row) {
    if (!ioVar_->ioVariableTable || !tabs_ || ioVariableTabIndex_ < 0 || ioVariableTabIndex_ >= tabs_->count()) {
        return;
    }
    ...
    const ConsistencyEvidenceRouteDecision route =
        consistencyEvidenceRouteDecision({.target = target, .evidence = evidence, .actual = actual, .action = action});
    ...
    int selectedRow = tableRowForObjectAddress(ioVar_->ioVariableTable, route.address.position, route.address.index,
    ...
    updateDiagnostics("Info", "Consistency",
                      uiText("Focused I/O Variables from Consistency row", "已从一致性行定位到 I/O 变量"));
}
...
void MainWindow::consistencyIssueCounts(int* errors, int* warnings, int* infos, int* ready) const {
    const ConsistencyIssueCounts counts = consistencyTableIssueCounts(consistency_->consistencyTable);
    if (errors) {
...
QStringList MainWindow::consistencyGateDetails(const QString& operation) const {
    ...
    consistencyIssueCounts(&counts.errors, &counts.warnings, &counts.infos, &counts.ready);

    QStringList details;
    const QString operationText =
        operation.trimmed().isEmpty() ? uiText("this operation", "此操作") : operation.trimmed();
    switch (consistencyGateState(consistencyCheckAvailable(), consistencyFresh_, counts)) {
        case ConsistencyGateState::NotRun:
            details << uiText("Consistency gate: not run before %1", "一致性门禁：%1 前尚未运行").arg(operationText);
            details << uiText("Run the read-only Consistency Check to compare topology, "
                              "Startup, Watch, I/O Variables, and project metadata.",
                              "请运行只读一致性检查，对比拓扑、Startup、Watch、I/O 变量"
                              "和工程元数据。");
            break;
        case ConsistencyGateState::Stale:
            details << uiText("Consistency gate: stale before %1", "一致性门禁：%1 前结果已过期").arg(operationText);
        ...
        case ConsistencyGateState::Blocking:
            details << uiText("Consistency gate: %1 error(s), %2 warning(s)", "一致性门禁：%1 个错误，%2 个警告")
                           .arg(counts.errors)
                           .arg(counts.warnings);
            details << uiText("Review Consistency rows before %1", "%1 前请审阅一致性行").arg(operationText);
            break;
