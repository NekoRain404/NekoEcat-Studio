...
void MainWindow::addStartupSdoFromWatchRow(int row) {
    ...
    if (!positionOk || position < 0 || index.isEmpty() || subIndex.isEmpty() || value.isEmpty()) {
        updateDiagnostics("Error", "Watch",
                          QString("Cannot create Startup SDO from watch row %1: missing value").arg(row + 1));
        return;
    }
    ...
    if (sdoInspector_->sdoWriteValue) {
        ...
        sdoInspector_->sdoWriteValue->setPlaceholderText(uiText("Value from Watch", "来自 Watch 的值"));
    }
    updateSdoInspector(
        uiText("Watch", "监视"),
        uiText("Creating Startup SDO from Watch row %1", "从 Watch 第 %1 行创建 Startup SDO").arg(row + 1));
    addStartupSdo();
    updateDiagnostics("Info", "Watch", QString("Created Startup SDO from watch row %1").arg(row + 1));
}
...
void MainWindow::syncWatchRowsToStartupSdo(const QVector<int>& rows) {
    ...
    for (const int row : uniqueRows) {
        ...
        if (!positionOk || position < 0 || rawIndex.isEmpty() || rawSubIndex.isEmpty() || value.isEmpty()) {
            ++skipped;
            continue;
        }
        ...
        for (int startupRow = 0; startupRow < startupSdoTable_->rowCount(); ++startupRow) {
            ...
            const int startupPosition = startupTextAt(startupRow, 0).toInt(&startupPositionOk);
            ...
            if (startupPositionOk && startupPosition == position && startupIndex == index &&
                startupSubIndex == subIndex) {
    ...
    if (candidates.isEmpty()) {
        updateDiagnostics("Warning", "Startup SDO",
                          QString("Startup sync skipped: no selected Watch rows with current "
                                  "values%1")
                              .arg(skipped > 0 ? QString(", skipped %1 row(s)").arg(skipped) : QString()));
        return;
    }
    ...
    QStringList details = {
        ...
        uiText("Accepted Watch values: %1", "可同步 Watch 值：%1").arg(candidates.size()),
        ...
        uiText("This changes only the Startup SDO table and does not write to the "
               "bus.",
    ...
    if (candidates.size() > previewRows) {
        details
            << uiText("...and %1 more Watch value(s)", "...另有 %1 个 Watch 值").arg(candidates.size() - previewRows);
    }
    if (skipped > 0) {
        details << uiText("Skipped rows without address or current value: %1", "已跳过缺少地址或当前值的行：%1")
                       .arg(skipped);
    }
    if (duplicateSkipped > 0) {
        details << uiText("Skipped duplicate selected Watch address(es): %1", "已跳过重复选中 Watch 地址：%1")
    ...
    if (!confirmDangerousOperation(
            ...
            uiText("Synchronize selected Watch values into Startup SDO.", "将选中 Watch 当前值同步到 Startup SDO。"),
    ...
    for (const auto& candidate : candidates) {
        if (candidate.startupRows.isEmpty()) {
            ...
            startupSdoTable_->setItem(
                startupRow, 6,
                // Create table cell
                new QTableWidgetItem(uiText("Created from Watch row %1 at %2", "由 Watch 第 %1 行在 %2 创建")
        ...
        for (const int startupRow : candidate.startupRows) {
            ...
            ensureStartupCell(startupRow, 6)
                ->setText(previousValue.compare(candidate.value, Qt::CaseInsensitive) == 0
                              ? uiText("Confirmed from Watch row %1 at %2", "由 Watch 第 %1 行在 %2 确认")
                                    .arg(candidate.watchRow + 1)
                                    .arg(timestamp)
    ...
    updateDiagnostics(
        ...
        QString("Synced Startup SDO from Watch: %1 updated, %2 unchanged, %3 "
            ...
            .arg(created)
            .arg(skipped > 0 ? QString(", skipped %1").arg(skipped) : QString())
            .arg(duplicateSkipped > 0 ? QString(", duplicate selections %1").arg(duplicateSkipped) : QString()));
}
    ...
    for (const int row : rows) {
        ...
        startupSdoTable_->setItem(
            ...
            new QTableWidgetItem(QString("%1 %2%3")
                                     ...
                                     .arg(row + 1)
                                     .arg(detail.isEmpty() ? QString() : QString(": %1").arg(detail))));
...
void MainWindow::addDictionaryEvidenceRowsToStartupSdo(const QVector<int>& rows) {
    ...
    for (const int dictionaryRow : uniqueRows) {
        ...
        const SdoDictionaryRow dictionary = sdoDictionaryRowFromTable(sdo_->sdoTable, dictionaryRow);
        if (position < 0 || !sdoDictionaryRowHasTarget(dictionary) || !sdoDictionaryRowHasValue(dictionary)) {
            ++skipped;
            continue;
        }

        const QString key = QString("%1|%2|%3").arg(position).arg(dictionary.index, dictionary.subIndex);
        if (processedKeys.contains(key)) {
            ++duplicateSkipped;
            continue;
        }
        ...
        for (int startupRow = 0; startupRow < startupSdoTable_->rowCount(); ++startupRow) {
            if (tableObjectAddressMatches(startupSdoTable_, startupRow, position, dictionary.index, dictionary.subIndex,
                                          0, 1, 2)) {
                candidate.startupRows.append(startupRow);
            }
        }
    ...
    for (const auto& candidate : candidates) {
        ...
        if (access.contains("ro") || access.contains("const") || candidate.access.contains(uiText("只读", "只读"))) {
            ++readOnlyRows;
        }
    }
    ...
    QStringList details = {
        ...
        uiText("Accepted OD evidence values: %1", "可使用 OD 证据值：%1").arg(candidates.size()),
        uiText("Existing Startup SDO rows to update: %1", "将更新的已有 Startup SDO 行：%1").arg(existingRowsAffected),
        uiText("Missing Startup SDO rows to create: %1", "将创建的缺失 Startup SDO 行：%1").arg(newRowsPlanned),
    ...
    if (candidates.size() > previewRows) {
        details << uiText("...and %1 more OD evidence value(s)", "...另有 %1 个 OD 证据值")
                       .arg(candidates.size() - previewRows);
    }
    if (skipped > 0) {
        details << uiText("Skipped rows without address or Last Value: %1", "已跳过缺少地址或 Last Value 的行：%1")
                       .arg(skipped);
    }
    if (duplicateSkipped > 0) {
        details << uiText("Skipped duplicate selected OD address(es): %1", "已跳过重复选中 OD 地址：%1")
                       .arg(duplicateSkipped);
    }

    // Safety gate: require explicit confirmation before bus write
    if (!confirmDangerousOperation(uiText("Confirm Startup from OD Evidence", "确认从 OD 证据创建 Startup SDO"),
    ...
    for (const auto& candidate : candidates) {
        if (candidate.startupRows.isEmpty()) {
            ...
            startupSdoTable_->setItem(
                ...
                new QTableWidgetItem(
                    uiText("Created from Object Dictionary row %1 at %2%3", "由对象字典第 %1 行在 %2 创建%3")
                        .arg(candidate.dictionaryRow + 1)
                        .arg(timestamp,
                             candidate.access.isEmpty() ? QString() : QString(" (%1)").arg(candidate.access))));
        ...
        for (const int startupRow : candidate.startupRows) {
            ...
            ensureStartupCell(startupRow, 6)
                ->setText(previousValue.compare(candidate.value, Qt::CaseInsensitive) == 0
                              ? uiText("Confirmed from Object Dictionary row %1 at %2", "由对象字典第 %1 行在 %2 确认")
                                    .arg(candidate.dictionaryRow + 1)
                                    .arg(timestamp)
