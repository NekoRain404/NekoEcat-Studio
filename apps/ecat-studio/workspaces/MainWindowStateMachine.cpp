...
QString MainWindow::recommendedEthercatState(const SlaveInfo& slave) const {
    ...
    const bool pdoLoaded = loadedPdoPosition_ == position && sdo_->pdoTable && sdo_->pdoTable->rowCount() > 0;
...
void MainWindow::updateStateMachineView() {
    ...
    if (stateMachine_->stateMachineTable->currentRow() >= 0) {
        const int rowPosition = stateMachinePositionFromTable(stateMachine_->stateMachineTable,
    ...
    for (const auto& slave : slaves_) {
        const int position = slave.position;
        const QString state = slave.state.trimmed().isEmpty() ? uiText("Unknown", "未知") : slave.state.trimmed();
        ...
        const int identityRows =
            loadedSlaveInfoPosition_ == position && identityTable_ ? identityTable_->rowCount() : 0;
        const int odRows = loadedSdoPosition_ == position && sdo_->sdoTable ? sdo_->sdoTable->rowCount() : 0;
        const int pdoRows = loadedPdoPosition_ == position && sdo_->pdoTable ? sdo_->pdoTable->rowCount() : 0;
        ...
        if (watch_->watchTable) {
            for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
                ...
                if (index == "0x6041" && !decoded.isEmpty()) {
                    ...
                    modeDisplay = decoded;
                } else if (index == "0x603f" && !decoded.isEmpty() && value != "0" && value.toLower() != "0x0000") {
                    errorCode = decoded;
                }
            }
        }
        ...
        evidence << (identityRows > 0 ? uiText("ID ready", "身份就绪") : uiText("ID missing", "身份缺失"));
        evidence << uiText("OD %1", "OD %1").arg(odRows);
        evidence << uiText("Watch %1/%2 values", "Watch %1/%2 有值").arg(watchValueRows).arg(watchRows);
        ...
        const QString drive =
            driveFacts.isEmpty() ? uiText("No CiA 402 Watch", "无 CiA 402 监视") : driveFacts.join(" | ");
        ...
        if ((normalizedState.contains("PREOP") || normalizedState.contains("SAFEOP")) && pdoRows <= 0) {
            risks << uiText("PDO missing", "PDO 缺失");
        }
        if ((normalizedState.contains("PREOP") || normalizedState.contains("SAFEOP")) && watchValueRows <= 0) {
        ...
        const QString action = !target.isEmpty()
                                   ? uiText("Send %1", "发送 %1").arg(target)
                                   : ((normalizedState == "OP" || normalizedState.startsWith("OP ")) && risks.isEmpty()
                                          ? uiText("Ready", "就绪")
                                          : uiText("Review evidence", "复核证据"));

        rows.append({QString::number(position),
                     slave.name.trimmed().isEmpty() ? uiText("Unnamed", "未命名") : slave.name.trimmed(), state, target,
                     evidence.join(" | "), drive,
                     uiText("%1 row(s), %2 diff(s)", "%1 行，%2 个偏差").arg(startupRows).arg(startupDiffs),
                     uiText("PDO %1 | Free Run %2 | Map issue %3", "PDO %1 | Free Run %2 | 映射问题 %3")
                         .arg(pdoRows)
                         .arg(freeRunRows)
                         .arg(mapIssues),
                     risks.join("; "), action});
    ...
    for (int row = 0; row < stateMachine_->stateMachineTable->rowCount(); ++row) {
        const StateMachineTableRow tableRow = stateMachineTableRowFromTable(stateMachine_->stateMachineTable, row);
        int rowPosition = -1;
        if (stateMachineTableRowPosition(tableRow, &rowPosition) && rowPosition == previousPosition) {
            restoreRow = row;
        }
        ...
        const QColor currentColor =
            (current == "OP" || current.startsWith("OP "))
                ? okColor
                : (current.contains("SAFEOP") || current.contains("PREOP") ? actionColor : infoColor);
...
void MainWindow::requestSlaveStateWithConfirmation(int position, const QString& state) {
    ...
    if (normalizedState != "INIT" && normalizedState != "PREOP" && normalizedState != "SAFEOP" &&
        normalizedState != "OP") {
        updateDiagnostics("Error", "State",
                          uiText("Invalid state transition requested: %1", "无效的状态切换请求：%1").arg(state));
        return;
    }
    ...
    for (const auto& slave : slaves_) {
        if (slave.position == position) {
            currentState = slave.state.trimmed().isEmpty() ? currentState : slave.state.trimmed();
    ...
    if (currentState.trimmed().toUpper() == normalizedState) {
        updateDiagnostics("Info", "State",
                          uiText("Slave #%1 is already in %2 state", "从站 #%1 已在 %2 状态").arg(position).arg(state));
        return;
    }
    ...
    if (!confirmDangerousOperation(uiText("Confirm Slave State Change", "确认从站状态切换"),
...
void MainWindow::requestAllSlaveState(const QString& state) {
    ...
    if (state.trimmed().compare("OP", Qt::CaseInsensitive) == 0 ||
        ...
        details << consistencyGateDetails(uiText("all-slave state change", "全部从站状态切换"));
    }
    if (!confirmDangerousOperation(uiText("Confirm All-Slave State Change", "确认全部从站状态切换"),
...
void MainWindow::runHostDiagnostics() {
    ...
    if (!client_.isConnected()) {
        updateDiagnostics("Warning", "Host", "Host check skipped: runtime is not connected");
        return;
    }
...
void MainWindow::copySelectedHostCommand() {
    ...
    if (command.isEmpty()) {
        updateDiagnostics("Info", "Host", "Selected health check has no command to copy");
...
void MainWindow::prepareSelectedSlaveSnapshot() {
    if (!client_.isConnected()) {
        updateDiagnostics("Warning", "Snapshot", "Snapshot skipped: runtime is not connected");
        return;
    }

    const int position = selectedPosition();
    if (position < 0) {
        updateDiagnostics("Warning", "Snapshot", "Snapshot skipped: no slave selected");
        return;
    }
    ...
    QStringList notes;
    notes << uiText("identity", "身份") << uiText("Object Dictionary", "对象字典") << uiText("PDO Map", "PDO 映射")
          << uiText("ESI XML", "ESI XML") << uiText("CiA 402 Watch", "CiA 402 Watch");
    const QStringList topologyIssues = topologyBaselineIssues();
    if (!topologyIssues.isEmpty()) {
        notes << uiText("topology baseline has %1 issue(s)", "拓扑基线有 %1 个问题").arg(topologyIssues.size());
    }

    updateDiagnostics("Info", "Snapshot",
...
void MainWindow::beginSelectedSlaveOnlineLoad(int position) {
    ...
    const QString target = position >= 0 ? QString("#%1").arg(position) : uiText("no slave selected", "尚未选择从站");
    if (rawText_->infoText) {
        rawText_->infoText->setPlainText(
            uiText("Loading selected slave identity for %1...", "正在加载 %1 的选中从站身份信息...").arg(target));
    }
    if (rawText_->pdoText) {
        rawText_->pdoText->setPlainText(uiText("Loading PDO Map for %1...", "正在加载 %1 的 PDO 映射...").arg(target));
    }
    if (rawText_->sdoText) {
        rawText_->sdoText->setPlainText(
            uiText("Loading Object Dictionary for %1...", "正在加载 %1 的对象字典...").arg(target));
    }
    if (rawText_->xmlText) {
        rawText_->xmlText->setPlainText(uiText("Loading ESI XML for %1...", "正在加载 %1 的 ESI XML...").arg(target));
    }
    // Clear all online-loaded views (SDO, PDO, identity, mailbox, etc.).

    setTableRows(identityTable_, {"Field", "Value"}, {{uiText("Loading", "加载中"), target}});
    ...
    setTableRows(sdo_->sdoTable,
                 {"Object", "Index", "Sub", "Access", "Type", "Bits", "Name", "Last Value", "Last Status"}, {});
    if (sdo_->pdoSummaryLabel) {
        sdo_->pdoSummaryLabel->setText(uiText("Loading PDO Map for %1", "正在加载 %1 的 PDO 映射").arg(target));
    }
    ...
    updateSdoInspector(
        uiText("Selected slave changed", "选中从站已切换"),
        uiText("Waiting for OD/PDO/identity evidence for %1", "等待 %1 的 OD/PDO/身份信息证据").arg(target));
