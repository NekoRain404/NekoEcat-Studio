...
QStringList MainWindow::sdoWriteImpactDetails(int position, const QString& index, const QString& subIndex,
    ...
    for (const auto& slave : slaves_) {
        if (slave.position == position) {
            slaveName = slave.name.trimmed().isEmpty() ? slaveName : slave.name.trimmed();
            slaveState = slave.state.trimmed().isEmpty() ? slaveState : slave.state.trimmed();
            break;
        }
    }
    details << uiText("Slave context: #%1 %2, state %3", "从站上下文：#%1 %2，状态 %3")
                   .arg(position)
    ...
    QString nameText;
    QString bitsText;
    if (sdo_->sdoTable && selectedPosition() == position && loadedSdoPosition_ == position) {
        const SdoDictionaryRow dictionary =
            sdoDictionaryRowForTarget(sdo_->sdoTable, normalizedIndex, normalizedSubIndex);
    ...
    const QString category = sdoObjectCategory(normalizedIndex, nameText, objectText, QString());
    ...
    if (!nameText.isEmpty() || !accessText.isEmpty() || !bitsText.isEmpty()) {
        ...
        details << uiText("Dictionary: %1", "对象字典：%1").arg(objectFacts.join(" | "));
    }

    const QStringList cachedEvidence = sdoEvidence_.value(sdoEvidenceKey(position, index, subIndex));
    const QVector<SdoEvidenceItem> localEvidence = sdoLocalEvidenceItemsFromTables(
        ...
        sdoInspector_->sdoValue && isCurrentSdoTarget(position, normalizedIndex, normalizedSubIndex),
    ...
    if (evidenceGroups.isEmpty()) {
        details << uiText("Evidence Set: no local comparable evidence", "证据集：无可比较的本地证据");
    } else {
        ...
        details << (evidenceGroups.size() == 1
                        ? uiText("Evidence Set: consistent %1", "证据集：一致 %1").arg(groupFacts.join("; "))
                        : uiText("Evidence Set conflict: %1", "证据集冲突：%1").arg(groupFacts.join("; ")));

        if (!target.isEmpty()) {
            const SdoWriteDeltaReview review = reviewSdoWriteDelta(localEvidence, target);
            QStringList differingFacts;
            for (const auto& group : review.differingGroups) {
                differingFacts << QString("%1=%2").arg(group.sources.join("/"), group.value);
            }
            if (differingFacts.isEmpty()) {
                details << uiText("Write target: matches all local evidence", "写入目标：匹配全部本地证据");
            } else if (!review.matchingSources.isEmpty()) {
                details << uiText("Write target: matches %1, differs from %2", "写入目标：匹配 %1，不同于 %2")
                               .arg(review.matchingSources.join("/"), differingFacts.join("; "));
            } else {
                details << uiText("Write target: differs from local evidence %1", "写入目标：不同于本地证据 %1")
    ...
    if (watch_->watchTable) {
        for (int row = 0; row < watch_->watchTable->rowCount(); ++row) {
            ...
            if (tableObjectAddressMatches(watch_->watchTable, row, position, normalizedIndex, normalizedSubIndex, 1, 2,
                                          3) &&
                !value.isEmpty()) {
                currentValue = value;
    ...
    if (currentValue.isEmpty()) {
        const QStringList evidence = sdoEvidence_.value(sdoEvidenceKey(position, index, subIndex));
    ...
    if (!currentValue.isEmpty()) {
        const bool same = normalizeComparableValue(currentValue) == normalizeComparableValue(target);
        details << (same ? uiText("Change preview: current %1 already matches target",
                                  "变更预览：当前值 %1 已匹配目标值")
                               .arg(currentValue)
                         : uiText("Change preview: %1 -> %2", "变更预览：%1 -> %2").arg(currentValue, target));
        details << uiText("Current evidence source: %1", "当前证据来源：%1").arg(currentSource);
    } else {
        details << uiText("Current evidence: no Watch or OD value available", "当前证据：没有可用的 Watch 或 OD 值");
    }
    ...
    const QString riskKey = QString("%1 %2 %3 %4").arg(normalizedIndex, nameText, objectText, type).toLower();
    QStringList riskHints;
    if (riskKey.contains("0x6040") || riskKey.contains("0x6060") || riskKey.contains("controlword") ||
        riskKey.contains("mode")) {
        riskHints << uiText("drive command/mode object; confirm actuator safety",
                            "驱动命令/模式对象；确认执行机构安全");
    }
    if (riskKey.contains("0x160") || riskKey.contains("0x1a0") || riskKey.contains("0x1c1") ||
        riskKey.contains("0x1c2") || riskKey.contains("pdo") || riskKey.contains("mapping")) {
        riskHints << uiText("PDO mapping object; verify state and process-image impact",
                            "PDO 映射对象；确认状态和过程映像影响");
    }
    if (riskKey.contains("0x1010") || riskKey.contains("0x1011") || riskKey.contains("store") ||
        riskKey.contains("save") || riskKey.contains("restore")) {
        riskHints << uiText("persistent storage object; change may survive reboot",
                            "持久化存储对象；变更可能跨重启保留");
    }
    if (!riskHints.isEmpty()) {
        details << uiText("Risk flags: %1", "风险标记：%1").arg(riskHints.join("; "));
    }

    const QStringList topologyIssues = topologyBaselineIssues();
    if (!topologyIssues.isEmpty()) {
        details << uiText("Topology baseline: %1 issue(s); review before writing", "拓扑基线：%1 个问题；写入前请复核")
...
QString MainWindow::startupSdoImpactLine(int row) const {
    ...
    if (current.isEmpty() && positionOk) {
        const QStringList evidence = sdoEvidence_.value(sdoEvidenceKey(numericPosition, index, subIndex));
    ...
    QString line = uiText("Row %1: #%2 %3:%4 %5 -> %6", "第 %1 行：#%2 %3:%4 %5 -> %6")
                       .arg(row + 1)
...
QStringList MainWindow::startupSdoBatchImpactDetails(const QVector<int>& rows, int previewLimit) const {
    QStringList details;
    ...
    for (const int row : rows) {
        ...
        const QString riskKey =
            QString("%1 %2")
                // Normalize hex address for consistent comparison
                .arg(normalizeHexText(tableText(startupSdoTable_, row, 1), 4), tableText(startupSdoTable_, row, 4))
                .toLower();
        if (riskKey.contains("0x6040") || riskKey.contains("0x6060")) {
            ++driveRisk;
        }
        if (riskKey.contains("0x160") || riskKey.contains("0x1a0") || riskKey.contains("0x1c1") ||
            riskKey.contains("0x1c2")) {
            ++pdoRisk;
        }
    ...
    details << uiText("Watch evidence: match %1, diff %2, pending %3, no watch %4",
    ...
    if (persistentRisk > 0) {
        riskFacts << uiText("persistent rows %1", "持久化行 %1").arg(persistentRisk);
    }
    if (!riskFacts.isEmpty()) {
        details << uiText("Risk summary: %1", "风险摘要：%1").arg(riskFacts.join("; "));
    }

    const QStringList topologyIssues = topologyBaselineIssues();
    if (!topologyIssues.isEmpty()) {
        details << uiText("Topology baseline: %1 issue(s); review before applying", "拓扑基线：%1 个问题；应用前请复核")
                       .arg(topologyIssues.size());
    }

    const int limit = std::max(0, std::min(previewLimit, static_cast<int>(rows.size())));
    ...
    if (rows.size() > limit) {
        details << uiText("...and %1 more item(s)", "...另有 %1 项").arg(rows.size() - limit);
    }
    return details;
}
// ── SDO Write Mode ──────────────────────────────────────────────────
...
bool MainWindow::isCurrentSdoTarget(int position, const QString& index, const QString& subIndex) const {
...
int MainWindow::currentSdoTargetTrailRow() const {
    ...
    return sdoEvidenceTableRowsForTarget({.targetTrailTable = sdoTargetTrailTable_},
                                         {.position = position,
                                          .index = sdoInspector_->sdoIndex->text(),
                                          .subIndex = sdoInspector_->sdoSubIndex->text()})
        .targetTrailRow;
...
void MainWindow::handleSdoValueResponse(int position, const QString& index, const QString& subIndex,
                                        const QString& value) {
    log(QString("SDO upload #%1 %2:%3 = %4").arg(position).arg(index, subIndex, value));
    ...
    if (currentTarget && sdoInspector_->sdoValue) {
        ...
        sdoInspector_->sdoValue->setPlaceholderText(
            uiText("Read-back for current SDO target", "当前 SDO 目标的读回值"));
        updateActionAvailability();
    }
    const QString source = pendingSdoReads_.take(key);
    const QString readType = pendingSdoReadTypes_.take(key);
    updateSdoTableEvidence(position, index, subIndex, value, uiText("Complete", "完成"),
    ...
    const QVector<int> startupCheckRows = hasStartupCheck ? pendingStartupSdoChecks_.take(key) : QVector<int>{};
    if (!verification.isEmpty()) {
        auto normalize = [](QString text) { return text.trimmed().remove(' ').toLower(); };
        const QString expected = verification.value(4);
        const bool match = normalize(expected) == normalize(value);
        const QString verifyDetail =
            match
                ? uiText("Read-back matched expected value %1", "读回值匹配期望值 %1").arg(expected)
                : uiText("Read-back mismatch, expected %1 got %2", "读回不匹配，期望 %1，实际 %2").arg(expected, value);
        appendSdoHistory(uiText("Verify", "校验"), position, index, subIndex, verification.value(3), value,
                         match ? uiText("OK", "成功") : uiText("Failed", "失败"), verifyDetail);
        updateSdoTableEvidence(position, index, subIndex, value,
                               match ? uiText("OK", "成功") : uiText("Failed", "失败"), verifyDetail);
        updateDiagnostics(match ? "Info" : "Error", "SDO",
                          match ? QString("SDO write verified #%1 %2:%3 = %4").arg(position).arg(index, subIndex, value)
    ...
    appendSdoHistory(
        ...
        !readType.isEmpty()
            ? readType
            : (currentTarget && sdoInspector_->sdoType ? sdoInspector_->sdoType->currentText() : QString()),
        value, uiText("Complete", "完成"), source.isEmpty() ? uiText("Runtime response", "运行时返回") : source);
    updateWatchTableFromSdo(position, index, subIndex, value, source, readType, currentTarget, key);
...
void MainWindow::updateWatchTableFromSdo(int position, const QString& index, const QString& subIndex,
                                         const QString& value, const QString& source, const QString& readType,
                                         bool currentTarget, const QString& key) {
    ensureWatchTable();
    int row = -1;
    for (int i = 0; i < watch_->watchTable->rowCount(); ++i) {
        const bool match =
            (watch_->watchTable->item(i, 1) && watch_->watchTable->item(i, 1)->text().toInt() == position) &&
            (watch_->watchTable->item(i, 2) &&
             watch_->watchTable->item(i, 2)->text().compare(index, Qt::CaseInsensitive) == 0) &&
            (watch_->watchTable->item(i, 3) &&
             watch_->watchTable->item(i, 3)->text().compare(subIndex, Qt::CaseInsensitive) == 0);
        if (match) {
            row = i;
            break;
        }
    ...
    const bool changed = watchValues_.contains(key) && watchValues_.value(key) != value;
    ...
    const QString currentType = watch_->watchTable->item(row, 6) ? watch_->watchTable->item(row, 6)->text() : QString();
    const QString currentMode = watch_->watchTable->item(row, 7) ? watch_->watchTable->item(row, 7)->text() : QString();
    if (!watch_->watchTable->item(row, 6) || watch_->watchTable->item(row, 6)->text().trimmed().isEmpty()) {
        const bool watchRefreshSource =
            source.contains("Watch", Qt::CaseInsensitive) || source.contains("监视", Qt::CaseInsensitive);
    ...
    const QString effectiveType =
        watch_->watchTable->item(row, 6) ? watch_->watchTable->item(row, 6)->text() : currentType;
    setCell(5, decodeWatchValue(index, subIndex, effectiveType, value, currentMode));
    ...
    if (changed) {
        valueItem->setBackground(settings_.theme == "Light" ? QColor("#fff7cc") : QColor("#3a2f16"));
        valueItem->setForeground(settings_.theme == "Light" ? QColor("#854d0e") : QColor("#fde68a"));
    } else {
...
void MainWindow::feedChartFromSdo(const QString& index, const QString& subIndex, const QString& value) {
...
void MainWindow::verifyStartupSdo(const QString& key, const QString& value, const QVector<int>& startupCheckRows) {
    auto normalize = [](QString text) { return text.trimmed().remove(' ').toLower(); };
    for (const int startupCheckRow : startupCheckRows) {
        if (startupCheckRow < 0 || startupCheckRow >= startupSdoTable_->rowCount()) {
            continue;
        }
        const QString expected =
            startupSdoTable_->item(startupCheckRow, 3) ? startupSdoTable_->item(startupCheckRow, 3)->text() : QString();
        const bool match = normalize(expected) == normalize(value);
        ...
        status->setText(match ? uiText("Verified", "已校验") : uiText("Mismatch", "不匹配"));
        status->setForeground(match ? QColor("#22c55e") : QColor("#ef4444"));
        status->setBackground(
            match ? QBrush() : (settings_.theme == "Light" ? QBrush(QColor("#fef2f2")) : QBrush(QColor("#3a1218"))));
        detail->setText(match ? uiText("Read-back matched %1", "读回值匹配 %1").arg(expected)
                              : uiText("Expected %1, got %2", "期望 %1，实际 %2").arg(expected, value));
        updateDiagnostics(match ? "Info" : "Error", "Startup SDO",
                          match ? QString("Startup SDO verified row %1").arg(startupCheckRow + 1)
