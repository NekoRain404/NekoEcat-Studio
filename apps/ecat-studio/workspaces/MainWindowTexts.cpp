...
StartupSdoRowDetailTexts MainWindow::startupSdoRowDetailTexts() const {
    return {
        ...
        .unavailableText = uiText("Startup SDO evidence is not available.", "当前没有可用的 Startup SDO 证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not write the bus.", "此预览仅在本地工作，不写入总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select a visible Startup SDO row to review target, expected "
                                  "value, Watch comparison, status, and write boundary.",
                                  "选择一条可见 Startup SDO 行，以复核目标、期望值、Watch 对照、"
                                  "状态和写入边界。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selecting rows, Diffs Only, and this detail strip are local review "
        ...
        .summaryPattern = uiText("Row %1 | #%2 %3:%4 | %5 = %6 | Status: %7 | Watch: %8 | %9",
                                 "第 %1 行 | #%2 %3:%4 | %5 = %6 | 状态：%7 | Watch：%8 | %9"),
        // selectedTitle field
        .selectedTitle = uiText("Selected Startup SDO row", "选中的 Startup SDO 行"),
        // rowLabel field
        ...
        // localBoundary field
        .localBoundary = uiText("Local preview boundary: selecting this row, Diffs Only, row "
        ...
        .executionBoundary = uiText("Execution boundary: Verify reads the target SDO; Apply Row, Apply "
...
SdoHistoryRowDetailTexts MainWindow::sdoHistoryRowDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("SDO history evidence is not available.", "当前没有可用的 SDO 历史证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select an SDO history row to review target, value, status, detail, "
                                  "reuse options, and operation boundary.",
                                  "选择一条 SDO 历史行，以复核目标、值、状态、详情、复用选项和操作边"
                                  "界。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selecting rows and reading this detail strip are local audit "
        ...
        // reusableValue field
        .reusableValue = uiText("Can seed Watch or Startup", "可作为 Watch 或 Startup 种子"),
        // reviewFailure field
        .reviewFailure = uiText("Review failure before reuse", "复用前先复核失败原因"),
        // waitingRuntime field
        .waitingRuntime = uiText("Waiting for runtime result", "等待运行时结果"),
        // summaryPattern field
        .summaryPattern = uiText("%1 %2 | #%3 %4:%5 %6 | Value: %7 | Status: %8 | %9",
        ...
        // localBoundary field
        .localBoundary = uiText("Local audit boundary: selecting this row, reading this detail "
                                ...
                                "Health。"),
        // executionBoundary field
        .executionBoundary = uiText("Execution boundary: double-click and Fill and Read explicitly read "
...
ObjectBookmarkDetailTexts MainWindow::objectBookmarkDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("Object bookmark evidence is not available.", "当前没有可用的对象书签证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select an object bookmark to review target, access, saved value, "
                                  "source, reuse options, and operation boundary.",
                                  "选择一个对象书签，以复核目标、权限、保存值、来源、复用选项和操作边"
                                  "界。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selecting bookmarks and reading this detail strip are local project "
        ...
        .readyForWatchStartup = uiText("Ready for Watch or Startup", "可加入 Watch 或生成 Startup"),
        // readOnlyWatchOnly field
        .readOnlyWatchOnly = uiText("Read-only evidence; Watch only", "只读证据；适合 Watch"),
        // missingAddress field
        .missingAddress = uiText("Missing bookmark address", "缺少书签地址"),
        // noSavedValue field
        .noSavedValue = uiText("No saved value; fill target only", "无保存值；仅回填目标"),
        // summaryPattern field
        .summaryPattern = uiText("#%1 %2:%3 %4 | %5 | %6 | Last: %7 | Source: %8 | %9",
        ...
        .reuseLabel = uiText("Reuse", "复用"),
        // localBoundary field
        .localBoundary = uiText("Local bookmark boundary: selecting this bookmark, reading this "
                                // ── Watch Row Texts ─────────────────────────────────────────────────
                                ...
                                "Health。"),
        // executionBoundary field
        .executionBoundary = uiText("Execution boundary: Fill and Read explicitly reads the bookmarked "
...
SdoTargetTrailDetailTexts MainWindow::sdoTargetTrailDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("SDO target trail evidence is not available.", "当前没有可用的 SDO 目标轨迹证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select an SDO target trail row to review target, source, value, "
                                  "write value, reuse readiness, and operation boundary.",
                                  "选择一条 SDO 目标轨迹行，以复核目标、来源、值、写入值、复用就绪度和"
                                  "操作边界。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selecting rows and reading this detail strip are local review "
        ...
        // readyForReuse field
        .readyForReuse = uiText("Ready for Watch, Bookmark, or Startup", "可加入 Watch、收藏或生成 Startup"),
        // watchBookmarkOnly field
        .watchBookmarkOnly = uiText("Can seed Watch or Bookmark; Startup needs writable value",
        ...
        .noSavedValue = uiText("No saved value; fill target only", "无保存值；仅回填目标"),
        // summaryPattern field
        .summaryPattern = uiText("%1 | #%2 %3:%4 %5 | Source: %6 | Value: %7 | Write: %8 | %9",
                                 "%1 | #%2 %3:%4 %5 | 来源：%6 | 值：%7 | 写入：%8 | %9"),
        // selectedTitle field
        .selectedTitle = uiText("Selected SDO target trail row", "选中的 SDO 目标轨迹行"),
        // timeLabel field
        .timeLabel = uiText("Time", "时间"),
        // slaveLabel field
        .slaveLabel = uiText("Slave", "从站"),
        ...
        .reuseLabel = uiText("Reuse", "复用"),
        // localBoundary field
        .localBoundary = uiText("Local trail boundary: selecting this row, reading this detail "
                                ...
                                "Health。"),
        // executionBoundary field
        .executionBoundary = uiText("Execution boundary: explicit Read SDO reads the current target; "
...
WatchStartupDeltaTexts MainWindow::watchStartupDeltaTexts() const {
    return {uiText("no watch", "无监视"), uiText("pending", "待比较"), uiText("match", "匹配"),
            uiText("diff", "不一致")};
}
...
StateMachineRowDetailTexts MainWindow::stateMachineRowDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("State evidence is not available.", "当前没有可用的状态机证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select a visible slave row to review current state, recommendation, "
                                  "evidence, risk, and confirmation boundary.",
                                  "选择一条可见从站行，以复核当前状态、推荐状态、证据、风险和确认边界"
                                  "。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selection only updates local evidence. State buttons and row "
        ...
        .confirmedRequestBoundary = uiText("Explicit confirmed state request", "显式确认状态请求"),
        // localReviewBoundaryLabel field
        .localReviewBoundaryLabel = uiText("Local evidence review", "本地证据复核"),
        // opBoundary field
        .opBoundary = uiText("OP transition requires confirmation", "OP 切换需要确认"),
        // safeopBoundary field
        .safeopBoundary = uiText("SAFEOP transition requires confirmation", "SAFEOP 切换需要确认"),
        // preopBoundary field
        .preopBoundary = uiText("PREOP transition requires confirmation", "PREOP 切换需要确认"),
        // summaryPattern field
        .summaryPattern = uiText("#%1 %2 | Current: %3 | Recommended: %4 | %5 | Risk: %6 | "
                                 "Action: %7",
        ...
        // localBoundary field
        .localBoundary = uiText("Local preview boundary: selecting this row and reading this detail "
        ...
        // executionBoundary field
        .executionBoundary = uiText("Execution boundary: Send Recommended, PREOP, SAFEOP, OP, All PREOP, "
...
WorkflowStepDetailTexts MainWindow::workflowStepDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("Workflow evidence is not available.", "当前没有可用的工作流证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("The detail strip is local only and does not access the bus.", "详情条仅在本地工作，不访问总线。"),
        // noSelectionText field
        .noSelectionText = uiText("Select a visible workflow row to review risk, evidence, "
                                  "next action, and execution boundary.",
                                  "选择一条可见工作流行，以复核风险、依据、下一步和执行边界。"),
        // noSelectionTip field
        .noSelectionTip = uiText("Selection, filtering, review, and copy are local UI actions. Run "
        ...
        .noRisk = uiText("No risk", "无风险"),
        // summaryPattern field
        .summaryPattern = uiText("#%1 %2 / %3 | %4 | Risk: %5 | Evidence: %6 | Next: %7",
        ...
        .localReviewBoundary = uiText("Local review boundary: selecting, filtering, Review First/Next, "
...
CommissioningWorkflowTexts MainWindow::commissioningWorkflowTexts() const {
    return {
        ...
        .connectRuntimeBoundary = uiText("Run Next or double-click may connect to the runtime or "
                                         "refresh online master data.",
                                         "执行下一步或双击可能连接运行时，或刷新在线主站数据。"),
        // scanTopologyBoundary field
        .scanTopologyBoundary = uiText("Run Next or double-click may request a topology scan and "
                                       "online refresh.",
                                       "执行下一步或双击可能请求总线重扫和在线刷新。"),
        // selectSlaveBoundary field
        .selectSlaveBoundary =
            uiText("Run Next or double-click focuses the topology tree only.", "执行下一步或双击只会聚焦拓扑树。"),
        // inspectObjectDictionaryBoundary field
        .inspectObjectDictionaryBoundary = uiText("Run Next or double-click opens Object Dictionary and may "
                                                  "load SDO metadata for the selected slave.",
                                                  "执行下一步或双击会打开对象字典，并可能加载当前从站的 SDO "
                                                  "元数据。"),
        // reviewObjectDictionaryEvidenceBoundary field
        .reviewObjectDictionaryEvidenceBoundary = uiText("Run Next or double-click opens already loaded failed OD "
                                                         "evidence.",
                                                         "执行下一步或双击会打开已加载的失败 OD 证据。"),
        // reviewPdoMapBoundary field
        .reviewPdoMapBoundary = uiText("Run Next or double-click opens PDO Map and may load process "
                                       "data mapping for the selected slave.",
                                       "执行下一步或双击会打开 PDO 映射，并可能加载当前从站的过程数据"
                                       "映射。"),
        // monitorWatchBoundary field
        .monitorWatchBoundary = uiText("Run Next or double-click opens Watch and adds the current "
        ...
        .reviewStartupDiffsBoundary = uiText("Run Next or double-click opens already loaded Startup diff "
                                             "evidence.",
                                             "执行下一步或双击会打开已加载的 Startup 偏差证据。"),
        // runConsistencyGateBoundary field
        .runConsistencyGateBoundary = uiText("Run Next or double-click opens Consistency evidence or the "
        ...
        .validateProcessImageBoundary = uiText("Run Next or double-click opens Free Run and may toggle "
...
NextBestActionTexts MainWindow::nextBestActionTexts() const {
    return {
        ...
        .commandPaletteTip = uiText("Open the command palette for focused operations.", "打开命令面板执行聚焦操作。"),
        // nextConnect field
        .nextConnect = uiText("Next: Connect", "下一步：连接"),
        // connectTip field
        .connectTip = uiText("Connect to the local ecatd runtime.", "连接本机 ecatd 运行时。"),
        // reviewDiagnostics field
        .reviewDiagnostics = uiText("Review Diagnostics", "查看诊断"),
        // diagnosticsTip field
        .diagnosticsTip = uiText("Open Diagnostics to review current runtime or host errors.",
                                 "打开诊断页复核当前运行时或主机错误。"),
        // nextRescan field
        .nextRescan = uiText("Next: Rescan", "下一步：重扫"),
        // rescanTip field
        .rescanTip = uiText("Rescan and refresh the active EtherCAT master.", "重新扫描并刷新当前 EtherCAT 主站。"),
        // nextSelectSlave field
        .nextSelectSlave = uiText("Next: Select Slave", "下一步：选从站"),
        // selectSlaveTip field
        .selectSlaveTip =
            uiText("Focus the I/O tree so a target slave can be selected.", "聚焦 I/O 树以选择目标从站。"),
        // nextLoadOd field
        ...
        .failedOdEvidenceTip = uiText("Open Object Dictionary and focus the first failed SDO "
                                      "evidence row.",
                                      "打开对象字典并聚焦第一条失败 SDO 证据行。"),
        // nextLoadPdo field
        .nextLoadPdo = uiText("Next: Load PDO", "下一步：加载 PDO"),
        // loadPdoTip field
        .loadPdoTip = uiText("Open PDO Map and load process-data mapping for the selected "
                             "slave.",
                             "打开 PDO 映射并加载当前从站的过程数据映射。"),
        // nextAddWatch field
        .nextAddWatch = uiText("Next: Add Watch", "下一步：加入监视"),
        // addWatchTip field
        .addWatchTip = uiText("Add the current SDO fields to Watch, then refresh key values.",
        ...
        .startupDiffsTip = uiText("Open Startup SDO and focus rows whose expected values differ "
        ...
        .consistencyEvidenceTip = uiText("Open the best loaded evidence table for the first blocking "
        ...
        .reviewConsistencyTip = uiText("Open Consistency Check and review blocking online/offline "
        ...
        .runConsistencyTip = uiText("Run the read-only Consistency Check before process image "
                                    "validation or state progression.",
                                    "在验证过程映像或推进状态前运行只读一致性检查。"),
        // nextFreeRun field
        .nextFreeRun = uiText("Next: Free Run", "下一步：自由运行"),
        // freeRunTip field
        .freeRunTip = uiText("Open Free Run and start process-image telemetry.", "打开 Free Run 并启动过程映像遥测。"),
        ...
        .matrixTipPattern = uiText("Open the highest-priority visible row in the Overview slave "
...
HostHealthTexts MainWindow::hostHealthTexts() const {
    return {
        ...
        // unchecked field
        .unchecked = uiText("Host health has not been checked", "尚未运行主机健康检查"),
...
ConsistencyDetailTexts MainWindow::consistencyDetailTexts() const {
    return {
        // unavailableText field
        .unavailableText = uiText("Consistency evidence is not available.", "当前没有可用的一致性证据。"),
        // unavailableTip field
        .unavailableTip =
            uiText("This preview is local only and does not access the bus.", "此预览仅在本地工作，不访问总线。"),
        // selectVisibleRowText field
        .selectVisibleRowText = uiText("Select a visible Consistency row to review severity, scope, "
                                       "target, expected/actual evidence, recommended action, and "
                                       "local route.",
                                       "选择一条可见一致性行，以复核级别、范围、目标、期望/实际证"
                                       "据、建议动作和本地路由。"),
        // selectVisibleRowTip field
        .selectVisibleRowTip = uiText("Filtering, selecting, reading this detail strip, and Open "
                                      // ── I/O Variable Texts ──────────────────────────────────────────────
                                      ...
                                      "Health。"),
        // summaryPattern field
        .summaryPattern = uiText("%1 | %2 | %3 | Expected: %4 | Actual: %5 | Action: %6",
        ...
        // routeTopology field
        .routeTopology = uiText("Open State Machine/topology evidence", "打开状态机/拓扑证据"),
        // routeStartup field
        .routeStartup = uiText("Open Startup SDO evidence", "打开 Startup SDO 证据"),
        // routeWatchOrIo field
        .routeWatchOrIo = uiText("Open Watch or I/O evidence", "打开 Watch 或 I/O 证据"),
        // routeReady field
        .routeReady = uiText("Ready row; continue commissioning", "就绪行；继续调试"),
        // localBoundary field
        .localBoundary = uiText("Local gate boundary: selecting this row, filtering "
                                ...
                                "加载的本地证据表。"),
        // executionBoundary field
        .executionBoundary = uiText("Execution boundary: Refresh Check rebuilds the Consistency "
...
WorkspaceBoundaryTexts MainWindow::workspaceBoundaryTexts() const {
    return {
        ...
        .overviewMixedActions = uiText("Overview mixes local evidence navigation with explicit "
                                       "online actions such as Connect, Refresh, Run Next, and Drive "
                                       "Next.",
                                       "总览页混合本地证据导航和显式在线动作，例如连接、刷新、执行"
                                       "下一步和驱动下一步。"),
        // overviewLocalEvidence field
        .overviewLocalEvidence = uiText("Session Brief rows and Slave Evidence Matrix review remain "
                                        "local evidence navigation.",
                                        "会话简报行和从站证据矩阵审阅仍是本地证据导航。"),
        // overviewMatrixPattern field
        .overviewMatrixPattern =
            uiText("Matrix queue: P0 %1 | P1 %2 | P2 %3 | P3 %4", "矩阵队列：P0 %1 | P1 %2 | P2 %3 | P3 %4"),
        // objectDictionaryLabel field
        .objectDictionaryLabel = uiText("Boundary: SDO", "边界：SDO"),
        // objectDictionaryLocalFill field
        .objectDictionaryLocalFill = uiText("Selecting rows and Alt+Enter only fill local SDO context.",
                                            "选择行和 Alt+Enter 只回填本地 SDO 上下文。"),
        // objectDictionaryOnlineAccess field
        .objectDictionaryOnlineAccess = uiText("Read, Write, batch reads, and failed-row retry explicitly "
        ...
        .pdoMapLoadedLocal = uiText("PDO Map review is local once loaded; loading or refreshing "
                                    "PDO data is an online action.",
                                    "PDO 映射加载后可本地审阅；加载或刷新 PDO 数据属于在线动作。"),
        // pdoMapLocalFill field
        .pdoMapLocalFill = uiText("Alt+Enter only fills the SDO target from the selected PDO "
                                  "row.",
                                  "Alt+Enter 只从选中 PDO 行回填 SDO 目标。"),
        // watchLabel field
        .watchLabel = uiText("Boundary: Watch Reads", "边界：Watch 读取"),
        // watchReadsOnline field
        .watchReadsOnline = uiText("Watch rows and scopes are local evidence; Refresh and Auto "
                                   "poll SDO objects.",
                                   "Watch 行和范围是本地证据；刷新和自动模式会轮询 SDO 对象。"),
        // watchStartupLocal field
        .watchStartupLocal = uiText("Startup sync changes only the Startup table until Apply is "
                                    "used.",
                                    "同步 Startup 只改 Startup 表，直到使用应用动作才写总线。"),
        // startupSdoLabel field
        .startupSdoLabel = uiText("Boundary: Startup Danger", "边界：Startup 风险"),
        // startupSdoLocalEditing field
        .startupSdoLocalEditing = uiText("Editing, preflight, Alt+Enter, and Watch diff review are "
                                         "local.",
                                         "编辑、预检查、Alt+Enter 和 Watch 偏差审阅是本地动作。"),
        // startupSdoOnlineApply field
        .startupSdoOnlineApply = uiText("Apply, Apply Selected, Apply Diffs, and Verify use online "
        ...
        .freeRunProcessData = uiText("Free Run telemetry can exchange process data with devices.",
                                     "Free Run 遥测可能与设备交换过程数据。"),
        // freeRunLocalFiltering field
        .freeRunLocalFiltering = uiText("Filtering rows and Alt+Enter are local; toggling Free Run "
        ...
        .ioVariablesMergedEvidence = uiText("I/O Variables merges loaded PDO, Free Run, Watch, Startup, "
                                            "and project metadata.",
                                            "I/O 变量合并已加载 PDO、Free Run、Watch、Startup 和工程元数"
                                            "据。"),
        // ioVariablesLocalEditing field
        .ioVariablesLocalEditing = uiText("Alias, tags, exports, Alt+Enter, and Watch creation are "
        ...
        .consistencyLoadedEvidence = uiText("Consistency uses already loaded session and project "
                                            "evidence.",
                                            "一致性只使用已加载会话和工程证据。"),
        // consistencyLocalNavigation field
        .consistencyLocalNavigation = uiText("Open Evidence and Alt+Enter navigate local evidence only.",
        ...
        .stateMachineOnlineRequests = uiText("The matrix is local evidence, but Send Recommended and "
        ...
        .stateMachineConfirmation =
            uiText("State requests keep the grouped risk confirmation flow.", "状态请求保留风险分组确认流程。"),
        // diagnosticsLabel field
        .diagnosticsLabel = uiText("Boundary: Host", "边界：主机"),
        // diagnosticsHostOnly field
        .diagnosticsHostOnly =
            uiText("Diagnostics is the only Host Health workspace.", "诊断页是唯一的 Host Health 工作区。"),
        // diagnosticsHostCheck field
        .diagnosticsHostCheck = uiText("Running Host Check inspects the host; Copy Command only "
                                       "copies repair text.",
                                       "运行 Host Check 会检查主机；复制命令只复制修复文本。"),
        // rtTestLabel field
        .rtTestLabel = uiText("Boundary: RT Test", "边界：RT 测试"),
        // rtTestOnlineCycle field
        .rtTestOnlineCycle = uiText("RT test exercises the bus with ecrt receive/send cycles.",
                                    "RT 测试通过 ecrt 收发周期对总线进行压力测试。"),
        // rtTestLocalStats field
        .rtTestLocalStats = uiText("Timeline and statistics are local display only.", "时间线和统计仅为本地显示。"),
        // esiLabel field
        .esiLabel = uiText("Boundary: File/ESI", "边界：文件/ESI"),
        // esiFileEvidence field
        .esiFileEvidence = uiText("ESI repository and XML review are file/project evidence.",
                                  "ESI 仓库和 XML 查看属于文件/工程证据。"),
        // esiImportAction field
        .esiImportAction = uiText("Importing ESI XML is an explicit file action.", "导入 ESI XML 是显式文件动作。"),
        // notesLabel field
        .notesLabel = uiText("Boundary: Project", "边界：工程"),
        // notesLocalRecords field
        .notesLocalRecords = uiText("Notes are project-local commissioning records.", "备注是工程内调试记录。"),
        // rawEvidenceLabel field
        .rawEvidenceLabel = uiText("Boundary: Raw Evidence", "边界：原始证据"),
        // rawEvidenceCachedOutput field
        .rawEvidenceCachedOutput = uiText("Raw output pages show cached command output and do not act "
...
WorkspaceTabBadgeTexts MainWindow::workspaceTabBadgeTexts() const {
    return {
        ...
        .overviewTipPattern = uiText("Slave matrix priority queue: P0 %1; P1 %2; P2 %3; P3 %4",
                                     "从站矩阵优先级队列：P0 %1；P1 %2；P2 %3；P3 %4"),
        // watchTipPattern field
        .watchTipPattern = uiText("Watch rows: %1; Startup mismatches: %2", "Watch 行：%1；Startup 不一致：%2"),
        // startupSdoTipPattern field
        .startupSdoTipPattern =
            uiText("Startup SDO rows: %1; Watch mismatches: %2", "Startup SDO 行：%1；Watch 不一致：%2"),
        // freeRunTipPattern field
        .freeRunTipPattern = uiText("Process-image rows: %1", "过程映像行：%1"),
        // ioVariablesTipPattern field
        .ioVariablesTipPattern = uiText("I/O variable rows: %1; review issues: %2", "I/O 变量行：%1；需复核问题：%2"),
        // consistencyTipPattern field
        .consistencyTipPattern = uiText("Consistency errors: %1; warnings: %2; info: %3; ready: %4",
                                        "一致性错误：%1；警告：%2；信息：%3；就绪：%4"),
        // stateMachineTipPattern field
        .stateMachineTipPattern = uiText("State-machine risk rows: %1", "状态机风险行：%1"),
        // diagnosticsTipPattern field
        .diagnosticsTipPattern =
            uiText("Diagnostics errors: %1; warnings: %2; info: %3", "诊断错误：%1；警告：%2；信息：%3"),
    };
}
