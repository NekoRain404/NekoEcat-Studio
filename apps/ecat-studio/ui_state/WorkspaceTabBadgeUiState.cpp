...
QString workspaceTabBadgeText(const QString &label, int count, bool issue) {
  ...
  return QString("%1 %2%3").arg(label, issue ? QStringLiteral("!") : QString(),
...
buildWorkspaceTabBadgeUiState(const WorkspaceTabBadgeCounts &counts,
  ...
  const int diagnosticIssues =
      counts.diagnosticErrors + counts.diagnosticWarnings;

  state.overview = {
      workspaceTabBadgeText(texts.overview, matrixIssues,
                            counts.matrixP0 + counts.matrixP1 > 0),
      texts.overviewTipPattern.arg(counts.matrixP0)
          .arg(counts.matrixP1)
          .arg(counts.matrixP2)
          .arg(counts.matrixP3)};
  state.watch = {workspaceTabBadgeText(texts.watch, counts.watchRows),
                 texts.watchTipPattern.arg(counts.watchRows)
                     .arg(counts.watchStartupDiffs)};
  state.startupSdo = {workspaceTabBadgeText(texts.startupSdo,
                                            counts.startupDiffs > 0
                                                ? counts.startupDiffs
                                                : counts.startupRows,
                                            counts.startupDiffs > 0),
                      texts.startupSdoTipPattern.arg(counts.startupRows)
                          .arg(counts.startupDiffs)};
  state.freeRun = {workspaceTabBadgeText(texts.freeRun, counts.freeRunRows),
                   texts.freeRunTipPattern.arg(counts.freeRunRows)};
  state.ioVariables = {
      workspaceTabBadgeText(texts.ioVariables,
                            counts.ioIssues > 0 ? counts.ioIssues
                                                : counts.ioRows,
                            counts.ioIssues > 0),
