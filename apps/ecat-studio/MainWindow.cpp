...
MainWindow::MainWindow(QWidget* parent) : QMainWindow(parent) {
    ...
    connect(pluginRegistry_, &PluginRegistry::registrationFailed, this,
            [this](const QString& reason, const QString& pluginId) {
                ...
                updateDiagnostics("Warning", "Plugins", QString("Plugin registration failed: %1").arg(reason));
            });
    ...
    connect(container_, &ServiceContainer::serviceInitFailed, this,
            [this](const QString& serviceName, const QString& reason) {
                log(QString("Service initialization failed: %1 — %2").arg(serviceName, reason));
                updateDiagnostics("Error", "Services",
                                  QString("Service '%1' failed to initialize: %2").arg(serviceName, reason));
            });
    if (!container_->isInitialized()) {
        const QStringList errors = container_->initializationErrors();
        log(QString("ServiceContainer has %1 initialization error(s): %2").arg(errors.size()).arg(errors.join(", ")));
        updateDiagnostics("Warning", "Services",
                          QString("Some services failed to initialize: %1").arg(errors.join(", ")));
    }
...
void MainWindow::clearOnlineViews() {
    ...
    for (auto* table : {metricTable_, workflow_->workflowTable, stateMachine_->stateMachineTable, identityTable_,
                        slaveEvidence_->slaveEvidenceMatrixTable, portTable_, mailboxTable_, sdo_->pdoTable,
                        sdo_->sdoTable, sdoHistoryTable_, freeRunWidgets_->freeRunTable,
                        freeRunWidgets_->freeRunEntryTable, ioVar_->ioVariableTable, watch_->watchTable}) {
        if (table) {
            table->clear();
            table->setRowCount(0);
    ...
    setMetricCard(masterStateLabel_, uiText("Master", "主站"), uiText("Idle", "空闲"));
    setMetricCard(slaveCountLabel_, uiText("Slaves", "从站"), "0");
    setMetricCard(linkStateLabel_, uiText("Link", "链路"), uiText("Unknown", "未知"));
...
void MainWindow::startEmbeddedDaemon() {
    connect(&daemon_, &QProcess::readyReadStandardError, this, [this] {
        const QString text = QString::fromLocal8Bit(daemon_.readAllStandardError()).trimmed();
        if (!text.isEmpty()) {
            log("ecatd: " + text);
        }
    });
    connect(&daemon_, &QProcess::readyReadStandardOutput, this, [this] {
        const QString text = QString::fromLocal8Bit(daemon_.readAllStandardOutput()).trimmed();
        if (!text.isEmpty()) {
            log("ecatd: " + text);
        }
    });
    connect(&daemon_, &QProcess::errorOccurred, this, [this](QProcess::ProcessError error) {
        QString message;
        switch (error) {
    ...
    connect(&daemon_, QOverload<int, QProcess::ExitStatus>::of(&QProcess::finished), this,
            [this](int exitCode, QProcess::ExitStatus exitStatus) {
                if (exitStatus == QProcess::CrashExit) {
                    const QString message = QString("Embedded ecatd crashed (exit code %1)").arg(exitCode);
                    log(message);
                    updateDiagnostics("Error", "Runtime", message);
                } else if (exitCode != 0) {
                    const QString message = QString("Embedded ecatd exited with code %1").arg(exitCode);
                    log(message);
                    updateDiagnostics("Warning", "Runtime", message);
                } else {
