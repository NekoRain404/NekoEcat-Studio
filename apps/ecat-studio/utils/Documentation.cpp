...
QString documentationBrowserStyle(const QString& objectName, bool lightTheme) {
    ...
    return lightTheme ? QStringLiteral("%1 { background: #ffffff; border: 1px solid #d9e1ec; "
                                       "border-radius: 8px; padding: 14px; }")
                            .arg(selector)
                      : QStringLiteral("%1 { background: #151b25; border: 1px solid #2a3546; "
                                       "border-radius: 8px; padding: 14px; }")
                            .arg(selector);
}

// Creates a read-only, frameless QTextBrowser styled for the active theme.
QTextBrowser* makeDocumentationBrowser(const QString& objectName, bool lightTheme, bool openExternalLinks) {
...
ManualSearchControls makeManualSearchControls(QObject* owner, QTextBrowser* browser, QStyle* style,
                                              const QString& searchPlaceholder, const QString& previousText,
                                              const QString& nextText, const QString& contentsText) {
    ...
    QObject::connect(search, &QLineEdit::textChanged, owner, [browser, findText](const QString& text) {
        QTextCursor cursor = browser->textCursor();
    ...
    QObject::connect(findPrevious, &QPushButton::clicked, owner, [findText] { findText(QTextDocument::FindBackward); });
    QObject::connect(findNext, &QPushButton::clicked, owner, [findText] { findText(); });
    QObject::connect(contents, &QPushButton::clicked, owner, [browser] { browser->scrollToAnchor("contents"); });
...
QString finalizeDocumentationHtml(const QString& htmlTemplate, const QString& version, const QString& activeMaster,
