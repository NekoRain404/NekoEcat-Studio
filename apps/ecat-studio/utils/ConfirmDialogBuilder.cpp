...
static bool containsAny(const QString& text, std::initializer_list<const char*> words) {
...
static bool startsWithAny(const QString& text, std::initializer_list<const char*> words) {
...
static QString sectionHtml(const QString& title, const QString& subtitle, const QString& color,
                           const QStringList& items, const QString& theme) {
    if (items.isEmpty()) {
        return QString();
    }
    QString html = QString("<section style='border:1px solid %1; border-left:4px solid "
    ...
    if (!subtitle.trimmed().isEmpty()) {
        html += QString("<p style='margin:0 0 6px 0; color:%1;'>%2</p>")
                    .arg(theme == "Light" ? "#475569" : "#b9c6d6", subtitle.toHtmlEscaped());
    }
    html += "<ul style='margin:6px 0 0 18px; padding:0;'>";
    for (const QString& item : items) {
        html += QString("<li style='margin:4px 0;'>%1</li>").arg(item.toHtmlEscaped());
    }
    html += "</ul></section>";
    return html;
}
...
bool confirm(QWidget* parent, const QString& title, const QString& summary, const QStringList& details,
    ...
    for (const QString& detail : details) {
        ...
        if (startsWithAny(trimmed, {"risk:", "风险：", "critical:", "critical："}) ||
            containsAny(trimmed,
                        {"drive command", "drive/mode", "actuator safety", "persistent storage", "output behavior",
                         "outputs and drive behavior", "op can make outputs", "can affect real hardware", "驱动命令",
                         "驱动/模式", "执行机构安全", "持久化", "输出行为", "op 可能"})) {
            ...
            evidenceItems << trimmed;
                       ...
                       trimmed,
                       {"master:", "slave:",           "row:",           "object:",          "type:",
                        "value:",  "requested state:", "current state:", "detected slaves:", "current state mix:",
                        "主站：",  "从站：",           "行：",           "对象：",           "类型：",
                        "值：",    "目标状态：",       "当前状态：",     "检测到从站：",     "当前状态分布："}) ||
                   containsAny(trimmed,
                               {"this operation", "this sends", "confirm machine safety", "此操作", "继续前请确认"})) {
    ...
    const QString severity =
        !criticalItems.isEmpty()
            ? uiText("Critical impact items require explicit review.", "存在关键影响项，需要明确复核。")
            : (!reviewItems.isEmpty() ? uiText("Review warnings before confirming.", "确认前请复核警告项。")
    ...
    severityLabel->setStyleSheet(!criticalItems.isEmpty()
                                     ? QStringLiteral("QLabel { color: #ef4444; font-weight: 700; }")
                                     : (!reviewItems.isEmpty() ? QStringLiteral("QLabel { color: #f59e0b; font-weight: "
                                                                                "700; }")
                                                               : QStringLiteral("QLabel { color: #16a34a; font-weight: "
                                                                                "700; }")));

    const QString foreground = theme == "Light" ? QStringLiteral("#172033") : QStringLiteral("#e6edf5");
    const QString muted = theme == "Light" ? QStringLiteral("#475569") : QStringLiteral("#b9c6d6");
    const QString background = theme == "Light" ? QStringLiteral("#ffffff") : QStringLiteral("#151b25");
    const QString border = theme == "Light" ? QStringLiteral("#d9e1ec") : QStringLiteral("#2a3546");
    ...
    html += sectionHtml(uiText("Other Details", "其他细节"), QString(), "#64748b", otherItems, theme);
    html += QString("<p style='color:%1; margin:10px 0 0 0;'>%2</p>")
                .arg(muted, uiText("The confirm button only authorizes the operation; "
    ...
    auto* confirmBtn = buttons->addButton(confirmText, QDialogButtonBox::AcceptRole);
