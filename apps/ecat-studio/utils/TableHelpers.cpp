...
bool tableObjectIndexMatches(QTableWidget* table, int row, const QString& index, const QString& subIndex,
                             int indexColumn, int subIndexColumn) {
    if (!table || row < 0 || row >= table->rowCount() || index.trimmed().isEmpty() || subIndex.trimmed().isEmpty()) {
        return false;
    }

    return normalizeHexText(tableText(table, row, indexColumn), 4) == normalizeHexText(index, 4) &&
           normalizeHexText(tableText(table, row, subIndexColumn), 2) == normalizeHexText(subIndex, 2);
}

// Checks full position + index:subIndex match for a row, used in evidence lookups.
bool tableObjectAddressMatches(QTableWidget* table, int row, int position, const QString& index,
                               const QString& subIndex, int positionColumn, int indexColumn, int subIndexColumn) {
    if (!table || position < 0) {
        return false;
    }
    ...
    return ok && rowPosition == position &&
           tableObjectIndexMatches(table, row, index, subIndex, indexColumn, subIndexColumn);
}

// Linear scan for the first row matching an index:subIndex pair.
int tableRowForObjectIndex(QTableWidget* table, const QString& index, const QString& subIndex, int indexColumn,
                           int subIndexColumn) {
    ...
    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        if (tableObjectIndexMatches(table, row, index, subIndex, indexColumn, subIndexColumn)) {
            return row;
        }
    }
    return -1;
...
int tableRowForObjectAddress(QTableWidget* table, int position, const QString& index, const QString& subIndex,
                             int positionColumn, int indexColumn, int subIndexColumn) {
    if (!table || position < 0 || index.trimmed().isEmpty() || subIndex.trimmed().isEmpty()) {
        return -1;
    }

    // Iterate over collection
    for (int row = 0; row < table->rowCount(); ++row) {
        if (tableObjectAddressMatches(table, row, position, index, subIndex, positionColumn, indexColumn,
                                      subIndexColumn)) {
            return row;
        }
    }
...
QVector<int> selectedTableRows(QTableWidget* table, bool visibleOnly, bool includeCurrentFallback) {
    QVector<int> rows;
    if (!table) {
        return rows;
    }
    ...
    if (rows.isEmpty() && includeCurrentFallback && current >= 0 && current < table->rowCount() &&
...
bool selectAndFocusTableRow(QTableWidget* table, int row, int column) {
    ...
    const int safeColumn = column >= 0 && column < table->columnCount() ? column : 0;
    table->selectRow(row);
...
void fitTableColumnsToViewport(QTableWidget* table, int stretchColumn) {
    ...
    const int safeStretchColumn =
        stretchColumn >= 0 && stretchColumn < table->columnCount() ? stretchColumn : table->columnCount() - 1;
...
void populateTable(QTableWidget* table, const QStringList& headers, const QList<QStringList>& rows) {
