...
QVector<ActionState> computeActionStates(const ActionFlags& f) {
    ...
    states << ActionState{"readSelectedDictionary", f.connected && f.hasSlave && f.hasDictionarySelection};
    states << ActionState{"readVisibleDictionary", f.connected && f.hasSlave && f.hasVisibleDictionaryRows};
    states << ActionState{"readFailedDictionary", f.connected && f.hasSlave && f.hasFailedSdoEvidence};
    states << ActionState{"writeSdo", f.connected && f.hasSlave && f.selectedSdoWritable};
    states << ActionState{"writeTargetSdo", f.connected && f.hasCurrentSdoTarget && f.selectedSdoWritable};
    states << ActionState{"useSdoValue", f.selectedSdoWritable && f.sdoValueNonEmpty};
    states << ActionState{"useSdoEvidence", f.selectedSdoWritable && f.sdoEvidenceNonEmpty};
    states << ActionState{"pickSdoEvidence", f.selectedSdoWritable && f.hasSdoEvidenceCandidates};
    ...
    states << ActionState{"startupTargetSdo", f.connected && f.hasCurrentSdoTarget};
    ...
    states << ActionState{"openSdoTargetTrailLink", f.currentSdoTargetTrailRow >= 0};

    // ── Dictionary / Bookmarks / Trail ─────────────────────────────
    states << ActionState{"watchSelectedDictionary", f.hasSlave && f.hasDictionarySelection};
    states << ActionState{"watchVisibleDictionary", f.hasSlave && f.hasVisibleDictionaryRows};
    states << ActionState{"startupSelectedEvidence", f.hasSlave && f.hasDictionaryValueSelection};
    states << ActionState{"bookmarkTargetSdo", f.hasSlave && f.hasCurrentSdoTarget};
    states << ActionState{"bookmarkSelectedDictionary", f.hasSlave && f.hasDictionarySelection};
    ...
    states << ActionState{"bookmarkSdoTargetTrail", f.hasSdoTargetTrailSelection};
    states << ActionState{"startupSdoTargetTrail", f.canCreateStartupFromSdoTargetTrail};
    ...
    states << ActionState{"readIoVariableSdo", f.connected && f.hasIoVariableSelection};
    ...
    states << ActionState{"startupSelectedIoVariables", f.hasIoVariableValueSelection};
    states << ActionState{"startupVisibleIoVariables", f.hasVisibleIoVariableValues};
    states << ActionState{"editIoVariableMetadata", f.hasIoVariableSelection};
    states << ActionState{"bulkNameIoVariables", f.hasIoVariableSelection || f.hasVisibleIoVariables};
    ...
    states << ActionState{"copySelectedPlcDeclarations", f.hasIoVariableSelection};
    states << ActionState{"copyVisiblePlcDeclarations", f.hasVisibleIoVariables};
    states << ActionState{"exportIoVariablesAction", f.ioVariableTableHasRows};
    states << ActionState{"exportIoVariablesCsv", f.ioVariableTableHasRows};
    states << ActionState{"exportIoPlcSymbolsAction", f.ioVariableTableHasRows};
    ...
    states << ActionState{"openIoVariablesFromConsistency", f.ioVariableTabIndex >= 0};

    // ── History ────────────────────────────────────────────────────
    states << ActionState{"watchSelectedHistory", f.hasHistorySelection};
    states << ActionState{"startupFromSelectedHistory", f.hasHistoryValueSelection};

    // ── Watch ──────────────────────────────────────────────────────
    states << ActionState{"addCia402WatchPreset", f.hasSlave};
    states << ActionState{"refreshWatch", f.connected && f.watchTableHasRows};
