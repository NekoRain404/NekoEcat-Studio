...
bool commissioningWorkflowNoneText(const QString& text, const WorkflowStepDetailTexts& texts) {
    const QString normalized = text.trimmed().toLower();
    return normalized.isEmpty() || normalized == QStringLiteral("none") || normalized == texts.none.trimmed().toLower();
}
...
bool commissioningWorkflowReadyStatus(const CommissioningWorkflowTableRow& row,
    ...
    return row.statusKey == QStringLiteral("ready") || row.status.trimmed() == texts.ready;
}

// Whether the row status indicates an action is required.
bool commissioningWorkflowActionStatus(const CommissioningWorkflowTableRow& row,
                                       const CommissioningWorkflowTexts& texts) {
    return row.statusKey == QStringLiteral("action") || row.status.trimmed() == texts.action;
}

// Whether the row status indicates a blocked/waiting state.
bool commissioningWorkflowBlockedStatus(const CommissioningWorkflowTableRow& row,
                                        const CommissioningWorkflowTexts& texts) {
    return row.statusKey == QStringLiteral("blocked") || row.status.trimmed() == texts.blocked;
}
...
bool commissioningWorkflowSevereRiskText(const QString& risk) {
    return risk.contains(QStringLiteral("failed"), Qt::CaseInsensitive) ||
           risk.contains(QStringLiteral("error"), Qt::CaseInsensitive) || risk.contains(QStringLiteral("失败")) ||
...
QString workflowStepDetailSeverityKey(const CommissioningWorkflowTableRow& row,
    ...
    const bool severeRisk = hasRisk && commissioningWorkflowSevereRiskText(row.risk);
...
WorkflowStepDetailState buildWorkflowStepDetailState(const CommissioningWorkflowTableRow& row,
    ...
    state.severeRisk = state.hasRisk && commissioningWorkflowSevereRiskText(row.risk);
    state.displayRisk = state.hasRisk ? row.risk : texts.noRisk;
    state.severityKey = workflowStepDetailSeverityKey(row, workflowTexts, texts);

    const CommissioningWorkflowStepBoundary boundary =
        commissioningWorkflowStepBoundary(commissioningWorkflowStepForIndex(row.row), workflowTexts);
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.riskLabel, state.displayRisk);
    state.tooltipLines << QString("%1: %2").arg(texts.evidenceLabel, row.evidence);
    state.tooltipLines << QString("%1: %2").arg(texts.nextActionLabel, row.nextAction);
    state.tooltipLines << QString("%1: %2").arg(texts.boundaryLabel, state.boundaryKind);
