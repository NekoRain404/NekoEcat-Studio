...
QString slaveEvidencePriorityText(SlaveEvidencePriority priority, const SlaveEvidenceUiTexts& texts) {
    switch (priority) {
...
QString slaveEvidenceNextActionText(SlaveEvidenceNextAction action, const SlaveEvidenceUiTexts& texts) {
    switch (action) {
...
QString slaveEvidenceRiskText(const SlaveEvidenceRisk& risk, const SlaveEvidenceUiTexts& texts) {
    switch (risk.kind) {
...
QStringList slaveEvidenceRiskTexts(const QVector<SlaveEvidenceRisk>& risks, const SlaveEvidenceUiTexts& texts) {
    QStringList labels;
...
QString slaveEvidenceDisplayName(const QString& name, const SlaveEvidenceUiTexts& texts) {
    const QString trimmed = name.trimmed();
    return trimmed.isEmpty() ? texts.unnamed : trimmed;
}

// Returns the display state, falling back to "unknown" if empty.
QString slaveEvidenceDisplayState(const QString& state, const SlaveEvidenceUiTexts& texts) {
...
QStringList slaveEvidenceMatrixHeaders(const SlaveEvidenceUiTexts& texts) {
    return {
        texts.priorityHeader,  texts.slaveHeader,   texts.nameHeader, texts.stateHeader,
        texts.readinessHeader, texts.odHeader,      texts.pdoHeader,  texts.watchHeader,
        texts.startupHeader,   texts.processHeader, texts.riskHeader, texts.nextHeader,
    };
}

// Converts a domain row into a UI row with table cells and detail lines for the panel.
SlaveEvidenceUiRow slaveEvidenceUiRow(const SlaveEvidenceRow& row, const SlaveEvidenceUiTexts& texts) {
    ...
    const QString readiness =
        QString("%1% (%2/%3)").arg(slaveEvidenceReadinessPercent(row)).arg(row.readiness).arg(row.maxReadiness);
    ...
    uiRow.cells = {
        ...
        row.watchRows > 0 ? texts.watchValuesPattern.arg(row.watchValueRows).arg(row.watchRows) : texts.missing,
        row.startupRows > 0 ? texts.startupRowsPattern.arg(row.startupRows).arg(row.startupDiffs) : texts.noRows,
        row.processRows > 0 ? texts.processRowsPattern.arg(row.processRows).arg(row.mapIssues) : texts.missing,
    ...
    uiRow.detailLines << texts.slavePattern.arg(row.position).arg(slaveEvidenceDisplayName(row.name, texts));
    uiRow.detailLines << texts.priorityPattern.arg(priority);
    uiRow.detailLines << texts.statePattern.arg(slaveEvidenceDisplayState(row.state, texts));
    ...
    uiRow.detailLines << texts.watchValuesDetailPattern.arg(row.watchValueRows).arg(row.watchRows);
    uiRow.detailLines << texts.startupRowsDetailPattern.arg(row.startupRows).arg(row.startupDiffs);
    uiRow.detailLines << texts.processRowsDetailPattern.arg(row.processRows).arg(row.mapIssues);
