...
bool containsAny(const QString& text, std::initializer_list<const char*> keys) {
...
bool statusHasValidationIssue(const QString& status) {
    return containsAny(status, {"error", "failed"}) || status.contains(QStringLiteral("错误")) ||
           status.contains(QStringLiteral("失败"));
}

// Detects in-progress status (applying/verifying).
bool statusIsApplying(const QString& status) {
    return containsAny(status, {"applying", "verifying"}) || status.contains(QStringLiteral("应用中")) ||
...
bool deltaIsWatchDiff(const QString& delta) {
    return containsAny(delta, {"diff"}) || delta.contains(QStringLiteral("不一致"));
}

// Whether the delta indicates no watch entry was found.
bool deltaIsNoWatch(const QString& delta) {
    return containsAny(delta, {"no watch"}) || delta.contains(QStringLiteral("无监视"));
}

// Whether the comparison is still pending.
bool deltaIsPending(const QString& delta) {
    return containsAny(delta, {"pending"}) || delta.contains(QStringLiteral("待比较"));
}

// Whether the startup value matches the watch value.
bool deltaIsMatch(const QString& delta) {
    return containsAny(delta, {"match"}) || delta.contains(QStringLiteral("匹配"));
}
...
QString startupSdoRowDetailSeverityKey(const WatchStartupStartupRow& row, const StartupSdoRowDetailTexts&) {
    ...
    const bool missingTarget =
        startupPositionText(row).isEmpty() || row.index.isEmpty() || row.subIndex.isEmpty() || row.value.isEmpty();
...
StartupSdoRowDetailState buildStartupSdoRowDetailState(const WatchStartupStartupRow& row,
    ...
    state.missingTarget = slave.isEmpty() || row.index.isEmpty() || row.subIndex.isEmpty() || row.value.isEmpty();
    state.severityKey = startupSdoRowDetailSeverityKey(row, texts);

    state.evidence = state.watchDiff
                         ? texts.watchMismatch
                         : (state.noWatch ? texts.noWatchEvidence
                                          : (state.pending ? texts.pendingComparison
                                                           : (state.match ? texts.watchMatches : texts.reviewRow)));
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.rowLabel, QString::number(row.row + 1));
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.watchValueLabel, row.watchValue);
    state.tooltipLines << QString("%1: %2").arg(texts.watchDeltaLabel, row.watchDelta);
