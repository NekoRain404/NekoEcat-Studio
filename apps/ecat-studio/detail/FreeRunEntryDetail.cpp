...
bool freeRunEntryMapWarning(const FreeRunEntryTableRow& row) {
    ...
    return mapLower.contains(QStringLiteral("warning")) || mapLower.contains(QStringLiteral("missing")) ||
           mapLower.contains(QStringLiteral("no pdo map")) || row.mapStatus.contains(QStringLiteral("警告")) ||
           row.mapStatus.contains(QStringLiteral("缺失")) || row.mapStatus.contains(QStringLiteral("无 PDO 映射"));
}
...
bool freeRunEntryBitMismatch(const FreeRunEntryTableRow& row) {
    const QString mapLower = row.mapStatus.toLower();
    return mapLower.contains(QStringLiteral("bit mismatch")) || row.mapStatus.contains(QStringLiteral("位宽不一致"));
}

// Prefers decoded value over raw, falling back to the empty-value placeholder.
QString freeRunEntryValueText(const FreeRunEntryTableRow& row, const FreeRunEntryDetailTexts& texts) {
...
QString freeRunEntryBoundaryText(const FreeRunEntryTableRow& row, const FreeRunEntryDetailTexts& texts) {
    return freeRunEntryDetailIsOutputLike(row) ? texts.outputBoundary : texts.inputBoundary;
}
...
bool freeRunEntryDetailIsOutputLike(const FreeRunEntryTableRow& row) {
    ...
    return directionLower.contains(QStringLiteral("rx")) || directionLower.contains(QStringLiteral("out")) ||
...
QString freeRunEntryDetailNameSource(const FreeRunEntryTableRow& row, const FreeRunEntryDetailTexts& texts) {
    ...
    for (const QString& marker : texts.nameSourceMarkers) {
        ...
        const int markerIndex = row.mapDetail.indexOf(marker, 0, Qt::CaseInsensitive);
        if (markerIndex < 0) {
            continue;
        }
...
FreeRunEntryDetailState buildFreeRunEntryDetailState(const FreeRunEntryTableRow& row,
    ...
    const QString slave = row.positionValid ? QString::number(row.position) : QString();
    ...
    state.text = texts.summaryPattern.arg(slave.isEmpty() ? QString::number(row.row) : slave)
                     .arg(row.direction.isEmpty() ? texts.directionFallback : row.direction)
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.syncManagerLabel, row.syncManager);
    state.tooltipLines << QString("%1: %2").arg(texts.directionLabel, row.direction);
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.nameSourceLabel, state.nameSource);
    state.tooltipLines << QString("%1: %2 bit @ %3.%4").arg(texts.locationLabel, row.bits, row.offset, row.bit);
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.mapStatusLabel, row.mapStatus);
    state.tooltipLines << QString("%1: %2").arg(texts.mapDetailLabel, row.mapDetail);
    state.tooltipLines << QString("%1: %2").arg(texts.changedLabel, row.changed ? texts.yesText : texts.noText);
    state.tooltipLines << QString("%1: %2").arg(texts.boundaryLabel, state.boundary);
