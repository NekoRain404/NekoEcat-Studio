...
QString objectBookmarkDetailSeverityKey(const SdoObjectBookmarkRow& row, const ObjectBookmarkDetailTexts& texts) {
    const bool readOnly = sdoObjectAccessIsReadOnly(row.access, texts.readOnlyText);
...
ObjectBookmarkDetailState buildObjectBookmarkDetailState(const SdoObjectBookmarkRow& row,
    ...
    const QString displayName =
        row.name.isEmpty() ? (row.slaveName.isEmpty() ? texts.unnamed : row.slaveName) : row.name;
    state.text = texts.summaryPattern.arg(row.positionText.isEmpty() ? QStringLiteral("?") : row.positionText)
    ...
    state.tooltipLines << QString("%1: #%2 %3").arg(texts.slaveLabel, row.positionText, row.slaveName);
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.lastValueLabel, row.lastValue);
