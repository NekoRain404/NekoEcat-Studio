...
QStringList displayParts(const SelectedDriveSummaryEvidence& evidence, const SelectedDriveSummaryTexts& texts) {
    QStringList parts;
    if (!evidence.status.isEmpty()) {
        parts << evidence.status;
    }
...
SelectedDriveSummaryDetail selectedDriveNoWatchEvidenceState(const SelectedDriveSummaryTexts& texts) {
...
SelectedDriveSummaryEvidence selectedDriveSummaryEvidence(const QVector<WatchStartupWatchRow>& watchRows,
                                                          int position) {
    ...
    for (const auto& row : watchRows) {
        ...
        } else if (index == "0x603f" && !decoded.isEmpty() && !isZeroErrorValue(value)) {
...
QString selectedDriveSummarySeverityKey(const SelectedDriveSummaryEvidence& evidence) {
    return driveSeverityKey(driveEvidenceSeverity(summaryEvidenceParts(evidence).join(" | ")));
}

// Assembles the full drive summary card with evidence, severity, and formatted text.
SelectedDriveSummaryDetail buildSelectedDriveSummaryDetail(const QVector<WatchStartupWatchRow>& watchRows, int position,
    ...
    state.text = state.parts.isEmpty() ? texts.noCia402Evidence : texts.summaryPattern.arg(state.parts.join(" | "));
    return state;
}

// Recommends the next CiA 402 controlword based on the current decoded statusword.
Cia402ControlwordRecommendation selectedDriveControlwordRecommendation(const QVector<WatchStartupWatchRow>& watchRows,
                                                                       int position) {
    if (position < 0) {
        return {};
    }
