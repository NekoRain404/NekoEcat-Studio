...
ConsistencyDetailState consistencyDetailUnavailableState(const ConsistencyDetailTexts& texts) {
...
ConsistencyDetailState consistencyDetailNoSelectionState(const ConsistencyDetailTexts& texts) {
...
QString consistencyDetailRoute(const ConsistencyDetailRow& row, const ConsistencyDetailTexts& texts) {
    const QString scopeLower = row.scope.toLower();
    const QString combined = QString("%1 %2 %3 %4").arg(row.scope, row.evidence, row.target, row.action).toLower();

    if (scopeLower.contains("topology") || row.scope.contains("拓扑")) {
        return texts.routeTopology;
    }
    if (scopeLower.contains("startup") || row.scope.contains("启动") || combined.contains("startup") ||
        combined.contains("启动")) {
        return texts.routeStartup;
    }
    if (combined.contains("watch") || combined.contains("值证据") || combined.contains("missing") ||
...
ConsistencyDetailState buildConsistencyDetailState(const ConsistencyDetailRow& row,
    ...
    state.text = texts.summaryPattern.arg(row.level.isEmpty() ? texts.levelFallback : row.level)
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.expectedLabel, row.expected);
