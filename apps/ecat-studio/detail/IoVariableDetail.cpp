...
QString ioVariableDetailDisplayName(const IoVariableTableRow& row, const IoVariableDetailTexts& texts) {
...
QString ioVariableDetailDisplayValue(const IoVariableTableRow& row, const IoVariableDetailTexts& texts) {
...
QString ioVariableDetailSeverityKey(const IoVariableTableRow& row, const QString& readyText) {
    ...
    if (ioVariableTableRowHasPdoMapIssue(row) || ioVariableTableRowHasPlcIssue(row, readyText) ||
    ...
    if (ioVariableTableRowHasProcessSource(row) || ioVariableTableRowHasWatchEvidence(row) || !row.raw.isEmpty()) {
...
IoVariableDetailState buildIoVariableDetailState(const IoVariableTableRow& row, const IoVariableDetailTexts& texts) {
    ...
    state.text = texts.summaryPattern.arg(slave.isEmpty() ? QStringLiteral("?") : slave)
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.signalStateLabel, state.signalState);
