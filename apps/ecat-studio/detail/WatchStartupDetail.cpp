...
void clearWatchStartupDeltaCells(QTableWidgetItem* valueItem, QTableWidgetItem* deltaItem) {
    if (valueItem) {
...
void applyWatchStartupDeltaCells(QTableWidgetItem* valueItem, QTableWidgetItem* deltaItem, WatchStartupDeltaState state,
                                 const QString& watchValue, const WatchStartupDeltaTexts& texts, bool lightTheme) {
    if (valueItem) {
    ...
    if (state == WatchStartupDeltaState::MissingWatch) {
        ...
        deltaItem->setForeground(lightTheme ? QColor("#64748b") : QColor("#b9c6d6"));
        return;
    }

    if (state == WatchStartupDeltaState::Pending) {
        deltaItem->setText(texts.pending);
        deltaItem->setBackground(lightTheme ? QColor("#eef2f7") : QColor("#1a2230"));
        deltaItem->setForeground(lightTheme ? QColor("#64748b") : QColor("#b9c6d6"));
        return;
    }
    ...
    deltaItem->setBackground(same ? (lightTheme ? QColor("#dcfce7") : QColor("#12351f"))
                                  : (lightTheme ? QColor("#fee2e2") : QColor("#3a1218")));
    deltaItem->setForeground(same ? (lightTheme ? QColor("#166534") : QColor("#86efac"))
