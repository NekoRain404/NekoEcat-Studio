...
bool stateMachineSevereRiskText(const QString& risk) {
    ...
    return lowered.contains(QStringLiteral("fault")) || lowered.contains(QStringLiteral("error")) ||
           lowered.contains(QStringLiteral("offline")) || risk.contains(QStringLiteral("故障")) ||
           risk.contains(QStringLiteral("错误")) || risk.contains(QStringLiteral("离线"));
}
...
QString stateMachineRowDetailSeverityKey(const StateMachineTableRow& row, const StateMachineRowDetailTexts&) {
    ...
    if (currentUpper == QStringLiteral("OP") || currentUpper.startsWith(QStringLiteral("OP "))) {
...
StateMachineRowDetailState buildStateMachineRowDetailState(const StateMachineTableRow& row,
    ...
    state.displayPosition = row.position.isEmpty() ? QString::number(row.row) : row.position;
    ...
    state.displayRecommended = row.recommended.isEmpty() ? texts.none : row.recommended;
    state.displayRisk = row.risk.isEmpty() ? texts.noRisk : row.risk;
    state.displayAction = row.action.isEmpty() ? texts.reviewEvidence : row.action;

    const QString recommendedUpper = stateMachineRecommendedUpper(row);
    state.boundary = state.hasRecommendation ? texts.confirmedRequestBoundary : texts.localReviewBoundaryLabel;
    ...
    state.tooltipLines << QString("%1: #%2 %3").arg(texts.slaveLabel, state.displayPosition, state.displayName);
    state.tooltipLines << QString("%1: %2").arg(texts.currentStateLabel, state.displayCurrent);
    state.tooltipLines << QString("%1: %2").arg(texts.recommendedStateLabel, state.displayRecommended);
