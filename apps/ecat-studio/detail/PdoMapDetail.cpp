...
QString pdoMapDetailDirectionText(const PdoMapTableRow& row, const PdoMapDetailTexts& texts) {
...
QString pdoMapDetailRoleText(const PdoMapTableRow& row, const PdoMapDetailTexts& texts) {
    if (pdoMapDetailIsRxOutput(row)) {
...
bool pdoMapDetailIsRxOutput(const PdoMapTableRow& row) {
    ...
    return pdoLower.contains(QStringLiteral("rxpdo")) || pdoLower.contains(QStringLiteral("rx")) ||
           pdoLower.contains(QStringLiteral("output")) || smLower.contains(QStringLiteral("rx"));
}
...
bool pdoMapDetailIsTxInput(const PdoMapTableRow& row) {
    const QString pdoLower = row.pdo.toLower();
    const QString smLower = row.syncManager.toLower();
    return pdoLower.contains(QStringLiteral("txpdo")) || pdoLower.contains(QStringLiteral("tx")) ||
           pdoLower.contains(QStringLiteral("input")) || smLower.contains(QStringLiteral("tx"));
}

// Whether the index matches a CiA 402 drive profile object or name hints.
bool pdoMapDetailIsCia402(const PdoMapTableRow& row) {
    return row.index == QStringLiteral("0x6040") || row.index == QStringLiteral("0x6041") ||
           row.index == QStringLiteral("0x6060") || row.index == QStringLiteral("0x6061") ||
           row.index == QStringLiteral("0x603f") || row.index == QStringLiteral("0x6064") ||
           row.index == QStringLiteral("0x606c") || row.index == QStringLiteral("0x6077") ||
           row.index == QStringLiteral("0x607a") || row.index == QStringLiteral("0x60ff") ||
           row.index == QStringLiteral("0x6071") || row.name.contains(QStringLiteral("cia"), Qt::CaseInsensitive) ||
           row.name.contains(QStringLiteral("controlword"), Qt::CaseInsensitive) ||
...
PdoMapDetailState buildPdoMapDetailState(const PdoMapTableRow& row, int selectedPosition,
    ...
    state.text = texts.summaryPattern.arg(state.direction)
                     ...
                     .arg(state.inferredType.isEmpty() ? texts.typeFallback : state.inferredType)
                     .arg(row.name.isEmpty() ? texts.unnamed : row.name)
                     .arg(state.role);

    state.tooltipLines << texts.selectedTitle;
    state.tooltipLines << QString("%1: #%2").arg(texts.slaveLabel, QString::number(selectedPosition));
    ...
    state.tooltipLines << QString("%1: %2").arg(texts.inferredTypeLabel, state.inferredType);
    state.tooltipLines << QString("%1: %2").arg(texts.nameLabel, row.name);
    state.tooltipLines << QString("%1: %2").arg(texts.directionLabel, state.direction);
