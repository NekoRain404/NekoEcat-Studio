...
bool watchRowDetailIsMatchText(const QString &text,
  ...
  return normalized.isEmpty() || normalized == QStringLiteral("0") ||
         normalized == QStringLiteral("match") ||
         normalized == QStringLiteral("pending") || text == texts.matchText ||
         text == texts.pendingText || text == QStringLiteral("匹配") ||
...
QString watchRowDetailSeverityKey(const WatchStartupWatchRow &row,
  ...
  const bool baselineDrift =
      !watchRowDetailIsMatchText(row.baselineDelta, texts);
...
buildWatchRowDetailState(const WatchStartupWatchRow &row,
  ...
  state.displayValue =
      !row.decoded.isEmpty()
          ? row.decoded
          : (row.value.isEmpty() ? texts.emptyValue : row.value);
  state.evidence =
      state.startupDrift
          ? texts.startupMismatch
          : (state.baselineDrift
                 ? texts.baselineDrift
                 : (row.changed ? texts.changed : texts.stableEvidence));

  const QString slave =
      row.position >= 0 ? QString::number(row.position) : QString();
  state.text =
      texts.summaryPattern
          ...
          .arg(row.baselineDelta.isEmpty() ? texts.noBaseline
                                           : row.baselineDelta)
          .arg(row.startupDelta.isEmpty() ? texts.noComparison
                                          : row.startupDelta)
          .arg(state.evidence);
  ...
  state.tooltipLines << QString("%1: %2").arg(texts.baselineLabel,
                                              row.baseline);
  state.tooltipLines << QString("%1: %2").arg(texts.baselineDeltaLabel,
                                              row.baselineDelta);
  state.tooltipLines << QString("%1: %2").arg(texts.startupLabel, row.startup);
  state.tooltipLines << QString("%1: %2").arg(texts.startupDeltaLabel,
                                              row.startupDelta);
