...
NextBestActionDetail buildNextBestActionDetail(const NextBestActionDecision& decision, const NextBestActionInput& input,
    ...
    switch (decision.kind) {
        ...
        case NextBestActionKind::Consistency:
            state.text = input.workflow.hasConsistencyCheck ? texts.reviewConsistency : texts.runConsistency;
            state.tip = input.workflow.hasConsistencyCheck ? texts.reviewConsistencyTip : texts.runConsistencyTip;
            state.icon = input.workflow.hasConsistencyBlockingIssues ? NextBestActionIconKey::Warning
        ...
        case NextBestActionKind::MatrixReview:
            state.text = input.matrixP0 + input.matrixP1 > 0 ? texts.reviewMatrixRisk : texts.reviewMatrixAction;
            state.tip = texts.matrixTipPattern.arg(input.matrixP0).arg(input.matrixP1).arg(input.matrixP2);
            state.icon = NextBestActionIconKey::DetailedView;
            break;
