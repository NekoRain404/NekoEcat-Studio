...
bool historyFailed(const QString& status) {
    return status.toLower().contains(QStringLiteral("failed")) || status.contains(QStringLiteral("失败"));
}

// Detects "requested" (pending) status.
bool historyRequested(const QString& status) {
    return status.toLower().contains(QStringLiteral("requested")) || status.contains(QStringLiteral("已请求"));
}

// Detects successful completion status.
bool historyComplete(const QString& status) {
    const QString lowered = status.toLower();
    return lowered.contains(QStringLiteral("complete")) || lowered.contains(QStringLiteral("ok")) ||
           status.contains(QStringLiteral("完成")) || status.contains(QStringLiteral("成功"));
}

// Whether the action is a write operation.
bool historyWriteAction(const QString& action) {
    return action.toLower().contains(QStringLiteral("write")) || action.contains(QStringLiteral("写入"));
}

// Whether the action is a verify operation.
bool historyVerifyAction(const QString& action) {
    return action.toLower().contains(QStringLiteral("verify")) || action.contains(QStringLiteral("校验"));
}

// Whether the action is a read operation.
bool historyReadAction(const QString& action) {
    return action.toLower().contains(QStringLiteral("read")) || action.contains(QStringLiteral("读取"));
}
...
QString sdoHistoryRowDetailSeverityKey(const SdoHistoryRow& row, const SdoHistoryRowDetailTexts&) {
...
SdoHistoryRowDetailState buildSdoHistoryRowDetailState(const SdoHistoryRow& row,
    ...
    state.hasReusableValue = !row.value.isEmpty() && !state.failed && !state.requested;
