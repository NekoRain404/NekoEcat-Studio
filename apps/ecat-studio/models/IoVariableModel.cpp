...
bool ioVariableFilterScopeMatches(const IoVariableFilterRowState &state,
                                  const QString &scope) {
  if (scope == kIoVariableScopeAll) return true;
  if (scope == kIoVariableScopeSelected) return state.selected;
  if (scope == kIoVariableScopeProcess) return state.process;
  if (scope == kIoVariableScopePdo) return state.pdo;
  if (scope == kIoVariableScopeWatch) return state.watchEvidence;
  if (scope == kIoVariableScopeStartupDiff) return state.startupDiff;
  if (scope == kIoVariableScopeMissingValue) return state.missingValue;
  if (scope == kIoVariableScopeRx) return state.rx;
  if (scope == kIoVariableScopeTx) return state.tx;
  if (scope == kIoVariableScopeCia402) return state.cia402;
  if (scope == kIoVariableScopeChanged) return state.changed;
  if (scope == kIoVariableScopePlcIssues) return state.plcIssue;
  return true;
}
...
void accumulateIoVariableFilterStats(IoVariableFilterStats *stats,
  ...
  if (!stats) return;
  ++stats->total;
  if (decision.visible) ++stats->visible;
  if (decision.state.process) ++stats->processRows;
  if (decision.state.watchEvidence) ++stats->watchRows;
  if (decision.state.startupDiff) ++stats->startupDiffs;
  if (decision.state.missingValue) ++stats->missingValues;
  if (decision.state.changed) ++stats->changedRows;
  if (decision.state.plcIssue) ++stats->plcIssues;
}
...
QString ioVariableHandoffPlcDirection(const IoVariableTableRow &row) {
  if (ioVariableTableRowIsRx(row)) return QStringLiteral("Output");
  if (ioVariableTableRowIsTx(row)) return QStringLiteral("Input");
...
QString ioVariableHandoffComment(const IoVariableTableRow &row,
  ...
  if (!direction.isEmpty()) facts << direction;
  if (!row.pdo.isEmpty()) facts << row.pdo;
  if (!row.meaning.isEmpty()) facts << row.meaning;
  if (!qualityLabels.isEmpty()) {
    facts << QString("Quality: %1").arg(qualityLabels.join(" | "));
  }
  if (!facts.isEmpty()) comment += " | " + facts.join(" | ");
  comment.replace("(*", "(");
  comment.replace("*)", ")");
  return comment;
}
...
IoVariableHandoffCsvRow ioVariableHandoffCsvRow(const IoVariableTableRow &row,
  ...
  const QString address =
      QString("#%1 %2:%3")
          .arg(row.positionValid ? QString::number(row.position) : QStringLiteral("?"),
               row.index, row.subIndex);
  ...
  return {{symbol,
           ...
           row.index, row.subIndex, row.bits, row.pdo, row.source,
           row.raw, row.decoded, row.meaning, row.watch, row.startup,
           row.map, row.changed, row.plcQuality, name.alias, row.tags,
           row.note, address, defaultName, exportedAt}};
}
