...
QString processDataNormalizedKnownType(const QString& type) {
    ...
    static const QSet<QString> knownTypes = {"bool",   "int8",   "int16", "int32",  "int64",  "uint8",       "uint16",
...
QString ioVariableTableRowIecType(const IoVariableTableRow& row) {
    const QString type =
        processDataIecTypeFromNormalizedType(processDataNormalizedKnownType(ioVariableTableRowSdoType(row)));
...
bool ioVariableTableRowHasProcessSource(const IoVariableTableRow& row) {
    ...
    return source.contains(QStringLiteral("process")) || source.contains(QStringLiteral("\u8fc7\u7a0b"));
}
...
bool ioVariableTableRowHasWatchEvidence(const IoVariableTableRow& row) {
    return !row.watch.isEmpty() || row.source.contains(QStringLiteral("watch"), Qt::CaseInsensitive);
}
...
bool ioVariableTableRowHasPlcIssue(const IoVariableTableRow& row, const QString& readyText) {
    return !row.plcQuality.isEmpty() && row.plcQuality.compare(readyText, Qt::CaseInsensitive) != 0;
}
...
bool ioVariableTableRowIsRx(const IoVariableTableRow& row) {
    ...
    return direction.contains(QStringLiteral("rx")) || direction.contains(QStringLiteral("output")) ||
...
bool ioVariableTableRowIsTx(const IoVariableTableRow& row) {
    const QString direction = row.direction.toLower();
    return direction.contains(QStringLiteral("tx")) || direction.contains(QStringLiteral("input")) ||
...
bool ioVariableTableRowIsCia402(const IoVariableTableRow& row) {
    return row.index == QStringLiteral("0x6040") || row.index == QStringLiteral("0x6041") ||
           ...
           row.index == QStringLiteral("0x6071") || row.symbol.contains(QStringLiteral("cia"), Qt::CaseInsensitive) ||
...
QString ioVariableTableObjectKey(int position, const QString& index, const QString& subIndex) {
    if (position < 0 || index.trimmed().isEmpty() || subIndex.trimmed().isEmpty()) {
        return QString();
    }
    return QString("%1|%2|%3").arg(position).arg(normalizeHexText(index, 4), normalizeHexText(subIndex, 2));
}
