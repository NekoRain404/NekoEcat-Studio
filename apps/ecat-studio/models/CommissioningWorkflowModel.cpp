...
CommissioningWorkflowStatus commissioningWorkflowStatus(bool done, bool ready) {
    ...
    return ready ? CommissioningWorkflowStatus::Action : CommissioningWorkflowStatus::Blocked;
}

// Evaluates each workflow step against current session inputs to determine its status.
QVector<CommissioningWorkflowStepState> buildCommissioningWorkflowStepStates(const CommissioningWorkflowInput& input) {
    QVector<CommissioningWorkflowStepState> states;
    const QVector<CommissioningWorkflowStep>& order = commissioningWorkflowStepOrder();
    ...
    for (const CommissioningWorkflowStep step : order) {
        switch (step) {
            ...
            case CommissioningWorkflowStep::ScanTopology:
                states.append({step, commissioningWorkflowStatus(input.hasSlaves, input.connected)});
                break;
            case CommissioningWorkflowStep::SelectSlave:
                states.append({step, commissioningWorkflowStatus(input.hasSelectedSlave, input.hasSlaves)});
                break;
            case CommissioningWorkflowStep::InspectObjectDictionary:
                states.append({step, commissioningWorkflowStatus(input.hasSdoRows, input.hasSelectedSlave)});
                break;
            case CommissioningWorkflowStep::ReviewObjectDictionaryEvidence:
                states.append({step, commissioningWorkflowStatus(input.hasSdoRows && !input.hasFailedOdEvidence,
                                                                 input.hasSdoRows)});
                break;
            case CommissioningWorkflowStep::ReviewPdoMap:
                states.append({step, commissioningWorkflowStatus(input.hasPdoRows, input.hasSelectedSlave)});
                break;
            case CommissioningWorkflowStep::MonitorWatch:
                states.append({step, commissioningWorkflowStatus(input.hasWatchRows, input.hasSelectedSlave)});
                break;
            case CommissioningWorkflowStep::ReviewStartupDiffs:
                states.append({step, commissioningWorkflowStatus(input.hasWatchRows && !input.hasStartupWatchDiffs,
                                                                 input.hasWatchRows)});
                break;
            case CommissioningWorkflowStep::RunConsistencyGate:
                states.append(
                    {step, commissioningWorkflowStatus(input.hasConsistencyCheck && !input.hasConsistencyBlockingIssues,
                                                       input.hasWatchRows && !input.hasStartupWatchDiffs)});
                break;
            case CommissioningWorkflowStep::ValidateProcessImage:
                states.append({step, commissioningWorkflowStatus(input.freeRunEnabled || input.hasFreeRunRows,
...
CommissioningWorkflowStatus commissioningWorkflowStepStatus(const QVector<CommissioningWorkflowStepState>& states,
