...
SessionBriefStatus targetStatus(const SessionBriefInput& input) {
    return input.connected && input.hasSlaves && input.hasSelectedSlave ? SessionBriefStatus::Ready
...
SessionBriefStatus mapStatus(const SessionBriefInput& input) {
    ...
    return input.hasFailedOdEvidence ? SessionBriefStatus::Warning : SessionBriefStatus::Ready;
}
...
SessionBriefStatus currentSdoStatus(const SessionBriefInput& input) {
    ...
    if (input.currentSdoEvidenceGroups > 1 || (input.currentSdoWriteDiffers && !input.currentSdoWriteMatches)) {
        return SessionBriefStatus::Warning;
    }
    return input.currentSdoHasEvidence ? SessionBriefStatus::Ready : SessionBriefStatus::Action;
}
...
SessionBriefStatus nextStatus(const SessionBriefInput& input) {
    return input.nextWorkflowStep >= 0 ? SessionBriefStatus::Action : SessionBriefStatus::Ready;
}
...
QVector<SessionBriefRow> buildSessionBriefRows(const SessionBriefInput& input) {
    return {
        {SessionBriefRowKind::Target, actionKey(SessionBriefRowKind::Target), targetStatus(input)},
        {SessionBriefRowKind::Gate, actionKey(SessionBriefRowKind::Gate), gateStatus(input)},
        {SessionBriefRowKind::Map, actionKey(SessionBriefRowKind::Map), mapStatus(input)},
        {SessionBriefRowKind::CurrentSdo, actionKey(SessionBriefRowKind::CurrentSdo), currentSdoStatus(input)},
        {SessionBriefRowKind::RuntimeEvidence, actionKey(SessionBriefRowKind::RuntimeEvidence), runtimeStatus(input)},
        {SessionBriefRowKind::Next, actionKey(SessionBriefRowKind::Next), nextStatus(input)},
    };
}
