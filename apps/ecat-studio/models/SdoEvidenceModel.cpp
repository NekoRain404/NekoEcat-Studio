...
bool isSdoHistoryStartupSource(const QString& status, const QString& value) {
    ...
    if (normalizedStatus == "failed" || normalizedStatus == "requested" || normalizedStatus == "失败" ||
        normalizedStatus == "已请求") {
        return false;
    }
    return true;
}
...
QString preferredSdoEvidenceValue(const SdoEvidenceCandidates& candidates, QString* source) {
    if (source) {
        source->clear();
    }
...
QVector<SdoEvidenceGroup> groupSdoEvidence(const QVector<SdoEvidenceItem>& items) {
    ...
    for (const auto& item : items) {
        ...
        if (item.source.trimmed().isEmpty() || value.isEmpty() || normalized.isEmpty()) {
            continue;
        }
...
SdoWriteDeltaReview reviewSdoWriteDelta(const QVector<SdoEvidenceItem>& items, const QString& writeValue) {
    ...
    review.state = review.hasConflict ? QStringLiteral("conflict")
...
SdoTargetPanelRouteDecision sdoTargetPanelRouteDecision(const QString& rowKey, bool writeDeltaReviewAvailable) {
    ...
    } else if (keyMatches(rowKey, "Evidence Set", "证据集") || keyMatches(rowKey, "Write Delta", "写入差异")) {
        decision.routeKind = writeDeltaReviewAvailable ? SdoTargetPanelRouteKind::EvidenceReview
                                                       : SdoTargetPanelRouteKind::EvidenceDigest;
        decision.copyActionKind = writeDeltaReviewAvailable ? SdoTargetPanelCopyActionKind::ReviewEvidence
                                                            : SdoTargetPanelCopyActionKind::CopyDigestNoDelta;
    } else if (keyMatches(rowKey, "Target", "目标") || keyMatches(rowKey, "Read Value", "读回值") ||
               keyMatches(rowKey, "OD Evidence", "OD 证据")) {
        decision.routeKind = SdoTargetPanelRouteKind::ObjectDictionary;
        decision.copyActionKind = SdoTargetPanelCopyActionKind::FocusObjectDictionary;
    } else {
