...
bool hasStartupDiffEvidence(const QString& status) {
    ...
    return normalized.contains("diff") || normalized == "不一致" || normalized.contains("mismatch") ||
           normalized.contains("不匹配") || normalized.contains("偏差");
}
...
bool hasPdoMapIssueEvidence(const QString& status) {
    ...
    return normalized.contains("warning") || normalized.contains("missing") || normalized.contains("no pdo map") ||
           normalized.contains("警告") || normalized.contains("缺失") || normalized.contains("无 pdo 映射");
}
...
DriveEvidenceSeverity driveEvidenceSeverity(const QString& evidence) {
    ...
    if (normalized.contains("fault") || normalized.contains("error code") || evidence.contains("错误")) {
        return DriveEvidenceSeverity::Error;
    }
    if (normalized.contains("quick stop") || normalized.contains("warning") || normalized.contains("internal limit")) {
...
QString recommendedEthercatState(const EthercatStateEvidence& evidence) {
    ...
    if (state.contains("PREOP")) {
        return evidence.pdoLoaded && evidence.watchValueRows > 0 ? QString("SAFEOP") : QString();
    }
    if (state.contains("SAFEOP")) {
        return evidence.freeRunRows > 0 && evidence.startupDiffs <= 0 && evidence.mapIssues <= 0 &&
                       evidence.consistencyOk
                   ? QString("OP")
                   : QString();
    }
    return QString();
