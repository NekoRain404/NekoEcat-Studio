...
bool watchStartupHasTarget(int position, const QString& index, const QString& subIndex) {
    return position >= 0 && !index.trimmed().isEmpty() && !subIndex.trimmed().isEmpty();
}
...
QString watchStartupTargetKey(int position, const QString& index, const QString& subIndex) {
    return sdoEvidenceKey(position, normalizeHexText(index, 4), normalizeHexText(subIndex, 2));
}
...
WatchStartupWatchMatch watchStartupMatchForWatchRow(const QVector<WatchStartupStartupRow>& startupRows,
    ...
    if (!watchStartupHasTarget(watchRow.position, watchRow.index, watchRow.subIndex)) {
        match.state = WatchStartupDeltaState::NoTarget;
        return match;
    }

    const QString key = watchStartupTargetKey(watchRow.position, watchRow.index, watchRow.subIndex);
    // Iterate over collection
    for (const auto& startupRow : startupRows) {
        if (!watchStartupHasTarget(startupRow.position, startupRow.index, startupRow.subIndex) ||
            watchStartupTargetKey(startupRow.position, startupRow.index, startupRow.subIndex) != key) {
            continue;
        }
    ...
    } else if (watchRow.value.trimmed().isEmpty() || match.expectedValue.trimmed().isEmpty()) {
        match.state = WatchStartupDeltaState::Pending;
    } else {
        match.state = sdoValuesComparableEqual(watchRow.value, match.expectedValue) ? WatchStartupDeltaState::Match
...
QVector<WatchStartupStartupDelta> evaluateStartupWatchDeltas(const QVector<WatchStartupStartupRow>& startupRows,
    ...
    for (const auto& watchRow : watchRows) {
        ...
        watchValues.insert(watchStartupTargetKey(watchRow.position, watchRow.index, watchRow.subIndex),
    ...
    for (const auto& startupRow : startupRows) {
        ...
        const QString key = watchStartupTargetKey(startupRow.position, startupRow.index, startupRow.subIndex);
        ...
        if (delta.watchValue.trimmed().isEmpty() || startupRow.value.trimmed().isEmpty()) {
            delta.state = WatchStartupDeltaState::Pending;
        } else {
            delta.state = sdoValuesComparableEqual(delta.watchValue, startupRow.value) ? WatchStartupDeltaState::Match
                                                                                       : WatchStartupDeltaState::Diff;
        }
        deltas.append(delta);
    }
