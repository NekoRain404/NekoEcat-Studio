...
DcSyncOptimizerPlugin::DcSyncOptimizerPlugin(EcatClient *client, EventBus *bus,
                                             QObject *parent)
...
QString DcSyncOptimizerPlugin::id() const { return "dcsyncoptimizer"; }
...
int DcSyncOptimizerPlugin::defaultOrder() const { return 40; }
bool DcSyncOptimizerPlugin::visible() const { return true; }
QWidget *DcSyncOptimizerPlugin::widget() { return containerWidget_; }
...
void DcSyncOptimizerPlugin::buildUi() {
    ...
    exportBtn_->setStyleSheet(
        "QPushButton { background: #6366f1; color: white; padding: 6px 12px;"
        "border-radius: 4px; }"
        "QPushButton:hover { background: #4f46e5; }");
    connect(exportBtn_, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleExport);
...
QWidget *DcSyncOptimizerPlugin::buildDriftTab() {
    ...
    connect(driftWidget_, &DriftOptimizerWidget::applyRequested, this,
            &DcSyncOptimizerPlugin::handleApplyDrift);
    ...
    optimizeBtn->setStyleSheet(
        ...
        "QPushButton:hover { background: #16a34a; }");
    connect(optimizeBtn, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleDriftOptimize);
...
QWidget *DcSyncOptimizerPlugin::buildJitterTab() {
    ...
    jitterOptimizeBtn_->setStyleSheet(
        "QPushButton { background: #22c55e; color: white; padding: 8px 16px;"
        "border-radius: 4px; font-weight: bold; }"
        "QPushButton:hover { background: #16a34a; }");
    connect(jitterOptimizeBtn_, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleJitterOptimize);
    ...
    jitterParamsTable_->setHorizontalHeaderLabels(
    ...
    jitterImprovementLabel_->setStyleSheet(
    ...
    jitterApplyBtn_->setStyleSheet(
        "QPushButton { background: #3b82f6; color: white; padding: 8px 16px;"
        "border-radius: 4px; font-weight: bold; }"
        "QPushButton:hover { background: #2563eb; }");
    connect(jitterApplyBtn_, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleApplySync);
...
QWidget *DcSyncOptimizerPlugin::buildConfigTab() {
    ...
    configOptimizeBtn_ = new QPushButton(
        tr("Analyze & Optimize Configuration"));
    configOptimizeBtn_->setStyleSheet(
        "QPushButton { background: #22c55e; color: white; padding: 8px 16px;"
        "border-radius: 4px; font-weight: bold; }"
        "QPushButton:hover { background: #16a34a; }");
    connect(configOptimizeBtn_, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleConfigOptimize);
    ...
    configParamsTable_->setHorizontalHeaderLabels(
        {tr("Parameter"), tr("Value")});
    ...
    configImprovementLabel_->setStyleSheet(
        "font-weight: bold; font-size: 14px; color: #22c55e;");
    ...
    configApplyBtn_->setStyleSheet(
        "QPushButton { background: #3b82f6; color: white; padding: 8px 16px;"
        "border-radius: 4px; font-weight: bold; }"
        "QPushButton:hover { background: #2563eb; }");
    connect(configApplyBtn_, &QPushButton::clicked, this,
            &DcSyncOptimizerPlugin::handleApplySync);
...
void DcSyncOptimizerPlugin::handleJitterOptimize() {
    ...
    for (const auto &key : keys) {
        ...
        jitterParamsTable_->setItem(
            row, 1,
            new QTableWidgetItem(
                lastJitterResult_.after[key].toVariant().toString()));
    }
    jitterImprovementLabel_->setText(
        tr("Improvement: %1%").arg(lastJitterResult_.improvement, 0, 'f', 1));
    QString recs;
    for (const auto &rec : lastJitterResult_.recommendations)
        recs += "  " + rec + "\n";
    jitterRecommendationsLabel_->setText(
        tr("Recommendations:\n") + recs);
}
...
void DcSyncOptimizerPlugin::handleConfigOptimize() {
    ...
    for (const auto &key : keys) {
        ...
        configParamsTable_->setItem(
            row, 1,
            new QTableWidgetItem(
                lastConfigResult_.after[key].toVariant().toString()));
    }
    configImprovementLabel_->setText(
        tr("Improvement: %1%").arg(lastConfigResult_.improvement, 0, 'f', 1));
    QString recs;
    for (const auto &rec : lastConfigResult_.recommendations)
        recs += "  " + rec + "\n";
    configRecommendationsLabel_->setText(
        tr("Recommendations:\n") + recs);
}
...
void DcSyncOptimizerPlugin::handleOptimizationCompleted(
    const DcSyncOptimizationResult & /*result*/) {}

void DcSyncOptimizerPlugin::handleOptimizationApplied(
    const DcSyncOptimizationResult & /*result*/) {}
...
void DcSyncOptimizerPlugin::handleExport() {
    ...
    for (const auto &r : results) {
        ...
        for (const auto &key : beforeKeys)
            report += QString("- %1: %2\n")
                          .arg(key,
                               r.before[key].toVariant().toString());
        report += "\n### After Parameters\n";
        const auto afterKeys = r.after.keys();
        for (const auto &key : afterKeys)
            report += QString("- %1: %2\n")
                          .arg(key,
                               r.after[key].toVariant().toString());
        report += "\n---\n\n";
    }

    emit updateDiagnostics("info", "DcSyncOptimizer",
                           report.length() > 200
                               ? report.left(200) + "..."
                               : report);
}
