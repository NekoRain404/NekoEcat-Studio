...
void SyncOptimizerWidget::buildUi() {
    ...
    connect(applyBtn_, &QPushButton::clicked, this, &SyncOptimizerWidget::applyRequested);
...
void SyncOptimizerWidget::displayResult(const DcSyncOptimizationResult& result) {
    ...
    for (const auto& key : beforeKeys) {
        ...
        beforeTable_->setItem(row, 1, new QTableWidgetItem(result.before[key].toVariant().toString()));
    }
    ...
    for (const auto& key : afterKeys) {
        ...
        afterTable_->setItem(row, 1, new QTableWidgetItem(result.after[key].toVariant().toString()));
    }
