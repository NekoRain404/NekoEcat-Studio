...
void SignalChartWidget::paintEvent(QPaintEvent * /*event*/) {
  ...
  const QRect chartRect(kMarginLeft, kMarginTop,
...
void SignalChartWidget::drawAxes(QPainter &p, const QRect &chartRect) {
  ...
  for (int i = 0; i <= 5; ++i) {
    ...
    p.drawText(2, y - 8, chartRect.left() - 8, 16,
               Qt::AlignRight | Qt::AlignVCenter,
               QString::number(val, 'g', 4));
  }

  // X-axis label.
  p.drawText(chartRect.left(), chartRect.bottom() + 4,
             chartRect.width(), 20,
             Qt::AlignCenter, tr("Time (samples)"));
}
...
void SignalChartWidget::drawChannels(QPainter &p, const QRect &chartRect) {
  ...
  for (int ci = 0; ci < channels_.size(); ++ci) {
    ...
    QPen linePen(ch.color.isValid() ? ch.color : kColors[ci % kColorCount],
    ...
    if (legendY + 12 < chartRect.bottom()) {
      ...
      p.drawText(chartRect.right() - 86, legendY, 80, 12,
