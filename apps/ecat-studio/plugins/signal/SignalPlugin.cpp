...
SignalPlugin::SignalPlugin(SignalService *service, QObject *parent)
    : service_(service) {
...
// ── Identity ──────────────────────────────────────────────────────────
QString SignalPlugin::id() const { return "signal"; }
QString SignalPlugin::displayName() const { return "Signal Analyzer"; }
QString SignalPlugin::displayNameZh() const { return QStringLiteral("信号分析"); }
int SignalPlugin::defaultOrder() const { return 70; }
bool SignalPlugin::visible() const { return true; }

QWidget *SignalPlugin::widget() { return container_; }
...
void SignalPlugin::showAddChannelDialog() {
  ...
  if (dlg.exec() == QDialog::Accepted) {
    ...
    const int id = service_->addChannel(name, slaveSpin->value(),
