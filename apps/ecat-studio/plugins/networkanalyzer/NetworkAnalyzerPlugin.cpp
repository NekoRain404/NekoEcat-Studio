...
QString NetworkAnalyzerPlugin::id() const { return "networkanalyzer"; }
...
void NetworkAnalyzerPlugin::startCapture() {
  ...
  if (stopBtn_) stopBtn_->setEnabled(true);
  if (statusLabel_) statusLabel_->setText("Capturing...");
  emit captureStarted();
}
...
void NetworkAnalyzerPlugin::stopCapture() {
  ...
  if (statusLabel_) statusLabel_->setText(QString("Stopped: %1 packets").arg(packets_.size()));
  emit captureStopped();
}

bool NetworkAnalyzerPlugin::isCapturing() const { return capturing_; }
...
int NetworkAnalyzerPlugin::packetCount() const { return packets_.size(); }
...
int NetworkAnalyzerPlugin::filterCount() const { return filters_.size(); }
...
void NetworkAnalyzerPlugin::applyFilters() {
  ...
  for (int i = 0; i < packets_.size(); ++i) {
    ...
    for (const auto &f : filters_) {
      QString fieldValue;
      if (f.field == "source") fieldValue = p.source;
      else if (f.field == "dest") fieldValue = p.destination;
      else if (f.field == "protocol") fieldValue = p.protocol;
      if (f.op == "==" && fieldValue != f.value) match = false;
      else if (f.op == "contains" && !fieldValue.contains(f.value)) match = false;
    }
    if (match) filteredIndices_.append(i);
  }
  rebuildPacketTable();
  if (statusLabel_) statusLabel_->setText(QString("Filtered: %1 of %2 packets").arg(filteredIndices_.size()).arg(packets_.size()));
}
...
QTableWidget *NetworkAnalyzerPlugin::statisticsTable() const { return statisticsTable_; }
QTableWidget *NetworkAnalyzerPlugin::filterTable() const { return filterTable_; }
QTextEdit *NetworkAnalyzerPlugin::decodeView() const { return decodeView_; }
QLabel *NetworkAnalyzerPlugin::statusLabel() const { return statusLabel_; }

void NetworkAnalyzerPlugin::selectPacket(int index) {
  if (index < 0 || index >= packets_.size()) return;
...
void NetworkAnalyzerPlugin::buildUi() {
  ...
  connect(packetTable_, &QTableWidget::cellClicked, this, [this](int row, int) {
    selectPacket(row);
  });
  connect(addFilterBtn_, &QPushButton::clicked, this, [this]() {
    addFilter({filterFieldEdit_->text(), "==", filterValueEdit_->text()});
  });
...
void NetworkAnalyzerPlugin::updateDecodeView(int index) {
  if (!decodeView_ || index < 0 || index >= packets_.size()) return;
