...
int CertificationManagerPlugin::defaultOrder() const {
    return 290;
}
bool CertificationManagerPlugin::visible() const {
    return false;
}
...
void CertificationManagerPlugin::updateCertificate(int index, const Certificate& cert) {
    if (index >= 0 && index < certificates_.size()) {
...
bool CertificationManagerPlugin::isExpiringSoon(int index, int daysThreshold) const {
    if (index < 0 || index >= certificates_.size())
        return false;
    auto now = QDateTime::currentDateTime();
    auto& cert = certificates_[index];
    return now <= cert.expiresAt && now.daysTo(cert.expiresAt) <= daysThreshold;
}
...
void CertificationManagerPlugin::buildUi() {
    ...
    certificateTable_->setHorizontalHeaderLabels({"ID", "Name", "Issuer", "Standard", "Issued", "Expires", "Status"});
    ...
    statusTable_->setHorizontalHeaderLabels({"Certificate", "Status", "Last Checked", "Notes"});
    ...
    renewalTable_->setHorizontalHeaderLabels({"Certificate", "Expiry Date", "Days Left", "Notified"});
    ...
    connect(checkRenewalsBtn_, &QPushButton::clicked, this, [this]() { checkRenewals(); });
    connect(exportReportBtn_, &QPushButton::clicked, this, [this]() { exportReport("/tmp/certification_report.txt"); });
}

void CertificationManagerPlugin::rebuildCertificateTable() {
    if (!certificateTable_)
        return;
    ...
    for (int i = 0; i < certificates_.size(); ++i) {
        ...
        certificateTable_->setItem(i, 4, new QTableWidgetItem(c.issuedAt.toString(Qt::ISODate)));
        certificateTable_->setItem(i, 5, new QTableWidgetItem(c.expiresAt.toString(Qt::ISODate)));
...
void CertificationManagerPlugin::rebuildStatusTable() {
    ...
    for (int i = 0; i < statuses_.size(); ++i) {
        ...
        statusTable_->setItem(i, 2, new QTableWidgetItem(s.lastChecked.toString(Qt::ISODate)));
...
void CertificationManagerPlugin::rebuildRenewalTable() {
    ...
    for (int i = 0; i < renewals_.size(); ++i) {
        ...
        renewalTable_->setItem(i, 1, new QTableWidgetItem(r.expiryDate.toString(Qt::ISODate)));
        renewalTable_->setItem(i, 2, new QTableWidgetItem(QString::number(r.daysUntilExpiry)));
        renewalTable_->setItem(i, 3, new QTableWidgetItem(r.notified ? "Yes" : "No"));
    }
    if (statusLabel_)
        statusLabel_->setText(
            QString("Certificates: %1 | Renewals: %2").arg(certificates_.size()).arg(renewals_.size()));
}
