...
QString KeyboardShortcutsPlugin::id() const { return "shortcuts"; }
QString KeyboardShortcutsPlugin::displayName() const { return "Keyboard Shortcuts"; }
...
int KeyboardShortcutsPlugin::shortcutCount() const { return shortcuts_.size(); }

QString KeyboardShortcutsPlugin::shortcutKey(int index) const {
  if (index < 0 || index >= shortcuts_.size()) return {};
...
void KeyboardShortcutsPlugin::buildUi() {
  ...
  categoryFilter_->addItems({tr("File"), tr("Edit"), tr("View"), tr("Navigation"),
                              tr("Tools"), tr("Help")});
...
void KeyboardShortcutsPlugin::addShortcut() {
  bool ok;
  QString action = QInputDialog::getText(container_, tr("Add Shortcut"), tr("Action name:"), QLineEdit::Normal, "", &ok);
  if (!ok || action.isEmpty()) return;
  QString key = QInputDialog::getText(container_, tr("Add Shortcut"), tr("Keyboard shortcut:"), QLineEdit::Normal, "", &ok);
...
void KeyboardShortcutsPlugin::searchShortcuts(const QString &) { refreshTable(); }

void KeyboardShortcutsPlugin::exportShortcuts() {
  QString path = QFileDialog::getSaveFileName(container_, tr("Export Shortcuts"), "", tr("Shortcut Files (*.shortcuts);;All (*)"));
...
void KeyboardShortcutsPlugin::importShortcuts() {
  QString path = QFileDialog::getOpenFileName(container_, tr("Import Shortcuts"), "", tr("Shortcut Files (*.shortcuts);;All (*)"));
  if (path.isEmpty()) return;
  QSettings file(path, QSettings::IniFormat);
  ...
  for (int i = 0; i < size; ++i) {
    file.setArrayIndex(i);
    shortcuts_.append({file.value("action").toString(),
                       file.value("defaultKey").toString(),
                       file.value("currentKey").toString(),
                       file.value("category").toString()});
  }
  file.endArray();
  refreshTable();
}
