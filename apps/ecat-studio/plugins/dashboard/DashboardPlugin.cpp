...
DashboardPlugin::DashboardPlugin(ChartService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
    setupDashboard();
...
QString DashboardPlugin::id() const {
    return "dashboard";
}
QString DashboardPlugin::displayName() const {
    return "Dashboard";
}
QString DashboardPlugin::displayNameZh() const {
    return QStringLiteral("仪表盘");
}
int DashboardPlugin::defaultOrder() const {
    return 130;
}
bool DashboardPlugin::visible() const {
    return false;
}
QWidget* DashboardPlugin::widget() {
    return container_;
}

int DashboardPlugin::gaugeCount() const {
    return gauges_.size();
}
int DashboardPlugin::counterCount() const {
...
void DashboardPlugin::buildUi() {
    ...
    // ── Connections ──
    connect(refreshSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [this](int ms) { refreshTimer_->setInterval(ms); });
...
void DashboardPlugin::setupDashboard() {
    ...
    const QStringList gaugeNames = {tr("CPU Load (%)"), tr("Memory (%)"), tr("Bandwidth (%)"), tr("Bus Jitter (us)")};
    ...
    const QStringList counterNames = {tr("Slaves"), tr("TX Frames"), tr("RX Errors"), tr("Cycle Time (us)")};
    const QString counterVals[] = {tr("No backend"), tr("No backend"), tr("No backend"), tr("No backend")};
