...
QString SyncManagerPlugin::id() const {
    return "syncmanager";
}
QString SyncManagerPlugin::displayName() const {
    return "Sync Manager";
}
QString SyncManagerPlugin::displayNameZh() const {
    return "同步管理器";
}
int SyncManagerPlugin::defaultOrder() const {
    return 330;
}
bool SyncManagerPlugin::visible() const {
    return false;
}
...
int SyncManagerPlugin::statusCount() const {
...
int SyncManagerPlugin::historyCount() const {
...
int SyncManagerPlugin::settingCount() const {
...
int SyncManagerPlugin::logCount() const {
    return logs_.size();
}
...
bool SyncManagerPlugin::exportReport(const QString& path) {
    ...
    for (const auto& s : statuses_) {
        out << s.name << " [" << s.type << "] " << s.state << " " << s.progress << "%\n";
    }
    out << "\n--- Sync History ---\n";
    for (const auto& h : history_) {
        out << h.timestamp.toString(Qt::ISODate) << " " << h.name << " " << h.result << " " << h.duration << "ms\n";
    }
    out << "\n--- Settings ---\n";
...
QTableWidget* SyncManagerPlugin::statusTable() const {
    return statusTable_;
}
QTableWidget* SyncManagerPlugin::historyTable() const {
    return historyTable_;
}
QTableWidget* SyncManagerPlugin::settingsTable() const {
    return settingsTable_;
}
QTableWidget* SyncManagerPlugin::logTable() const {
    return logTable_;
}
QLabel* SyncManagerPlugin::statusLabel() const {
    return statusLabel_;
}
...
void SyncManagerPlugin::buildUi() {
    ...
    statusTable_->setHorizontalHeaderLabels({"ID", "Name", "Type", "State", "Progress", "Last Sync", "Message"});
    ...
    historyTable_->setHorizontalHeaderLabels({"Timestamp", "Sync ID", "Name", "Result", "Duration (ms)", "Details"});
    ...
    logTable_->setHorizontalHeaderLabels({"Timestamp", "Source", "Level", "Message"});
    ...
    connect(exportReportBtn_, &QPushButton::clicked, this, [this]() { exportReport("/tmp/sync_report.txt"); });
...
void SyncManagerPlugin::rebuildStatusTable() {
    ...
    for (int i = 0; i < statuses_.size(); ++i) {
        ...
        statusTable_->setItem(i, 5, new QTableWidgetItem(s.lastSync.toString(Qt::ISODate)));
...
void SyncManagerPlugin::rebuildLogTable() {
    ...
    if (statusLabel_)
        statusLabel_->setText(QString("Sync Logs: %1 entries shown").arg(filteredLogs_.size()));
}
