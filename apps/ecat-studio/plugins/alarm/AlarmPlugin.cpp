...
static QString categoryToString(AlarmCategory cat) {
    switch (cat) {
        case AlarmCategory::Communication:
            return QStringLiteral("Communication");
        case AlarmCategory::Device:
            return QStringLiteral("Device");
        case AlarmCategory::Network:
            return QStringLiteral("Network");
        case AlarmCategory::Configuration:
...
AlarmPlugin::AlarmPlugin(AlarmService* alarmService, LoggingService* logService, QObject* parent)
    : alarmService_(alarmService), logService_(logService) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(alarmService_, &AlarmService::alarmRaised, this, &AlarmPlugin::onAlarmRaised);
    connect(alarmService_, &AlarmService::alarmAcknowledged, this, &AlarmPlugin::onAlarmAcknowledged);
    connect(alarmService_, &AlarmService::alarmCleared, this, &AlarmPlugin::onAlarmCleared);
}

QString AlarmPlugin::id() const {
    return "alarm";
}
QString AlarmPlugin::displayName() const {
    return "Alarms";
}
QString AlarmPlugin::displayNameZh() const {
    return QStringLiteral("告警");
}
int AlarmPlugin::defaultOrder() const {
    return 110;
}
bool AlarmPlugin::visible() const {
    return true;
}

QWidget* AlarmPlugin::widget() {
    return container_;
}
...
void AlarmPlugin::buildUi() {
    ...
    levelFilter_->addItems({tr("All Levels"), tr("Info"), tr("Warning"), tr("Error"), tr("Critical")});
    toolbarLayout->addWidget(levelFilter_);

    categoryFilter_ = new QComboBox;
    categoryFilter_->addItems(
        {tr("All Categories"), tr("Communication"), tr("Device"), tr("Network"), tr("Configuration")});
    toolbarLayout->addWidget(categoryFilter_);

    stateFilter_ = new QComboBox;
    stateFilter_->addItems({tr("All States"), tr("Active"), tr("Acknowledged"), tr("Cleared")});
    ...
    table_->setHorizontalHeaderLabels(
        {tr("ID"), tr("Time"), tr("Level"), tr("Category"), tr("State"), tr("Source"), tr("Message")});
    ...
    connect(levelFilter_, &QComboBox::currentIndexChanged, this, &AlarmPlugin::applyFilter);
    connect(categoryFilter_, &QComboBox::currentIndexChanged, this, &AlarmPlugin::applyFilter);
    connect(stateFilter_, &QComboBox::currentIndexChanged, this, &AlarmPlugin::applyFilter);
    connect(ackBtn_, &QPushButton::clicked, this, &AlarmPlugin::acknowledgeSelected);
    connect(clearBtn_, &QPushButton::clicked, this, &AlarmPlugin::clearSelected);
    connect(exportBtn_, &QPushButton::clicked, this, &AlarmPlugin::exportHistory);
...
void AlarmPlugin::exportHistory() {
    QString path =
        QFileDialog::getSaveFileName(container_, tr("Export Alarm History"), "alarms.csv", tr("CSV Files (*.csv)"));
    if (path.isEmpty())
        return;
...
bool AlarmPlugin::exportHistoryToFile(const QString& path) {
    ...
    for (const auto& a : alarmService_->alarmHistory(10000)) {
        out << a.id << "," << a.timestamp.toString(Qt::ISODate) << "," << levelToString(a.level) << ","
            << categoryToString(a.category) << "," << stateToString(a.state) << "," << a.source << "," << a.message
            << "\n";
    }
...
void AlarmPlugin::populateRow(int row, const Alarm& alarm) {
    ...
    table_->setItem(row, kColTime, new QTableWidgetItem(alarm.timestamp.toString(Qt::ISODate)));
    table_->setItem(row, kColLevel, new QTableWidgetItem(levelToString(alarm.level)));
    table_->setItem(row, kColCategory, new QTableWidgetItem(categoryToString(alarm.category)));
