...
int DiagramWidget::nodeCount() const {
    return nodes_.size();
}

const QVector<DiagramNode>& DiagramWidget::nodes() const {
    return nodes_;
}
...
void DiagramWidget::paintEvent(QPaintEvent* /*event*/) {
    ...
    for (const auto& n : nodes_) {
        ...
        f.setPointSize(9);
        p.setFont(f);
        p.drawText(QRectF(n.x - 40, n.y + kRadius + 2, 80, 20), Qt::AlignHCenter | Qt::AlignTop, n.name);
    }
}
