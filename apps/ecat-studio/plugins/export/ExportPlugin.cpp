...
ExportPlugin::ExportPlugin(ServiceContainer *container, QObject *parent)
    : container_(container) {
  if (parent) setParent(parent);
  buildUi();
}

QString ExportPlugin::id() const { return "export"; }
QString ExportPlugin::displayName() const { return "Export"; }
...
QWidget *ExportPlugin::widget() { return containerWidget_; }
...
bool ExportPlugin::exportTableCsv(QWidget *parent, QTableWidget *table,
                                  const QString &defaultName,
                                  const QString &logSource,
                                  bool visibleOnly) {
  if (!table || table->rowCount() <= 0) {
    QMessageBox::information(parent,
        tr("Export %1").arg(logSource),
        tr("No entries are available to export."));
    return false;
  }
  const QString path = promptSavePath(parent,
      tr("Export %1 CSV").arg(logSource),
      QString("%1-%2.csv").arg(defaultName)
  ...
  emit updateDiagnostics("Info", logSource,
      tr("Exported %1 row(s): %2").arg(n).arg(path));
  return true;
}

bool ExportPlugin::exportTreeCsv(QWidget *parent, QTreeWidget *tree,
                                 const QString &defaultName,
                                 const QString &logSource) {
  if (!tree || tree->topLevelItemCount() == 0) {
    QMessageBox::information(parent,
        tr("Export %1").arg(logSource),
        tr("No data is available to export."));
    return false;
  }
  const QString path = promptSavePath(parent,
      tr("Export %1 CSV").arg(logSource),
  ...
  emit updateDiagnostics("Info", logSource,
      tr("Exported %1 node(s): %2").arg(count).arg(path));
  return true;
}

bool ExportPlugin::exportPlainText(QWidget *parent, QPlainTextEdit *textEdit,
                                   const QString &defaultName,
                                   const QString &logSource,
                                   const QString &filter) {
  ...
  const QString path = promptSavePath(parent,
      tr("Export %1").arg(logSource),
      QString("%1-%2.txt").arg(defaultName)
          .arg(QDateTime::currentDateTime().toString("yyyyMMdd-HHmmss")),
      filter);
  if (path.isEmpty()) return false;
