...
SoEPlugin::SoEPlugin(EcatClient *client, EventBus *eventBus, QObject *parent)
...
QString SoEPlugin::id() const { return "soe"; }
QString SoEPlugin::displayName() const { return "SoE Drive"; }
QString SoEPlugin::displayNameZh() const { return QStringLiteral("伺服驱动"); }
int SoEPlugin::defaultOrder() const { return 156; }
bool SoEPlugin::visible() const { return true; }
...
void SoEPlugin::buildUi() {
    ...
    typeCombo_->addItems({"", "int8", "int16", "int32", "int64",
...
void SoEPlugin::onTopologyChanged(const QVector<SlaveInfo> &slaves) {
    if (!slaveCombo_) return;
    const int prev = slaveCombo_->currentData().isValid()
        ? slaveCombo_->currentData().toInt() : -1;
    slaveCombo_->clear();
    ...
    if (prev >= 0) {
        ...
        if (idx >= 0) slaveCombo_->setCurrentIndex(idx);
    }
}

int SoEPlugin::selectedPosition() const {
    if (!slaveCombo_ || slaveCombo_->currentData().isNull()) return -1;
...
void SoEPlugin::doRead() {
    ...
    log_->appendPlainText(tr("Reading IDN %1 from slave %2 (drive %3)...")
                              .arg(idn).arg(pos).arg(driveSpin_->value()));
}
...
void SoEPlugin::doWrite() {
    ...
    log_->appendPlainText(tr("Writing IDN %1 = %2 to slave %3 (drive %4)...")
                              .arg(idn, value).arg(pos).arg(driveSpin_->value()));
}
