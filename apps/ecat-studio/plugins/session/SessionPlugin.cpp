...
SessionPlugin::SessionPlugin(ServiceContainer *container, QObject *parent)
...
// ── Identity ──────────────────────────────────────────────────────────
QString SessionPlugin::id() const { return "session"; }
QString SessionPlugin::displayName() const { return "Session"; }
QString SessionPlugin::displayNameZh() const {
  return QStringLiteral("\u4f1a\u8bdd");
}
int SessionPlugin::defaultOrder() const { return 80; }
bool SessionPlugin::visible() const { return true; }

QIcon SessionPlugin::icon() const { return QIcon::fromTheme("system-users"); }
...
QWidget *SessionPlugin::widget() { return containerWidget_; }

QTableWidget *SessionPlugin::sessionBriefTable() const { return table_; }
...
void SessionPlugin::buildUi() {
  ...
  table_->setHorizontalHeaderLabels(
      {tr("Area"), tr("Status"), tr("Evidence"), tr("Next")});
  ...
  connect(table_, &QTableWidget::cellActivated, this,
          [this](int row, int /*column*/) {
            emit sessionBriefRowActivated(row);
          });
...
// ── Table population ──────────────────────────────────────────────────
void SessionPlugin::updateSessionBrief(const QStringList &headers,
...
void SessionPlugin::updateCopyButtonState() {
  ...
  copyButton_->setText(area.isEmpty() ? tr("Copy Row")
                                      : tr("Copy: %1").arg(area));
}
...
QString SessionPlugin::currentRowArea() const {
  ...
  if (auto *item = table_->item(row, 0)) return item->text().trimmed();
  return {};
}
