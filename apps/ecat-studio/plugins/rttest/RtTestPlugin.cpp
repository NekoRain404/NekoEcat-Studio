...
public:
    explicit RtTestLatencyChart(QWidget* parent = nullptr) : QWidget(parent) { setMinimumSize(300, 200); }

    void appendData(const QJsonArray& avg, const QJsonArray& minArr, const QJsonArray& maxArr) {
...
protected:
    void paintEvent(QPaintEvent*) override {
        ...
        if (bufAvg_.size() < 2 || r.width() < 10 || r.height() < 10)
            return;
        ...
        // Max line (pink dashed)
        {
            ...
            for (int i = 0; i < n; ++i)
                pts[i] = QPointF(toX(i), toY(bufMax_[i]));
        ...
        // Min line (green dashed)
        {
            QVector<QPointF> pts(n);
            for (int i = 0; i < n; ++i)
                pts[i] = QPointF(toX(i), toY(bufMin_[i]));
        ...
        // Avg line (solid blue)
        {
            QVector<QPointF> pts(n);
            for (int i = 0; i < n; ++i)
                pts[i] = QPointF(toX(i), toY(bufAvg_[i]));
...
public:
    explicit RtTestJitterSpark(QWidget* parent = nullptr) : QWidget(parent) { setFixedHeight(48); }

    void pushSample(double jitterUs) {
        samples_.append(jitterUs);
        if (samples_.size() > kMax)
...
RtTestPlugin::RtTestPlugin(ServiceContainer* container, QObject* parent)
    ...
    connect(client_, &EcatClient::rtTestTelemetry, this, &RtTestPlugin::handleRtTestTelemetry);
}

// ── Identity ──────────────────────────────────────────────────────────────
QString RtTestPlugin::id() const {
    return "rttest";
}
QString RtTestPlugin::displayName() const {
    return "RT Test";
}
QString RtTestPlugin::displayNameZh() const {
    return QStringLiteral("实时测试");
}
int RtTestPlugin::defaultOrder() const {
    return 75;
}
bool RtTestPlugin::visible() const {
    return false;
}

QIcon RtTestPlugin::icon() const {
    return QIcon::fromTheme("appointment-soon");
}
...
void RtTestPlugin::buildUi() {
    ...
    connect(cycleCombo_, &QComboBox::currentIndexChanged, this, [this](int idx) {
        const bool custom = cycleCombo_->itemData(idx).toInt() < 0;
        customCycle_->setVisible(custom);
        if (custom)
            customCycle_->setFocus();
        updateFreqLabel();
    });
    connect(customCycle_, &QLineEdit::textChanged, this, &RtTestPlugin::updateFreqLabel);
    ctrl->addStretch();
    ...
    connect(startButton_, &QPushButton::clicked, this, [this] {
        const int usec = customCycle_->isVisible() ? customCycle_->text().toInt() : cycleCombo_->currentData().toInt();
        if (usec > 0)
            client_->rtTestStart(usec);
    });
    connect(stopButton_, &QPushButton::clicked, this, [this] { client_->rtTestStop(); });
}
...
void RtTestPlugin::handleRtTestTelemetry(const QJsonObject& telemetry) {
    ...
    if (cycles > 0 && avgUs > 0) {
        durationLabel_->setText(formatDuration(static_cast<double>(cycles) * avgUs / 1e6));
    }
    ...
    // Jitter sparkline
    if (jitterSpark_)
        jitterSpark_->pushSample(jitterUs);
    ...
    if (running && cycles > 0 && (cycles % 5000 == 0)) {
        timelineText_->appendPlainText(QString("#%1  avg=%2\xC2\xB5s  jit=%3\xC2\xB5s  loss=%4%")
                                           .arg(cycles)
                                           .arg(avgUs, 0, 'f', 1)
                                           .arg(jitterUs, 0, 'f', 1)
                                           .arg(lossRate, 0, 'f', 3));
...
void RtTestPlugin::updateFreqLabel() {
    ...
    if (usec > 0) {
        ...
        freqLabel_->setText(hz >= 1000 ? QString::number(hz / 1000.0, 'f', 1) + " kHz"
