...
OdPlugin::OdPlugin(ServiceContainer *container, QObject *parent)
...
// ── Identity ──────────────────────────────────────────────────────────
QString OdPlugin::id() const { return "od"; }
QString OdPlugin::displayName() const { return "Object Dictionary"; }
QString OdPlugin::displayNameZh() const {
  return QStringLiteral("对象字典");
}
QIcon OdPlugin::icon() const { return QIcon::fromTheme("text-x-generic"); }
int OdPlugin::defaultOrder() const { return 20; }
bool OdPlugin::visible() const { return true; }
...
QWidget *OdPlugin::widget() { return containerWidget_; }

// ── Accessors ─────────────────────────────────────────────────────────
QTableWidget *OdPlugin::sdoTable() const { return sdoTable_; }
QLineEdit *OdPlugin::sdoFilter() const { return sdoFilter_; }
QComboBox *OdPlugin::sdoObjectFilter() const { return sdoObjectFilter_; }
QComboBox *OdPlugin::sdoAccessFilter() const { return sdoAccessFilter_; }
QLabel *OdPlugin::sdoSummaryLabel() const { return sdoSummaryLabel_; }

QLineEdit *OdPlugin::sdoIndex() const { return sdoIndex_; }
QLineEdit *OdPlugin::sdoSubIndex() const { return sdoSubIndex_; }
QComboBox *OdPlugin::sdoType() const { return sdoType_; }
QLineEdit *OdPlugin::sdoValue() const { return sdoValue_; }
QLineEdit *OdPlugin::sdoWriteValue() const { return sdoWriteValue_; }
QPushButton *OdPlugin::useSdoValueButton() const {
  return useSdoValueButton_;
}
QLabel *OdPlugin::sdoInspectorLabel() const { return sdoInspectorLabel_; }

QTableWidget *OdPlugin::sdoTargetTable() const { return sdoTargetTable_; }
...
QTableWidget *OdPlugin::sdoHistoryTable() const { return sdoHistoryTable_; }
...
void OdPlugin::buildOdTab(QWidget *widget) {
  ...
  sdoTable_->setHorizontalHeaderLabels(
      {tr("Object"), tr("Index"), tr("Sub"), tr("Access"), tr("Bits"),
       tr("Type"), tr("Name"), tr("Last Value"), tr("Status")});
  ...
  connect(sdoFilter_, &QLineEdit::textChanged, this,
          &OdPlugin::sdoFilterChanged);
  connect(sdoTable_, &QTableWidget::currentCellChanged, this,
          &OdPlugin::sdoTableSelectionChanged);
}
...
void OdPlugin::buildInspectorPanel(QWidget *widget) {
  ...
  sdoType_->addItems({"",    "bool",  "int8",   "int16",  "int32",
  ...
  connect(runActionBtn, &QPushButton::clicked, this,
          &OdPlugin::sdoTargetPanelRowActionRequested);
  ...
  connect(copyBtn, &QPushButton::clicked, this,
          &OdPlugin::sdoTargetPanelCopyRequested);
  ...
  sdoTargetTable_->setHorizontalHeaderLabels(
      {tr("Key"), tr("Value"), tr("Action")});
...
void OdPlugin::buildTargetTrailTab(QWidget *widget) {
  ...
  sdoTargetTrailTable_->setHorizontalHeaderLabels(
      {tr("Time"), tr("Slave"), tr("Index"), tr("Sub"), tr("Type"),
  ...
  connect(sdoTargetTrailTable_, &QTableWidget::currentCellChanged, this,
          &OdPlugin::sdoTargetTrailSelectionChanged);
...
void OdPlugin::buildBookmarkTab(QWidget *widget) {
  ...
  objectBookmarkTable_->setHorizontalHeaderLabels(
      {tr("Slave"), tr("Slave Name"), tr("Index"), tr("Sub"),
       tr("Access"), tr("Type"), tr("Bits"), tr("Name"),
       tr("Last Value"), tr("Source")});
  ...
  connect(objectBookmarkTable_, &QTableWidget::currentCellChanged, this,
          &OdPlugin::objectBookmarkSelectionChanged);
...
void OdPlugin::buildHistoryTab(QWidget *widget) {
  ...
  sdoHistoryTable_->setHorizontalHeaderLabels(
      {tr("Time"), tr("Action"), tr("Slave"), tr("Index"), tr("Sub"),
  ...
  connect(sdoHistoryTable_, &QTableWidget::currentCellChanged, this,
          &OdPlugin::sdoHistorySelectionChanged);
  layout->addWidget(sdoHistoryTable_);
}

// ── Table management ──────────────────────────────────────────────────
void OdPlugin::updateSdoTableSummary(int total, int visible, int withEvidence,
                                     int failed, int writable) {
  if (sdoSummaryLabel_) {
    sdoSummaryLabel_->setText(
        tr("%1 obj | %2 vis | %3 evidence | %4 fail | %5 rw")
            .arg(total)
            .arg(visible)
            .arg(withEvidence)
            .arg(failed)
...
void OdPlugin::ensureSdoTargetTrailTable() {
  if (!sdoTargetTrailTable_ ||
      sdoTargetTrailTable_->columnCount() != 9) {
    if (sdoTargetTrailTable_) {
      sdoTargetTrailTable_->setColumnCount(9);
    }
  }
...
void OdPlugin::updateSdoTargetTrailRowDetail(const QString &text,
                                              const QString &severity,
                                              const QString &tooltip) {
  if (!sdoTargetTrailDetailLabel_) return;
...
void OdPlugin::ensureObjectBookmarkTable() {
  if (!objectBookmarkTable_ ||
      objectBookmarkTable_->columnCount() != 10) {
    if (objectBookmarkTable_) {
      objectBookmarkTable_->setColumnCount(10);
    }
  }
...
void OdPlugin::updateObjectBookmarkRowDetail(const QString &text,
                                              const QString &severity,
                                              const QString &tooltip) {
  if (!objectBookmarkDetailLabel_) return;
...
void OdPlugin::updateSdoTableEvidenceRow(int row, const QString &value,
                                          const QString &status,
                                          const QString &time,
                                          const QColor &statusColor,
                                          const QColor &valueBackground) {
  if (!sdoTable_ || row < 0 || row >= sdoTable_->rowCount()) return;
...
void OdPlugin::updateSdoTargetPanelRows(
  ...
  for (int r = 0; r < rows.size(); ++r) {
    ...
    const QString actionText =
        rowActions.value(rows.at(r).first, tr("Copy row"));
...
void OdPlugin::updateSdoInspectorLabel(const QString &text,
                                        const QString &state) {
  if (!sdoInspectorLabel_) return;
