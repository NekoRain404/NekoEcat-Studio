...
QString ScriptLibraryPlugin::id() const {
    return "scriptlibrary";
}
QString ScriptLibraryPlugin::displayName() const {
    return "Script Library";
}
QString ScriptLibraryPlugin::displayNameZh() const {
    return QStringLiteral("脚本库");
}
QIcon ScriptLibraryPlugin::icon() const {
    return QIcon::fromTheme("text-x-script");
}
int ScriptLibraryPlugin::defaultOrder() const {
    return 200;
}
bool ScriptLibraryPlugin::visible() const {
    return false;
}
...
QWidget* ScriptLibraryPlugin::widget() {
    return containerWidget_;
}
QTreeWidget* ScriptLibraryPlugin::scriptTree() const {
    return scriptTree_;
}
QPlainTextEdit* ScriptLibraryPlugin::scriptEditor() const {
    return scriptEditor_;
}
QTextEdit* ScriptLibraryPlugin::outputConsole() const {
    return outputConsole_;
}
QTextEdit* ScriptLibraryPlugin::docViewer() const {
    return docViewer_;
}
...
void ScriptLibraryPlugin::buildUi() {
    ...
    connect(importBtn_, &QPushButton::clicked, this, [this]() {
        QString path = QFileDialog::getOpenFileName(containerWidget_, tr("Import Script"), QString(),
                                                    "Scripts (*.py *.lua *.js);;All (*)");
        if (!path.isEmpty())
            importScript(path);
    });
    connect(exportBtn_, &QPushButton::clicked, this, [this]() {
        ...
        if (item && item->parent()) {
            QString path = QFileDialog::getSaveFileName(containerWidget_, tr("Export Script"), item->text(0) + ".py",
                                                        "Scripts (*.py);;All (*)");
            if (!path.isEmpty())
                exportScript(path, item->text(0));
        }
    });
