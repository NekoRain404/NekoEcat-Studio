...
FreeRunPlugin::FreeRunPlugin(ServiceContainer* container, QObject* parent) : container_(container) {
    if (parent)
        setParent(parent);
    buildUi();

    auto* frMon = container_->freeRunMonitor();
    connect(frMon, &FreeRunMonitoringService::processDataUpdated, this, [this](const FreeRunProcessData& data) {
        ...
        for (int i = 0; i < maxLen; ++i) {
            ...
            row << (i < data.inputs.size() ? QString::number(data.inputs[i], 16).rightJustified(2, QChar('0'))
                                           : QString());
            row << (i < data.outputs.size() ? QString::number(data.outputs[i], 16).rightJustified(2, QChar('0'))
                                            : QString());
            rows << row;
        }
        setEntryRows(headers, rows);
        setSummary(tr("%1 entries | cycle %2 | err %3").arg(maxLen).arg(data.cycleCount).arg(data.errorCount));
    });

    connect(frMon, &FreeRunMonitoringService::statusChanged, this, [this](const FreeRunStatus& status) {
        setSummary(tr("State: %1 | uptime %2 ms | active %3 slaves")
                       .arg(status.stateString)
                       .arg(status.uptimeMs)
    ...
    connect(frMon, &FreeRunMonitoringService::errorOccurred, this,
            [this](const FreeRunErrorInfo& error) { setDetail(tr("Error [%1]: %2").arg(error.code, error.message)); });
}

// ── Identity ──────────────────────────────────────────────────────────
QString FreeRunPlugin::id() const {
    return "freerun";
}
QString FreeRunPlugin::displayName() const {
    return "Free Run";
}
QString FreeRunPlugin::displayNameZh() const {
    return QStringLiteral("自由运行");
}
QIcon FreeRunPlugin::icon() const {
    return QIcon::fromTheme("media-playback-start");
}
int FreeRunPlugin::defaultOrder() const {
    return 35;
}
bool FreeRunPlugin::visible() const {
    return true;
}
...
// ── Accessors ─────────────────────────────────────────────────────────
QTableWidget* FreeRunPlugin::entryTable() const {
    return entryTable_;
}
QLineEdit* FreeRunPlugin::filter() const {
    return filter_;
}
QCheckBox* FreeRunPlugin::changedOnly() const {
    return changedOnly_;
}
QLabel* FreeRunPlugin::summaryLabel() const {
    return summaryLabel_;
}
QLabel* FreeRunPlugin::detailLabel() const {
    return detailLabel_;
}

// ── Table population ──────────────────────────────────────────────────
void FreeRunPlugin::setEntryRows(const QStringList& headers, const QList<QStringList>& rows) {
...
// ── Free Run state ────────────────────────────────────────────────────
bool FreeRunPlugin::freeRunEnabled() const {
    return freeRunEnabled_;
}
...
QString FreeRunPlugin::lastStatus() const {
    return lastStatus_;
}
