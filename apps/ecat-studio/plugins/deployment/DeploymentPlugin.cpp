...
QString DeploymentPlugin::id() const { return "deployment"; }
QString DeploymentPlugin::displayName() const { return "Deployment"; }
...
QWidget *DeploymentPlugin::widget() { return containerWidget_; }
QTreeWidget *DeploymentPlugin::deploymentTargets() const { return deploymentTargets_; }
QTableWidget *DeploymentPlugin::deploymentPackages() const { return deploymentPackages_; }
QTableWidget *DeploymentPlugin::deploymentHistory() const { return deploymentHistory_; }
QTextEdit *DeploymentPlugin::deploymentStatus() const { return deploymentStatus_; }
...
void DeploymentPlugin::buildUi() {
  ...
  deploymentHistory_->setHorizontalHeaderLabels({tr("Target"), tr("Package"), tr("Version"), tr("Status"), tr("Time")});
  ...
  connect(exportButton_, &QPushButton::clicked, this, [this]() {
    ...
    if (!path.isEmpty()) exportDeploymentLog(path);
  });
...
int DeploymentPlugin::targetCount() const { return targets_.size(); }
...
int DeploymentPlugin::packageCount() const { return packages_.size(); }
...
void DeploymentPlugin::deploy(const QString &targetId, const QString &packageId) {
  ...
  for (const auto &t : targets_) {
    if (t.id == targetId) { record.targetName = t.name; break; }
  }
...
int DeploymentPlugin::deploymentHistoryCount() const { return records_.size(); }
