...
MasterManagerPlugin::MasterManagerPlugin(MasterManagerService* masterService, DistributedClockService* dcService,
                                         QObject* parent)
    ...
    connect(dcService_, &DistributedClockService::driftDetected, this, [this](int slave, double drift) {
        appendLog(QString("DC drift on slave %1: %2 ns").arg(slave).arg(drift));
    });
}

QString MasterManagerPlugin::id() const {
    return "master";
}
QString MasterManagerPlugin::displayName() const {
...
int MasterManagerPlugin::defaultOrder() const {
    return 155;
}
bool MasterManagerPlugin::visible() const {
    return false;
}

void MasterManagerPlugin::activate() {
    updateMasterInfo();
}
void MasterManagerPlugin::deactivate() {}

QWidget* MasterManagerPlugin::widget() {
    return container_;
}
...
void MasterManagerPlugin::buildUi() {
    ...
    const QStringList props = {tr("Version"),    tr("Build Date"), tr("Adapter Name"), tr("Adapter MAC"),
    ...
    connect(diagnoseBtn_, &QPushButton::clicked, this, [this]() { updateDiagnostics(); });
...
void MasterManagerPlugin::updateMasterInfo() {
    ...
    const auto stateToString = [](MasterMgrState s) -> QString {
        switch (s) {
            case MasterMgrState::Idle:
                return "Idle";
            case MasterMgrState::Active:
                return "Active";
            case MasterMgrState::Error:
                return "Error";
            case MasterMgrState::Configuring:
                return "Configuring";
            default:
                return "Unknown";
        }
...
void MasterManagerPlugin::updateDiagnostics() {
    ...
    appendLog(QString("DC drift: slave=%1 drift=%2 ns status=%3").arg(drift.slave).arg(drift.drift).arg(drift.status));
...
void MasterManagerPlugin::appendLog(const QString& message) {
    logViewer_->appendPlainText(QDateTime::currentDateTime().toString("hh:mm:ss") + " " + message);
}
