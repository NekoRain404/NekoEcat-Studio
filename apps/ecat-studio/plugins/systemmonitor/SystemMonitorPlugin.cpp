...
QString SystemMonitorPlugin::id() const { return "systemmonitor"; }
...
double SystemMonitorPlugin::cpuUsage() const { return cpuUsage_; }
double SystemMonitorPlugin::memoryUsage() const { return memoryUsage_; }
double SystemMonitorPlugin::diskUsage() const { return diskUsage_; }
double SystemMonitorPlugin::networkUsage() const { return networkUsage_; }

QVector<SystemMonitorPlugin::UsageSample> SystemMonitorPlugin::cpuHistory() const { return cpuHistory_; }
QVector<SystemMonitorPlugin::UsageSample> SystemMonitorPlugin::memoryHistory() const { return memoryHistory_; }
QVector<SystemMonitorPlugin::UsageSample> SystemMonitorPlugin::diskHistory() const { return diskHistory_; }
QVector<SystemMonitorPlugin::UsageSample> SystemMonitorPlugin::networkHistory() const { return networkHistory_; }
...
int SystemMonitorPlugin::alertCount() const { return alerts_.size(); }

int SystemMonitorPlugin::triggeredAlertCount() const {
  int count = 0;
  for (const auto &a : alerts_) {
    if (a.triggered) ++count;
  }
  return count;
}
...
void SystemMonitorPlugin::checkAlerts() {
  for (auto &a : alerts_) {
    double value = 0.0;
    if (a.metric == "CPU") value = cpuUsage_;
    else if (a.metric == "Memory") value = memoryUsage_;
    else if (a.metric == "Disk") value = diskUsage_;
    else if (a.metric == "Network") value = networkUsage_;

    bool wasTriggered = a.triggered;
    if (a.condition == ">") a.triggered = value > a.threshold;
    else if (a.condition == "<") a.triggered = value < a.threshold;
    else if (a.condition == ">=") a.triggered = value >= a.threshold;
    else if (a.condition == "<=") a.triggered = value <= a.threshold;
...
QTableWidget *SystemMonitorPlugin::overviewTable() const { return overviewTable_; }
QTableWidget *SystemMonitorPlugin::alertTable() const { return alertTable_; }
QTextEdit *SystemMonitorPlugin::historyView() const { return historyView_; }
QLabel *SystemMonitorPlugin::statusLabel() const { return statusLabel_; }
...
void SystemMonitorPlugin::buildUi() {
  ...
  connect(addAlertBtn_, &QPushButton::clicked, this, [this]() {
    addAlert({"Custom", 50.0, ">", "Custom alert", false});
  });
  connect(removeAlertBtn_, &QPushButton::clicked, this, [this]() {
    int row = alertTable_->currentRow();
    if (row >= 0) removeAlert(row);
  });
}

void SystemMonitorPlugin::rebuildOverviewTable() {
  if (!overviewTable_) return;
  ...
  setRow(0, makeRow(0, "CPU", cpuUsage_, cpuHistory_.size(),
                     cpuUsage_ > 90 ? "CRITICAL" : cpuUsage_ > 70 ? "WARNING" : "OK"));
  setRow(1, makeRow(1, "Memory", memoryUsage_, memoryHistory_.size(),
                     memoryUsage_ > 85 ? "CRITICAL" : memoryUsage_ > 70 ? "WARNING" : "OK"));
  setRow(2, makeRow(2, "Disk", diskUsage_, diskHistory_.size(),
                     diskUsage_ > 95 ? "CRITICAL" : diskUsage_ > 80 ? "WARNING" : "OK"));
...
void SystemMonitorPlugin::rebuildHistoryView() {
  ...
  auto appendHistory = [&text](const QString &label, const QVector<UsageSample> &h) {
    ...
    for (int i = start; i < h.size(); ++i) {
      text += "  " + h[i].timestamp.toString(Qt::ISODate) + "  " +
              QString::number(h[i].value, 'f', 1) + "\n";
    }
    text += "\n";
  };
...
void SystemMonitorPlugin::recordSample(QVector<UsageSample> &history, double value) {
  ...
  if (history.size() > kMaxHistorySize) history.removeFirst();
}
