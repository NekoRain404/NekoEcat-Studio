...
AnalyticsDashboardPlugin::AnalyticsDashboardPlugin(QObject* parent) : WorkspacePlugin() {}
...
int AnalyticsDashboardPlugin::defaultOrder() const {
    return 92;
}

bool AnalyticsDashboardPlugin::visible() const {
    return false;
}
...
QVector<AnalyticsDashboardPlugin::AnalyticsMetric> AnalyticsDashboardPlugin::metrics() const {
    return metrics_;
}
...
QVector<AnalyticsDashboardPlugin::AnalyticsTrend> AnalyticsDashboardPlugin::trends() const {
    return trends_;
}
...
QVector<AnalyticsDashboardPlugin::AnalyticsReport> AnalyticsDashboardPlugin::reports() const {
    return reports_;
}
...
QVector<AnalyticsDashboardPlugin::AnalyticsFilter> AnalyticsDashboardPlugin::filters() const {
    return filters_;
}
...
QString AnalyticsDashboardPlugin::exportReport() const {
    ...
    report +=
        QStringLiteral("\n## Active Filters: %1\n\n")
            .arg(std::count_if(filters_.begin(), filters_.end(), [](const AnalyticsFilter& f) { return f.active; }));
...
void AnalyticsDashboardPlugin::refresh() {
    ...
    rebuildReportView();
    if (statusLabel_)
        statusLabel_->setText(QStringLiteral("Last refreshed: %1 | Metrics: %2 | Trends: %3")
...
void AnalyticsDashboardPlugin::buildUi() {
    ...
    metricsTable_->setHorizontalHeaderLabels({tr("Name"), tr("Value"), tr("Min"), tr("Max"), tr("Avg"), tr("Samples")});
    tabs_->addTab(metricsTable_, tr("Metrics"));

    trendsTable_ = new QTableWidget;
    trendsTable_->setColumnCount(4);
    trendsTable_->setHorizontalHeaderLabels({tr("Metric"), tr("Direction"), tr("Slope"), tr("Points")});
    tabs_->addTab(trendsTable_, tr("Trends"));

    reportsTable_ = new QTableWidget;
    reportsTable_->setColumnCount(3);
    reportsTable_->setHorizontalHeaderLabels({tr("Title"), tr("Generated"), tr("Summary")});
    tabs_->addTab(reportsTable_, tr("Reports"));

    filtersTable_ = new QTableWidget;
    filtersTable_->setColumnCount(4);
    filtersTable_->setHorizontalHeaderLabels({tr("Field"), tr("Operator"), tr("Value"), tr("Active")});
    ...
    connect(refreshBtn_, &QPushButton::clicked, this, &AnalyticsDashboardPlugin::refresh);
...
void AnalyticsDashboardPlugin::rebuildMetricsTable() {
    ...
    for (int i = 0; i < metrics_.size(); ++i) {
        ...
        metricsTable_->setItem(i, 2, new QTableWidgetItem(QString::number(metrics_[i].min, 'f', 2)));
        metricsTable_->setItem(i, 3, new QTableWidgetItem(QString::number(metrics_[i].max, 'f', 2)));
        metricsTable_->setItem(i, 4, new QTableWidgetItem(QString::number(metrics_[i].avg, 'f', 2)));
        metricsTable_->setItem(i, 5, new QTableWidgetItem(QString::number(metrics_[i].samples)));
    }
}
...
void AnalyticsDashboardPlugin::rebuildTrendsTable() {
    ...
    for (int i = 0; i < trends_.size(); ++i) {
        ...
        trendsTable_->setItem(i, 2, new QTableWidgetItem(QString::number(trends_[i].slope, 'f', 4)));
        trendsTable_->setItem(i, 3, new QTableWidgetItem(QString::number(trends_[i].values.size())));
    }
}
...
void AnalyticsDashboardPlugin::rebuildReportsTable() {
    ...
    for (int i = 0; i < reports_.size(); ++i) {
        ...
        reportsTable_->setItem(i, 1, new QTableWidgetItem(reports_[i].generated.toString(Qt::ISODate)));
...
void AnalyticsDashboardPlugin::rebuildFiltersTable() {
    ...
    for (int i = 0; i < filters_.size(); ++i) {
        ...
        filtersTable_->setItem(i, 1, new QTableWidgetItem(filters_[i].operator_));
        filtersTable_->setItem(i, 2, new QTableWidgetItem(filters_[i].value));
        filtersTable_->setItem(i, 3, new QTableWidgetItem(filters_[i].active ? tr("Yes") : tr("No")));
    }
}
