...
AlEventPlugin::AlEventPlugin(EventBus *bus, AlEventService *service,
                             QObject *parent)
...
// ── Identity ──────────────────────────────────────────────────────────────
QString AlEventPlugin::id() const { return "alevent"; }
QString AlEventPlugin::displayName() const { return "AL Events"; }
QString AlEventPlugin::displayNameZh() const { return QStringLiteral("AL事件"); }
int AlEventPlugin::defaultOrder() const { return 65; }
bool AlEventPlugin::visible() const { return true; }

QWidget *AlEventPlugin::widget() { return container_; }
...
void AlEventPlugin::buildUi() {
  ...
  table_->setHorizontalHeaderLabels({
    tr("Time"), tr("Slave"), tr("Name"),
    tr("Code"), tr("Severity"), tr("Description")
  });
  ...
  connect(filterCombo_, &QComboBox::currentIndexChanged,
          this, &AlEventPlugin::applySeverityFilter);
  connect(clearBtn, &QPushButton::clicked, service_,
          &AlEventService::clearEvents);
}
...
void AlEventPlugin::populateTable(const QJsonObject &data) {
  ...
  for (int i = 0; i < events.size(); ++i) {
    ...
    table_->setItem(row, kColTime,
                    new QTableWidgetItem(ev.value("timestamp").toString()));
    table_->setItem(row, kColSlave,
                    new QTableWidgetItem(ev.value("slavePosition").toString()));
    table_->setItem(row, kColName,
                    new QTableWidgetItem(ev.value("name").toString()));
    table_->setItem(row, kColCode,
                    new QTableWidgetItem(ev.value("code").toString()));
    table_->setItem(row, kColSeverity,
                    new QTableWidgetItem(ev.value("severity").toString()));
    table_->setItem(row, kColDescription,
                    new QTableWidgetItem(ev.value("description").toString()));
  }
...
void AlEventPlugin::updateFilterVisibility() {
  ...
  for (int r = 0; r < table_->rowCount(); ++r) {
    ...
    const bool match = filter == tr("All") ||
                       (sevItem && sevItem->text() == filter);
