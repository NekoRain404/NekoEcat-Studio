...
DiagnosticsPlugin::DiagnosticsPlugin(ServiceContainer *container,
                                     QObject *parent)
...
QString DiagnosticsPlugin::id() const { return "diagnostics"; }
QString DiagnosticsPlugin::displayName() const { return "Diagnostics"; }
QString DiagnosticsPlugin::displayNameZh() const {
  return QStringLiteral("诊断");
}
QIcon DiagnosticsPlugin::icon() const { return QIcon::fromTheme("dialog-warning"); }
int DiagnosticsPlugin::defaultOrder() const { return 70; }
bool DiagnosticsPlugin::visible() const { return true; }
...
QWidget *DiagnosticsPlugin::widget() { return containerWidget_; }

QTableWidget *DiagnosticsPlugin::diagnosticsTable() const { return table_; }
QLineEdit *DiagnosticsPlugin::diagnosticsFilter() const { return filter_; }
...
void DiagnosticsPlugin::buildUi() {
  ...
  table_->setHorizontalHeaderLabels(
      {tr("Timestamp"), tr("Level"), tr("Message")});
  ...
  connect(levelFilter_, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, &DiagnosticsPlugin::filterDiagnosticsTable);
}

void DiagnosticsPlugin::filterDiagnosticsTable() {
  if (!table_) return;
  const QString needle = filter_ ? filter_->text().trimmed() : QString();
  const QString level = levelFilter_
                            ? levelFilter_->currentData().toString()
                            : QString();

  for (int row = 0; row < table_->rowCount(); ++row) {
    const QString rowLevel =
        table_->item(row, 1) ? table_->item(row, 1)->text() : QString();
...
void DiagnosticsPlugin::updateDiagnosticsSummary() {
  if (!summaryLabel_ || !table_) return;
  int visible = 0;
  int errors = 0;
  int warnings = 0;
  for (int row = 0; row < table_->rowCount(); ++row) {
    if (!table_->isRowHidden(row)) ++visible;
    const QString level =
        table_->item(row, 1) ? table_->item(row, 1)->text() : QString();
    if (level == "Error")
      ++errors;
  ...
  summaryLabel_->setText(tr("%1 shown | %2 errors | %3 warnings")
                             .arg(visible)
                             .arg(errors)
                             .arg(warnings));
}
...
void DiagnosticsPlugin::exportDiagnosticsReport(QWidget *parentWidget) {
  const QString path = QFileDialog::getSaveFileName(
      parentWidget, tr("Export Diagnostics Report"), QString(),
  ...
  out << "| " << tr("Timestamp") << " | " << tr("Level") << " | "
      << tr("Message") << " |\n";
  out << "| --- | --- | --- |\n";
  for (int row = 0; row < table_->rowCount(); ++row) {
    const QString ts =
        table_->item(row, 0) ? table_->item(row, 0)->text() : QString();
    const QString lvl =
        table_->item(row, 1) ? table_->item(row, 1)->text() : QString();
    const QString msg =
