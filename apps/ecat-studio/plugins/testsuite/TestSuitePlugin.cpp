...
QString TestSuitePlugin::id() const { return "testsuite"; }
QString TestSuitePlugin::displayName() const { return "Test Suite"; }
...
QWidget *TestSuitePlugin::widget() { return containerWidget_; }
QTreeWidget *TestSuitePlugin::testList() const { return testList_; }
QTableWidget *TestSuitePlugin::testResults() const { return testResults_; }
QTextEdit *TestSuitePlugin::testReport() const { return testReport_; }
QLabel *TestSuitePlugin::runnerStatus() const { return runnerStatus_; }
...
void TestSuitePlugin::buildUi() {
  ...
  connect(exportButton_, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getSaveFileName(containerWidget_, tr("Export Report"), QString(), "JSON (*.json);;Text (*.txt)");
...
int TestSuitePlugin::testCount() const { return tests_.size(); }
...
void TestSuitePlugin::updateTestResult(const QString &testId, const QString &status, const QString &result, int durationMs) {
  for (int i = 0; i < tests_.size(); ++i) {
    if (tests_[i].id == testId) {
...
int TestSuitePlugin::passedCount() const {
  int count = 0;
  for (const auto &t : tests_) if (t.status == "Passed") count++;
  return count;
}

int TestSuitePlugin::failedCount() const {
  int count = 0;
  for (const auto &t : tests_) if (t.status == "Failed") count++;
  return count;
}

int TestSuitePlugin::skippedCount() const {
  int count = 0;
  for (const auto &t : tests_) if (t.status == "Skipped") count++;
  return count;
}

void TestSuitePlugin::updateStatsLabel() {
  statsLabel_->setText(tr("Passed: %1 | Failed: %2 | Skipped: %3")
    .arg(passedCount()).arg(failedCount()).arg(skippedCount()));
}
