...
QString VisualizationStudioPlugin::id() const {
    return "visualizationstudio";
}
QString VisualizationStudioPlugin::displayName() const {
    return "Visualization Studio";
}
QString VisualizationStudioPlugin::displayNameZh() const {
    return QStringLiteral("可视化工作室");
}
QIcon VisualizationStudioPlugin::icon() const {
    return QIcon::fromTheme("preferences-desktop-display");
}
int VisualizationStudioPlugin::defaultOrder() const {
    return 340;
}
bool VisualizationStudioPlugin::visible() const {
    return false;
}
...
QWidget* VisualizationStudioPlugin::widget() {
    return containerWidget_;
}
QWidget* VisualizationStudioPlugin::canvas() const {
    return canvas_;
}
QListWidget* VisualizationStudioPlugin::dataSources() const {
    return dataSources_;
}
QTreeWidget* VisualizationStudioPlugin::chartTypes() const {
    return chartTypes_;
}
QTextEdit* VisualizationStudioPlugin::exportOptions() const {
...
void VisualizationStudioPlugin::buildUi() {
    ...
    connect(addDsBtn, &QPushButton::clicked, this,
            [this]() { addDataSource(tr("New Source %1").arg(dataSources_->count() + 1)); });
    connect(removeDsBtn, &QPushButton::clicked, this, [this]() {
        auto* item = dataSources_->currentItem();
        if (item)
            removeDataSource(item->text());
    });
...
void VisualizationStudioPlugin::clearDataSources() {
    dataSources_->clear();
}
int VisualizationStudioPlugin::dataSourceCount() const {
...
void VisualizationStudioPlugin::clearChartTypes() {
    chartTypes_->clear();
}
...
bool VisualizationStudioPlugin::importConfig(const QString& filePath) {
    ...
    if (!root.value("dataSources").isArray())
        return false;
    ...
    const QString importedExportOptions = root.contains("exportOptions") ? root["exportOptions"].toString() : QString();
