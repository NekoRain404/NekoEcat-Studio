...
QString ComplianceCheckerPlugin::id() const {
    return "compliancechecker";
}
QString ComplianceCheckerPlugin::displayName() const {
    return "Compliance Checker";
}
QString ComplianceCheckerPlugin::displayNameZh() const {
    return "合规检查器";
}
int ComplianceCheckerPlugin::defaultOrder() const {
    return 285;
}
bool ComplianceCheckerPlugin::visible() const {
    return false;
}
...
int ComplianceCheckerPlugin::checkCount() const {
    return checks_.size();
}
...
ComplianceCheckerPlugin::ComplianceReport ComplianceCheckerPlugin::generateReport() const {
...
QLabel* ComplianceCheckerPlugin::statusLabel() const {
    return statusLabel_;
}
QLabel* ComplianceCheckerPlugin::scoreLabel() const {
    return scoreLabel_;
}
...
void ComplianceCheckerPlugin::buildUi() {
    ...
    categoryFilter_->addItems({"All", "Network", "Timing", "Communication", "Safety", "Maintenance"});
    ...
    checkTable_->setHorizontalHeaderLabels({"ID", "Name", "Category", "Status", "Last Checked"});
    ...
    violationTable_->setHorizontalHeaderLabels({"ID", "Severity", "Description", "Recommendation", "Detected"});
    ...
    recommendationTable_->setHorizontalHeaderLabels({"ID", "Priority", "Title", "Description"});
    ...
    connect(removeRecBtn_, &QPushButton::clicked, this, [this]() {
        ...
        if (row >= 0)
            removeRecommendation(row);
    });
    connect(exportReportBtn_, &QPushButton::clicked, this, [this]() { exportReport("/tmp/compliance_report.txt"); });
}
...
void ComplianceCheckerPlugin::rebuildCheckTable() {
    ...
    for (int i = 0; i < checks_.size(); ++i) {
        ...
        checkTable_->setItem(i, 3, new QTableWidgetItem(c.passed ? "PASS" : "FAIL"));
        checkTable_->setItem(i, 4, new QTableWidgetItem(c.checkedAt.toString(Qt::ISODate)));
    }
}
...
void ComplianceCheckerPlugin::rebuildViolationTable() {
    ...
    for (int i = 0; i < violations_.size(); ++i) {
        ...
        violationTable_->setItem(i, 4, new QTableWidgetItem(v.detectedAt.toString(Qt::ISODate)));
    }
}

void ComplianceCheckerPlugin::rebuildRecommendationTable() {
    if (!recommendationTable_)
        return;
...
void ComplianceCheckerPlugin::updateScore() {
    ...
    if (scoreLabel_)
        scoreLabel_->setText(QString("Compliance Score: %1%").arg(QString::number(score, 'f', 1)));
    if (statusLabel_)
        statusLabel_->setText(QString("Checks: %1 | Violations: %2 | Score: %3%")
                                  .arg(checks_.size())
