...
AutomationPlugin::AutomationPlugin(ScriptingService* scriptingService, QObject* parent)
    : scriptingService_(scriptingService) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(scriptingService_, &ScriptingService::scriptStarted, this, &AutomationPlugin::onScriptStarted);
    connect(scriptingService_, &ScriptingService::scriptCompleted, this, &AutomationPlugin::onScriptCompleted);
    connect(scriptingService_, &ScriptingService::scriptError, this, &AutomationPlugin::onScriptError);
    connect(scriptingService_, &ScriptingService::logMessage, this, &AutomationPlugin::onLogMessage);
}

QString AutomationPlugin::id() const {
    return "automation";
}
QString AutomationPlugin::displayName() const {
    return "Automation";
}
QString AutomationPlugin::displayNameZh() const {
    return QStringLiteral("\xe8\x87\xaa\xe5\x8a\xa8\xe5\x8c\x96");
}
int AutomationPlugin::defaultOrder() const {
    return 120;
}
bool AutomationPlugin::visible() const {
    return false;
}

QWidget* AutomationPlugin::widget() {
    return container_;
}
...
void AutomationPlugin::buildUi() {
    ...
    connect(runBtn_, &QPushButton::clicked, this, &AutomationPlugin::runCurrentScript);
    connect(stopBtn_, &QPushButton::clicked, this, &AutomationPlugin::stopScript);
    connect(saveBtn_, &QPushButton::clicked, this, &AutomationPlugin::saveCurrentScript);
    connect(deleteBtn_, &QPushButton::clicked, this, &AutomationPlugin::deleteSelectedScript);
    connect(scriptList_, &QListWidget::currentRowChanged, this, &AutomationPlugin::onScriptSelected);
    connect(templateBtn_, &QPushButton::clicked, this, &AutomationPlugin::insertTemplate);
...
void AutomationPlugin::saveCurrentScript() {
    QString name = scriptList_->currentItem() ? scriptList_->currentItem()->text() : tr("untitled");
    scriptingService_->saveScript(name, editor_->toPlainText());
    refreshScriptList();
}

void AutomationPlugin::deleteSelectedScript() {
    if (!scriptList_->currentItem())
        return;
    scriptingService_->saveScript(scriptList_->currentItem()->text(), QString());
    refreshScriptList();
}
...
void AutomationPlugin::onScriptCompleted(const QString& name,
    ...
    runBtn_->setEnabled(true);
    stopBtn_->setEnabled(false);
    console_->appendPlainText(tr("Script completed: %1 => %2").arg(name, resultStr));
}

void AutomationPlugin::onScriptError(const QString& name, const QString& error) {
    running_ = false;
    runBtn_->setEnabled(true);
    stopBtn_->setEnabled(false);
...
void AutomationPlugin::insertTemplate() {
    ...
    menu->addAction(tr("Read SDO"), [this] {
        editor_->setPlainText(QStringLiteral("var val = readSDO(0, '1000', '0');\nlog('Value: ' + val);\n"));
    });
    menu->addAction(tr("Write SDO"), [this] {
        editor_->setPlainText(QStringLiteral("writeSDO(0, '1000', '0', '1', 'uint32');\n"
                                             "log('Write requested; wait for daemon confirmation');\n"));
    });
    menu->addAction(tr("Scan Topology"), [this] {
        editor_->setPlainText(QStringLiteral("var slaves = scanTopology();\n"
                                             "for (var i = 0; i < slaves.length; i++) {\n"
                                             "  log(slaves[i].position + ': ' + slaves[i].name);\n"
                                             "}\n"));
    });
    menu->addAction(tr("Set All States"), [this] {
        editor_->setPlainText(QStringLiteral("setState(0, 'OP');\n"
