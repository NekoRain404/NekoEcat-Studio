...
QString DeploymentManagerPlugin::id() const {
    return "deploymentmanager";
}
QString DeploymentManagerPlugin::displayName() const {
    return "Deployment Manager";
}
QString DeploymentManagerPlugin::displayNameZh() const {
    return QStringLiteral("部署管理器");
}
QIcon DeploymentManagerPlugin::icon() const {
    return QIcon::fromTheme("system-software-install");
}
int DeploymentManagerPlugin::defaultOrder() const {
    return 310;
}
bool DeploymentManagerPlugin::visible() const {
    return false;
}
void DeploymentManagerPlugin::activate() {}
void DeploymentManagerPlugin::deactivate() {}

QWidget* DeploymentManagerPlugin::widget() {
    return container_;
}
QTableWidget* DeploymentManagerPlugin::targetTable() const {
    return targetTable_;
}
QTableWidget* DeploymentManagerPlugin::packageTable() const {
    return packageTable_;
}
QTableWidget* DeploymentManagerPlugin::historyTable() const {
    return historyTable_;
}
QTextEdit* DeploymentManagerPlugin::statusLog() const {
    return statusLog_;
}
QLabel* DeploymentManagerPlugin::statusLabel() const {
    return statusLabel_;
}

int DeploymentManagerPlugin::targetCount() const {
    return targets_.size();
}
int DeploymentManagerPlugin::packageCount() const {
    return packages_.size();
}
int DeploymentManagerPlugin::historyCount() const {
...
void DeploymentManagerPlugin::addTarget(const DeploymentMgrTarget& target) {
    ...
    if (t.id.isEmpty())
        t.id = QString("dt_%1").arg(nextId_++);
    targets_.append(t);
...
void DeploymentManagerPlugin::addPackage(const DeploymentMgrPackage& pkg) {
    ...
    if (p.id.isEmpty())
        p.id = QString("dp_%1").arg(nextId_++);
    if (p.createdAt.isEmpty())
        p.createdAt = QDateTime::currentDateTime().toString(Qt::ISODate);
    packages_.append(p);
...
void DeploymentManagerPlugin::deploy(int targetIndex, int packageIndex) {
    if (targetIndex < 0 || targetIndex >= targets_.size())
        return;
    if (packageIndex < 0 || packageIndex >= packages_.size())
        return;
    ...
    rec.log =
        tr("Deployment of %1 v%2 to %3 requires a deployment backend acknowledgement.").arg(p.name, p.version, t.name);
    records_.append(rec);
...
void DeploymentManagerPlugin::rollback(int historyIndex) {
    if (historyIndex < 0 || historyIndex >= records_.size())
        return;
