...
OptimizationDashboardPlugin::OptimizationDashboardPlugin(QObject* parent) : WorkspacePlugin() {}
...
int OptimizationDashboardPlugin::defaultOrder() const {
    return 90;
}

bool OptimizationDashboardPlugin::visible() const {
    return false;
}
...
void OptimizationDashboardPlugin::updateMetric(int index, double value) {
    if (index >= 0 && index < metrics_.size()) {
        ...
        metrics_[index].improvement =
            (metrics_[index].target > 0) ? ((value - metrics_[index].target) / metrics_[index].target * 100) : 0.0;
...
QVector<OptimizationDashboardPlugin::OptimizationMetric> OptimizationDashboardPlugin::metrics() const {
    return metrics_;
}

void OptimizationDashboardPlugin::addHistoryEntry(const OptimizationHistoryEntry& entry) {
...
QVector<OptimizationDashboardPlugin::OptimizationHistoryEntry> OptimizationDashboardPlugin::history() const {
    return history_;
}

void OptimizationDashboardPlugin::addRecommendation(const OptimizationRecommendation& rec) {
...
QVector<OptimizationDashboardPlugin::OptimizationRecommendation> OptimizationDashboardPlugin::recommendations() const {
...
QVector<OptimizationDashboardPlugin::OptimizationAction> OptimizationDashboardPlugin::actions() const {
    return actions_;
}
...
void OptimizationDashboardPlugin::refresh() {
    ...
    if (statusLabel_)
        statusLabel_->setText(
            QStringLiteral("Last refreshed: %1").arg(QDateTime::currentDateTime().toString(Qt::ISODate)));
}
...
void OptimizationDashboardPlugin::buildUi() {
    ...
    metricsTable_->setHorizontalHeaderLabels({tr("Name"), tr("Value"), tr("Target"), tr("Improvement")});
    tabs_->addTab(metricsTable_, tr("Metrics"));

    historyTable_ = new QTableWidget;
    historyTable_->setColumnCount(4);
    historyTable_->setHorizontalHeaderLabels({tr("Timestamp"), tr("Action"), tr("Result"), tr("Improvement")});
    tabs_->addTab(historyTable_, tr("History"));

    recommendationsTable_ = new QTableWidget;
    recommendationsTable_->setColumnCount(4);
    recommendationsTable_->setHorizontalHeaderLabels({tr("Title"), tr("Description"), tr("Priority"), tr("Category")});
    tabs_->addTab(recommendationsTable_, tr("Recommendations"));

    actionsTable_ = new QTableWidget;
    actionsTable_->setColumnCount(4);
    actionsTable_->setHorizontalHeaderLabels({tr("Name"), tr("Description"), tr("Executed"), tr("Result")});
    ...
    connect(refreshBtn_, &QPushButton::clicked, this, &OptimizationDashboardPlugin::refresh);
...
void OptimizationDashboardPlugin::rebuildMetricsTable() {
    ...
    for (int i = 0; i < metrics_.size(); ++i) {
        ...
        metricsTable_->setItem(i, 2, new QTableWidgetItem(QString::number(metrics_[i].target, 'f', 2)));
        metricsTable_->setItem(
            i, 3, new QTableWidgetItem(QString::number(metrics_[i].improvement, 'f', 2) + QStringLiteral("%")));
    }
}
...
void OptimizationDashboardPlugin::rebuildHistoryTable() {
    ...
    for (int i = 0; i < history_.size(); ++i) {
        historyTable_->setItem(i, 0, new QTableWidgetItem(history_[i].timestamp.toString(Qt::ISODate)));
        ...
        historyTable_->setItem(
            i, 3, new QTableWidgetItem(QString::number(history_[i].improvement, 'f', 2) + QStringLiteral("%")));
    }
}
...
void OptimizationDashboardPlugin::rebuildRecommendationsTable() {
    ...
    for (int i = 0; i < recommendations_.size(); ++i) {
        recommendationsTable_->setItem(i, 0, new QTableWidgetItem(recommendations_[i].title));
        recommendationsTable_->setItem(i, 1, new QTableWidgetItem(recommendations_[i].description));
        recommendationsTable_->setItem(i, 2, new QTableWidgetItem(recommendations_[i].priority));
        recommendationsTable_->setItem(i, 3, new QTableWidgetItem(recommendations_[i].category));
    }
}
...
void OptimizationDashboardPlugin::rebuildActionsTable() {
    ...
    for (int i = 0; i < actions_.size(); ++i) {
        ...
        actionsTable_->setItem(i, 1, new QTableWidgetItem(actions_[i].description));
        actionsTable_->setItem(i, 2, new QTableWidgetItem(actions_[i].executed ? tr("Yes") : tr("No")));
