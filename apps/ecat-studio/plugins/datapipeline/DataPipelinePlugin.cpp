...
DataPipelinePlugin::DataPipelinePlugin(DataPipelineService *service,
                                       QObject *parent)
    : service_(service) {
  if (parent) setParent(parent);
  buildUi();

  connect(service_, &DataPipelineService::pipelineUpdated, this,
          [this]() { updateDisplay(); });
}

QString DataPipelinePlugin::id() const { return "datapipeline"; }
QString DataPipelinePlugin::displayName() const { return "Data Pipeline"; }
...
QWidget *DataPipelinePlugin::widget() { return containerWidget_; }
...
void DataPipelinePlugin::buildUi() {
  ...
  stageTable_->setHorizontalHeaderLabels(
      {tr("Stage"), tr("Type"), tr("Status"), tr("Processed"), tr("Errors")});
  ...
  connect(addStageBtn_, &QPushButton::clicked, this,
          [this]() { service_->addDefaultStage(); });

  connect(removeStageBtn_, &QPushButton::clicked, this, [this]() {
    int row = stageTable_->currentRow();
    if (row >= 0) service_->removeStage(row);
  });
...
void DataPipelinePlugin::updateDisplay() {
  ...
  for (const auto &s : stages) {
    ...
    stageTable_->setItem(r, 2,
                        new QTableWidgetItem(s.active ? tr("Active") : tr("Idle")));
    stageTable_->setItem(r, 3,
                        new QTableWidgetItem(QString::number(s.processed)));
    stageTable_->setItem(r, 4,
                        new QTableWidgetItem(QString::number(s.errors)));
  }

  throughputLabel_->setText(
      tr("Throughput: %1 msg/s")
          .arg(QString::number(metrics.throughput, 'f', 1)));
  latencyLabel_->setText(
      tr("Latency: %1 ms")
          .arg(QString::number(metrics.latencyMs, 'f', 2)));
}
