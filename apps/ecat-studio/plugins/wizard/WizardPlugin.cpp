...
WizardPlugin::WizardPlugin(QObject* parent) {
    if (parent)
        setParent(parent);
    wizards_ = {
        {"network_setup", "Network Setup", "Configuration", "Configure EtherCAT network topology and master settings.",
         4},
        {"slave_discovery", "Slave Discovery", "Scanning", "Discover and register all slaves on the network.", 3},
        {"pdo_mapping", "PDO Mapping", "Configuration", "Map process data objects for real-time exchange.", 5},
        {"sdo_config", "SDO Configuration", "Configuration", "Configure service data objects for slave parameters.", 4},
        {"dc_sync_setup", "DC Sync Setup", "Synchronization", "Configure distributed clock synchronization.", 3},
        {"diagnostics_check", "Diagnostics Check", "Maintenance", "Run comprehensive diagnostics on the network.", 3},
    };
    buildUi();
}

QString WizardPlugin::id() const {
    return "wizard";
}
QString WizardPlugin::displayName() const {
    return "Wizard";
}
QString WizardPlugin::displayNameZh() const {
    return "向导";
}
int WizardPlugin::defaultOrder() const {
    return 220;
}
bool WizardPlugin::visible() const {
    return false;
}
...
int WizardPlugin::wizardCount() const {
    return wizards_.size();
}

void WizardPlugin::setWizardSteps(const QString& wizardId, const QVector<WizardStep>& steps) {
...
int WizardPlugin::currentStep() const {
    return currentStep_;
}
int WizardPlugin::totalSteps() const {
    return currentSteps_.size();
}
bool WizardPlugin::isRunning() const {
    return running_;
}
...
void WizardPlugin::nextStep() {
    if (!running_ || currentStep_ >= currentSteps_.size() - 1)
        return;
    ++currentStep_;
    updateStepView();
...
int WizardPlugin::historyCount() const {
    return history_.size();
}
QListWidget* WizardPlugin::wizardList() const {
    return wizardList_;
}
QTableWidget* WizardPlugin::stepTable() const {
    return stepTable_;
}
QTableWidget* WizardPlugin::historyTable() const {
...
bool WizardPlugin::exportHistory(const QString& path) {
    ...
    for (const auto& h : history_) {
        out << h.wizardId << "," << h.wizardName << "," << h.completedAt.toString(Qt::ISODate) << ","
...
void WizardPlugin::updateStepView() {
    if (!running_) {
        if (stepLabel_)
            stepLabel_->setText("No wizard running");
        if (instructionView_)
            instructionView_->clear();
        if (stepTable_) {
