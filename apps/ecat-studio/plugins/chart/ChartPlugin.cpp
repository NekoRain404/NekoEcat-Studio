...
ChartPlugin::ChartPlugin(ChartService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
    loadSampleData();
...
QString ChartPlugin::displayName() const {
    return "Charts";
}
QString ChartPlugin::displayNameZh() const {
    return QStringLiteral("图表");
}
int ChartPlugin::defaultOrder() const {
    return 125;
}
bool ChartPlugin::visible() const {
    return false;
}
QWidget* ChartPlugin::widget() {
    return container_;
}
...
void ChartPlugin::buildUi() {
    ...
    // ── Connections ──
    connect(chartTypeCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &ChartPlugin::onChartTypeChanged);
    connect(dataSourceCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &ChartPlugin::onDataSourceChanged);
...
void ChartPlugin::onChartTypeChanged(int index) {
    static const EcatChartWidget::ChartType types[] = {EcatChartWidget::Line, EcatChartWidget::Bar,
                                                       EcatChartWidget::Pie, EcatChartWidget::Scatter,
                                                       EcatChartWidget::Gauge};
    if (index < 0 || index >= 5)
        return;
    ...
    currentChartId_ = service_->createChart(chartTypeCombo_->currentText(), tr("Sample Chart"), data);
...
void ChartPlugin::exportChart() {
    ...
    const QString path =
        QFileDialog::getSaveFileName(container_, tr("Export Chart"), QString(), tr("Images (*.png *.jpg)"));
    if (path.isEmpty())
        return;

    if (service_->exportChart(currentChartId_, path)) {
        QMessageBox::information(container_, tr("Export"), tr("Chart exported to %1").arg(path));
    }
}
