...
void EcatChartWidget::paintEvent(QPaintEvent *) {
  ...
  const QRect area(kMarginLeft, kMarginTop,
  ...
  switch (type_) {
  case Line:    drawLineChart(p, area); break;
  case Bar:     drawBarChart(p, area); break;
  case Pie:     drawPieChart(p, area); break;
  case Scatter: drawScatterChart(p, area); break;
  case Gauge:   drawGaugeChart(p, area); break;
  }
...
void EcatChartWidget::drawLineChart(QPainter &p, const QRect &area) {
  ...
  for (const auto &ds : datasets_) {
    for (double v : ds.values) {
      if (first || v < yMin) yMin = v;
      if (first || v > yMax) yMax = v;
      first = false;
    }
  }
  ...
  for (int i = 0; i <= 5; ++i) {
    ...
    p.drawText(2, y - 8, kMarginLeft - 8, 16,
...
void EcatChartWidget::drawBarChart(QPainter &p, const QRect &area) {
  ...
  for (int gi = 0; gi < nGroups; ++gi) {
    ...
    p.drawText(cx - 40, area.bottom() + 4, 80, 20,
               Qt::AlignCenter, labels_[gi]);
  }
}
...
void EcatChartWidget::drawPieChart(QPainter &p, const QRect &area) {
  ...
  const QRect pieRect(area.center().x() - diameter / 2,
                      area.center().y() - diameter / 2,
                      diameter, diameter);

  double startAngle = 0.0;
  for (int i = 0; i < values.size(); ++i) {
    const double span = (qAbs(values[i]) / total) * 360.0 * 16;
    const QColor color = (i < datasets_[0].color.isValid())
        ? datasets_[0].color
        : kColors[i % kColorCount];
    p.setBrush(color);
    ...
    lf.setPointSize(qMax(7, lf.pointSize() - 1));
    p.setFont(lf);
    p.drawText(lx - 30, ly - 10, 60, 20, Qt::AlignCenter,
...
void EcatChartWidget::drawScatterChart(QPainter &p, const QRect &area) {
  ...
  for (const auto &ds : datasets_) {
    for (int i = 0; i + 1 < ds.values.size(); i += 2) {
      ...
      if (first || x > xMax) xMax = x;
      if (first || y < yMin) yMin = y;
      if (first || y > yMax) yMax = y;
      first = false;
    }
  }
  if (qFuzzyCompare(xMin + 1.0, xMax + 1.0)) { xMin -= 1.0; xMax += 1.0; }
  if (qFuzzyCompare(yMin + 1.0, yMax + 1.0)) { yMin -= 1.0; yMax += 1.0; }
...
void EcatChartWidget::drawGaugeChart(QPainter &p, const QRect &area) {
  ...
  if (frac > 0.7) arcColor = QColor(255, 127, 14);
  if (frac > 0.9) arcColor = QColor(214, 39, 40);
  ...
  vf.setBold(true);
  p.setFont(vf);
  p.drawText(cx - 80, cy - 20, 160, 40, Qt::AlignCenter,
             QString::number(gaugeValue_, 'f', 1));
  ...
  p.setFont(sf);
  p.drawText(cx - radius - 20, cy + radius + 4, 60, 16, Qt::AlignCenter,
             QString::number(gaugeMin_, 'f', 0));
  p.drawText(cx + radius - 40, cy + radius + 4, 60, 16, Qt::AlignCenter,
             QString::number(gaugeMax_, 'f', 0));
}

void EcatChartWidget::drawLegend(QPainter &p, const QRect &area) {
  if (datasets_.isEmpty()) return;
  ...
  for (int i = 0; i < datasets_.size(); ++i) {
    ...
    if (y + 12 > area.bottom()) break;
    const QColor color = datasets_[i].color.isValid()
        ? datasets_[i].color : kColors[i % kColorCount];
    p.fillRect(area.right() - 100, y, 10, 10, color);
    p.drawText(area.right() - 86, y, 80, 12,
               Qt::AlignLeft | Qt::AlignVCenter, datasets_[i].name);
  }
}

void EcatChartWidget::drawTooltip(QPainter &p) {
  if (!rect().contains(mousePos_)) return;
  if (type_ == Gauge || type_ == Pie) return;
  if (datasets_.isEmpty() || labels_.isEmpty()) return;

  const QRect area(kMarginLeft, kMarginTop,
                   width() - kMarginLeft - kMarginRight,
                   height() - kMarginTop - kMarginBottom);
  if (!area.contains(mousePos_)) return;

  const int labelIdx = static_cast<int>(
      (mousePos_.x() - area.left()) * labels_.size() / area.width());
  if (labelIdx < 0 || labelIdx >= labels_.size()) return;
