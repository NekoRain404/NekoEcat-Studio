...
PdoValidationReport PdoMappingValidator::validateSmSize(const QVector<SyncManagerBlock> &sms) {
  ...
  for (int i = 0; i < sms.size(); ++i) {
    ...
    int totalBits = 0;
    for (const auto &entry : sm.entries) {
      if (entry.enabled) totalBits += entry.bitSize;
    }
  ...
  for (int i = 0; i < sms.size(); ++i) {
    for (int j = 0; j < sms[i].entries.size(); ++j) {
      ...
      if (seen.contains(key)) {
        ...
        err.entryIndex = j;
        err.message = QString("Duplicate PDO entry %1.%2 (%3)")
                          .arg(entry.index, entry.subIndex, entry.name);
        report.errors.append(err);
        report.valid = false;
      } else {
        seen.insert(key);
      ...
      if (entry.direction == PdoEntryDirection::Input) {
        for (const auto &range : inputRanges) {
          if (start < range.second && end > range.first) {
            ...
            err.smIndex = sms[i].index;
            err.entryIndex = j;
            err.message = QString("Input PDO %1.%2 overlaps with another entry")
                              .arg(entry.index, entry.subIndex);
            report.errors.append(err);
            report.valid = false;
          }
        }
        ...
        for (const auto &range : outputRanges) {
          if (start < range.second && end > range.first) {
            ...
            err.smIndex = sms[i].index;
            err.entryIndex = j;
            err.message = QString("Output PDO %1.%2 overlaps with another entry")
                              .arg(entry.index, entry.subIndex);
            report.errors.append(err);
            report.valid = false;
          }
...
PdoValidationReport PdoMappingValidator::validateRequired(const QVector<SyncManagerBlock> &sms) {
  ...
  for (const auto &sm : sms) {
    ...
    for (const auto &entry : sm.entries) {
      if (!entry.enabled) continue;
      if (entry.direction == PdoEntryDirection::Input) hasInput = true;
      if (entry.direction == PdoEntryDirection::Output) hasOutput = true;
    }
  }
