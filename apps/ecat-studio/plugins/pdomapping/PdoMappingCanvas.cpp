...
QVector<SyncManagerBlock> PdoMappingCanvas::syncManagers() const {
    return sms_;
}
...
int PdoMappingCanvas::selectedSmIndex() const {
    return selectedSm_;
}
int PdoMappingCanvas::selectedEntryIndex() const {
...
QSize PdoMappingCanvas::minimumSizeHint() const {
...
void PdoMappingCanvas::paintSyncManager(QPainter& p, const SyncManagerBlock& sm, int x, int y) {
    ...
    p.drawText(x + 8, y, metrics_.smWidth - 16, metrics_.smHeaderHeight, Qt::AlignVCenter | Qt::AlignLeft,
...
void PdoMappingCanvas::paintEntry(QPainter& p, const PdoCanvasEntry& entry, int x, int y, int width, bool selected,
                                  bool hasError) {
...
void PdoMappingCanvas::paintBitIndicator(QPainter& p, int bitSize, int x, int y, int height, PdoEntryDirection dir) {
