...
QString PdoMappingEditorPlugin::id() const {
    return "pdomapping";
}
QString PdoMappingEditorPlugin::displayName() const {
    return "PDO Mapping";
}
QString PdoMappingEditorPlugin::displayNameZh() const {
    return "PDO 映射";
}
int PdoMappingEditorPlugin::defaultOrder() const {
    return 25;
}
bool PdoMappingEditorPlugin::visible() const {
    return true;
}
...
int PdoMappingEditorPlugin::slavePosition() const {
    return slavePosition_;
}

void PdoMappingEditorPlugin::addPdoEntry(int smIndex, const QString& index, const QString& subIndex,
                                         const QString& name, const QString& dataType, int bitSize, bool isOutput) {
...
bool PdoMappingEditorPlugin::hasErrors() const {
    return hasErrors_;
}
int PdoMappingEditorPlugin::errorCount() const {
    return errorCount_;
}
...
PdoMappingCanvas* PdoMappingEditorPlugin::canvas() const {
    return canvas_;
}
QTreeWidget* PdoMappingEditorPlugin::pdoTree() const {
    return pdoTree_;
}
QTableWidget* PdoMappingEditorPlugin::propertyTable() const {
    return propertyTable_;
}
QTextEdit* PdoMappingEditorPlugin::validationPanel() const {
    return validationPanel_;
}
...
void PdoMappingEditorPlugin::buildUi() {
    ...
    connect(exportBtn_, &QPushButton::clicked, this, [this]() {
        QString path =
            QFileDialog::getSaveFileName(containerWidget_, tr("Export PDO Mapping"), "", tr("JSON Files (*.json)"));
        if (!path.isEmpty())
            exportMapping(path);
    });
    connect(importBtn_, &QPushButton::clicked, this, [this]() {
        QString path =
            QFileDialog::getOpenFileName(containerWidget_, tr("Import PDO Mapping"), "", tr("JSON Files (*.json)"));
        if (!path.isEmpty())
            importMapping(path);
    });
    connect(addEntryBtn_, &QPushButton::clicked, this,
            [this]() { addPdoEntry(2, "0x6000", "0x00", tr("New Entry"), "UINT16", 16, false); });
    ...
    connect(canvas_, &PdoMappingCanvas::entrySelected, this, [this](int smIdx, int entryIdx) {
        auto sms = canvas_->syncManagers();
        for (const auto& sm : sms) {
            if (sm.index == smIdx && entryIdx >= 0 && entryIdx < sm.entries.size()) {
                ...
                propertyTable_->setItem(
                    5, 1, new QTableWidgetItem(e.direction == PdoEntryDirection::Input ? tr("Input") : tr("Output")));
                break;
            }
        }
    });
...
void PdoMappingEditorPlugin::updateValidationDisplay() {
    ...
    text += tr("\nInput: %1 bits | Output: %2 bits").arg(report.totalInputBits).arg(report.totalOutputBits);
