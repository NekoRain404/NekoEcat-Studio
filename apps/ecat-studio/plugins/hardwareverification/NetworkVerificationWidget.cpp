...
NetworkVerificationWidget::NetworkVerificationWidget(
  ...
  connect(runButton_, &QPushButton::clicked, this,
          &NetworkVerificationWidget::runVerification);
}
...
void NetworkVerificationWidget::displayResult(
  ...
  for (int i = 0; i < result.tests.size(); ++i) {
    ...
    resultTable_->setItem(
        i, 2, new QTableWidgetItem(t.passed ? tr("PASS") : tr("FAIL")));
    resultTable_->setItem(
        i, 3,
