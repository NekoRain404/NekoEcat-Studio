...
HardwareVerificationPlugin::HardwareVerificationPlugin(HardwareVerificationService* service, QObject* parent)
    : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
    ...
    connect(service_, &HardwareVerificationService::verificationCompleted, this,
            [this](const VerificationResult& result) {
                emit verificationCompleted(result.verificationName, result.passed, result.failed);
            });
}
...
QWidget* HardwareVerificationPlugin::widget() {
    return containerWidget_;
}

int HardwareVerificationPlugin::defaultOrder() const {
    return 36;
}

bool HardwareVerificationPlugin::visible() const {
    return false;
}
...
void HardwareVerificationPlugin::onConnectionChanged(bool connected) {
    if (deviceWidget_ && !connected)
        deviceWidget_->clear();
    if (networkWidget_ && !connected)
        networkWidget_->clear();
}

HardwareVerificationService* HardwareVerificationPlugin::verificationService() const {
    return service_;
}
...
void HardwareVerificationPlugin::buildUi() {
    ...
    connect(timingRunButton, &QPushButton::clicked, this, &HardwareVerificationPlugin::updateTimingTab);
    ...
    auto* complianceRunButton = new QPushButton(tr("Run Compliance Verification"));
    ...
    complianceLabel_ = new QLabel(tr("Click to run compliance verification tests."));
    ...
    connect(complianceRunButton, &QPushButton::clicked, this, &HardwareVerificationPlugin::updateComplianceTab);
...
void HardwareVerificationPlugin::updateTimingTab() {
    ...
    for (const auto& t : result.tests) {
        ...
        lines << QStringLiteral("%1: %2 (%3 ms) - %4").arg(t.testName, status).arg(t.durationMs).arg(t.details);
    }
