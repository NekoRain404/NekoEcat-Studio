...
QString ReportPlugin::id() const { return "report"; }
...
int ReportPlugin::reportTemplateCount() const { return reportTemplates_.size(); }
...
int ReportPlugin::dataSourceCount() const { return dataSources_.size(); }

void ReportPlugin::selectTemplate(int index) {
  if (index < 0 || index >= reportTemplates_.size()) return;
...
void ReportPlugin::generateReport() {
  if (selectedTemplate_ < 0 || selectedTemplate_ >= reportTemplates_.size()) return;
...
int ReportPlugin::reportCount() const { return history_.size(); }

QTableWidget *ReportPlugin::templateTable() const { return templateTable_; }
QTableWidget *ReportPlugin::dataSourceTable() const { return dataSourceTable_; }
QTableWidget *ReportPlugin::historyTable() const { return historyTable_; }
...
void ReportPlugin::buildUi() {
  ...
  connect(exportBtn_, &QPushButton::clicked, this, [this]() {
    exportReport("/tmp/report_export.txt");
  });
  connect(exportHistoryBtn_, &QPushButton::clicked, this, [this]() {
    exportHistory("/tmp/report_history.csv");
  });
}
