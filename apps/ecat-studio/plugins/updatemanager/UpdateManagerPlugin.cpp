...
QString UpdateManagerPlugin::id() const {
    return "updatemanager";
}
QString UpdateManagerPlugin::displayName() const {
    return "Update Manager";
}
QString UpdateManagerPlugin::displayNameZh() const {
    return QStringLiteral("更新管理器");
}
QIcon UpdateManagerPlugin::icon() const {
    return QIcon::fromTheme("system-software-update");
}
int UpdateManagerPlugin::defaultOrder() const {
    return 315;
}
bool UpdateManagerPlugin::visible() const {
    return false;
}
void UpdateManagerPlugin::activate() {}
void UpdateManagerPlugin::deactivate() {}

QWidget* UpdateManagerPlugin::widget() {
    return container_;
}
QTableWidget* UpdateManagerPlugin::availableTable() const {
    return availableTable_;
}
QTableWidget* UpdateManagerPlugin::historyTable() const {
    return historyTable_;
}
QTextEdit* UpdateManagerPlugin::settingsPanel() const {
    return settingsPanel_;
}
QTextEdit* UpdateManagerPlugin::statusPanel() const {
    return statusPanel_;
}
QLabel* UpdateManagerPlugin::statusLabel() const {
    return statusLabel_;
}

int UpdateManagerPlugin::availableCount() const {
    return available_.size();
}
int UpdateManagerPlugin::historyCount() const {
...
void UpdateManagerPlugin::buildUi() {
    ...
    availableTable_->setHorizontalHeaderLabels(
        {tr("Name"), tr("Current"), tr("Available"), tr("Description"), tr("Status")});
    ...
    connect(rollbackBtn_, &QPushButton::clicked, this, [this]() {
        if (!history_.isEmpty())
            rollbackUpdate(history_.size() - 1);
    });
...
void UpdateManagerPlugin::addAvailableUpdate(const UpdateEntry& entry) {
    UpdateEntry e = entry;
    if (e.id.isEmpty())
        e.id = QString("upd_%1").arg(nextId_++);
    available_.append(e);
...
void UpdateManagerPlugin::removeAvailableUpdate(int index) {
    if (index < 0 || index >= available_.size())
        return;
...
void UpdateManagerPlugin::rollbackUpdate(int historyIndex) {
    if (historyIndex < 0 || historyIndex >= history_.size())
        return;
