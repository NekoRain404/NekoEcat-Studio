...
QString CalibrationPlugin::id() const { return "calibration"; }
QString CalibrationPlugin::displayName() const { return "Calibration"; }
...
QWidget *CalibrationPlugin::widget() { return containerWidget_; }

CalibrationPlugin::WizardStep CalibrationPlugin::currentStep() const { return currentStep_; }
CalibrationPlugin::CalibrationType CalibrationPlugin::calibrationType() const { return calType_; }
void CalibrationPlugin::setCalibrationType(CalibrationType type) {
  calType_ = type;
  typeCombo_->setCurrentIndex(static_cast<int>(type));
}

bool CalibrationPlugin::isCalibrating() const { return calibrating_; }
int CalibrationPlugin::collectedSamples() const { return collectedSamples_; }
int CalibrationPlugin::requiredSamples() const { return requiredSamples_; }
void CalibrationPlugin::setRequiredSamples(int count) {
  requiredSamples_ = count;
  sampleCountSpin_->setValue(count);
}

double CalibrationPlugin::offsetResult() const { return offsetResult_; }
double CalibrationPlugin::gainResult() const { return gainResult_; }
double CalibrationPlugin::linearityError() const { return linearityError_; }

QTableWidget *CalibrationPlugin::dataTable() const { return dataTable_; }
QTableWidget *CalibrationPlugin::historyTable() const { return historyTable_; }
QTextEdit *CalibrationPlugin::resultsView() const { return resultsView_; }
...
void CalibrationPlugin::buildUi() {
  ...
  connect(exportDataBtn_, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getSaveFileName(
        containerWidget_, tr("Export Calibration Data"), "calibration_data.csv",
        tr("CSV Files (*.csv)"));
    if (!path.isEmpty()) exportCalibrationData(path);
  });
  connect(exportHistoryBtn_, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getSaveFileName(
        containerWidget_, tr("Export Calibration History"), "calibration_history.csv",
        tr("CSV Files (*.csv)"));
    if (!path.isEmpty()) exportHistory(path);
  });
}
...
QWidget *CalibrationPlugin::buildSelectPage() {
  ...
  deviceCombo_->addItems({tr("Slave 0 (Auto-detect)"), tr("Slave 1"), tr("Slave 2"),
                          tr("Slave 3"), tr("Custom Address...")});
...
QWidget *CalibrationPlugin::buildCollectingPage() {
  ...
  dataTable_->setHorizontalHeaderLabels(
      {tr("Sample #"), tr("Raw Value"), tr("Reference Value"), tr("Error (%)")});
...
void CalibrationPlugin::analyzeResults() {
  if (sampleValues_.isEmpty()) return;

  double sum = 0.0;
  for (double v : sampleValues_) sum += v;
...
void CalibrationPlugin::resetWizard() {
  ...
  linearityError_ = 0.0;
  dataTable_->setRowCount(0);
  if (resultsView_) resultsView_->clear();
...
void CalibrationPlugin::addHistoryEntry(const QString &device, CalibrationType type,
                                        double offset, double gain, double linearity) {
  int row = historyTable_->rowCount();
  historyTable_->insertRow(row);
  historyTable_->setItem(row, 0,
  ...
  QString typeStr;
  switch (type) {
  case CalibrationType::Offset: typeStr = "Offset"; break;
  case CalibrationType::Gain: typeStr = "Gain"; break;
  case CalibrationType::Linearity: typeStr = "Linearity"; break;
  case CalibrationType::Full: typeStr = "Full"; break;
  }
...
int CalibrationPlugin::historyCount() const { return historyTable_->rowCount(); }

void CalibrationPlugin::updateStepDisplay() {
  static const char *stepNames[] = {"Select Device", "Configure", "Collecting",
