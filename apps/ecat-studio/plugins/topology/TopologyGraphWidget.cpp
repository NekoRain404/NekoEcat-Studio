...
TopologyGraphWidget::TopologyGraphWidget(QWidget *parent)
...
void TopologyGraphWidget::setSlaves(const QVector<SlaveInfo> &slaves) {
  ...
  for (const auto &s : slaves) {
    ...
    connect(node, &SlaveNodeItem::clicked,
            this, &TopologyGraphWidget::slaveClicked);
    connect(node, &SlaveNodeItem::doubleClicked,
            this, &TopologyGraphWidget::slaveDoubleClicked);
    scene_->addItem(node);
    nodes_.append(node);
  }
...
void TopologyGraphWidget::applyTreeLayout() {
  ...
  for (int i = 0; i < nodes_.size(); ++i) {
    ...
    nodes_[i]->setPos(kMargin + col * kNodeSpacingX,
                      kMargin + row * kNodeSpacingY);
  }
}
...
void TopologyGraphWidget::drawConnections() {
  for (int i = 0; i + 1 < nodes_.size(); ++i) {
    ...
    const QPointF arrowP2 = p2 - QPointF(8.0 * std::cos(angle - 0.4),
                                         8.0 * std::sin(angle - 0.4));
    const QPointF arrowP3 = p2 - QPointF(8.0 * std::cos(angle + 0.4),
                                         8.0 * std::sin(angle + 0.4));
    scene_->addPolygon({p2, arrowP2, arrowP3},
