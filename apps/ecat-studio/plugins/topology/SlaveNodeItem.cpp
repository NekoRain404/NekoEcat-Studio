...
SlaveNodeItem::SlaveNodeItem(int position, const QString& name, const QString& state, QGraphicsItem* parent)
...
void SlaveNodeItem::paint(QPainter* painter, const QStyleOptionGraphicsItem* option, QWidget*) {
    Q_UNUSED(option);
    ...
    font.setBold(true);
    painter->setFont(font);
    painter->drawText(QRectF(kNodeWidth - 60, 8, 52, 20), Qt::AlignRight | Qt::AlignVCenter, state_);
    font.setBold(false);
    painter->setFont(font);
    ...
    const QString elidedName = fm.elidedText(name_, Qt::ElideRight, static_cast<int>(kNodeWidth - 20));
    painter->drawText(QRectF(8, 30, kNodeWidth - 16, 22), Qt::AlignLeft | Qt::AlignVCenter, elidedName);
    ...
    if (!vendorId_.isEmpty()) {
        ...
        font.setPointSize(8);
        painter->setFont(font);
        painter->drawText(QRectF(8, 54, kNodeWidth - 16, 18), Qt::AlignLeft | Qt::AlignVCenter, vendorId_);
    }
}
