...
TopologyPlugin::TopologyPlugin(EventBus* bus, QObject* parent) : bus_(bus) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(bus_, &EventBus::slaveChanged, this, &TopologyPlugin::handleSlaveChanged);
}

// ── Identity ──────────────────────────────────────────────────────────
QString TopologyPlugin::id() const {
    return "topology";
}
QString TopologyPlugin::displayName() const {
    return "Topology";
}
QString TopologyPlugin::displayNameZh() const {
    return QStringLiteral("拓扑");
}
int TopologyPlugin::defaultOrder() const {
    return 15;
}
bool TopologyPlugin::visible() const {
    return true;
}

QWidget* TopologyPlugin::widget() {
    return container_;
}
...
void TopologyPlugin::buildUi() {
    ...
    // Connections
    connect(layoutCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int index) {
        graph_->setLayoutMode(index == 0 ? TopologyGraphWidget::Layout::Linear : TopologyGraphWidget::Layout::Tree);
    });
