...
IoVariablePlugin::IoVariablePlugin(ServiceContainer* container, QObject* parent) : container_(container) {
    if (parent)
        setParent(parent);
    buildUi();
}
...
// ── Identity ──────────────────────────────────────────────────────────
QString IoVariablePlugin::id() const {
    return "iovariable";
}
QString IoVariablePlugin::displayName() const {
    return "I/O Variables";
}
QString IoVariablePlugin::displayNameZh() const {
    return QStringLiteral("I/O 变量");
}
QIcon IoVariablePlugin::icon() const {
    return QIcon::fromTheme("view-sort-ascending");
}
int IoVariablePlugin::defaultOrder() const {
    return 40;
}
bool IoVariablePlugin::visible() const {
    return true;
}
...
QWidget* IoVariablePlugin::widget() {
...
void IoVariablePlugin::buildUi() {
    ...
    connect(scopeFilter_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &IoVariablePlugin::scopeFilterChanged);
    connect(table_, &QTableWidget::currentCellChanged, this,
            [this](int row, int, int, int) { emit rowSelectionChanged(row); });
}

// ── Table population ──────────────────────────────────────────────────
void IoVariablePlugin::setRows(const QStringList& headers, const QList<QStringList>& rows) {
...
// ── Summary label ─────────────────────────────────────────────────────
void IoVariablePlugin::setSummary(const QString& text, const QString& severity) {
...
// ── Detail label ──────────────────────────────────────────────────────
void IoVariablePlugin::setDetail(const QString& text, const QString& severity) {
...
// ── Selection ─────────────────────────────────────────────────────────
int IoVariablePlugin::currentRow() const {
...
int IoVariablePlugin::rowCount() const {
...
// ── Accessors ─────────────────────────────────────────────────────────
QTableWidget* IoVariablePlugin::ioVariableTable() const {
    return table_;
}
QLineEdit* IoVariablePlugin::ioVariableFilter() const {
    return filter_;
}
QComboBox* IoVariablePlugin::ioVariableScopeFilter() const {
    return scopeFilter_;
}
QLabel* IoVariablePlugin::ioVariableSummaryLabel() const {
    return summaryLabel_;
}
QLabel* IoVariablePlugin::ioVariableDetailLabel() const {
    return detailLabel_;
}
