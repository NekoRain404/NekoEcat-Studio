...
QString SimulationPlugin::id() const {
    return "simulation";
}
QString SimulationPlugin::displayName() const {
    return "Simulation";
}
QString SimulationPlugin::displayNameZh() const {
    return QStringLiteral("仿真");
}
QIcon SimulationPlugin::icon() const {
    return QIcon::fromTheme("media-playback-start");
}
int SimulationPlugin::defaultOrder() const {
    return 205;
}
bool SimulationPlugin::visible() const {
    return false;
}
...
QWidget* SimulationPlugin::widget() {
    return containerWidget_;
}

SimulationPlugin::SimState SimulationPlugin::simulationState() const {
    return state_;
}
bool SimulationPlugin::isRunning() const {
    return state_ == SimState::Running;
}
bool SimulationPlugin::isPaused() const {
    return state_ == SimState::Paused;
}

void SimulationPlugin::setCycleTimeUs(int us) {
    cycleTimeSpin_->setValue(us);
}
int SimulationPlugin::cycleTimeUs() const {
    return cycleTimeSpin_->value();
}

void SimulationPlugin::setSlaveCount(int count) {
    slaveCountSpin_->setValue(count);
}
int SimulationPlugin::slaveCount() const {
    return slaveCountSpin_->value();
}

void SimulationPlugin::setSimulationDuration(int seconds) {
    durationSpin_->setValue(seconds);
}
int SimulationPlugin::simulationDuration() const {
    return durationSpin_->value();
}

int SimulationPlugin::frameCount() const {
    return frameCount_;
}
double SimulationPlugin::averageLatencyUs() const {
    return frameCount_ > 0 ? totalLatencyUs_ / frameCount_ : 0.0;
}
double SimulationPlugin::maxLatencyUs() const {
    return maxLatencyUs_;
}
double SimulationPlugin::minLatencyUs() const {
    return minLatencyUs_ < 1e9 ? minLatencyUs_ : 0.0;
}
int SimulationPlugin::errorCount() const {
    return errorCount_;
}

QTableWidget* SimulationPlugin::statisticsTable() const {
    return statisticsTable_;
}
QTableWidget* SimulationPlugin::dataViewTable() const {
    return dataViewTable_;
}
QTextEdit* SimulationPlugin::logView() const {
    return logView_;
}
...
void SimulationPlugin::buildUi() {
    ...
    QStringList statsLabels = {tr("Frames"),           tr("Avg Latency (us)"), tr("Max Latency (us)"),
                               tr("Min Latency (us)"), tr("Errors"),           tr("Elapsed (s)")};
    ...
    connect(exportBtn_, &QPushButton::clicked, this, [this]() {
        QString path = QFileDialog::getSaveFileName(containerWidget_, tr("Export Simulation Results"),
                                                    "simulation_results.csv", tr("CSV Files (*.csv)"));
        if (!path.isEmpty())
            exportResults(path);
    });
}
...
void SimulationPlugin::onSimulationTimer() {
    ...
    if (latency > maxLatencyUs_)
        maxLatencyUs_ = latency;
    if (latency < minLatencyUs_)
        minLatencyUs_ = latency;
    ...
    dataViewTable_->setItem(row, 1, new QTableWidgetItem(QString::number(elapsedMs_ / 1000.0, 'f', 2)));
    dataViewTable_->setItem(row, 2, new QTableWidgetItem(QString::number(frameCount_ % slaveCountSpin_->value())));
    dataViewTable_->setItem(row, 3, new QTableWidgetItem(frameCount_ % 3 == 0 ? "PDO" : "SDO"));
    dataViewTable_->setItem(row, 4, new QTableWidgetItem(QString("0x%1").arg(frameCount_ & 0xFFFF, 4, 16, QChar('0'))));
...
void SimulationPlugin::addLogEntry(const QString& entry) {
    logView_->append(QStringLiteral("[%1] %2").arg(QTime::currentTime().toString("HH:mm:ss.zzz")).arg(entry));
}

void SimulationPlugin::clearLog() {
    logView_->clear();
}
