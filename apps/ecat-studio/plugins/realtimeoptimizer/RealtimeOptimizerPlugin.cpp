...
RealtimeOptimizerPlugin::RealtimeOptimizerPlugin(
    RealtimeOptimizerService *service, QObject *parent)
    : service_(service) {
  if (parent) setParent(parent);
  buildUi();

  connect(service_, &RealtimeOptimizerService::optimizationCompleted,
          this, [this](const OptimizationResult &result) {
            if (result.category == "Latency") {
              latencyImprovementLabel_->setText(
                  tr("+%1%").arg(result.improvement, 0, 'f', 1));
            } else if (result.category == "Throughput") {
              throughputImprovementLabel_->setText(
                  tr("+%1%").arg(result.improvement, 0, 'f', 1));
            } else if (result.category == "Resources") {
              resourceImprovementLabel_->setText(
                  tr("-%1%").arg(result.improvement, 0, 'f', 1));
            } else if (result.category == "Priorities") {
              priorityImprovementLabel_->setText(
                  tr("+%1%").arg(result.improvement, 0, 'f', 1));
            }
            statusLabel_->setText(
                tr("Optimization completed: %1").arg(result.category));
          });

  connect(service_, &RealtimeOptimizerService::optimizationApplied,
          this, [this](const OptimizationResult &result) {
            statusLabel_->setText(
                tr("Applied: %1 optimization").arg(result.category));
          });
}

QString RealtimeOptimizerPlugin::id() const { return "realtimeoptimizer"; }
...
QWidget *RealtimeOptimizerPlugin::widget() { return containerWidget_; }
...
void RealtimeOptimizerPlugin::buildUi() {
  ...
  connect(exportBtn_, &QPushButton::clicked, this,
          &RealtimeOptimizerPlugin::exportReport);
}
...
void RealtimeOptimizerPlugin::buildLatencyTab() {
  ...
  connect(latencyOptimizer_, &LatencyOptimizerWidget::optimizeRequested,
          this, [this]() {
...
void RealtimeOptimizerPlugin::buildThroughputTab() {
  ...
  connect(throughputOptimizer_, &ThroughputOptimizerWidget::optimizeRequested,
          this, [this]() {
...
void RealtimeOptimizerPlugin::exportReport() {
  QString path = QFileDialog::getSaveFileName(
      containerWidget_, tr("Export Optimization Report"),
