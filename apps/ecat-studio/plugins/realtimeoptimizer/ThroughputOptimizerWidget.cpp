...
ThroughputOptimizerWidget::ThroughputOptimizerWidget(QWidget* parent) : QWidget(parent) {
    buildUi();
}
...
void ThroughputOptimizerWidget::buildUi() {
    ...
    currentThroughputLabel_->setStyleSheet("color: #cccccc; font-weight: bold; font-size: 24px;");
    ...
    optimizedThroughputLabel_->setStyleSheet("color: #22c55e; font-size: 18px;");
    ...
    improvementLabel_->setStyleSheet("color: #60a5fa; font-weight: bold; font-size: 16px;");
    ...
    bottleneckEdit_->setStyleSheet("background-color: #1e1e2e; color: #cccccc;");
    bottleneckEdit_->setText(tr("• PDO mapping size below optimal\n"
    ...
    recommendationsEdit_->setStyleSheet("background-color: #1e1e2e; color: #cccccc;");
    ...
    connect(optimizeBtn_, &QPushButton::clicked, this, &ThroughputOptimizerWidget::optimizeRequested);
...
void ThroughputOptimizerWidget::updateResult(const OptimizationResult& result) {
    lastResult_ = result;

    currentThroughputLabel_->setText(tr("%1 f/s").arg(result.before, 0, 'f', 0));
    optimizedThroughputLabel_->setText(tr("%1 f/s").arg(result.after, 0, 'f', 0));
