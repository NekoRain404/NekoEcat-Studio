...
WorkflowDashboardPlugin::WorkflowDashboardPlugin(
    WorkflowMonitoringService *monitoring, QObject *parent)
...
QString WorkflowDashboardPlugin::id() const { return "workflowdashboard"; }
QString WorkflowDashboardPlugin::displayName() const { return "Workflow Dashboard"; }
...
QWidget *WorkflowDashboardPlugin::widget() { return containerWidget_; }
...
void WorkflowDashboardPlugin::buildUi() {
  ...
  connect(exportButton_, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getSaveFileName(containerWidget_, tr("Export Dashboard"), QString(), "JSON (*.json)");
    if (!path.isEmpty()) exportDashboard(path);
  });
...
void WorkflowDashboardPlugin::addActiveWorkflow(const QString &workflowId,
                                                  const QString &name,
...
void WorkflowDashboardPlugin::updateWorkflowStatus(const QString &workflowId,
                                                     const QString &status) {
...
int WorkflowDashboardPlugin::activeWorkflowCount() const { return activeWorkflows_.size(); }

void WorkflowDashboardPlugin::addAlert(const QString &severity,
...
int WorkflowDashboardPlugin::alertCount() const { return alerts_.size(); }

void WorkflowDashboardPlugin::addNotification(const QString &channel,
                                                const QString &message) {
  Notification notif;
...
int WorkflowDashboardPlugin::notificationCount() const { return notifications_.size(); }

void WorkflowDashboardPlugin::refreshActiveWorkflows() {
  activeWorkflowsList_->clear();
  for (const auto &wf : activeWorkflows_) {
    activeWorkflowsList_->addItem(QStringLiteral("[%1] %2 — %3")
        .arg(wf.id, wf.name, wf.status));
  }
}
