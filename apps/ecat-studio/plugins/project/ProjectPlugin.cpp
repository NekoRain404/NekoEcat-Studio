...
ProjectPlugin::ProjectPlugin(ProjectManagerService* projectService, ConfigurationService* configService,
                             QObject* parent)
    : WorkspacePlugin(), projectService_(projectService), configService_(configService) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(projectService_, &ProjectManagerService::projectOpened, this, [this](const QString& name) {
        projectLabel_->setText(name);
        refreshProjectTree();
    });
    connect(projectService_, &ProjectManagerService::projectSaved, this,
            [this](const QString& name) { projectLabel_->setText(name); });
    connect(projectService_, &ProjectManagerService::projectClosed, this, [this]() {
...
void ProjectPlugin::buildUi() {
    ...
    connect(tree_, &QTreeWidget::itemSelectionChanged, this, &ProjectPlugin::onTreeSelectionChanged);
}
...
void ProjectPlugin::refreshProjectTree() {
    ...
    for (const auto& sc : configService_->slaveConfigs()) {
        auto* item = new QTreeWidgetItem(slaves, {QString("%1: %2").arg(sc.position).arg(sc.name)});
    }
...
void ProjectPlugin::onNewProject() {
    projectService_->createProject(projectNameEdit_->text().isEmpty() ? "Untitled" : projectNameEdit_->text());
}

void ProjectPlugin::onOpenProject() {
    QString path = QFileDialog::getOpenFileName(container_, "Open Project", QString(),
...
void ProjectPlugin::onSaveProjectAs() {
    QString path = QFileDialog::getSaveFileName(container_, "Save Project As", QString(),
                                                "Ecat Project (*.ecat.json);;All Files (*)");
    if (!path.isEmpty())
...
void ProjectPlugin::onExportProject() {
    QString path = QFileDialog::getSaveFileName(container_, "Export Project", QString(),
                                                "Ecat Project (*.ecat.json);;All Files (*)");
    if (!path.isEmpty())
...
void ProjectPlugin::onImportProject() {
    QString path = QFileDialog::getOpenFileName(container_, "Import Project", QString(),
                                                "Ecat Project (*.ecat.json);;All Files (*)");
    if (!path.isEmpty())
