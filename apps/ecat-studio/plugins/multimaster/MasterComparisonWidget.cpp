...
MasterComparisonWidget::MasterComparisonWidget(QWidget* parent) : QWidget(parent) {
    buildUi();
}
...
void MasterComparisonWidget::buildUi() {
    ...
    diffTable_->setHorizontalHeaderLabels({tr("Property"), tr("Left Value"), tr("Right Value"), tr("Status")});
...
void MasterComparisonWidget::setLeftMaster(const MmMasterInfo& info, const MmMasterStatus& status) {
...
void MasterComparisonWidget::setRightMaster(const MmMasterInfo& info, const MmMasterStatus& status) {
...
void MasterComparisonWidget::updateDiffHighlight() {
    if (leftTable_->rowCount() == 0 || rightTable_->rowCount() == 0)
        return;
