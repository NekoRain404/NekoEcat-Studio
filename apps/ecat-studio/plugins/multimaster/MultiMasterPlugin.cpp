...
MultiMasterPlugin::MultiMasterPlugin(MultiMasterService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
    ...
    connect(service_, &MultiMasterService::masterStatusChanged, this, [this](int masterId, const MmMasterStatus&) {
        if (masterId == selectedMasterId_) {
            updateMasterDetails(masterId);
        }
    });

    connect(service_, &MultiMasterService::masterSyncCompleted, this, [this](const MmMasterSyncResult& result) {
        syncLog_->appendPlainText(QDateTime::currentDateTime().toString("hh:mm:ss") + " " + result.message);
    });

    connect(service_, &MultiMasterService::masterError, this, [this](int masterId, const QString& error) {
        syncLog_->appendPlainText(QDateTime::currentDateTime().toString("hh:mm:ss") +
                                  QString(" Error on master %1: %2").arg(masterId).arg(error));
    });
}

QString MultiMasterPlugin::id() const {
    return "multimaster";
}
QString MultiMasterPlugin::displayName() const {
    return "Multi-Master";
}
...
int MultiMasterPlugin::defaultOrder() const {
    return 30;
}
bool MultiMasterPlugin::visible() const {
    return false;
}

void MultiMasterPlugin::activate() {
    refreshMasterList();
}
void MultiMasterPlugin::deactivate() {}

QWidget* MultiMasterPlugin::widget() {
    return containerWidget_;
}

int MultiMasterPlugin::masterCount() const {
    return service_->masterCount();
}

int MultiMasterPlugin::selectedMasterId() const {
    return selectedMasterId_;
}
...
void MultiMasterPlugin::buildUi() {
    ...
    connect(refreshBtn_, &QPushButton::clicked, this, &MultiMasterPlugin::refreshMasterList);
    ...
    connect(exportBtn_, &QPushButton::clicked, this, &MultiMasterPlugin::exportReport);

    connect(masterListTable_, &QTableWidget::cellClicked, this, [this](int row, int col) {
        Q_UNUSED(col);
...
void MultiMasterPlugin::refreshMasterList() {
    ...
    auto stateToString = [](MultiMasterState s) -> QString {
        switch (s) {
            case MultiMasterState::Idle:
                return "Idle";
            case MultiMasterState::Active:
                return "Active";
            case MultiMasterState::Error:
                return "Error";
            case MultiMasterState::Syncing:
                return "Syncing";
            default:
                return "Unknown";
        }
...
void MultiMasterPlugin::exportReport() {
    QString path = QFileDialog::getSaveFileName(containerWidget_, tr("Export Master Report"), "master_report.csv",
...
bool MultiMasterPlugin::exportReportToFile(const QString& path) {
    ...
    for (const auto& m : masters) {
        out << m.masterId << "," << m.adapterName << "," << m.slaveCount << "," << static_cast<int>(m.state) << ","
