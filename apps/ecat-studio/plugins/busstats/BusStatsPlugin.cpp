...
BusStatsPlugin::BusStatsPlugin(BusStatsService *service, QObject *parent)
    : service_(service) {
  if (parent) setParent(parent);
  buildUi();

  connect(service_, &BusStatsService::statsUpdated, this,
          [this](const QJsonObject &) { updateDisplay(); });
}

QString BusStatsPlugin::id() const { return "busstats"; }
QString BusStatsPlugin::displayName() const { return "Bus Statistics"; }
...
int BusStatsPlugin::defaultOrder() const { return 95; }
bool BusStatsPlugin::visible() const { return true; }

void BusStatsPlugin::activate() {}
void BusStatsPlugin::deactivate() {}

QWidget *BusStatsPlugin::widget() { return containerWidget_; }
...
void BusStatsPlugin::updateDisplay() {
  ...
  frameRateLabel_->setText(
      tr("Frame Rate: %1 f/s").arg(stats.frameRate, 0, 'f', 1));
  bandwidthLabel_->setText(
      tr("Bandwidth: %1 Mbps").arg(stats.bandwidthMbps, 0, 'f', 2));
}
