...
SdoCachePlugin::SdoCachePlugin(SdoCacheService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(service_, &SdoCacheService::cacheUpdated, this, [this](int) {
        updateStats();
        updateEntries();
    });
    connect(service_, &SdoCacheService::cacheInvalidated, this, [this](int) {
        updateStats();
        updateEntries();
    });
}

QString SdoCachePlugin::id() const {
    return "sdocache";
}
QString SdoCachePlugin::displayName() const {
    return "SDO Cache";
}
...
int SdoCachePlugin::defaultOrder() const {
    return 160;
}
bool SdoCachePlugin::visible() const {
    return false;
}

void SdoCachePlugin::activate() {
    updateStats();
    updateEntries();
}
void SdoCachePlugin::deactivate() {}

QWidget* SdoCachePlugin::widget() {
...
void SdoCachePlugin::buildUi() {
    ...
    connect(maxSizeSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this, [this](int val) {
        auto cfg = service_->config();
        cfg.maxEntriesPerSlave = val;
        service_->setConfig(cfg);
    });
    connect(ttlSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this, [this](int val) {
        auto cfg = service_->config();
        cfg.ttlMs = val;
        service_->setConfig(cfg);
    });
    connect(evictionCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this, [this](int idx) {
        auto cfg = service_->config();
    ...
    entriesTable_->setHorizontalHeaderLabels({tr("Position"), tr("Index:SubIndex"), tr("Value"), tr("Accesses")});
    ...
    connect(clearBtn_, &QPushButton::clicked, this, [this]() { service_->invalidateAll(); });
    connect(exportBtn_, &QPushButton::clicked, this, [this]() {
        QString path =
            QFileDialog::getSaveFileName(containerWidget_, tr("Export Cache"), QString(), tr("JSON Files (*.json)"));
        if (!path.isEmpty()) {
    ...
    connect(importBtn_, &QPushButton::clicked, this, [this]() {
        QString path =
            QFileDialog::getOpenFileName(containerWidget_, tr("Import Cache"), QString(), tr("JSON Files (*.json)"));
        if (!path.isEmpty()) {
            // Import cache data
        }
    });
