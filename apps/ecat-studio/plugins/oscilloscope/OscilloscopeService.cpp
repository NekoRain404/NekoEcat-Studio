...
OscilloscopeService::OscilloscopeService(QObject* parent) : QObject(parent) {}

int OscilloscopeService::addChannel(int slave, const QString& index, const QString& subIndex) {
    ...
    ch.name = QStringLiteral("CH%1 [%2:%3.%4]").arg(ch.id).arg(slave).arg(index, subIndex);
    channels_.append(ch);
...
int OscilloscopeService::timebase() const {
    return timebaseMs_;
}
...
bool OscilloscopeService::isAcquiring() const {
    return acquiring_;
}
