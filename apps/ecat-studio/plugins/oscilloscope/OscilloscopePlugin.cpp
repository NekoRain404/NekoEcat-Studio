...
OscilloscopePlugin::OscilloscopePlugin(OscilloscopeService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(service_, &OscilloscopeService::waveformUpdated, this, &OscilloscopePlugin::refreshWaveforms);
    connect(service_, &OscilloscopeService::channelAdded, this, [this](int) { refreshWaveforms(); });
    connect(service_, &OscilloscopeService::channelRemoved, this, [this](int) { refreshWaveforms(); });
}

QString OscilloscopePlugin::id() const {
    return "oscilloscope";
}
QString OscilloscopePlugin::displayName() const {
    return "Oscilloscope";
}
QString OscilloscopePlugin::displayNameZh() const {
    return QStringLiteral("示波器");
}
int OscilloscopePlugin::defaultOrder() const {
    return 100;
}
bool OscilloscopePlugin::visible() const {
    return true;
}

QWidget* OscilloscopePlugin::widget() {
    return container_;
}
...
void OscilloscopePlugin::buildUi() {
    ...
    timebaseCombo_->addItems(
        {"1 ms", "2 ms", "5 ms", "10 ms", "20 ms", "50 ms", "100 ms", "500 ms", "1 s", "5 s", "10 s"});
    ...
    // ── Connections ──
    connect(addBtn, &QPushButton::clicked, this, &OscilloscopePlugin::showAddChannelDialog);
    connect(removeBtn, &QPushButton::clicked, this, &OscilloscopePlugin::removeSelectedChannel);
    connect(runStopBtn_, &QPushButton::clicked, this, &OscilloscopePlugin::toggleAcquisition);
    connect(timebaseCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &OscilloscopePlugin::onTimebaseChanged);
    connect(triggerModeCombo_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            &OscilloscopePlugin::onTriggerModeChanged);
    connect(triggerLevelSpin_, QOverload<int>::of(&QSpinBox::valueChanged), this,
            [this](int val) { service_->setTriggerLevel(val); });
...
void OscilloscopePlugin::showAddChannelDialog() {
    ...
    if (dlg.exec() == QDialog::Accepted) {
        const int id = service_->addChannel(slaveSpin->value(), idxEdit->text().trimmed(), subEdit->text().trimmed());
        if (id > 0) {
