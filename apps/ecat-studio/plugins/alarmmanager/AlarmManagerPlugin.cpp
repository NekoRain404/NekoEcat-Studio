...
QString AlarmManagerPlugin::id() const {
    return "alarmmanager";
}
QString AlarmManagerPlugin::displayName() const {
    return "Alarm Manager";
}
QString AlarmManagerPlugin::displayNameZh() const {
    return "告警管理器";
}
int AlarmManagerPlugin::defaultOrder() const {
    return 240;
}
bool AlarmManagerPlugin::visible() const {
    return false;
}
...
int AlarmManagerPlugin::ruleCount() const {
    return rules_.size();
}
...
int AlarmManagerPlugin::recordCount() const {
...
void AlarmManagerPlugin::filterRecords(const QString& query) {
    if (query.isEmpty()) {
        ...
        for (const auto& r : records_) {
            if (r.message.contains(query, Qt::CaseInsensitive) || r.severity.contains(query, Qt::CaseInsensitive) ||
    ...
    rebuildHistoryTable();
    if (statusLabel_)
        statusLabel_->setText(tr("%1 records").arg(filteredRecords_.size()));
}
...
int AlarmManagerPlugin::channelCount() const {
...
int AlarmManagerPlugin::escalationPolicyCount() const {
...
QTabWidget* AlarmManagerPlugin::tabs() const {
    return tabs_;
}
QTableWidget* AlarmManagerPlugin::rulesTable() const {
    return rulesTable_;
}
QTableWidget* AlarmManagerPlugin::historyTable() const {
    return historyTable_;
}
QTableWidget* AlarmManagerPlugin::channelsTable() const {
    return channelsTable_;
}
QTableWidget* AlarmManagerPlugin::escalationTable() const {
    return escalationTable_;
}
QLabel* AlarmManagerPlugin::statusLabel() const {
    return statusLabel_;
}
...
void AlarmManagerPlugin::buildUi() {
    ...
    connect(filterEdit_, &QLineEdit::textChanged, this, [this](const QString& text) { filterRecords(text); });
    connect(ackBtn_, &QPushButton::clicked, this, [this]() {
        int row = historyTable_->currentRow();
        if (row >= 0)
            acknowledgeRecord(row);
    });
    connect(addChannelBtn_, &QPushButton::clicked, this,
            [this]() { addChannel("channel_" + QString::number(channels_.size())); });
