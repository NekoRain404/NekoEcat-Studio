...
RealtimePerformancePlugin::RealtimePerformancePlugin(
    RealtimePerformanceService *service, QObject *parent)
    : service_(service) {
  if (parent) setParent(parent);
  buildUi();

  connect(service_, &RealtimePerformanceService::latencyUpdated,
          this, [this](const LatencyMetrics &m) {
            latencyMonitor_->updateMetrics(m);
            latencyAvgLabel_->setText(
                tr("Avg: %1 us").arg(m.avgUs, 0, 'f', 1));
          });

  connect(service_, &RealtimePerformanceService::throughputUpdated,
          this, [this](const ThroughputMetrics &m) {
            throughputMonitor_->updateMetrics(m);
            throughputLabel_->setText(
                tr("%1 f/s").arg(m.framesPerSecond, 0, 'f', 0));
          });

  connect(service_, &RealtimePerformanceService::resourceUpdated,
          this, [this](const ResourceMetrics &m) {
            ...
            filesLabel_->setText(
                tr("Open Files: %1%").arg(m.openFilesPercent, 0, 'f', 0));
          });

  connect(service_, &RealtimePerformanceService::qualityUpdated,
          this, [this](const QualityAssessment &q) {
            qualityScoreLabel_->setText(
                tr("Score: %1").arg(q.score, 0, 'f', 0));
            qualityLabel_->setText(tr("Quality: %1").arg(q.grade));
            QString color = q.grade == "Excellent"   ? "#22c55e"
                            : q.grade == "Good"      ? "#60a5fa"
                            : q.grade == "Fair"      ? "#f59e0b"
                                                     : "#ef4444";
            qualityLabel_->setStyleSheet(
                QStringLiteral("color: %1; font-weight: bold;").arg(color));
          });
}

QString RealtimePerformancePlugin::id() const { return "realtimeperf"; }
...
QWidget *RealtimePerformancePlugin::widget() { return containerWidget_; }
...
void RealtimePerformancePlugin::buildUi() {
  ...
  connect(exportBtn_, &QPushButton::clicked, this,
          &RealtimePerformancePlugin::exportReport);
}
...
void RealtimePerformancePlugin::buildDashboardTab() {
  ...
  auto makeCard = [&](const QString &name, int row, int col) -> QLabel * {
    ...
    auto *val = new QLabel("--");
    val->setStyleSheet(
        "color: #cccccc; font-weight: bold; font-size: 18px;");
...
void RealtimePerformancePlugin::exportReport() {
  QString path = QFileDialog::getSaveFileName(
      containerWidget_, tr("Export Performance Report"),
