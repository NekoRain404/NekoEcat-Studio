...
LatencyMonitorWidget::LatencyMonitorWidget(QWidget *parent)
    : QWidget(parent) {
...
void LatencyMonitorWidget::drawLatencyCurve(QPainter &p, const QRect &area) {
  ...
  for (int i = 0; i < samples_.size(); ++i) {
    double x = area.left() +
               (static_cast<double>(i) / (samples_.size() - 1)) * area.width();
    double y = yPos(samples_[i]);
    curve << QPointF(x, qBound(static_cast<qreal>(area.top()),
                                static_cast<qreal>(y),
