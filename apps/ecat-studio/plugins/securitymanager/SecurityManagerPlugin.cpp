...
SecurityManagerPlugin::SecurityManagerPlugin(QObject *parent) {
  ...
  roles_ = {
      {"r1", "Administrator", "Full system access",
       {"read", "write", "delete", "configure", "audit"}},
      {"r2", "Engineer", "Engineering access",
  ...
  auditLog_ = {
      ...
      {now, "engineer", "sdo_write", "slave_01",
       "Wrote SDO 0x1600:01", "info"},
  };
  filteredAudit_ = {0, 1};
  buildUi();
}

QString SecurityManagerPlugin::id() const { return "securitymanager"; }
...
int SecurityManagerPlugin::userCount() const { return users_.size(); }
...
int SecurityManagerPlugin::roleCount() const { return roles_.size(); }
...
int SecurityManagerPlugin::permissionCount() const { return permissions_.size(); }
...
int SecurityManagerPlugin::auditCount() const { return auditLog_.size(); }

void SecurityManagerPlugin::filterAudit(const QString &user,
  ...
  for (int i = 0; i < auditLog_.size(); ++i) {
    ...
    bool matchSev = severity.isEmpty() || severity == "All" ||
                    e.severity == severity;
    if (matchUser && matchSev) filteredAudit_.append(i);
  }
  rebuildAuditTable();
}
...
QTableWidget *SecurityManagerPlugin::roleTable() const { return roleTable_; }
QTableWidget *SecurityManagerPlugin::permissionTable() const {
  return permissionTable_;
}
QTableWidget *SecurityManagerPlugin::auditTable() const { return auditTable_; }
QLabel *SecurityManagerPlugin::statusLabel() const { return statusLabel_; }
...
void SecurityManagerPlugin::buildUi() {
  ...
  userTable_->setHorizontalHeaderLabels(
      {"ID", "Username", "Role", "Email", "Active", "Last Login"});
  ...
  roleTable_->setHorizontalHeaderLabels(
  ...
  auditTable_->setHorizontalHeaderLabels(
  ...
  connect(removePermBtn_, &QPushButton::clicked, this, [this]() {
    ...
    if (row >= 0) removePermission(row);
  });
  connect(filterAuditBtn_, &QPushButton::clicked, this, [this]() {
    filterAudit(auditUserFilter_->text(), auditSeverityFilter_->currentText());
  });
  connect(exportReportBtn_, &QPushButton::clicked, this, [this]() {
    exportSecurityReport("/tmp/security_report.txt");
  });
}
...
void SecurityManagerPlugin::rebuildUserTable() {
  ...
  for (int i = 0; i < users_.size(); ++i) {
    ...
    userTable_->setItem(i, 4,
                        new QTableWidgetItem(u.active ? "Yes" : "No"));
    userTable_->setItem(
        i, 5, new QTableWidgetItem(u.lastLogin.toString(Qt::ISODate)));
  }
}

void SecurityManagerPlugin::rebuildRoleTable() {
  if (!roleTable_) return;
  ...
  for (int i = 0; i < roles_.size(); ++i) {
    ...
    roleTable_->setItem(i, 3,
                        new QTableWidgetItem(r.permissions.join(", ")));
  }
}

void SecurityManagerPlugin::rebuildPermissionTable() {
  if (!permissionTable_) return;
...
void SecurityManagerPlugin::rebuildAuditTable() {
  ...
  if (statusLabel_)
    statusLabel_->setText(
        QString("Audit: %1 entries shown").arg(filteredAudit_.size()));
}
