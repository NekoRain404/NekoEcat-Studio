...
QString WorkflowDesignerPlugin::id() const {
    return "workflowdesigner";
}
QString WorkflowDesignerPlugin::displayName() const {
    return "Workflow Designer";
}
QString WorkflowDesignerPlugin::displayNameZh() const {
    return QStringLiteral("工作流设计器");
}
QIcon WorkflowDesignerPlugin::icon() const {
    return QIcon::fromTheme("system-run");
}
int WorkflowDesignerPlugin::defaultOrder() const {
    return 250;
}
bool WorkflowDesignerPlugin::visible() const {
    return false;
}
...
QWidget* WorkflowDesignerPlugin::widget() {
    return containerWidget_;
}
QWidget* WorkflowDesignerPlugin::canvas() const {
    return canvas_;
}
QTreeWidget* WorkflowDesignerPlugin::nodePalette() const {
    return nodePalette_;
}
QTextEdit* WorkflowDesignerPlugin::propertyEditor() const {
    return propertyEditor_;
}
QTableWidget* WorkflowDesignerPlugin::executionMonitor() const {
    return executionMonitor_;
}
...
void WorkflowDesignerPlugin::buildUi() {
    ...
    connect(importButton_, &QPushButton::clicked, this, [this]() {
        QString path =
            QFileDialog::getOpenFileName(containerWidget_, tr("Import Workflow"), QString(), "JSON (*.json)");
        if (!path.isEmpty())
            importWorkflow(path);
    });
}
...
int WorkflowDesignerPlugin::nodeCount() const {
    return nodes_.size();
}
...
int WorkflowDesignerPlugin::connectionCount() const {
...
QString WorkflowDesignerPlugin::executionStatus() const {
...
bool WorkflowDesignerPlugin::importWorkflow(const QString& filePath) {
    ...
    for (const auto& c : root["connections"].toArray()) {
        ...
        if (conn.fromNodeId.isEmpty() || conn.toNodeId.isEmpty())
            return false;
