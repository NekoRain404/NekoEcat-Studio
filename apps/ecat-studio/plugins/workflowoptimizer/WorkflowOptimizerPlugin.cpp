...
WorkflowOptimizerPlugin::WorkflowOptimizerPlugin(WorkflowAnalyticsService* analytics, QObject* parent)
...
QString WorkflowOptimizerPlugin::id() const {
    return "workflowoptimizer";
}
QString WorkflowOptimizerPlugin::displayName() const {
    return "Workflow Optimizer";
}
QString WorkflowOptimizerPlugin::displayNameZh() const {
    return QStringLiteral("工作流优化器");
}
QIcon WorkflowOptimizerPlugin::icon() const {
    return QIcon::fromTheme("system-run");
}
int WorkflowOptimizerPlugin::defaultOrder() const {
    return 385;
}
bool WorkflowOptimizerPlugin::visible() const {
    return false;
}
...
QWidget* WorkflowOptimizerPlugin::widget() {
...
void WorkflowOptimizerPlugin::buildUi() {
    ...
    metricsTable_->setHorizontalHeaderLabels(
        {tr("Workflow"), tr("Success Rate"), tr("Avg Duration"), tr("Throughput")});
    ...
    connect(optimizeButton_, &QPushButton::clicked, this, [this]() {
        ...
        if (!wfId.isEmpty())
            emit optimizationRequested(wfId);
    });
...
int WorkflowOptimizerPlugin::workflowCount() const {
    return workflows_.size();
}

void WorkflowOptimizerPlugin::addSuggestion(const QString& workflowId, const QString& priority,
...
int WorkflowOptimizerPlugin::suggestionCount() const {
    return suggestions_.size();
}

void WorkflowOptimizerPlugin::addExecutionRecord(const QString& workflowId, const QString& status, double durationMs) {
    HistoryEntry entry;
    entry.workflowId = workflowId;
    entry.status = status;
...
int WorkflowOptimizerPlugin::executionHistoryCount() const {
...
void WorkflowOptimizerPlugin::refreshMetrics() {
    ...
    for (const auto& wf : workflows_) {
        ...
        metricsTable_->setItem(row, 1, new QTableWidgetItem(QString::number(analysis.successRate, 'f', 1) + "%"));
        metricsTable_->setItem(row, 2,
                               new QTableWidgetItem(QString::number(analysis.averageDurationMs, 'f', 1) + " ms"));
        metricsTable_->setItem(row, 3, new QTableWidgetItem(QString::number(analysis.totalExecutions)));
    }
}
