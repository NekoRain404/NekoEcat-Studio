...
QString FormulaPlugin::id() const { return "formula"; }
QString FormulaPlugin::displayName() const { return "Formula Editor"; }
...
QWidget *FormulaPlugin::widget() { return containerWidget_; }
QTextEdit *FormulaPlugin::formulaEditor() const { return formulaEditor_; }
QTableWidget *FormulaPlugin::variableTable() const { return variableTable_; }
QLabel *FormulaPlugin::resultLabel() const { return resultLabel_; }
QListWidget *FormulaPlugin::historyList() const { return historyList_; }
...
void FormulaPlugin::buildUi() {
  ...
  connect(removeVarBtn_, &QPushButton::clicked, this, [this]() {
    ...
    if (row >= 0) variableTable_->removeRow(row);
  });
  connect(clearHistoryBtn, &QPushButton::clicked, this, &FormulaPlugin::clearHistory);
  connect(formulaEditor_, &QTextEdit::textChanged, this, [this]() {
    emit formulaChanged(formulaEditor_->toPlainText());
  });
...
bool FormulaPlugin::validateFormula(const QString &formula) const {
  if (formula.trimmed().isEmpty()) return false;
  int parenDepth = 0;
