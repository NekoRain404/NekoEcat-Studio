...
DriftMonitorWidget::DriftMonitorWidget(QWidget *parent)
    : QWidget(parent) {
...
void DriftMonitorWidget::drawGrid(QPainter &p, const QRect &area) {
  ...
  if (!samples_.isEmpty()) {
    for (double s : samples_) {
      if (qAbs(s) > maxVal) maxVal = qAbs(s) * 1.2;
    }
  }
...
void DriftMonitorWidget::drawDriftCurve(QPainter &p, const QRect &area) {
  ...
  for (int i = 0; i < samples_.size(); ++i) {
    double x =
        area.left() + (static_cast<double>(i) / (samples_.size() - 1)) * area.width();
    double y = yPos(samples_[i]);
    curve << QPointF(x, qBound(static_cast<qreal>(area.top()), static_cast<qreal>(y),
                                static_cast<qreal>(area.bottom())));
  }
