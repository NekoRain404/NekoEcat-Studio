...
JitterAnalysisWidget::JitterAnalysisWidget(QWidget *parent)
    : QWidget(parent) {
  setMinimumSize(300, 200);
  setSizePolicy(QSizePolicy::Expanding, QSizePolicy::Expanding);
}

void JitterAnalysisWidget::setJitterData(double min, double max, double avg,
                                         double stddev, int sampleCount) {
  min_ = min;
  max_ = max;
  avg_ = avg;
  stddev_ = stddev;
...
void JitterAnalysisWidget::setHistogram(const QVector<int> &bins,
                                        double binWidth, double minVal) {
  histogram_ = bins;
...
void JitterAnalysisWidget::paintEvent(QPaintEvent *) {
  ...
  QRect trendRect(histRect.right() + margin, 80,
                  (width() - 3 * margin) / 2, halfH - 80);
...
void JitterAnalysisWidget::drawHistogram(QPainter &p, const QRect &area) {
  ...
  for (int i = 0; i < barCount; ++i) {
    ...
    double barH =
        (static_cast<double>(histogram_[i]) / maxCount) * area.height() * 0.9;
    int x = area.left() + static_cast<int>(i * barW);
    int w = static_cast<int>(barW) - 1;
    if (w < 1) w = 1;
    QRect barRect(x, area.bottom() - static_cast<int>(barH), w,
  ...
  p.setPen(QColor("#666688"));
  p.drawText(area.bottomLeft() + QPoint(4, 12),
             QString::number(histMin_, 'f', 0) + " ns");
  p.drawText(area.bottomRight() + QPoint(-40, 12),
             QString::number(histMin_ + histBinWidth_ * barCount, 'f', 0) +
                 " ns");
}
...
void JitterAnalysisWidget::drawTrendLine(QPainter &p, const QRect &area) {
  ...
  for (int i = 0; i < trendHistory_.size(); ++i) {
    double x = area.left() +
               (static_cast<double>(i) / (trendHistory_.size() - 1)) *
                   area.width();
