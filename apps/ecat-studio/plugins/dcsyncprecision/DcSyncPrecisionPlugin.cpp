...
DcSyncPrecisionPlugin::DcSyncPrecisionPlugin(EcatClient *client,
                                             EventBus *bus,
                                             QObject *parent) {
  ...
  connect(service_, &DcSyncPrecisionService::driftUpdated,
          this, &DcSyncPrecisionPlugin::handleDriftUpdated);
  connect(service_, &DcSyncPrecisionService::jitterUpdated,
          this, &DcSyncPrecisionPlugin::handleJitterUpdated);
  connect(service_, &DcSyncPrecisionService::syncQualityChanged,
          this, &DcSyncPrecisionPlugin::handleSyncQualityChanged);
  connect(service_, &DcSyncPrecisionService::monitoringStateChanged,
          this, &DcSyncPrecisionPlugin::handleMonitoringStateChanged);
}

QString DcSyncPrecisionPlugin::id() const { return "dcsyncprecision"; }
...
int DcSyncPrecisionPlugin::defaultOrder() const { return 26; }
bool DcSyncPrecisionPlugin::visible() const { return true; }
QWidget *DcSyncPrecisionPlugin::widget() { return containerWidget_; }
...
void DcSyncPrecisionPlugin::buildUi() {
  ...
  connect(startStopBtn_, &QPushButton::clicked,
          this, &DcSyncPrecisionPlugin::handleStartStop);
  connect(exportBtn_, &QPushButton::clicked,
          this, &DcSyncPrecisionPlugin::handleExport);
}
...
QWidget *DcSyncPrecisionPlugin::buildSyncStatusTab() {
  ...
  syncTable_->setHorizontalHeaderLabels({
    tr("Slave"), tr("Drift (ns)"), tr("Threshold (ns)"),
    tr("Status"), tr("Jitter Avg"), tr("Last Update")
  });
...
QWidget *DcSyncPrecisionPlugin::buildConfigTab() {
  ...
  connect(thresholdSpin_, QOverload<double>::of(&QDoubleSpinBox::valueChanged),
          this, &DcSyncPrecisionPlugin::handleThresholdChanged);
  connect(historyWindowSpin_, QOverload<int>::of(&QSpinBox::valueChanged),
          this, &DcSyncPrecisionPlugin::handleHistoryWindowChanged);
...
void DcSyncPrecisionPlugin::handleMonitoringStateChanged(bool active) {
  startStopBtn_->setText(active ? tr("Stop Monitoring")
                                : tr("Start Monitoring"));
...
void DcSyncPrecisionPlugin::handleJitterUpdated(const JitterStatsEx &stats) {
  jitterAnalysis_->setJitterData(stats.min, stats.max, stats.avg,
                                 stats.stddev, stats.sampleCount);
  jitterAnalysis_->setHistogram(stats.histogram, stats.histogramBinWidth,
                                stats.min);
}
...
void DcSyncPrecisionPlugin::updateSyncStatusTable() {
  ...
  refClockLabel_->setText(
      ref >= 0 ? tr("Reference Clock: Slave %1").arg(ref)
               : tr("Reference Clock: N/A"));

  for (int i = 0; i < drifts.size(); ++i) {
    const auto &ds = drifts[i];
    syncTable_->setItem(i, 0,
                        new QTableWidgetItem(QString::number(ds.slave)));
    syncTable_->setItem(
        i, 1, new QTableWidgetItem(QString::number(ds.drift, 'f', 1)));
    syncTable_->setItem(
        i, 2, new QTableWidgetItem(QString::number(ds.threshold, 'f', 0)));
...
void DcSyncPrecisionPlugin::handleExport() {
  const QString path = QFileDialog::getSaveFileName(
      containerWidget_, tr("Export DC Sync Precision Report"), QString(),
