...
QString DataLoggerPlugin::id() const {
    return "datalogger";
}
QString DataLoggerPlugin::displayName() const {
    return "Data Logger";
}
QString DataLoggerPlugin::displayNameZh() const {
    return "数据记录器";
}
int DataLoggerPlugin::defaultOrder() const {
    return 245;
}
bool DataLoggerPlugin::visible() const {
    return false;
}
...
int DataLoggerPlugin::filterCount() const {
...
void DataLoggerPlugin::addLogEntry(const LogEntry& entry) {
    ...
    if (statusLabel_)
        statusLabel_->setText(tr("%1 entries").arg(logEntries_.size()));
}

int DataLoggerPlugin::logEntryCount() const {
...
int DataLoggerPlugin::logFileCount() const {
...
void DataLoggerPlugin::setMaxFileSize(qint64 bytes) {
    maxFileSize_ = bytes;
    if (maxSizeEdit_)
        maxSizeEdit_->setText(QString::number(bytes));
}

qint64 DataLoggerPlugin::maxFileSize() const {
    return maxFileSize_;
}
...
void DataLoggerPlugin::setMaxFileCount(int count) {
    maxFileCount_ = count;
    if (maxFilesEdit_)
        maxFilesEdit_->setText(QString::number(count));
}

int DataLoggerPlugin::maxFileCount() const {
...
QTabWidget* DataLoggerPlugin::tabs() const {
    return tabs_;
}
QTableWidget* DataLoggerPlugin::filtersTable() const {
    return filtersTable_;
}
QTableWidget* DataLoggerPlugin::logFilesTable() const {
    return logFilesTable_;
}
QTextEdit* DataLoggerPlugin::logViewer() const {
    return logViewer_;
}
QTableWidget* DataLoggerPlugin::statisticsTable() const {
    return statisticsTable_;
}
QLabel* DataLoggerPlugin::statusLabel() const {
    return statusLabel_;
}
...
void DataLoggerPlugin::buildUi() {
    ...
    connect(refreshStatsBtn_, &QPushButton::clicked, this, [this]() { rebuildStatisticsTable(); });
    connect(searchEdit_, &QLineEdit::textChanged, this, [this](const QString& text) { searchLogs(text); });
    connect(levelFilterCombo_, &QComboBox::currentTextChanged, this,
            [this](const QString& level) { filterByLevel(level == "All" ? QString() : level); });
}
...
void DataLoggerPlugin::rebuildStatisticsTable() {
    ...
    while (it != stats.constEnd()) {
        QDateTime lastSeen;
        for (const auto& e : logEntries_) {
            if (e.source == it.key() && e.timestamp > lastSeen)
...
void DataLoggerPlugin::updateLogViewer() {
    ...
    logViewer_->clear();
    for (const auto& e : logEntries_) {
        if (!activeLevelFilter_.isEmpty() && e.level != activeLevelFilter_)
            continue;
        if (!activeSearchQuery_.isEmpty() && !e.message.contains(activeSearchQuery_, Qt::CaseInsensitive) &&
            !e.source.contains(activeSearchQuery_, Qt::CaseInsensitive))
            continue;
        logViewer_->append(e.timestamp.toString(Qt::ISODate) + " [" + e.level.toUpper() + "] " + e.source + ": " +
                           e.message);
    }
}
