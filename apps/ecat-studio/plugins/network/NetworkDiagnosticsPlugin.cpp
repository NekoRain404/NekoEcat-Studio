...
NetworkDiagnosticsPlugin::NetworkDiagnosticsPlugin(NetworkDiagnosticsService* service, QObject* parent)
    : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
...
QString NetworkDiagnosticsPlugin::id() const {
    return "network";
}
...
int NetworkDiagnosticsPlugin::defaultOrder() const {
    return 135;
}
bool NetworkDiagnosticsPlugin::visible() const {
    return false;
}
...
QWidget* NetworkDiagnosticsPlugin::widget() {
...
void NetworkDiagnosticsPlugin::buildUi() {
    ...
    portTable_->setHorizontalHeaderLabels({tr("Port"), tr("Link"), tr("Speed"), tr("Duplex"), tr("Errors")});
    ...
    connect(exportBtn_, &QPushButton::clicked, this, &NetworkDiagnosticsPlugin::exportReport);
}
...
void NetworkDiagnosticsPlugin::updateDisplay() {
    ...
    for (const auto& p : ports) {
        ...
        portTable_->setItem(r, 1, new QTableWidgetItem(p.linkUp ? tr("Up") : tr("Down")));
        portTable_->setItem(r, 2, new QTableWidgetItem(QString::number(p.speedMbps) + " Mbps"));
        portTable_->setItem(r, 3, new QTableWidgetItem(p.fullDuplex ? tr("Full") : tr("Half")));
        portTable_->setItem(r, 4, new QTableWidgetItem(QString::number(p.errorCount)));
    }

    auto setError = [&](int row, const QString& val) {
        if (auto* item = errorTable_->item(row, 1))
            item->setText(val);
    };
    ...
    healthLabel_->setText(
        QStringLiteral("<span style='color:%1'>%2: %3/%4 active</span>")
            .arg(color, tr("Health"), QString::number(health.activePorts), QString::number(health.portCount)));

    bandwidthLabel_->setText(tr("Bandwidth: %1%").arg(QString::number(health.bandwidth * 100.0, 'f', 1)));
    latencyLabel_->setText(
        tr("Latency: %1 ms / Jitter: %2 ms")
            .arg(QString::number(health.latencyMs, 'f', 2), QString::number(health.jitterMs, 'f', 2)));
}

void NetworkDiagnosticsPlugin::exportReport() {
    QString path = QFileDialog::getSaveFileName(nullptr, tr("Export Diagnostics Report"), "network_diagnostics.csv",
                                                tr("CSV Files (*.csv)"));
    if (path.isEmpty())
        return;
...
bool NetworkDiagnosticsPlugin::exportReportToFile(const QString& path) {
    ...
    auto ports = service_->allPortStatus();
    for (const auto& p : ports) {
        out << p.port << "," << (p.linkUp ? "Up" : "Down") << "," << p.speedMbps << " Mbps,"
            << (p.fullDuplex ? "Full" : "Half") << "," << p.errorCount << "\n";
    }
