...
OnlineDiagnosticsPlugin::OnlineDiagnosticsPlugin(OnlineDiagnosticsService *service,
                                                 QObject *parent)
    : service_(service) {
  ...
  connect(service_, &OnlineDiagnosticsService::errorRateUpdated, this,
          [this](const ErrorRate &r) {
            busMonitor_->updateErrorRate(r);
            errorAnalyzer_->updateErrors(r);
          });
  connect(service_, &OnlineDiagnosticsService::performanceUpdated, this,
          [this](const PerformanceMetrics &m) {
            perfLabel_->setText(tr("Perf: %1 f/s | Cycle: %2 us")
                                    .arg(m.pdoUpdateRate, 0, 'f', 1)
                                    .arg(m.cycleTimeUs, 0, 'f', 1));
          });
  connect(service_, &OnlineDiagnosticsService::healthUpdated, this,
          [this](const HealthStatus &h) {
...
QString OnlineDiagnosticsPlugin::id() const { return "onlinediagnostics"; }
QString OnlineDiagnosticsPlugin::displayName() const { return "Online Diagnostics"; }
...
int OnlineDiagnosticsPlugin::defaultOrder() const { return 28; }
bool OnlineDiagnosticsPlugin::visible() const { return true; }

void OnlineDiagnosticsPlugin::activate() {}
void OnlineDiagnosticsPlugin::deactivate() {}

QWidget *OnlineDiagnosticsPlugin::widget() { return containerWidget_; }
...
void OnlineDiagnosticsPlugin::exportReport() {
  QString path = QFileDialog::getSaveFileName(
      containerWidget_, tr("Export Diagnostics Report"), "diagnostics_report.csv",
