...
PerformanceDashboardPlugin::PerformanceDashboardPlugin(PerformanceMonitorService* service, QObject* parent)
    : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();
    ...
    connect(service_, &PerformanceMonitorService::performanceAlert, this,
            [this](const QString& category, const QString& message, double value, double threshold) {
                if (alertsTable_) {
                    int row = alertsTable_->rowCount();
                    alertsTable_->insertRow(row);
                    alertsTable_->setItem(row, 0,
                    ...
                    alertsTable_->setItem(row, 3, new QTableWidgetItem(QString::number(value, 'f', 2)));
                    alertsTable_->setItem(row, 4, new QTableWidgetItem(QString::number(threshold, 'f', 2)));
                    alertsTable_->scrollToBottom();
                }
            });
}

QString PerformanceDashboardPlugin::id() const {
    return "performancedashboard";
}
QString PerformanceDashboardPlugin::displayName() const {
    return "Performance";
}
QString PerformanceDashboardPlugin::displayNameZh() const {
    return QStringLiteral("性能监控");
}
int PerformanceDashboardPlugin::defaultOrder() const {
    return 135;
}
bool PerformanceDashboardPlugin::visible() const {
    return false;
}
QWidget* PerformanceDashboardPlugin::widget() {
    return container_;
}
...
void PerformanceDashboardPlugin::buildOverviewTab() {
    ...
    alertsTable_->setHorizontalHeaderLabels({tr("Time"), tr("Category"), tr("Message"), tr("Value"), tr("Threshold")});
...
void PerformanceDashboardPlugin::buildStartupTab() {
    ...
    startupPhasesTable_->setHorizontalHeaderLabels({tr("Phase"), tr("Duration (ms)"), tr("Percentage")});
...
void PerformanceDashboardPlugin::buildRuntimeTab() {
    ...
    runtimeTable_->setHorizontalHeaderLabels({tr("Metric"), tr("Current"), tr("Average"), tr("Min"), tr("Max")});
...
void PerformanceDashboardPlugin::buildHistoryTab() {
    ...
    historyTable_->setHorizontalHeaderLabels({tr("Timestamp"), tr("SDO Read"), tr("SDO Write"), tr("State Trans"),
                                              tr("Free Run"), tr("UI Update"), tr("Memory (MB)"), tr("Cycle Time")});
...
void PerformanceDashboardPlugin::buildReportsTab() {
    ...
    connect(generateBtn, &QPushButton::clicked, this, &PerformanceDashboardPlugin::generateReport);
    btnLayout->addWidget(generateBtn);

    auto* exportBtn = new QPushButton(tr("Export to File"));
    connect(exportBtn, &QPushButton::clicked, this, [this]() {
        QString fileName = QFileDialog::getSaveFileName(container_, tr("Export Performance Report"), "",
...
void PerformanceDashboardPlugin::updateOverview() {
    ...
    if (service_->startupComplete()) {
        ...
        totalStartupTime_->setText(tr("Total: %1 ms").arg(startup["totalMs"].toDouble(), 0, 'f', 1));
    } else {
    ...
    // Key metrics
    sdoReadLatencyLabel_->setText(QString::number(metrics["sdoReadLatencyMs"].toDouble(), 'f', 2));
    sdoWriteLatencyLabel_->setText(QString::number(metrics["sdoWriteLatencyMs"].toDouble(), 'f', 2));
    stateTransitionLabel_->setText(QString::number(metrics["stateTransitionMs"].toDouble(), 'f', 2));
    freeRunCycleLabel_->setText(QString::number(metrics["freeRunCycleUs"].toDouble(), 'f', 1));
    uiUpdateLabel_->setText(QString::number(metrics["uiUpdateMs"].toDouble(), 'f', 2));
    memoryUsageLabel_->setText(QString::number(metrics["memoryMB"].toDouble(), 'f', 1));
    ...
    colorLabel(sdoReadLatencyLabel_, metrics["sdoReadLatencyMs"].toDouble(), thresholds.sdoLatencyMs);
    colorLabel(sdoWriteLatencyLabel_, metrics["sdoWriteLatencyMs"].toDouble(), thresholds.sdoLatencyMs);
    colorLabel(stateTransitionLabel_, metrics["stateTransitionMs"].toDouble(), thresholds.stateTransitionMs);
    colorLabel(freeRunCycleLabel_, metrics["freeRunCycleUs"].toDouble(), thresholds.freeRunCycleUs);
    colorLabel(uiUpdateLabel_, metrics["uiUpdateMs"].toDouble(), thresholds.uiUpdateMs);
    colorLabel(memoryUsageLabel_, metrics["memoryMB"].toDouble(), thresholds.memoryMB);
}
...
void PerformanceDashboardPlugin::updateStartup() {
    ...
    if (startup["complete"].toBool()) {
        startupTotalLabel_->setText(tr("Total Startup Time: %1 ms").arg(startup["totalMs"].toDouble(), 0, 'f', 1));
    } else {
    ...
    for (auto it = phases.constBegin(); it != phases.constEnd(); ++it) {
        ...
        startupPhasesTable_->setItem(row, 1, new QTableWidgetItem(QString::number(it.value().toDouble(), 'f', 2)));
        double pct = percentages[it.key()].toDouble();
        startupPhasesTable_->setItem(row, 2, new QTableWidgetItem(QString::number(pct, 'f', 1) + "%"));
        ++row;
    }
}
...
void PerformanceDashboardPlugin::updateRuntime() {
    ...
    auto setRow = [&](const QString& name, const QJsonObject& s) {
        ...
        runtimeTable_->setItem(row, 1, new QTableWidgetItem(QString::number(s["avg"].toDouble(), 'f', 2)));
        runtimeTable_->setItem(row, 2, new QTableWidgetItem(QString::number(s["avg"].toDouble(), 'f', 2)));
        runtimeTable_->setItem(row, 3, new QTableWidgetItem(QString::number(s["min"].toDouble(), 'f', 2)));
        runtimeTable_->setItem(row, 4, new QTableWidgetItem(QString::number(s["max"].toDouble(), 'f', 2)));
        ++row;
    };
...
void PerformanceDashboardPlugin::updateMemory() {
    ...
    totalMemoryLabel_->setText(tr("Total Memory: %1 MB").arg(memory["totalMB"].toDouble(), 0, 'f', 2));
    ...
    for (auto it = services.constBegin(); it != services.constEnd(); ++it) {
        ...
        serviceMemoryTable_->setItem(row, 1, new QTableWidgetItem(QString::number(it.value().toDouble(), 'f', 0)));
        ++row;
    }
    ...
    for (auto it = plugins.constBegin(); it != plugins.constEnd(); ++it) {
        ...
        pluginMemoryTable_->setItem(row, 1, new QTableWidgetItem(QString::number(it.value().toDouble(), 'f', 0)));
        ++row;
    }
    ...
    for (auto it = caches.constBegin(); it != caches.constEnd(); ++it) {
        ...
        cacheMemoryTable_->setItem(row, 1, new QTableWidgetItem(QString::number(it.value().toDouble(), 'f', 0)));
        ++row;
    }
}
...
void PerformanceDashboardPlugin::updateHistory() {
    ...
    for (int i = start; i < history.size(); ++i) {
        ...
        historyTable_->setItem(row, 0, new QTableWidgetItem(QDateTime::fromMSecsSinceEpoch(ts).toString("hh:mm:ss")));
        historyTable_->setItem(row, 1,
                               new QTableWidgetItem(QString::number(entry["sdoReadLatencyMs"].toDouble(), 'f', 2)));
        historyTable_->setItem(row, 2,
                               new QTableWidgetItem(QString::number(entry["sdoWriteLatencyMs"].toDouble(), 'f', 2)));
        historyTable_->setItem(row, 3,
                               new QTableWidgetItem(QString::number(entry["stateTransitionMs"].toDouble(), 'f', 2)));
        historyTable_->setItem(row, 4,
                               new QTableWidgetItem(QString::number(entry["freeRunCycleUs"].toDouble(), 'f', 1)));
        historyTable_->setItem(row, 5, new QTableWidgetItem(QString::number(entry["uiUpdateMs"].toDouble(), 'f', 2)));
        historyTable_->setItem(row, 6, new QTableWidgetItem(QString::number(entry["memoryMB"].toDouble(), 'f', 1)));
        historyTable_->setItem(row, 7, new QTableWidgetItem(QString::number(entry["cycleTimeUs"].toDouble(), 'f', 1)));
    }
}
