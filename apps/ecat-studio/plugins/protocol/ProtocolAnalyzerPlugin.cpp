...
ProtocolAnalyzerPlugin::ProtocolAnalyzerPlugin(ProtocolAnalyzerService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(service_, &ProtocolAnalyzerService::frameCaptured, this, &ProtocolAnalyzerPlugin::onFrameCaptured);
    connect(service_, &ProtocolAnalyzerService::statisticsUpdated, this, &ProtocolAnalyzerPlugin::updateStatistics);
}

QString ProtocolAnalyzerPlugin::id() const {
    return "protocol";
}
...
int ProtocolAnalyzerPlugin::defaultOrder() const {
    return 105;
}
bool ProtocolAnalyzerPlugin::visible() const {
    return false;
}

QWidget* ProtocolAnalyzerPlugin::widget() {
    return container_;
}
...
void ProtocolAnalyzerPlugin::buildUi() {
    ...
    frameTable_->setHorizontalHeaderLabels({tr("Time"), tr("Dir"), tr("Type"), tr("WKC"), tr("Len"), tr("Summary")});
    ...
    // ── Connections ──
    connect(captureBtn_, &QPushButton::clicked, this, &ProtocolAnalyzerPlugin::toggleCapture);
    connect(clearBtn_, &QPushButton::clicked, this, &ProtocolAnalyzerPlugin::clearFrames);
    connect(filterBtn_, &QPushButton::clicked, this, &ProtocolAnalyzerPlugin::applyFilter);
    connect(exportBtn_, &QPushButton::clicked, this, &ProtocolAnalyzerPlugin::exportPcap);
}
...
void ProtocolAnalyzerPlugin::applyFilter() {
    ...
    if (dirIdx == 1)
        ...
        f.direction = ProtocolDirection::RX;
...
void ProtocolAnalyzerPlugin::onFrameCaptured(const ProtocolFrame& frame) {
    ...
    frameTable_->setItem(
        row, 0, new QTableWidgetItem(QDateTime::fromMSecsSinceEpoch(frame.timestamp).toString("HH:mm:ss.zzz")));
    frameTable_->setItem(row, 1, new QTableWidgetItem(frame.direction == ProtocolDirection::TX ? "TX" : "RX"));
...
void ProtocolAnalyzerPlugin::updateStatistics(const ProtocolStatistics& stats) {
    statsLabel_->setText(QStringLiteral("Frames: %1 | TX: %2 | RX: %3 | Errors: %4 | CoE: %5 | EoE: %6 | BW: %7 B/s")
...
void ProtocolAnalyzerPlugin::exportPcap() {
    const QString path =
        QFileDialog::getSaveFileName(container_, tr("Export PCAP"), QString(), tr("PCAP files (*.pcap)"));
    if (path.isEmpty())
        return;
