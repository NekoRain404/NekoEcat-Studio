...
QString IntegrationHubPlugin::id() const {
    return "integrationhub";
}
QString IntegrationHubPlugin::displayName() const {
    return "Integration Hub";
}
QString IntegrationHubPlugin::displayNameZh() const {
    return "集成中心";
}
int IntegrationHubPlugin::defaultOrder() const {
    return 325;
}
bool IntegrationHubPlugin::visible() const {
    return false;
}
...
int IntegrationHubPlugin::connectionCount() const {
...
int IntegrationHubPlugin::mappingCount() const {
...
int IntegrationHubPlugin::syncStatusCount() const {
    return syncStatuses_.size();
}
...
int IntegrationHubPlugin::logCount() const {
    return logs_.size();
}
...
bool IntegrationHubPlugin::exportReport(const QString& path) {
    ...
    for (const auto& e : logs_) {
        out << e.timestamp.toString(Qt::ISODate) << " [" << e.level << "] " << e.source << ": " << e.message << "\n";
    }
    return out.status() == QTextStream::Ok && f.flush();
}

QTableWidget* IntegrationHubPlugin::connectionTable() const {
    return connectionTable_;
}
QTableWidget* IntegrationHubPlugin::mappingTable() const {
    return mappingTable_;
}
QTableWidget* IntegrationHubPlugin::syncTable() const {
    return syncTable_;
}
QTableWidget* IntegrationHubPlugin::logTable() const {
    return logTable_;
}
QLabel* IntegrationHubPlugin::statusLabel() const {
    return statusLabel_;
}
...
void IntegrationHubPlugin::buildUi() {
    ...
    connectionTable_->setHorizontalHeaderLabels({"ID", "Name", "Type", "Endpoint", "Status", "Last Sync"});
    ...
    mappingTable_->setHorizontalHeaderLabels({"ID", "Source", "Destination", "Transformation", "Enabled"});
    ...
    syncTable_->setHorizontalHeaderLabels({"Connection ID", "Name", "State", "Progress", "Last Update", "Message"});
    ...
    logTable_->setHorizontalHeaderLabels({"Timestamp", "Source", "Level", "Message", "Details"});
    ...
    connect(filterLogBtn_, &QPushButton::clicked, this,
            [this]() { filterLogs(logLevelFilter_->currentText(), logSourceFilter_->text()); });
    connect(exportReportBtn_, &QPushButton::clicked, this, [this]() { exportReport("/tmp/integration_report.txt"); });
}
...
void IntegrationHubPlugin::rebuildConnectionTable() {
    ...
    for (int i = 0; i < connections_.size(); ++i) {
        ...
        connectionTable_->setItem(i, 5, new QTableWidgetItem(c.lastSync.toString(Qt::ISODate)));
    }
}
...
void IntegrationHubPlugin::rebuildSyncTable() {
    ...
    for (int i = 0; i < syncStatuses_.size(); ++i) {
        ...
        syncTable_->setItem(i, 4, new QTableWidgetItem(s.lastUpdate.toString(Qt::ISODate)));
...
void IntegrationHubPlugin::rebuildLogTable() {
    ...
    for (int i = 0; i < filteredLogs_.size(); ++i) {
        ...
        logTable_->setItem(i, 0, new QTableWidgetItem(e.timestamp.toString(Qt::ISODate)));
    ...
    if (statusLabel_)
        statusLabel_->setText(QString("Integration Logs: %1 entries shown").arg(filteredLogs_.size()));
}
