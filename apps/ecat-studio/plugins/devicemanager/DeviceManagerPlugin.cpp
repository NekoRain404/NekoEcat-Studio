...
DeviceManagerPlugin::DeviceManagerPlugin(DeviceManagerService* service, QObject* parent) : service_(service) {
    if (parent)
        setParent(parent);
    buildUi();

    connect(service_, &DeviceManagerService::deviceListChanged, this, [this]() { updateDisplay(); });
}

QString DeviceManagerPlugin::id() const {
    return "devicemanager";
}
QString DeviceManagerPlugin::displayName() const {
...
int DeviceManagerPlugin::defaultOrder() const {
    return 95;
}
bool DeviceManagerPlugin::visible() const {
    return false;
}
...
QWidget* DeviceManagerPlugin::widget() {
...
void DeviceManagerPlugin::buildUi() {
    ...
    deviceTable_->setHorizontalHeaderLabels(
        {tr("Address"), tr("Name"), tr("State"), tr("Vendor"), tr("Product"), tr("Revision")});
...
void DeviceManagerPlugin::updateDisplay() {
    ...
    for (const auto& d : devices) {
        ...
        deviceTable_->setItem(r, 0, new QTableWidgetItem(QString::number(d.address)));
        ...
        deviceTable_->setItem(r, 5, new QTableWidgetItem(QString::number(d.revision)));
    }
