...
DocumentationPlugin::DocumentationPlugin(QObject* parent) {
    ...
    documents_ = {
        ...
        {"installation", "Installation", "Getting Started", "System requirements and installation steps.",
         "install,setup"},
        {"protocol", "EtherCAT Protocol", "Reference", "EtherCAT is a real-time industrial Ethernet protocol.",
         "ethercat,protocol"},
        {"od_ref", "Object Dictionary", "Reference", "The Object Dictionary contains all configurable parameters.",
         "od,object"},
        {"pdo_ref", "PDO Mapping", "Reference", "Process Data Objects define real-time data exchange.", "pdo,mapping"},
        {"sdo_api", "SDO Service API", "API Reference",
         "The SDO Service provides methods for reading and writing SDOs.", "sdo,api"},
        {"plugin_api", "Plugin API", "API Reference", "The Plugin API allows extending NekoEcat Studio.", "plugin,api"},
    };
    buildUi();
}

QString DocumentationPlugin::id() const {
    return "documentation";
}
QString DocumentationPlugin::displayName() const {
    return "Documentation";
}
QString DocumentationPlugin::displayNameZh() const {
    return "文档";
}

QIcon DocumentationPlugin::icon() const {
    return QIcon();
}

int DocumentationPlugin::defaultOrder() const {
    return 215;
}
bool DocumentationPlugin::visible() const {
    return false;
}
...
int DocumentationPlugin::documentCount() const {
    return documents_.size();
}

void DocumentationPlugin::addBookmark(const QString& docId, const QString& title, const QString& category) {
    Bookmark bm;
    bm.docId = docId;
    bm.title = title;
...
int DocumentationPlugin::bookmarkCount() const {
    return bookmarks_.size();
}

void DocumentationPlugin::addAnnotation(const QString& docId, const QString& text) {
    Annotation ann;
    ann.docId = docId;
    ann.text = text;
...
void DocumentationPlugin::search(const QString& query) {
    ...
    for (int i = 0; i < documents_.size(); ++i) {
        ...
        if (doc.title.contains(query, Qt::CaseInsensitive) || doc.content.contains(query, Qt::CaseInsensitive) ||
    ...
    if (statusLabel_)
        statusLabel_->setText(tr("%1 results").arg(searchResults_.size()));
...
QTreeWidget* DocumentationPlugin::docTree() const {
    return docTree_;
}
QTextEdit* DocumentationPlugin::contentView() const {
    return contentView_;
}
