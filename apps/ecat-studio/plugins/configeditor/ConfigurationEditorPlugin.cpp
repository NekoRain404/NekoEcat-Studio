...
QString ConfigurationEditorPlugin::id() const {
    return "configeditor";
}
QString ConfigurationEditorPlugin::displayName() const {
    return "Configuration Editor";
}
QString ConfigurationEditorPlugin::displayNameZh() const {
    return "配置编辑器";
}
int ConfigurationEditorPlugin::defaultOrder() const {
    return 265;
}
bool ConfigurationEditorPlugin::visible() const {
    return false;
}
...
int ConfigurationEditorPlugin::configCount() const {
...
int ConfigurationEditorPlugin::selectedConfig() const {
...
void ConfigurationEditorPlugin::validate() {
    ...
    if (statusLabel_)
        statusLabel_->setText(QString("Validation: %1 errors").arg(errors_.size()));
    emit validationCompleted(errors_.size());
}

int ConfigurationEditorPlugin::errorCount() const {
    return errors_.size();
}
...
bool ConfigurationEditorPlugin::importConfig(const QString& path) {
    ...
    if (configs_.size() == initialCount)
        return false;
    rebuildConfigTree();
    return true;
}

QTreeWidget* ConfigurationEditorPlugin::configTree() const {
    return configTree_;
}
QTextEdit* ConfigurationEditorPlugin::configEditor() const {
    return configEditor_;
}
QTextEdit* ConfigurationEditorPlugin::configPreview() const {
    return configPreview_;
}
QTableWidget* ConfigurationEditorPlugin::validationTable() const {
    return validationTable_;
}
QLabel* ConfigurationEditorPlugin::statusLabel() const {
    return statusLabel_;
}
...
void ConfigurationEditorPlugin::buildUi() {
    ...
    connect(addBtn_, &QPushButton::clicked, this,
            [this]() { addConfig({"General", "new_key", "", "New configuration entry"}); });
    connect(removeBtn_, &QPushButton::clicked, this, [this]() {
        if (selectedIndex_ >= 0)
            removeConfig(selectedIndex_);
    });
    connect(validateBtn_, &QPushButton::clicked, this, [this]() { validate(); });
