...
QString ReportDesignerPlugin::id() const { return "reportdesigner"; }
QString ReportDesignerPlugin::displayName() const { return "Report Designer"; }
...
QWidget *ReportDesignerPlugin::widget() { return containerWidget_; }
QWidget *ReportDesignerPlugin::layoutEditor() const { return layoutEditor_; }
QListWidget *ReportDesignerPlugin::templates() const { return templates_; }
QTreeWidget *ReportDesignerPlugin::dataBindings() const { return dataBindings_; }
QTextEdit *ReportDesignerPlugin::previewPane() const { return previewPane_; }
...
void ReportDesignerPlugin::buildUi() {
  ...
  connect(addTmplBtn, &QPushButton::clicked, this, [this]() {
    addTemplate(tr("Custom Template %1").arg(templates_->count() + 1));
  });
  connect(removeTmplBtn, &QPushButton::clicked, this, [this]() {
    auto *item = templates_->currentItem();
    if (item) removeTemplate(item->text());
  });
  connect(addBindBtn, &QPushButton::clicked, this, [this]() {
    addDataBinding(tr("Custom"), tr("Field %1").arg(dataBindingCount() + 1));
  });
  connect(removeBindBtn, &QPushButton::clicked, this, [this]() {
    auto *item = dataBindings_->currentItem();
    if (item && !item->parent()) return;
    if (item) removeDataBinding(item->text(0));
  });
  ...
  connect(importButton_, &QPushButton::clicked, this, [this]() {
    QString path = QFileDialog::getOpenFileName(containerWidget_, tr("Import Template"), QString(), "JSON (*.json)");
    if (!path.isEmpty()) importTemplate(path);
  });
}
...
void ReportDesignerPlugin::clearTemplates() { templates_->clear(); }
int ReportDesignerPlugin::templateCount() const { return templates_->count(); }
...
void ReportDesignerPlugin::clearDataBindings() { dataBindings_->clear(); }
