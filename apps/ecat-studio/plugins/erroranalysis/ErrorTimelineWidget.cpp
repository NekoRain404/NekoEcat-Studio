...
ErrorTimelineWidget::ErrorTimelineWidget(QWidget *parent)
    : QWidget(parent) {
...
int ErrorTimelineWidget::eventCount() const { return events_.size(); }
...
void ErrorTimelineWidget::drawTimeline(QPainter &painter, const QRect &rect) {
  ...
  qint64 totalSpan =
      events_.first().timestamp.msecsTo(events_.last().timestamp);
  ...
  for (int i = 0; i <= markers; ++i) {
    ...
    QDateTime t = events_.first().timestamp.addMSecs(
        static_cast<qint64>(frac * totalSpan));
    painter.drawText(x - 30, rect.bottom() + 8, 60, 20,
                     Qt::AlignHCenter, t.toString("hh:mm:ss"));
  }
}
...
void ErrorTimelineWidget::drawEvents(QPainter &painter, const QRect &rect) {
  ...
  for (const auto &ev : events_) {
    double frac = static_cast<double>(
                      events_.first().timestamp.msecsTo(ev.timestamp)) /
                  totalSpan;
...
void ErrorTimelineWidget::mousePressEvent(QMouseEvent *event) {
  if (event->button() == Qt::LeftButton) dragStartX_ = event->pos().x();
}
