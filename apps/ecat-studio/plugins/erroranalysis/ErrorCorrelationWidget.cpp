...
ErrorCorrelationWidget::ErrorCorrelationWidget(QWidget *parent)
    : QWidget(parent) {
  buildUi();
}
...
void ErrorCorrelationWidget::buildUi() {
  ...
  correlationTable_->setHorizontalHeaderLabels(
      {tr("Type A"), tr("Type B"), tr("Correlation"), tr("Relationship")});
...
void ErrorCorrelationWidget::setCorrelationData(
    const QVector<CorrelationDisplayEntry> &entries) {
  correlationTable_->setRowCount(entries.size());
  for (int i = 0; i < entries.size(); ++i) {
    correlationTable_->setItem(
        i, 0, new QTableWidgetItem(entries[i].typeA));
    correlationTable_->setItem(
        i, 1, new QTableWidgetItem(entries[i].typeB));
    correlationTable_->setItem(
        i, 2,
        new QTableWidgetItem(QString::number(entries[i].value, 'f', 2)));
    correlationTable_->setItem(
        i, 3, new QTableWidgetItem(entries[i].relationship));
  }
...
void ErrorCorrelationWidget::setRootCause(const RootCauseDisplay &rca) {
  ...
  for (const auto &f : rca.factors) text += QStringLiteral("  - %1\n").arg(f);
  text += tr("\nRecommended Actions:\n");
  for (const auto &a : rca.actions) text += QStringLiteral("  - %1\n").arg(a);
  rootCauseText_->setText(text);
}

void ErrorCorrelationWidget::setRecommendations(
