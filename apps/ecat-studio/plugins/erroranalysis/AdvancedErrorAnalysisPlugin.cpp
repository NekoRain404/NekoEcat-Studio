...
int AdvancedErrorAnalysisPlugin::defaultOrder() const {
    return 34;
}
bool AdvancedErrorAnalysisPlugin::visible() const {
    return false;
}
...
QWidget* AdvancedErrorAnalysisPlugin::widget() {
...
void AdvancedErrorAnalysisPlugin::buildUi() {
    ...
    errorTable_->setHorizontalHeaderLabels(
        {tr("ID"), tr("Timestamp"), tr("Slave"), tr("Category"), tr("Severity"), tr("Message")});
    ...
    connect(filter_, &QLineEdit::textChanged, this, [this]() {
        ...
        const QString sev = severityFilter_ ? severityFilter_->currentData().toString() : QString();
        ...
        int visible = 0;
        for (int row = 0; row < errorTable_->rowCount(); ++row)
            if (!errorTable_->isRowHidden(row))
                ++visible;
        summaryLabel_->setText(tr("%1 of %2 errors shown").arg(visible).arg(errorTable_->rowCount()));
    });

    connect(severityFilter_, QOverload<int>::of(&QComboBox::currentIndexChanged), this,
            [this]() { filter_->textChanged(filter_->text()); });

    connect(analyzeBtn_, &QPushButton::clicked, this, &AdvancedErrorAnalysisPlugin::runAnalysis);
    connect(exportBtn_, &QPushButton::clicked, this, [this]() { exportReport(containerWidget_); });
...
void AdvancedErrorAnalysisPlugin::populateTestData() {
    ...
    for (int i = 0; i < tests.size(); ++i) {
        ...
        errorTable_->setItem(row, 0, new QTableWidgetItem(QString::number(i + 1)));
        errorTable_->setItem(row, 1, new QTableWidgetItem(ts.toString("hh:mm:ss")));
        errorTable_->setItem(row, 2, new QTableWidgetItem(QString::number(t.slave)));
    ...
    summaryLabel_->setText(tr("%1 errors loaded").arg(errorTable_->rowCount()));
}
...
void AdvancedErrorAnalysisPlugin::runAnalysis() {
    ...
    if (!errors.isEmpty()) {
        ...
        QStringList recs;
        for (const auto& p : analysisService_->detectPatterns(errors))
            recs.append(p.description);
        for (const auto& r : rca.recommendedActions)
            recs.append(r);
...
void AdvancedErrorAnalysisPlugin::exportReport(QWidget* parentWidget) {
    const QString path = QFileDialog::getSaveFileName(parentWidget, tr("Export Error Analysis Report"), QString(),
...
bool AdvancedErrorAnalysisPlugin::exportReportToFile(const QString& path) {
    ...
    out << "## Error Log\n\n";
    out << "| " << tr("ID") << " | " << tr("Timestamp") << " | " << tr("Slave") << " | " << tr("Category") << " | "
    ...
    for (int row = 0; row < ct->rowCount(); ++row) {
        out << "- " << ct->item(row, 0)->text() << " <-> " << ct->item(row, 1)->text() << " ("
            << ct->item(row, 3)->text() << ")\n";
    }
