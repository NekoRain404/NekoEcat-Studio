...
MonitoringDashboardPlugin::MonitoringDashboardPlugin(QObject *parent)
    : WorkspacePlugin() {}
...
void MonitoringDashboardPlugin::updateMetric(int index, double value) {
  if (index >= 0 && index < metrics_.size()) {
    ...
    metrics_[index].status =
        value > metrics_[index].threshold ? QStringLiteral("ALERT") : QStringLiteral("OK");
...
QVector<MonitoringDashboardPlugin::MonitoringMetric>
MonitoringDashboardPlugin::metrics() const {
  return metrics_;
}
...
QVector<MonitoringDashboardPlugin::MonitoringAlert>
MonitoringDashboardPlugin::alerts() const {
  return alerts_;
}
...
QVector<MonitoringDashboardPlugin::MonitoringEvent>
MonitoringDashboardPlugin::events() const {
  return events_;
}
...
QVector<MonitoringDashboardPlugin::MonitoringDashboard>
MonitoringDashboardPlugin::dashboards() const {
  return dashboards_;
}
  ...
  for (const auto &a : alerts_) {
    if (!a.acknowledged) {
      report += QStringLiteral("- [%1] %2: %3\n")
                    .arg(a.severity)
                    .arg(a.timestamp.toString(Qt::ISODate))
                    .arg(a.message);
    }
  }
  report += QStringLiteral("\n## Recent Events\n\n");
...
void MonitoringDashboardPlugin::refresh() {
  ...
  if (statusLabel_)
    statusLabel_->setText(
        QStringLiteral("Last refreshed: %1 | Metrics: %2 | Active Alerts: %3")
...
void MonitoringDashboardPlugin::buildUi() {
  ...
  metricsTable_->setHorizontalHeaderLabels(
      {tr("Name"), tr("Value"), tr("Threshold"), tr("Status")});
  ...
  alertsTable_->setHorizontalHeaderLabels(
      {tr("Timestamp"), tr("Severity"), tr("Message"), tr("Acknowledged")});
  tabs_->addTab(alertsTable_, tr("Alerts"));

  eventsTable_ = new QTableWidget;
  eventsTable_->setColumnCount(4);
  eventsTable_->setHorizontalHeaderLabels(
      {tr("Timestamp"), tr("Category"), tr("Description"), tr("Level")});
  tabs_->addTab(eventsTable_, tr("Events"));

  dashboardsTable_ = new QTableWidget;
  dashboardsTable_->setColumnCount(3);
  dashboardsTable_->setHorizontalHeaderLabels(
      {tr("Name"), tr("Metrics"), tr("Last Update")});
  ...
  connect(refreshBtn_, &QPushButton::clicked, this,
          &MonitoringDashboardPlugin::refresh);
...
void MonitoringDashboardPlugin::rebuildMetricsTable() {
  ...
  for (int i = 0; i < metrics_.size(); ++i) {
    ...
    metricsTable_->setItem(
        i, 2,
        new QTableWidgetItem(QString::number(metrics_[i].threshold, 'f', 2)));
...
void MonitoringDashboardPlugin::rebuildAlertsTable() {
  ...
  for (int i = 0; i < alerts_.size(); ++i) {
    alertsTable_->setItem(
        i, 0,
        new QTableWidgetItem(alerts_[i].timestamp.toString(Qt::ISODate)));
    ...
    alertsTable_->setItem(
        i, 3,
        new QTableWidgetItem(alerts_[i].acknowledged ? tr("Yes") : tr("No")));
  }
}
...
void MonitoringDashboardPlugin::rebuildEventsTable() {
  ...
  for (int i = 0; i < events_.size(); ++i) {
    eventsTable_->setItem(
        i, 0,
        new QTableWidgetItem(events_[i].timestamp.toString(Qt::ISODate)));
...
void MonitoringDashboardPlugin::rebuildDashboardsTable() {
  ...
  for (int i = 0; i < dashboards_.size(); ++i) {
    ...
    dashboardsTable_->setItem(
        i, 1,
        new QTableWidgetItem(QString::number(dashboards_[i].metrics.size())));
    dashboardsTable_->setItem(
        i, 2,
        new QTableWidgetItem(
            dashboards_[i].lastUpdate.toString(Qt::ISODate)));
  }
}
