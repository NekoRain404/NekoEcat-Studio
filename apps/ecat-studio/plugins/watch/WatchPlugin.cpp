...
WatchPlugin::WatchPlugin(ServiceContainer* container, QObject* parent) : container_(container) {
    if (parent)
        setParent(parent);
    buildUi();

    auto* watchSvc = container_->watch();
    connect(watchSvc, &WatchService::entryUpdated, this, [this](int row, const WatchEntry& entry) {
    ...
    connect(watchSvc, &WatchService::refreshComplete, this, [this](int requested, int succeeded) {
...
// ── Identity ──────────────────────────────────────────────────────────
QString WatchPlugin::id() const {
    return "watch";
}
QString WatchPlugin::displayName() const {
    return "Watch";
}
QString WatchPlugin::displayNameZh() const {
    return QStringLiteral("监视");
}
QIcon WatchPlugin::icon() const {
    return QIcon::fromTheme("utilities-system-monitor");
}
int WatchPlugin::defaultOrder() const {
    return 30;
}
bool WatchPlugin::visible() const {
    return true;
}
...
void WatchPlugin::buildUi() {
    ...
    table_->setHorizontalHeaderLabels({tr("Time"), tr("Slave"), tr("Index"), tr("Sub"), tr("Value"), tr("Decoded"),
                                       tr("Type"), tr("Mode"), tr("Baseline"), tr("Delta"), tr("Startup"),
                                       tr("Startup Delta")});
    ...
    connect(autoRefresh_, &QCheckBox::toggled, this, [this]() { emit watchModified(); });
}

// ── Table access ──────────────────────────────────────────────────────
QTableWidget* WatchPlugin::watchTable() const {
    return table_;
}
QLineEdit* WatchPlugin::filterInput() const {
    return filter_;
}
QCheckBox* WatchPlugin::autoRefreshCheckBox() const {
    return autoRefresh_;
}
QComboBox* WatchPlugin::refreshIntervalCombo() const {
    return refreshInterval_;
}
QLabel* WatchPlugin::summaryLabel() const {
...
int WatchPlugin::rowCount() const {
