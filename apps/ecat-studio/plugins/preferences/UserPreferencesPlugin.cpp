...
QString UserPreferencesPlugin::id() const { return "preferences"; }
QString UserPreferencesPlugin::displayName() const { return "User Preferences"; }
...
void UserPreferencesPlugin::populateDefaults() {
  categories_ = {
      {tr("General"), {
          {"language", tr("Language"), tr("Interface language"), "English", "English", "combo", {"English", "Chinese", "Japanese", "German"}},
      ...
      {tr("Appearance"), {
          {"theme", tr("Theme"), tr("Application theme"), "Light", "Light", "combo", {"Light", "Dark", "High Contrast"}},
      ...
      {tr("Notifications"), {
          ...
          {"logLevel", tr("Log Level"), tr("Minimum log level to display"), "Info", "Info", "combo", {"Debug", "Info", "Warning", "Error"}},
      }},
      {tr("Export"), {
          {"defaultFormat", tr("Default Format"), tr("Default export format"), "CSV", "CSV", "combo", {"CSV", "JSON", "XML", "Text"}},
...
void UserPreferencesPlugin::exportPreferences() {
  QString path = QFileDialog::getSaveFileName(container_, tr("Export Preferences"), "", tr("Preference Files (*.prefs);;All (*)"));
...
void UserPreferencesPlugin::importPreferences() {
  QString path = QFileDialog::getOpenFileName(container_, tr("Import Preferences"), "", tr("Preference Files (*.prefs);;All (*)"));
  if (path.isEmpty()) return;
  QSettings file(path, QSettings::IniFormat);
  for (int ci = 0; ci < categories_.size(); ++ci) {
    file.beginGroup(categories_[ci].name);
