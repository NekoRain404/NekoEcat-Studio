...
QString ReplicationManagerPlugin::id() const {
    return "replicationmanager";
}
QString ReplicationManagerPlugin::displayName() const {
    return "Replication Manager";
}
QString ReplicationManagerPlugin::displayNameZh() const {
    return "复制管理器";
}
int ReplicationManagerPlugin::defaultOrder() const {
    return 335;
}
bool ReplicationManagerPlugin::visible() const {
    return false;
}
...
int ReplicationManagerPlugin::targetCount() const {
...
int ReplicationManagerPlugin::statusCount() const {
...
int ReplicationManagerPlugin::historyCount() const {
...
int ReplicationManagerPlugin::settingCount() const {
...
bool ReplicationManagerPlugin::exportReport(const QString& path) {
    ...
    for (const auto& t : targets_) {
        out << t.name << " [" << t.type << "] " << t.endpoint << " " << (t.enabled ? "enabled" : "disabled") << "\n";
    }
    out << "\n--- Status ---\n";
    ...
    for (const auto& h : history_) {
        out << h.timestamp.toString(Qt::ISODate) << " " << h.targetName << " " << h.result << " " << h.objectsReplicated
            << " objects\n";
    }
    return out.status() == QTextStream::Ok && f.flush();
}

QTableWidget* ReplicationManagerPlugin::targetTable() const {
    return targetTable_;
}
QTableWidget* ReplicationManagerPlugin::statusTable() const {
    return statusTable_;
}
QTableWidget* ReplicationManagerPlugin::historyTable() const {
    return historyTable_;
}
QTableWidget* ReplicationManagerPlugin::settingsTable() const {
    return settingsTable_;
}
QLabel* ReplicationManagerPlugin::statusLabel() const {
    return statusLabel_;
}
...
void ReplicationManagerPlugin::buildUi() {
    ...
    targetTable_->setHorizontalHeaderLabels({"ID", "Name", "Endpoint", "Type", "Enabled", "Last Replicated"});
    ...
    statusTable_->setHorizontalHeaderLabels({"Target ID", "Name", "State", "Progress", "Last Update", "Message"});
    ...
    historyTable_->setHorizontalHeaderLabels({"Timestamp", "Target ID", "Name", "Result", "Objects", "Details"});
    ...
    settingsTable_->setHorizontalHeaderLabels({"ID", "Name", "Description", "Value", "Default"});
    ...
    connect(exportReportBtn_, &QPushButton::clicked, this, [this]() { exportReport("/tmp/replication_report.txt"); });
}
...
void ReplicationManagerPlugin::rebuildTargetTable() {
    ...
    for (int i = 0; i < targets_.size(); ++i) {
        ...
        targetTable_->setItem(i, 5, new QTableWidgetItem(t.lastReplicated.toString(Qt::ISODate)));
    }
}
...
void ReplicationManagerPlugin::rebuildStatusTable() {
    ...
    for (int i = 0; i < statuses_.size(); ++i) {
        ...
        statusTable_->setItem(i, 4, new QTableWidgetItem(s.lastUpdate.toString(Qt::ISODate)));
...
void ReplicationManagerPlugin::rebuildHistoryTable() {
    ...
    for (int i = 0; i < history_.size(); ++i) {
        ...
        historyTable_->setItem(i, 0, new QTableWidgetItem(h.timestamp.toString(Qt::ISODate)));
        ...
        historyTable_->setItem(i, 4, new QTableWidgetItem(QString::number(h.objectsReplicated)));
