...
QString TemplatePlugin::displayName() const {
    return "Templates";
}
QString TemplatePlugin::displayNameZh() const {
    return "模板";
}
int TemplatePlugin::defaultOrder() const {
    return 225;
}
bool TemplatePlugin::visible() const {
    return false;
}
...
int TemplatePlugin::templateCount() const {
...
void TemplatePlugin::selectTemplate(int index) {
    ...
    if (editor_)
        editor_->setText(templates_[index].content);
    refreshPreview();
    emit templateSelected(index);
}

int TemplatePlugin::selectedTemplate() const {
...
void TemplatePlugin::search(const QString& query) {
    ...
    for (int i = 0; i < templates_.size(); ++i) {
        ...
        if (t.name.contains(query, Qt::CaseInsensitive) || t.category.contains(query, Qt::CaseInsensitive) ||
...
int TemplatePlugin::searchResultCount() const {
...
bool TemplatePlugin::exportTemplate(const QString& path) {
    if (selectedIndex_ < 0 || selectedIndex_ >= templates_.size())
        return false;
    if (path.isEmpty())
        return false;
    QFile f(path);
...
QTableWidget* TemplatePlugin::templateTable() const {
    return templateTable_;
}
QTableWidget* TemplatePlugin::searchResultsTable() const {
    return searchResultsTable_;
}
QTextEdit* TemplatePlugin::editor() const {
    return editor_;
}
QTextEdit* TemplatePlugin::preview() const {
    return preview_;
}
QLabel* TemplatePlugin::statusLabel() const {
    return statusLabel_;
}
...
void TemplatePlugin::buildUi() {
    ...
    connect(searchBtn_, &QPushButton::clicked, this, [this]() { search(searchEdit_->text()); });
