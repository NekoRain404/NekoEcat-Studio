...
ConsistencyPlugin::ConsistencyPlugin(ServiceContainer *container,
                                     QObject *parent)
...
QString ConsistencyPlugin::id() const { return "consistency"; }
QString ConsistencyPlugin::displayName() const { return "Consistency"; }
QString ConsistencyPlugin::displayNameZh() const {
  return QStringLiteral("一致性");
}
QIcon ConsistencyPlugin::icon() const { return QIcon::fromTheme("dialog-ok-apply"); }
int ConsistencyPlugin::defaultOrder() const { return 67; }
bool ConsistencyPlugin::visible() const { return true; }
...
QWidget *ConsistencyPlugin::widget() { return containerWidget_; }

QTableWidget *ConsistencyPlugin::consistencyTable() const { return table_; }
QLineEdit *ConsistencyPlugin::consistencyFilter() const { return filter_; }
...
void ConsistencyPlugin::buildUi() {
  ...
  table_->setHorizontalHeaderLabels({tr("Level"), tr("Scope"), tr("Target"),
                                     tr("Evidence"), tr("Expected"),
                                     tr("Actual"), tr("Action")});
  ...
  connect(filter_, &QLineEdit::textChanged, this, [this]() {
    const QString scope =
        scopeFilter_ ? scopeFilter_->currentData().toString()
                     : QStringLiteral("all");
  ...
  connect(scopeFilter_, QOverload<int>::of(&QComboBox::currentIndexChanged),
          this, [this]() {
            ...
            filterConsistencyTableRows(table_, scope,
                                       filter_->text().trimmed());
          });
}

void ConsistencyPlugin::updateConsistencyView(const QList<QStringList> &rows) {
  const QStringList headers = {tr("Level"),     tr("Scope"),   tr("Target"),
                               tr("Evidence"),  tr("Expected"), tr("Actual"),
                               tr("Action")};
  ...
  for (int r = 0; r < table_->rowCount(); ++r) {
    ...
    const ConsistencyIssueLevel level =
