...
PerformanceMonitorService::PerformanceMonitorService(EventBus* bus, EcatClient* client, QObject* parent)
    : QObject(parent), bus_(bus), client_(client) {
    ringBuffer_.resize(kHistorySize);

    connect(bus_, &EventBus::dcSyncUpdate, this, &PerformanceMonitorService::onDcSyncUpdate);

    timer_ = new QTimer(this);
    connect(timer_, &QTimer::timeout, this, &PerformanceMonitorService::collectMetrics);
}
...
void PerformanceMonitorService::recordStartupPhase(const QString& phase, double durationMs) {
...
void PerformanceMonitorService::recordServiceMemory(const QString& serviceName, qint64 bytes) {
    serviceMemory_[serviceName] = bytes;
}

void PerformanceMonitorService::recordPluginMemory(const QString& pluginName, qint64 bytes) {
    pluginMemory_[pluginName] = bytes;
}

void PerformanceMonitorService::recordCacheMemory(const QString& cacheName, qint64 bytes) {
...
void PerformanceMonitorService::checkAlerts(const QJsonObject& metrics) {
    ...
    if (sdoRead > thresholds_.sdoLatencyMs) {
        emit performanceAlert("SDO Read", "Latency exceeded threshold", sdoRead, thresholds_.sdoLatencyMs);
    }
    if (sdoWrite > thresholds_.sdoLatencyMs) {
        emit performanceAlert("SDO Write", "Latency exceeded threshold", sdoWrite, thresholds_.sdoLatencyMs);
    }
    if (stateTrans > thresholds_.stateTransitionMs) {
        emit performanceAlert("State Transition", "Duration exceeded threshold", stateTrans,
                              thresholds_.stateTransitionMs);
    }
    if (freeRun > thresholds_.freeRunCycleUs) {
        emit performanceAlert("Free Run", "Cycle time exceeded threshold", freeRun, thresholds_.freeRunCycleUs);
    }
    if (uiUpdate > thresholds_.uiUpdateMs) {
        emit performanceAlert("UI Update", "Duration exceeded threshold", uiUpdate, thresholds_.uiUpdateMs);
    }
    if (memMB > thresholds_.memoryMB) {
        emit performanceAlert("Memory", "Usage exceeded threshold", memMB, thresholds_.memoryMB);
    }
}
