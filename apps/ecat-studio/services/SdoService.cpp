...
SdoService::SdoService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
...
void SdoService::download(int position, const QString& index, const QString& subIndex, const QString& value,
                          const QString& type) {
    Q_UNUSED(value);
    Q_UNUSED(type);
...
void SdoService::trackPendingOperation(int position, const QString& index, const QString& subIndex,
...
void SdoService::clearPendingOperation(int position, const QString& index, const QString& subIndex) {
    pendingOps_.removeIf(
        [&](const PendingOp& op) { return op.position == position && op.index == index && op.subIndex == subIndex; });
}
