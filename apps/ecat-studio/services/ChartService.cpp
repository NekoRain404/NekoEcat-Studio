...
int ChartService::createChart(const QString& type, const QString& title, const ChartData& data) {
...
bool ChartService::exportChart(int chartId, const QString& filePath) {
    ...
    p.drawText(image.rect(), Qt::AlignCenter,
               QStringLiteral("%1 chart with %2 datasets").arg(rec.type).arg(rec.data.datasets.size()));
    p.end();
