...
MasterManagerService::MasterManagerService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
    if (!client_)
        return;

    connect(client_, &EcatClient::masterText, this, [this](const QString& text) { updateFromMasterText(text); });
    ...
    connect(client_, &EcatClient::disconnected, this, [this]() { setState(MasterMgrState::Unknown); });

    connect(client_, &EcatClient::errorMessage, this, [this](const QString& msg) { emit masterError(msg); });
}

bool MasterManagerService::configureMaster(const MasterMgrConfig& config) {
    if (config.adapterName.trimmed().isEmpty() || config.cycleTime <= 0 || config.sync0Time < 0 ||
        config.watchdogTimeout < 0 || config.debugLevel < 0 || config.debugLevel > 3) {
    ...
    Q_UNUSED(config);
    emit masterError(QStringLiteral("Master configuration requires a backend acknowledgement"));
    return false;
}
...
bool MasterManagerService::restartMaster() {
    ...
    emit masterError(QStringLiteral("Master restart requires a backend acknowledgement"));
    return false;
}
