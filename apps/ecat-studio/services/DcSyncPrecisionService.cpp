...
DcSyncPrecisionService::DcSyncPrecisionService(EcatClient* client, EventBus* bus, QObject* parent)
    : QObject(parent), client_(client), bus_(bus) {
    pollTimer_ = new QTimer(this);
    pollTimer_->setInterval(500);
    connect(pollTimer_, &QTimer::timeout, this, &DcSyncPrecisionService::pollSyncData);

    if (bus_) {
        connect(bus_, &EventBus::dcSyncUpdate, this, &DcSyncPrecisionService::handleDcSyncUpdate);
    }
}
...
void DcSyncPrecisionService::processData(const QJsonObject& data) {
    ...
    for (const auto& entry : slaves) {
        ...
        auto it = std::find_if(slaveDrifts_.begin(), slaveDrifts_.end(),
                               [&](const DriftStatusEx& ex) { return ex.slave == ds.slave; });
        Q_UNUSED(it);
...
void DcSyncPrecisionService::computeJitterStats() {
    ...
    if (stats.histogramBinWidth < 1.0)
        stats.histogramBinWidth = 1.0;
...
void DcSyncPrecisionService::computeSyncQuality() {
    ...
    if (q.totalSlaves == 0) {
        ...
        double syncRatio = static_cast<double>(q.syncedSlaves) / q.totalSlaves;
        double driftPenalty = qMin(q.maxDrift / driftThreshold_, 1.0) * 30.0;
        double jitterPenalty = qMin(q.avgJitter / (driftThreshold_ * 0.5), 1.0) * 20.0;
