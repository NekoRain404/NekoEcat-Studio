...
DeviceManagerService::DeviceManagerService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
...
QJsonObject DeviceManagerService::deviceInfo(int position) const {
    for (const auto& dev : devices_) {
        if (dev["position"].toInt() == position)
            return dev;
    }
    return {};
}
...
int DeviceManagerService::deviceCount() const {
