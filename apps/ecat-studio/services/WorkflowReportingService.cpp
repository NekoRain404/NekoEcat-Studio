...
WorkflowReportingService::WorkflowReportingService(QObject *parent)
    : QObject(parent) {}

WfReport WorkflowReportingService::generateExecutionReport(
...
WfReport WorkflowReportingService::generatePerformanceReport(
    const QString &workflowId) {
  QVector<WfReportSection> sections;
  sections << makeSection("Performance Overview",
                           "Average execution time: 1250ms",
                           {{"avg_ms", 1250},
                            {"p95_ms", 2100},
                            {"throughput_per_sec", 0.8}});
  sections << makeSection("Bottleneck Analysis",
                           "Step 4 (Data Processing) accounts for 40% of total time.",
                           {{"bottleneck_step", "Step 4"},
  ...
  auto report = makeReport(workflowId, "Performance Report",
                           "Performance analysis for " + workflowId,
                           sections, recs);

  report.charts << makeChart("Performance Distribution", "histogram",
                             {{"mean", 1250}, {"stddev", 350}});
  report.tables << makeTable(
      "Step Performance", {"Step", "Avg (ms)", "P95 (ms)", "% Total"},
      {{"Step 1", "150", "200", "12%"}, {"Step 2", "200", "350", "16%"},
       {"Step 3", "100", "180", "8%"}, {"Step 4", "500", "800", "40%"}});

  emit reportGenerated(report);
  return report;
}

WfReport WorkflowReportingService::generateErrorReport(
    const QString &workflowId) {
  QVector<WfReportSection> sections;
  sections << makeSection("Error Summary",
                           "2 errors encountered during execution",
                           {{"total_errors", 2},
                            {"critical", 0},
                            {"warnings", 2}});
  sections << makeSection(
      "Error Details",
      "Timeout on step 5 (recovered via retry). Validation warning on step 7.",
  ...
  auto report = makeReport(workflowId, "Error Report",
                           "Error analysis for " + workflowId, sections, recs);

  report.tables << makeTable(
      "Error Log", {"Step", "Type", "Message", "Severity"},
...
WfReport WorkflowReportingService::generateResourceReport(
    const QString &workflowId) {
  QVector<WfReportSection> sections;
  sections << makeSection("Resource Utilization",
                           "Average CPU: 65%, Memory: 512MB, Network: 10Mbps",
                           {{"avg_cpu_percent", 65},
                            {"avg_memory_mb", 512},
                            {"avg_network_mbps", 10}});
  sections << makeSection("Resource Bottlenecks",
                           "Memory usage peaked at 1.2GB during step 4.",
                           {{"peak_memory_mb", 1200},
  ...
  auto report = makeReport(workflowId, "Resource Report",
                           "Resource utilization for " + workflowId,
                           sections, recs);

  report.charts << makeChart("Resource Usage Over Time", "line",
                             {{"metrics", QJsonArray{"cpu", "memory", "network"}}});
  report.tables << makeTable(
      "Resource Summary", {"Resource", "Avg", "Peak", "Capacity"},
      {{"CPU", "65%", "90%", "100%"}, {"Memory", "512MB", "1.2GB", "2GB"},
...
WfReport WorkflowReportingService::makeReport(
    const QString &workflowId, const QString &title, const QString &summary,
...
WfReportSection WorkflowReportingService::makeSection(const QString &title,
...
WfReportChart WorkflowReportingService::makeChart(const QString &title,
                                                  const QString &type,
                                                  const QJsonObject &data) {
  WfReportChart chart;
  chart.title = title;
...
WfReportTable WorkflowReportingService::makeTable(
    const QString &title, const QStringList &headers,
