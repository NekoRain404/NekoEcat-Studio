...
SdoCacheService::SdoCacheService(QObject *parent)
    : QObject(parent) {
  clock_.start();
}

QString SdoCacheService::makeKey(const QString &index,
                                 const QString &subIndex) const {
  return index + QStringLiteral(":") + subIndex;
}

void SdoCacheService::cacheDictionary(int position,
                                       const SdoDictionary &dict) {
...
void SdoCacheService::cacheValue(int position, const QString &index,
...
QString SdoCacheService::getCachedValue(int position, const QString &index,
                                         const QString &subIndex) const {
  QWriteLocker locker(&lock_);
  const QString key = makeKey(index, subIndex);
  ...
  if (config_.ttlMs > 0 &&
      (now - it.value()->entry.timestampMs) > config_.ttlMs) {
    ++const_cast<SdoCacheService *>(this)->misses_;
    return {};
  }
...
void SdoCacheService::evictExpired(int position) {
  ...
  while (node != order.rend() && config_.ttlMs > 0 &&
         (now - node->entry.timestampMs) > config_.ttlMs) {
    idx.remove(node->key);
