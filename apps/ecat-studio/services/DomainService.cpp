...
bool DomainService::registerPdoEntry(int domain, int position, int index, int subIndex) {
    ...
    for (const auto& existing : entries_[domain]) {
        if (existing.position == position && existing.index == index && existing.subIndex == subIndex) {
