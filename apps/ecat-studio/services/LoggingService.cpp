...
void LoggingService::log(LogLevel level, LogCategory category, const QString& message, const QString& source) {
    if (level < minLevel_) {
        return;
    }
...
bool LoggingService::exportLogs(const QString& filePath) const {
    ...
    for (const auto& entry : entries_) {
        out << entry.timestamp.toString(Qt::ISODate) << " [" << levelToString(entry.level) << "] ["
            << categoryToString(entry.category) << "] " << (entry.source.isEmpty() ? "" : entry.source + ": ")
            << entry.message << "\n";
    }
...
void LoggingService::writeToFile(const LogEntry& entry) {
    ...
    QString line = entry.timestamp.toString(Qt::ISODate) + " [" + levelToString(entry.level) + "] [" +
                   categoryToString(entry.category) + "] " + (entry.source.isEmpty() ? "" : entry.source + ": ") +
                   entry.message + "\n";
