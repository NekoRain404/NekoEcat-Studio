...
FreeRunMonitoringService::FreeRunMonitoringService(EcatClient* client, EventBus* bus, QObject* parent)
    : QObject(parent), client_(client), bus_(bus) {
    pollTimer_ = new QTimer(this);
    pollTimer_->setInterval(100);
    connect(pollTimer_, &QTimer::timeout, this, &FreeRunMonitoringService::pollData);
...
void FreeRunMonitoringService::updatePerformance(const FreeRunPerformanceMetrics& metrics) {
