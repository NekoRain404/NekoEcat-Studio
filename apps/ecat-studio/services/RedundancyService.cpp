...
RedundancyService::RedundancyService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
    ...
    connect(client_, &EcatClient::redundancyStatusResult, this, [this](const QJsonObject& data) {
    ...
    connect(client_, &EcatClient::redundancyHistoryResult, this,
            [this](const QJsonObject& data) { emit historyReceived(data); });
...
RedundancyState RedundancyService::currentState() const {
    return state_;
}
RedundancyPath RedundancyService::primaryPath() const {
    return primaryPath_;
}
RedundancyPath RedundancyService::secondaryPath() const {
    return secondaryPath_;
}
QVector<RedundancyEvent> RedundancyService::redundancyHistory() const {
    return history_;
}
bool RedundancyService::isRedundant() const {
    return state_ == RedundancyState::DualPath;
}
