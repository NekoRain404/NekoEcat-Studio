...
BatchOperationService::BatchOperationService(EcatClient* client, SdoService* sdo, TopologyService* topology,
                                             QObject* parent)
...
void BatchOperationService::cancelBatch() {
    cancelled_ = true;
}

int BatchOperationService::progress() const {
    return progress_;
}
...
BatchResult BatchOperationService::executeWriteSDO(const BatchOperation& op) {
    ...
    for (int i = 0; i < op.writes.size(); ++i) {
        ...
        sdo_->download(item.position, item.index, item.subIndex, item.value, item.type);
        r.completedItems = i + 1;
        progress_ = (i + 1) * 100 / r.totalItems;
        emit batchProgress(progress_);
    }
...
BatchResult BatchOperationService::executeScanTopology(const BatchOperation& op) {
    BatchResult r;
