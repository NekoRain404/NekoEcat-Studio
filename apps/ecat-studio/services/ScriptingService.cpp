...
ScriptingBuiltin::ScriptingBuiltin(EcatClient* client, SdoService* sdo, TopologyService* topology, QJSEngine* engine,
                                   QObject* parent)
    : QObject(parent), client_(client), sdo_(sdo), topology_(topology), engine_(engine) {}

QJSValue ScriptingBuiltin::readSDO(int position, const QString& index, const QString& subIndex) {
...
bool ScriptingBuiltin::writeSDO(int position, const QString& index, const QString& subIndex, const QString& value,
                                const QString& type) {
    if (!client_->isConnected())
        return false;
...
void ScriptingBuiltin::wait(int ms) {
    ...
    if (ms > kMaxWaitMs) {
        emit logMessage(
            QStringLiteral("wait(%1ms) exceeded the 10s cap; the GUI stayed responsive by slicing the wait").arg(ms));
    }
}
...
ScriptingService::ScriptingService(EcatClient* client, SdoService* sdo, TopologyService* topology, QObject* parent)
    : QObject(parent) {
#ifdef ECAT_SCRIPTING_ENABLED
    engine_ = new QJSEngine(this);
    builtins_ = new ScriptingBuiltin(client, sdo, topology, engine_, this);
    connect(builtins_, &ScriptingBuiltin::logMessage, this, &ScriptingService::logMessage);
    installBuiltins();
#else
    Q_UNUSED(client);
    Q_UNUSED(sdo);
...
void ScriptingService::installBuiltins() {
    ...
    global.setProperty(QStringLiteral("readSDO"), builtinObj.property(QStringLiteral("readSDO")));
    global.setProperty(QStringLiteral("writeSDO"), builtinObj.property(QStringLiteral("writeSDO")));
    global.setProperty(QStringLiteral("scanTopology"), builtinObj.property(QStringLiteral("scanTopology")));
    global.setProperty(QStringLiteral("setState"), builtinObj.property(QStringLiteral("setState")));
    global.setProperty(QStringLiteral("wait"), builtinObj.property(QStringLiteral("wait")));
    global.setProperty(QStringLiteral("log"), builtinObj.property(QStringLiteral("log")));
    global.setProperty(QStringLiteral("alert"), builtinObj.property(QStringLiteral("alert")));
}
#endif
...
QJSValue ScriptingService::executeScript(const QString& script) {
    ...
    if (result.isError()) {
        QString errMsg = result.toString() + " (line " + result.property(QStringLiteral("lineNumber")).toString() + ")";
...
QVariant ScriptingService::executeScript(const QString& script) {
    Q_UNUSED(script);
    emit scriptError(QStringLiteral("<inline>"), QStringLiteral("Scripting not available: Qt6::Qml not installed"));
    return QVariant();
}

QVariant ScriptingService::executeScriptFile(const QString& filePath) {
    Q_UNUSED(filePath);
    emit scriptError(filePath, QStringLiteral("Scripting not available: Qt6::Qml not installed"));
    return QVariant();
}
#endif
...
void ScriptingService::abortRunning() {
