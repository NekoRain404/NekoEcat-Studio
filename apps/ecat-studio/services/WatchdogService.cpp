...
WatchdogService::WatchdogService(EventBus* bus, EcatClient* client, QObject* parent)
    : QObject(parent), bus_(bus), client_(client) {
    connect(bus_, &EventBus::topologyChanged, this, &WatchdogService::onTopologyChanged);
...
void WatchdogService::updateSlaveStatuses(const QVector<SlaveInfo>& slaves) {
    ...
    for (const auto& slave : slaves) {
        ...
        auto it = std::find_if(slaveStatuses_.begin(), slaveStatuses_.end(), [&](const WatchdogSlaveStatus& existing) {
