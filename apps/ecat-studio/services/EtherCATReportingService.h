...
class EtherCATReportingService : public QObject {
    Q_OBJECT
public:
    explicit EtherCATReportingService(EventBus* bus, EcatClient* client, QObject* parent = nullptr);
    ...
    bool exportReport(const Report& report, const QString& filePath, const QString& format);

signals:
    void reportGenerated(const Report& report);

private:
    Report makeReport(const QString& title, const QString& summary, const QVector<ReportSection>& sections,
