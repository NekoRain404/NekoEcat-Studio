...
EcatHealthService::EcatHealthService(EcatClient *client, EventBus *bus,
                                     ...
                                     QObject *parent)
    : QObject(parent), client_(client), bus_(bus), topology_(topology),
      dcSync_(dcSync), alEvent_(alEvent), watchdog_(watchdog) {
...
void EcatHealthService::stopMonitoring() { pollTimer_->stop(); }

bool EcatHealthService::isMonitoring() const { return pollTimer_->isActive(); }

MasterState EcatHealthService::masterState() const { return master_; }
...
DcSyncStatus EcatHealthService::dcSyncStatus() const { return dcSyncStatus_; }

AlEventStatus EcatHealthService::alEventStatus() const { return alEventStatus_; }

WatchdogStatus EcatHealthService::watchdogStatus() const { return watchdogStatus_; }

HealthScore EcatHealthService::overallHealth() const { return health_; }
...
void EcatHealthService::poll() {
  ...
  for (const auto &s : slaves_) {
    if (s.state == "OP") ++op;
    else if (s.state == "SAFEOP") ++safeOp;
    else if (s.state == "PREOP") ++preOp;
    else if (s.state == "INIT") ++init;
    if (s.hasError) ++err;
  }
  ...
  if (preOp > 0) score -= preOp * 8;
  if (safeOp > 0) score -= safeOp * 3;
  if (watchdogStatus_.triggered) score -= 20;
  if (!dcSyncStatus_.inSync) score -= 5;
  ...
  health_ = {score,   grade,   summary, total,  op,
             safeOp,  preOp,   init,    err};

  for (int i = 0; i < slaves_.size(); ++i) {
    bool changed = i >= prevSlaves.size() ||
                   slaves_[i].state != prevSlaves[i].state ||
                   slaves_[i].hasError != prevSlaves[i].hasError;
    if (changed) emit stateChanged(slaves_[i].position, slaves_[i]);
  }
