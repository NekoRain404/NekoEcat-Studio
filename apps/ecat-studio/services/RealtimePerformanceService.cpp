...
RealtimePerformanceService::RealtimePerformanceService(EcatClient* client, QObject* parent)
...
LatencyMetrics RealtimePerformanceService::latency() const {
    return latency_;
}
ThroughputMetrics RealtimePerformanceService::throughput() const {
    return throughput_;
}
ResourceMetrics RealtimePerformanceService::resources() const {
    return resources_;
}
QualityAssessment RealtimePerformanceService::quality() const {
    return quality_;
}
...
void RealtimePerformanceService::measureThroughput() {
    ...
    throughput_.utilizationPercent = qBound(0.0, throughput_.bytesPerSecond / (100.0 * 1024 * 1024) * 100.0, 100.0);
...
void RealtimePerformanceService::assessQuality() {
    ...
    quality_.packetLossPercent = throughput_.totalFrames > 0
                                     ? static_cast<double>(throughput_.totalErrors) / throughput_.totalFrames * 100.0
                                     : 0.0;
