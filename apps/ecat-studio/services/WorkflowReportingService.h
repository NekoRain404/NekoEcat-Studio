...
private:
  WfReport makeReport(const QString &workflowId, const QString &title,
                      const QString &summary,
                      const QVector<WfReportSection> &sections,
                      const QStringList &recommendations);
  WfReportSection makeSection(const QString &title, const QString &content,
                              const QJsonObject &data = {});
  WfReportChart makeChart(const QString &title, const QString &type,
                          const QJsonObject &data);
  WfReportTable makeTable(const QString &title, const QStringList &headers,
