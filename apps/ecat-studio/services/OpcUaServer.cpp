...
bool OpcUaServer::start(quint16 port) {
    ...
    if (!server_) { qWarning() << "OPC UA: failed to create server"; return false; }
    ...
    if (retval != UA_STATUSCODE_GOOD) {
        ...
        UA_Server_delete(server_); server_ = nullptr; return false;
    }
...
void OpcUaServer::stop() {
    if (!running_) return;
    running_ = false;
    if (sdoPollTimer_) sdoPollTimer_->stop();
    QThread::msleep(200);
    if (server_) {
        UA_Server_run_shutdown(server_);
        UA_Server_delete(server_); server_ = nullptr; config_ = nullptr;
    }
...
void OpcUaServer::addSlaveNodes() {
    ...
    for (int i = 0; i < slaves_.size(); ++i) {
        ...
        auto addStatic = [&](UA_UInt32 sub, const char *n, const QString &v) {
            ...
            UA_Server_addVariableNode(server_, nid, slaveNodeId,
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), q,
                                      UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
                                      va, nullptr, nullptr);
            UA_String_clear(&us); UA_QualifiedName_clear(&q);
        };
...
void OpcUaServer::addSdoNodes(int slaveIndex, UA_UInt32 nodeBase,
                               const UA_NodeId &) {
    struct SdoDef { const char *name; const char *index; const char *sub; };
    static const SdoDef sdos[] = {
        {"DeviceType", "0x1000", "0"}, {"ErrorRegister", "0x1001", "0"},
        {"DeviceName", "0x1008", "0"}, {"VendorID", "0x1018", "1"},
        {"ProductCode", "0x1018", "2"}, {"Revision", "0x1018", "3"},
        {"SerialNo", "0x1018", "4"},
    };
    ...
    UA_Server_addObjectNode(server_, UA_NODEID_NUMERIC(NS_ID, nodeBase + 9),
                            UA_NODEID_NUMERIC(NS_ID, nodeBase),
                            UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), sdoQn,
                            UA_NODEID_NUMERIC(0, UA_NS0ID_FOLDERTYPE),
    ...
    for (int s = 0; s < 7; ++s) {
        ...
        { QMutexLocker lock(&sdoCacheMutex_); sdoCache_.insert(key, QStringLiteral("—")); }

        MonitoredSdo ms;
        ms.slavePos = slaveIndex; ms.index = def.index; ms.subIndex = def.sub;
        ...
        UA_Server_addVariableNode(server_,
                                  UA_NODEID_NUMERIC(NS_ID, nodeBase + 20 + s),
                                  UA_NODEID_NUMERIC(NS_ID, nodeBase + 9),
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_ORGANIZES), varQn,
                                  UA_NODEID_NUMERIC(0, UA_NS0ID_BASEDATAVARIABLETYPE),
                                  va, new QString(key), nullptr);
...
void OpcUaServer::updateSlaves(const QVector<SlaveInfo> &slaves) {
    { QMutexLocker lock(&mutex_); slaves_ = slaves; }
    removeSlaveNodes();
    addSlaveNodes();
}