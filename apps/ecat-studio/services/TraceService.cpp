...
TraceService::TraceService(EcatClient* client, QObject* parent)
    ...
    if (client_) {
        connect(client_, &EcatClient::sdoValue, this, &TraceService::onSdoValue);
    }
}

int TraceService::addChannel(const QString& name, int slave, const QString& index, const QString& subIndex) {
    ...
    ch.name = name.isEmpty() ? QStringLiteral("CH%1 [%2:%3.%4]").arg(ch.id).arg(slave).arg(index, subIndex) : name;
    channels_.append(ch);
...
bool TraceService::isTracing() const {
    return tracing_;
}
...
int TraceService::sampleRate() const {
    return sampleRate_;
}
...
int TraceService::bufferSize() const {
    return bufferSize_;
}
...
TraceTriggerMode TraceService::triggerMode() const {
    return triggerMode_;
}
...
void TraceService::tick() {
    ...
    for (const auto& ch : channels_) {
        QString key = QStringLiteral("%1:%2:%3").arg(ch.slave).arg(ch.index).arg(ch.subIndex);
        pendingRequests_.insert(key, ch.id);
        client_->upload(ch.slave, ch.index, ch.subIndex);
    }
}

void TraceService::onSdoValue(int position, const QString& index, const QString& subIndex, const QString& value) {
    if (!tracing_)
        return;

    QString key = QStringLiteral("%1:%2:%3").arg(position).arg(index).arg(subIndex);
