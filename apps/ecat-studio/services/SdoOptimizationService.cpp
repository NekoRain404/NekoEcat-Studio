...
SdoOptimizationService::SdoOptimizationService(EcatClient* client, EventBus* bus, QObject* parent)
    : QObject(parent), client_(client), bus_(bus) {}

SdoOptimizationResult SdoOptimizationService::createRejectedResult(const QString& category,
                                                                   const QStringList& recommendations) {
    SdoOptimizationResult r;
    r.category = category;
    r.description = tr("%1 optimization requires a connected SDO optimization backend").arg(category);
