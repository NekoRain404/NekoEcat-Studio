...
void BusStatsService::stopMonitoring() { pollTimer_->stop(); }

bool BusStatsService::isMonitoring() const { return pollTimer_->isActive(); }

BusStats BusStatsService::currentStats() const { return stats_; }
...
void BusStatsService::poll() {
  ...
  if (prevTxFrames_ > 0) {
    ...
    stats_.bandwidthMbps =
        (static_cast<double>(deltaTx + deltaRx) * 12000.0) / 1000000.0;
  }
