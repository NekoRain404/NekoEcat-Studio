...
AdvancedErrorAnalysisService::AdvancedErrorAnalysisService(QObject* parent) : QObject(parent) {}
...
void AdvancedErrorAnalysisService::clearHistory() {
    history_.clear();
}

QVector<ErrorPattern> AdvancedErrorAnalysisService::detectPatterns(const QVector<AdvancedErrorInfo>& errors) {
    ...
    for (auto it = byCategory.begin(); it != byCategory.end(); ++it) {
        ...
        for (int i = 1; i < indices.size(); ++i) {
            qint64 ms = errors[indices[i - 1]].timestamp.msecsTo(errors[indices[i]].timestamp);
            if (ms > 0 && ms < 60000) {
        ...
        pat.description = QStringLiteral("Repeated %1 errors (%2 occurrences)").arg(it.key()).arg(indices.size());
...
CorrelationMatrix AdvancedErrorAnalysisService::analyzeCorrelation(const QVector<AdvancedErrorInfo>& errors) {
    ...
    for (int i = 0; i < types.size(); ++i) {
        for (int j = i + 1; j < types.size(); ++j) {
            ...
            if (coCount > 0) {
                ...
                entry.correlation = qBound(0.0, static_cast<double>(coCount) / qMax(typeCounts[a], typeCounts[b]), 1.0);
                entry.relationship = entry.correlation > 0.7   ? QStringLiteral("Strong")
                                     : entry.correlation > 0.3 ? QStringLiteral("Moderate")
                                                               : QStringLiteral("Weak");
    ...
    std::sort(matrix.entries.begin(), matrix.entries.end(),
              [](const CorrelationEntry& a, const CorrelationEntry& b) { return a.correlation > b.correlation; });

    for (int i = 0; i < qMin(5, matrix.entries.size()); ++i)
        matrix.strongestCorrelations.append(QStringLiteral("%1 <-> %2 (%3)")
...
QVector<ErrorPrediction> AdvancedErrorAnalysisService::predictErrors(const QVector<AdvancedErrorInfo>& errors) {
    ...
    for (auto it = timestamps.begin(); it != timestamps.end(); ++it) {
        ...
        double probability = qBound(0.0, static_cast<double>(timeSinceLast) / avgGap, 1.0);
    ...
    std::sort(predictions.begin(), predictions.end(),
              [](const ErrorPrediction& a, const ErrorPrediction& b) { return a.probability > b.probability; });

    return predictions;
}

RootCauseAnalysis AdvancedErrorAnalysisService::analyzeRootCause(const AdvancedErrorInfo& error) {
    ...
    if (error.category == "Communication") {
        ...
        rca.confidence = 0.85;
        rca.contributingFactors = {"Cable disconnection", "EMI interference", "Switch port failure"};
        rca.recommendedActions = {"Check cable connections", "Verify switch port status", "Run cable diagnostics"};
    } else if (error.category == "Device") {
        rca.rootCause = "Slave device failure";
        rca.confidence = 0.80;
        rca.contributingFactors = {"Device firmware issue", "Power supply problem", "Hardware fault"};
        rca.recommendedActions = {"Check device power supply", "Update device firmware",
                                  "Replace device if persistent"};
    } else if (error.category == "Configuration") {
        rca.rootCause = "Configuration mismatch";
        rca.confidence = 0.90;
        rca.contributingFactors = {"Invalid SDO parameter", "PDO mapping conflict", "DC sync configuration error"};
        rca.recommendedActions = {"Validate SDO parameters", "Check PDO mapping consistency",
                                  "Verify DC sync settings"};
    } else if (error.category == "Protocol") {
        rca.rootCause = "Protocol violation";
        rca.confidence = 0.75;
        rca.contributingFactors = {"Frame corruption", "Timing violation", "State machine error"};
        rca.recommendedActions = {"Check frame integrity", "Verify timing parameters",
        ...
        rca.recommendedActions = {"Collect more error data", "Enable detailed logging"};
    }

    for (const auto& e : history_) {
        if (e.category == error.category && e.id != error.id && !rca.relatedErrors.contains(e.message))
            rca.relatedErrors.append(e.message);
        if (rca.relatedErrors.size() >= 5)
            break;
    }

    rca.explanation =
        QStringLiteral("Error '%1' at slave %2: %3").arg(error.message).arg(error.slavePosition).arg(rca.rootCause);
