...
WorkflowAnalyticsService::WorkflowAnalyticsService(QObject *parent)
    : QObject(parent)
{
}

void WorkflowAnalyticsService::recordExecution(const QString &workflowId,
...
void WorkflowAnalyticsService::recordError(const QString &workflowId,
...
void WorkflowAnalyticsService::recordResourceUsage(const QString &workflowId,
                                                     double cpuPercent,
                                                     double memoryMb,
                                                     double networkKbps)
{
...
WfPerformanceAnalysis WorkflowAnalyticsService::analyzePerformance(const QString &workflowId)
{
  ...
  if (p95Index >= durations.size()) p95Index = durations.size() - 1;
  analysis.p95StepDurationMs = durations[p95Index];

  double totalTimeSec = sum / 1000.0;
  analysis.throughputPerSecond = (totalTimeSec > 0.0)
      ? static_cast<double>(records.size()) / totalTimeSec : 0.0;

  analysis.parallelismEfficiency = (analysis.p95StepDurationMs > 0.0)
      ? analysis.averageStepDurationMs / analysis.p95StepDurationMs * 100.0 : 100.0;
...
WfErrorAnalysis WorkflowAnalyticsService::analyzeErrors(const QString &workflowId)
{
  ...
  analysis.errorRate = execs.isEmpty() ? 0.0
      : static_cast<double>(records.size()) / execs.size() * 100.0;
...
WfResourceAnalysis WorkflowAnalyticsService::analyzeResources(const QString &workflowId)
{
  ...
  if (!analysis.overutilizedResources.isEmpty())
    analysis.recommendations << QStringLiteral("Resource contention on: %1").arg(analysis.overutilizedResources.join(", "));
  if (!analysis.underutilizedResources.isEmpty())
    analysis.recommendations << QStringLiteral("Underutilized resources: %1 — consider scaling down").arg(analysis.underutilizedResources.join(", "));
