...
PdoValidationResult
PdoMappingService::validateMapping(const PdoMapping &mapping) const {
  ...
  if (mapping.bitSize > r.maxBitSize) {
    r.valid = false;
    r.errorMessage = QStringLiteral("Bit size exceeds maximum (%1)")
                         .arg(r.maxBitSize);
    return r;
  }
...
bool PdoMappingService::exportMapping(int position,
  ...
  for (const auto &m : *it) {
    ...
    obj[QStringLiteral("direction")] =
        m.direction == PdoDirection::Output ? QStringLiteral("output")
...
bool PdoMappingService::importMapping(int position,
  ...
  for (const auto &v : arr) {
    ...
    m.direction = obj[QStringLiteral("direction")].toString() ==
...
MappingLayout PdoMappingService::getMappingLayout(int position) const {
  ...
  auto it = mappings_.constFind(position);
  if (it == mappings_.constEnd()) return layout;
