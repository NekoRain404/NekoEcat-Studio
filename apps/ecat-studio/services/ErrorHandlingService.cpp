...
ErrorHandlingService::ErrorHandlingService(QObject* parent) : QObject(parent) {}

QVector<EcatErrorInfo> ErrorHandlingService::detectErrors() {
    QVector<EcatErrorInfo> active;
    for (const auto& e : errors_) {
        if (e.severity == EcatErrorSeverity::Error || e.severity == EcatErrorSeverity::Critical) {
            active.append(e);
        }
    }
    return active;
...
int ErrorHandlingService::reportError(int position, int errorCode, const QString& message, EcatErrorSeverity severity,
                                      EcatErrorCategory category, const QString& recoveryAction) {
    EcatErrorInfo info;
    info.id = nextId_++;
