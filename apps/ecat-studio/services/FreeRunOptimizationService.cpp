...
FreeRunOptimizationResult FreeRunOptimizationService::createRejectedResult(
    const QString &category, const QStringList &recommendations) {
  FreeRunOptimizationResult r;
  r.category = category;
  r.description = tr("%1 optimization requires a connected Free Run optimization backend")
                      .arg(category);
  r.improvement = 0.0;
