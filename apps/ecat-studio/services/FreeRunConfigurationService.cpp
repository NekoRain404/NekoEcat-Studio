...
FreeRunConfigurationService::FreeRunConfigurationService(EcatClient* client, EventBus* bus, QObject* parent)
    : QObject(parent), client_(client), bus_(bus) {}

bool FreeRunConfigurationService::configureProcessData(const ProcessDataConfig& config) {
...
bool FreeRunConfigurationService::configureDataMapping(const DataMappingConfig& config) {
    if (!config.autoMap && config.inputOffsets.isEmpty() && config.outputOffsets.isEmpty())
        return false;
...
bool FreeRunConfigurationService::configureErrorHandling(const ErrorHandlingConfig& config) {
