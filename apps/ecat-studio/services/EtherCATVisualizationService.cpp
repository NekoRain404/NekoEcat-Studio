...
EtherCATVisualizationService::EtherCATVisualizationService(
...
QGraphicsScene *EtherCATVisualizationService::createTopologyView(
    const QVector<SlaveInfo> &slaves)
{
    auto *scene = makeScene();

    QColor masterColor = config_.colors.value(
        QStringLiteral("master"), QColor(60, 120, 200));
    QColor slaveColor = config_.colors.value(
        QStringLiteral("slave"), QColor(80, 180, 80));
    QColor lineColor = config_.colors.value(
        QStringLiteral("line"), QColor(100, 100, 100));
...
QGraphicsScene *EtherCATVisualizationService::createDataView(
    const QVector<DataPoint> &data)
{
    ...
    QColor barColor = config_.colors.value(
        QStringLiteral("bar"), QColor(60, 140, 220));
...
QGraphicsScene *EtherCATVisualizationService::createPerformanceView(
    const PerformanceMetrics &metrics)
{
    auto *scene = makeScene();

    QColor goodColor = config_.colors.value(
        QStringLiteral("good"), QColor(80, 180, 80));
    QColor warnColor = config_.colors.value(
        QStringLiteral("warning"), QColor(220, 180, 40));
    QColor critColor = config_.colors.value(
        QStringLiteral("critical"), QColor(220, 60, 60));

    auto addMetric = [&](double x, double y, const QString &name,
        ...
        if (name.contains(QStringLiteral("jitter"), Qt::CaseInsensitive) ||
            ...
            if (value > 10.0) color = critColor;
            else if (value > 1.0) color = warnColor;
        }
        ...
        auto *val = new QGraphicsTextItem(
...
QGraphicsScene *EtherCATVisualizationService::createErrorView(
    const QVector<ErrorInfo> &errors)
{
    auto *scene = makeScene();
    ...
    QColor errColor = config_.colors.value(
        QStringLiteral("error"), QColor(220, 60, 60));
    ...
    for (int i = 0; i < count; ++i) {
        ...
        auto *text = new QGraphicsTextItem(
            QStringLiteral("[%1] Port %2: %3 - %4")
                .arg(err.timestampMs)
                .arg(err.port)
                .arg(err.type, err.description));
        text->setPos(15, y);
    ...
    if (errors.size() > count) {
        auto *more = new QGraphicsTextItem(
            QStringLiteral("... and %1 more errors").arg(errors.size() - count));
        more->setPos(15, y);
        scene->addItem(more);
    }
