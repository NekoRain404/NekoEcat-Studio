...
QString AsyncOperationManager::execute(const QString& name, OperationFunc func, OperationPriority priority,
                                       int timeoutMs) {
    ...
    if (runningCount_ < maxConcurrent_) {
        ...
        QThreadPool::globalInstance()->start([manager, opId, opFunc, cancelled]() {
            ...
            QMetaObject::invokeMethod(
                manager, [manager, opId, result]() { manager->onOperationFinished(opId, result); },
                Qt::QueuedConnection);
        });
    ...
    if (timeoutMs > 0) {
        QTimer::singleShot(timeoutMs, this, [this, id]() { cancel(id); });
    }
...
OperationResult AsyncOperationManager::result(const QString& operationId) const {
    QMutexLocker locker(&mutex_);
    auto it = operations_.find(operationId);
    return it != operations_.end() ? (*it)->result : OperationResult{false, {}, "Not found"};
}
...
void AsyncOperationManager::processQueue() {
    while (runningCount_ < maxConcurrent_ && !queue_.isEmpty()) {
        ...
        for (int i = 0; i < queue_.size(); ++i) {
            ...
            if (cand->state == OperationState::Cancelled)
                continue;
        ...
        if (it == operations_.end() || (*it)->state == OperationState::Cancelled)
            continue;
...
void AsyncOperationManager::onOperationFinished(const QString& operationId, const OperationResult& result) {
