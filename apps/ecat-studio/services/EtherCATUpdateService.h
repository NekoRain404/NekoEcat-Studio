...
class EtherCATUpdateService : public QObject {
    Q_OBJECT
public:
    explicit EtherCATUpdateService(EventBus* bus, EcatClient* client, QObject* parent = nullptr);
...
private:
    UpdateResult makeResult(int position, const QString& version, const QString& status, int progress,
                            const QString& log);
