...
class EtherCATMaintenanceService : public QObject {
    Q_OBJECT
public:
    explicit EtherCATMaintenanceService(EventBus* bus, EcatClient* client, QObject* parent = nullptr);

    MaintenanceTaskInfo scheduleTask(const QString& taskType, const QString& schedule);
...
private:
    MaintenanceTaskInfo makeTask(const QString& id, const QString& taskType, const QString& schedule,
