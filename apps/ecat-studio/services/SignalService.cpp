...
SignalService::SignalService(EventBus *bus, QObject *parent)
...
int SignalService::addChannel(const QString &name, int slave,
...
ChannelStats SignalService::stats(int channelId) const {
  ...
  for (const auto &ch : channels_) {
    if (ch.id != channelId || ch.values.isEmpty()) return s;
...
void SignalService::pushData(int channelId, const QVector<double> &values,
...
void SignalService::handleSignalData(int channel, const QVector<double> &values,
                                     const QVector<qint64> &timestamps) {
