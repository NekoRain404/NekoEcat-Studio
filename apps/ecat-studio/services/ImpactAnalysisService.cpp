...
QStringList freeRunImpactDetails(int selectedPosition, const QVector<SlaveInfo>& slaves, const QTableWidget* pdoTable,
    ...
    details << uiText("Selected slave context: #%1 %2, state %3", "选中从站上下文：#%1 %2，状态 %3")
    ...
    if (pdoTable) {
        ...
        for (int row = 0; row < pdoTable->rowCount(); ++row) {
            ...
            const QString address = QString("%1:%2").arg(tableText(const_cast<QTableWidget*>(pdoTable), row, 2),
    ...
    if (pdoRows <= 0) {
        ...
        if (!rxPreview.isEmpty()) {
            details << uiText("Output preview: %1", "输出预览：%1").arg(rxPreview.join(" | "));
        }
    }
    ...
    if (freeRunEntryTable && freeRunEntryTable->rowCount() > 0) {
        ...
        for (int row = 0; row < freeRunEntryTable->rowCount(); ++row) {
            const QString direction = tableText(const_cast<QTableWidget*>(freeRunEntryTable), row, 2).toLower();
            if (direction.contains("rx") || direction.contains("out")) {
                ++outputEntries;
            } else if (direction.contains("tx") || direction.contains("in")) {
                ++inputEntries;
            }
            const QString meaning = tableText(const_cast<QTableWidget*>(freeRunEntryTable), row, 12);
        ...
        if (!meaningPreview.isEmpty()) {
            details << uiText("Decoded evidence: %1", "解析证据：%1").arg(meaningPreview.join(" | "));
        }
    }
    ...
    if (watchTable && selectedPosition >= 0) {
        for (int row = 0; row < watchTable->rowCount(); ++row) {
            const int rowPosition = tableText(const_cast<QTableWidget*>(watchTable), row, 1).toInt();
            if (rowPosition != selectedPosition) {
                continue;
            }
            const QString index = normalizeHexText(tableText(const_cast<QTableWidget*>(watchTable), row, 2), 4);
            const QString value = tableText(const_cast<QTableWidget*>(watchTable), row, 4);
            const QString decoded = tableText(const_cast<QTableWidget*>(watchTable), row, 5);
            if (index == "0x6041" && !decoded.isEmpty()) {
                statusword = decoded;
            } else if (index == "0x603f" && !decoded.isEmpty() && !value.isEmpty() && value != "0" &&
    ...
    if (!statusword.isEmpty() || !errorCode.isEmpty()) {
        ...
        details << uiText("Drive Watch evidence: %1", "驱动 Watch 证据：%1").arg(driveFacts.join(" | "));
    } else {
        details << uiText("Drive Watch evidence: no CiA 402 status/error watch rows",
    ...
    details << uiText("GUI refresh will switch to 500 ms while Free Run is active.",
...
QStringList stateTransitionImpactDetails(int position, const QString& requestedState, int loadedSlaveInfoPosition,
    ...
    const int identityRows = identityLoaded && identityTable ? identityTable->rowCount() : 0;
    ...
    if (watchTable) {
        for (int row = 0; row < watchTable->rowCount(); ++row) {
            if (tableText(const_cast<QTableWidget*>(watchTable), row, 1).toInt() != position) {
                continue;
            }
            ++watchRows;
            const QString value = tableText(const_cast<QTableWidget*>(watchTable), row, 4);
    ...
    if (startupSdoTable) {
        for (int row = 0; row < startupSdoTable->rowCount(); ++row) {
            if (tableText(const_cast<QTableWidget*>(startupSdoTable), row, 0).toInt() != position) {
                continue;
            }
            ++startupRows;
            if (hasStartupDiffEvidence(tableText(const_cast<QTableWidget*>(startupSdoTable), row, 8))) {
                ++startupDiffs;
            }
        }
    }
    ...
    if (freeRunEntryTable) {
        for (int row = 0; row < freeRunEntryTable->rowCount(); ++row) {
            if (tableText(const_cast<QTableWidget*>(freeRunEntryTable), row, 0).toInt() != position) {
                continue;
            }
            ++freeRunRows;
            if (hasPdoMapIssueEvidence(tableText(const_cast<QTableWidget*>(freeRunEntryTable), row, 13))) {
                ++mapIssues;
            }
        }
    }
    ...
    details << uiText("Evidence score: %1/5 (ID %2, OD %3, PDO %4, Watch "
                      ...
                      "有值，Free Run %7）")
                   .arg(evidenceScore)
                   .arg(identityRows > 0 ? uiText("ready", "就绪") : uiText("missing", "缺失"))
                   .arg(odRows)
                   .arg(pdoRows)
                   .arg(watchValueRows)
                   .arg(watchRows)
    ...
    if (!statusword.isEmpty() || !modeDisplay.isEmpty() || !errorCode.isEmpty()) {
        ...
        details << uiText("Drive evidence: %1", "驱动证据：%1").arg(driveFacts.join(" | "));
    } else {
        details << uiText("Drive evidence: no CiA 402 Watch values", "驱动证据：没有 CiA 402 Watch 值");
    }
    ...
    if (mapIssues > 0) {
        details << uiText("PDO map evidence: %1 Free Run map issue(s)", "PDO 映射证据：Free Run 有 %1 个映射问题")
                       .arg(mapIssues);
    }
    ...
    if (target == "OP" || target == "SAFEOP") {
        if (pdoRows <= 0) {
            details << uiText("Risk: PDO Map is not loaded for this slave", "风险：当前从站尚未加载 PDO 映射");
        }
        if (watchValueRows <= 0) {
            details << uiText("Risk: no live Watch values for this slave", "风险：当前从站没有实时 Watch 值");
        }
        if (target == "OP" && freeRunRows <= 0) {
            details << uiText("Risk: no process-image evidence before OP", "风险：进入 OP 前没有过程映像证据");
        }
        if (startupDiffs > 0) {
