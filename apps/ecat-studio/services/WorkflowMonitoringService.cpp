...
WorkflowMonitoringService::WorkflowMonitoringService(QObject* parent) : QObject(parent) {}
...
void WorkflowMonitoringService::recordExecution(const ExecutionStatus& status) {
    ...
    perf.totalDurationMs =
        status.startTime.msecsTo(status.endTime.isValid() ? status.endTime : QDateTime::currentDateTime());
    ...
    perf.throughput = (perf.totalDurationMs > 0.0) ? (status.currentStep * 1000.0) / perf.totalDurationMs : 0.0;
