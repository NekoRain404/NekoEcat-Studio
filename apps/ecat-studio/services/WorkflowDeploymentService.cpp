...
WorkflowDeploymentService::WorkflowDeploymentService(QObject* parent) : QObject(parent) {}
...
bool WorkflowDeploymentService::deployConfiguration(int position, const WfConfigData& data) {
    ...
    if (!validateChecksum(data.configuration, data.checksum))
        return false;
    return executeDeployment(position, QStringLiteral("configuration"), data.configuration, data.version);
}
...
bool WorkflowDeploymentService::deployFirmware(int position, const WfFirmwareData& data) {
    ...
    if (!validateChecksum(data.firmware, data.checksum))
        return false;
    return executeDeployment(position, QStringLiteral("firmware"), data.firmware, data.version);
}
...
bool WorkflowDeploymentService::deploySoftware(int position, const WfSoftwareData& data) {
    ...
    if (!validateChecksum(data.software, data.checksum))
        return false;
    return executeDeployment(position, QStringLiteral("software"), data.software, data.version);
}
...
bool WorkflowDeploymentService::deploySystem(int position, const WfSystemData& data) {
    ...
    if (!validateChecksum(data.system, data.checksum))
        return false;
    return executeDeployment(position, QStringLiteral("system"), data.system, data.version);
}

bool WorkflowDeploymentService::validateChecksum(const QByteArray& payload, const QString& checksum) {
    if (checksum.isEmpty())
        return true;
...
bool WorkflowDeploymentService::executeDeployment(int position, const QString& type, const QByteArray& payload,
