...
ValidationResult SafetyController::validateStateTransition(int from, int to) {
    ...
    const bool validFrom = (from == kStateInit || from == kStatePreOp || from == kStateSafeOp || from == kStateOp);
    const bool validTo = (to == kStateInit || to == kStatePreOp || to == kStateSafeOp || to == kStateOp);
...
ValidationResult SafetyController::validateSdoWrite(int position, const QString& index, const QString& value) {
    ValidationResult r;
    ...
    bool ok;
    int idx = index.startsWith("0x") ? index.toInt(&ok, 16) : index.toInt(&ok, 16);
    if (ok && idx >= 0x1000 && idx < 0x1FFF) {
        if (idx == 0x1000 || idx == 0x1001 || idx == 0x1018) {
            r.allowed = false;
            r.reason = QString("Object 0x%1 is read-only and cannot be written.").arg(idx, 4, 16, QChar('0'));
            emit safetyViolation("sdoWrite", r.reason);
            return r;
        }
    }
...
ValidationResult SafetyController::validateSdoWriteDuringFreeRun(bool freeRunActive) {
    ValidationResult r;
