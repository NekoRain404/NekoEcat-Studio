...
FirmwareUpdateService::FirmwareUpdateService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
    progressTimer_ = new QTimer(this);
    connect(progressTimer_, &QTimer::timeout, this, &FirmwareUpdateService::advanceProgress);
}
...
bool FirmwareUpdateService::startUpdate(int position, const QString& firmwarePath) {
    if (updating_)
        return false;
    Q_UNUSED(position);
...
int FirmwareUpdateService::updateProgress() const {
    return progress_;
}

bool FirmwareUpdateService::isUpdating() const {
    return updating_;
}
