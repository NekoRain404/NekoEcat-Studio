...
DataPipelineService::DataPipelineService(QObject* parent) : QObject(parent) {
    throughputTimer_ = new QTimer(this);
    connect(throughputTimer_, &QTimer::timeout, this, &DataPipelineService::updateThroughput);
    throughputTimer_->start(1000);
}

int DataPipelineService::addStage(const QString& name, const QVariantMap& config) {
    PipelineStage stage;
    stage.name = name;
...
int DataPipelineService::stageCount() const {
    return stages_.size();
}
    ...
    for (int i = 0; i < stages_.size(); ++i) {
        ...
        if (stage.name == "filter") {
            ...
            for (int j = 0; j < data.size(); ++j)
                data[j] = static_cast<char>(static_cast<quint8>(data[j]) + offset);
        }
...
int DataPipelineService::bufferSize() const {
    return bufferSize_;
}

double DataPipelineService::throughput() const {
    return throughput_;
}
...
bool DataPipelineService::isRunning() const {
    return running_;
}
