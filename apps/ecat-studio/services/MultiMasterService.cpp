...
MultiMasterService::MultiMasterService(EcatClient *client, EventBus *eventBus,
                                       QObject *parent)
...
bool MultiMasterService::synchronizeMasters(int sourceId, int targetId) {
  ...
  for (const auto &m : masters_) {
    if (m.masterId == sourceId) hasSource = true;
    if (m.masterId == targetId) hasTarget = true;
  }
...
bool MultiMasterService::addMaster(const MmMasterInfo &info) {
  for (const auto &m : masters_) {
    if (m.masterId == info.masterId) return false;
  }
