...
WorkflowVisualizationService::WorkflowVisualizationService(QObject* parent) : QObject(parent) {}

QGraphicsScene* WorkflowVisualizationService::createFlowchart(const WfVisualizationConfig& config) {
...
QGraphicsScene* WorkflowVisualizationService::createGanttChart(const QVector<WfVizTask>& tasks) {
...
QGraphicsScene* WorkflowVisualizationService::createDependencyGraph(const QJsonObject& graph) {
    WfVisualizationConfig config;
    ...
    for (auto it = edges.begin(); it != edges.end(); ++it) {
        ...
        for (const auto& t : targets) {
            ...
            if (positions.contains(from) && positions.contains(to)) {
                addGraphEdge(scene, positions[from].x(), positions[from].y(), positions[to].x(), positions[to].y());
            }
        }
    }
...
QGraphicsScene* WorkflowVisualizationService::createResourceTimeline(const QVector<WfVizResource>& resources) {
    WfVisualizationConfig config;
...
QGraphicsScene* WorkflowVisualizationService::makeScene(const WfVisualizationConfig& config) {
...
void WorkflowVisualizationService::addFlowchartNode(QGraphicsScene* scene, const QString& id, const QString& label,
                                                    double x, double y) {
...
void WorkflowVisualizationService::addFlowchartEdge(QGraphicsScene* scene, double x1, double y1, double x2, double y2) {
    scene->addLine(x1, y1, x2, y2, QPen(Qt::black, 2));
}

void WorkflowVisualizationService::addGanttBar(QGraphicsScene* scene, const WfGanttEntry& entry, int row) {
...
void WorkflowVisualizationService::addGraphNode(QGraphicsScene* scene, const QString& id, double x, double y,
                                                const QColor& color) {
...
void WorkflowVisualizationService::addGraphEdge(QGraphicsScene* scene, double x1, double y1, double x2, double y2) {
    QPen pen(Qt::gray, 1, Qt::DashLine);
    scene->addLine(x1, y1, x2, y2, pen);
}

void WorkflowVisualizationService::addTimelineRow(QGraphicsScene* scene, const WfVizResource& resource, int row,
                                                  double maxTime) {
    double y = 30.0 + row * 35.0;
