...
FoEService::FoEService(EcatClient* client, QObject* parent) : QObject(parent), client_(client) {
    ...
    connect(client_, &EcatClient::foeReadResult, this, [this](int position, const QString& filePath, qint64 fileSize) {
        emit firmwareReadComplete(position, filePath, fileSize);
    });
    connect(client_, &EcatClient::foeWriteResult, this,
            [this](int position, qint64 bytesWritten) { emit firmwareWriteComplete(position, bytesWritten); });
}
...
bool FoEService::writeFile(int position, const QString& fileName, const QByteArray& data) {
    Q_UNUSED(position);
    Q_UNUSED(fileName);
    Q_UNUSED(data);
    if (!client_ || !client_->isConnected()) {
