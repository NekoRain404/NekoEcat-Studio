...
OpStateService::OpStateService(QObject* parent) : QObject(parent) {}
...
bool OpStateService::requestOpState(int position) {
    ...
    tr.success = false;
    tr.error = QStringLiteral("OP state request requires a connected EtherCAT backend");
...
OpStateStatus OpStateService::checkOpState(int position) const {
    ...
    if (status.currentState == kOpState) {
        ...
        status.details = QString("Slave in state %1, target is OP (8)").arg(status.currentState);
    }
