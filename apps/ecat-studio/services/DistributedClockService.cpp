...
DistributedClockService::DistributedClockService(EcatClient* client, QObject* parent)
...
void DistributedClockService::processDcSyncData(const QJsonObject& data) {
    ...
    if (maxDriftSlave >= 0) {
        ...
        driftStatus_.status =
            qAbs(maxDrift) > driftStatus_.threshold ? QStringLiteral("excessive") : QStringLiteral("ok");
