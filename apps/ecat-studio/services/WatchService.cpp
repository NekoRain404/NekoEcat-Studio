...
void WatchService::addEntry(int position, const QString &index, const QString &subIndex,
                            const QString &type) {
  WatchEntry entry;
...
void WatchService::removeEntry(int position, const QString &index, const QString &subIndex) {
  for (int i = 0; i < entries_.size(); ++i) {
    if (entries_[i].position == position &&
        entries_[i].index == index &&
        entries_[i].subIndex == subIndex) {
      entries_.removeAt(i);
      return;
    }
  }
...
int WatchService::entryCount() const { return entries_.size(); }

const WatchEntry &WatchService::entryAt(int i) const { return entries_[i]; }
