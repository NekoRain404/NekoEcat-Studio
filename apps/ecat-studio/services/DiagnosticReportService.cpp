...
DiagnosticReportService::DiagnosticReportService(EventBus* bus, EcatClient* client, TopologyService* topology,
                                                 DcSyncService* dcSync, PerformanceMonitorService* perfMonitor,
                                                 WatchdogService* watchdog, QObject* parent)
    : QObject(parent), bus_(bus), client_(client), topology_(topology), dcSync_(dcSync), perfMonitor_(perfMonitor),
      watchdog_(watchdog) {}
...
QString DiagnosticReportService::generateReport() const {
    ...
    ts << "**Generated:** " << QDateTime::currentDateTimeUtc().toString(Qt::ISODate) << " UTC\n";
...
QString DiagnosticReportService::topologySection() const {
    ...
    for (const auto& sl : slaves) {
        ts << "| " << sl.position << " | " << sl.name << " | " << sl.state << " | " << sl.flags << " |\n";
    }
    ts << "\n";
    return s;
}
...
QString DiagnosticReportService::performanceSection() const {
    ...
    ts << "- **PDO update rate:** " << m["pdoUpdateRate"].toDouble() << " Hz\n";
...
QString DiagnosticReportService::watchdogSection() const {
    ...
    if (!slaves.isEmpty()) {
        ...
        for (const auto& entry : slaves) {
            ...
            ts << "| " << obj["position"].toInt() << " | " << (obj["watchdogOk"].toBool() ? "Yes" : "No") << " | "
               << obj["timeoutCount"].toInt() << " | " << obj["triggerCount"].toInt() << " |\n";
        }
    }
    ts << "\n";
    return s;
