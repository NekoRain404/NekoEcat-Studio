...
MasterGuard::MasterGuard(std::mutex& mutex, ec_master_t*& cachedMaster, int& cachedMasterIndex, int index)
    : lock_(mutex), cachedMaster_(cachedMaster), cachedMasterIndex_(cachedMasterIndex) {
...
MasterGuard::MasterGuard(MasterGuard&& other) noexcept
    : lock_(std::move(other.lock_)), master_(other.master_), cachedMaster_(other.cachedMaster_),
...
SlaveInfo EthercatNativeBackend::convertSlaveInfo(const ec_slave_info_t& info) const {
    SlaveInfo slave;
    ...
    slave.rawLine =
        QString("%1  %2  %3  %4").arg(info.position).arg(info.alias).arg(slave.state, slave.flags, slave.name);
    return slave;
}

#endif // HAVE_IGH

EthercatNativeBackend::EthercatNativeBackend(QObject* parent) : QObject(parent) {}
...
QString EthercatNativeBackend::masterText(const QString& master, QString* error) const {
    ...
    if (!guard.get()) {
        if (error)
            *error = "Failed to open EtherCAT master";
        return {};
    }
    ...
    Q_UNUSED(master)
    if (error)
        *error = "IgH support not compiled";
    return {};
#endif
}
...
QVector<SlaveInfo> EthercatNativeBackend::scanSlaves(const QString& master, QString* error) const {
    ...
    for (unsigned int i = 0; i < info.slave_count; ++i) {
        ec_slave_info_t si;
        if (ecrt_master_get_slave(guard.get(), static_cast<uint16_t>(i), &si) == 0) {
...
QString EthercatNativeBackend::slaveInfo(const QString& master, int position, QString* error) const {
    ...
    ec_slave_info_t si;
    if (ecrt_master_get_slave(guard.get(), static_cast<uint16_t>(position), &si) != 0) {
        if (error)
            *error = "ecrt_master_get_slave() failed";
        return {};
    }
    ...
    for (int p = 0; p < EC_MAX_PORTS; ++p) {
        ...
        if (port.desc == EC_PORT_NOT_IMPLEMENTED)
            continue;
        ts << "Port " << p << ": ";
        switch (port.desc) {
        ...
        ts << " link=" << (port.link.link_up ? "up" : "down") << " loop=" << (port.link.loop_closed ? "closed" : "open")
...
QString EthercatNativeBackend::pdos(const QString& master, int position, QString* error) const {
    ...
    for (uint8_t sm = 0; sm < si.sync_count; ++sm) {
        ec_sync_info_t sync;
        if (ecrt_master_get_sync_manager(guard.get(), static_cast<uint16_t>(position), sm, &sync) != 0) {
            continue;
        }
        ...
        ts << "SM" << sm << " dir=" << dirStr << " wd=" << sync.watchdog_mode << "\n";

        for (unsigned int p = 0;; ++p) {
            ec_pdo_info_t pdo;
            if (ecrt_master_get_pdo(guard.get(), static_cast<uint16_t>(position), sm, static_cast<uint16_t>(p), &pdo) !=
                0) {
                break;
            }
            ts << "  PDO 0x" << QString::number(pdo.index, 16) << " (" << pdo.n_entries << " entries)\n";

            for (unsigned int e = 0; e < pdo.n_entries; ++e) {
                ec_pdo_entry_info_t entry;
                if (ecrt_master_get_pdo_entry(guard.get(), static_cast<uint16_t>(position), sm,
                                              static_cast<uint16_t>(p), static_cast<uint16_t>(e), &entry) != 0) {
                    break;
                }
                ts << "    0x" << QString::number(entry.index, 16) << ":" << entry.subindex << " (" << entry.bit_length
                   << " bit)\n";
            }
        }
    }
...
QString EthercatNativeBackend::upload(const QString& master, int position, const QString& index,
                                      const QString& subIndex, const QString& type, QString* error) const {
#ifdef HAVE_IGH
    auto guard = acquireMaster(master);
    if (!guard.get()) {
        if (error)
    ...
    if (!ok) {
        if (error)
            *error = "Invalid SDO index: " + index;
        return {};
    }
    uint8_t sub = static_cast<uint8_t>(subIndex.toUShort(&ok, 16));
    if (!ok) {
        if (error)
            *error = "Invalid SDO subindex: " + subIndex;
        return {};
    }
    ...
    int ret = ecrt_master_sdo_upload(guard.get(), static_cast<uint16_t>(position), idx, sub, buf, sizeof(buf),
                                     &resultSize, &abortCode);
    if (ret != 0) {
        if (error) {
            *error = QString("SDO upload failed (ret=%1, abort=0x%2)").arg(ret).arg(abortCode, 8, 16, QChar('0'));
        }
        return {};
    }
    ...
    if (typeLower == "uint8" || typeLower == "u8") {
        ...
        if (resultSize >= 1)
            return QString::number(static_cast<int8_t>(buf[0]));
        ...
        return QString::fromLatin1(reinterpret_cast<const char*>(buf), static_cast<int>(resultSize));
    }
...
bool EthercatNativeBackend::download(const QString& master, int position, const QString& index, const QString& subIndex,
    ...
    if (typeLower == "uint8" || typeLower == "u8") {
        ...
        if (dataSize > MAX_SDO_DATA_SIZE)
            dataSize = MAX_SDO_DATA_SIZE;
    ...
    uint32_t abortCode = 0;
    int ret =
        ecrt_master_sdo_download(guard.get(), static_cast<uint16_t>(position), idx, sub, data, dataSize, &abortCode);
    if (ret != 0) {
        if (error) {
            *error = QString("SDO download failed (ret=%1, abort=0x%2)").arg(ret).arg(abortCode, 8, 16, QChar('0'));
        }
        return false;
    }
    return true;
...
bool EthercatNativeBackend::setState(const QString& master, int position, const QString& state, QString* error) const {
#ifdef HAVE_IGH
...
bool EthercatNativeBackend::setAllStates(const QString& master, const QString& state, QString* error) const {
#ifdef HAVE_IGH
    // Native ecrt API doesn't support AL state transitions.
    // Fall back to CLI for this operation.
    lastFallback_ = true;
