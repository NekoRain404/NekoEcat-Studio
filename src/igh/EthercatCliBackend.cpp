...
ProcessResult runProgram(const QString& program, const QStringList& arguments = {}, int timeoutMs = 5000) {
    QProcess process;
    ...
    result.stdOut = QString::fromLocal8Bit(process.readAllStandardOutput()).trimmed();
    result.stdErr = QString::fromLocal8Bit(process.readAllStandardError()).trimmed();
    return result;
}

// Build a standardized diagnostic check object for the GUI's host-diagnostics panel.
QJsonObject diagnosticItem(const QString& level, const QString& source, const QString& message,
                           const QString& detail = {}, const QString& hint = {}, const QString& command = {}) {
    return QJsonObject{
...
QStringList matchingBlacklistLines() {
    QStringList matches;
    for (const QString& root : {QStringLiteral("/etc/modprobe.d"), QStringLiteral("/usr/lib/modprobe.d")}) {
        QDirIterator it(root, QStringList{"*.conf"}, QDir::Files, QDirIterator::Subdirectories);
        while (it.hasNext()) {
            ...
            while (!file.atEnd()) {
                ...
                if (line.contains("blacklist") &&
                    (line.contains("r8152") || line.contains("rt8152") || line.contains("rtl815"))) {
...
QString EthercatCliBackend::masterText(const QString& master, QString* error) const {
    int exitCode = 0;
    QString stdErr;
...
QVector<SlaveInfo> EthercatCliBackend::scanSlaves(const QString& master, QString* error) const {
    int exitCode = 0;
    QString stdErr;
...
QString EthercatCliBackend::slaveInfo(const QString& master, int position, QString* error) const {
    int exitCode = 0;
    QString stdErr;
    const QString text = run(master, {"slaves", "-p", QString::number(position), "-v"}, &exitCode, &stdErr);
    if (exitCode != 0 && error) {
        *error = stdErr.isEmpty() ? text : stdErr;
    }
    return text;
...
QString EthercatCliBackend::slaveXml(const QString& master, int position, QString* error) const {
    int exitCode = 0;
    QString stdErr;
    const QString text = run(master, {"xml", "-p", QString::number(position)}, &exitCode, &stdErr);
    if (exitCode != 0 && error) {
        *error = stdErr.isEmpty() ? text : stdErr;
    }
    return text;
...
QString EthercatCliBackend::pdos(const QString& master, int position, QString* error) const {
    int exitCode = 0;
    QString stdErr;
    const QString text = run(master, {"pdos", "-p", QString::number(position)}, &exitCode, &stdErr);
    if (exitCode != 0 && error) {
        *error = stdErr.isEmpty() ? text : stdErr;
    }
    return text;
...
QString EthercatCliBackend::sdos(const QString& master, int position, QString* error) const {
    int exitCode = 0;
    QString stdErr;
    const QString text = run(master, {"sdos", "-p", QString::number(position)}, &exitCode, &stdErr);
    if (exitCode != 0 && error) {
        *error = stdErr.isEmpty() ? text : stdErr;
    }
    return text;
...
QString EthercatCliBackend::upload(const QString& master, int position, const QString& index, const QString& subIndex,
...
bool EthercatCliBackend::download(const QString& master, int position, const QString& index, const QString& subIndex,
                                  const QString& value, const QString& type, QString* error) const {
    int exitCode = 0;
    QString stdErr;
...
bool EthercatCliBackend::setState(const QString& master, int position, const QString& state, QString* error) const {
    int exitCode = 0;
    QString stdErr;
    run(master, {"states", "-p", QString::number(position), state}, &exitCode, &stdErr);
    if (exitCode != 0 && error) {
        *error = stdErr;
    }
    return exitCode == 0;
...
bool EthercatCliBackend::setAllStates(const QString& master, const QString& state, QString* error) const {
    int exitCode = 0;
    QString stdErr;
...
QJsonArray EthercatCliBackend::hostDiagnostics(QString* error) const {
    ...
    if (kernel.started && kernel.exitCode == 0) {
        checks.append(diagnosticItem("Info", "Host", QString("Kernel: %1").arg(kernel.stdOut), {},
                                     "Keep DKMS modules installed for every active kernel."));
    } else {
        checks.append(diagnosticItem("Warning", "Host", "Unable to read running kernel", kernel.stdErr,
    ...
    if (ecMaster.started && ecMaster.exitCode == 0) {
        ...
        checks.append(diagnosticItem("Info", "Kernel Module", "ec_master is available for the running kernel", detail,
                                     "No action required."));
    } else {
        checks.append(diagnosticItem("Error", "Kernel Module", "ec_master is not available for the running kernel",
    ...
    if (configFile.open(QIODevice::ReadOnly | QIODevice::Text)) {
        ...
        checks.append(diagnosticItem("Info", "Config",
                                     QString("MASTER0_DEVICE=%1, DEVICE_MODULES=%2").arg(master0Device, deviceModules),
                                     QString("UPDOWN_INTERFACES=%1").arg(updownInterfaces),
                                     "Keep MASTER0_DEVICE aligned with the dedicated EtherCAT NIC."));
    } else {
        checks.append(diagnosticItem("Error", "Config", "/etc/ethercat.conf is not readable", {},
        ...
        if (links.started && links.exitCode == 0) {
            ...
            for (const auto& line : links.stdOut.split('\n', Qt::SkipEmptyParts)) {
                if (line.startsWith(master0Device + ' ')) {
                    found = true;
                    checks.append(diagnosticItem("Info", "Network", QString("%1 is present").arg(master0Device),
            ...
            if (!found) {
                checks.append(diagnosticItem(
                    "Error", "Network", QString("%1 is not present in current network interfaces").arg(master0Device),
                    links.stdOut,
            ...
            checks.append(diagnosticItem("Warning", "Network", "Unable to list network interfaces", links.stdErr,
    ...
    const auto deviceStat = runProgram("stat", {"-Lc", "%A %U:%G major=%t minor=%T", "/dev/EtherCAT0"});
    const QString deviceDetail = deviceStat.started && deviceStat.exitCode == 0 ? deviceStat.stdOut : deviceStat.stdErr;
    if (ethercatDevice.exists()) {
        errno = 0;
        const bool canAccessDevice = ::access("/dev/EtherCAT0", R_OK | W_OK) == 0;
        if (canAccessDevice) {
            checks.append(diagnosticItem("Info", "Device Node",
                                         "/dev/EtherCAT0 exists and is readable/writable by the current user",
                                         deviceDetail, "No action required."));
        } else {
            checks.append(diagnosticItem("Warning", "Device Node",
        ...
        checks.append(diagnosticItem("Error", "Device Node", "/dev/EtherCAT0 is missing", deviceDetail,
    ...
    if (blacklistLines.isEmpty()) {
        checks.append(diagnosticItem("Info", "Driver", "No r8152/rt8152/rtl815x firmware/module blacklist entry found",
                                     {}, "No action required."));
    } else {
        checks.append(diagnosticItem("Warning", "Driver", "r8152/rt8152/rtl815x appears in firmware/module blacklist",
                                     blacklistLines.join('\n'),
    ...
    if (dkms.started && dkms.exitCode == 0) {
        ...
        if (hasR8152) {
            checks.append(diagnosticItem("Info", "DKMS", "r8152/rt8152/rtl815x DKMS driver is installed", dkms.stdOut,
                                         "Use only if this host relies on a Realtek USB Ethernet adapter."));
        } else {
            checks.append(diagnosticItem("Info", "DKMS", "No r8152/rt8152/rtl815x DKMS driver is installed",
                                         dkms.stdOut,
                                         "No action required unless the EtherCAT NIC is a Realtek USB "
                                         "adapter."));
        }
        if (hasR8125) {
            checks.append(diagnosticItem("Info", "DKMS", "r8125 DKMS driver is installed", dkms.stdOut,
                                         "Keep r8125 DKMS if the EtherCAT NIC is RTL8125."));
        }
    } else {
        checks.append(diagnosticItem("Warning", "DKMS", "Unable to query DKMS status", dkms.stdErr,
    ...
    if (status.started && status.exitCode == 0) {
        checks.append(diagnosticItem("Info", "EtherCAT", "ethercatctl reports master running", status.stdOut,
                                     "Ready for online scan and commissioning."));
    } else {
        checks.append(diagnosticItem("Error", "EtherCAT", "ethercatctl reports master not running",
...
QString EthercatCliBackend::run(const QString& master, const QStringList& arguments, int* exitCode,
