...
private slots:
    ...
    void testDesignerToSchedulingPipeline() {
        ...
        QCOMPARE(scheduler.runs(QStringLiteral("deploy_wf")).last().status, WorkflowStatus::Running);
    }
    ...
    void testFullWorkflowLifecycle() {
        ...
        QCOMPARE(spy.count(), 1);
        QCOMPARE(scheduler.runs(QStringLiteral("full_lifecycle")).last().status, WorkflowStatus::Running);
