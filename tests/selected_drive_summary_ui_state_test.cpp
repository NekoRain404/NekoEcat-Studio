...
void testOperationEnabledSummary() {
  ...
  const SelectedDriveSummaryUiState state =
      buildSelectedDriveSummaryUiState(rows, 2, englishTexts());
  expectEqual(
      state.text,
...
void testErrorCodeEvidence() {
  const SelectedDriveSummaryTexts texts = englishTexts();
  SelectedDriveSummaryUiState state = buildSelectedDriveSummaryUiState(
