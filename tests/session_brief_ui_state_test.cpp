...
void testStatusTextAndColorKeys() {
  ...
  expectEqual(sessionBriefStatusText(SessionBriefStatus::Ready, texts), "Ready",
              "ready status text");
  expectEqual(sessionBriefStatusText(SessionBriefStatus::Action, texts),
              "Action", "action status text");
  expectEqual(sessionBriefStatusText(SessionBriefStatus::Warning, texts),
              "Warning", "warning status text");
  expectEqual(sessionBriefStatusText(SessionBriefStatus::Error, texts), "Error",
              "error status text");
  expectEqual(sessionBriefStatusText(SessionBriefStatus::Info, texts), "Info",
              "info status text");
  expectEqual(sessionBriefStatusColorKey(SessionBriefStatus::Ready), "ready",
              "ready color key");
  expectEqual(sessionBriefStatusColorKey(SessionBriefStatus::Warning),
              "warning", "warning color key");
  expectEqual(sessionBriefStatusColorKey(SessionBriefStatus::Error), "error",
              "error color key");
}
...
void testHeadersAndUiRow() {
  ...
  const SessionBriefUiRow uiRow =
      sessionBriefUiRow(row, "Gate", "2 warnings", "Open gate", texts);
  ...
  expectEqual(uiRow.tooltips.at(2), "2 warnings\nOpen local evidence.",
              "evidence tooltip uses cell text");
}
