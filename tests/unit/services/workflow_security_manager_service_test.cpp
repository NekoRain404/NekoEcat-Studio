...
private slots:
    void testAddPolicy() {
        ...
        auto id = svc.addPolicy(QStringLiteral("Auth Policy"), QStringLiteral("Require authentication"),
                                QStringLiteral("high"));
        QVERIFY(!id.isEmpty());
    ...
    void testGetPolicy() {
        WorkflowSecurityManagerService svc;
        auto id = svc.addPolicy(QStringLiteral("Auth"), QStringLiteral("Desc"), QStringLiteral("critical"));
