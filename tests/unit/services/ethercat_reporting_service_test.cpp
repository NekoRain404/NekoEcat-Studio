...
private slots:
    ...
    void testExportEmptyReport() {
        ...
        QTemporaryDir dir;
        QVERIFY(dir.isValid());
        bool ok = svc.exportReport(empty, dir.filePath(QStringLiteral("empty.txt")), QStringLiteral("text"));
        QVERIFY(!ok);
    }
    ...
    void testExportInvalidArguments() {
        ...
        QVERIFY(!svc.exportReport(report, dir.filePath(QStringLiteral("report.pdf")), QStringLiteral("pdf")));
    }
