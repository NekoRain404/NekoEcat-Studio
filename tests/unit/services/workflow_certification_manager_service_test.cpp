...
private slots:
    void testAddRequirement() {
        ...
        auto id = svc.addRequirement(QStringLiteral("ISO 9001 Certification"), QStringLiteral("ISO 9001"));
        QVERIFY(!id.isEmpty());
    ...
    void testGetRequirement() {
        WorkflowCertificationManagerService svc;
        auto id = svc.addRequirement(QStringLiteral("CE Mark"), QStringLiteral("CE Directive"));
    ...
    void testCustomExpiry() {
        ...
        auto id = svc.addRequirement(QStringLiteral("Test"), QStringLiteral("Std"), customExpiry);
        auto r = svc.requirement(id);
        QVERIFY(r.expiry.isValid());
    }

    void testSourceDoesNotSynthesizeCertificationValidity() {
        QFile file(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/services/WorkflowCertificationManagerService.cpp"));
