...
void expectChangeKind(TopologyChangeKind actual, TopologyChangeKind expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}

SlaveInfo slave(int position, QString name, QString state = QString(), QString flags = QString()) {
    SlaveInfo info;
...
void testTopologyChangeOrderAndPayloads() {
    ...
    expectEqual(changes.size(), 5, "topology change detector reports all changes");
    ...
    expectEqual(changes.at(0).previous.name, "Drive A", "name change keeps previous name");
    expectEqual(changes.at(0).current.name, "Drive A2", "name change keeps current name");
    expectChangeKind(changes.at(1).kind, TopologyChangeKind::StateChanged,
                     "state change follows name change for same slave");
    expectEqual(changes.at(1).previous.state, "PREOP", "state change keeps previous state");
    expectEqual(changes.at(1).current.state, "OP", "state change keeps current state");
    expectChangeKind(changes.at(2).kind, TopologyChangeKind::FlagsChanged,
                     "flags change follows state change for same slave");
    expectChangeKind(changes.at(3).kind, TopologyChangeKind::Added, "added slave is reported in current pass");
    expectEqual(changes.at(3).current.name, "Extra", "added slave keeps current data");
    expectChangeKind(changes.at(4).kind, TopologyChangeKind::Removed, "removed slave is reported after current pass");
    expectEqual(changes.at(4).previous.name, "Drive B", "removed slave keeps previous data");
}
