...
void testInitRecommendsPreop() {
    expectEqual(recommendedEthercatState(evidence("INIT")), "PREOP", "INIT recommends PREOP");
...
void testPreopRequiresPdoAndWatchEvidence() {
    ...
    expectEqual(recommendedEthercatState(item), QString(), "PREOP without evidence has no recommendation");

    item.pdoLoaded = true;
    expectEqual(recommendedEthercatState(item), QString(), "PREOP with PDO but no Watch values has no recommendation");

    item.watchValueRows = 1;
    expectEqual(recommendedEthercatState(item), "SAFEOP", "PREOP with PDO and Watch values recommends SAFEOP");
}
...
void testSafeopRequiresCleanProcessEvidenceAndConsistency() {
    ...
    expectEqual(recommendedEthercatState(item), "OP", "SAFEOP with clean process evidence recommends OP");

    item.startupDiffs = 1;
    expectEqual(recommendedEthercatState(item), QString(), "SAFEOP with Startup diffs does not recommend OP");

    item.startupDiffs = 0;
    item.mapIssues = 1;
    expectEqual(recommendedEthercatState(item), QString(), "SAFEOP with map issues does not recommend OP");
    ...
    item.consistencyOk = true;
    item.freeRunRows = 0;
    expectEqual(recommendedEthercatState(item), QString(), "SAFEOP without process evidence does not recommend OP");
}

// Test OP and unknown states have no recommendation
void testOpAndUnknownHaveNoRecommendation() {
    expectEqual(recommendedEthercatState(evidence("OP")), QString(), "OP has no next-state recommendation");
    expectEqual(recommendedEthercatState(evidence("BOOT")), QString(), "unknown state has no recommendation");
}
