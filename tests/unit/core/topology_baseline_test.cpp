...
void expectIssueKind(TopologyBaselineIssueKind actual, TopologyBaselineIssueKind expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}

SlaveInfo slave(int position, QString name, QString state = QString(), QString rawLine = QString()) {
    SlaveInfo info;
...
void testMissingNameStateAndUnexpectedIssues() {
    ...
    expectEqual(issues.at(0).baseline.name, "Drive A", "name change keeps baseline name");
    expectEqual(issues.at(0).current.name, "Drive A2", "name change keeps current name");
    ...
    expectIssueKind(issues.at(2).kind, TopologyBaselineIssueKind::MissingSlave, "missing baseline slave is reported");
    expectEqual(issues.at(2).position, 1, "missing slave keeps baseline position");
    ...
    expectEqual(issues.at(3).current.name, "Extra", "unexpected slave keeps current slave data");
}
...
void testTopologyDisplayNameFallback() {
    expectEqual(topologySlaveDisplayName(slave(1, "", "OP", "1  OP  Raw")), "1  OP  Raw",
                "empty slave name falls back to raw line");
    expectEqual(topologySlaveDisplayName(slave(1, " ", "OP", "raw")), " ",
                "blank but non-empty slave name preserves previous display");
    expectEqual(topologySlaveDisplayName(slave(1, "Drive", "OP", "raw")), "Drive", "non-empty slave name is preferred");
}
