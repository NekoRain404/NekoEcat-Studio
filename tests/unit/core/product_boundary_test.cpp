...
void ProductBoundaryTest::experimentalServicesAreOptInByDefault() {
    const QString cmake = readTextFile(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/CMakeLists.txt"));
    const QRegularExpression optionPattern(
        QStringLiteral(R"(^\s*option\s*\(\s*ECAT_EXPERIMENTAL_SERVICES\b.*\b(ON|OFF)\s*\)\s*$)"),
...
void ProductBoundaryTest::experimentalSourcesAreCompileGuarded() {
    ...
    for (const QString& sourcePath : deletedSourcePaths) {
        QVERIFY2(!cmake.contains(sourcePath),
                 qPrintable(QStringLiteral("Deleted source %1 must not appear in CMakeLists.txt").arg(sourcePath)));
    }
}
...
void ProductBoundaryTest::experimentalIncludeDirsAreCompileGuarded() {
    ...
    for (const QString& includeDir : deletedIncludeDirs) {
        QVERIFY2(
            !cmake.contains(includeDir),
            qPrintable(QStringLiteral("Deleted include dir %1 must not appear in CMakeLists.txt").arg(includeDir)));
    }
}

void ProductBoundaryTest::experimentalTestsAreOptInByDefault() {
    const QString cmake = readTextFile(QStringLiteral(SOURCE_ROOT "/tests/CMakeLists.txt"));
    ...
    for (const QString& testTarget : deletedTestTargets) {
        ...
        QVERIFY2(!cmake.contains(declaration),
                 qPrintable(
                     QStringLiteral("Deleted test target %1 must not appear in tests/CMakeLists.txt").arg(testTarget)));
    }
}
...
void ProductBoundaryTest::experimentalTestSourcesAreCompileGuarded() {
    ...
    for (const QString& sourcePath : deletedSourcePaths) {
        QVERIFY2(!cmake.contains(sourcePath),
                 qPrintable(
                     QStringLiteral("Deleted test source %1 must not appear in tests/CMakeLists.txt").arg(sourcePath)));
    }
}

void ProductBoundaryTest::experimentalPluginsAreCompileGuarded() {
    const QString mainWindow = readTextFile(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/MainWindow.cpp"));
    const QStringList deletedPluginTypes = {
        QStringLiteral("CloudManagerPlugin"),       QStringLiteral("EdgeComputingPlugin"),
        QStringLiteral("AIAssistantPlugin"),        QStringLiteral("DigitalTwinStudioPlugin"),
        QStringLiteral("BlockchainExplorerPlugin"), QStringLiteral("QuantumSecurityPlugin"),
    };

    for (const QString& pluginType : deletedPluginTypes) {
        const QString registration = QStringLiteral("registerPlugin(new %1").arg(pluginType);
...
void ProductBoundaryTest::readmeMarksExperimentalWorkspaces() {
    ...
    const QStringList deletedWorkspaceNames = {
        QStringLiteral("Digital Twin Studio"), QStringLiteral("Blockchain Explorer"),
        QStringLiteral("Quantum Security"),    QStringLiteral("Cloud Manager"),
        QStringLiteral("Edge Computing"),      QStringLiteral("AI Assistant"),
    };
...
void ProductBoundaryTest::docsMarkExperimentalSurfaces() {
    ...
    const QStringList deletedSurfaceNames = {
        ...
        QStringLiteral("AIAssistantPlugin"),         QStringLiteral("DigitalTwinStudioPlugin"),
        QStringLiteral("BlockchainExplorerPlugin"),  QStringLiteral("QuantumSecurityPlugin"),
        QStringLiteral("EtherCATCloudService"),      QStringLiteral("EtherCATEdgeService"),
        QStringLiteral("EtherCATAIService"),         QStringLiteral("EtherCATDigitalTwinService"),
        QStringLiteral("EtherCATBlockchainService"), QStringLiteral("EtherCATQuantumService"),
    };
...
void ProductBoundaryTest::publicDocsDoNotUseStaleReleaseNumbers() {
    ...
    for (const QString& docPath : docPaths) {
        ...
        QVERIFY2(!doc.contains(QStringLiteral("v3.5.0")),
                 qPrintable(QStringLiteral("%1 must not advertise stale v3.5.0 release artifacts.").arg(docPath)));
        QVERIFY2(!doc.contains(QStringLiteral("3.5.0")),
                 qPrintable(QStringLiteral("%1 must not advertise stale 3.5.0 release artifacts.").arg(docPath)));
    }
}
...
void ProductBoundaryTest::publicDocsDoNotAdvertiseStaleProjectStats() {
    ...
    const QStringList staleClaims = {
        ...
        QStringLiteral("335+ registered tests"),
        QStringLiteral("400+ (100% pass rate)"),
        QStringLiteral("100% pass rate"),
        QStringLiteral("10x improvement"),
        QStringLiteral("5x improvement"),
        QStringLiteral("10-100x SDO performance improvement"),
        QStringLiteral("Clean build with no warnings"),
        QStringLiteral("All packages created and validated"),
        ...
        QStringLiteral("all workflow plugins"),
        QStringLiteral("all services for faster startup"),
        QStringLiteral("initialization of all plugins"),
        QStringLiteral("all SDO/PDO data"),
        QStringLiteral("all slow tasks"),
        QStringLiteral("all multiple requests"),
        QStringLiteral("all frequent allocations"),
        QStringLiteral("all temporary data"),
        QStringLiteral("Test Pass Rate**: 100%"),
        QStringLiteral("Build Warnings**: 0"),
        ...
        QStringLiteral("120+ domain services"),
        ...
        QStringLiteral("NekoEcat Studio v3.7.0 Release Notes"),
    ...
    for (const QString& docPath : docPaths) {
        ...
        for (const QString& staleClaim : staleClaims) {
            QVERIFY2(
                !doc.contains(staleClaim),
                qPrintable(QStringLiteral("%1 contains stale public project statistic: %2").arg(docPath, staleClaim)));
        }
    }
}
...
void ProductBoundaryTest::digitalTwinPluginHasSingleCanonicalPathAndId() {
    QVERIFY2(
        !QFile::exists(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/digitaltwin/DigitalTwinStudioPlugin.cpp")),
        "Legacy apps/ecat-studio/plugins/digitaltwin duplicate must not exist.");
    QVERIFY2(
        !QFile::exists(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/digitaltwin/DigitalTwinStudioPlugin.h")),
        "Legacy apps/ecat-studio/plugins/digitaltwin duplicate must not exist.");
    QVERIFY2(!QFile::exists(
                 QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/digitaltwinstudio/DigitalTwinStudioPlugin.cpp")),
             "Deleted apps/ecat-studio/plugins/digitaltwinstudio must not exist.");
    QVERIFY2(!QFile::exists(
                 QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/digitaltwinstudio/DigitalTwinStudioPlugin.h")),
             "Deleted apps/ecat-studio/plugins/digitaltwinstudio must not exist.");

    const QString architecture = readTextFile(QStringLiteral(SOURCE_ROOT "/docs/ARCHITECTURE.md"));
...
void ProductBoundaryTest::managerPluginsHaveSingleCanonicalPathAndId() {
    ...
    for (const QString& legacyFile : legacyFiles) {
        QVERIFY2(!QFile::exists(legacyFile),
                 qPrintable(QStringLiteral("Legacy duplicate plugin file must not exist: %1").arg(legacyFile)));
    }
    ...
    for (const QString& staleRow : staleRows) {
        QVERIFY2(!architecture.contains(staleRow),
                 qPrintable(QStringLiteral("Docs must not use stale plugin id row: %1").arg(staleRow)));
    }
}

void ProductBoundaryTest::pluginIdsAreUniqueAcrossSourceTree() {
    QDirIterator it(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins"), QStringList() << QStringLiteral("*.cpp"),
                    QDir::Files, QDirIterator::Subdirectories);
    const QRegularExpression idPattern(QStringLiteral("QString\\s+\\w+::id\\s*\\(\\s*\\)\\s+const\\s*\\{\\s*"
                                                      "return\\s+\"([^\"]+)\";\\s*\\}"));
    ...
    while (it.hasNext()) {
        ...
        QVERIFY2(!seenIds.contains(id),
                 qPrintable(QStringLiteral("Duplicate plugin id '%1' found while scanning %2").arg(id, path)));
        seenIds.insert(id);
    }
}
...
void ProductBoundaryTest::productPluginSourcesAreRegisteredOrExperimental() {
    const QString cmake = readTextFile(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/CMakeLists.txt"));
    const QString mainWindow = readTextFile(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/MainWindow.cpp"));
    const QRegularExpression pluginSourcePattern(QStringLiteral("plugins/[^\\s]+/([A-Za-z0-9_]+Plugin)\\.cpp"));
    QRegularExpressionMatchIterator matches = pluginSourcePattern.globalMatch(cmake);
    ...
    while (matches.hasNext()) {
        ...
        QVERIFY2(sourceIndex >= 0, qPrintable(QStringLiteral("Unable to locate plugin source %1").arg(sourcePath)));

        const qsizetype guardIndex = cmake.lastIndexOf(QStringLiteral("if(ECAT_EXPERIMENTAL_SERVICES)"), sourceIndex);
        const qsizetype endGuardIndex = cmake.lastIndexOf(QStringLiteral("endif()"), sourceIndex);
        if (guardIndex > endGuardIndex) {
            continue;
        }

        const QString registration = QStringLiteral("registerPlugin(new %1").arg(pluginClass);
        QVERIFY2(
            mainWindow.contains(registration),
            qPrintable(
                QStringLiteral("%1 is built into ecat-studio but is not registered in MainWindow.").arg(pluginClass)));
    }
    QVERIFY2(scannedPluginSources > 0, "Product plugin source scan did not find any plugin sources.");
}
