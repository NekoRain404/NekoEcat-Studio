...
void testRowStateAndScope() {
    ...
    const IoVariableFilterRowState state = ioVariableFilterRowState(row, 3, "Ready");
    ...
    expectTrue(ioVariableFilterScopeMatches(state, kIoVariableScopeSelected), "selected scope matches");
    expectTrue(ioVariableFilterScopeMatches(state, kIoVariableScopeProcess), "process scope matches");
    expectTrue(ioVariableFilterScopeMatches(state, kIoVariableScopeRx), "Rx scope matches");
    expectFalse(ioVariableFilterScopeMatches(state, kIoVariableScopePdo), "PDO scope excludes process-backed rows");

    row.source = "PDO | uint16";
    const IoVariableFilterRowState pdoState = ioVariableFilterRowState(row, 3, "Ready");
    expectTrue(ioVariableFilterScopeMatches(pdoState, kIoVariableScopePdo), "PDO scope matches non-process PDO rows");
}

// Verify text search and filter decision logic
void testSearchAndDecision() {
    const IoVariableTableRow row = baseRow();
    expectTrue(ioVariableFilterTextMatches({"Drive.Controlword", "0x6040"}, "control"),
               "search matches any cell case-insensitively");
    expectFalse(ioVariableFilterTextMatches({"Drive.Controlword", "0x6040"}, "status"), "search rejects missing text");

    IoVariableFilterDecision decision =
        evaluateIoVariableFilterRow(row, {"Drive.Controlword", "0x6040"}, kIoVariableScopeProcess, "drive", 3, "Ready");
    expectTrue(decision.visible, "scope and search match");

    decision =
        evaluateIoVariableFilterRow(row, {"Drive.Controlword", "0x6040"}, kIoVariableScopeTx, "drive", 3, "Ready");
    expectFalse(decision.visible, "scope mismatch hides row");

    decision = evaluateIoVariableFilterRow(row, {"Drive.Controlword", "0x6040"}, kIoVariableScopeProcess, "status", 3,
                                           "Ready");
...
void testStatsAndSummary() {
    ...
    IoVariableFilterDecision decision =
        evaluateIoVariableFilterRow(process, {"Drive.Controlword"}, kIoVariableScopeAll, QString(), 3, "Ready");
    ...
    decision =
        evaluateIoVariableFilterRow(issue, {"Axis.Statusword"}, kIoVariableScopeStartupDiff, QString(), 3, "Ready");
    accumulateIoVariableFilterStats(&stats, decision);
    ...
    expectEqual(ioVariableFilterSummaryText(stats, "All",
