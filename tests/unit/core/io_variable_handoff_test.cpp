...
void testSuggestedAlias() {
    ...
    expectEqual(suggestedIoVariableAlias(row, QString(), true), "S3_6040_00_Drive_Controlword",
                "address-prefixed alias from symbol");
    expectEqual(suggestedIoVariableAlias(row, "Line 1", true), "Line_1_S3_6040_00_Drive_Controlword",
                "prefixed alias from symbol");

    row.symbol.clear();
    row.meaning = "Control word & enable";
    expectEqual(suggestedIoVariableAlias(row, QString(), false), "Control_word_and_enable", "alias from meaning");
}
...
void testIssuesAndDuplicates() {
    ...
    const QVector<IoVariableHandoffIssue> readyIssues = ioVariableHandoffIssues(ready, nullptr);
    expectTrue(readyIssues.isEmpty(), "ready row has no handoff issue");
    expectEqual(ioVariableHandoffName(ready).symbol, "AxisX_Controlword", "handoff symbol uses alias");
    expectEqual(ioVariableHandoffPlcDirection(ready), "Output", "handoff PLC direction");
    expectEqual(ioVariableHandoffPlcType(ready), "UINT", "handoff PLC type");

    IoVariableTableRow missing = baseRow();
    const QVector<IoVariableHandoffIssue> missingIssues = ioVariableHandoffIssues(missing, nullptr);
    expectTrue(ioVariableHandoffHasIssue(missingIssues, IoVariableHandoffIssue::MissingAlias),
               "missing alias issue detected");
    expectTrue(ioVariableHandoffIssueKeys(missingIssues).contains("missingAlias"), "missing alias key emitted");

    IoVariableTableRow generated = baseRow();
    generated.alias = suggestedIoVariableAlias(generated, QString(), true);
    const QVector<IoVariableHandoffIssue> generatedIssues = ioVariableHandoffIssues(generated, nullptr);
    expectTrue(ioVariableHandoffHasIssue(generatedIssues, IoVariableHandoffIssue::AutoName),
    ...
    expectTrue(duplicates.contains("axisx_controlword"), "duplicate symbol detected");
    const QVector<IoVariableHandoffIssue> duplicateIssues = ioVariableHandoffIssues(ready, &duplicates);
    expectTrue(ioVariableHandoffHasIssue(duplicateIssues, IoVariableHandoffIssue::DuplicateSymbol),
...
void testComment() {
    ...
    const QString comment = ioVariableHandoffComment(row, {"Missing Alias", "No Tags"});
    expectTrue(comment.contains("#3 0x6040:0x00"), "comment includes object address");
    expectTrue(comment.contains("Output"), "comment includes direction");
    expectTrue(comment.contains("0x1600"), "comment includes PDO");
    expectTrue(comment.contains("Quality: Missing Alias | No Tags"), "comment includes quality labels");
    expectTrue(!comment.contains("(*") && !comment.contains("*)"), "comment strips nested ST comment delimiters");
}
...
void testDeclarationsAndCsv() {
    ...
    expectEqual(ioVariableHandoffUniqueSymbol(first, &usedSymbols), "Axis_Controlword", "first symbol keeps alias");
    expectEqual(ioVariableHandoffUniqueSymbol(second, &usedSymbols), "Axis_Controlword_2",
                "duplicate symbol receives suffix");

    const QString block = ioVariableHandoffDeclarationBlock({first, second}, {{}, {"Duplicate Symbol"}});
    expectTrue(block.startsWith("VAR_GLOBAL"), "declaration block starts");
    expectTrue(block.contains("Axis_Controlword : UINT;"), "declaration block contains first symbol");
    expectTrue(block.contains("Axis_Controlword_2 : UINT;"), "declaration block contains suffixed duplicate");
    expectTrue(block.contains("Quality: Duplicate Symbol"), "declaration block keeps quality label");
    ...
    usedSymbols.clear();
    const IoVariableHandoffCsvRow csv = ioVariableHandoffCsvRow(first, &usedSymbols, "2026-06-09T12:00:00");
    ...
    expectEqual(csv.values.value(21), "Drive.Controlword", "PLC CSV default name");
    expectEqual(csv.values.value(22), "2026-06-09T12:00:00", "PLC CSV exported time");
}
