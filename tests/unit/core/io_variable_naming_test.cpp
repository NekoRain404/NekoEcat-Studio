...
IoVariableTableRow rowFor(int rowNumber, int position, const QString& index, const QString& alias,
...
void testKeepExistingAliasAndMergeTags() {
    const IoVariableTableRow first = rowFor(0, 3, "0x6040", "Existing_Alias", "Drive.Controlword");
    const IoVariableTableRow second = rowFor(1, 3, "0x6041", QString(), "Drive.Statusword");
    const IoVariableTableRow reserved = rowFor(2, 4, "0x6040", "Reserved_Alias", "Other.Controlword");
    ...
    const IoVariableBulkNamingResult result =
        buildIoVariableBulkNamingPlan({first, second, reserved}, {0, 1}, metadata, options);

    expectEqual(result.updated, 2, "both keyed rows receive metadata updates");
    expectEqual(result.skippedExistingAliases, 1, "existing alias is counted as skipped");
    ...
    expectEqual(firstUpdate.value(0), "Existing_Alias", "kept row preserves alias");
    expectEqual(firstUpdate.value(1), "axis, plc, output, uint", "kept row merges requested and inferred tags");
    expectEqual(firstUpdate.value(2), "keep", "kept row preserves note");

    const QStringList secondUpdate = result.metadataUpdates.value("3|0x6041|0x00");
    expectEqual(secondUpdate.value(0), "Line_1_S3_6041_00_Drive_Statusword", "new row gets prefixed generated alias");
    expectEqual(secondUpdate.value(1), "plc, axis, output, uint", "new row receives requested and inferred tags");
}

// Test replacing aliases with unique suffix for duplicates
void testReplaceExistingAliasesAndSuffixDuplicates() {
    IoVariableTableRow first = rowFor(0, 3, "0x6040", "Axis_Controlword", "Axis Controlword");
    IoVariableTableRow second = rowFor(1, 3, "0x6041", QString(), "Axis Controlword");
    ...
    const IoVariableBulkNamingResult result = buildIoVariableBulkNamingPlan({first, second}, {0, 1}, metadata, options);

    expectEqual(result.updated, 2, "replacement updates both rows");
    expectEqual(result.skippedExistingAliases, 0, "replacement does not skip existing aliases");
    expectEqual(result.metadataUpdates.value("3|0x6040|0x00").value(0), "Axis_Controlword",
                "first replacement takes base alias");
    expectEqual(result.metadataUpdates.value("3|0x6041|0x00").value(0), "Axis_Controlword_2",
                "second replacement gets unique suffix");
}
...
void testInvalidRowsAreSkipped() {
    ...
    const IoVariableBulkNamingResult result =
        buildIoVariableBulkNamingPlan({invalid}, {0, 99}, {}, IoVariableBulkNamingOptions{});
