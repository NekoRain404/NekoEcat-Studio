...
void testSelectAndFocusTableRow() {
    ...
    for (int row = 0; row < table.rowCount(); ++row) {
        for (int column = 0; column < table.columnCount(); ++column) {
            table.setItem(row, column, new QTableWidgetItem(QString("%1:%2").arg(row).arg(column)));
        }
    }
    ...
    expectTrue(selectAndFocusTableRow(&table, 1, -4), "negative column falls back to zero");
    expectEqual(table.currentRow(), 1, "fallback selected row");
    expectEqual(table.currentColumn(), 0, "fallback selected column");

    expectTrue(selectAndFocusTableRow(&table, 0, table.columnCount() + 3), "out-of-range column falls back to zero");
    ...
    expectTrue(!selectAndFocusTableRow(&table, -1, 0), "negative row is rejected");
    expectTrue(!selectAndFocusTableRow(&table, table.rowCount(), 0), "out-of-range row is rejected");
...
void testRowLookupHelpers() {
    ...
    expectEqual(tableRowForPosition(&table, 2, 0), 1, "position lookup finds matching row");
    expectEqual(tableRowForPosition(&table, 9, 0), -1, "position lookup rejects missing rows");
    expectEqual(tableRowForPosition(nullptr, 2, 0), -1, "position lookup rejects null tables");
    ...
    expectEqual(tableRowForObjectIndex(&table, "6060", "0", 1, 2), 1, "object index lookup normalizes target hex text");
    ...
    expectEqual(firstVisibleTableRow(nullptr), -1, "visible row lookup rejects null tables");
}
