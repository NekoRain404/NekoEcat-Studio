...
void expectState(WatchStartupDeltaState actual, WatchStartupDeltaState expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}

WatchStartupWatchRow watch(int row, int position, QString index, QString subIndex, QString value) {
    return {row, position, index, subIndex, value};
}

WatchStartupStartupRow startup(int row, int position, QString index, QString subIndex, QString value,
                               QString type = "uint16") {
...
void testWatchRowMatch() {
    expectTrue(watchStartupHasTarget(2, "6040", "0"), "watch/startup target accepts complete addresses");
    expectTrue(!watchStartupHasTarget(-1, "6040", "0"), "watch/startup target rejects invalid positions");
    ...
    const WatchStartupWatchMatch match =
        watchStartupMatchForWatchRow(startupRows, watch(4, 2, "0x6040", "0x00", "0x0006"));
    expectEqual(match.matchingStartupRows, 2, "watch target finds all matching startup rows");
    expectState(match.state, WatchStartupDeltaState::Match, "matching watch value reports match");
...
void testWatchRowPendingAndDiff() {
    ...
    expectState(watchStartupMatchForWatchRow(startupRows, watch(4, 2, "0x6040", "0x00", "")).state,
                WatchStartupDeltaState::Pending, "empty watch value is pending");
    expectState(watchStartupMatchForWatchRow(startupRows, watch(4, 2, "0x6040", "0x00", "0x0007")).state,
                WatchStartupDeltaState::Diff, "different watch value reports diff");
}
...
void testStartupDeltasAndSummary() {
    ...
    const QVector<WatchStartupStartupRow> startupRows = {
        startup(0, 1, "0x6040", "0x00", "0x0006"),  startup(1, 1, "0x6060", "0x00", "9"),
        startup(2, 1, "0x607a", "0x00", "100"),     startup(3, 1, "0x6081", "0x00", "1000"),
    ...
    expectState(deltas.at(0).state, WatchStartupDeltaState::Match, "same value reports match");
    expectState(deltas.at(1).state, WatchStartupDeltaState::Diff, "different value reports diff");
    expectState(deltas.at(2).state, WatchStartupDeltaState::Pending, "empty watch value reports pending");
    expectState(deltas.at(3).state, WatchStartupDeltaState::MissingWatch, "missing watch row reports missing watch");
    expectState(deltas.at(4).state, WatchStartupDeltaState::NoTarget, "invalid startup target reports no target");
