...
void testTypeRules() {
    ...
    expectEqual(processDataNormalizedKnownType(" Word "), "uint16", "IEC alias normalized to SDO type");
    expectEqual(processDataNormalizedKnownType("Real64"), "double", "real64 normalized to double");
    expectEqual(processDataIecTypeFromNormalizedType("uint16"), "UINT", "uint16 maps to IEC UINT");
    expectEqual(processDataIecTypeFromNormalizedType("double"), "LREAL", "double maps to IEC LREAL");
}
...
void testIoRowRules() {
    ...
    expectEqual(ioVariableTableRowPreferredValue(row), "0x0006", "preferred value keeps Raw priority");
    expectEqual(ioVariableTableRowStartupValue(row), "0x000F", "startup value keeps Watch priority");
    expectEqual(ioVariableTableRowTypeFromBits(row), "uint16", "type inferred from bits");
    expectEqual(ioVariableTableRowSdoType(row), "uint16", "type inferred from source alias");
    expectEqual(ioVariableTableRowIecType(row), "UINT", "IEC type inferred");
    expectEqual(ioVariableTableRowKey(row), "2|0x6040|0x00", "I/O row key generated");
    expectTrue(ioVariableTableRowHasProcessSource(row), "process source detected");
    ...
    expectFalse(ioVariableTableRowHasMissingValue(row), "row with raw/watch is not missing value");
}
...
void testFallbackRules() {
    ...
    expectEqual(ioVariableTableRowSdoType(row), "uint8", "source without type falls back to bits");
    expectEqual(ioVariableTableRowIecType(row), "USINT", "IEC type falls back to bit width");
    expectTrue(ioVariableTableRowHasPdoSource(row), "PDO source detected");
    expectTrue(ioVariableTableRowIsTx(row), "Tx input detected");
    expectTrue(ioVariableTableRowHasMissingValue(row), "row without raw/watch is missing value");

    row.positionValid = false;
    expectFalse(ioVariableTableRowHasTarget(row), "invalid position has no target");
    expectEqual(ioVariableTableRowKey(row), QString(), "invalid target has no key");
}
