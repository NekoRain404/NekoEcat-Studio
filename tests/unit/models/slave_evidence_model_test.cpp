...
void expectPriority(SlaveEvidencePriority actual, SlaveEvidencePriority expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}

void expectNext(SlaveEvidenceNextAction actual, SlaveEvidenceNextAction expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}

void expectRoute(SlaveEvidenceRouteTarget actual, SlaveEvidenceRouteTarget expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}
...
void testPriorityOrderingAndCounts() {
    ...
    const SlaveEvidenceMatrix matrix = buildSlaveEvidenceMatrix({ready, action, risk, fault});
    ...
    expectPriority(matrix.rows.at(0).priority, SlaveEvidencePriority::Fault, "startup diff is P0");
    expectEqual(matrix.rows.at(1).position, 1, "P1 row sorts before action");
    expectPriority(matrix.rows.at(1).priority, SlaveEvidencePriority::Risk, "missing PREOP watch value is P1");
    expectEqual(matrix.rows.at(2).position, 2, "P2 row sorts before ready");
    expectPriority(matrix.rows.at(2).priority, SlaveEvidencePriority::Action, "partial INIT evidence is P2");
    expectEqual(matrix.rows.at(3).position, 3, "ready row sorts last");
    expectPriority(matrix.rows.at(3).priority, SlaveEvidencePriority::Ready, "complete row is P3");
}
...
void testNextActionOrder() {
    ...
    expectNext(matrix.rows.first().nextAction, SlaveEvidenceNextAction::ReviewOd, "missing OD is reviewed before PDO");

    input.odRows = 10;
    matrix = buildSlaveEvidenceMatrix({input});
    expectNext(matrix.rows.first().nextAction, SlaveEvidenceNextAction::LoadPdo, "missing PDO is next after OD");

    input.pdoRows = 4;
    matrix = buildSlaveEvidenceMatrix({input});
    expectNext(matrix.rows.first().nextAction, SlaveEvidenceNextAction::AddWatch, "missing Watch follows PDO");
    ...
    matrix = buildSlaveEvidenceMatrix({input});
    expectNext(matrix.rows.first().nextAction, SlaveEvidenceNextAction::ReviewStartup, "startup diff follows Watch");

    input.startupDiffs = 0;
    input.processRows = 0;
    matrix = buildSlaveEvidenceMatrix({input});
    expectNext(matrix.rows.first().nextAction, SlaveEvidenceNextAction::ValidateProcess,
...
void testRiskKinds() {
    ...
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::IdentityMissing), "missing identity risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::OdMissing), "missing OD risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::PdoMissing), "missing PDO risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::WatchMissing), "missing Watch risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::ProcessMissing), "missing process risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::StartupDiff), "startup diff risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::PdoMapIssue), "PDO map risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::TopologyBaselineIssue), "topology risk");
    expectTrue(hasRisk(row, SlaveEvidenceRiskKind::DriveFault), "drive fault risk");
}
...
void testRouteTargets() {
    ...
    expectRoute(slaveEvidenceRouteTarget(buildSlaveEvidenceMatrix({input}).rows.first()),
                SlaveEvidenceRouteTarget::ObjectDictionary, "missing OD routes to Object Dictionary");
    ...
    input.processRows = 0;
    expectRoute(slaveEvidenceRouteTarget(buildSlaveEvidenceMatrix({input}).rows.first()),
                SlaveEvidenceRouteTarget::Process, "missing OP process evidence routes to Free Run process");

    input.processRows = 3;
    input.mapIssues = 1;
    expectRoute(slaveEvidenceRouteTarget(buildSlaveEvidenceMatrix({input}).rows.first()),
                SlaveEvidenceRouteTarget::Process, "PDO map issue routes to Free Run process");

    input.mapIssues = 0;
    input.topologyIssue = true;
    expectRoute(slaveEvidenceRouteTarget(buildSlaveEvidenceMatrix({input}).rows.first()),
                SlaveEvidenceRouteTarget::StateMachine, "topology risk routes to State Machine");
