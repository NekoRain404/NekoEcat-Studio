...
void expectKind(ConsistencyEvidenceRouteKind actual, ConsistencyEvidenceRouteKind expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}
...
void testAddressAndStartupParsing() {
    ConsistencyEvidenceAddress address = parseConsistencyEvidenceAddress("#12 0x6040:0x0");
    ...
    expectEqual(parseConsistencyStartupRow("Startup row 4"), 3, "English startup row");
...
void testRouteKinds() {
    expectKind(consistencyEvidenceRouteDecision({.scope = "Topology"}).kind, ConsistencyEvidenceRouteKind::Topology,
               "topology route");
    expectKind(consistencyEvidenceRouteDecision({.scope = "启动"}).kind, ConsistencyEvidenceRouteKind::Startup,
               "startup route");
    expectKind(consistencyEvidenceRouteDecision({.evidence = "watch missing"}).kind,
...
void testIoScopes() {
    ...
    expectEqual(consistencyEvidenceIoScope({.evidence = "Startup diff"}),
                QString::fromLatin1(kConsistencyIoScopeStartupDiff), "startup diff scope");
    expectEqual(consistencyEvidenceIoScope({.actual = "Missing Raw value"}),
                QString::fromLatin1(kConsistencyIoScopeMissingValue), "missing value scope");
    expectEqual(consistencyEvidenceIoScope({.evidence = "PDO map warning"}),
                QString::fromLatin1(kConsistencyIoScopePdo), "PDO scope");
    expectEqual(consistencyEvidenceIoScope({}), QString::fromLatin1(kConsistencyIoScopeAll), "default scope");
}

// Verify full decision payload with address, startup row, and scope
void testDecisionPayload() {
    const ConsistencyEvidenceRouteDecision route = consistencyEvidenceRouteDecision(
        {.scope = "Startup", .target = "#7 0x6060:0x00 Startup row 3", .evidence = "diff"});
    expectKind(route.kind, ConsistencyEvidenceRouteKind::Startup, "decision kind");
    ...
    expectEqual(route.ioScope, QString::fromLatin1(kConsistencyIoScopeStartupDiff), "decision I/O scope");
}
