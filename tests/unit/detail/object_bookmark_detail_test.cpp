...
void testEmptyStates() {
    ObjectBookmarkDetailState state = objectBookmarkDetailUnavailableState(englishTexts());
    expectEqual(state.text, "Object bookmark evidence is not available.", "unavailable text");
...
void testSeverityRules() {
    ...
    expectEqual(objectBookmarkDetailSeverityKey(row, texts), "ok", "writable bookmark with value is ok");

    row.access = "ro";
    expectEqual(objectBookmarkDetailSeverityKey(row, texts), "action", "read-only value is action");

    row.access = "rw";
    row.lastValue.clear();
    expectEqual(objectBookmarkDetailSeverityKey(row, texts), "neutral", "target without value is neutral");

    row.positionValid = false;
    expectEqual(objectBookmarkDetailSeverityKey(row, texts), "warning", "missing target is warning");
}
...
void testSelectedRowState() {
    ...
    const ObjectBookmarkDetailState state = buildObjectBookmarkDetailState(row, englishTexts());
    ...
    expectEqual(state.text,
                ...
                "summary text");
    expectContains(state.tooltipLines, "Selected object bookmark", "tooltip title");
    ...
    expectContains(state.tooltipLines, "Reuse: Ready for Watch or Startup", "tooltip reuse");
...
void testReadOnlyAndFallbackState() {
    ...
    ObjectBookmarkDetailState state = buildObjectBookmarkDetailState(row, englishTexts());
    ...
    row.positionValid = false;
    row.lastValue.clear();
    state = buildObjectBookmarkDetailState(row, englishTexts());
