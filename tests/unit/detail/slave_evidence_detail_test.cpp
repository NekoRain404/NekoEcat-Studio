...
void testDisplayFallbacks() {
    ...
    expectEqual(slaveEvidenceDisplayName("  ", texts), "Unnamed", "blank slave name fallback");
    expectEqual(slaveEvidenceDisplayState("\t", texts), "Unknown", "blank slave state fallback");
    expectEqual(slaveEvidenceDisplayName(" Axis ", texts), "Axis", "slave name is trimmed");
}
...
void testUiRowCellsAndDetails() {
    ...
    row.risks = {{SlaveEvidenceRiskKind::PdoMissing, 0}, {SlaveEvidenceRiskKind::StartupDiff, 1}};
    ...
    expectEqual(uiRow.detailLines.last(), "Risk: PDO missing; Startup diff 1", "detail risk line");
}
