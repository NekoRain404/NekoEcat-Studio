...
StateMachineRowDetailTexts englishTexts() {
    return {
        ...
        .summaryPattern = "#%1 %2 | Current: %3 | Recommended: %4 | %5 | Risk: %6 | Action: "
                          "%7",
...
void testEmptyStates() {
    StateMachineRowDetailState state = stateMachineRowDetailUnavailableState(englishTexts());
    expectEqual(state.text, "State evidence is not available.", "unavailable text");
...
void testSeverityRules() {
    ...
    expectEqual(stateMachineRowDetailSeverityKey(row, texts), "action", "non-OP recommendation is action");

    row.recommended = "OP";
    expectEqual(stateMachineRowDetailSeverityKey(row, texts), "warning", "OP recommendation is warning");

    row.risk = "offline";
    expectEqual(stateMachineRowDetailSeverityKey(row, texts), "error", "severe risk is error");

    row.risk.clear();
    row.recommended.clear();
    row.current = "OP";
    expectEqual(stateMachineRowDetailSeverityKey(row, texts), "ok", "OP current row is ok");

    row.current = "SAFEOP";
    expectEqual(stateMachineRowDetailSeverityKey(row, texts), "neutral", "review-only row is neutral");
}

// Test selected row detail state, boundary, and tooltip
void testSelectedRowState() {
    const StateMachineTableRow row = readyRow();
    const StateMachineRowDetailState state = buildStateMachineRowDetailState(row, englishTexts());

    expectEqual(state.severityKey, "action", "selected severity");
    expectEqual(state.boundary, "SAFEOP transition requires confirmation", "safeop boundary");
    ...
    expectEqual(state.text,
                ...
                "Send SAFEOP",
                "summary text");
    expectContains(state.tooltipLines, "Selected state-machine row", "tooltip title");
    expectContains(state.tooltipLines, "Slave: #3 Drive A", "tooltip slave");
    expectContains(state.tooltipLines, "Current state: PREOP", "tooltip current state");
    expectContains(state.tooltipLines, "Recommended state: SAFEOP", "tooltip recommended state");
    expectContains(state.tooltipLines, "Boundary: SAFEOP transition requires confirmation", "tooltip boundary");
