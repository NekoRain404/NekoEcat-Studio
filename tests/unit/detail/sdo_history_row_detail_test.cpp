...
void testEmptyStates() {
    SdoHistoryRowDetailState state = sdoHistoryRowDetailUnavailableState(englishTexts());
    expectEqual(state.text, "SDO history evidence is not available.", "unavailable text");
...
void testSeverityRules() {
    ...
    expectEqual(sdoHistoryRowDetailSeverityKey(row, texts), "ok", "completed read is ok");

    row.status = "Failed";
    expectEqual(sdoHistoryRowDetailSeverityKey(row, texts), "error", "failed history is error");

    row.status = "Requested";
    expectEqual(sdoHistoryRowDetailSeverityKey(row, texts), "warning", "requested history is warning");

    row.status = "Complete";
    row.action = "Write";
    expectEqual(sdoHistoryRowDetailSeverityKey(row, texts), "action", "write history is action");

    row.action = "Read";
    row.positionValid = false;
    expectEqual(sdoHistoryRowDetailSeverityKey(row, texts), "warning", "missing target is warning");
}
...
void testSelectedRowState() {
    ...
    const SdoHistoryRowDetailState state = buildSdoHistoryRowDetailState(row, englishTexts());
    ...
    expectEqual(state.text,
                ...
                "summary text");
    expectContains(state.tooltipLines, "Selected SDO history row", "tooltip title");
    ...
    expectContains(state.tooltipLines, "Reuse: Can seed Watch or Startup", "tooltip reuse");
