...
WatchRowDetailTexts englishTexts() {
    return {
        ...
        .matchText = "Match",
        .pendingText = "Pending",
        .summaryPattern = "#%1 %2:%3 | %4 | Value: %5 | Baseline: %6 | Startup: %7 | %8",
...
void testEmptyStates() {
    ...
    expectEqual(state.text, "Watch evidence is not available.", "unavailable text");
...
void testSeverityRules() {
    ...
    expectEqual(watchRowDetailSeverityKey(row, texts), "ok", "stable valued row is ok");
    expectTrue(watchRowDetailIsMatchText("0", texts), "zero is match text");
    expectTrue(watchRowDetailIsMatchText("Pending", texts), "pending is match text");
    expectTrue(watchRowDetailIsCia402(row), "CiA mode is drive evidence");

    row.changed = true;
    expectEqual(watchRowDetailSeverityKey(row, texts), "action", "changed row severity");

    row.changed = false;
    row.baselineDelta = "0x0001";
    expectEqual(watchRowDetailSeverityKey(row, texts), "warning", "baseline drift severity");

    row.baselineDelta = "Match";
    row.startupDelta = "Diff";
    expectEqual(watchRowDetailSeverityKey(row, texts), "error", "startup drift severity");

    row.startupDelta = "Match";
    row.value.clear();
    expectEqual(watchRowDetailSeverityKey(row, texts), "warning", "missing value severity");
}
...
void testSelectedRowState() {
    ...
    const WatchRowDetailState state = buildWatchRowDetailState(row, englishTexts());
    ...
    expectContains(state.tooltipLines, "Drive Evidence: CiA 402 candidate", "tooltip drive evidence");
