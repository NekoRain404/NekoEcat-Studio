...
void testEmptyStates() {
    WorkflowStepDetailState state = workflowStepDetailUnavailableState(detailTexts());
    expectEqual(state.text, "Workflow evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select a visible workflow row.", "no selection text");
...
void testSeverityRules() {
    ...
    expectEqual(workflowStepDetailSeverityKey(row, workflow, detail), "action", "action status severity");

    row.status = "Translated Ready";
    row.statusKey = "ready";
    row.risk = "None";
    expectEqual(workflowStepDetailSeverityKey(row, workflow, detail), "ok", "status key drives ready severity");

    row.statusKey = "blocked";
    expectEqual(workflowStepDetailSeverityKey(row, workflow, detail), "warning", "blocked status severity");

    row.risk = "Drive failed";
    expectEqual(workflowStepDetailSeverityKey(row, workflow, detail), "error", "severe risk overrides status");
}

// Test selected row detail state with boundary, risk, and tooltip
void testSelectedRowState() {
    const WorkflowStepDetailState state = buildWorkflowStepDetailState(actionRow(), workflowTexts(), detailTexts());
    ...
    expectContains(state.tooltipLines, "Step: Inspect Object Dictionary", "tooltip step");
    expectContains(state.tooltipLines, "Boundary: Online OD read", "tooltip boundary");
    expectContains(state.tooltipLines, "Local review boundary.", "tooltip local boundary");
...
void testLocalizedRiskAndNoRiskFallback() {
    ...
    workflow.blocked = "受阻";
    WorkflowStepDetailState state = buildWorkflowStepDetailState(row, workflow, detailTexts());
    expectEqual(state.severityKey, "error", "localized severe risk");
    expectTrue(state.severeRisk, "localized severe risk flag");

    row.risk.clear();
    state = buildWorkflowStepDetailState(row, workflow, detailTexts());
