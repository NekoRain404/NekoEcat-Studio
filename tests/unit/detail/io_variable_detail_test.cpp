...
void testEmptyStates() {
    IoVariableDetailState state = ioVariableDetailUnavailableState(englishTexts());
    expectEqual(state.text, "I/O variable evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select a visible I/O variable.", "no selection text");
...
void testSeverityAndSignalState() {
    ...
    expectEqual(ioVariableDetailSeverityKey(row, texts.readyText), "ok", "ready row severity");
    expectEqual(ioVariableDetailSignalState(row, texts), "Ready evidence", "ready signal state");

    row.startup = "Diff";
    expectEqual(ioVariableDetailSeverityKey(row, texts.readyText), "error", "startup diff severity");
    expectEqual(ioVariableDetailSignalState(row, texts), "Startup mismatch", "startup diff signal state");

    row.startup.clear();
    row.map = "Missing in PDO map";
    expectEqual(ioVariableDetailSeverityKey(row, texts.readyText), "warning", "map issue severity");
    expectEqual(ioVariableDetailSignalState(row, texts), "Map issue", "map issue signal state");

    row.map.clear();
    row.plcQuality = "Missing Alias";
    expectEqual(ioVariableDetailSignalState(row, texts), "PLC review", "PLC issue signal state");
}

// Verify selected row state, summary, and tooltip
void testSelectedRowState() {
    const IoVariableDetailState state = buildIoVariableDetailState(readyRow(), englishTexts());
    ...
    expectContains(state.tooltipLines, "Signal State: Ready evidence", "tooltip signal state");
...
void testFallbackText() {
    ...
    const IoVariableDetailState state = buildIoVariableDetailState(row, englishTexts());
