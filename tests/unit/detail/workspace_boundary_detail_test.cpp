...
void testOverviewActionAndWarningStates() {
    ...
    WorkspaceBoundaryDetail state =
        buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::Overview, "Overview 3", counts, englishTexts());
    ...
    expectContains(state.details, "Matrix queue: P0 0 | P1 0 | P2 3 | P3 4", "overview matrix detail");
    expectEqual(state.tooltip, state.details.join('\n'), "overview tooltip");

    counts.matrixP1 = 1;
    state = buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::Overview, "Overview", counts, englishTexts());
...
void testHighRiskWorkspaceKinds() {
    WorkspaceBoundaryDetail state =
        buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::StartupSdo, "Startup SDO", {}, englishTexts());
    expectEqual(state.label, "Boundary: Startup Danger", "startup label");
    expectEqual(state.severityKey, "error", "startup severity");
    expectContains(state.details, "Apply and Verify use online SDO access.", "startup online detail");

    state = buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::FreeRun, "Free Run", {}, englishTexts());
    expectEqual(state.label, "Boundary: Process Data", "free run label");
    expectEqual(state.severityKey, "error", "free run severity");

    state = buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::StateMachine, "State Machine", {}, englishTexts());
...
void testDiagnosticsAndLocalWorkspaceKinds() {
    WorkspaceBoundaryDetail state =
        buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::Diagnostics, "Diagnostics", {}, englishTexts());
    expectEqual(state.label, "Boundary: Host", "diagnostics label");
    expectEqual(state.severityKey, "warning", "diagnostics severity");
    expectContains(state.details, "Diagnostics is the only Host Health workspace.", "diagnostics detail");

    state = buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::Consistency, "Consistency", {}, englishTexts());
    expectEqual(state.label, "Boundary: Local Gate", "consistency label");
    expectEqual(state.severityKey, "ok", "consistency severity");

    state = buildWorkspaceBoundaryDetail(WorkspaceBoundaryKind::RawEvidence, "Raw", {}, englishTexts());
    expectEqual(state.label, "Boundary: Raw Evidence", "raw label");
    expectEqual(state.severityKey, "neutral", "raw severity");
    expectContains(state.details, "Raw output pages show cached output.", "raw detail");
}
