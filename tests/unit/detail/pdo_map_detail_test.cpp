...
void testEmptyStates() {
    ...
    expectEqual(state.text, "PDO Map evidence is not available.", "unavailable text");
...
void testDirectionSeverityAndDriveEvidence() {
    ...
    expectEqual(pdoMapDetailSeverityKey(row), "action", "Rx output gets action severity");
    ...
    row.bits = "0";
    expectEqual(pdoMapDetailSeverityKey(row), "warning", "invalid bits warn before direction role");
...
void testSelectedRowState() {
    ...
    const PdoMapDetailState state = buildPdoMapDetailState(row, 3, englishTexts());
...
void testFallbackState() {
    PdoMapTableRow row;
    const PdoMapDetailState state = buildPdoMapDetailState(row, 9, englishTexts());
