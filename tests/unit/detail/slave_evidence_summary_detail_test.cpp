...
SlaveEvidenceSummaryTexts englishTexts() {
    return {
        .selectSlaveText = "Evidence: select a slave",
        .ready = "ready",
        .missing = "missing",
        .summaryPattern = "Evidence %1/5 | ID %2 | OD %3 | PDO %4 | Watch %5/%6 values | "
...
void testNoSelectionState() {
    const SlaveEvidenceSummaryDetail state = slaveEvidenceNoSelectionState(englishTexts());
...
void testSummaryAndSeverityRules() {
    ...
    expectEqual(slaveEvidenceSummaryGroupCount(input), 5, "ready evidence groups");
    expectEqual(slaveEvidenceSummarySeverityKey(input, 0), "ok", "ready severity");

    SlaveEvidenceSummaryDetail state = buildSlaveEvidenceSummaryDetail(input, 0, texts);
    ...
    expectEqual(state.text,
                ...
                "ready summary text");
    expectEqual(state.tooltipLines.first(), "Selected slave evidence score: 5/5", "score tooltip");
    expectEqual(state.tooltip, state.tooltipLines.join('\n'), "tooltip text");

    input.startupDiffs = 1;
    expectEqual(slaveEvidenceSummarySeverityKey(input, 0), "warning", "startup diff severity");
    state = buildSlaveEvidenceSummaryDetail(input, 2, texts);
    expectEqual(state.severityKey, "warning", "warning state severity");
    expectContains(state.tooltipLines, "Startup SDO has 1 mismatch row(s)", "startup diff tooltip");
    expectContains(state.tooltipLines, "Topology baseline has 2 issue(s)", "topology tooltip");
}
...
void testMissingEvidenceFallbacks() {
    ...
    const SlaveEvidenceSummaryDetail state = buildSlaveEvidenceSummaryDetail(input, 0, texts);
    ...
    expectContains(state.tooltipLines, "Missing identity evidence", "missing identity tooltip");
    expectContains(state.tooltipLines, "Missing PDO evidence", "missing PDO tooltip");
    expectContains(state.tooltipLines, "Missing Watch values", "missing Watch tooltip");
    expectContains(state.tooltipLines, "Missing process evidence", "missing process tooltip");
}
