...
WatchStartupWatchRow watchRow(int position, const QString& index, const QString& value, const QString& decoded) {
...
void testNoWatchAndNoCia402Fallbacks() {
    ...
    state = buildSelectedDriveSummaryDetail({watchRow(2, "0x2000", "1", "Vendor object")}, 2, texts);
    expectEqual(state.text, "Drive: no CiA 402 Watch evidence", "no CiA 402 text");
...
void testOperationEnabledSummary() {
    ...
    const SelectedDriveSummaryDetail state = buildSelectedDriveSummaryDetail(rows, 2, englishTexts());
...
void testErrorCodeEvidence() {
    ...
    SelectedDriveSummaryDetail state =
        buildSelectedDriveSummaryDetail({watchRow(2, "0x603f", "0", "No error")}, 2, texts);
    expectEqual(state.text, "Drive: no CiA 402 Watch evidence", "zero error code is not surfaced");

    state = buildSelectedDriveSummaryDetail({watchRow(2, "0x603f", "0x2310", "Error code 0x2310")}, 2, texts);
    expectEqual(state.text, "Drive: Error code 0x2310", "non-zero error code summary text");
...
void testRecommendedControlword() {
    Cia402ControlwordRecommendation recommendation =
        selectedDriveControlwordRecommendation({watchRow(2, "0x6041", "0x0040", "Switch on disabled (0x0040)")}, 2);
    expectEqual(recommendation.label, "Shutdown", "recommended label");
    expectEqual(recommendation.value, "0x0006", "recommended value");
    expectEqual(recommendation.reason, "Switch on disabled (0x0040)", "recommended reason");

    recommendation =
        selectedDriveControlwordRecommendation({watchRow(2, "0x6041", "0x0027", "Operation enabled (0x0027)")}, 2);
