...
void expectContains(const QStringList& actual, const QString& expected, const QString& message) {
...
SdoTargetTrailDetailTexts englishTexts() {
    return {
        ...
        .watchBookmarkOnly = "Can seed Watch or Bookmark; Startup needs writable value",
        .missingAddress = "Missing target address",
        .noSavedValue = "No saved value; fill target only",
        .summaryPattern = "%1 | #%2 %3:%4 %5 | Source: %6 | Value: %7 | Write: %8 | %9",
...
void testEmptyStates() {
    SdoTargetTrailDetailState state = sdoTargetTrailDetailUnavailableState(englishTexts());
    expectEqual(state.text, "SDO target trail evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select an SDO target trail row.", "no selection text");
...
void testSeverityRules() {
    ...
    expectEqual(sdoTargetTrailDetailSeverityKey(row, true, texts), "ok", "startup-capable row is ok");
    expectEqual(sdoTargetTrailDetailSeverityKey(row, false, texts), "action", "valued non-startup row is action");

    row.value.clear();
    row.writeValue.clear();
    expectEqual(sdoTargetTrailDetailSeverityKey(row, false, texts), "neutral", "target without saved value is neutral");

    row.positionValid = false;
    expectEqual(sdoTargetTrailDetailSeverityKey(row, false, texts), "warning", "missing target is warning");
}
...
void testSelectedRowState() {
    ...
    const SdoTargetTrailDetailState state = buildSdoTargetTrailDetailState(row, true, englishTexts());

    expectEqual(state.severityKey, "ok", "selected severity");
    expectEqual(state.reuse, "Ready for Watch, Bookmark, or Startup", "reuse text");
    ...
    expectEqual(state.text,
                ...
                "Startup",
                "summary text");
    expectContains(state.tooltipLines, "Selected SDO target trail row", "tooltip title");
    ...
    expectContains(state.tooltipLines, "Startup Candidate: 0x000F", "tooltip startup value");
    expectContains(state.tooltipLines, "Reuse: Ready for Watch, Bookmark, or Startup", "tooltip reuse");
...
void testFallbackState() {
    ...
    row.subIndex = "0x00";
    row.value = "8";
    const SdoTargetTrailDetailState state = buildSdoTargetTrailDetailState(row, false, englishTexts());
    ...
    expectEqual(state.reuse, "Can seed Watch or Bookmark; Startup needs writable value", "fallback reuse");
    expectEqual(state.text,
