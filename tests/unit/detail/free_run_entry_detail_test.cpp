...
FreeRunEntryDetailTexts englishTexts() {
    return {
        ...
        .mappedText = "Mapped",
        .summaryPattern = "#%1 %2 %3:%4 | %5 | %6 bit @ %7.%8 | %9 | Value: %10 | Map: %11",
...
void testEmptyStates() {
    FreeRunEntryDetailState state = freeRunEntryDetailUnavailableState(englishTexts());
    expectEqual(state.text, "Process-image evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select a visible process-image row.", "no selection text");
...
void testSeverityRules() {
    ...
    expectEqual(freeRunEntryDetailSeverityKey(row, texts), "ok", "mapped input severity");

    row.changed = true;
    expectEqual(freeRunEntryDetailSeverityKey(row, texts), "action", "changed input severity");

    row.changed = false;
    row.direction = "Rx Output";
    expectTrue(freeRunEntryDetailIsOutputLike(row), "rx row is output-like");
    expectEqual(freeRunEntryDetailSeverityKey(row, texts), "warning", "output row severity");

    row.mapStatus = "Mapped warning: bit mismatch 16/32";
    expectEqual(freeRunEntryDetailSeverityKey(row, texts), "error", "bit mismatch severity");

    row.mapStatus = "Missing in PDO map";
    row.direction = "Tx Input";
    expectEqual(freeRunEntryDetailSeverityKey(row, texts), "warning", "missing map severity");
}

// Verify selected row state, summary, and tooltip
void testSelectedRowState() {
    const FreeRunEntryTableRow row = readyInputRow();
    const FreeRunEntryDetailState state = buildFreeRunEntryDetailState(row, englishTexts());
    ...
    expectEqual(state.text,
                ...
                "Mapped",
                "summary text");
    expectContains(state.tooltipLines, "Selected Free Run process-image entry", "tooltip title");
    ...
    expectContains(state.tooltipLines, "Name Source: PDO map", "tooltip name source");
