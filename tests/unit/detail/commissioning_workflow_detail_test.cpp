...
void testStatusRulesAndKeys() {
    expectStatus(commissioningWorkflowStatus(true, false), CommissioningWorkflowStatus::Ready, "done row is ready");
    expectStatus(commissioningWorkflowStatus(false, true), CommissioningWorkflowStatus::Action,
                 "ready row needs action");
    expectStatus(commissioningWorkflowStatus(false, false), CommissioningWorkflowStatus::Blocked,
                 "not ready row blocks");
    expectEqual(commissioningWorkflowColorKey(CommissioningWorkflowStatus::Ready), "ready", "ready color key");
    expectEqual(commissioningWorkflowColorKey(CommissioningWorkflowStatus::Action), "action", "action color key");
    expectEqual(commissioningWorkflowColorKey(CommissioningWorkflowStatus::Blocked), "blocked", "blocked color key");
}
...
void testHeadersUiRowAndStats() {
    ...
    expectEqual(rows.at(1).tooltip,
                ...
                "workflow tooltip");
    expectEqual(commissioningWorkflowTableRows(rows).size(), 2, "table row count");
...
void testStepBoundaries() {
    ...
    const CommissioningWorkflowStepBoundary odBoundary =
        commissioningWorkflowStepBoundary(CommissioningWorkflowStep::InspectObjectDictionary, texts);
    ...
    const CommissioningWorkflowStepBoundary processBoundary =
        commissioningWorkflowStepBoundary(CommissioningWorkflowStep::ValidateProcessImage, texts);
    expectEqual(processBoundary.kind, "Process data action", "process boundary kind");
    expectEqual(processBoundary.detail, "Validate process image boundary", "process boundary detail");
}
