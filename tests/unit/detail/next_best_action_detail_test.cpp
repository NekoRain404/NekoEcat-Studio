...
void expectIcon(NextBestActionIconKey actual, NextBestActionIconKey expected, const QString& message) {
    if (actual != expected) {
        fail(message);
    }
}
...
void testBasicActionDetail() {
    ...
    NextBestActionDecision decision{NextBestActionKind::Connect, NextBestActionSeverity::Action};
    const NextBestActionDetail state = buildNextBestActionDetail(decision, input, englishTexts());
...
void testConsistencyTextVariants() {
    NextBestActionInput input;
    NextBestActionDecision decision{NextBestActionKind::Consistency, NextBestActionSeverity::Action};
    NextBestActionDetail state = buildNextBestActionDetail(decision, input, englishTexts());
    expectEqual(state.text, "Run Consistency", "run consistency text");
    expectEqual(state.tip, "Run consistency", "run consistency tip");
    expectIcon(state.icon, NextBestActionIconKey::DetailedView, "run consistency icon");
    ...
    state = buildNextBestActionDetail(decision, input, englishTexts());
    ...
    expectIcon(state.icon, NextBestActionIconKey::Warning, "review consistency icon");
}
...
void testMatrixAndCommandPaletteDetail() {
    ...
    input.matrixP1 = 2;
    input.matrixP2 = 3;
    NextBestActionDecision decision{NextBestActionKind::MatrixReview, NextBestActionSeverity::Warning};
    NextBestActionDetail state = buildNextBestActionDetail(decision, input, englishTexts());
    ...
    decision = {NextBestActionKind::CommandPalette, NextBestActionSeverity::Neutral};
    state = buildNextBestActionDetail(decision, input, englishTexts());
...
int main(int argc, char** argv) {
    ...
    testBasicActionDetail();
    testConsistencyTextVariants();
    testMatrixAndCommandPaletteDetail();
    return 0;
}
