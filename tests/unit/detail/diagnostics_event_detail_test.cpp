...
void testHeadersAndColorKeys() {
    ...
    expectEqual(diagnosticsEventColorKey("Warning"), "warning", "warning color key");
...
void testEmptySummary() {
    const DiagnosticsEventSummary summary = diagnosticsEventSummary({}, englishTexts());
...
void testSummaryCountsVisibleAndHiddenRows() {
    ...
    const DiagnosticsEventSummary summary = diagnosticsEventSummary(rows, englishTexts());
    ...
    expectEqual(summary.text, "3/4 shown   Error: 1   Warning: 1   Info: 1", "summary text");
}
...
void testLevelCountsIgnoreUnknownLevels() {
    const DiagnosticsEventSummary counts = diagnosticsEventCounts({"Error", "Warning", "Info", "Debug", "Error"});
