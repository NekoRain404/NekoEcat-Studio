...
QJsonObject check(const QString& level, const QString& source, const QString& message, const QString& hint,
...
void testEmptyState() {
    const HostHealthDetail state = buildHostHealthDetail(QJsonArray(), englishTexts());
    expectEqual(state.headers.size(), 6, "header count");
    expectEqual(state.rows.size(), 0, "empty row count");
    expectEqual(state.summary, "Host health has not been checked", "unchecked summary");
}
...
void testRowsCountsSummaryAndColors() {
    QJsonArray checks;
    checks.append(
        check("Error", "Service", "offline", "restart", "sudo systemctl restart ethercat", "service stopped"));
    checks.append(check("Warning", "NIC", "driver", "review", "modinfo r8152", "dkms preferred"));
    checks.append(check("Info", "Device", "ready", "", "", "ok"));

    const HostHealthDetail state = buildHostHealthDetail(checks, englishTexts());
    ...
    expectEqual(state.summary, "Needs attention: 1   Warning: 1   OK: 1", "error summary");
}
...
void testWarningAndReadySummaries() {
    ...
    HostHealthDetail state = buildHostHealthDetail(warningChecks, englishTexts());
    expectEqual(state.summary, "Usable with warnings   Warning: 1   OK: 0", "warning summary");
