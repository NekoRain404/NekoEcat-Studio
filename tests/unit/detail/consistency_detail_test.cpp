...
void testEmptyStates() {
    ConsistencyDetailState state = consistencyDetailUnavailableState(englishTexts());
    expectEqual(state.text, "Consistency evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select a visible Consistency row.", "no selection text");
...
void testSeverityAndRoutes() {
    ...
    expectEqual(consistencyDetailSeverityKey("警告"), "warning", "warning severity");
    ...
    expectEqual(consistencyDetailRoute({.scope = "Topology"}, texts), "Open State Machine/topology evidence",
                "topology route");
    expectEqual(consistencyDetailRoute({.scope = "Startup"}, texts), "Open Startup SDO evidence", "startup route");
    expectEqual(consistencyDetailRoute({.evidence = "watch missing"}, texts), "Open Watch or I/O evidence",
                "watch route");
    expectEqual(consistencyDetailRoute({.scope = "Project"}, texts), "Ready row; continue commissioning",
                "ready route");
    expectEqual(consistencyDetailRoute({}, texts), "Open I/O Variables evidence", "default route");
}

// Verify selected row builds correct summary, severity, route, and tooltip
void testSelectedRowState() {
    const ConsistencyDetailState state = buildConsistencyDetailState({.level = "Warning",
                                                                      .scope = "Startup",
    ...
    expectEqual(state.text,
                ...
                "selected summary");
    expectContains(state.tooltipLines, "Selected Consistency row", "tooltip title");
    expectContains(state.tooltipLines, "Level: Warning", "tooltip level");
    expectContains(state.tooltipLines, "Open Evidence Route: Open Startup SDO evidence", "tooltip route");
    expectContains(state.tooltipLines, "Local gate boundary.", "tooltip local boundary");
    expectEqual(state.tooltip, state.tooltipLines.join('\n'), "tooltip text");
}

// Verify fallback text when all fields are empty
void testFallbackText() {
    const ConsistencyDetailState state = buildConsistencyDetailState({}, englishTexts());
    expectEqual(state.text,
