...
WorkspaceTabBadgeTexts englishTexts() {
    return {
        ...
        .overviewTipPattern = "Slave matrix priority queue: P0 %1; P1 %2; P2 %3; P3 %4",
        ...
        .consistencyTipPattern = "Consistency errors: %1; warnings: %2; info: %3; ready: %4",
...
void testBadgeText() {
    ...
    expectEqual(workspaceTabBadgeText("Overview", 3, true), "Overview !3", "issue count text");
    expectEqual(workspaceTabBadgeText("Free Run", 5), "Free Run 5", "normal count text");
}
...
void testOverviewPriorityCounts() {
    ...
    const WorkspaceTabBadgeDetail state = buildWorkspaceTabBadgeDetail(counts, englishTexts());

    expectEqual(state.overview.text, "Overview !6", "overview badge");
    expectEqual(state.overview.tip, "Slave matrix priority queue: P0 1; P1 2; P2 3; P3 4", "overview tip");
}
...
void testStartupUsesDiffIssuesBeforeRows() {
    ...
    WorkspaceTabBadgeDetail state = buildWorkspaceTabBadgeDetail(counts, englishTexts());
    expectEqual(state.startupSdo.text, "Startup SDO !2", "startup issue badge");
    expectEqual(state.startupSdo.tip, "Startup SDO rows: 12; Watch mismatches: 2", "startup tip");
...
void testConsistencyUsesBlockingIssuesBeforeRows() {
    ...
    WorkspaceTabBadgeDetail state = buildWorkspaceTabBadgeDetail(counts, englishTexts());
    expectEqual(state.consistency.text, "Consistency !4", "consistency issue badge");
    expectEqual(state.consistency.tip, "Consistency errors: 1; warnings: 3; info: 4; ready: 5", "consistency tip");
...
void testDiagnosticsUsesIssuesBeforeRows() {
    ...
    WorkspaceTabBadgeDetail state = buildWorkspaceTabBadgeDetail(counts, englishTexts());
    expectEqual(state.diagnostics.text, "Diagnostics !3", "diagnostics issue badge");
    expectEqual(state.diagnostics.tip, "Diagnostics errors: 2; warnings: 1; info: 5", "diagnostics tip");
...
void testOtherWorkspaceBadges() {
    ...
    expectEqual(state.watch.tip, "Watch rows: 7; Startup mismatches: 2", "watch tip");
    ...
    expectEqual(state.stateMachine.text, "State Machine !1", "state machine badge");
}
