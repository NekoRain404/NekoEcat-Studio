...
StartupSdoRowDetailTexts englishTexts() {
    return {
        ...
        .summaryPattern = "Row %1 | #%2 %3:%4 | %5 = %6 | Status: %7 | Watch: %8 | %9",
...
void testEmptyStates() {
    StartupSdoRowDetailState state = startupSdoRowDetailUnavailableState(englishTexts());
    expectEqual(state.text, "Startup SDO evidence is not available.", "unavailable text");
    ...
    expectEqual(state.text, "Select a visible Startup SDO row.", "no selection text");
...
void testSeverityRules() {
    ...
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "ok", "matching row is ok");

    row.watchDelta = "diff";
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "error", "watch diff is error");

    row.watchDelta = "match";
    row.status = "Preflight Error";
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "error", "validation issue is error");

    row.status = "Applying";
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "action", "applying row is action");

    row.status = "Synced";
    row.watchDelta = "pending";
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "warning", "pending comparison is warning");

    row.watchDelta = "match";
    row.value.clear();
    expectEqual(startupSdoRowDetailSeverityKey(row, texts), "warning", "missing target value is warning");
}

// Test selected row detail state and tooltip content
void testSelectedRowState() {
    const WatchStartupStartupRow row = readyRow();
    const StartupSdoRowDetailState state = buildStartupSdoRowDetailState(row, englishTexts());
    ...
    expectEqual(state.text,
                ...
                "summary text");
    expectContains(state.tooltipLines, "Selected Startup SDO row", "tooltip title");
    ...
    expectContains(state.tooltipLines, "Watch Delta: match", "tooltip watch delta");
...
void testLocalizedAndFallbackState() {
    ...
    row.status = "校验中";
    row.watchDelta = "无监视";
    StartupSdoRowDetailState state = buildStartupSdoRowDetailState(row, englishTexts());
