...
private slots:
    ...
    void testSetSummary() {
        IoVariablePlugin p(container_);
        p.setSummary("100/100 | All | process 50 | watch 30 | startup diff 5 | missing 2 | changed 10 | plc issues 3",
                     "warning");
        QCOMPARE(
            p.ioVariableSummaryLabel()->text(),
            QString("100/100 | All | process 50 | watch 30 | startup diff 5 | missing 2 | changed 10 | plc issues 3"));
        QCOMPARE(p.ioVariableSummaryLabel()->property("severity").toString(), QString("warning"));
    }
