...
private slots:
    ...
    void testRemoveViolation() {
        ComplianceCheckerPlugin plugin;
        plugin.addViolation({"v_remove", "c1", "info", "remove", "fix", QDateTime::currentDateTime()});
        int initial = plugin.violationCount();
    ...
    void testRemoveRecommendation() {
        ComplianceCheckerPlugin plugin;
        plugin.addRecommendation({"r_remove", "low", "Remove Recommendation", "remove", "General"});
        int initial = plugin.recommendationCount();
    ...
    void testScoreChangedSignal() {
        ...
        plugin.addViolation({"v_test", "c1", "info", "test", "fix", QDateTime::currentDateTime()});
        QCOMPARE(spy.count(), 1);
    }
    ...
    void testExportReport() {
        ...
        plugin.addViolation(
            {"v_export", "c1", "warning", "Export violation", "Fix export issue", QDateTime::currentDateTime()});
        plugin.addRecommendation({"r_export", "high", "Export recommendation", "Review compliance export", "General"});
    ...
    void testSourceDoesNotMintSyntheticComplianceResults() {
        QFile file(
            QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/compliancechecker/ComplianceCheckerPlugin.cpp"));
