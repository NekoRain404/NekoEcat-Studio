...
void TestUpdateManagerPlugin::applyUpdate() {
    ...
    QVERIFY(plugin_->historyTable()->item(0, 5)->text().contains("backend", Qt::CaseInsensitive));
...
void TestUpdateManagerPlugin::sourceDoesNotContainSyntheticApplySuccess() {
    QFile file(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/plugins/updatemanager/UpdateManagerPlugin.cpp"));
