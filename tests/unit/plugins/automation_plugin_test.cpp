            ...
            while (auto* socket = nextPendingConnection()) {
                ...
                connect(socket, &QTcpSocket::readyRead, this, [this, socket]() {
                    ...
                    while ((newline = buffer_.indexOf('\n')) >= 0) {
                        ...
                        if (doc.isObject())
                            methods_.append(doc.object().value("method").toString());
                    }
                });
            }
        });
...
private slots:
    ...
    void testScriptingServiceRejectsUnsafeScriptNames() {
        ...
        QVERIFY(!scripting.saveScript(QStringLiteral("../escape"), QStringLiteral("log('escape');")));
        QVERIFY(!scripting.saveScript(QStringLiteral("nested/name"), QStringLiteral("log('nested');")));
        QVERIFY(scripting.loadScript(QStringLiteral("../escape")).isEmpty());

        const QString tooLongName(129, QLatin1Char('x'));
        QVERIFY(!scripting.saveScript(tooLongName, QStringLiteral("log('long');")));
    ...
    void testScriptingBuiltinsDoNotClaimAsyncBusSuccess() {
        ...
        QJSValue writeResult = scripting.executeScript("writeSDO(0, '0x2000', '0x00', '1', 'UINT8')");
    ...
    void testBatchOperationServiceDoesNotMintBatchSuccess() {
        QFile file(QStringLiteral(SOURCE_ROOT "/apps/ecat-studio/services/BatchOperationService.cpp"));
