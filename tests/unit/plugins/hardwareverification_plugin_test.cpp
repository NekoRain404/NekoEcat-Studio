...
private slots:
    ...
    void testVerificationCompletedSignalEmission() {
        EcatClient client;
        HardwareVerificationService service(&client);
        QSignalSpy spy(&service, &HardwareVerificationService::verificationCompleted);
        QVERIFY(spy.isValid());
        service.verifyDevice(0);
    ...
    void testVerificationStartedSignal() {
        EcatClient client;
        HardwareVerificationService service(&client);
        QSignalSpy spy(&service, &HardwareVerificationService::verificationStarted);
        QVERIFY(spy.isValid());
        service.verifyDevice(0);
        QCOMPARE(spy.count(), 1);
    }
    ...
    void testVerificationProgressSignal() {
        EcatClient client;
        HardwareVerificationService service(&client);
        QSignalSpy spy(&service, &HardwareVerificationService::verificationProgress);
        QVERIFY(spy.isValid());
        service.verifyNetwork();
        QCOMPARE(spy.count(), 4);
    }
    ...
    void testNetworkVerificationWidgetSignals() {
        ...
        HardwareVerificationService service(&client);
        NetworkVerificationWidget widget(&service);
        QSignalSpy spy(&widget, &NetworkVerificationWidget::verificationRequested);
        QVERIFY(spy.isValid());
    }
    ...
    void testDisconnectedClientDoesNotReportHardwareVerified() {
        ...
        for (const auto& result : results) {
            QVERIFY2(!result.allPassed(),
                     qPrintable(result.verificationId + " must not report allPassed without daemon connection"));
            QCOMPARE(result.passed, 0);
            QCOMPARE(result.failed, 0);
