...
public:
    ...
    void run() override {
        ...
        for (int i = 0; i < 100; ++i) {
            ...
            QMetaObject::invokeMethod(bus, [bus, slaves] { bus->emitSlaveChanged(slaves); }, Qt::QueuedConnection);
...
class ConcurrentAccessTest : public QObject {
    ...
    template <typename Spy> static int drain(Spy& spy, int expected, int timeoutMs = 8000) {
        QElapsedTimer t;
        t.start();
        int last = -1, quiet = 0;
        while (t.elapsed() < timeoutMs) {
            ...
            if (n == last) {
                if (++quiet >= 30 && n >= expected)
                    break; // stable AND sufficient
            } else {
                last = n;
                quiet = 0;
            }
