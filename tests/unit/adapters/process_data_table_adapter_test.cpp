...
void testPdoMapRowParsing() {
    ...
    expectFalse(pdoMapTableRowHasTarget(pdoMapTableRowFromTable(&table, 3)), "missing PDO row has no target");
}
...
void testFreeRunEntryRowParsing() {
    ...
    expectEqual(row.mapStatus, "Mapped warning: bit mismatch 16/32", "Free Run map status");
    expectEqual(row.mapDetail, "0x1600 32 bit | Name source: PDO map", "Free Run map detail");
...
void testIoVariableRowParsing() {
    ...
    expectTrue(ioVariableTableRowHasValue(&table, 0), "I/O row table value helper has value");
    ...
    expectEqual(ioVariableTableRowPreferredValue(row), "0x0006", "I/O preferred value keeps Raw priority");
    expectEqual(ioVariableTableRowStartupValue(row), "0x000F", "I/O startup value keeps Watch priority");
    expectEqual(ioVariableTableRowTypeFromBits(row), "uint16", "I/O type from bits");
    expectEqual(ioVariableTableRowSdoType(row), "uint16", "I/O type from source alias");
    expectEqual(ioVariableTableRowKey(row), "3|0x6040|0x00", "I/O object key");
    expectEqual(ioVariableTableRowKey(&table, 0), "3|0x6040|0x00", "I/O object key from table");
    expectEqual(ioVariableTableObjectKey(3, "6040", "0"), "3|0x6040|0x00", "I/O object key normalizes address");
    expectTrue(ioVariableTableRowHasProcessSource(row), "I/O process source detected");
    expectTrue(ioVariableTableRowHasStartupDiff(row), "I/O startup diff detected");
    expectTrue(ioVariableTableRowHasPdoMapIssue(row), "I/O PDO map issue detected");
    expectTrue(ioVariableTableRowHasChangedValue(row), "I/O changed value detected");
    expectTrue(ioVariableTableRowHasPlcIssue(row, "Ready"), "I/O PLC issue detected");
    ...
    expectTrue(ioVariableTableRowsContainValue(&table, visible), "visible I/O rows contain a value");
    expectFalse(ioVariableTableRowHasTarget(ioVariableTableRowFromTable(&table, 2)),
                "invalid position I/O row has no target");
    expectTrue(ioVariableTableRowIsTx(ioVariableTableRowFromTable(&table, 1)), "I/O Tx input detected");
    expectTrue(ioVariableTableRowHasPdoSource(ioVariableTableRowFromTable(&table, 1)), "I/O PDO source detected");
    expectFalse(ioVariableTableRowHasMissingValue(row), "I/O row with Raw/Watch is not missing value");
}
