...
void testSimpleIssueClassifiers() {
    expectTrue(!workspaceTabBadgeWatchDeltaIsIssue("match"), "watch match is not an issue");
    expectTrue(!workspaceTabBadgeWatchDeltaIsIssue("待比较"), "watch pending zh is not an issue");
    expectTrue(workspaceTabBadgeWatchDeltaIsIssue("diff"), "watch diff is an issue");
}
...
void testWorkspaceTabBadgeCountsFromTables() {
    ...
    const WorkspaceTabBadgeCounts counts = workspaceTabBadgeCounts({.watchTable = &watch,
