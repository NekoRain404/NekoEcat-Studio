...
void testStructuredRowExtraction() {
    ...
    int position = -1;
    expectTrue(stateMachineTableRowPosition(row, &position), "valid position parses");
    expectEqual(position, 3, "parsed position");
    expectEqual(stateMachinePositionFromTable(&table, 0), 3, "position from table");
    expectTrue(stateMachineTableRowHasRecommendation(row), "recommendation from row");
    expectTrue(stateMachineRowHasRecommendation(&table, 0), "recommendation from table");
}
...
void testInvalidRowsAndFallbacks() {
    ...
    const StateMachineTableRow invalid = stateMachineTableRowFromTable(&table, -1);
    expectEqual(invalid.row, -1, "invalid row index");
    expectEqual(invalid.position, QString(), "invalid row position");
    expectEqual(stateMachinePositionFromTable(&table, -1), -1, "invalid table position");
    expectFalse(stateMachineRowHasRecommendation(&table, -1), "invalid recommendation");

    const StateMachineTableRow row = stateMachineTableRowFromTable(&table, 1);
    int position = -1;
    expectFalse(stateMachineTableRowPosition(row, &position), "invalid position fails");
    expectEqual(position, -1, "invalid position untouched");
    expectFalse(stateMachineTableRowHasRecommendation(row), "blank recommendation");
}
