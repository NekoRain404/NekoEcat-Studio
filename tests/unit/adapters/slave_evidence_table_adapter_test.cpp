...
void testAppliesLoadedTableEvidence() {
    ...
    expectEqual(firstSlaveEvidenceDriveWatchRow(&watch, 2), 0, "first CiA 402 Watch row");
    expectEqual(firstSlaveEvidenceRowForPosition(&watch, 4, kSlaveEvidenceWatchPositionColumn), 3,
                "generic Watch position lookup");
    expectEqual(firstSlaveEvidenceStartupDiffRow(&startup, 2), 0, "first Startup diff row");
    expectEqual(firstSlaveEvidenceProcessIssueRow(&process, 2), 0, "first process issue row");
}
...
void testMatrixFilteringAndPriorityCounts() {
    ...
    setCell(&matrix, 0, kSlaveEvidenceMatrixProcessColumn, "3 row(s), 0 issue(s)");
    ...
    setCell(&matrix, 3, kSlaveEvidenceMatrixProcessColumn, "3 row(s), 0 issue(s)");
    ...
    const SlaveEvidenceMatrixPriorityCounts priorityCounts = slaveEvidenceMatrixPriorityCounts(&matrix);
    ...
    const SlaveEvidenceMatrixFilterStats missingWatch =
        filterSlaveEvidenceMatrixTable(&matrix, QString::fromLatin1(kSlaveEvidenceScopeMissingWatch), QString());
    ...
    const SlaveEvidenceMatrixFilterStats ready =
        filterSlaveEvidenceMatrixTable(&matrix, QString::fromLatin1(kSlaveEvidenceScopeReady), QString());
    expectEqual(ready.visible, 1, "ready visible rows");
    expectTrue(!ready.hasVisibleIssue, "ready filter has no visible issue");

    const SlaveEvidenceMatrixFilterStats search =
        filterSlaveEvidenceMatrixTable(&matrix, QString::fromLatin1(kSlaveEvidenceScopeAll), "Axis");
...
void testMatrixRouteTargetStorage() {
    ...
    setSlaveEvidenceMatrixRouteTarget(&matrix, 0, SlaveEvidenceRouteTarget::ObjectDictionary);
    setSlaveEvidenceMatrixRouteTarget(&matrix, 1, SlaveEvidenceRouteTarget::Process);

    expectRoute(slaveEvidenceMatrixRouteTargetForRow(&matrix, 0), SlaveEvidenceRouteTarget::ObjectDictionary,
                "first matrix row stores OD route");
    expectRoute(slaveEvidenceMatrixRouteTargetForRow(&matrix, 1), SlaveEvidenceRouteTarget::Process,
                "second matrix row stores process route");
    expectRoute(slaveEvidenceMatrixRouteTargetForRow(&matrix, -1), SlaveEvidenceRouteTarget::Overview,
