...
void testRowParsing() {
    ...
    expectFalse(sdoDictionaryRowIsWritable(readOnly), "read-only row is not writable");
...
void testRowCollections() {
    ...
    expectTrue(sdoDictionaryRowsContainValue(&table, {0, 1}), "selected rows contain value");
    expectFalse(sdoDictionaryRowsContainValue(&table, {1, 2}), "selected rows contain no values");
}
...
void testTargetLookup() {
    ...
    const SdoDictionaryRow missing = sdoDictionaryRowForTarget(&table, "0x9999", "0x00");
    expectFalse(sdoDictionaryRowHasTarget(missing), "missing target lookup has no target");
}
