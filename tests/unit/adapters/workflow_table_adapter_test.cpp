...
void initWorkflowTable(QTableWidget* table) {
    ...
    setCell(table, 1, kCommissioningWorkflowStepColumn, "Inspect Object Dictionary");
    setCell(table, 1, kCommissioningWorkflowRiskColumn, "OD missing");
    setCell(table, 1, kCommissioningWorkflowEvidenceColumn, "Object data not loaded");
    setCell(table, 1, kCommissioningWorkflowActionColumn, "Open Object Dictionary");
    ...
    setCell(table, 2, kCommissioningWorkflowEvidenceColumn, "PDO data not loaded");
...
void testStatusStorageAndState() {
    ...
    expectEqual(commissioningWorkflowStatusKeyForRow(&table, 1), "action", "workflow status key is stored");
    const CommissioningWorkflowRowState state = commissioningWorkflowRowState(&table, 1, "None");
    ...
    const CommissioningWorkflowTableRow row = commissioningWorkflowTableRowFromTable(&table, 1);
    ...
    expectEqual(row.nextAction, "Open Object Dictionary", "workflow row next action");
}
...
void testFilteringAndIssueLookup() {
    ...
    const CommissioningWorkflowFilterStats blocked = filterCommissioningWorkflowTable(
        &table, QString::fromLatin1(kCommissioningWorkflowScopeBlocked), QString(), "None");
    ...
    expectEqual(firstCommissioningWorkflowIssueRow(&table), 2, "first visible issue row");

    const CommissioningWorkflowFilterStats search =
        filterCommissioningWorkflowTable(&table, QString::fromLatin1(kCommissioningWorkflowScopeAll), "Object", "None");
    expectEqual(search.visible, 1, "search visible count");
    expectTrue(!table.isRowHidden(1), "search keeps matching action row");
    expectEqual(firstCommissioningWorkflowIssueRow(&table), 1, "first visible action issue row");
    expectEqual(nextCommissioningWorkflowIssueRow(&table, 1), 1, "single visible issue wraps to itself");

    filterCommissioningWorkflowTable(&table, QString::fromLatin1(kCommissioningWorkflowScopeOpen), QString(), "None");
    expectEqual(nextCommissioningWorkflowIssueRow(&table, 1), 2, "next visible issue advances");
    expectEqual(nextCommissioningWorkflowIssueRow(&table, 2), 1, "next visible issue wraps");
}
