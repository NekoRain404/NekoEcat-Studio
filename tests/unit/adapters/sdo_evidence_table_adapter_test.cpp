...
void testCandidateExtraction() {
    ...
    const SdoEvidenceCandidates candidates = sdoEvidenceCandidatesFromTables(
        "0x0005",
        ...
        {.dictionaryRow = 0, .watchRow = 0, .startupRow = 0, .bookmarkRow = 0, .targetTrailRow = 0},
        {.readValue = "Read",
         .watch = "Watch",
         .dictionary = "OD",
         .startup = "Startup",
...
void testTargetRowLookup() {
    ...
    const SdoEvidenceTableRows rows = sdoEvidenceTableRowsForTarget(
        {.dictionaryTable = &dictionary,
         ...
         .bookmarkTable = &bookmark,
         .targetTrailTable = &trail},
        {.position = 3, .index = "0x6040", .subIndex = "0x00", .dictionaryLoadedForPosition = true});
    ...
    const SdoEvidenceTableRows missing = sdoEvidenceTableRowsForTarget(
        {.dictionaryTable = &dictionary},
        {.position = 3, .index = "0x6040", .subIndex = "0x00", .dictionaryLoadedForPosition = false});
    expectEqual(missing.dictionaryRow, -1, "dictionary row waits for loaded position");
}
...
void testEmptyRowsAndTrailFallback() {
    ...
    const SdoEvidenceCandidates candidates = sdoEvidenceCandidatesFromTables(
        QString(), {.targetTrailTable = &trail},
        {.dictionaryRow = -1, .watchRow = -1, .startupRow = -1, .bookmarkRow = -1, .targetTrailRow = 0},
        {.readValue = "Read",
         .watch = "Watch",
         .dictionary = "OD",
         .startup = "Startup",
...
void testLocalEvidenceExtraction() {
    ...
    const QVector<SdoEvidenceItem> items = sdoLocalEvidenceItemsFromTables(
        ...
        {.dictionaryTable = &dictionary, .watchTable = &watch, .startupTable = &startup},
        {.read = "Read",
         .watchPrefix = "Watch",
         .dictionary = "OD",
...
void testWriteEvidenceExtraction() {
    QVector<SdoEvidenceItem> items =
        sdoWriteEvidenceItemsFromValues(QString(), "0x0006", "0x0007", QString(), "0x0008", "0x0009", QString(),
                                        {.read = "Read",
                                         .dictionary = "OD",
                                         .watch = "Watch",
                                         .startup = "Startup",
    ...
    items = sdoWriteEvidenceItemsFromValues("0x0005", "0x0006", QString(), QString(), QString(), "0x0009", "0x000A",
                                            {.read = "Read",
                                             .dictionary = "OD",
                                             .watch = "Watch",
                                             .startup = "Startup",
...
void testWriteDeltaAvailability() {
    ...
    expectEqual(sdoWriteDeltaReviewEvidenceAvailable(
                    "0x0005", {},
                    {.dictionaryRow = -1, .watchRow = -1, .startupRow = -1, .bookmarkRow = -1, .targetTrailRow = -1}),
                true, "read value makes review available");
    expectEqual(sdoWriteDeltaReviewEvidenceAvailable(
                    QString(), {.dictionaryTable = &dictionary},
                    {.dictionaryRow = 0, .watchRow = -1, .startupRow = -1, .bookmarkRow = -1, .targetTrailRow = -1}),
...
void testTargetTrailRowParsing() {
    ...
    expectEqual(sdoTargetTrailRowStartupValue(row), "0x000F", "trail write value has startup priority");
    expectEqual(sdoTargetTrailStartupValueFromTable(&trail, 1), "8", "trail startup fallback uses read value");
    expectEqual(
        sdoTargetTrailRowKeyFromTable(&trail, 0),
        sdoTargetTrailRowKey(3, "0x6040", "0x00", "uint16", "Object Bookmark", "Controlword evidence with detail text"),
        "trail row key matches value constructor");
    expectEqual(sdoTargetTrailKeysFromTable(&trail).size(), 2, "trail key set size");

    const SdoTargetTrailRow missing = sdoTargetTrailRowFromTable(&trail, 9);
    expectFalse(sdoTargetTrailRowHasTarget(missing), "missing trail row has no target");
}
...
void testObjectBookmarkRowParsing() {
    ...
    expectTrue(sdoObjectAccessIsReadOnly(row.access, "只读"), "english ro access is read-only");
    expectFalse(sdoObjectAccessIsReadOnly(sdoObjectBookmarkRowFromTable(&bookmark, 1).access, "只读"),
                "rw bookmark access is writable");
    expectTrue(sdoObjectAccessIsReadOnly("只读", "只读"), "localized read-only access is read-only");

    const SdoObjectBookmarkRow missing = sdoObjectBookmarkRowFromTable(&bookmark, -1);
    expectFalse(sdoObjectBookmarkRowHasTarget(missing), "missing bookmark row has no target");
}
