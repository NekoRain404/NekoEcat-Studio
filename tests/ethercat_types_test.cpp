...
void testToJson() {
  ...
  expectEqual(obj["rawLine"].toString(), QString("2  OP  -I-  EL1008"),
              "toJson rawLine");
}
...
void testBatchSerialization() {
  ...
  expectEqual(arr[0].toObject()["name"].toString(), QString("EK1100"),
              "batch toJson [0] name");
  expectEqual(arr[1].toObject()["name"].toString(), QString("EL1008"),
              "batch toJson [1] name");
