...
private slots:
    ...
    void testStatusUpdateThroughput() {
        ...
        QVector<QString> ids;
        for (int i = 0; i < 1000; i++)
            ids.append(svc.addRequirement(QStringLiteral("R%1").arg(i), QStringLiteral("ISO")));
        QElapsedTimer timer;
        timer.start();
        for (int i = 0; i < 10000; i++) {
    ...
    void testStandardFilterThroughput() {
        ...
        for (int i = 0; i < 5000; i++)
            svc.addRequirement(QStringLiteral("R%1").arg(i), i % 2 == 0 ? QStringLiteral("ISO") : QStringLiteral("CE"));
        QElapsedTimer timer;
        timer.start();
        for (int i = 0; i < 10000; i++) {
