...
private slots:
  void testExecutionRecordingThroughput() {
    ...
    for (int i = 0; i < count; i++) {
      svc.recordExecution(QStringLiteral("wf_%1").arg(i % 100),
                          i % 3 != 0, 50.0 + (i % 200));
    }
  ...
  void testErrorRecordingThroughput() {
    ...
    const int count = 5000;
    for (int i = 0; i < count; i++) {
      svc.recordError(QStringLiteral("wf_%1").arg(i % 100),
                      QStringLiteral("Type_%1").arg(i % 10),
  ...
  void testResourceRecordingThroughput() {
    ...
    const int count = 5000;
    for (int i = 0; i < count; i++) {
      svc.recordResourceUsage(QStringLiteral("wf_%1").arg(i % 100),
                              50.0 + (i % 50), 256.0 + (i % 512), 100.0 + (i % 200));
    }
  ...
  void testAnalysisLatency() {
    ...
    for (int i = 0; i < 1000; i++) {
      ...
      svc.recordError(QStringLiteral("wf_perf"), QStringLiteral("Timeout"),
  ...
  void testMemoryStability() {
    ...
    for (int round = 0; round < 10; round++) {
      for (int i = 0; i < 100; i++) {
        svc.recordExecution(QStringLiteral("wf_%1_%2").arg(round).arg(i),
                            i % 2 == 0, 50.0 + i);
        svc.recordError(QStringLiteral("wf_%1_%2").arg(round).arg(i),
                        QStringLiteral("Type"), QStringLiteral("msg"));
        svc.recordResourceUsage(QStringLiteral("wf_%1_%2").arg(round).arg(i),
                                50.0, 256.0, 100.0);
      }
    }
