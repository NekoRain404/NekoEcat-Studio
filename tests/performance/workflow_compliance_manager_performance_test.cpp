...
private slots:
  void testAddRuleThroughput() {
      ...
      for (int i = 0; i < 10000; i++) {
          svc.addRule(QStringLiteral("R%1").arg(i), QStringLiteral("cat"),
                      QStringLiteral("req"));
      }
  ...
  void testAuditThroughput() {
      ...
      QVector<QString> ids;
      for (int i = 0; i < 1000; i++)
          ids.append(svc.addRule(QStringLiteral("R%1").arg(i), QStringLiteral("c"),
                                 QStringLiteral("r")));
      QElapsedTimer timer;
      timer.start();
      for (int i = 0; i < 10000; i++) {
  ...
  void testCategoryFilterThroughput() {
      ...
      for (int i = 0; i < 5000; i++)
          svc.addRule(QStringLiteral("R%1").arg(i),
                      i % 3 == 0 ? QStringLiteral("safety") : QStringLiteral("quality"),
                      QStringLiteral("req"));
      QElapsedTimer timer;
      timer.start();
      for (int i = 0; i < 10000; i++) {
